...
- 本轮 `cloud_support_handoff_safe_export` / `trashbot.cloud_support_handoff_safe_export_summary.v1` / `trashbot.robot_diagnostics_cloud_support_handoff_safe_export_summary.v1` / mobile-web support export panel 只能证明当前 repo 的本地 `software_proof` / metadata-only fail-closed support handoff 能力，且必须保留 `source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false` 和 `software_proof_docker_cloud_support_handoff_safe_export_gate`。它不证明真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue、production worker/cutover、真实手机/browser、HIL、真实电梯、真实 Nav2/fixed-route、真实 route/elevator field pass、真实投放、O5 external proof、PR #5 `PRRT_kwDOSWB9286CJ3tX` resolved、PR #6 runtime proof、dropoff/cancel completion、delivery result 或真实交付；support export only means the user/support operator can copy sanitized degraded-state context, not real pass or delivery success。
