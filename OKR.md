...
**当前进度：约 85%** | 主要缺口：真实隧道、生产 DB/queue、OSS、TLS/4G、真实机器人数据、生产级查询容量和现场长期数据回灌。archive、event/evidence、labeling、model inference、consumer read API 已有 local/mock software proof；2026-07-10 07-13 在既有 `same_task_mission_evidence_gate` 和 SQLite shadow readback 基础上，新增 O6 `trashbot.o6.cloud_external_probe_readback.v1` 与 `trashbot.o6.cloud_db_queue_external_probe_readback.v1`，支持 archive detail、`field_evidence`、`artifact_bundle`、consumer detail 顶层 alias，以及 `include=cloud_external_probe,cloud_db_queue_external_probe` 回读。same-task smoke 证明两类 probe 摘要可与同一 `task_id` 一起安全回读，并对 hostile probe payload fail-closed，只把对应 section 降级为 `blocked_not_proven`。Robot Software 验证 `Ran 3 tests in 2.338s OK` 与 relay `Ran 167 tests in 64.655s OK`，并继续固定 `safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`、`robot_control_executed=false`、`connects_cloud_production=false`。新增 readback 只证明 live endpoint probe summary 的 additive archive/read model，不证明真实 production cloud、production DB/queue、多实例一致性、真实 live Nav2 route execution、真实 robot motion、真实 delivery record、真实 operator confirmation、真实 delivery success、真实 OSS/CDN 或真实 annotation API/export。
...
**当前进度：约 85%** | 主要缺口：真实 RTC/视频、真实 ASR/TTS、真实 wheel raw 非零反馈、真实电梯/回放/标注数据流、云端生产链路和完整路线长期上车验证。PC/O7 consumer detail 已可围绕同一 `task_id` 消费 route replay、labeling、artifact readiness、field motion packet、Nav2 goal、delivery result、route bag evidence、route bag payload replay、route bag semantic replay、route bag pose progress replay、route execution result delivery readiness、route bag full semantic decode matrix、route delivery closure packet、cloud terminal result source、`same_task_mission_evidence_gate` 和本轮新增 `same_task_mission_material_checklist` 等软件证据；2026-07-10 06-10 新增 additive schema `trashbot.pc_tools_workstation.o7_same_task_mission_material_checklist.v1`，把同一 `task_id` 的 O6 gate 派生成 8 个 operator material item，并在 UI 中邻近 gate 展示。验证为 `Tests 484 passed (484)`、build 通过但有 Vite chunk-size warning、lint 通过、`git diff --check` 通过；固定 `delivery_success=false`、`safe_to_control=false`、`primary_actions_enabled=false`、`robot_control_executed=false`。证据边界为 `software_proof_o7_same_task_mission_material_checklist_only`，不证明真实 production cloud、production DB/queue、真实 live Nav2 route execution、真实 delivery record、真实 operator confirmation、真实 annotation API/export、真实 dataset export、真实关键帧媒体可访问、真实机器人运动、hardware safety、真实 delivery success 或长期路线验收。（允许使用 Mock 视频数据流、ASR/TTS 本地模拟和轨迹 Mock 优先进行 PC 端功能开发，最终再利用真实数据跑通）
...
| O5：云中转控制面 | ~85% | `2026.07.10_07-13_o5_o6_live_endpoint_probe_readback` 已把 same-task smoke 推进到 `cloud_external_probe` / `cloud_db_queue_external_probe` additive readback；`2026.07.10_08-14_same_task_mission_artifact_credit_gate` 进一步把 same-task mission credit 误判固化为软件 hard gate，新增 `same_task_id_consumed`、`live_or_field_command_executed`、`support_only_reason`、`okr_credit_allowed`，明确 local/mock probe、readback-only、checklist-only 不能再计主 OKR 增量；本轮百分比不变，仍缺真实公网 HTTPS/TLS、4G/SIM、production DB/queue、production worker/cutover、OSS/CDN live traffic、真实手机/browser 验收 |
| O6：云端核心后端 | ~85% | local/mock archive/read model 已覆盖 field evidence、annotation、artifact bundle/access、offline seed、route-root seed、field motion packet、Nav2 goal evidence、delivery result evidence、route bag evidence、route bag payload replay、route bag semantic replay、route bag pose progress replay、route execution result delivery readiness、route bag full semantic decode matrix、route delivery closure packet、cloud terminal result delivery bridge、same task mission evidence gate、`cloud_external_probe` / `cloud_db_queue_external_probe` additive readback；本 sprint `2026.07.10_08-14_same_task_mission_artifact_credit_gate` 又把 credit fields 接到 archive/readback，并对 support-only、缺字段、legacy unstructured delta fail-closed；本轮百分比不变，仍缺真实 production cloud、真实隧道、生产 DB/queue、OSS、TLS/4G、真实机器人数据、真实 live Nav2 route execution、真实 delivery record、真实 operator confirmation、真实 annotation API/export、真实 delivery success 和生产级查询容量 |
| O7：PC 端运营调试平台 | ~85% | PC/O7 consumer detail 已可围绕同一 `task_id` 展示 route replay、labeling、artifact readiness、field motion packet、Nav2 goal evidence、delivery result evidence、route bag evidence、route bag payload replay、route bag semantic replay、route bag pose progress replay、route execution result delivery readiness、route bag full semantic decode matrix、route delivery closure packet、same task mission evidence gate 与 `same_task_mission_material_checklist`；本 sprint `2026.07.10_08-14_same_task_mission_artifact_credit_gate` 新增 credit gate 字段展示，并把 `okr_credit_allowed=false` 收紧为 support-only/blocked；本轮百分比不变，仍缺真实 production cloud、production DB/queue、真实 RTC/视频、真实 ASR/TTS、真实回放/标注数据流、真实关键帧媒体可访问、真实 annotation API/export、真实 live Nav2 route execution、wheel raw 非零、真实 delivery record、真实 operator confirmation、真实 delivery success 和完整路线长期验收 |
...
1. **O5（~85%）**：当前最高优先级不是再做 probe wrapper，而是拿真实 production cloud、production DB/queue external probe 或真实 live endpoint evidence。`okr_credit_allowed=false` 的 support-only probe/readback 守护可以继续做回归，但不再计 OKR 增量。
2. **O6（~85%）**：archive/readback 的 credit gate 已经够用，下一步必须接真实 production cloud、真实隧道、生产 DB/queue、OSS 与真实机器人数据，产出真正的 `mission_artifact_delta`；否则 O6 只能保持回归守护，不再靠 readback surface 提升百分比。
3. **O7（~85%）**：当前与 O5/O6/O1 并列。下一步必须消费真实或准现场 same-task materials，如 live route execution、delivery record、operator confirmation、route bag、keyframe 或 replay JSONL；只读 checklist/surface 现在默认视为 support-only，不再提升 OKR。
4. **O1（~85%）**：真实 WAVE ROVER 上车，补轮速非零原始反馈、轮速方向和 HIL 证据，PR #5 硬件材料到位后继续提升。
