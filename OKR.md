...
更新时间：2026-05-20 15:18 Asia/Shanghai。最新 sprint：`2026.05.20_15-16_field-evidence-rerun-callback-review-handoff`。详细历史见 `docs/process/okr_progress_log.md`。

| Objective | 当前进度 | 本轮证据与边界 | 主要缺口 |
| --- | --- | --- | --- |
| Objective 1：硬件协议可信底盘 | 约 81% | 最新 sprint `field_evidence_rerun_callback_review_handoff` 未触碰 WAVE ROVER/UART/HIL、hardware bridge 或 PR #5 真实 2D LiDAR / ToF 材料，只把 O2/O3/O4 现场复跑 review-decision output 做成 callback-review-handoff software proof；Objective 1 保守保持约 81%。PR #5 thread state：`PRRT_kwDOSWB9286CJ3tQ` 与 `PRRT_kwDOSWB9286CJ3tU` 已 resolved，`PRRT_kwDOSWB9286CJ3tX` 仍 unresolved / `is_resolved=false` / material pending，manual reply `3269642220` 不是硬件 proof。 | 仍缺真实 WAVE ROVER `hil_pass`、真实串口日志、真实 `feedback_T1001.log`、真实 feedback interval、真实 `/odom`、`/imu/data`、`/battery` 实机样本、真实 UART、真实上车 HIL packet 和 operator HIL report；PR #5 `PRRT_kwDOSWB9286CJ3tX` 仍 unresolved / `is_resolved=false` / material pending，仍需真实 2D LiDAR / ToF SKU/source、receipt、采购、安装、接线、电源、标定、真实 HIL-entry 和 field evidence。本轮 callback review handoff 不是 WAVE ROVER/UART/HIL，不是 PR #5 thread resolved。 |
| Objective 2：可送垃圾任务 + 电梯 assisted delivery 必达闭环 | 约 99% | 最新 sprint `field_evidence_rerun_callback_review_handoff` 输出 `trashbot.field_evidence_rerun_callback_review_handoff.v1` / `trashbot.field_evidence_rerun_callback_review_handoff_summary.v1` / `robot_diagnostics_field_evidence_rerun_callback_review_handoff_summary` / mobile-web “现场证据复跑复核交接”只读 panel，把 review decision 中的电梯门、楼层、人工协助、dropoff/cancel completion 和 delivery result 缺口转成 owner handoff、next required evidence、rerun commands 和 blocker summary。边界为 `software_proof_docker_field_evidence_rerun_callback_review_handoff_gate`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`。Objective 2 保守保持约 99%。 | 仍缺真实电梯、真实喇叭/TTS、真实 Nav2/fixed-route 运行、真实 route completion signal、真实现场 task record、真实路线采集、真实 WAVE ROVER/UART/HIL、同一 safe `evidence_ref` 的上车实机复账、真实门状态、真实楼层确认、人工协助现场记录、真实送达、失败恢复实测、真实 dropoff completion、真实 cancel completion 和 delivery success；PR #4 route/elevator field materials 仍需真实材料回填、复核和现场通过。 |
| Objective 3：可验证导航与固定路线 | 约 99% | 最新 sprint `field_evidence_rerun_callback_review_handoff` 把 review decision 中真实 Nav2/fixed-route runtime log、route completion signal、field task record 和同一 safe `evidence_ref` 的缺口转成 owner handoff / rerun commands；这是 `software_proof_docker_field_evidence_rerun_callback_review_handoff_gate`，不是路线实跑。Objective 3 保守保持约 99%。 | 仍缺真实路线采集、Nav2 waypoint/fixed-route 实跑、关键帧实景证据、WAVE ROVER/HIL、真实串口、真实 route completion signal、真实现场 task record、真实电梯材料、真实 dropoff/cancel completion、delivery result 和同一 safe `evidence_ref` 上车实机复账；PR #4 route/elevator field materials 仍需真实现场证据。 |
| Objective 4：手机用户体验与低成本量产边界 | 约 99% | 最新 sprint 让 mobile/web 只读展示“现场证据复跑复核交接”，优先消费 `robot_diagnostics_field_evidence_rerun_callback_review_handoff_summary` 或兼容 summary，展示 handoff status、safe `evidence_ref`、handoff owner/reason、next required evidence、rerun guidance、blocker summary、same-evidence-ref status、`software_proof_docker_field_evidence_rerun_callback_review_handoff_gate`、`not_proven`、`safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`，且 Start Delivery / Confirm Dropoff / Cancel gating 不变。该收益是 phone-safe local fixture software proof，不是真实手机/browser 验收，Objective 4 保守保持约 99%。 | `not_proven` 仍包括真实手机 / 真实 iPhone/Android device behavior、production app、真实 PWA prompt/userChoice 现场验收、true phone/browser acceptance、真实 route/elevator field pass、真实 Nav2/fixed-route、真实 dropoff/cancel completion、真实 cancel completion、delivery success、真实 2D LiDAR / ToF SKU/source/receipt/procurement/installation/wiring/power/calibration/HIL-entry 材料、O5 external proof、真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue、production worker/migration/cutover、WAVE ROVER、HIL 和量产实物验收；PR #5 2D LiDAR / ToF hardware materials 仍是独立缺口。 |
| Objective 5：云中转 + OSS/CDN 数据通路产品化（历史 O6） | 约 68% | 最新 sprint `field_evidence_rerun_callback_review_handoff` 不改 cloud commands/status/ack、不新增公网入口或外部材料，只把 O2/O3/O4 现场复跑复核结果做成 `software_proof_docker_field_evidence_rerun_callback_review_handoff_gate`。上一轮 `cloud_command_id_conflict_visibility_guard` 仍是 O5 最新 command conflict software proof。Objective 5 保持约 68%。 | 仍没有真实公网 HTTPS/TLS、真实 4G/SIM、真实手机设备/browser、production app、真实 PWA prompt/userChoice、OSS/CDN live traffic、真实 production DB/queue connectivity、真实 production worker/migration/cutover、多实例一致性、queue ordering、transaction isolation、backup/recovery、Nav2/fixed-route、WAVE ROVER、HIL 或真实送达；PR #4 route/elevator field materials 和 PR #5 2D LiDAR / ToF hardware materials 仍是独立缺口，`PRRT_kwDOSWB9286CJ3tX` 仍 unresolved / `is_resolved=false` / material pending。本轮 callback review handoff 不是真实外部云 proof、不是 delivery success。 |
...
- 若 O5 外部材料和 O1 真实硬件材料都不可用，下一轮优先要求现场 owner 提供 Objective 2/O3/O4 的真实材料：同一 safe `evidence_ref` 的真实 task record、真实 dropoff/cancel completion 材料、真实 Nav2/fixed-route runtime log、route completion signal、真实电梯门状态、真实楼层确认、人工协助记录、delivery result、真实 route/elevator field pass 和真实手机/browser 证据。本轮 `field_evidence_rerun_callback_review_handoff`、`field_evidence_rerun_callback_review_decision`、`field_evidence_rerun_callback_intake`、`real_material_followup_escalation_status`、`real_material_manifest_template`、`real_material_evidence_intake`、Robot diagnostics safe summary 和 mobile/web “现场证据复跑复核交接”/“现场证据复跑回执复核”/“现场证据复跑回执入口”/“真实材料升级状态”/“真实材料回填入口”只能作为后续真实材料采集的 callback-review-handoff/callback-review-decision/callback-intake/follow-up/template/intake software-proof 入口，不得写成真实手机通过、真实 PWA prompt/user choice、真实 route/elevator field pass、HIL、真实投放、external proof 或 delivery success。
