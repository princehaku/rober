...
更新时间：2026-05-13 00:33 Asia/Shanghai。
...
| Objective 5 手机体验与量产边界 | 约 54%（software_proof_docker_phone_offline_resume_gate） | 电梯 assisted delivery 已升级为 MVP 必须；手机端 UI 美观可直接使用作为新 KR7 写入。02-03 remote 4G command loop 只补充 operator HTTP local mock cloud 的 phone-safe command/status/ack 入口，不暴露 `/cmd_vel`、串口或硬件参数。03-04 sprint 新增 `remote_readiness` / phone-readable readiness 口径，手机侧未来可区分 cloud/status/ack/cursor 状态，且敏感字段过滤不展示 token、串口、ROS topic 或硬件参数。04-05 sprint 进一步补齐 `auth_state`、`degradation_state`、`safe_phone_copy` 和用户可读 retry hint；05-06 independent relay 补齐 `status_missing`、`status_stale`、auth failed、bad request、not found、malformed JSON 等 phone-safe errors；06-07 Docker relay `/readyz` 增加 credential gate、state store 和 phone-safe failure checks，支撑 future phone UI API contract；07-08 production preflight gate 继续输出 `safe_summary` / `retry_hint` / phone-safe blocked reason，只作为未来手机提示素材；10-11 sprint 在本地 operator 首屏与 `/api/status.phone_readiness` 中落地 `trashbot.phone_readiness.v1` readiness gate，聚合 local delivery、action permissions、remote readiness、可选 preflight/backup restore 和普通用户恢复提示，targeted tests `Ran 68 tests ... OK`；13-14 sprint 新增 `trashbot.command_safety.v1` 按钮级 gate，Start/Confirm/Cancel/Diagnostics 消费 `command_safety.actions.*.enabled`，覆盖 stale/pending/auth/cloud/malformed/manifest/manual takeover，并明确 ACK 不等于送达成功，Full-stack targeted tests `Ran 64 tests in 17.299s OK`；14-15 sprint 新增 network recovery phone-safe summary，可解释 missing/invalid/stale/failed/ready，但只作为 O6 支撑不提升 O5；15-16 sprint 把 O5 从 API/handler proof 推进到真实本地 Chrome browser proof，`390x844` 与 `768x900` 均通过 44px hit area、首屏主按钮可见、无重叠/无溢出、ACK copy 可见、Diagnostics accessible 且 primary actions disabled，targeted tests `Ran 73 tests in 17.910s OK`；18-19 sprint 进一步补齐 PWA/installability shell：manifest、mobile meta、service worker registration、offline shell 和 API bypass policy，HTTP/static tests `Ran 46 tests in 20.155s OK`，robot compatibility fence `Ran 29 tests` + `Ran 37 tests` OK；20-21 sprint 新增 `trashbot.phone_task_flow_readiness.v1`，把本地首屏/API/diagnostics 组织成连接/就绪、目的地、装载确认、发车、状态解释、求助/诊断任务步骤，HTTP/static tests `Ran 46 tests in 20.115s OK`，remote bridge compatibility fence `Ran 33 tests in 16.314s OK`；22-23 sprint 新增 `trashbot.phone_support_bundle.v1`，把失败/blocked/等待 ACK/人工接管场景整理为 phone-safe support handoff bundle，`/api/status`、`phone_readiness`、`/api/diagnostics` 同口径暴露，本地 fallback 在 command safety blocked 时仍可打开 Diagnostics/Support Handoff，Task A `Ran 48 tests in 20.053s OK`，Task B `Ran 76 tests in 18.878s OK`，并确认 support metadata 不污染 remote command/status/ACK envelope、不触发 robot action、不推进 cursor；24-25 sprint 新增 `trashbot.voice_prompt_readiness.v1`，把 current prompt、trigger、human help、`playback_ready=false`、safe copy、not_proven 和电梯求助提示落到首屏/API/diagnostics，同步确认 voice metadata 不污染 remote envelope、不触发 robot action、不 ACK、不推进 cursor，Task A `Ran 50 tests in 21.150s OK`，Task B `Ran 82 tests in 20.943s OK`；00-01 sprint 新增 `trashbot.phone_offline_resume_readiness.v1`，把 offline shell、恢复提示、stale/pending ACK、command safety、Support Handoff、ACK 语义和 not-proven boundary 合成 phone-safe readiness gate，Task A `Ran 94 tests in 22.777s OK`，Task B `Ran 57 tests in 28.160s OK`，并确认 metadata-only offline/resume 不污染 remote envelope、不触发 robot action、不 ACK、不推进 cursor。 | 仍无 production app、真实手机设备 Safari/Chrome、physical phone service worker runtime、普通用户实机验收、真实喇叭/TTS 播放、生产账号或真实远程手机流程；本轮是 local/Docker phone offline/resume readiness software proof，不等于真实云、真实 4G、OSS/CDN 实流量、Nav2/fixed-route、WAVE ROVER、HIL 或真实送达。 |
| Objective 6 4G 云中转 + OSS/CDN | 约 53%（software_proof_docker_production_recovery_gate） | 23-24 sprint 顶层设计纳入 4G 云中转、OSS/CDN 和凭证边界；02-03 sprint 完成本地 mock cloud / remote bridge 最小命令闭环。03-04 sprint 在 local mock control-plane 上补齐 `MockCloudStore(state_path=...)` 原子持久化、队列/status/ack reload、敏感字段过滤、`remote_readiness` 派生；`remote_bridge` 新增 `cursor_state_path`，能恢复 `last_terminal_ack_id` 并在 terminal ACK 成功后原子持久化 cursor。04-05 sprint 补齐 bearer auth gate、phone-safe readiness/degradation、敏感字段过滤、`RemoteCloudError` 分类、cloud unreachable/auth failed/malformed response 不推进 cursor/不触发本地 action 的保守语义；05-06 sprint 完成 independent Docker/local HTTP relay service、file-backed persistence、bearer auth、phone-safe errors、robot client compatibility，并补齐 `docs/product/cloud_4g_infrastructure.md`；06-07 sprint 完成 Dockerfile/compose/env、`/healthz`、`/readyz`、state store writable check、phone-safe failure redaction self-check 和 Docker deploy smoke；07-08 sprint 新增 production preflight gate；08-09 sprint 新增 SQLite state backend proof；09-10 sprint 新增 SQLite backup/restore drill；11-12 sprint 新增 OSS/CDN manifest artifact proof；12-13 sprint 把 manifest artifact 推进到 phone/API consumption gate；13-14 sprint 把 remote degradation、ACK pending、status stale 和 manifest 异常消费到手机命令 gate；14-15 sprint 完成 network recovery drill artifact、preflight/diagnostics/phone-safe consumption 和 remote_bridge cursor compatibility fence；16-17 sprint 新增 credential rotation artifact/preflight/phone-safe 摘要与 robot compatibility fence；17-18 sprint 新增 provisioning audit artifact/preflight/phone-safe 摘要与 robot compatibility fence；19-20 sprint 新增 production store/queue gate artifact、preflight/status/diagnostics phone-safe 消费和 robot compatibility fence；21-22 sprint 新增 queue ordering drill artifact、CLI、preflight/status/diagnostics phone-safe 消费和 robot compatibility fence，Full-stack targeted tests `Ran 117 tests in 26.630s OK`，Robot targeted tests `Ran 46 tests in 23.041s OK`，CLI smoke 保持 `production_ready=False`；23-24 sprint 新增 transaction isolation drill artifact、CLI、preflight/status/diagnostics phone-safe 消费和 robot compatibility fence，Task A targeted tests `Ran 125 tests in 26.988s OK`，Task B targeted tests `Ran 40 tests in 19.857s OK`，artifact/preflight 均保持 `production_ready=False` 与 `overall_status=blocked`；25-26 sprint 新增 production recovery gate artifact、CLI、preflight/status/diagnostics phone-safe 消费和 robot compatibility fence，Task A targeted tests `Ran 133 tests in 28.079s OK`，Task B targeted tests `Ran 44 tests in 21.928s OK`，artifact/preflight 输出 `production_ready=false`、`overall_status=blocked` 与 `production_recovery=pass`。 | 真实云部署、HTTPS/TLS、公网入口、真实 4G/SIM、生产鉴权/rotate、真实 STS issuance、受限 AK、真实 OSS upload、CDN origin fetch、生命周期策略、production account、生产账号 provisioning、真实 audit log、OSS/CDN 实流量、真实生产 DB/queue、多实例一致性、真实生产 queue ordering、transaction isolation、生产备份策略、真实灾备、正式手机 app/真实手机设备验收和生产运维仍未实现或验证；Docker relay/auth/readiness/degradation/ack/status/cursor/preflight gate、SQLite 单实例恢复 proof、backup/restore drill、manifest artifact、phone/API 摘要消费、按钮级降级消费、Docker/local 弱网恢复语义、credential rotation gate、provisioning audit gate、production store/queue gate、queue ordering drill、transaction isolation gate 和 production recovery gate 只证明控制面入口形态、恢复语义、上线前置阻断能力、本地恢复演练、对象引用 contract、手机/API 摘要消费、本地凭证轮换边界、provisioning/STS/audit 合同消费边界、生产持久化/队列缺口可解释 gate、Docker/local 顺序/隔离/恢复演练，不等于真实云、真实 4G、真实生产备份/灾备、真实垃圾送达、Nav2/fixed-route、WAVE ROVER 或 HIL。 |
...
- 下一轮仍需按 live `OKR.md` 重排。当前 O5 约 52%、O6 约 51%；若仍是 Docker-only 环境，优先在 O6 生产化前置 gate 或 O5 真实设备前置 gate 中选择最低且最可行动项，但不能声明真实云/4G、真实手机、真实喇叭/TTS 或真实送达；若具备真实手机设备、音频硬件、云或 4G 条件，则优先补 O5 真实手机/喇叭/TTS 验收或 O6 真实云公网入口、生产 DB/queue、真实 4G/SIM 证据。O1/O2/O3/O4 仍受真实硬件、路线和感知证据约束，不能用 Docker/local software proof 冒充 HIL、真实云、4G 或真实送达。
