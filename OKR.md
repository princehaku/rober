...
更新时间：2026-05-15 10:14 Asia/Shanghai。最新 sprint：`2026.05.15_11-12_elevator-field-material-validation`。详细历史见 `docs/process/okr_progress_log.md`。

| Objective | 当前进度 | 本轮证据与边界 | 主要缺口 |
| --- | --- | --- | --- |

| Objective 1：硬件协议可信底盘 | 约 73% | 本轮未改硬件、WAVE ROVER、UART、Orange Pi 或真实串口证据；`software_proof_docker_elevator_field_material_validation_gate` 明确 `not_proven` 包含 HIL，且 `delivery_success=false`，不提升硬件完成度。 | 仍缺真实 WAVE ROVER `hil_pass`、真实串口日志、`T=1001` feedback、`/odom`、`/imu/data`、`/battery` 实机样本。 |
| Objective 2：可送垃圾任务 + 电梯 assisted delivery 必达闭环 | 约 71% | `2026.05.15_11-12_elevator-field-material-validation` 完成 `software_proof_docker_elevator_field_material_validation_gate`：Autonomy 新增 `elevator_field_run_material_validation.py`，输出 `trashbot.elevator_field_run_material_validation.v1` 和 `trashbot.elevator_field_run_material_validation_summary.v1`，校验 door state、target floor confirmation、human assistance/operator note、Nav2/fixed-route runtime log、task record、completion signal、diagnostics/mobile safe summary 的缺失、模板、坏 JSON、`evidence_ref` mismatch、unsafe copy、`primary_actions_enabled=true` 与 `delivery_success=true` fail closed；Robot diagnostics 只读暴露 `elevator_field_run_material_validation` / `_summary`，固定 `delivery_success=false`、`primary_actions_enabled=false`；`mobile/web` 新增只读“电梯现场材料校验” panel，不改变 Start/Confirm/Cancel gating。验证包括 Autonomy py_compile pass、`test_elevator_field_run_material_validation.py` `Ran 7 tests ... OK`、CLI `--help` pass；Robot py_compile pass、diagnostics unittest `Ran 75 tests ... OK`；Full-stack mobile unittest `Ran 42 tests ... OK`、py_compile pass、`node --check` pass；required rg pass；scoped diff check pass。该证据把 O2 KR6/KR7 的现场补证材料推进为本机软件校验链，因此从约 70% 保守上调到约 71%。 | 仍缺真实电梯、真实喇叭/TTS、真实 Nav2/fixed-route 运行、真实路线采集、真实 WAVE ROVER/UART/HIL、同一 `evidence_ref` 的上车实机复账、真实门状态、真实楼层确认、人工协助现场记录、真实送达、失败恢复实测、真实 dropoff completion、真实 cancel completion 和 delivery success。 |
| Objective 3：可验证导航与固定路线 | 约 70% | 同一 `software_proof_docker_elevator_field_material_validation_gate` 将 Nav2/fixed-route runtime log 纳入电梯现场材料校验的必填项，并要求它与 task record、completion signal、diagnostics/mobile safe summary 使用同一 `evidence_ref`；Robot diagnostics 和 mobile read-only panel 均保留 blocked/not_proven 边界。该证据只说明固定路线运行材料已有 Docker/local material validation 准备能力，因此从约 69% 保守上调到约 70%。 | 仍缺真实路线采集、Nav2 waypoint/fixed-route 实跑、关键帧实景证据、WAVE ROVER/HIL、真实串口和同一 `evidence_ref` 上车实机复账。 |
| Objective 4：手机用户体验与低成本量产边界 | 约 73% | 本轮新增 O2/O3 电梯现场材料校验的 `mobile/web` 只读 panel，展示 validation status、safe `evidence_ref`、missing/template/mismatch categories、operator next steps、`delivery_success=false`、`primary_actions_enabled=false` 和 `not_proven`，但不放行 Start/Confirm/Cancel；Browser render check 未运行，原因是 Browser runtime reported `Browser is not available: iab`。因此 Objective 4 不上调。 | `not_proven` 仍包括真实 iPhone/Android device behavior、production app、真实 PWA prompt/user choice 现场验收、O5 external proof、公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue、production worker/migration、Nav2/fixed-route、WAVE ROVER、HIL、真实 dropoff/cancel completion、真实 delivery 和量产实物验收。 |
| Objective 5：云中转 + OSS/CDN 数据通路产品化（历史 O6） | 约 66% | 本轮没有新增真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue connectivity、production worker/migration 或真实外部 O5 材料；`software_proof_docker_elevator_field_material_validation_gate` 只证明本机 Docker/local material validation artifact、Robot diagnostics metadata 和 mobile 只读解释，不替代外部云/4G/OSS/CDN/DB/queue proof。因此 Objective 5 保持约 66%，不因本轮上调；not real O5 external proof。 | 仍没有真实公网 HTTPS/TLS、真实 4G/SIM、真实手机设备/browser、production app、真实 PWA prompt/user choice、OSS/CDN live traffic、真实 production DB/queue connectivity、migration、worker、多实例一致性、queue ordering、transaction isolation、backup/recovery、Nav2/fixed-route、WAVE ROVER、HIL 或真实送达。 |
...
- 下一轮按 `OKR.md` 4.1 重新排序。当前数值最低完成度为 Objective 5（约 66%），但只有拿到至少一种真实外部材料时才应继续推进 O5 completion：OSS/CDN live traffic、HTTPS/TLS 公网入口、4G/SIM、production DB/queue connectivity 或 production worker/migration 证据；若外部材料仍不可用，不要重复本地 O5 metadata depth。当前最高可行动作是把本轮 `software_proof_docker_elevator_field_material_validation_gate` 用到 Objective 2/O3 的受控楼宇实景补证：真实电梯门状态、真实楼层确认、人工协助记录、Nav2/fixed-route runtime log、WAVE ROVER/HIL、同一 `evidence_ref` 的 task completion、失败恢复、真实 dropoff completion、真实 cancel completion 或 delivery success 证据，不得把 validation artifact pass、diagnostics summary、mobile read-only panel 或 ACK 写成 delivery success。

## 7. 整体风险与待办

- 当前仍缺真实手机设备、真实 iPhone/Android device behavior、production app、真实 PWA prompt/user choice 现场验收、真实云/4G、production DB/queue、OSS/CDN live traffic、Nav2/fixed-route、真实路线采集、真实电梯门状态、真实楼层确认、真实人工协助记录、WAVE ROVER、真实串口/UART、HIL、同一 `evidence_ref` 的上车实机复账、真实 cancel completed、真实 dropoff completion 和真实 delivery 证据；`software_proof_docker_elevator_field_material_validation_gate` 只证明电梯现场材料 validation artifact、Robot diagnostics metadata、phone-safe mobile read-only panel 和只读控制边界在本机 Docker/local 软件环境可验证，不证明真实电梯、真实喇叭/TTS、真实 Nav2、HIL、真实投放或真实交付。
