...
更新时间：2026-05-15 00:58 Asia/Shanghai。最新 sprint：`2026.05.15_00-01_route-task-field-run-readiness`。详细历史见 `docs/process/okr_progress_log.md`。

| Objective | 当前进度 | 本轮证据与边界 | 主要缺口 |
| --- | --- | --- | --- |
| Objective 1：硬件协议可信底盘 | 约 75% | 本轮未改硬件、WAVE ROVER、UART、Orange Pi、launch 参数或 HIL 证据；`software_proof_docker_route_task_field_run_readiness_gate` 继续把 HIL / `not_proven` 写为未证明，不提升硬件完成度。 | 仍缺真实 WAVE ROVER `hil_pass`、真实串口日志、`T=1001` feedback、`/odom`、`/imu/data`、`/battery` 实机样本。 |
| Objective 2：可送垃圾任务完整闭环 | 约 83% | `2026.05.15_00-01_route-task-field-run-readiness` 完成 `software_proof_docker_route_task_field_run_readiness_gate`：Task A `autonomy-engineer` 新增 `pc-tools/evidence/route_task_field_run_readiness.py` 和 `test_route_task_field_run_readiness.py`，输出 `schema=trashbot.route_task_field_run_readiness.v1`、`same_evidence_ref_required=true`、`required_field_run_materials`、`commands_to_run`、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`，并更新 PC/navigation 文档；Task B `robot-software-engineer` 在 diagnostics 中 metadata-only 消费 readiness summary，不触发 collect/dropoff/cancel、ACK、cursor、Nav2、HIL 或 delivery success；Task C `full-stack-software-engineer` 在 `mobile/web` 增加只读 field-run readiness panel，不读取 raw artifact，不改变 Start/Confirm/Cancel gating。任务闭环价值是下一次真实路线-任务联跑所需材料、同一 `evidence_ref` 和 phone-safe 复盘口径已形成软件 handoff。 | 仍缺真实 Nav2/fixed-route 运行、同一 `evidence_ref` 的上车复账、真实送达、失败恢复实测、dropoff/cancel completion 和 delivery success。 |
| Objective 3：可验证导航与固定路线 | 约 83% | 同一 `software_proof_docker_route_task_field_run_readiness_gate` 把 PC route debug console、route_task operator review、execution bundle、diagnostics 和 mobile 只读摘要汇总为下一次 field run readiness handoff。Autonomy 验证 py_compile pass、`test_route_task_field_run_readiness.py` `Ran 5 tests OK`、`--help` pass、临时 `--once-json` drill pass，输出 `overall_status=ready_for_field_run_materials`、`evidence_ref=file:run-001.json`、`delivery_success=false`；Robot 验证 py_compile pass、diagnostics unittest `Ran 55 tests OK`；Full-stack 验证 mobile unittest `Ran 6 tests OK`、py_compile pass、`node --check mobile/web/app.js` pass；三方 required `rg` 与 scoped diff check 均 pass。 | 仍缺真实路线采集、Nav2 waypoint/fixed-route 实跑、关键帧实景证据、WAVE ROVER/HIL、真实串口和上车复账。 |
...
| Objective 5：云中转 + OSS/CDN 数据通路产品化（历史 O6） | 约 68% | 本轮没有新增真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue connectivity、production worker/migration 或真实外部 O5 材料；field-run readiness 只证明本机 Docker/local 软件材料可被 readiness artifact、diagnostics 和 mobile 只读摘要安全消费，不能替代外部云/4G/OSS/CDN/DB/queue proof。因此 Objective 5 保持约 68%，不因 `software_proof_docker_route_task_field_run_readiness_gate` 上调。 | 仍没有真实公网 HTTPS/TLS、真实 4G/SIM、真实手机设备/browser、production app、真实 PWA prompt/user choice、OSS/CDN live traffic、真实 production DB/queue connectivity、migration、worker、多实例一致性、queue ordering、transaction isolation、backup/recovery、Nav2/fixed-route、WAVE ROVER、HIL 或真实送达。 |

本轮 OKR 口径：Objective 2 和 Objective 3 均从约 82% 谨慎上调到约 83%，理由是 field-run readiness artifact、diagnostics metadata-only summary 和 mobile read-only panel 已把下一次真实路线-任务联跑材料清单、同一 `evidence_ref` handoff、缺失材料和 phone-safe 复盘口径打通为软件证据。证据边界是 `software_proof_docker_route_task_field_run_readiness_gate`；这不是真实 Nav2/fixed-route 实跑、真实路线采集、WAVE ROVER、真实串口/UART、HIL、同一 `evidence_ref` 的上车复账、dropoff/cancel completion 或 delivery success。Objective 5 仍是最低（约 68%），但本轮没有真实外部 O5 材料，tech-plan 中切到 O2/O3 的理由仍成立；Objective 1 和 Objective 4 不调整。
