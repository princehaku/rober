...
| Objective 5 手机体验与量产边界 | 约 30% | 21-22 sprint 把人工复核从“可提交 decision”推进到“可观察复核进度”：operator 页面与 diagnostics 可直接看到 coverage、decision 分布和下一条待处理样本，支持更快定位 pending 样本并做售后/运营复盘 | 仍未完成真实硬件/HIL、真实相机/上车验证与普通用户实机验收；本轮不包含语音/喇叭和量产实物验收；电梯 assisted delivery 已升级为 MVP 必须，仍无行为、感知、TTS 或实机场景证据；手机端 UI 美观可直接使用作为新 KR7 写入，但尚无美观 UI 实现 |
| Objective 6 4G 云中转 + OSS/CDN | 约 5%（文档/方向） | 23-24 sprint 顶层设计纳入：服务端 4C 8G 无 GPU `14.103.37.144:7878` 域名 `esp-ai.bytegallop.com`、OSS bucket `bytegallop`、CDN `https://cdn.bytegallop.com/rober/`、凭证 `.env` 占位与 `.gitignore` 隔离；`docs/product/cloud_4g_infrastructure.md`、`docs/product/remote_4g_mvp.md` 更新落地 | 云端服务尚未实现、未部署；小车侧 OSS 上传客户端、bearer 鉴权与 STS 凭证派发流程未实现；4G 中断/OSS 失败 graceful degradation 未验证 |
