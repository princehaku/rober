...
更新时间：2026-05-15 02:44 Asia/Shanghai。最新 sprint：`2026.05.15_03-04_route-task-field-run-execution-pack`。详细历史见 `docs/process/okr_progress_log.md`。

| Objective | 当前进度 | 本轮证据与边界 | 主要缺口 |
| --- | --- | --- | --- |

| Objective 1：硬件协议可信底盘 | 约 73% | 本轮未改硬件、WAVE ROVER、UART、Orange Pi、launch 参数或 HIL 证据；`software_proof_docker_route_task_field_run_execution_pack_gate` 明确把 HIL / `not_proven` / `delivery success` 写为未证明，不提升硬件完成度。 | 仍缺真实 WAVE ROVER `hil_pass`、真实串口日志、`T=1001` feedback、`/odom`、`/imu/data`、`/battery` 实机样本。 |
| Objective 2：可送垃圾任务 + 电梯 assisted delivery 必达闭环 | 约 63% | `2026.05.15_03-04_route-task-field-run-execution-pack` 完成 `software_proof_docker_route_task_field_run_execution_pack_gate`：Task A `autonomy-engineer` 新增 execution pack CLI 与测试，输出 `schema=trashbot.route_task_field_run_execution_pack.v1`、`same_evidence_ref_required=true`、材料模板、first-run/rerun commands、phone-safe summary、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`；Task B `robot-software-engineer` 在 diagnostics 中 metadata-only 消费 `route_task_field_run_execution_pack` / summary，支持 explicit ref 和 `TRASHBOT_ROUTE_TASK_FIELD_RUN_EXECUTION_PACK`，不触发 collect/dropoff/cancel、ACK、cursor、Nav2、HIL 或 delivery success；Task C `full-stack-software-engineer` 在 `mobile/web` 增加只读“路线任务现场执行包” panel，不读取 raw artifact，不改变 Start/Confirm/Cancel gating。任务闭环价值是把下一次真实现场 route/task run 需要采集的 task record、robot-side task evidence、dropoff/cancel completion 材料清单和重跑动作前移为可执行软件包；因此仅从约 62% 保守上调到约 63%。 | 仍缺真实 Nav2/fixed-route 运行、真实路线采集、同一 `evidence_ref` 的上车复账、真实送达、失败恢复实测、dropoff/cancel completion 和 delivery success。 |
| Objective 3：可验证导航与固定路线 | 约 63% | 同一 `software_proof_docker_route_task_field_run_execution_pack_gate` 把 review console 推进为现场执行包：route status、Nav2/fixed-route runtime log、task record、robot-side task evidence、support-safe mobile summary 和上一轮 review console 被固化为同一 `evidence_ref` 的 required materials，并在 PC artifact、Robot diagnostics 和 mobile 只读面板中保留 blocked/not_proven 边界。Autonomy 验证 py_compile pass、`test_route_task_field_run_execution_pack.py` `Ran 6 tests OK`、`--help` pass、required rg pass、scoped diff check pass；Robot 验证 py_compile pass、diagnostics unittest `Ran 61 tests OK`；Full-stack 验证 mobile unittest `Ran 12 tests OK`、py_compile pass、`node --check mobile/web/app.js` pass；三方 required `rg` 与 scoped diff check 均 pass。该证据只说明固定路线现场材料执行链更可复跑，因此仅从约 62% 保守上调到约 63%。 | 仍缺真实路线采集、Nav2 waypoint/fixed-route 实跑、关键帧实景证据、WAVE ROVER/HIL、真实串口和同一 `evidence_ref` 上车复账。 |
| Objective 4：手机用户体验与低成本量产边界 | 约 73% | 本轮只新增 O2/O3 execution pack 的 `mobile/web` 只读展示，不放行 Start/Confirm/Cancel，不新增真实 iPhone/Android device behavior、production app 或真实 PWA prompt/user choice；因此 Objective 4 不上调。 | `not_proven` 仍包括真实 iPhone/Android device behavior、production app、真实 PWA prompt/user choice 现场验收、O5 external proof、公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue、production worker/migration、Nav2/fixed-route、WAVE ROVER、HIL、真实 dropoff/cancel completion、真实 delivery 和量产实物验收。 |
| Objective 5：云中转 + OSS/CDN 数据通路产品化（历史 O6） | 约 66% | 本轮没有新增真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue connectivity、production worker/migration 或真实外部 O5 材料；execution pack 只证明本机 Docker/local 软件材料可被 artifact、diagnostics 和 mobile 只读摘要安全消费，不能替代外部云/4G/OSS/CDN/DB/queue proof。因此 Objective 5 保持约 66%，不因 `software_proof_docker_route_task_field_run_execution_pack_gate` 上调。 | 仍没有真实公网 HTTPS/TLS、真实 4G/SIM、真实手机设备/browser、production app、真实 PWA prompt/user choice、OSS/CDN live traffic、真实 production DB/queue connectivity、migration、worker、多实例一致性、queue ordering、transaction isolation、backup/recovery、Nav2/fixed-route、WAVE ROVER、HIL 或真实送达。 |
...
- 下一轮按 `OKR.md` 4.1 重新排序。当前最低完成度为 Objective 2 和 Objective 3（均约 63%）。后续若继续推进 O2/O3，必须从本轮 `software_proof_docker_route_task_field_run_execution_pack_gate` 走向真实 Nav2/fixed-route、真实路线采集、同一 `evidence_ref` 的上车复账、dropoff/cancel completion 或 delivery success 证据，不得把 execution pack、diagnostics summary、mobile read-only panel、artifact pass 或 ACK 写成 delivery success。Objective 5 约 66%，只有拿到至少一种真实外部材料时才应继续推进 O5 completion：OSS/CDN live traffic、HTTPS/TLS 公网入口、4G/SIM、production DB/queue connectivity 或 production worker/migration 证据；若外部材料仍不可用，不要重复本地 O5 metadata depth。

## 7. 整体风险与待办

- 当前仍缺真实手机设备、真实 iPhone/Android device behavior、production app、真实 PWA prompt/user choice 现场验收、真实云/4G、production DB/queue、OSS/CDN live traffic、Nav2/fixed-route、真实路线采集、WAVE ROVER、真实串口/UART、HIL、同一 `evidence_ref` 的上车复账、真实 cancel completed、真实 dropoff completion 和真实 delivery 证据；`software_proof_docker_route_task_field_run_execution_pack_gate` 只证明 execution pack、diagnostics summary、phone-safe mobile read-only panel 和只读控制边界在本机 Docker/local 软件环境可验证，不证明真实固定路线运行、真实底盘运动、真实投放或真实交付。
