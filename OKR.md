...
- KR7 ：手机端 UI **美观且能直接使用**：视觉系统统一（配色 token、字号、间距、卡片、按钮态）、主操作主路径 ≤ 3 步、文案中文优先、iPhone/Android 主流尺寸适配、最小可点击区域 ≥ 44pt、首屏可交互 < 3 秒。详细验收口径见 `docs/product/mobile_ui_quality.md`；本地 phone-first HTML 仅作 fallback 调试入口，正式手机端必须满足该口径。
...
| Objective 5 手机体验与量产边界 | 约 33% | 电梯 assisted delivery 已升级为 MVP 必须；手机端 UI 美观可直接使用作为新 KR7 写入。02-03 remote 4G command loop 只补充 operator HTTP local mock cloud 的 phone-safe command/status/ack 入口，不暴露 `/cmd_vel`、串口或硬件参数。03-04 sprint 新增 `remote_readiness` / phone-readable readiness 口径，手机侧未来可区分 cloud/status/ack/cursor 状态，且敏感字段过滤不展示 token、串口、ROS topic 或硬件参数。04-05 sprint 进一步补齐 `auth_state`、`degradation_state`、`safe_phone_copy` 和用户可读 retry hint；05-06 independent relay 补齐 `status_missing`、`status_stale`、auth failed、bad request、not found、malformed JSON 等 phone-safe errors；06-07 Docker relay `/readyz` 增加 credential gate、state store 和 phone-safe failure checks，支撑 future phone UI API contract；07-08 production preflight gate 继续输出 `safe_summary` / `retry_hint` / phone-safe blocked reason，只作为未来手机提示素材。 | 尚无美观正式手机 UI、普通用户实机验收、生产账号或真实远程手机流程；phone-safe errors/readiness 只能支撑未来 UI，不等于正式手机 UI 完成，因此本轮不提升 O5。 |
| Objective 6 4G 云中转 + OSS/CDN | 约 34%（backup/restore drill software proof） | 23-24 sprint 顶层设计纳入 4G 云中转、OSS/CDN 和凭证边界；02-03 sprint 完成本地 mock cloud / remote bridge 最小命令闭环。03-04 sprint 在 local mock control-plane 上补齐 `MockCloudStore(state_path=...)` 原子持久化、队列/status/ack reload、敏感字段过滤、`remote_readiness` 派生；`remote_bridge` 新增 `cursor_state_path`，能恢复 `last_terminal_ack_id` 并在 terminal ACK 成功后原子持久化 cursor。04-05 sprint 补齐 bearer auth gate、phone-safe readiness/degradation、敏感字段过滤、`RemoteCloudError` 分类、cloud unreachable/auth failed/malformed response 不推进 cursor/不触发本地 action 的保守语义；05-06 sprint 完成 independent Docker/local HTTP relay service、file-backed persistence、bearer auth、phone-safe errors、robot client compatibility，并补齐 `docs/product/cloud_4g_infrastructure.md`；06-07 sprint 完成 Dockerfile/compose/env、`/healthz`、`/readyz`、state store writable check、phone-safe failure redaction self-check 和 Docker deploy smoke；07-08 sprint 新增 production preflight gate：`/preflightz` 与 `--preflight` 输出 `evidence_boundary=software_proof_docker_preflight_gate`、`production_ready=false`、`overall_status=blocked`，并识别 missing/placeholder credential、HTTPS/public ingress missing、OSS/CDN not production ready、file-backed store only、state store not writable 和 phone-safe redaction pass；08-09 sprint 新增 SQLite state backend proof：`TRASHBOT_REMOTE_CLOUD_STATE_BACKEND=sqlite` 下 command/status/ack 可跨 store reopen 或 relay/container restart 恢复，`/preflightz` 输出 `evidence_boundary=software_proof_docker_sqlite_state_store`，并明确 production DB/queue、多实例一致性、backup/restore、disaster recovery 仍未证明；09-10 sprint 新增 SQLite backup artifact、checksum、restore 到新 state path、CLI drill 和 preflight artifact check，Docker smoke 输出 `backup_status=passed`、`restore_status=passed`、`drill_status=passed`、`evidence_boundary=software_proof_docker_backup_restore_drill`，Task B remote bridge compatibility 输出 `Ran 31 tests in 15.219s OK`，确认 backup/restore/preflight 未改变 robot polling、status、ACK 和 cursor 保守语义。 | 真实云部署、HTTPS/TLS、公网入口、真实 4G/SIM、弱网/断网恢复、生产鉴权/rotate、STS/OSS/CDN 图片链路、OSS/CDN 实流量、生产云端持久化 DB/queue、多实例一致性、生产备份策略、真实灾备和生产运维仍未实现或验证；Docker relay/auth/readiness/degradation/ack/status/cursor/preflight gate、SQLite 单实例恢复 proof 和 backup/restore drill 只证明控制面入口形态、恢复语义、上线前置阻断能力和本地恢复演练，不等于正式手机 UI、真实垃圾送达、Nav2/fixed-route、WAVE ROVER 或 HIL。 |
