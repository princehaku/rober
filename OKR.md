...
- 下一轮仍需按 live `OKR.md` 重排。当前 O5 约 48%、O6 约 49%；若仍是 Docker-only 环境，最低可行动项大概率回到 O5 的真实手机设备/正式 app 验收；若具备云或 4G 条件，则优先补 O6 的真实云公网入口、生产 DB/queue 或真实 4G/SIM 证据。O1/O2/O3/O4 仍受真实硬件、路线和感知证据约束，不能用 Docker/local software proof 冒充 HIL、真实云、4G 或真实送达。
