...
- 本轮 `cloud_ack_accepted_result_pending_guard` / `software_proof_docker_cloud_ack_accepted_result_pending_guard` / mobile-web accepted-result-pending rendering 只能证明当前 repo 的本地 `software_proof` / metadata-only ACK 语义能力，且必须保留 `not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。它不证明真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue、production worker/cutover、真实手机/browser、HIL、真实电梯、真实 Nav2/fixed-route、真实 route/elevator field pass、真实投放、O5 external proof、PR #5 `PRRT_kwDOSWB9286CJ3tX` resolved、PR #6 runtime proof、真实 cancel completion、dropoff completion、delivery result 或真实交付；accepted/processing ACK only means the command was accepted or is processing, not command success or delivery success。
