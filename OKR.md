...
更新时间：2026-05-20 12:21 Asia/Shanghai。最新 sprint：`2026.05.20_12-13_field-evidence-rerun-material-dispatch`。详细历史见 `docs/process/okr_progress_log.md`。

| Objective | 当前进度 | 本轮证据与边界 | 主要缺口 |
| --- | --- | --- | --- |
| Objective 1：硬件协议可信底盘 | 约 81% | 最新 sprint `field_evidence_rerun_material_dispatch` 未触碰 WAVE ROVER/UART/HIL、hardware bridge 或 PR #5 真实 2D LiDAR / ToF 材料，只把 O2/O3/O4 现场复跑缺口做成 software-proof dispatch package；Objective 1 保守保持约 81%。上一轮 `hardware_sensor_hil_entry_callback_intake` 仍是 O1 最新硬件材料 callback software proof。 | 仍缺真实 WAVE ROVER `hil_pass`、真实串口日志、真实 `feedback_T1001.log`、真实 feedback interval、真实 `/odom`、`/imu/data`、`/battery` 实机样本、真实 UART、真实上车 HIL packet 和 operator HIL report；PR #5 `PRRT_kwDOSWB9286CJ3tX` 仍 unresolved / `is_resolved=false` / material pending，仍需真实 2D LiDAR / ToF SKU/source、receipt、采购、安装、接线、电源、标定、真实 HIL-entry 和 field evidence。本轮 dispatch package 不是 WAVE ROVER/UART/HIL，不是 PR #5 thread resolved。 |
| Objective 2：可送垃圾任务 + 电梯 assisted delivery 必达闭环 | 约 99% | 最新 sprint `field_evidence_rerun_material_dispatch` 输出 `trashbot.field_evidence_rerun_material_dispatch.v1` / `robot_diagnostics_field_evidence_rerun_material_dispatch_summary` / mobile-web “现场证据复跑材料派发”只读 panel，把真实 route completion signal、field task record、Nav2/fixed-route runtime log、电梯门/楼层/人工协助 summaries、dropoff/cancel completion 和 delivery result 转成 owner work orders、rerun commands 与 callback packet requirements。边界为 `software_proof_docker_field_evidence_rerun_material_dispatch_gate`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`。Objective 2 保守保持约 99%。 | 仍缺真实电梯、真实喇叭/TTS、真实 Nav2/fixed-route 运行、真实 route completion signal、真实现场 task record、真实路线采集、真实 WAVE ROVER/UART/HIL、同一 safe `evidence_ref` 的上车实机复账、真实门状态、真实楼层确认、人工协助现场记录、真实送达、失败恢复实测、真实 dropoff completion、真实 cancel completion 和 delivery success；PR #4 route/elevator field materials 仍需真实材料回填、复核和现场通过。 |
| Objective 3：可验证导航与固定路线 | 约 99% | 最新 sprint `field_evidence_rerun_material_dispatch` 把真实 Nav2/fixed-route runtime log、route completion signal、field task record 和同一 safe `evidence_ref` 要求写入复跑材料派发包，并提供 downstream rerun guidance；这是 `software_proof_docker_field_evidence_rerun_material_dispatch_gate`，不是路线实跑。Objective 3 保守保持约 99%。 | 仍缺真实路线采集、Nav2 waypoint/fixed-route 实跑、关键帧实景证据、WAVE ROVER/HIL、真实串口、真实 route completion signal、真实现场 task record、真实电梯材料、真实 dropoff/cancel completion、delivery result 和同一 safe `evidence_ref` 上车实机复账；PR #4 route/elevator field materials 仍需真实现场证据。 |
| Objective 4：手机用户体验与低成本量产边界 | 约 99% | 最新 sprint 让 mobile/web 只读展示“现场证据复跑材料派发”，优先消费 Robot safe alias 或兼容 summary，展示 owner work orders、required material groups、rerun commands、callback packet requirements、safe `evidence_ref`、`software_proof_docker_field_evidence_rerun_material_dispatch_gate`、`not_proven`、`safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`，且 Start Delivery / Confirm Dropoff / Cancel gating 不变。该收益是 phone-safe local fixture software proof，不是真实手机/browser 验收，Objective 4 保守保持约 99%。 | `not_proven` 仍包括真实手机 / 真实 iPhone/Android device behavior、production app、真实 PWA prompt/userChoice 现场验收、true phone/browser acceptance、真实 route/elevator field pass、真实 Nav2/fixed-route、真实 dropoff/cancel completion、真实 cancel completion、delivery success、真实 2D LiDAR / ToF SKU/source/receipt/procurement/installation/wiring/power/calibration/HIL-entry 材料、O5 external proof、真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue、production worker/migration/cutover、WAVE ROVER、HIL 和量产实物验收；PR #5 2D LiDAR / ToF hardware materials 仍是独立缺口。 |
| Objective 5：云中转 + OSS/CDN 数据通路产品化（历史 O6） | 约 68% | 最新 sprint `field_evidence_rerun_material_dispatch` 不改 cloud commands/status/ack、不新增公网入口或外部材料，只把 O2/O3/O4 现场复跑材料缺口做成 `software_proof_docker_field_evidence_rerun_material_dispatch_gate`。上一轮 `cloud_command_id_conflict_visibility_guard` 仍是 O5 最新 command conflict software proof。Objective 5 保持约 68%。 | 仍没有真实公网 HTTPS/TLS、真实 4G/SIM、真实手机设备/browser、production app、真实 PWA prompt/userChoice、OSS/CDN live traffic、真实 production DB/queue connectivity、真实 production worker/migration/cutover、多实例一致性、queue ordering、transaction isolation、backup/recovery、Nav2/fixed-route、WAVE ROVER、HIL 或真实送达；PR #4 route/elevator field materials 和 PR #5 2D LiDAR / ToF hardware materials 仍是独立缺口，`PRRT_kwDOSWB9286CJ3tX` 仍 unresolved / `is_resolved=false` / material pending。本轮 dispatch package 不是真实外部云 proof、不是 delivery success。 |
