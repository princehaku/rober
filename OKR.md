...
更新时间：2026-05-14 23:16 Asia/Shanghai。最新 sprint：`2026.05.14_23-24_pc-route-debug-console`。详细历史见 `docs/process/okr_progress_log.md`。
...
| Objective 2：可送垃圾任务完整闭环 | 约 82% | `2026.05.14_23-24_pc-route-debug-console` 完成 `software_proof_docker_pc_route_debug_console_gate`：Task A `autonomy-engineer` 新增 `pc-tools/route/route_debug_web.py`、`test_route_debug_web.py`、`README.md`，并更新 `pc-tools/README.md` 与 `docs/navigation/fixed_route_workflow.md`；Task B `robot-software-engineer` 在 diagnostics 中接入 `pc_route_debug_console_ref` / `TRASHBOT_PC_ROUTE_DEBUG_CONSOLE`，只接受 `schema=trashbot.pc_route_debug_console.v1` + `evidence_boundary=software_proof_docker_pc_route_debug_console_gate`；Task C `full-stack-software-engineer` 在 `mobile/web` 增加只读 PC route debug availability 面板，不读取 PC 文件或 raw artifact，不改变 Start/Confirm/Cancel gating。任务闭环价值是 recent task、失败原因、`not_proven` 和 `delivery_success=false` 能通过 PC console、diagnostics 与手机只读摘要被同一口径复盘。 | 仍缺真实 Nav2/fixed-route 运行、同一 `evidence_ref` 的上车复账、真实送达、失败恢复实测、dropoff/cancel completion 和 delivery success。 |
| Objective 3：可验证导航与固定路线 | 约 82% | 同一 `software_proof_docker_pc_route_debug_console_gate` 补齐 Objective 3 KR5 的 PC 关键帧调试页面软件证据：`schema=trashbot.pc_route_debug_console.v1` 输出 `route_progress`、`keyframe_preflight`、`current_position`、`current_checkpoint`、`target`、`match_status`、`failure`、`recent_task`、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`、`console_controls=read_only`。Autonomy 验证 py_compile pass、`test_route_debug_web.py` `Ran 5 tests OK`、`--help` pass、`--once-json` 临时 drill pass、required `rg` pass、diff check pass；Robot 验证 py_compile pass、diagnostics unittest `Ran 53 tests OK`、required `rg` pass、diff check pass；Full-stack 验证 mobile unittest `Ran 4 tests OK`、py_compile pass、`node --check` pass、required `rg` pass、diff check pass。 | 仍缺真实路线采集、Nav2 waypoint/fixed-route 实跑、关键帧实景证据、WAVE ROVER/HIL、真实串口和上车复账。 |
...
| Objective 5：云中转 + OSS/CDN 数据通路产品化（历史 O6） | 约 68% | 本轮没有新增真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue connectivity、production worker/migration 或真实外部 O5 材料；PC route debug console 只证明本机 Docker/local 软件材料可被 PC console、diagnostics 和 mobile 只读摘要安全消费，不能替代外部云/4G/OSS/CDN/DB/queue proof。因此 Objective 5 保持约 68%，不因 `software_proof_docker_pc_route_debug_console_gate` 上调。 | 仍没有真实公网 HTTPS/TLS、真实 4G/SIM、真实手机设备/browser、production app、真实 PWA prompt/user choice、OSS/CDN live traffic、真实 production DB/queue connectivity、migration、worker、多实例一致性、queue ordering、transaction isolation、backup/recovery、Nav2/fixed-route、WAVE ROVER、HIL 或真实送达。 |

本轮 OKR 口径：Objective 2 和 Objective 3 均从约 81% 谨慎上调到约 82%，理由是当前 PC route debug console 已把 `route_progress`、`keyframe_preflight`、当前位置、当前 checkpoint、目标点、匹配状态、失败原因和最近任务状态形成可独立运行的 PC 只读调试页面/API，并通过 diagnostics 与 mobile 只读摘要保持 metadata-only 控制边界。证据边界是 `software_proof_docker_pc_route_debug_console_gate`；这不是真实 Nav2/fixed-route 实跑、真实路线采集、WAVE ROVER、真实串口、HIL、同一 `evidence_ref` 的上车复账、dropoff/cancel completion 或 delivery success。Objective 5 仍是最低（约 68%），但本轮没有真实外部 O5 材料，tech-plan 中切到 O2/O3 的理由仍成立；Objective 1 和 Objective 4 不调整。
