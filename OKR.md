...
更新时间：2026-05-17 19:17 Asia/Shanghai。最新 sprint：`2026.05.17_19-20_route-task-result-callback-review-decision`。详细历史见 `docs/process/okr_progress_log.md`。

| Objective | 当前进度 | 本轮证据与边界 | 主要缺口 |
| --- | --- | --- | --- |
| Objective 1：硬件协议可信底盘 | 约 77% | 本轮 sprint `2026.05.17_19-20_route-task-result-callback-review-decision` 推进 O2/O3 的 `route_task_field_retest_result_callback_review_decision` review-decision gate，未新增真实 WAVE ROVER、UART、HIL、`T=1001` feedback、`/odom`、`/imu/data`、`/battery` 或真实 2D LiDAR / ToF 材料；因此 Objective 1 保持约 77%。本轮证据边界是 Docker-only 的 `software_proof_docker_route_task_field_retest_result_callback_review_decision_gate`，`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`。 | 仍缺真实 WAVE ROVER `hil_pass`、真实串口日志、`T=1001` feedback、`/odom`、`/imu/data`、`/battery` 实机样本；PR #5 相关 2D LiDAR / ToF 仍缺真实 SKU/source、receipt、采购、安装、接线、电源、标定、真实 HIL-entry 和 field evidence。 |
| Objective 2：可送垃圾任务 + 电梯 assisted delivery 必达闭环 | 约 99% | 本轮完成 `software_proof_docker_route_task_field_retest_result_callback_review_decision_gate`：PC gate 接收上一轮 callback-intake artifact / summary，输出 `trashbot.route_task_field_retest_result_callback_review_decision.v1` / `_summary.v1`，把 accepted/missing/rejected updates 转成 `ready_for_result_review`、`needs_material_backfill`、`needs_callback_rerun`、`evidence_ref_mismatch_rerun` 或 `rejected_unsafe_callback`；Robot diagnostics 新增 metadata-only consumer，支持 file/env/top-level/nested summary 并 fail closed；mobile/web 新增只读“路线任务回调复核决策” panel，展示 review decision、material status、owner handoff、next required evidence、safe evidence ref、`not_proven` 和 boundary flags，copy/export 仍需 `safe_copy` 白名单授权，Start/Confirm/Cancel gating 不变。PR #4 route/elevator 材料链从回调摄取推进到复核决策，但已经接近软件证明上限，Objective 2 保持约 99%；仍是 Docker-only，`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`。 | 仍缺真实电梯、真实喇叭/TTS、真实 Nav2/fixed-route 运行、真实 route completion signal、真实 task record、真实路线采集、真实 WAVE ROVER/UART/HIL、同一 `evidence_ref` 的上车实机复账、真实门状态、真实楼层确认、人工协助现场记录、真实送达、失败恢复实测、真实 dropoff completion、真实 cancel completion 和 delivery success；PR #4 route/elevator field materials 仍需真实材料回填。 |
| Objective 3：可验证导航与固定路线 | 约 99% | 本轮 callback-review-decision gate 将 Nav2/fixed-route runtime log、route completion signal、task record、door state、target floor confirmation、human assistance note、dropoff/cancel completion、delivery result 八类 result materials 的 accepted/missing/rejected updates 转成下一步 owner backfill、callback rerun、evidence-ref mismatch rerun 或 result review 判断，并在 PC / Robot / mobile 三面共同只读呈现 same-`evidence_ref` alignment、review decision、next required evidence 与复跑要求。Objective 3 保持约 99%；仍是 Docker-only 的 `software_proof_docker_route_task_field_retest_result_callback_review_decision_gate`，`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`。 | 仍缺真实路线采集、Nav2 waypoint/fixed-route 实跑、关键帧实景证据、WAVE ROVER/HIL、真实串口、真实 route completion signal、真实 task record、真实电梯材料、真实 dropoff/cancel completion、delivery result 和同一 `evidence_ref` 上车实机复账；PR #4 route/elevator field materials 仍需真实现场证据。 |
| Objective 4：手机用户体验与低成本量产边界 | 约 99% | 本轮 mobile/web 新增只读“路线任务回调复核决策” panel：展示 review decision、material status、owner handoff、next required evidence、safe evidence ref、safe copy、`not_proven`、`delivery_success=false` 和 `primary_actions_enabled=false`；copy/export 由 `safe_copy` 白名单授权，缺失时显示 `blocked copy unavailable`；Start Delivery、Confirm Dropoff、Cancel gating 不变。Objective 4 保持约 99%，因为本轮没有真实手机设备、真实 iPhone/Android device behavior、production app、真实 PWA prompt/user choice 或真实现场 phone behavior。 | `not_proven` 仍包括真实手机 / 真实 iPhone/Android device behavior、production app、真实 PWA prompt/user choice 现场验收、真实 route/elevator field pass、真实 Nav2/fixed-route、真实 dropoff/cancel completion、delivery success、真实 2D LiDAR / ToF SKU/source/receipt/procurement/installation/wiring/power/calibration/HIL-entry 材料、O5 external proof、公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue、production worker/migration/cutover、WAVE ROVER、HIL 和量产实物验收；PR #5 2D LiDAR / ToF hardware materials 仍是独立缺口。 |
| Objective 5：云中转 + OSS/CDN 数据通路产品化（历史 O6） | 约 68% | 本轮只推进 O2/O3 的 `route_task_field_retest_result_callback_review_decision` chain，没有真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue connectivity、production worker/migration/cutover 或其他真实 external proof；因此 Objective 5 保持约 68%。本轮边界仍是 Docker-only，`software_proof_docker_route_task_field_retest_result_callback_review_decision_gate`，`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`，not real Objective 5 external proof。 | 仍没有真实公网 HTTPS/TLS、真实 4G/SIM、真实手机设备/browser、production app、真实 PWA prompt/user choice、OSS/CDN live traffic、真实 production DB/queue connectivity、真实 production worker/migration/cutover、多实例一致性、queue ordering、transaction isolation、backup/recovery、Nav2/fixed-route、WAVE ROVER、HIL 或真实送达；PR #4 route/elevator field materials 和 PR #5 2D LiDAR / ToF hardware materials 仍是独立缺口。 |
