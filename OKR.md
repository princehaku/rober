...
更新时间：2026-05-10 16:50 Asia/Shanghai。
...
| Objective 3 可验证导航与固定路线 | 约 71% | fixed-route debug web 已从 raw JSON 升级为可读状态面板，展示 route state、checkpoint、current target、visual gate、keyframe preflight、失败原因和 recent task 引用；route debug 目标测试 4 OK，nav 包 31 OK，完整 smoke 通过 | 仍需真实 keyframe/live frame 匹配样例、Nav2 waypoint 实跑、Docker/Humble build 和真实路线采集 |
