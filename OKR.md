...
更新时间：2026-05-10 06:17 Asia/Shanghai。
...
| Objective 3 可验证导航与固定路线 | 约 67% | fixed-route dry-run 在 `enable_visual_gate=true` 时新增全路线 keyframe preflight，启动后即可暴露缺失/无效关键帧；`learn.launch.py` 现在可通过 `route_recorder:=true` 同步启动 `route_data_recorder`，学习阶段能标准化产出 route CSV/keyframes | 仍需真实 keyframe/live frame 匹配样例、Nav2 waypoint 实跑、Docker/Humble build 和真实路线采集 |
| Objective 4 感知模块产品化 | 约 54% | 视觉节点已支持原图、标注图、检测 JSON、manifest 落盘；`/api/diagnostics` 现在会消费 manifest 并输出样本数量、最新样本、检测数量、上下文和读取错误 | 仍需真实路线数据集、runtime 文件写入单测和持续标注流程 |
