...
更新时间：2026-05-14 22:22 Asia/Shanghai。最新 sprint：`2026.05.14_22-03_route-task-rehearsal-operator-review`。详细历史见 `docs/process/okr_progress_log.md`。

| Objective | 当前进度 | 本轮证据与边界 | 主要缺口 |
| --- | --- | --- | --- |
| Objective 1：硬件协议可信底盘 | 约 75% | 本轮未改硬件、WAVE ROVER、UART、Orange Pi、launch 参数或 HIL 证据；`software_proof_docker_route_task_rehearsal_operator_review_gate` 继续把 HIL / `not_proven` 写为未证明，不提升硬件完成度。 | 仍缺真实 WAVE ROVER `hil_pass`、真实串口日志、`T=1001` feedback、`/odom`、`/imu/data`、`/battery` 实机样本。 |
| Objective 2：可送垃圾任务完整闭环 | 约 81% | `2026.05.14_22-03_route-task-rehearsal-operator-review` 完成 `software_proof_docker_route_task_rehearsal_operator_review_gate`：Task A `autonomy-engineer` 新增 `pc-tools/evidence/route_task_rehearsal_operator_review.py`，消费 `route_task_rehearsal_execution_bundle.json`，输出 `schema=trashbot.route_task_rehearsal_operator_review.v1`、`next_rehearsal_decision`、`not_proven`、whitelist-only `safe_copy`、`primary_actions_enabled=false` 和 `delivery_success=false`；Task B `robot-software-engineer` 新增 diagnostics summary `route_task_rehearsal_operator_review`，支持 explicit ref 与 `TRASHBOT_ROUTE_TASK_REHEARSAL_OPERATOR_REVIEW`，异常输入和 unsafe copy 均保守降级；Task C `full-stack-software-engineer` 在 `mobile/web` 首屏新增“路线/任务排练复盘”只读摘要，展示 decision、mismatch、boundary 与 `not_proven`。该进展让任务复盘从可复跑 execution bundle 推进到操作员可读的下一轮重跑/补证据决策。 | 仍缺真实 Nav2/fixed-route 运行、同一 `evidence_ref` 的上车复账、真实送达、失败恢复实测、dropoff/cancel completion 和 delivery success。 |
| Objective 3：可验证导航与固定路线 | 约 81% | 同一 `software_proof_docker_route_task_rehearsal_operator_review_gate` 把 route/task execution bundle、crosscheck/HIL boundary、mismatch 摘要、diagnostics summary 和手机首屏只读复盘串成操作员可执行的固定路线软件排练复盘链。Autonomy 验证 `py_compile`、`--help`、临时 valid bundle drill、schema/boundary/decision/not_proven/safe_copy blacklist assertions、missing/read_error/unsupported schema smoke、required `rg`、scoped diff check 均通过；Robot 验证 diagnostics py_compile、`test_operator_gateway_diagnostics.py` `Ran 51 tests ... OK`、required `rg`、scoped diff check 通过；Full-stack 验证 `test_mobile_web_entrypoint` `Ran 3 tests ... OK`、py_compile、`node --check mobile/web/app.js`、required `rg`、scoped diff check 通过。 | 仍缺真实路线采集、Nav2 waypoint/fixed-route 实跑、关键帧实景证据、WAVE ROVER/HIL、真实串口和上车复账。 |
...
| Objective 5：云中转 + OSS/CDN 数据通路产品化（历史 O6） | 约 68% | 本轮没有新增真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue connectivity、production worker/migration 或真实外部 O5 材料；route/task rehearsal operator review 只证明 Docker/local execution bundle 可被转成 diagnostics/mobile-safe 的操作员复盘和下一轮决策，不能替代外部云/4G/OSS/CDN/DB/queue proof。因此 Objective 5 保持约 68%，不因 `software_proof_docker_route_task_rehearsal_operator_review_gate` 上调。 | 仍没有真实公网 HTTPS/TLS、真实 4G/SIM、真实手机设备/browser、production app、真实 PWA prompt/user choice、OSS/CDN live traffic、真实 production DB/queue connectivity、migration、worker、多实例一致性、queue ordering、transaction isolation、backup/recovery、Nav2/fixed-route、WAVE ROVER、HIL 或真实送达。 |

本轮 OKR 口径：Objective 2 和 Objective 3 均从约 80% 谨慎上调到约 81%，理由是当前 route/task rehearsal 已从可复跑 execution bundle + diagnostics 可消费 manifest，推进到操作员可读的 operator review、`next_rehearsal_decision`、phone-safe `safe_copy` 和 mobile 首屏只读复盘。证据边界是 `software_proof_docker_route_task_rehearsal_operator_review_gate`；这不是真实 Nav2/fixed-route 实跑、真实路线采集、WAVE ROVER、真实串口、HIL、同一 `evidence_ref` 的上车复账、dropoff/cancel completion 或 delivery success。Objective 5 仍是最低（约 68%），但本轮没有真实外部 O5 材料，tech-plan 中切到 O2/O3 的理由仍成立；Objective 1 和 Objective 4 不调整。
...
- 下一轮按 `OKR.md` 4.1 重新排序。当前最低完成度仍为 Objective 5（约 68%），但只有拿到至少一种真实外部材料时才应继续推进 O5 completion：OSS/CDN live traffic、HTTPS/TLS 公网入口、4G/SIM、production DB/queue connectivity 或 production worker/migration 证据。若外部材料仍不可用，则不要重复本地 O5 metadata depth；可继续转向 Objective 2/3 的真实 Nav2/fixed-route、真实路线采集、同一 `evidence_ref` 的上车复账，或 Objective 4 的真实 iPhone/Android device behavior、production app、真实 PWA prompt/user choice 现场材料。O2/O3 后续若继续推进，必须从本轮 `software_proof_docker_route_task_rehearsal_operator_review_gate` 走向真实路线/任务证据，不得把 operator review、execution bundle、diagnostics summary、artifact pass 或 ACK 写成 delivery success。

## 7. 整体风险与待办

- 当前仍缺真实手机设备、真实 iPhone/Android device behavior、production app、真实 PWA prompt/user choice 现场验收、真实云/4G、production DB/queue、OSS/CDN live traffic、Nav2/fixed-route、真实路线采集、WAVE ROVER、真实串口、HIL、同一 `evidence_ref` 的上车复账、真实 cancel completed、真实 dropoff completion 和真实 delivery 证据；`software_proof_docker_route_task_rehearsal_operator_review_gate` 只证明 route/task execution bundle 可被转成操作员复盘、diagnostics summary、phone-safe copy、mobile 只读摘要和下一轮重跑/补证据决策，不证明真实固定路线运行、真实底盘运动、真实投放或真实交付。
