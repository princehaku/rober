...
更新时间：2026-05-16 17:25 Asia/Shanghai。最新 sprint：`2026.05.16_17-18_hardware-baseline-source-alignment`。详细历史见 `docs/process/okr_progress_log.md`。

| Objective | 当前进度 | 本轮证据与边界 | 主要缺口 |
| --- | --- | --- | --- |

| Objective 1：硬件协议可信底盘 | 约 74% | 本轮完成 `software_proof_docker_hardware_baseline_source_alignment_gate`：Hardware PC gate 读取 `docs/product/production_hardware_boundary.md` 与 `docs/vendor/VENDOR_INDEX.md`，输出 `trashbot.hardware_baseline_source_alignment.v1` / summary v1；Robot diagnostics metadata-only 消费 source-alignment summary；mobile/web 只读展示“硬件基线来源对齐”。该链路把 PR #5 review 的默认硬件集 / mandatory sensor baseline 矛盾风险和传感器 source attribution 风险转成可机器验收的 `not_proven` software proof，因此 Objective 1 从约 73% 保守上调到约 74%。 | 仍缺真实 WAVE ROVER `hil_pass`、真实串口日志、`T=1001` feedback、`/odom`、`/imu/data`、`/battery` 实机样本；2D LiDAR / ToF 仍缺真实 SKU/source、receipt、采购、安装、接线、电源、标定和 HIL entry evidence。本轮 vendor coverage 只证明本地资料覆盖 Orange Pi / WAVE ROVER / UART JSON / firmware/vendor app / camera material，不证明项目 2D LiDAR / ToF 已采购、安装、接线、标定、HIL 或 field pass。 |
