...
H2/高阶场景可以纳入“电梯 assisted delivery”：在受控楼宇内，小车识别电梯是否开门，进入电梯后通过语音请求好心人帮忙按目标楼层，持续识别是否到达目标楼层，并在目标楼层开门时尽快驶出。这个方向服务于“普通用户只用手机，小车完成跨楼层 trash delivery”的北极星，但它不是当前 MVP 已完成能力，也不默认要求机械臂、昂贵传感器或新增下位机硬件。
...
4. **电梯能力是 assisted delivery，不是无人全自动乘梯承诺**
   - 目标是让跨楼层送垃圾在受控环境中可被产品化验证：小车看门、进门、语音求助按键、判断目标楼层、开门后驶出。
   - 小车不按电梯按钮，不默认改造电梯，不默认增加机械臂或电梯控制硬件；人工协助是产品流程的一部分。
...
- KR6（H2/受控场景）：定义并验证 elevator assisted delivery 状态链路：等待电梯开门 -> 进入电梯 -> 语音请求“你好,好心人,.我要去1楼扔垃圾,请帮我按一下电梯,” -> 等待目标楼层 -> 目标楼层开门后驶出；验证前不得写成当前 MVP 完成能力。
...
- KR6（H2/受控场景）：补齐电梯场景感知 contract，至少覆盖电梯门开/关、是否已进入轿厢、目标楼层到达证据、目标楼层开门可驶出证据；优先用现有摄像头/语音/日志证据验证，不把新硬件作为默认前提。
...
- KR6（H2/受控场景）：定义跨楼层 trash delivery 的手机/语音体验和人工协助边界：用户只选择目标楼层和垃圾站，小车在电梯内主动求助按键，失败时能在手机端解释“未开门、未确认目标楼层、需要人工接管”。

## 4.1 当前 OKR 进度快照

更新时间：2026-05-11 17:30 Asia/Shanghai。

补充：`2026.05.11_10-11_hil-docker-preflight-to-real-run` 继续执行 O1→O2→O3 顺序。O1 Docker preflight 因 `osrf/ros:humble-desktop` 拉取/解包异常 blocked，且本机仍无真实串口硬件；本轮只补强 O2/O3 same-`evidence_ref` 软件复盘支撑，O1/O2/O3 完成度不因软件证据上调。

| Objective | 当前进度 | 本轮新增证据 | 剩余关键缺口 |
| --- | --- | --- | --- |
| Objective 1 硬件协议可信底盘 | 约 75% | `sprints/2026.05.11_10-11_hil-docker-preflight-to-real-run` 已执行 Docker preflight；`bash scripts/docker_humble_build.sh` 卡在 `osrf/ros:humble-desktop` metadata/unpack，日志含 `encountered unknown type text/html`；`hardware_smoke_wave_rover.py --status` 可输出 `pyserial_available=true`、`hil_ready=false`、`blocked_reason=no_serial_candidates_found` 与 required evidence files/source boundary。 | 仍缺本轮真实 WAVE ROVER `hil_pass` evidence packet（`command.txt`/`serial.log`/`feedback_T1001.log`）与 `/odom`、`/imu/data`、`/battery` 实机样本；还需先修复 Docker registry/proxy/cache 或镜像层解包问题，并接入真实串口设备。 |
| Objective 2 可恢复送垃圾任务闭环 | 约 76% | 本轮补强 O1 real run 后的 O2 消费链路：`task_record.route_progress` 顶层持久化，`nav_results[-1].evidence.route_progress`、`failure_code`、`state_transition_history`、`human_intervention_required`、`evidence_ref` 可在 Docker 中严格对账；targeted behavior tests 为 `Ran 42 tests ... OK`。 | 仍缺真实 Nav2/fixed-route 运行下的任务复盘 evidence；O1 `hil_pass` 未解锁前，O2 不能声明实机 closed。 |
| Objective 3 可验证导航与固定路线 | 约 76% | `scripts/evidence_crosscheck.py` 支持 `--task-record-dir` 按同一 `evidence_ref` 自动查找 task record，并在缺 route-level 字段时返回 mismatch；软件样例可输出 `CHECK summary: mismatches=0`。 | 仍缺真实路线采集、Nav2 waypoint/fixed-route 实跑、关键帧实景证据，以及与 O1 HIL 同一 `evidence_ref` 的上车复账。 |
...
| Objective 5：手机体验与量产边界 | 新增 KR6，要求手机/语音体验解释人工协助边界和失败原因 | 当前未新增 TTS/喇叭实现、真实手机验收或量产实物验证 |
