...
3. **移除行为层占位逻辑**
   - `task_orchestrator` 中模拟 patrol/collection/delivery 的 sleep 逻辑要替换成真实 action client 或导航接口。
   - 所有占位项要么落入明确 backlog，要么在当前阶段实现。
...
4. 增加 `run_smoke_tests.ps1` 或等价脚本，在 Windows 开发机上至少跑 Python 静态测试；在 Orange Pi 上跑 ROS2 build 和硬件 smoke test。
