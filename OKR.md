...
### Objective 4：把摄像头从“捡垃圾检测”调整为送达任务的可迭代感知模块

**目标说明**：先让摄像头服务于固定路线、垃圾站识别、障碍/异常记录、远程查看和数据沉淀；垃圾检测只作为增强能力，不作为没有机械臂阶段的核心闭环前提。

**Key Results**

- KR1：`TrashStatus` 发布字段语义固定：坐标系、置信度、垃圾类型、是否垃圾桶、时间戳。
- KR2：视觉节点支持阈值、最小面积、ROI、debug image 输出参数化。
- KR3：检测结果写入样本目录，至少保存原图、标注图、检测 JSON，支持后续训练数据积累。
...
更新时间：2026-05-10 00:59 Asia/Shanghai。
...
| Objective 2 可恢复送垃圾任务闭环 | 约 65% | 主 `task_orchestrator` 已具备装载确认、delivery、dropoff、return、action result、task record 和 fixed-route status reader 证据；主 patrol 已不再是硬编码 5 点 sleep | `use_saved_map=false` 学习阶段仍有模拟完成口径；legacy server 仍保留 sleep demo |
...
| Objective 4 感知模块产品化 | 约 25% | 本轮把 fixed-route 视觉门控状态接入 dry-run 证据链，为后续站点识别/异常记录打基础 | 样本落盘、标注图、检测 JSON 和感知评估表仍未完成 |
| Objective 5 手机体验与量产边界 | 约 35% | 任务终态、失败原因和 fixed-route debug status 更适合远程诊断消费 | 手机端最小流程、语音提示词、量产硬件约束表仍需正式文档和入口实现 |
