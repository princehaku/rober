...
更新时间：2026-05-26 04:29 Asia/Shanghai。最新 sprint：`2026.05.26_00-01_cloud-phone-command-api-mainline`。详细历史见 `docs/process/okr_progress_log.md`。
...
| Objective 5：云中转 + OSS/CDN 数据通路产品化（历史 O6） | 约 72% | Objective 5 仍最低，但本轮从历史只读 metadata wrapper 转为真实可调用的 `cloud_phone_command_api` 主链路：independent relay 新增 bearer-gated `POST /api/commands/collect`、`POST /api/commands/confirm-dropoff`、`POST /api/commands/cancel`，将手机任务级请求规范化后写入现有 command queue；robot 仍通过 `/robots/{robot_id}/commands/next` outbound polling 领取。`mobile/web` 主动作 endpoint 已切到 `/api/commands/*`，提交 `trashbot.cloud_phone_command_api_request.v1` envelope，并展示 sanitized receipt：`ack_semantics=queued_not_delivery_success`、`delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false`、`software_proof_docker_cloud_phone_command_api_gate`。追加 hardening 后，command store 不可用或写入失败时 API fail closed，返回 `503 command_store_unavailable`，不返回 queued receipt；Robot focused unittest 更新为 `Ran 4 tests in 2.769s OK`。Objective 5 按软件功能主链路进展保守保持约 72%。 | 仍没有真实公网 HTTPS/TLS、真实 4G/SIM、真实手机设备/browser、production app、真实 PWA prompt/userChoice、OSS/CDN live traffic、真实 production DB/queue connectivity、真实 production worker/migration/cutover、多实例一致性、真实 production queue ordering、transaction isolation、backup/recovery、verified terminal result、Nav2/fixed-route、WAVE ROVER、HIL 或真实送达；命令入队不是 delivery/dropoff/cancel terminal result，`command_store_unavailable` 也只证明本地 fail-closed，不证明真实 production DB/queue recovery。PR #4 route/elevator field materials 和 PR #5 2D LiDAR / ToF hardware materials 仍是独立缺口，`PRRT_kwDOSWB9286CJ3tX` 仍 unresolved / hardware_material_pending；本轮 not true phone/browser proof、不是 public HTTPS/TLS、不是 4G/SIM、不是 OSS/CDN live traffic、not production DB/queue、not worker/cutover、not verified terminal result、not HIL、not PR #5 resolved、not route/elevator field pass、not delivery success。 |
...
- 当前最新 sprint 为 `cloud_phone_command_api_mainline`，证据边界是 `software_proof_docker_cloud_phone_command_api_gate`；它证明当前 repo 的 relay 已有面向手机/云端的任务级 command enqueue 主链路，`mobile/web` 也已从旧 `/api/collect` / `/api/dropoff/confirm` / `/api/cancel` 切到 `/api/commands/*` 并展示 queued receipt。追加 store unavailable hardening 后，未成功写入 command store 时返回 `503 command_store_unavailable`，不会产出 `queued_not_delivery_success` receipt。它仍保持 `delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false`，不代表真实送达、真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue、worker/cutover、HIL、WAVE ROVER/UART、route/elevator field pass、PR #5 resolved 或 true phone/browser proof。
- 下一轮按 `OKR.md` 4.1 重新排序。当前数值最低完成度仍是 Objective 5（约 72%）。O5 下一步应优先把 `cloud_phone_command_api` 接到真实部署链路：公网 HTTPS/TLS、真实 4G/SIM、production DB/queue connectivity、production worker/cutover、OSS/CDN live traffic、真实手机/browser 证据，或 verified terminal delivery/dropoff/cancel result。若外部材料仍不可用，后续 Docker/local 工作必须继续围绕真实 command lifecycle、replay、ACK/result reconciliation、部署 cutover 和故障恢复，不再重复只读 review/handoff/escalation wrapper。
