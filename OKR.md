...
更新时间：2026-05-17 07:22 Asia/Shanghai。最新 sprint：`2026.05.17_07-08_cloud-worker-migration-rehearsal`。详细历史见 `docs/process/okr_progress_log.md`。

| Objective | 当前进度 | 本轮证据与边界 | 主要缺口 |
| --- | --- | --- | --- |

| Objective 1：硬件协议可信底盘 | 约 77% | 本轮 sprint `2026.05.17_07-08_cloud-worker-migration-rehearsal` 只推进 Objective 5 的 `cloud_worker_migration_rehearsal`，未新增真实 WAVE ROVER、UART、HIL、`T=1001` feedback、`/odom`、`/imu/data`、`/battery` 或真实 2D LiDAR / ToF 材料；因此 Objective 1 保持约 77%。本轮证据边界仍是 Docker-only 的 `software_proof_docker_cloud_worker_migration_rehearsal_gate`，`production_ready=false`、`delivery_success=false`、`primary_actions_enabled=false`，not real external proof。 | 仍缺真实 WAVE ROVER `hil_pass`、真实串口日志、`T=1001` feedback、`/odom`、`/imu/data`、`/battery` 实机样本；PR #5 相关 2D LiDAR / ToF 仍缺真实 SKU/source、receipt、采购、安装、接线、电源、标定、真实 HIL-entry 和 field evidence。 |
| Objective 2：可送垃圾任务 + 电梯 assisted delivery 必达闭环 | 约 89% | 本轮 sprint `2026.05.17_07-08_cloud-worker-migration-rehearsal` 只推进 Objective 5 的 `cloud_worker_migration_rehearsal`，未新增真实电梯、真实 Nav2/fixed-route、真实 route completion signal、真实 task record、真实 dropoff/cancel completion 或 delivery success；因此 Objective 2 保持约 89%。本轮证据边界仍是 Docker-only 的 `software_proof_docker_cloud_worker_migration_rehearsal_gate`，`production_ready=false`、`delivery_success=false`、`primary_actions_enabled=false`，not real external proof。 | 仍缺真实电梯、真实喇叭/TTS、真实 Nav2/fixed-route 运行、真实 route completion signal、真实 task record、真实路线采集、真实 WAVE ROVER/UART/HIL、同一 `evidence_ref` 的上车实机复账、真实门状态、真实楼层确认、人工协助现场记录、真实送达、失败恢复实测、真实 dropoff completion、真实 cancel completion 和 delivery success。 |
| Objective 3：可验证导航与固定路线 | 约 89% | 本轮 sprint `2026.05.17_07-08_cloud-worker-migration-rehearsal` 只推进 Objective 5 的 `cloud_worker_migration_rehearsal`，未新增真实 SLAM/Nav2、fixed-route runtime log、route completion signal、task record、路线采集或关键帧实景证据；因此 Objective 3 保持约 89%。本轮证据边界仍是 Docker-only 的 `software_proof_docker_cloud_worker_migration_rehearsal_gate`，`production_ready=false`、`delivery_success=false`、`primary_actions_enabled=false`，not real external proof。 | 仍缺真实路线采集、Nav2 waypoint/fixed-route 实跑、关键帧实景证据、WAVE ROVER/HIL、真实串口、真实 route completion signal、真实 task record、真实电梯材料、真实 dropoff/cancel completion、delivery result 和同一 `evidence_ref` 上车实机复账。 |
| Objective 4：手机用户体验与低成本量产边界 | 约 99% | 本轮 sprint `2026.05.17_07-08_cloud-worker-migration-rehearsal` 只推进 Objective 5 的 `cloud_worker_migration_rehearsal`，未新增真实手机设备、真实 iPhone/Android device behavior、production app、真实 PWA prompt/user choice、真实公网入口或真实量产验收；因此 Objective 4 保持约 99%。本轮证据边界仍是 Docker-only 的 `software_proof_docker_cloud_worker_migration_rehearsal_gate`，`production_ready=false`、`delivery_success=false`、`primary_actions_enabled=false`，not real external proof。 | `not_proven` 仍包括真实手机 / 真实 iPhone/Android device behavior、production app、真实 PWA prompt/user choice 现场验收、真实 route/elevator field pass、真实 Nav2/fixed-route、真实 dropoff/cancel completion、delivery success、真实 2D LiDAR / ToF SKU/source/receipt/procurement/installation/wiring/power/calibration/HIL-entry 材料、O5 external proof、公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue、production worker/migration、WAVE ROVER、HIL 和量产实物验收。 |
| Objective 5：云中转 + OSS/CDN 数据通路产品化（历史 O6） | 约 67% | 本轮完成 `software_proof_docker_cloud_worker_migration_rehearsal_gate`：新增 `cloud_worker_migration_rehearsal` CLI/env/artifact consumption，artifact schema `trashbot.cloud_worker_migration_rehearsal.v1`，summary schema `trashbot.cloud_worker_migration_rehearsal_summary.v1`，Docker smoke 覆盖 build/start、artifact generation、preflight consumption、status/command/ACK、backup/restore 和 restart recovery；Robot diagnostics 只读消费 metadata，证明 summary 不进入 command payload、不触发 collect/dropoff/cancel/ACK、不推进 cursor。Objective 5 从约 66% 保守上调到约 67%；仍是 Docker-only，`production_ready=false`、`delivery_success=false`、`primary_actions_enabled=false`，not real external proof。 | 仍没有真实公网 HTTPS/TLS、真实 4G/SIM、真实手机设备/browser、production app、真实 PWA prompt/user choice、OSS/CDN live traffic、真实 production DB/queue connectivity、真实 migration、真实 worker、多实例一致性、queue ordering、transaction isolation、backup/recovery、Nav2/fixed-route、WAVE ROVER、HIL 或真实送达；PR #5 相关 2D LiDAR / ToF 仍缺真实 SKU/source、receipt、采购、安装、接线、电源、标定、真实 HIL-entry 和 field evidence。 |
