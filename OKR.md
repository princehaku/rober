...
更新时间：2026-05-14 19:20 Asia/Shanghai。最新 sprint：`2026.05.14_19-20_route-task-rehearsal-artifact`。详细历史见 `docs/process/okr_progress_log.md`。

| Objective | 当前进度 | 本轮证据与边界 | 主要缺口 |
| --- | --- | --- | --- |
| Objective 1：硬件协议可信底盘 | 约 75% | 本轮未改硬件、WAVE ROVER、UART、Orange Pi、launch 参数或 HIL 证据；`software_proof_docker_route_task_rehearsal_artifact_gate` 明确把 HIL alignment 写为未证明，不提升硬件完成度。 | 仍缺真实 WAVE ROVER `hil_pass`、真实串口日志、`T=1001` feedback、`/odom`、`/imu/data`、`/battery` 实机样本。 |
| Objective 2：可送垃圾任务完整闭环 | 约 78% | `2026.05.14_19-20_route-task-rehearsal-artifact` 完成 `software_proof_docker_route_task_rehearsal_artifact_gate`：Task A `autonomy-engineer` 为 `pc-tools/evidence/evidence_crosscheck.py` 新增 `--rehearsal-artifact`，artifact 写出 schema/version、`evidence_ref`、route/task summary、crosscheck status、HIL alignment status 与 `not_proven`，并在 blocked HIL 时仍可保存脱敏 artifact；Task B `robot-software-engineer` 让 `task_record.py` 忽略空 `route_progress`、必要时把 `route_progress.evidence_ref` 提升为 top-level `evidence_ref`，并让 `task_orchestrator.py` 优先使用 fixed-route evidence 或 nested `route_progress` 的真实 `evidence_ref`。该进展只证明 status/replay/task_record 软件对账和任务证据 anchor 更稳定。 | 仍缺真实 Nav2/fixed-route 运行、同一 `evidence_ref` 的上车复账、真实送达、失败恢复实测、dropoff/cancel completion 和 delivery success。 |
| Objective 3：可验证导航与固定路线 | 约 78% | 同一 `software_proof_docker_route_task_rehearsal_artifact_gate` 把 fixed-route status、software proof replay、task record 与 crosscheck 结果收束为可保存 artifact；`pc-tools/README.md`、`docs/navigation/fixed_route_workflow.md` 和 `docs/interfaces/ros_contracts.md` 已同步 route/task rehearsal、`route_progress`、`evidence_ref` 与非 HIL 边界。Task A 验证 `py_compile` pass、`test_task_record.py` Ran 8 tests OK、required `rg` pass、scoped `git diff --check` pass；Task B 验证 behavior py_compile pass、`test_task_record.py` Ran 8 tests OK、collection execution Ran 10 tests OK、fixed-route status Ran 6 tests OK、required `rg` pass、scoped diff check pass。 | 仍缺真实路线采集、Nav2 waypoint/fixed-route 实跑、关键帧实景证据、WAVE ROVER/HIL、真实串口和上车复账。 |
...
| Objective 5：云中转 + OSS/CDN 数据通路产品化（历史 O6） | 约 68% | 本轮没有新增真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue connectivity、production worker/migration 或真实外部 O5 材料；route/task rehearsal artifact 只证明 Docker/local status/replay/task_record 软件对账，不能替代外部云/4G/OSS/CDN/DB/queue proof。因此 Objective 5 保持约 68%，不因 `software_proof_docker_route_task_rehearsal_artifact_gate` 上调。 | 仍没有真实公网 HTTPS/TLS、真实 4G/SIM、真实手机设备/browser、production app、真实 PWA prompt/user choice、OSS/CDN live traffic、真实 production DB/queue connectivity、migration、worker、多实例一致性、queue ordering、transaction isolation、backup/recovery、Nav2/fixed-route、WAVE ROVER、HIL 或真实送达。 |

本轮 OKR 口径：Objective 2 和 Objective 3 均从约 77% 谨慎上调到约 78%，理由是当前 route/task rehearsal artifact 已把 fixed-route status、software proof replay、task record 和 evidence crosscheck 对账结果收束为同一 `evidence_ref` 下可保存、可脱敏、可复核的软件证据包，并由 Task A/B 的 fenced validation 证明 artifact pass 只表示 status/replay/task_record 软件对账通过。证据边界是 `software_proof_docker_route_task_rehearsal_artifact_gate`；这不是真实 Nav2/fixed-route 实跑、真实路线采集、WAVE ROVER、真实串口、HIL、同一 `evidence_ref` 的上车复账、dropoff/cancel completion 或 delivery success。Objective 5 仍是最低（约 68%），但本轮没有真实外部 O5 材料，tech-plan 中切到 O2/O3 的理由仍成立；Objective 1 和 Objective 4 不调整。
...
- 下一轮按 `OKR.md` 4.1 重新排序。当前最低完成度仍为 Objective 5（约 68%），但只有拿到至少一种真实外部材料时才应继续推进 O5 completion：OSS/CDN live traffic、HTTPS/TLS 公网入口、4G/SIM、production DB/queue connectivity 或 production worker/migration 证据。若外部材料仍不可用，则不要重复本地 O5 metadata depth；可继续转向 Objective 2/3 的真实 Nav2/fixed-route、真实路线采集、同一 `evidence_ref` 的上车复账，或 Objective 4 的真实 iPhone/Android device behavior、production app、真实 PWA prompt/user choice 现场材料。O2/O3 后续若继续推进，必须从本轮 `software_proof_docker_route_task_rehearsal_artifact_gate` 走向真实路线/任务证据，不得把 artifact pass 写成 delivery success。

## 7. 整体风险与待办

- 当前仍缺真实手机设备、真实 iPhone/Android device behavior、production app、真实 PWA prompt/user choice 现场验收、真实云/4G、production DB/queue、OSS/CDN live traffic、Nav2/fixed-route、真实路线采集、WAVE ROVER、真实串口、HIL、同一 `evidence_ref` 的上车复账、真实 cancel completed、真实 dropoff completion 和真实 delivery 证据；`software_proof_docker_route_task_rehearsal_artifact_gate` 只证明 status/replay/task_record 软件对账、route/task summary、crosscheck status、HIL alignment status 和 evidence anchor 可保存复核，不证明真实固定路线运行、真实底盘运动、真实投放或真实交付。
