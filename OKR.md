...
更新时间：2026-05-19 12:25 Asia/Shanghai。最新 sprint：`2026.05.19_12-13_task-terminal-field-material-intake`。详细历史见 `docs/process/okr_progress_log.md`。

| Objective | 当前进度 | 本轮证据与边界 | 主要缺口 |
| --- | --- | --- | --- |
| Objective 1：硬件协议可信底盘 | 约 81% | 本轮 `task_terminal_field_material_intake` 不触碰硬件协议、不采集 WAVE ROVER/UART/HIL、不补 PR #5 2D LiDAR / ToF 真实材料；Objective 1 保守保持约 81%。Robot/Full-Stack 均保持 `software_proof`、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false`，不写成硬件通过。 | 仍缺真实 WAVE ROVER `hil_pass`、真实串口日志、真实 `feedback_T1001.log`、真实 feedback interval、真实 `/odom`、`/imu/data`、`/battery` 实机样本、真实 UART、真实上车 HIL packet 和 operator HIL report；PR #5 相关 2D LiDAR / ToF 仍缺真实 SKU/source、receipt、采购、安装、接线、电源、标定、真实 HIL-entry 和 field evidence。 |
| Objective 2：可送垃圾任务 + 电梯 assisted delivery 必达闭环 | 约 99% | 本轮新增 `task_terminal_field_material_intake` software-proof 回填入口：Robot diagnostics 输出 `robot_diagnostics_task_terminal_field_material_intake_summary`，包含 safe evidence ref、accepted safe refs、missing materials、next required evidence、phone-safe copy 和 evidence boundary。Robot 验证 `Ran 206 tests in 0.512s OK`；证据边界保持 `software_proof`、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false`。Objective 2 只记录 terminal field material 后续可回填性，保守保持约 99%。 | 仍缺真实电梯、真实喇叭/TTS、真实 Nav2/fixed-route 运行、真实 route completion signal、真实现场 task record、真实路线采集、真实 WAVE ROVER/UART/HIL、同一 safe `evidence_ref` 的上车实机复账、真实门状态、真实楼层确认、人工协助现场记录、真实送达、失败恢复实测、真实 dropoff completion、真实 cancel completion 和 delivery success；PR #4 route/elevator field materials 仍需真实材料回填、复核和现场通过。 |
| Objective 3：可验证导航与固定路线 | 约 99% | 本轮只让 `task_terminal_field_material_intake` 围绕同一 safe `evidence_ref` 承接后续 route/elevator/Nav2 materials；Autonomy 只读建议已纳入：route/elevator/Nav2 字段只能作为 missing materials / next required evidence，不使用 `route_passed`、`fixed_route_passed`、`nav2_passed`、`field_pass` 或 completion claim。Objective 3 保守保持约 99%。 | 仍缺真实路线采集、Nav2 waypoint/fixed-route 实跑、关键帧实景证据、WAVE ROVER/HIL、真实串口、真实 route completion signal、真实现场 task record、真实电梯材料、真实 dropoff/cancel completion、delivery result 和同一 safe `evidence_ref` 上车实机复账；PR #4 route/elevator field materials 仍需真实现场证据。 |
| Objective 4：手机用户体验与低成本量产边界 | 约 99% | Full-Stack worker 在 mobile/web 增加只读“现场材料回填入口”panel，优先消费 `robot_diagnostics_task_terminal_field_material_intake_summary`，展示 intake status、safe evidence ref、accepted safe refs、missing materials、next required evidence、phone-safe copy 和 evidence boundary；Start Delivery、Confirm Dropoff、Cancel gating 不变。Full-Stack 验证 `Ran 122 tests in 0.863s OK`、`node --check` 通过。Objective 4 只记录 phone-safe visibility，保守保持约 99%。 | `not_proven` 仍包括真实手机 / 真实 iPhone/Android device behavior、production app、真实 PWA prompt/user choice 现场验收、真实 route/elevator field pass、真实 Nav2/fixed-route、真实 dropoff/cancel completion、真实 cancel completion、delivery success、真实 2D LiDAR / ToF SKU/source/receipt/procurement/installation/wiring/power/calibration/HIL-entry 材料、O5 external proof、公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue、production worker/migration/cutover、WAVE ROVER、HIL 和量产实物验收；PR #5 2D LiDAR / ToF hardware materials 仍是独立缺口。 |
