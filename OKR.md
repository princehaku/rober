...
更新时间：2026-05-13 21:15 Asia/Shanghai。最新 sprint：`2026.05.13_21-22_mobile-recovery-decision-gate`。详细历史见 `docs/process/okr_progress_log.md`。
...
| Objective 4：手机用户体验与低成本量产边界 | 约 74% | `2026.05.13_21-22_mobile-recovery-decision-gate` 完成 `software_proof_docker_mobile_recovery_decision_gate`：Task A 在 `mobile/web/` 三步主路径之后新增只读“恢复决策”面板，显示恢复状态、下一步、阻塞原因、支持入口、ACK 语义、证据边界和 `not_proven`；消费可选 `mobile_recovery_decision_gate` / `mobile_recovery_decision_summary`，缺 summary 时仅从 phone-safe 字段派生 blocked-by-design，不调用 Start/Confirm/Cancel，不声称 delivery、dropoff 或 cancel completed。Task B robot compatibility fence 证明恢复决策 metadata-only responses 不触发 collect/confirm_dropoff/cancel，不 POST ACK，不推进 `last_ack_id`，不持久化 terminal cursor，不写 cursor override；valid collect mixed metadata 只执行 command envelope。Task A 验证 `Ran 17 tests in 0.007s OK`，Task B 验证 `Ran 109 tests in 56.010s OK`，两侧 `py_compile` 和 scoped `git diff --check` 通过，Task A 额外 `node --check mobile/web/app.js` 和 fixture JSON parse 通过。 | `not_proven` 仍包括真实 iPhone/Android device behavior、production app、真实 PWA install prompt、真实 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue、TTS/喇叭实放、Nav2/fixed-route、WAVE ROVER、HIL、真实投放、真实取消完成、真实 dropoff completion、真实 delivery 和量产实物验收。 |
| Objective 5：云中转 + OSS/CDN 数据通路产品化（历史 O6） | 约 68% | 本轮没有新增真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue connectivity、production worker/migration 或真实外部 O5 材料；恢复决策只把已有 cloud/device/browser readiness 作为 Objective 4 手机异常处理输入背景。因此 Objective 5 保持约 68%，不因 `software_proof_docker_mobile_recovery_decision_gate` 上调。 | 仍没有真实公网 HTTPS/TLS、真实 4G/SIM、真实手机设备/browser、production app、真实 PWA install prompt、OSS/CDN live traffic、真实 production DB/queue connectivity、migration、worker、多实例一致性、queue ordering、transaction isolation、backup/recovery、Nav2/fixed-route、WAVE ROVER、HIL 或真实送达。 |

本轮 OKR 口径：Objective 4 从约 73% 谨慎上调到约 74%，理由是 `mobile/web/` 已在三步主路径后补齐普通用户异常时的恢复决策说明，并由 targeted mobile unittest 和 robot metadata-only fence 证明恢复决策只读、fail closed、summary metadata 不污染 command/ACK/cursor。证据边界是 `software_proof_docker_mobile_recovery_decision_gate`；这不是真实 iPhone/Android device behavior、production app、真实 PWA install prompt、真实公网 HTTPS/TLS、真实 4G/SIM、OSS/CDN live traffic、production DB/queue、Nav2/fixed-route、WAVE ROVER、HIL、真实 cancel completed、真实 dropoff completed 或真实 delivery 证据。ACK、HTTP accepted、receipt 和 recovery decision 仍只是 accepted/processing/support evidence，不是 delivery success。Objective 5 没有真实外部 O5 材料，保持约 68%；Objective 1/2/3 不调整。
