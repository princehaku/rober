...
补充：`2026.05.11_09-10_hil-pass-route-replay-crosscheck` 的本轮优先级固定为 O1→O2→O3。`software_proof` 对账可继续推进，但在 O1 `hil_pass` 缺样本前，O2/O3 不得升级为实机通过。

| Objective | 当前进度 | 本轮新增证据 | 剩余关键缺口 |
| --- | --- | --- | --- |
| Objective 1 硬件协议可信底盘 | 约 75% | `sprints/2026.05.11_06-07_hil-route-e2e-hardening-2` 收口复核已确认 `source=hil_pass/software_proof`、`evidence_ref` 链路在 `scripts/hardware_smoke_wave_rover.py` 与 `docs/acceptance/*`、`docs/hardware/wave_rover_json_bridge.md` 持续贯通；本轮验收检索命中相关字段。 | 仍缺本轮真实 WAVE ROVER `hil_pass` evidence packet（`command.txt`/`serial.log`/`feedback_T1001.log`）与 `/odom`、`/imu/data`、`/battery` 实机样本；软件与模板证据不能替代实机通过。 |
| Objective 2 可恢复送垃圾任务闭环 | 约 76% | 本轮复核确认 `task_orchestrator/task_record/delivery_state_machine` 已包含 `failure_code`、`state_transition_history`、`human_intervention_required`、`result_path`、`evidence_ref` 字段链路；继承上轮行为侧测试通过结论作为当前可用基线。 | 本轮尚未沉淀新的失败/超时/取消实跑日志样本；仍需真实 Nav2/fixed-route 运行下的任务复盘 evidence 以提升完成度。 |
| Objective 3 可验证导航与固定路线 | 约 76% | 本轮复核确认 `route_utils/fixed_route_autonomy` 保持 `NO_ROUTE`、`CHECKPOINT_MISSING`、`NAVIGATION_ABORT` 与 `checkpoint/current_index/target/evidence_ref` 字段契约。 | 本轮未新增 autonomy 实现日志与测试输出；仍缺真实路线采集、Nav2 waypoint/fixed-route 实跑与关键帧实景证据。 |
