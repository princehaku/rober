...
| --- | --- | --- | --- |
| Objective 1 硬件协议可信底盘 | 约 75% | `sprints/2026.05.11_10-11_hil-docker-preflight-to-real-run` 已执行 Docker preflight；`bash scripts/docker_humble_build.sh` 卡在 `osrf/ros:humble-desktop` metadata/unpack，日志含 `encountered unknown type text/html`，Docker smoke 因本地无 `ros-rbs-humble:dev` 且拉取同类失败退出 `125`；串口只读检查确认 `/dev/ttyUSB0`、`/dev/ttyAMA0`、`/dev/serial0` 均不可见。 | 仍缺本轮真实 WAVE ROVER `hil_pass` evidence packet（`command.txt`/`serial.log`/`feedback_T1001.log`）与 `/odom`、`/imu/data`、`/battery` 实机样本；还需先修复 Docker registry/proxy/cache 或镜像层解包问题。 |
