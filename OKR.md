...
更新时间：2026-05-10 11:34 Asia/Shanghai。
...
| Objective 2 可恢复送垃圾任务闭环 | 约 72% | `TrashCollection` fixed-route 送达现在会把 fixed-route status JSON 的结构化证据写入 `nav_results` 和 task record，包含 route contract、route file、checkpoint/index、visual gate、last nav result 等字段 | legacy server 仍保留 sleep demo；真实 fixed-route/Nav2 行驶、真实 SLAM/Nav2 学习到巡逻 E2E 还缺实测 |
...
| Objective 4 感知模块产品化 | 约 63% | `route_data_recorder` runtime-style callback 测试已证明 fake image/odom 回调能通过真实 `_on_image()` / `_on_odom()` 写出 route CSV、keyframe、companion JSON 和 manifest | 仍需真实路线数据集、真实 ROS2 camera/odom 采集和持续标注/人工标注闭环 |
| Objective 5 手机体验与量产边界 | 约 69% | 手机诊断链可从 task record 看到 fixed-route 送达证据，不再只知道任务成败；远程支持可复盘路线版本、checkpoint 和 visual gate 状态 | 真实手机浏览器、喇叭/TTS、量产硬件约束实物验收仍需落地 |
