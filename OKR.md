...
更新时间：2026-05-10 08:18 Asia/Shanghai。
...
| Objective 2 可恢复送垃圾任务闭环 | 约 69% | `use_saved_map=false` patrol 已从 fake learning drive 改为 waypoint proof-gated；缺少或无效学习 waypoint 证据会 abort，不再自增 `learn_count` 或继续巡逻 | legacy server 仍保留 sleep demo；真实 SLAM/Nav2 学习到巡逻的端到端运行还缺实测 |
...
| Objective 4 感知模块产品化 | 约 58% | 视觉节点已支持原图、标注图、检测 JSON、manifest 落盘；`route_data_recorder` 现在也会把学习阶段 route keyframe 写成 `trashbot.vision_samples.v1` companion JSON + manifest，`/api/diagnostics` 可复用现有 summary 消费路线关键帧证据 | 仍需真实路线数据集、真实 runtime 文件写入验证和持续标注流程 |
| Objective 5 手机体验与量产边界 | 约 67% | operator gateway 手机操作台继续保留；诊断包从“只给 manifest 路径”升级为可读视觉样本摘要，支持远程售后判断是否有摄像头证据 | 真实手机浏览器、喇叭/TTS、量产硬件约束实物验收仍需落地 |
