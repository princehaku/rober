...
更新时间：2026-05-10 19:59 Asia/Shanghai。
...
| Objective 4 感知模块产品化 | 约 70% | 17-18 visual gate proof 把视觉门控从页面状态推进为可复盘离线 evidence artifact，失败路径覆盖缺 route、缺 keyframe、缺 live frame、匹配不足和 descriptor 不可用；各包 smoke 通过：interfaces 6、hardware 14、nav 39、bringup 9、behavior 111、vision 13 tests OK | 仍需真实路线数据集、真实 ROS2 camera/odom 采集、持续标注/人工标注闭环、真实 manifest 复盘和上车验证；离线 proof 只能证明证据结构和匹配流程 |
| Objective 5 手机体验与量产边界 | 约 77% | `/api/diagnostics` 现在总是包含 `hardware_proof`，operator 页面新增 Hardware proof 诊断卡，能展示 `software_proof`、`needs_hil`、`invalid_config`、`read_error` 和下一步动作；targeted tests、py_compile、full smoke、diff check 均通过 | 真实手机浏览器截图、真实 HIL、真实 UART/IMU/电池/轮向证据、喇叭/TTS、量产硬件约束实物验收仍需落地；未配置 artifact ref 时仍会保守显示 `read_error` |
...
本轮验证：`test_route_debug_web.py` 4 OK、`py_compile route_debug_web.py` OK、`git diff --check` OK、nav 包测试 31 OK、完整 smoke 全部 OK（interfaces 6 / hardware 14 / nav 31 / bringup 9 / behavior 111 / vision 13），结果见 `sprints/2026.05.10_16-17_route-debug-status-panel/tech-done.md`。
