...
describe("workstation fail-closed API contracts", () => {
  ...
  it("workstation live-summary route exposes a flat read-only current card for field curl checks", async () => {
    ...
    try {
      ...
      expect(summary.current_goal_status).toBe(summary.goal_checklist_summary?.status);
      expect(summary.current_goal_total_count).toBe(7);
      expect(summary.current_goal_done_count).toBe(1);
      expect(summary.current_goal_remaining_count).toBe(6);
      expect(summary.current_goal_progress_plain).toBe("1/7");
      ...
      expect(summary.current_goal_sends_motion_when_clicked).toBe(false);
      ...
      expect(live.field_acceptance_packet).toEqual(summary.field_acceptance_packet);
      expect(live.field_acceptance_status).toBe(summary.field_acceptance_status);
      expect(live.field_acceptance_next_step_id).toBe(summary.field_acceptance_next_step_id);
      expect(live.field_acceptance_next_step_label).toBe(summary.field_acceptance_next_step_label);
      expect(live.field_acceptance_next_step_start_endpoint).toBe(summary.field_acceptance_next_step_start_endpoint);
      expect(live.field_acceptance_next_step_sends_motion).toBe(summary.field_acceptance_next_step_sends_motion);
      expect(live.field_acceptance_next_step_requires_safety_confirm).toBe(summary.field_acceptance_next_step_requires_safety_confirm);
      expect(live.field_acceptance_wysiwyg_missing_surface_ids).toEqual(summary.field_acceptance_wysiwyg_missing_surface_ids);
      expect(live.field_acceptance_wysiwyg_primary_refresh_endpoint).toBe("/api/robot-control/camera/first-frame/probe");
      expect(live.field_acceptance_wysiwyg_primary_refresh_label).toBe(summary.field_acceptance_wysiwyg_primary_refresh_label);
      expect(live.field_acceptance_wysiwyg_refresh_sequence).toEqual(summary.field_acceptance_wysiwyg_refresh_sequence);
      expect(live.field_acceptance_wysiwyg_refresh_sequence_labels).toEqual(summary.field_acceptance_wysiwyg_refresh_sequence_labels);
      expect(live.field_acceptance_wysiwyg_refresh_mode).toBe(summary.field_acceptance_wysiwyg_refresh_mode);
      expect(live.field_acceptance_wysiwyg_refresh_mode).toBe("all_wysiwyg");
      expect(live.field_acceptance_wysiwyg_refresh_sends_motion).toBe(false);
      expect(live.field_acceptance_wysiwyg_refresh_starts_nav2).toBe(false);
      expect(live.field_acceptance_wysiwyg_refresh_starts_manual).toBe(false);
      expect(live.field_acceptance_wysiwyg_refresh_starts_keyboard).toBe(false);
      expect(live.field_acceptance_wysiwyg_refresh_starts_free_roam).toBe(false);
      expect(live.field_acceptance_wysiwyg_refresh_starts_radar_lifecycle).toBe(false);
      expect(live.field_acceptance_wysiwyg_refresh_starts_map_runtime).toBe(false);
      ...
      expect(summary.current_mapping_action_required).toBe(true);
      expect(summary.current_mapping_action_ready).toBe(summary.mapping_start_ready);
      expect(summary.current_mapping_action_id).toBe("start_mapping_when_sensors_ready");
      expect(summary.current_mapping_action_start_endpoint).toBe(summary.mapping_start_endpoint);
      expect(summary.current_mapping_action_stop_endpoint).toBe("/api/robot-control/free-roam/autonomy/stop");
      expect(summary.current_mapping_action_preview_endpoint).toBe(summary.mapping_preview_endpoint);
      expect(summary.current_mapping_action_acceptance_endpoints).toEqual(summary.mapping_acceptance_endpoints);
      ...
      expect(summary.current_mapping_action_camera_required).toBe(true);
      expect(summary.current_mapping_action_radar_required).toBe(true);
      ...
      expect(live.map_display_direct_map_default_zoom_percent).toBe("400%");
  ...
  it("workstation proof refresh proxies only allow fixed radar, map, and Nav2 POST bodies", async () => {
    ...
    try {
      ...
      expect(mapBody.readback_only).toBe(true);
      expect(mapBody.no_motion_refresh).toBe(true);
      expect(mapBody.sends_motion_when_clicked).toBe(false);
      expect(mapBody.starts_radar_lifecycle).toBe(false);
      expect(mapBody.starts_manual).toBe(false);
      expect(mapBody.starts_keyboard).toBe(false);
      expect(mapBody.starts_free_roam).toBe(false);
      expect(mapBody.starts_map_runtime).toBe(false);
      expect(mapBody.starts_nav2).toBe(false);
      expect(mapBody.submits_delivery).toBe(false);
      expect(mapBody.stops_motion).toBe(false);
      ...
      expect(nav2Body.readback_only).toBe(true);
      expect(nav2Body.no_motion_refresh).toBe(true);
      expect(nav2Body.sends_motion_when_clicked).toBe(false);
      expect(nav2Body.starts_radar_lifecycle).toBe(false);
      expect(nav2Body.starts_nav2).toBe(false);
      expect(nav2Body.starts_manual).toBe(false);
      expect(nav2Body.starts_keyboard).toBe(false);
      expect(nav2Body.starts_free_roam).toBe(false);
      expect(nav2Body.starts_map_runtime).toBe(false);
      expect(nav2Body.submits_delivery).toBe(false);
      expect(nav2Body.stops_motion).toBe(false);
  ...
  it("workstation map lifecycle proxies use fixed endpoints and whitelist short request body fields", async () => {
    ...
    try {
      ...
      const previewBody = (await previewResponse.json()) as {
        ...
        safe_to_control: boolean;
        readback_only: boolean;
        map_preview_readback_only: boolean;
        no_motion_refresh: boolean;
        sends_motion_when_clicked: boolean;
        starts_radar_lifecycle: boolean;
        starts_nav2: boolean;
      ...
      expect(previewBody.map_preview_readback_only).toBe(true);
      expect(previewBody.no_motion_refresh).toBe(true);
      expect(previewBody.sends_motion_when_clicked).toBe(false);
      expect(previewBody.starts_radar_lifecycle).toBe(false);
      expect(previewBody.starts_nav2).toBe(false);
      expect(previewBody.starts_manual).toBe(false);
      expect(previewBody.starts_keyboard).toBe(false);
      expect(previewBody.starts_free_roam).toBe(false);
      expect(previewBody.starts_map_runtime).toBe(false);
      expect(previewBody.submits_delivery).toBe(false);
      expect(previewBody.stops_motion).toBe(false);
  ...
  it("Nav2 latest execution proxy reads fixed GET artifact without replaying navigation", async () => {
    ...
    try {
      ...
      const body = (await response.json()) as {
        ...
        hard_dangerous_true_fields: string[];
        readback_only: boolean;
        nav2_goal_execution_latest_readback_only: boolean;
        sends_motion_when_clicked: boolean;
      ...
      expect(body.readback_only).toBe(true);
      expect(body.nav2_goal_execution_latest_readback_only).toBe(true);
      expect(body.sends_motion_when_clicked).toBe(false);
  ...
  it("free-roam autonomy latest proxy reads fixed runtime artifact without starting autonomy", async () => {
    ...
    try {
      ...
      const body = (await response.json()) as {
        ...
        hard_dangerous_true_fields: string[];
        readback_only: boolean;
        free_roam_latest_readback_only: boolean;
        sends_motion_when_clicked: boolean;
        starts_camera_exclusive_capture: boolean;
        starts_radar_lifecycle: boolean;
        starts_nav2: boolean;
      ...
      expect(body.hard_dangerous_true_fields).toEqual([]);
      expect(body.readback_only).toBe(true);
      expect(body.free_roam_latest_readback_only).toBe(true);
      expect(body.sends_motion_when_clicked).toBe(false);
      expect(body.starts_camera_exclusive_capture).toBe(false);
      expect(body.starts_radar_lifecycle).toBe(false);
      expect(body.starts_nav2).toBe(false);
  ...
  it("workstation camera MJPEG proxy forwards only fixed readonly multipart stream", async () => {
    ...
    try {
      ...
      expect(statusBody.shared_preview_starts_camera_exclusive_capture).toBe(false);
      expect(statusBody.shared_preview_sends_motion).toBe(false);
      expect(statusBody.shared_preview_starts_nav2).toBe(false);
      expect(statusBody.shared_preview_starts_manual).toBe(false);
      expect(statusBody.shared_preview_starts_keyboard).toBe(false);
      expect(statusBody.shared_preview_starts_free_roam).toBe(false);
      expect(statusBody.shared_preview_starts_map_runtime).toBe(false);
      expect(statusBody.shared_preview_submits_delivery).toBe(false);
      expect(statusBody.shared_preview_stops_motion).toBe(false);
  ...
  it("workstation camera MJPEG status translates full-speed USB diagnosis", async () => {
    ...
    try {
      ...
      expect(statusBody.usb_speed).toBe("12M");
      expect(statusBody.usb_full_speed_detected).toBe(true);
      expect(statusBody.hardware_action_required).toBe(true);
      expect(statusBody.hardware_action_label).toBe("换高速USB后复测");
  ...
  it("workstation camera first-frame probe reports configured proxy timeout without losing robot base url", async () => {
    ...
    try {
      ...
      expect(quickBody.failure_reason).toBe("fetch_timeout_12000ms");
      expect(quickBody.blocked_reasons).toEqual(["fetch_timeout_12000ms"]);
  ...
  it("workstation base feedback samples GET returns latest wheel raw JSON without sampling POST", async () => {
    ...
    try {
      ...
      expect(body.submits_delivery).toBe(false);
      expect(body.stops_motion).toBe(false);
      expect(body.next_action_plain).toBe("轮速样本只读完成；同窗口 wheel L/R 非零证据已存在。");
      expect(body.sends_motion_commands).toBe(false);
      expect(body.robot_control_executed).toBe(false);
