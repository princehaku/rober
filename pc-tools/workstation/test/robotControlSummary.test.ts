...
describe("robotControlSummary", () => {
  ...
  it("exposes minimal precheck fields for same-window wheel rerun", async () => {
    ...
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("默认 300% 细节大图");
    ...
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("PC 首页默认用 300% 细节大图显示真实地图，地图画布先吃满首屏；要看全局就点“适配”，仍觉得小就点“进入地图大屏”");
