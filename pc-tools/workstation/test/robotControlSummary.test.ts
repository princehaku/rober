...
describe("robotControlSummary", () => {
  ...
  it("exposes minimal precheck fields for same-window wheel rerun", async () => {
    ...
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("首页 PC 大地图默认 200% 大图");
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("进入 /map 也是 200% 大图");
    ...
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("/map 默认也是 200% 大图");
