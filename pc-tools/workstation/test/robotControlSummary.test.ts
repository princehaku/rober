...
describe("robotControlSummary", () => {
  ...
  it("exposes minimal precheck fields for same-window wheel rerun", async () => {
    ...
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("首页 PC 大地图默认 400% 现场细节视角");
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("进入 /map 使用 400% 细节大屏");
