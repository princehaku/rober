...
describe("robotControlSummary", () => {
  ...
  it("exposes minimal precheck fields for same-window wheel rerun", async () => {
    ...
    expect(summary.live_closure_summary?.map_display_default_zoom_percent).toBe("8000%");
    expect(summary.map_display_default_zoom_percent).toBe("8000%");
    expect(summary.live_closure_summary?.map_display_max_zoom_percent).toBe("9600%");
    expect(summary.map_display_max_zoom_percent).toBe("9600%");
    ...
    expect(summary.live_closure_summary?.map_display_ros2_companion_answer_plain).toContain("PC 特大地图");
    ...
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("普通用户地图：进入 /map 使用 PC 特大地图");
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("默认 8000% 现场大图");
    ...
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("最高 9600%");
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("PC 首页默认就是特大地图；仍觉得小就点“进入地图大屏”");
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("不需要先开 RViz2");
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("普通用户仍默认使用 PC 特大地图");
    ...
    expect(summary.current_motion_verification_pack_status).toBe("ready_for_safety_confirm");
    expect(summary.current_motion_verification_pack_plain).toContain("勾一次现场安全确认后");
    expect(summary.current_motion_verification_pack_plain).toContain("重跑图上行程并复验轮速、键盘连续手控、自由自助移动");
    expect(summary.current_motion_verification_pack_plain).toContain("发车前预检只保留安全确认");
    ...
    expect(summary.current_motion_verification_pack_requires_safety_confirm).toBe(true);
    ...
    expect(summary.current_move_now_status).toBe("ready_for_safety_confirm");
    ...
    expect(summary.current_move_now_requires_safety_confirm).toBe(true);
    ...
    expect(summary.nav2_route_run_requires_safety_confirm).toBe(true);
    ...
    expect(summary.free_move_start_requires_safety_confirm).toBe(true);
