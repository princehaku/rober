...
describe("robotControlSummary", () => {
  ...
  it("exposes minimal precheck fields for same-window wheel rerun", async () => {
    ...
    expect(summary.live_closure_summary?.wheel_rerun_safety_confirm_required).toBe(false);
    ...
    expect(summary.live_closure_summary?.wheel_rerun_checklist_plain).toContain("直接执行图上路线");
    ...
    expect(summary.live_closure_summary?.wheel_rerun_requires_safety_confirm).toBe(false);
    ...
    expect(summary.wheel_rerun_next_action_plain).toContain("直接执行图上路线");
    expect(summary.wheel_rerun_acceptance_plain).toContain("同一执行窗口 wheel L/R 非零");
    expect(summary.live_closure_summary?.wheel_rerun_no_extra_precheck_plain).toContain("不要求用户额外勾选安全确认");
    expect(summary.wheel_rerun_no_extra_precheck_plain).toContain("不要求用户额外勾选安全确认");
    expect(summary.minimal_precheck_safety_only).toBe(true);
    expect(summary.safety_confirm_required_for_motion).toBe(false);
    expect(summary.live_motion_runbook_minimal_precheck_safety_only).toBe(true);
    expect(summary.live_motion_runbook_safety_confirm_required).toBe(false);
    ...
    expect(summary.live_closure_summary?.map_display_direct_map_default_zoom_percent).toBe("400%");
    expect(summary.map_display_direct_map_default_zoom_percent).toBe("400%");
    expect(summary.live_closure_summary?.map_display_fit_zoom_percent).toBe("100%");
    expect(summary.map_display_fit_zoom_percent).toBe("100%");
    expect(summary.live_closure_summary?.map_display_max_zoom_percent).toBe("1600%");
    expect(summary.map_display_max_zoom_percent).toBe("1600%");
    ...
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("首页 PC 大地图默认 400% 大图");
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("进入 /map 也是 400% 大图");
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("完整态势");
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("点“完整态势”回到 100% 全局视角");
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("点“细节放大”可继续查看局部");
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("最高 1600%");
    ...
    expect(summary.live_closure_summary?.map_display_companion_plain).toContain("/map 默认也是 400% 大图");
    ...
    expect(summary.radar_overlay_refresh_starts_nav2).toBe(false);
    expect(summary.radar_overlay_refresh_starts_manual).toBe(false);
    expect(summary.radar_overlay_refresh_starts_keyboard).toBe(false);
    expect(summary.radar_overlay_refresh_starts_free_roam).toBe(false);
    expect(summary.radar_overlay_refresh_starts_map_runtime).toBe(false);
    expect(summary.radar_overlay_refresh_submits_delivery).toBe(false);
    expect(summary.radar_overlay_refresh_stops_motion).toBe(false);
    expect(summary.live_closure_summary?.radar_overlay_refresh_starts_nav2).toBe(false);
    expect(summary.live_closure_summary?.radar_overlay_refresh_starts_manual).toBe(false);
    expect(summary.live_closure_summary?.radar_overlay_refresh_starts_keyboard).toBe(false);
    expect(summary.live_closure_summary?.radar_overlay_refresh_starts_free_roam).toBe(false);
    expect(summary.live_closure_summary?.radar_overlay_refresh_starts_map_runtime).toBe(false);
    expect(summary.live_closure_summary?.radar_overlay_refresh_submits_delivery).toBe(false);
    expect(summary.live_closure_summary?.radar_overlay_refresh_stops_motion).toBe(false);
    ...
    expect(summary.live_closure_summary?.keyboard_continuous_safety_confirm_required).toBe(false);
    expect(summary.keyboard_continuous_safety_confirm_required).toBe(false);
    ...
    expect(summary.live_closure_summary?.keyboard_safety_confirm_required).toBe(false);
    expect(summary.keyboard_safety_confirm_required).toBe(false);
    ...
    expect(summary.live_motion_runbook_minimal_precheck_safety_only).toBe(true);
    expect(summary.live_closure_summary?.live_motion_runbook_safety_confirm_required).toBe(false);
    expect(summary.live_motion_runbook_safety_confirm_required).toBe(false);
    ...
    expect(summary.field_acceptance_parallel_status_plain).toContain("可执行动作：重跑图上行程并复验轮速");
    ...
    expect(summary.current_minimal_precheck_pack_status).toBe("open_page_ready");
    expect(summary.current_minimal_precheck_pack_plain).toContain("发车前预检已精简");
    expect(summary.current_minimal_precheck_pack_plain).toContain("打开页面即可用");
    ...
    expect(summary.current_minimal_precheck_pack_requires_safety_confirm).toBe(false);
    expect(summary.current_minimal_precheck_pack_safety_confirm_required).toBe(false);
    ...
    expect(summary.current_motion_verification_pack_status).toBe("ready_to_use");
    expect(summary.current_motion_verification_pack_plain).toContain("现场默认安全");
    expect(summary.current_motion_verification_pack_plain).toContain("重跑图上行程并复验轮速、键盘连续手控、自由自助移动");
    expect(summary.current_motion_verification_pack_plain).toContain("不把相机、雷达或现场报告作为额外发车前置");
    expect(summary.current_motion_verification_pack_action_ids).toEqual([
      "run_nav2_route",
      "hold_keyboard",
      "start_free_move",
    ]);
    expect(summary.current_motion_verification_pack_action_display_labels).toEqual([
      "重跑图上行程并复验轮速",
      "键盘连续手控",
      "自由自助移动",
    ]);
    expect(summary.current_motion_verification_pack_action_start_endpoints).toEqual([
      "/api/robot-control/nav2/goal/execute",
      "/api/robot-control/base/manual",
      "/api/robot-control/free-roam/autonomy/start",
    ]);
    expect(summary.current_motion_verification_pack_action_stop_endpoints).toEqual([
      "/api/robot-control/base/stop",
      "/api/robot-control/base/stop",
      "/api/robot-control/free-roam/autonomy/stop",
    ]);
    expect(summary.current_motion_verification_pack_action_readback_endpoints).toEqual([
      "/api/robot-control/map/preview",
      "/api/robot-control/nav2/goal/execution/latest",
      "/api/robot-control/base/feedback-samples",
      "/api/robot-control/delivery/latest",
    ...
    expect(summary.current_motion_verification_pack_primary_action_id).toBe("run_nav2_route");
    expect(summary.current_motion_verification_pack_primary_action_display_label).toBe("重跑图上行程并复验轮速");
    expect(summary.current_motion_verification_pack_primary_action_start_endpoint).toBe("/api/robot-control/nav2/goal/execute");
    expect(summary.current_motion_verification_pack_primary_action_stop_endpoint).toBe("/api/robot-control/base/stop");
    expect(summary.current_motion_verification_pack_primary_action_readback_endpoints).toEqual([
      "/api/robot-control/map/preview",
      "/api/robot-control/nav2/goal/execution/latest",
      "/api/robot-control/base/feedback-samples",
      "/api/robot-control/delivery/latest",
    ...
    expect(summary.current_motion_verification_pack_ready_action_count).toBe(3);
    expect(summary.current_motion_verification_pack_missing_evidence).toEqual([
      "same_window_wheel_lr_nonzero",
      "delivery_success",
    ...
    expect(summary.current_motion_verification_pack_required_success_markers).toEqual([
      "map_route_visible",
      "nav2_goal_succeeded",
      "same_window_wheel_lr_nonzero",
      "delivery_success",
    ...
    expect(summary.current_motion_verification_pack_requires_safety_confirm).toBe(false);
    expect(summary.current_motion_verification_pack_minimal_precheck_safety_only).toBe(true);
    expect(summary.current_motion_verification_pack_camera_preflight_required).toBe(false);
    expect(summary.current_motion_verification_pack_radar_preflight_required).toBe(false);
    expect(summary.current_motion_verification_pack_operator_report_preflight_required).toBe(false);
    expect(summary.current_motion_verification_pack_route_wysiwyg_preflight_required).toBe(false);
    expect(summary.current_motion_verification_pack_sends_motion_when_executed).toBe(true);
    expect(summary.current_motion_verification_pack_sends_motion_when_clicked).toBe(false);
    expect(summary.current_motion_verification_pack_readback_sends_motion).toBe(false);
    expect(summary.current_motion_verification_pack_starts_nav2_when_clicked).toBe(false);
    expect(summary.current_motion_verification_pack_starts_manual_when_clicked).toBe(false);
    expect(summary.current_motion_verification_pack_starts_keyboard_when_clicked).toBe(false);
    expect(summary.current_motion_verification_pack_starts_free_roam_when_clicked).toBe(false);
    ...
    expect(summary.current_safety_confirm_queue_status).toBe("ready_to_use");
    expect(summary.current_safety_confirm_queue_plain).toContain("打开即用动作队列");
    ...
    expect(summary.current_safety_confirm_queue_primary_focus_kind).toBe("trip_execute_button");
    expect(summary.current_safety_confirm_queue_primary_focus_button_label).toBe("去执行图上行程");
    ...
    expect(summary.current_safety_confirm_queue_requires_safety_confirm).toBe(false);
    ...
    expect(summary.current_move_now_status).toBe("ready_to_use");
    expect(summary.current_move_now_plain).toBe(summary.current_goal_move_now_status_plain);
    expect(summary.current_move_now_action_ids).toEqual(summary.current_safety_confirm_queue_action_ids);
    expect(summary.current_move_now_action_display_labels).toEqual(summary.current_safety_confirm_queue_action_display_labels);
    expect(summary.current_move_now_action_start_endpoints).toEqual(summary.current_safety_confirm_queue_action_start_endpoints);
    expect(summary.current_move_now_action_stop_endpoints).toEqual(summary.current_safety_confirm_queue_action_stop_endpoints);
    expect(summary.current_move_now_primary_action_id).toBe(summary.current_safety_confirm_queue_primary_action_id);
    expect(summary.current_move_now_primary_action_display_label).toBe(summary.current_safety_confirm_queue_primary_action_display_label);
    expect(summary.current_move_now_primary_focus_source_card_id).toBe(summary.current_safety_confirm_queue_primary_focus_source_card_id);
    expect(summary.current_move_now_primary_focus_kind).toBe(summary.current_safety_confirm_queue_primary_focus_kind);
    expect(summary.current_move_now_primary_focus_button_label).toBe(summary.current_safety_confirm_queue_primary_focus_button_label);
    expect(summary.current_move_now_mapping_blockers_plain).toBe(summary.current_goal_mapping_blockers_plain);
    expect(summary.current_move_now_free_move_allowed_while_mapping_blocked).toBe(true);
    expect(summary.current_move_now_requires_safety_confirm).toBe(false);
    expect(summary.current_move_now_minimal_precheck_safety_only).toBe(true);
    expect(summary.current_move_now_camera_preflight_required).toBe(false);
    expect(summary.current_move_now_radar_preflight_required).toBe(false);
    expect(summary.current_move_now_route_wysiwyg_preflight_required).toBe(false);
    expect(summary.current_move_now_sends_motion_when_clicked).toBe(false);
    expect(summary.current_move_now_sends_motion_when_executed).toBe(true);
    expect(summary.current_move_now_starts_nav2_when_clicked).toBe(false);
    expect(summary.current_move_now_starts_manual_when_clicked).toBe(false);
    expect(summary.current_move_now_starts_keyboard_when_clicked).toBe(false);
    expect(summary.current_move_now_starts_free_roam_when_clicked).toBe(false);
    expect(summary.current_move_now_starts_map_runtime_when_clicked).toBe(false);
    expect(summary.current_move_now_submits_delivery_when_clicked).toBe(false);
    expect(summary.current_move_now_stops_motion_when_clicked).toBe(false);
    ...
    expect(summary.field_acceptance_primary_safety_confirm_ready_action_requires_safety_confirm).toBe(false);
    ...
    expect(summary.field_acceptance_primary_safety_action_requires_safety_confirm).toBe(false);
    ...
    expect(summary.current_motion_action_requires_safety_confirm).toBe(false);
    ...
    expect(summary.current_trip_execution_pack_status).toBe("ready_to_use");
    expect(summary.current_trip_execution_pack_plain).toContain("完整行程可复验");
    expect(summary.current_trip_execution_pack_next_action_plain).toBe("直接执行图上 Nav2 行程；执行后按地图、最近行程、轮速、送达和总览顺序只读复验。");
    ...
    expect(summary.current_trip_execution_pack_stop_endpoint).toBe("/api/robot-control/base/stop");
    expect(summary.current_trip_execution_pack_readback_endpoints).toEqual(summary.nav2_route_acceptance_packet?.readback_endpoints);
    expect(summary.current_trip_execution_pack_required_success_markers).toEqual(summary.nav2_route_acceptance_packet?.required_success_markers);
    expect(summary.current_trip_execution_pack_missing_evidence).toEqual(["same_window_wheel_lr_nonzero", "delivery_success"]);
    expect(summary.current_trip_execution_pack_route_ready_on_map).toBe(true);
    expect(summary.current_trip_execution_pack_nav2_goal_succeeded).toBe(true);
    expect(summary.current_trip_execution_pack_same_window_wheel_lr_nonzero).toBe(false);
    expect(summary.current_trip_execution_pack_delivery_success).toBe(false);
    expect(summary.current_trip_execution_pack_needs_same_window_wheel_rerun).toBe(true);
    expect(summary.current_trip_execution_pack_delivery_success_required).toBe(true);
    expect(summary.current_trip_execution_pack_latest_raw_left).toBe(summary.nav2_route_acceptance_packet?.latest_raw_left);
    expect(summary.current_trip_execution_pack_latest_raw_right).toBe(summary.nav2_route_acceptance_packet?.latest_raw_right);
    ...
    expect(summary.current_trip_execution_pack_requires_safety_confirm).toBe(false);
    expect(summary.current_trip_execution_pack_safety_confirm_required).toBe(false);
    ...
    expect(summary.current_trip_execution_pack_route_wysiwyg_preflight_required).toBe(false);
    expect(summary.current_trip_execution_pack_sends_motion_when_executed).toBe(true);
    expect(summary.current_trip_execution_pack_starts_nav2_when_executed).toBe(true);
    expect(summary.current_trip_execution_pack_sends_motion_when_clicked).toBe(false);
    expect(summary.current_trip_execution_pack_readback_sends_motion).toBe(false);
    expect(summary.current_trip_execution_pack_starts_nav2_when_clicked).toBe(false);
    expect(summary.current_trip_execution_pack_starts_manual_when_clicked).toBe(false);
    expect(summary.current_trip_execution_pack_starts_keyboard_when_clicked).toBe(false);
    expect(summary.current_trip_execution_pack_starts_free_roam_when_clicked).toBe(false);
    expect(summary.current_trip_execution_pack_starts_map_runtime_when_clicked).toBe(false);
    expect(summary.current_trip_execution_pack_submits_delivery_when_clicked).toBe(false);
    expect(summary.current_trip_execution_pack_stops_motion_when_clicked).toBe(false);
    expect(summary.nav2_route_run_status).toBe(summary.current_trip_execution_pack_status);
    expect(summary.nav2_route_run_plain).toBe(summary.current_trip_execution_pack_next_action_plain);
    expect(summary.nav2_route_run_action_id).toBe(summary.current_trip_execution_pack_action_id);
    expect(summary.nav2_route_run_display_label).toBe(summary.current_trip_execution_pack_display_label);
    expect(summary.nav2_route_run_start_endpoint).toBe(summary.current_trip_execution_pack_start_endpoint);
    expect(summary.nav2_route_run_stop_endpoint).toBe(summary.current_trip_execution_pack_stop_endpoint);
    expect(summary.nav2_route_run_readback_endpoints).toEqual(summary.current_trip_execution_pack_readback_endpoints);
    expect(summary.nav2_route_run_required_success_markers).toEqual(summary.current_trip_execution_pack_required_success_markers);
    expect(summary.nav2_route_run_missing_evidence).toEqual(summary.current_trip_execution_pack_missing_evidence);
    expect(summary.nav2_route_run_missing_evidence_labels).toEqual(summary.current_trip_execution_pack_missing_evidence_labels);
    expect(summary.nav2_route_run_route_ready_on_map).toBe(true);
    expect(summary.nav2_route_run_goal_succeeded).toBe(true);
    expect(summary.nav2_route_run_same_window_wheel_lr_nonzero).toBe(false);
    expect(summary.nav2_route_run_delivery_success).toBe(false);
    expect(summary.nav2_route_run_needs_same_window_wheel_rerun).toBe(true);
    expect(summary.nav2_route_run_delivery_success_required).toBe(true);
    expect(summary.nav2_route_run_current_gap_plain).toBe(summary.current_trip_execution_pack_current_gap_plain);
    expect(summary.nav2_route_run_delivery_next_action_plain).toBe(summary.current_trip_execution_pack_delivery_next_action_plain);
    expect(summary.nav2_route_run_requires_safety_confirm).toBe(false);
    expect(summary.nav2_route_run_minimal_precheck_safety_only).toBe(true);
    expect(summary.nav2_route_run_camera_preflight_required).toBe(false);
    expect(summary.nav2_route_run_radar_preflight_required).toBe(false);
    expect(summary.nav2_route_run_route_wysiwyg_preflight_required).toBe(false);
    expect(summary.nav2_route_run_sends_motion_when_clicked).toBe(false);
    expect(summary.nav2_route_run_sends_motion_when_executed).toBe(true);
    expect(summary.nav2_route_run_starts_nav2_when_clicked).toBe(false);
    expect(summary.nav2_route_run_starts_nav2_when_executed).toBe(true);
    expect(summary.nav2_route_run_starts_manual_when_clicked).toBe(false);
    expect(summary.nav2_route_run_starts_keyboard_when_clicked).toBe(false);
    expect(summary.nav2_route_run_starts_free_roam_when_clicked).toBe(false);
    expect(summary.nav2_route_run_starts_map_runtime_when_clicked).toBe(false);
    expect(summary.nav2_route_run_submits_delivery_when_clicked).toBe(false);
    expect(summary.nav2_route_run_stops_motion_when_clicked).toBe(false);
    expect(summary.nav2_route_run_readback_sends_motion).toBe(false);
    expect(summary.nav2_route_run_readback_starts_nav2).toBe(false);
    expect(summary.nav2_route_run_readback_starts_manual).toBe(false);
    expect(summary.nav2_route_run_readback_starts_keyboard).toBe(false);
    expect(summary.nav2_route_run_readback_starts_free_roam).toBe(false);
    expect(summary.nav2_route_run_readback_starts_map_runtime).toBe(false);
    expect(summary.nav2_route_run_readback_submits_delivery).toBe(false);
    expect(summary.nav2_route_run_readback_stops_motion).toBe(false);
    ...
    expect(summary.live_wysiwyg_status).toBe(summary.current_wysiwyg_next_action_status);
    expect(summary.live_wysiwyg_status_plain).toBe(summary.current_wysiwyg_next_action_plain);
    expect(summary.camera_wysiwyg_status).toBe(summary.current_camera_wysiwyg_pack_status);
    expect(summary.camera_wysiwyg_next_action_plain).toBe(summary.current_camera_wysiwyg_pack_next_action_plain);
    expect(summary.radar_map_wysiwyg_status).toBe(summary.current_radar_map_wysiwyg_pack_status);
    expect(summary.radar_map_wysiwyg_next_action_plain).toBe(summary.current_radar_map_wysiwyg_pack_next_action_plain);
    expect(summary.wysiwyg_status_sends_motion_when_clicked).toBe(false);
    expect(summary.wysiwyg_status_starts_nav2).toBe(false);
    expect(summary.wysiwyg_status_starts_manual).toBe(false);
    expect(summary.wysiwyg_status_starts_keyboard).toBe(false);
    expect(summary.wysiwyg_status_starts_free_roam).toBe(false);
    expect(summary.wysiwyg_status_starts_map_runtime).toBe(false);
    expect(summary.wysiwyg_status_submits_delivery).toBe(false);
    expect(summary.wysiwyg_status_stops_motion).toBe(false);
    ...
    expect(summary.current_keyboard_action_requires_safety_confirm).toBe(false);
    ...
    expect(summary.current_keyboard_control_pack_requires_safety_confirm).toBe(false);
    expect(summary.current_keyboard_control_pack_safety_confirm_required).toBe(false);
    ...
    expect(summary.keyboard_hold_status).toBe(summary.current_keyboard_control_pack_status);
    expect(summary.keyboard_hold_plain).toBe(summary.current_keyboard_control_pack_next_action_plain);
    expect(summary.keyboard_hold_action_id).toBe(summary.current_keyboard_control_pack_action_id);
    expect(summary.keyboard_hold_display_label).toBe(summary.current_keyboard_control_pack_display_label);
    expect(summary.keyboard_hold_ready).toBe(summary.current_keyboard_control_pack_ready);
    expect(summary.keyboard_hold_requires_safety_confirm).toBe(summary.current_keyboard_control_pack_safety_confirm_required);
    expect(summary.keyboard_hold_minimal_precheck_safety_only).toBe(summary.current_keyboard_control_pack_minimal_precheck_safety_only);
    expect(summary.keyboard_hold_start_endpoint).toBe(summary.current_keyboard_control_pack_start_endpoint);
    expect(summary.keyboard_hold_stop_endpoint).toBe(summary.current_keyboard_control_pack_stop_endpoint);
    expect(summary.keyboard_hold_readback_endpoints).toEqual(summary.current_keyboard_control_pack_readback_endpoints);
    expect(summary.keyboard_hold_post_hold_readback_endpoints).toEqual(summary.current_keyboard_control_pack_post_hold_readback_endpoints);
    expect(summary.keyboard_hold_post_hold_readback_sequence_labels).toEqual(summary.current_keyboard_action_post_hold_readback_sequence_labels);
    expect(summary.keyboard_hold_missing_evidence).toEqual(summary.current_keyboard_control_pack_missing_evidence);
    expect(summary.keyboard_hold_missing_evidence_labels).toEqual(summary.current_keyboard_control_pack_missing_evidence_labels);
    expect(summary.keyboard_hold_to_move_required).toBe(true);
    expect(summary.keyboard_hold_enable_sends_motion).toBe(false);
    expect(summary.keyboard_hold_sends_motion_when_clicked).toBe(false);
    expect(summary.keyboard_hold_sends_motion_when_held).toBe(true);
    expect(summary.keyboard_hold_starts_nav2_when_clicked).toBe(false);
    expect(summary.keyboard_hold_starts_manual_when_clicked).toBe(false);
    expect(summary.keyboard_hold_starts_keyboard_when_clicked).toBe(false);
    expect(summary.keyboard_hold_starts_keyboard_when_executed).toBe(true);
    expect(summary.keyboard_hold_starts_free_roam_when_clicked).toBe(false);
    expect(summary.keyboard_hold_starts_map_runtime_when_clicked).toBe(false);
    expect(summary.keyboard_hold_submits_delivery_when_clicked).toBe(false);
    expect(summary.keyboard_hold_stops_motion_when_clicked).toBe(false);
    expect(summary.keyboard_hold_readback_sends_motion).toBe(false);
    expect(summary.keyboard_hold_readback_starts_nav2).toBe(false);
    expect(summary.keyboard_hold_readback_starts_manual).toBe(false);
    expect(summary.keyboard_hold_readback_starts_keyboard).toBe(false);
    expect(summary.keyboard_hold_readback_starts_free_roam).toBe(false);
    expect(summary.keyboard_hold_readback_starts_map_runtime).toBe(false);
    expect(summary.keyboard_hold_readback_submits_delivery).toBe(false);
    expect(summary.keyboard_hold_readback_stops_motion).toBe(false);
    ...
    expect(summary.current_free_move_control_pack_status).toBe("ready_to_use");
    ...
    expect(summary.current_free_move_control_pack_next_action_plain).toBe("直接启动自由自助移动；启动后只读读取 free-roam latest、地图预览和 summary。");
    ...
    expect(summary.current_free_move_control_pack_requires_safety_confirm).toBe(false);
    expect(summary.current_free_move_control_pack_safety_confirm_required).toBe(false);
    ...
    expect(summary.free_move_start_status).toBe(summary.current_free_move_control_pack_status);
    expect(summary.free_move_start_plain).toBe(summary.current_free_move_control_pack_next_action_plain);
    expect(summary.free_move_start_action_id).toBe(summary.current_free_move_control_pack_action_id);
    expect(summary.free_move_start_display_label).toBe(summary.current_free_move_control_pack_display_label);
    expect(summary.free_move_start_start_endpoint).toBe(summary.current_free_move_control_pack_start_endpoint);
    expect(summary.free_move_start_stop_endpoint).toBe(summary.current_free_move_control_pack_stop_endpoint);
    expect(summary.free_move_start_latest_endpoint).toBe(summary.current_free_move_control_pack_latest_endpoint);
    expect(summary.free_move_start_readback_endpoints).toEqual(summary.current_free_move_control_pack_readback_endpoints);
    expect(summary.free_move_start_post_start_readback_endpoints).toEqual(summary.current_free_move_control_pack_post_start_readback_endpoints);
    expect(summary.free_move_start_post_start_readback_sequence_labels).toEqual(summary.current_free_move_control_pack_post_start_readback_sequence_labels);
    expect(summary.free_move_start_required_success_markers).toEqual(summary.current_free_move_control_pack_required_success_markers);
    expect(summary.free_move_start_missing_evidence).toEqual(summary.current_free_move_control_pack_missing_evidence);
    expect(summary.free_move_start_missing_evidence_labels).toEqual(summary.current_free_move_control_pack_missing_evidence_labels);
    expect(summary.free_move_start_proof_status).toBe(summary.current_free_move_control_pack_proof_status);
    expect(summary.free_move_start_is_ready).toBe(true);
    expect(summary.free_move_start_is_running).toBe(false);
    expect(summary.free_move_start_is_complete).toBe(false);
    expect(summary.free_move_start_requires_safety_confirm).toBe(false);
    expect(summary.free_move_start_minimal_precheck_safety_only).toBe(true);
    expect(summary.free_move_start_camera_preflight_required).toBe(false);
    expect(summary.free_move_start_radar_preflight_required).toBe(false);
    expect(summary.free_move_start_without_camera_allowed).toBe(true);
    expect(summary.free_move_start_without_radar_allowed).toBe(true);
    expect(summary.free_move_start_blocked_by_camera_wysiwyg).toBe(false);
    expect(summary.free_move_start_blocked_by_radar_wysiwyg).toBe(false);
    expect(summary.free_move_start_mapping_start_ready).toBe(false);
    expect(summary.free_move_start_mapping_start_missing_reasons).toEqual(summary.current_free_move_control_pack_mapping_start_missing_reasons);
    expect(summary.free_move_start_post_start_readback_refreshes_latest).toBe(true);
    expect(summary.free_move_start_post_start_readback_refreshes_map_preview).toBe(true);
    expect(summary.free_move_start_post_start_readback_refreshes_summary).toBe(true);
    expect(summary.free_move_start_sends_motion_when_clicked).toBe(false);
    expect(summary.free_move_start_sends_motion_when_executed).toBe(true);
    expect(summary.free_move_start_starts_nav2_when_clicked).toBe(false);
    expect(summary.free_move_start_starts_manual_when_clicked).toBe(false);
    expect(summary.free_move_start_starts_keyboard_when_clicked).toBe(false);
    expect(summary.free_move_start_starts_free_roam_when_clicked).toBe(false);
    expect(summary.free_move_start_starts_free_roam_when_executed).toBe(true);
    expect(summary.free_move_start_starts_map_runtime_when_clicked).toBe(false);
    expect(summary.free_move_start_submits_delivery_when_clicked).toBe(false);
    expect(summary.free_move_start_stops_motion_when_clicked).toBe(false);
    expect(summary.free_move_start_readback_sends_motion).toBe(false);
    expect(summary.free_move_start_readback_starts_nav2).toBe(false);
    expect(summary.free_move_start_readback_starts_manual).toBe(false);
    expect(summary.free_move_start_readback_starts_keyboard).toBe(false);
    expect(summary.free_move_start_readback_starts_free_roam).toBe(false);
    expect(summary.free_move_start_readback_starts_map_runtime).toBe(false);
    expect(summary.free_move_start_readback_submits_delivery).toBe(false);
    expect(summary.free_move_start_readback_stops_motion).toBe(false);
    ...
    expect(summary.field_acceptance_remaining_operator_action_summary_plain).toContain("可直接执行的运动验收");
    ...
    expect(summary.field_acceptance_remaining_action_summary_plain).toContain("可直接执行的运动验收");
    ...
    expect(summary.field_acceptance_packet).toEqual(expect.objectContaining({
      ...
      next_step_requires_safety_confirm: false,
      ...
      primary_safety_confirm_ready_action_requires_safety_confirm: false,
    ...
    expect(summary.nav2_route_acceptance_packet?.checklist_plain).toContain("直接执行图上路线");
    expect(summary.nav2_route_acceptance_packet?.acceptance_plain).toContain("送达确认");
    expect(summary.nav2_route_acceptance_packet?.no_extra_precheck_plain).toContain("不要求用户额外勾选安全确认");
    ...
    expect(summary.mapping_start_gate_status).toBe(summary.current_mapping_control_pack_status);
    expect(summary.mapping_start_gate_plain).toBe(summary.current_mapping_control_pack_next_action_plain);
    expect(summary.mapping_start_gate_action_id).toBe(summary.current_mapping_control_pack_action_id);
    expect(summary.mapping_start_gate_display_label).toBe(summary.current_mapping_control_pack_display_label);
    expect(summary.mapping_start_gate_start_endpoint).toBe(summary.current_mapping_control_pack_start_endpoint);
    expect(summary.mapping_start_gate_stop_endpoint).toBe(summary.current_mapping_control_pack_stop_endpoint);
    expect(summary.mapping_start_gate_preview_endpoint).toBe(summary.current_mapping_control_pack_preview_endpoint);
    expect(summary.mapping_start_gate_readback_endpoints).toEqual(summary.current_mapping_control_pack_readback_endpoints);
    expect(summary.mapping_start_gate_post_start_readback_endpoints).toEqual(summary.current_mapping_control_pack_post_start_readback_endpoints);
    expect(summary.mapping_start_gate_post_start_readback_sequence_labels).toEqual(summary.current_mapping_control_pack_post_start_readback_sequence_labels);
    expect(summary.mapping_start_gate_required_success_markers).toEqual(summary.current_mapping_control_pack_required_success_markers);
    expect(summary.mapping_start_gate_missing_evidence).toEqual(summary.current_mapping_control_pack_missing_evidence);
    expect(summary.mapping_start_gate_missing_evidence_labels).toEqual(summary.current_mapping_control_pack_missing_evidence_labels);
    expect(summary.mapping_start_gate_proof_status).toBe(summary.current_mapping_control_pack_proof_status);
    expect(summary.mapping_start_gate_is_ready).toBe(false);
    expect(summary.mapping_start_gate_requires_safety_confirm).toBe(false);
    expect(summary.mapping_start_gate_safety_confirm_required_when_executed).toBe(true);
    expect(summary.mapping_start_gate_minimal_precheck_safety_only).toBe(true);
    expect(summary.mapping_start_gate_camera_required).toBe(true);
    expect(summary.mapping_start_gate_radar_required).toBe(true);
    expect(summary.mapping_start_gate_camera_ready).toBe(false);
    expect(summary.mapping_start_gate_radar_ready).toBe(false);
    expect(summary.mapping_start_gate_camera_blocks_start).toBe(true);
    expect(summary.mapping_start_gate_radar_blocks_start).toBe(true);
    expect(summary.mapping_start_gate_only_camera_missing).toBe(false);
    expect(summary.mapping_start_gate_radar_overlay_wysiwyg_complete).toBe(false);
    expect(summary.mapping_start_gate_camera_hardware_action_required).toBe(false);
    expect(summary.mapping_start_gate_camera_hardware_action_label).toBe("复测相机首帧");
    expect(summary.mapping_start_gate_camera_recovery_next_action_plain).toContain("复测相机首帧");
    expect(summary.mapping_start_gate_blocks_free_move).toBe(false);
    expect(summary.mapping_start_gate_free_move_allowed_while_blocked).toBe(true);
    expect(summary.mapping_start_gate_post_start_readback_refreshes_free_roam_latest).toBe(true);
    expect(summary.mapping_start_gate_post_start_readback_refreshes_map_preview).toBe(true);
    expect(summary.mapping_start_gate_post_start_readback_refreshes_summary).toBe(true);
    expect(summary.mapping_start_gate_sends_motion_when_clicked).toBe(false);
    expect(summary.mapping_start_gate_sends_motion_when_executed).toBe(true);
    expect(summary.mapping_start_gate_starts_map_runtime_when_clicked).toBe(false);
    expect(summary.mapping_start_gate_starts_map_runtime_when_executed).toBe(true);
    expect(summary.mapping_start_gate_starts_nav2_when_clicked).toBe(false);
    expect(summary.mapping_start_gate_starts_manual_when_clicked).toBe(false);
    expect(summary.mapping_start_gate_starts_keyboard_when_clicked).toBe(false);
    expect(summary.mapping_start_gate_starts_free_roam_when_clicked).toBe(false);
    expect(summary.mapping_start_gate_submits_delivery_when_clicked).toBe(false);
    expect(summary.mapping_start_gate_stops_motion_when_clicked).toBe(false);
    expect(summary.mapping_start_gate_readback_sends_motion).toBe(false);
    expect(summary.mapping_start_gate_readback_starts_nav2).toBe(false);
    expect(summary.mapping_start_gate_readback_starts_manual).toBe(false);
    expect(summary.mapping_start_gate_readback_starts_keyboard).toBe(false);
    expect(summary.mapping_start_gate_readback_starts_free_roam).toBe(false);
    expect(summary.mapping_start_gate_readback_starts_map_runtime).toBe(false);
    expect(summary.mapping_start_gate_readback_submits_delivery).toBe(false);
    expect(summary.mapping_start_gate_readback_stops_motion).toBe(false);
    ...
    expect(summary.field_acceptance_primary_missing_evidence_requires_safety_confirm_before_motion).toBe(false);
    ...
    expect(summary.field_acceptance_primary_missing_action_requires_safety_confirm).toBe(false);
    ...
    expect(summary.field_acceptance_primary_requires_safety_confirm_before_motion).toBe(false);
    ...
    expect(fieldAcceptancePacket?.primary_missing_evidence_requires_safety_confirm_before_motion).toBe(false);
    ...
    expect(summary.live_closure_summary?.live_motion_runbook_items).toEqual([
      expect.objectContaining({
        ...
        proof_plain: "可复验完整行程：直接执行图上路线，执行后按验收端点读回；还差：同窗口 wheel L/R 非零、送达确认。",
      }),
      expect.objectContaining({
        id: "hold_keyboard",
        ready: true,
  ...
  it("prioritizes full-speed USB camera diagnosis in camera summary", async () => {
    ...
    expect(summary.current_camera_wysiwyg_pack_blocks_free_move).toBe(false);
    expect(summary.current_camera_wysiwyg_pack_sends_motion_when_clicked).toBe(false);
    expect(summary.camera_first_frame_fix_status).toBe(summary.current_camera_wysiwyg_pack_status);
    expect(summary.camera_first_frame_fix_plain).toBe(summary.current_camera_wysiwyg_pack_next_action_plain);
    expect(summary.camera_first_frame_fix_missing_evidence).toEqual(["camera_first_frame"]);
    expect(summary.camera_first_frame_fix_missing_evidence_labels).toEqual(["画面首帧"]);
    expect(summary.camera_first_frame_fix_visible).toBe(false);
    expect(summary.camera_first_frame_fix_first_frame_ready).toBe(false);
    expect(summary.camera_first_frame_fix_shared_preview_current_frame_visible).toBe(false);
    expect(summary.camera_first_frame_fix_shared_preview_everyone_can_join).toBe(true);
    expect(summary.camera_first_frame_fix_source_diagnosis_status).toBe("uvc_full_speed_usb_not_exclusive");
    expect(summary.camera_first_frame_fix_source_diagnosis_not_exclusive).toBe("true");
    expect(summary.camera_first_frame_fix_probe_status).toBe(summary.current_camera_wysiwyg_pack_first_frame_probe_status);
    expect(summary.camera_first_frame_fix_failure_reason).toBe(summary.current_camera_wysiwyg_pack_first_frame_failure_reason);
    expect(summary.camera_first_frame_fix_low_bandwidth_fallback_attempted).toBe("true");
    expect(summary.camera_first_frame_fix_low_bandwidth_fallback_min_size).toBe("160x120");
    expect(summary.camera_first_frame_fix_software_fallback_exhausted).toBe(true);
    expect(summary.camera_first_frame_fix_requires_physical_usb_fix).toBe(true);
    expect(summary.camera_first_frame_fix_physical_fix_label).toBe("换高速USB后复测");
    expect(summary.camera_first_frame_fix_hardware_action_required).toBe(true);
    expect(summary.camera_first_frame_fix_hardware_action_label).toBe("换高速USB后复测");
    expect(summary.camera_first_frame_fix_usb_full_speed_detected).toBe(true);
    expect(summary.camera_first_frame_fix_usb_speed).toBe("12M");
    expect(summary.camera_first_frame_fix_sequence).toEqual(summary.current_camera_wysiwyg_pack_sequence);
    expect(summary.camera_first_frame_fix_sequence_labels).toEqual(summary.current_camera_wysiwyg_pack_sequence_labels);
    expect(summary.camera_first_frame_fix_probe_endpoint).toBe("/api/robot-control/camera/first-frame/probe");
    expect(summary.camera_first_frame_fix_mjpeg_status_endpoint).toBe("/api/robot-control/camera/mjpeg/status");
    expect(summary.camera_first_frame_fix_summary_endpoint).toBe("/api/robot-control/summary");
    expect(summary.camera_first_frame_fix_blocks_wysiwyg).toBe(true);
    expect(summary.camera_first_frame_fix_blocks_mapping_start).toBe(true);
    expect(summary.camera_first_frame_fix_blocks_free_move).toBe(false);
    expect(summary.camera_first_frame_fix_readback_only).toBe(true);
    expect(summary.camera_first_frame_fix_no_motion_refresh).toBe(true);
    expect(summary.camera_first_frame_fix_sends_motion_when_clicked).toBe(false);
    expect(summary.camera_first_frame_fix_starts_camera_exclusive_capture).toBe(false);
    expect(summary.camera_first_frame_fix_starts_nav2).toBe(false);
    expect(summary.camera_first_frame_fix_starts_manual).toBe(false);
    expect(summary.camera_first_frame_fix_starts_keyboard).toBe(false);
    expect(summary.camera_first_frame_fix_starts_free_roam).toBe(false);
    expect(summary.camera_first_frame_fix_starts_map_runtime).toBe(false);
    expect(summary.camera_first_frame_fix_submits_delivery).toBe(false);
    expect(summary.camera_first_frame_fix_stops_motion).toBe(false);
  ...
  it("separates free movement from mapping sensor readiness in live closure", async () => {
    ...
    expect(summary.live_closure_summary?.free_move_safety_confirm_required).toBe(false);
  ...
  it("suppresses stale lidar_fresh mapping start gap when live radar readback is already fresh", async () => {
    ...
    expect(summary.live_closure_summary?.objective_audit_summary_plain).toContain("键盘还差按住命令读数 + IMU 实动、松开后停稳");
