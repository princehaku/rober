...
describe("App", () => {
  ...
  it("splits free movement from mapping acceptance when camera and radar are not ready", async () => {
    ...
    expect(wrapper.find('[data-testid="plain-current-facts"]').text()).toContain("建图：上车端缺口：画面首帧未出、地图记录未启动、地图画面未刷新；自由移动不受影响。");
    expect(wrapper.find('[data-testid="plain-current-facts"]').text()).toContain("自由移动：上次记录停在停止请求：现场请求停止；当前没有运动发布，可启动；低速自移动不依赖雷达新鲜度。");
    expect(wrapper.find('[data-testid="plain-map-free-roam-runtime-marker"]').text()).toBe("自由移动记录：上次停止请求");
  ...
  it("shows current camera probe failure on the plain first screen without submitting material", async () => {
    ...
    expect(wrapper.find('[data-testid="robot-camera-preview-frame"]').attributes("data-state")).toBe("失败");
    expect(wrapper.find('[data-testid="robot-camera-preview-overlay"]').text()).toContain("不是页面独占：摄像头能打开，OpenCV/V4L2 后端尝试 4 种方式也没有取到视频帧");
    expect(wrapper.find('[data-testid="robot-camera-wysiwyg-status"]').text()).toBe("画面状态：不是页面独占：摄像头能打开，OpenCV/V4L2 后端尝试 4 种方式也没有取到视频帧；检查 USB、摄像头输入、格式或供电。");
    expect((wrapper.find('input[name="plainExternalVideoRef"]').element as HTMLInputElement).value).toBe("");
