...
describe("App", () => {
  ...
  it("keeps free-roam keyboard locked until map recording starts", async () => {
    ...
    expect(wrapper.find('[data-testid="plain-free-roam-drive-status"]').text()).toBe("扫图状态：地图记录中，先启用键盘扫图；启用本身不会移动。");
    ...
    expect(wrapper.find('[data-testid="plain-map-free-roam-action-marker"]').text()).toBe("地图记录中");
    expect(wrapper.find('[data-testid="plain-map-free-roam-action-marker"]').attributes("data-state")).toBe("recording");
    expect(wrapper.find('[data-testid="plain-map-free-roam-action-marker"]').attributes("aria-label")).toBe("地图记录已启动，等待扫图移动，机器人地图位置未读到，标记不代表坐标");
  ...
  it("allows plain trip preflight but blocks execution until the route is visible while lidar is stopped", async () => {
    // 路线预检不发车；执行按钮还要求地图上看得到当前路线。
    const summaryFixture = cloneFixture(fixtures["/api/robot-control/summary"]) as Record<string, any>;
    summaryFixture.readback_summary.lidar.continuous_scan_status = "lifecycle_not_running";
    summaryFixture.readback_summary.lidar.lifecycle_running = "false";
    summaryFixture.readback_summary.lidar.lifecycle_state = "stopped";
