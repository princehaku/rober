...
describe("App", () => {
  ...
  it("renders Robot Control V1 by default with Robot API proxy and locked command boundary", async () => {
    ...
    expect(workstationStyles).toContain('.plain-free-roam-map[data-state="可移动"]');
    expect(workstationStyles).toContain('.plain-free-roam-map[data-state="可建图"]');
    expect(workstationStyles).toContain('.status-chip[data-state="可移动"]');
    expect(workstationStyles).toContain('.status-chip[data-state="可建图"]');
  ...
  it("shows start-ready free-roam autonomy as startable while runtime is still artifact-only", async () => {
    ...
    summaryFixture.safe_command_boundary.free_roam_autonomy_gates = [
      ...
      { id: "lidar_fresh", label: "雷达新鲜", state: "ready", evidence: "雷达 proof fresh", next_action: "继续监看雷达" },
    ];
    summaryFixture.safe_command_boundary.free_roam_autonomy_runtime = {
      status: "loaded",
      state: "stopping",
    ...
    expect(wrapper.find('[data-testid="plain-current-facts"]').text()).toContain("自由移动：可启动，但当前没有运动发布。");
  ...
  it("splits free movement from mapping acceptance when camera and radar are not ready", async () => {
    // live 形状：start_ready=true，但 camera 无首帧且 radar 端点冲突；车可低速自由移动，本轮不能按建图验收。
    const summaryFixture = structuredClone(fixtures["/api/robot-control/summary"] as RobotControlSummaryResponse);
    summaryFixture.safe_command_boundary.free_roam_autonomy = "locked";
    summaryFixture.safe_command_boundary.free_roam_autonomy_start_ready = true;
    summaryFixture.safe_command_boundary.free_roam_autonomy_runtime = {
    ...
    summaryFixture.read_endpoints = [
      ...summaryFixture.read_endpoints,
      {
        ...
        status: "scan_once_hz_raw_packet_tf_observed",
        evidence_ref: "radar-status-stopped",
        key_values: {
      ...
      },
      {
        ...
        status: "loaded",
        evidence_ref: "radar-proof-running",
        key_values: {
          lifecycle_running: "true",
          lifecycle_state: "running",
          latest_scan_proof_fresh: "false",
  ...
  it("allows low-speed free-roam recording while marking mapping degraded when camera has no first frame", async () => {
    // 自由低速移动只要求安全确认和停止兜底；摄像头缺首帧只影响本轮是否能算可建图。
    const summaryFixture = cloneFixture(fixtures["/api/robot-control/summary"]) as Record<string, any>;
    markMappingSensorsReady(summaryFixture);
    summaryFixture.readback_summary.camera.status = "source_first_frame_failed";
    summaryFixture.readback_summary.camera.source_readiness = "first_frame_failed";
    ...
    expect(wrapper.find('[data-testid="plain-free-roam-hint"]').text()).toContain("可先启动记录并低速移动");
    ...
    expect(freeMoveStartCall).toBeDefined();
    expect(JSON.parse(String(freeMoveStartCall?.[1]?.body ?? "{}"))).toEqual({
      confirm_operator_safety: true,
      confirm_mapping_active: false,
    });
    expect(wrapper.find('[data-testid="plain-free-roam-auto-stop"]').text()).toBe("停止自由移动（随时可点）");
  ...
  it("allows recording on stale radar proof while keeping plain keyboard control independent", async () => {
    // 雷达 ready 是建图质量门禁；普通键盘手控仍只依赖安全确认和自己的手控门禁。
    const summaryFixture = cloneFixture(fixtures["/api/robot-control/summary"]) as Record<string, any>;
    markMappingSensorsReady(summaryFixture);
  ...
  it("uses Robot Control summary latest Nav2 execution readback on the plain map when latest key values are empty", async () => {
    // summary 已经聚合上位机 latest，只读 latest 端点为空时普通首屏也不能退回“未读到行程”。
    const summaryFixture = structuredClone(fixtures["/api/robot-control/summary"] as RobotControlSummaryResponse);
    summaryFixture.o3_proof_summary.path_preview_points = [
      { x: 0.1, y: 0.1, frame_id: "map", source_index: 0 },
      { x: 0.4, y: 0.1, frame_id: "map", source_index: 7 },
  ...
  it("keeps a Nav2 success with explicit unproven execution out of the complete route gate", async () => {
    ...
    const mockedFetch = stubWorkstationFetch({
      ...
      "/api/robot-control/nav2/goal/execution/latest": {
        ...
        goal_execution_key_values: {
          status: "goal_succeeded",
          evidence_ref: "o11-nav2-goal-execution-unproven-fixture",
          generated_at_ms: "1782150441201",
          response_generated_at_ms: "1782150442201",
          result_status: "succeeded",
          feedback_sample_count: "8",
  ...
  it("shows current wheel L/R and frame count in plain goal progress from summary", async () => {
    ...
    expect(wrapper.find('[data-testid="plain-wheel-zero-check-summary"]').exists()).toBe(false);
  ...
  it("keeps Nav2 success with IMU motion signal out of complete route evidence until wheel raw L/R is nonzero", async () => {
    // 当前真机形态：Nav2 action 成功，bridge 发出 PWM164，L/R 长日志仍可能为 0；IMU 只能解释运动迹象，不能替代 wheel raw L/R。
    stubWorkstationFetch({
      "/api/robot-control/nav2/goal/execution/latest": {
        schema: "trashbot.pc_tools_workstation.robot_control_nav_goal_execution_latest_proxy.v1",
        proxy_status: "latest_loaded",
  ...
  it("checks the plain camera frame as a read-only WYSIWYG action", async () => {
    // 实时画面卡的检查按钮只读取上位机样张，不打开 WebRTC、不保存材料、不发送控制。
    const summaryFixture = cloneFixture(fixtures["/api/robot-control/summary"]) as Record<string, any>;
    summaryFixture.readback_summary.camera.status = "ready";
    summaryFixture.operator_hil_material_summary.external_video = "not_loaded";
    summaryFixture.operator_hil_material_summary.camera_visible = "not_loaded";
    const mockedFetch = stubWorkstationFetch({
      ...
      "/api/robot-control/camera/first-frame/probe": {
        ...
        probe_key_values: {
          ...
          visible_content_proven: "true",
          visible_content_candidate: "true",
          sample_path: "/root/rober/onboard/runtime/camera/plain_camera_probe.jpg",
          sample_write_ok: "true",
          elapsed_ms: "120",
          mean_luma: "42.0",
          max_luma: "220",
    ...
    const button = wrapper.find('[data-testid="plain-camera-probe"]');
    expect(button.exists()).toBe(true);
  ...
  it("marks a low-light camera probe as dark instead of usable visual material", async () => {
    // 真机可能能读到首帧但画面接近黑；这种样张不能被保存成 visible_content_proven。
    const summaryFixture = cloneFixture(fixtures["/api/robot-control/summary"]) as Record<string, any>;
    summaryFixture.readback_summary.camera.status = "ready";
    summaryFixture.operator_hil_material_summary.external_video = "not_loaded";
    summaryFixture.operator_hil_material_summary.camera_visible = "not_loaded";
    const mockedFetch = stubWorkstationFetch({
      ...
      "/api/robot-control/camera/first-frame/probe": {
        ...
        probe_key_values: {
          ...
          visible_content_proven: "true",
          visible_content_candidate: "true",
          sample_path: "/root/rober/onboard/runtime/camera/dark_probe.jpg",
          sample_write_ok: "true",
          elapsed_ms: "2926",
          mean_luma: "6.5746",
          max_luma: "21",
          dynamic_range_luma: "21",
    ...
    await wrapper.find('[data-testid="plain-camera-probe"]').trigger("click");
    await flushPromises();
    await wrapper.vm.$nextTick();
    ...
    expect(wrapper.find('[data-testid="robot-camera-preview-overlay"]').text()).toContain("上位机样张已读到，但画面太暗");
    expect(wrapper.find('[data-testid="robot-camera-wysiwyg-status"]').text()).toBe("画面状态：当前画面偏暗，先检查镜头或光线。");
    expect(wrapper.find('[data-testid="plain-camera-probe-summary"]').text()).toBe("只读检查：上位机样张已读到，但画面偏暗；检查镜头/光线后重试。");
    expect(wrapper.find('[data-testid="plain-record-current-camera"]').text()).toBe("先检查画面光线");
    expect(wrapper.find('[data-testid="plain-record-current-camera"]').attributes("disabled")).toBeDefined();
  ...
  it("shows current camera probe failure on the plain first screen without submitting material", async () => {
    ...
    expect(wrapper.find('[data-testid="robot-camera-preview-frame"]').attributes("data-state")).toBe("失败");
    expect(wrapper.find('[data-testid="robot-camera-preview-overlay"]').text()).toContain("不是页面独占：摄像头能打开，后端尝试 4 种方式也没有取到视频帧");
    expect(wrapper.find('[data-testid="robot-camera-wysiwyg-status"]').text()).toBe("画面状态：不是页面独占：摄像头能打开，后端尝试 4 种方式也没有取到视频帧；检查 USB、摄像头输入、格式或供电。");
    expect((wrapper.find('input[name="plainExternalVideoRef"]').element as HTMLInputElement).value).toBe("");
  ...
  it("enables non-stop motion only after complete operator material and still uses the fixed workstation proxy", async () => {
    ...
    expect(wrapper.find('[data-testid="keyboard-control-guide"]').text()).toBe("W/A/S/D 或方向键：前进、左转、后退、右转。按住会持续低速移动，约每 0.26 秒发送 0.24 秒低速脉冲，最高 0.12 m/s、单次上限 800 ms；松开、窗口失焦或切页面都会停。");
  ...
  it("stops screen keyboard control when the pointer is cancelled", async () => {
    // 触屏取消事件不能依赖 pointerup；取消后必须走固定 stop 代理，防止连续点动残留。
    const summaryFixture = cloneFixture(fixtures["/api/robot-control/summary"]) as Record<string, any>;
    summaryFixture.operator_hil_material_summary.report_status = "ready_for_execution";
    summaryFixture.operator_hil_material_summary.external_video = "true; ref=phone-video-0605.mp4";
    summaryFixture.operator_hil_material_summary.camera_visible = "true; ref=runtime/camera/latest_metrics.json";
    ...
    await flushPromises();
    await wrapper.vm.$nextTick();
    const screenForward = wrapper.find('[data-testid="keyboard-screen-forward"]');
    await screenForward.trigger("pointerdown");
    await flushPromises();
    await wrapper.vm.$nextTick();
    ...
    await screenForward.trigger("pointercancel");
    await flushPromises();
    await wrapper.vm.$nextTick();

    expect(mockedFetch.mock.calls.filter(([url]) => String(url).startsWith("/api/robot-control/base/stop?"))).toHaveLength(1);
    expect(wrapper.find('[data-testid="keyboard-current-direction"]').text()).toBe("当前方向：未按键");
    expect(wrapper.find('[data-testid="keyboard-last-stop-summary"]').text()).toBe("上次方向：前进；停止原因：方向键触控取消。");
    expect(wrapper.find('[data-testid="keyboard-live-status"]').text()).toBe("已停止，按住方向键可继续点动。");
  });
  ...
  it("updates the map radar marker from the latest read-only radar status during map preview refresh", async () => {
    ...
    expect(mockedFetch.mock.calls.some(([url]) => String(url).startsWith("/api/robot-control/radar/status?"))).toBe(true);
    expect(wrapper.find('[data-testid="plain-radar-panel"]').attributes("data-state")).toBe("雷达待刷新");
    expect(wrapper.find('[data-testid="plain-map-radar-marker"]').text()).toBe("雷达待刷新，待刷新雷达点 42 个，最近障碍 0.30m");
    expect(wrapper.find('[data-testid="plain-map-radar-marker"]').attributes("data-state")).toBe("雷达待刷新");
    expect(wrapper.find('[data-testid="plain-map-radar-marker"]').attributes("aria-label")).toBe("雷达待刷新，地图位置未读到，待刷新雷达点 42 个，最近障碍 0.30m，仅点数没有点数组，未贴到地图");
    expect(wrapper.find('[data-testid="plain-map-radar-freshness-label"]').text()).toBe("雷达点口径：正在确认实时性，当前只有待刷新雷达点 42 个（仅点数，没有点数组，未显示局部轮廓），同时只读门禁显示最近障碍 0.30m；刷新后再确认点位。");
    expect(wrapper.find('[data-testid="plain-map-coordinate-truth-label"]').text()).toContain("待刷新雷达点 42 个（仅点数，没有点数组，未显示局部轮廓），最近障碍 0.30m");
    expect(mockedFetch.mock.calls.some(([url]) => String(url).startsWith("/api/robot-control/radar/start?"))).toBe(false);
    expect(mockedFetch.mock.calls.some(([url]) => String(url).startsWith("/api/robot-control/base/manual?"))).toBe(false);
    expect(mockedFetch.mock.calls.some(([url]) => String(url).startsWith("/api/robot-control/nav2/goal/execute?"))).toBe(false);
  });

  it("keeps count-only running radar and near obstacle visible together on the map", async () => {
    // live 形状：雷达 lifecycle running，但 proof incomplete；地图不能在点数和最近障碍之间二选一。
    const summaryFixture = cloneFixture(fixtures["/api/robot-control/summary"]) as Record<string, any>;
    summaryFixture.o3_proof_summary.scan_preview_points = [];
    summaryFixture.o3_proof_summary.scan_preview_point_count = 0;
    summaryFixture.o3_proof_summary.robot_pose = null;
    ...
    expect(marker.attributes("aria-label")).toBe("雷达待刷新，地图位置未读到，待刷新雷达点 72 个，最近障碍 0.04m，仅点数没有点数组，未贴到地图");
    expect(wrapper.find('[data-testid="plain-map-radar-freshness-label"]').text()).toBe("雷达点口径：正在确认实时性，当前只有待刷新雷达点 72 个（仅点数，没有点数组，未显示局部轮廓），同时只读门禁显示最近障碍 0.04m；刷新后再确认点位。");
    expect(wrapper.find('[data-testid="plain-map-coordinate-truth-label"]').text()).toContain("待刷新雷达点 72 个（仅点数，没有点数组，未显示局部轮廓），最近障碍 0.04m");
    expect(mockedFetch.mock.calls.some(([url]) => String(url).startsWith("/api/robot-control/radar/start?"))).toBe(false);
    expect(mockedFetch.mock.calls.some(([url]) => String(url).startsWith("/api/robot-control/base/manual?"))).toBe(false);
    expect(mockedFetch.mock.calls.some(([url]) => String(url).startsWith("/api/robot-control/nav2/goal/execute?"))).toBe(false);
    expect(mockedFetch.mock.calls.some(([url]) => String(url).includes("/cmd_vel"))).toBe(false);
  ...
  it("shows final confirmation as next step when latest draft material matches the fresh Nav2 route", async () => {
    // 真实现场已经有本轮 Nav2 成功和 delivery 草稿材料时，普通首页应直接引导人工确认，不重复发车或保存。
    const mockedFetch = stubWorkstationFetch({
      "/api/robot-control/nav2/goal/execution/latest": {
        schema: "trashbot.pc_tools_workstation.robot_control_nav_goal_execution_latest_proxy.v1",
        proxy_status: "latest_loaded",
        ...
        goal_execution_key_values: {
          status: "goal_succeeded",
          evidence_ref: "o11-nav2-goal-execution-1782099547218",
          generated_at_ms: "1782150441201",
          response_generated_at_ms: "1782150442201",
          result_status: "succeeded",
          feedback_sample_count: "8",
  ...
  it("uses camera health first-frame failure when shared preview has no latest failure", async () => {
    ...
    mjpegStatusFixture.last_failure_reason = "";
    mjpegStatusFixture.last_remote_http_status = null;
    const mockedFetch = stubWorkstationFetch({
      "/api/robot-control/summary": summaryFixture,
    ...
    expect(wrapper.find('[data-testid="plain-current-facts"]').text()).toContain("画面：不是独占，摄像头没人占用但没有输出视频帧。");
  ...
  it("translates shared camera MJPEG upstream timeout without exposing internal tokens", async () => {
    ...
    summaryFixture.readback_summary.camera.source_usage_owner_count = "0";
    summaryFixture.readback_summary.camera.shared_preview_exclusive_camera_claim = "false";
    summaryFixture.readback_summary.camera.shared_preview_last_failure_reason = "camera_mjpeg_upstream_timeout";
    summaryFixture.readback_summary.camera.shared_preview_last_remote_http_status = "none";
    const mjpegStatusFixture = cloneFixture(fixtures["/api/robot-control/camera/mjpeg/status"]) as Record<string, any>;
    mjpegStatusFixture.client_count = 0;
    mjpegStatusFixture.upstream_active = false;
    mjpegStatusFixture.content_type_loaded = false;
    mjpegStatusFixture.exclusive_camera_claim = false;
    mjpegStatusFixture.last_failure_reason = "camera_mjpeg_upstream_timeout";
    mjpegStatusFixture.last_remote_http_status = null;
    const mockedFetch = stubWorkstationFetch({
      "/api/robot-control/summary": summaryFixture,
      "/api/robot-control/camera/mjpeg/status": mjpegStatusFixture,
  ...
  it("explains a live not-in-use camera first-frame failure as not exclusive access", async () => {
    ...
    summaryFixture.readback_summary.camera.first_frame_probe_fallback_attempts_summary = "MJPG@640x480:first_frame_timeout/capture_read_call_timeout";
    const mjpegStatusFixture = cloneFixture(fixtures["/api/robot-control/camera/mjpeg/status"]) as Record<string, any>;
    mjpegStatusFixture.client_count = 0;
    mjpegStatusFixture.upstream_active = false;
    mjpegStatusFixture.content_type_loaded = false;
    ...
    expect(mjpegPreview.attributes("src")).toContain("/api/robot-control/camera/mjpeg?");
    expect(wrapper.find('[data-testid="plain-camera-panel"]').attributes("data-state")).toBe("失败");
    expect(wrapper.find('[data-testid="plain-current-facts"]').text()).toContain("画面：不是独占，后端多种方式也没有取到视频帧。");
    expect(wrapper.find('[data-testid="robot-camera-preview-overlay"]').text()).toContain("不是页面独占：USB Composite Device: DV20 USB：摄像头能打开，后端尝试 4 种方式也没有取到视频帧");
    expect(wrapper.find('[data-testid="robot-camera-wysiwyg-status"]').text()).toBe("画面状态：不是页面独占：USB Composite Device: DV20 USB：摄像头能打开，后端尝试 4 种方式也没有取到视频帧；检查 USB、摄像头输入、格式或供电。");
