...
function buildLiveClosureSummary(
  ...
  const mapDisplayTooSmallNextActionPlain = "PC 首页现在让地图独占首行，真实地图按画布高度优先放大，宽图横向滚动，默认用 200% 可读大图显示地图、路线、小车、雷达和目标；图传和 WASD 放到地图下方；需要全局总览点“完整态势”回到 100%，需要局部排障再点“细节放大”到 1200%，或点“进入地图大屏”打开 /map；/map 默认也是 200% 可读大图，工具条悬浮在地图上，不再占画布高度，只保留缩放、只读刷新和工程观察入口；建图、保存和其他卡片都会收起；不需要先开 RViz2。";
  ...
  const mapDisplayCompanionPlain = `普通用户地图：首页 PC 大地图默认 200% 可读大图；进入 /map 也是 200% 可读大图，地图画布按 viewport-dominant full-height 处理，真实地图按高度优先铺满画布，宽图横向滚动，/map 工具条悬浮在地图上不再占画布高度，点“完整态势”回到 100% 全局视角，点“细节放大”可继续查看局部，最高 1200%，地图、路线、目标点、小车位置和雷达点共用同一张 WYSIWYG 画布；${mapDisplayTooSmallNextActionPlain}${mapDisplayRos2CompanionAnswerPlain}ROS2 配套只作工程观察，本地用 RViz2，远程浏览器观察先部署 Foxglove bridge 后打开 Foxglove Web 连接 ws://192.168.1.11:8765；观察项固定为地图、雷达、TF、路径、定位和 costmap，不提供 GoalTool，不发送底盘移动命令。`;
