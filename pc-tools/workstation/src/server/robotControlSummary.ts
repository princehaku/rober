...
function keyboardSummaryReadback(): RobotControlSummaryResponse["readback_summary"]["keyboard"] {
  ...
  return {
    ...
    continuous_control_contract_plain: `按住时约每 ${ROBOT_CONTROL_KEYBOARD_JOG_INTERVAL_MS / 1000} 秒发送一次 ${ROBOT_CONTROL_KEYBOARD_JOG_DURATION_MS / 1000} 秒 PWM/T=11 低速脉冲；松开、失焦、切页、换方向或点击停止都会停。`,
...
function buildLiveClosureSummary(
  ...
  const mapDisplayTooSmallNextActionPlain = "PC 首页默认用 300% 细节大图显示真实地图，地图画布先吃满首屏；要看全局就点“适配”，仍觉得小就点“进入地图大屏”打开 /map；/map 只保留缩放、只读刷新和工程观察入口；建图、保存和其他卡片都会收起；不需要先开 RViz2。";
