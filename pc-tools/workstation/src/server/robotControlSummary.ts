...
function cameraSummaryFromReadbacks(
  ...
  const overlayCmaMemoryDiagnostics: JsonRecord | null = mjpegRelayOverlay?.cma_memory_diagnostics_status
    ? {
      status: mjpegRelayOverlay.cma_memory_diagnostics_status,
      plain_hint: mjpegRelayOverlay.cma_memory_diagnostics_plain_hint,
      next_action: mjpegRelayOverlay.cma_memory_diagnostics_next_action,
      cma_total_kb: mjpegRelayOverlay.cma_memory_diagnostics_cma_total_kb,
      cma_free_kb: mjpegRelayOverlay.cma_memory_diagnostics_cma_free_kb,
      failure_count: mjpegRelayOverlay.cma_memory_diagnostics_failure_count,
      latest_failure: mjpegRelayOverlay.cma_memory_diagnostics_latest_failure,
    }
    : null;
  const cmaMemoryDiagnostics = asRecord(findFirstKey(healthPayload, ["cma_memory_diagnostics"]) ?? mediaDiagnostics?.cma_memory_diagnostics) ?? overlayCmaMemoryDiagnostics;
  ...
  const sourceTransportHasOwnFirstFrameFailure = asString(sourceDiagnosis?.status, "") === "uvc_transport_error_not_exclusive"
    && rawSourceReadiness === "first_frame_failed"
    && Boolean(rawSourceFailureReason)
    && !["none", "not_loaded", "source_selected_not_probed"].includes(rawSourceFailureReason);
  ...
  const cmaAllocFailedObserved = asString(cmaMemoryDiagnostics?.status, "") === "cma_alloc_failed_recent"
    || asString(sourceDiagnosis?.cma_memory_diagnostics_status, "") === "cma_alloc_failed_recent";
  ...
  const cmaAllocFailedPlainHint = asString(cmaMemoryDiagnostics?.plain_hint, "")
    || `不是页面独占：${selectedName} 当前无人占用，但内核最近出现 CMA 连续内存分配失败，UVC 采集缓冲可能无法分配。`;
  ...
  const derivedSourceDiagnosis = sourceNoFrameNotExclusive && uvcVideoOnFullSpeedUsb
    ...
    : sourceNoFrameNotExclusive && uvcKernelTransportErrorsObserved && sourceTransportHasOwnFirstFrameFailure
    ? {
    ...
    : sourceNoFrameNotExclusive && cmaAllocFailedObserved
    ? {
      status: "uvc_cma_alloc_failed_not_exclusive",
      plain_hint: cmaAllocFailedPlainHint,
      next_action: "free_memory_or_reboot_then_probe_known_good_uvc",
      next_action_plain: cameraActionPlainText("free_memory_or_reboot_then_probe_known_good_uvc"),
      not_exclusive: true,
    }
  ...
  return {
    ...
    cma_memory_diagnostics_status: asString(cmaMemoryDiagnostics?.status ?? sourceDiagnosis?.cma_memory_diagnostics_status ?? findFirstKey(healthPayload, ["cma_memory_diagnostics_status"]), "not_loaded"),
    cma_memory_diagnostics_plain_hint: asString(cmaMemoryDiagnostics?.plain_hint ?? findFirstKey(healthPayload, ["cma_memory_diagnostics_plain_hint"]), "not_loaded"),
    cma_memory_diagnostics_next_action: asString(cmaMemoryDiagnostics?.next_action ?? findFirstKey(healthPayload, ["cma_memory_diagnostics_next_action"]), "not_loaded"),
    cma_memory_diagnostics_cma_total_kb: cmaMemoryDiagnostics?.cma_total_kb === undefined
      ? summaryValueText(healthPayload, ["cma_memory_diagnostics_cma_total_kb"])
      : compactValueText(cmaMemoryDiagnostics.cma_total_kb),
    cma_memory_diagnostics_cma_free_kb: cmaMemoryDiagnostics?.cma_free_kb === undefined
      ? summaryValueText(healthPayload, ["cma_memory_diagnostics_cma_free_kb"])
      : compactValueText(cmaMemoryDiagnostics.cma_free_kb),
    cma_memory_diagnostics_failure_count: cmaMemoryDiagnostics?.failure_count === undefined
      ? summaryValueText(healthPayload, ["cma_memory_diagnostics_failure_count"])
      : compactValueText(cmaMemoryDiagnostics.failure_count),
    cma_memory_diagnostics_latest_failure: asString(cmaMemoryDiagnostics?.latest_failure ?? findFirstKey(healthPayload, ["cma_memory_diagnostics_latest_failure"]), ""),
...
function mapPreviewPathNextActionPlain(
  ...
  return "图上路线和小车位置已显示；确认起点、终点和路线后可直接执行。";
}
...
function nav2RouteExecutionPlainSummary(args: {
  ...
  const minimalPrecheck = `现场默认安全；相机、雷达和现场报告不作为额外发车前置；执行会用${modeActionText}跑图上路线${runtimeAutoStartText}。`;
  ...
  return {
    ...
    precheckPlain: "路线准备完成后可直接执行；现场默认安全，不要求额外勾选。",
  };
}
...
function nav2ReadbackPlainSummary(args: {
  ...
  if (args.goalSucceeded && args.wheelFeedbackProven === "false") {
    return {
      ...
      next_action_plain: `直接用 ${nextMode}重跑图上路线${runtimeAutoStartText}，并在同窗口确认轮速 L/R 非零。`,
    };
  }
  if (pathReady) {
    return {
      execution_status_plain: "图上路线已准备，但本轮完整执行和轮速 L/R 还未证明。",
      next_action_plain: `直接执行图上路线${runtimeAutoStartText}，并在同窗口确认轮速 L/R 非零。`,
    };
  }
...
function nav2WheelRawLrPlainSummary(args: {
  ...
  if (args.goalSucceeded && args.wheelFeedbackProven === "false") {
    ...
    return {
      ...
      nextActionPlain: `直接用 ${modeText}重跑图上路线${runtimeAutoStartText}，并在同窗口确认轮速 L/R 非零。`,
    };
  }
  return {
...
function freeRoamMotionSensorDependencyPlain(motionCanStart: boolean): string {
  ...
  return motionCanStart
    ? "自由移动打开即用，只保留停止兜底；相机、雷达和地图记录只影响建图验收。"
...
function freeRoamPlainNextParts(nextActionPlain: string, existingParts: string[]): string[] {
  ...
  return nextActionPlain
    ...
    .filter((item) => {
      ...
      if (runningAlreadyExplained && (item.includes("可直接先自由移动") || item.includes("勾选现场安全确认后可先自由移动"))) {
        return false;
      }
...
function failClosed(reason: string, sourceBaseUrl: string): RobotControlSummaryResponse {
  ...
  const notLoadedNav2Summary: RobotControlSummaryResponse["readback_summary"]["nav2"] = {
    status: "not_loaded",
    plain_hint: "图上路线还未准备完成。下一步：先准备图上路线并刷新地图画面，然后直接执行。",
...
function freeRoamRuntimeGatesFromReadbacks(
  ...
  if (externalStopRequested && !hasGate("external_stop_request")) {
    // stop 是上一次会话留下的安全状态；它不是雷达问题，下一次 start 会在固定代理里显式清掉。
    gateRows.push({
      ...
      next_action: "直接点击开始自由移动；start 会清除停止请求并打开低速运动双锁",
    });
  }
...
function freeRoamAutonomyNextAction(
  ...
  if (status === "start_ready") {
    const motionAction = externalStopRequested
      ? "停止请求会在开始自由移动时自动解除，不作为启动阻塞；可直接先自由移动"
      : mappingReady
        ? "可直接开始自动扫图（低速）"
        : "可直接先自由移动";
    if (mappingReady) {
  ...
  if (manualMotionFallbackActive) {
    return "可先用键盘或低速手控移动；要启动上车自由移动状态机，先连接状态机并确认停止兜底";
  }
...
function freeRoamMotionNextAction(startReady: boolean, motionReady: boolean, externalStopRequested: boolean, manualMotionFallbackActive = false): string {
  // 自由移动只看固定代理和停止兜底；相机/雷达/地图记录只影响建图验收，不该写成不能动。
  if (motionReady) {
  ...
  if (startReady) {
    const stopPrefix = externalStopRequested
      ...
      : "";
    return `${stopPrefix}可直接先自由移动；相机和雷达只影响建图验收。`;
  }
  if (manualMotionFallbackActive) {
    return "上车自由移动状态机未加载；可先用键盘或低速手控移动；相机和雷达只影响建图。";
  }
  return "先连接上车自由移动状态机，并确认停止兜底可用。";
}
...
function freeRoamMappingStartNextAction(startReady: boolean, mappingStartReady: boolean, mappingStartMissingReasons: string[]): string {
  ...
  if (mappingStartReady) {
    return "相机和雷达已满足建图启动；可直接启动建图记录，再看地图画面完成验收。";
  }
  const missingText = freeRoamMissingPlainLabels(mappingStartMissingReasons).join("、");
  if (!startReady) {
    return missingText
...
> {
  ...
  return {
    ...
    nav2_goal_next_action: ready ? `直接执行图上路线${poseHint}` : "先生成图上路线",
    nav2_goal_next_action_plain: nav2GoalNextActionPlainText(ready ? `直接执行图上路线${poseHint}` : "先生成图上路线"),
...
> {
  ...
  if (succeeded && nav2.goal_execution_base_feedback_lr_nonzero_proven === "false") {
    ...
    const rerunNextAction = !base.nav2_goal_ready
      ? `当前图上路线未就绪，先${routePrepActions.join("，再")}，再用 ${rerunMode} 重跑并复验 wheel raw L/R`
      : inactiveServiceNames.length
        ? `当前自动驾驶服务未就绪，先${routePrepActions.join("，再")}，再用 ${rerunMode} 重跑并复验 wheel raw L/R`
        : `直接用 ${rerunMode} 重跑图上路线${managedRuntimeHint}，并复验 wheel raw L/R`;
  ...
  if (base.nav2_goal_ready && nav2ServiceInactive) {
    ...
    const serviceNextAction = inactiveServiceNames.length
      ? `先${nav2StackNotRunning ? "启动" : "恢复"}${joinChineseList(inactiveServiceNames)}，再直接执行图上路线，并在同窗口复验 wheel raw L/R`
      : "先恢复自动驾驶服务，再直接执行图上路线，并在同窗口复验 wheel raw L/R";
    return {
      ...base,
  ...
  if (base.nav2_goal_ready) {
    const poseHint = proof?.robot_pose ? "" : "；小车位置未显示时建议先重新定位或刷新地图";
    const nextAction = `直接执行图上路线${managedRuntimeHint}，并在同窗口复验 wheel raw L/R${poseHint}`;
    return {
      ...base,
      nav2_goal_wheel_feedback_status: "awaiting_route_execution",
      nav2_goal_next_action: nextAction,
...
function lockedBoundary(
  ...
  return {
    ...
    free_roam_motion_minimal_precheck_plain: "自由移动打开即用，现场默认安全并保留停止兜底；相机、雷达、地图记录只影响建图验收。",
    ...
    free_roam_autonomy_policy: {
      // 自由移动与建图验收分层：低速移动只看固定代理和停止兜底，建图才要求画面/雷达材料。
      mode: "free_move_open_page_ready_stop_fallback",
    ...
    locked_reason: "bounded manual and keyboard pulse control use fixed low-speed proxy; primary autonomy and safe_control remain locked",
    manual_motion_entry_status: "controlled_jog_open_page_ready",
    manual_motion_entry_label: "低速手控（打开即用）",
...
function buildActionStatusCards(
  ...
  return [
    ...
    },
    {
      ...
      next_action_plain: actionCardText(readback.keyboard.next_action_plain || boundary.keyboard_control_next_action, "直接启用键盘"),
    ...
    },
    {
      ...
      next_action_plain: actionCardText(readback.free_roam.motion_next_action_plain || boundary.free_roam_autonomy_next_action, "直接启动自由移动"),
...
function buildGoalChecklist(
  ...
  return [
    ...
    },
    {
      ...
      title: nav2Title,
      status: nav2Done ? "done" : boundary.nav2_goal_ready ? "ready" : "not_ready",
      status_label: nav2Done ? "已完成" : boundary.nav2_goal_ready ? "可执行" : "未就绪",
    ...
    },
    {
      ...
      title: "键盘连续手控",
      status: keyboardReady ? "ready" : "not_ready",
      status_label: keyboardReady ? "可启用" : "未就绪",
    ...
    },
    {
      id: "free_move",
      title: "自由自助移动",
      status: freeMoveRunning ? "ready" : boundary.free_roam_motion_start_ready ? "ready" : "not_ready",
      status_label: freeMoveRunning ? "运行中" : boundary.free_roam_motion_start_ready ? "可启动" : "未就绪",
...
function buildGoalChecklistSummary(
  ...
  const nav2ReadyForRerun = remaining.some((item) =>
    item.id === "nav2_route_execution" && item.status === "ready"
  );
  ...
  const moveNowStatusPlain = motionReadyItems.length > 0
    ? `可先动：${motionReadyItems.map((item) => item.title).join("、")}；现场默认安全，不要求用户额外勾选；相机和雷达只影响建图验收。`
  ...
  const firstMotion = [freeMove, keyboard, nav2].find((item) => item && item.status === "ready") ?? null;
  const firstSafetyPrecheck = [freeMove, nav2, keyboard, mapping].find((item) => item?.status === "ready") ?? null;
  const motionSummary = (() => {
    if (freeMove?.status === "ready" && freeMove.status_label === "运行中") {
    ...
    if (nav2?.status === "ready") {
      return `图上行程可执行；现场默认安全，雷达和相机问题不应改写这个读数。下一步：${nav2.next_action_plain}`;
    }
    return "当前还没有可直接发车的入口；先处理自由移动、键盘或图上行程门禁。";
  })();
  const safetyPrecheckSummary = (() => {
    if (firstSafetyPrecheck) {
      return `发车前预检已精简：现场默认安全，不要求用户额外勾选；相机和雷达不作为移动或行程发车前额外预检。下一步：${firstSafetyPrecheck.next_action_plain}`;
    }
    return "当前没有可直接执行的发车入口；继续按画面、雷达、行程和建图缺口处理。";
  })();
  const radarSummary = (() => {
  ...
  const nav2Summary = (() => {
    ...
    if (nav2?.status === "ready") {
      return `完整图上行程可复验；现场默认安全，不要求用户额外勾选。下一步：${nav2.next_action_plain}`;
    }
    if (nav2) {
  ...
  const mappingSummary = (() => {
    if (mapping?.status === "ready") {
      return `相机和雷达已就绪；建图可直接启动。下一步：${mapping.next_action_plain}`;
    }
    if (mapping) {
...
function buildLiveClosureSummary(
  ...
  const cameraSourceDiagnosisLabel = (status: string): string => ({
    ...
    uvc_cma_alloc_failed_not_exclusive: "CMA 分配失败",
  ...
  const cameraCmaHardwareActionRequired = readback.camera.source_diagnosis_status === "uvc_cma_alloc_failed_not_exclusive";
  ...
  const cameraHardwareFallbackHint = cameraUsbFullSpeedDetected
    ...
    : cameraTransportHardwareActionRequired
      ...
      : cameraCmaHardwareActionRequired
        ? "内核最近出现 CMA 连续内存分配失败，先释放内存或重启上位机后复测相机首帧"
  ...
  const mappingStartUnblockPlain = (() => {
    if (mappingStartReady) {
      return "建图启动已就绪：画面首帧和雷达新鲜都满足；可直接启动建图记录。";
    }
  ...
  const wheelRerunChecklistPlain = needsSameWindowWheelRerun
    ? "重跑闭环：直接执行图上路线；执行后依次读取地图路线画面、latest、底盘轮速采样和 summary，确认图上路线仍可见并确认同窗口 wheel L/R 非零；轮速闭环后再到送达区做送达确认，确认送达不发车。"
    : "当前不需要重跑图上路线；如果后续出现同窗口轮速缺口，再按执行、轮速读回、送达确认顺序收口。";
  ...
  const wheelRerunCurrentGapPlain = needsSameWindowWheelRerun
    ...
    : "当前不需要同窗口轮速复验。";
  const wheelRerunNoExtraPrecheckPlain = "重跑图上路线不要求用户额外勾选安全确认；相机、雷达、地图所见缺口不作为额外发车前置，路线执行后再按读回端点验收。";
  ...
  const liveMotionRunbookItems: NonNullable<RobotControlSummaryResponse["live_closure_summary"]>["live_motion_runbook_items"] = [
    ...
    },
    {
      ...
      proof_plain: proofPlain(
        ...
        "可验证键盘连续手控：打开页面后按住方向键或 WASD，再读轮速与 summary",
        "键盘连续手控暂不可验证",
      ),
      minimal_precheck_safety_only: true,
      safety_confirm_required: false,
    ...
    },
    {
      ...
      proof_plain: proofPlain(
        ...
        "可验证自由自助移动：直接启动，再读 free-roam latest、地图预览和 summary",
        "自由自助移动暂不可验证",
      ),
      minimal_precheck_safety_only: true,
      safety_confirm_required: false,
  ...
  const liveMotionRunbookBlockedPlain = liveMotionRunbookBlockedLabels.length > 0
    ...
    : "暂无被阻塞的运动入口。";
  const liveMotionRunbookMinimalPrecheckPlain = "发车前预检已精简：执行运动打开即用；相机、雷达和现场报告不作为额外发车前置。";
  ...
  const mapDisplayTooSmallNextActionPlain = "PC 首页现在让地图独占首行，真实地图按画布高度优先放大，宽图横向滚动，默认用 1600% 可读大图显示地图、路线、小车、雷达和目标；图传和 WASD 放到地图下方；需要全局总览点“完整态势”回到 100%，需要局部排障再点“细节放大”到 4800%，或点“进入地图大屏”打开 /map；/map 默认也是 1600% 可读大图，工具条悬浮在地图上，不再占画布高度，只保留缩放、只读刷新和工程观察入口；建图、保存和其他卡片都会收起；不需要先开 RViz2。";
  ...
  const mapDisplayCompanionPlain = `普通用户地图：首页 PC 大地图默认 1600% 可读大图；进入 /map 也是 1600% 可读大图，地图画布按 viewport-dominant full-height 处理，真实地图按高度优先铺满画布，宽图横向滚动，/map 工具条悬浮在地图上不再占画布高度，点“完整态势”回到 100% 全局视角，点“细节放大”可继续查看局部，最高 4800%，地图、路线、目标点、小车位置和雷达点共用同一张 WYSIWYG 画布；${mapDisplayTooSmallNextActionPlain}${mapDisplayRos2CompanionAnswerPlain}ROS2 配套只作工程观察，本地用 RViz2，远程浏览器观察先部署 Foxglove bridge 后打开 Foxglove Web 连接 ws://192.168.1.11:8765；观察项固定为地图、雷达、TF、路径、定位和 costmap，不提供 GoalTool，不发送底盘移动命令。`;
  ...
  const objectiveAuditItems: RobotControlLiveObjectiveAuditItem[] = [
    ...
    },
    {
      id: "precheck",
      title: "发车前确认",
      state: minimalPrecheckSafetyOnly ? "打开即用" : "未收敛",
      summary_plain: minimalPrecheckSafetyOnly
        ? "发车前预检已精简：执行运动打开即用；相机、雷达和现场报告不作为额外发车前置。"
        : "发车前预检还未收敛到打开即用。",
      next_action_plain: goalSummary.safety_precheck_next_action_plain || "当前没有待执行入口。",
  ...
  return {
    ...
    map_display_direct_map_default_zoom_percent: "1600%",
    ...
    camera_hardware_action_required: cameraHardwareActionRequired,
    camera_hardware_action_label: cameraHardwareActionLabel,
    camera_input_signal_check_required: readback.camera.camera_input_signal_check_required === true,
    camera_input_signal_check_label: readback.camera.camera_input_signal_check_label || "无需输入信号处理",
    camera_input_signal_check_plain: readback.camera.camera_input_signal_check_plain || "当前相机诊断未指向输入信号处理。",
...
export async function buildRobotControlSummary(
  ...
  const keyboardPostHoldReadbackSequenceLabels = ["复验键盘轮速采样", "刷新总览"];
  ...
  const fieldAcceptanceSafetyConfirmReadyStepIds = fieldAcceptanceSteps
    .filter((item) => item.ready && !item.completed && item.sends_motion_when_executed)
    .map((item) => item.id);
  ...
  const fieldAcceptanceOperatorActionPlain = fieldAcceptanceSafetyConfirmReadyStepIds.length > 0
    ? `可直接执行的运动验收：${fieldAcceptanceSafetyConfirmReadyDisplayLabels.join("、")}；现场默认安全，手动执行后只读读回复验。`
    : "当前没有可直接执行的运动验收入口。";
  ...
  const currentMotionVerificationPackStatus: "complete" | "ready_to_use" | "blocked" = motionObjectiveAlias?.completed === true
    ? "complete"
  ...
  const currentMotionVerificationPackPlain = (() => {
    ...
    if (currentMotionVerificationPackStatus === "ready_to_use") {
      return `运动验收可执行：现场默认安全，按顺序验证 ${currentMotionVerificationPackActionDisplayLabels.join("、")}；不把相机、雷达或现场报告作为额外发车前置；执行后按读回端点复验 ${currentMotionVerificationPackMissingEvidence.join("、") || "运动闭环"}。`;
    }
    return "运动验收暂未就绪；先按当前卡点补齐缺失证据，再回到打开即用动作区。";
  })();
  const currentMinimalPrecheckPackStatus: "complete" | "open_page_ready" | "blocked" = currentMotionVerificationPackStatus === "complete"
    ? "complete"
  ...
  const currentMinimalPrecheckPackPlain = (() => {
    ...
    if (currentMinimalPrecheckPackStatus === "open_page_ready") {
      return `发车前预检已精简：${currentMotionVerificationPackActionDisplayLabels.join("、")} 打开页面即可用，不要求用户额外勾选安全确认；相机、雷达、现场报告和路线 WYSIWYG 不作为额外发车前置。相机和雷达 ready 仍只影响建图启动/验收。`;
    }
  ...
  const currentKeyboardControlPackStatus: "complete" | "ready_to_use" | "blocked" = keyboardRunbookItem?.completed === true
    ? "complete"
  ...
  const currentFreeMoveControlPackStatus: "complete" | "ready_to_use" | "blocked" = freeMoveComplete
    ? "complete"
  ...
  const currentFreeMoveControlPackPlain = (() => {
    ...
    if (currentFreeMoveControlPackStatus === "ready_to_use") {
      return `自由自助移动可复验：可直接启动；相机和雷达不作为发车前置，启动后只读读取 free-roam latest、地图预览和 summary。当前缺口：${freeMoveActionMissingEvidence.join("、") || "无"}。`;
    }
  ...
  const currentFreeMoveControlPackNextActionPlain = (() => {
    ...
    if (currentFreeMoveControlPackStatus === "ready_to_use") {
      return "直接启动自由自助移动；启动后只读读取 free-roam latest、地图预览和 summary。";
    }
    return "先恢复 free-roam 状态机、停止兜底或上车连接，再回到安全确认启动。";
  })();
  const mappingComplete = mappingRunbookItem?.completed === true;
  const currentMappingControlPackStatus: "complete" | "ready_to_use" | "blocked" = mappingComplete
    ? "complete"
  ...
  const currentMappingControlPackPlain = (() => {
    ...
    if (currentMappingControlPackStatus === "ready_to_use") {
      return "建图可启动：摄像头首帧和雷达新鲜读数都已满足；可直接启动建图记录，启动后只读读取 free-roam latest、地图预览和 summary。";
    }
  ...
  const currentMappingControlPackNextActionPlain = (() => {
    ...
    if (currentMappingControlPackStatus === "ready_to_use") {
      return "直接启动建图记录；启动后只读读取 free-roam latest、地图预览和 summary。";
    }
  ...
  const currentTripExecutionPackStatus: "complete" | "ready_to_use" | "blocked" = nav2RouteAcceptancePacket.completed
    ? "complete"
  ...
  const currentTripExecutionPackPlain = (() => {
    ...
    if (currentTripExecutionPackStatus === "ready_to_use") {
      return `完整行程可复验：可直接执行图上行程；执行后按地图、最近行程、轮速、送达、总览顺序只读复验。当前缺口：${nav2RouteAcceptancePacket.missing_evidence.join("、") || "无"}。`;
    }
  ...
  const currentTripExecutionPackNextActionPlain = (() => {
    ...
    if (currentTripExecutionPackStatus === "ready_to_use") {
      return "直接执行图上 Nav2 行程；执行后按地图、最近行程、轮速、送达和总览顺序只读复验。";
    }
  ...
  const fieldAcceptanceParallelStatusPlain = [
    ...
    fieldAcceptancePacket.primary_safety_confirm_ready_action_id === "none"
      ? "可执行动作：暂无"
      : `可执行动作：${fieldAcceptancePacket.primary_safety_confirm_ready_action_display_label ?? fieldAcceptancePacket.primary_safety_confirm_ready_action_label}`,
    ...
    `自由移动：${liveClosureSummary.free_move_start_ready ? "可直接先做" : "暂不可做"}`,
  ].join("；");
  ...
  const currentGoalCameraOnlyBlocksMappingPlain = currentGoalMappingBlockedOnlyByCamera
    ? liveClosureSummary.free_move_start_ready
      ? "当前建图只差画面首帧；自由移动仍可直接先做，画面恢复后再启动建图。"
      : "当前建图只差画面首帧；先恢复相机首帧后再启动建图。"
    : liveClosureSummary.mapping_start_ready
      ? "相机和雷达已满足建图启动条件；可直接启动建图。"
  ...
  return {
    ...
    // 给 PC 首屏和现场 curl 留一组短字段；值仍来自 current_* 包，不产生第二套判断。
    live_wysiwyg_status: currentWysiwygNextActionStatus,
    live_wysiwyg_status_plain: currentWysiwygNextActionPlain,
    camera_wysiwyg_status: currentCameraWysiwygPackStatus,
    camera_wysiwyg_next_action_plain: liveClosureSummary.live_wysiwyg_camera_recovery_next_action_plain,
    radar_map_wysiwyg_status: currentRadarMapWysiwygPackStatus,
    radar_map_wysiwyg_next_action_plain: liveClosureSummary.radar_overlay_refresh_next_action_plain,
    ...
    field_acceptance_safety_confirm_ready_action_operator_report_preflight_required: fieldAcceptancePacket.safety_confirm_ready_actions.map((action) => action.operator_report_preflight_required),
    ...
    field_acceptance_primary_safety_confirm_ready_action_operator_report_preflight_required: fieldAcceptancePrimarySafetyAction?.operator_report_preflight_required ?? false,
    ...
    field_acceptance_primary_safety_action_id: fieldAcceptancePacket.primary_safety_confirm_ready_action_id,
    field_acceptance_primary_safety_action_label: fieldAcceptancePacket.primary_safety_confirm_ready_action_label,
    field_acceptance_primary_safety_action_display_label: fieldAcceptancePacket.primary_safety_confirm_ready_action_display_label,
    field_acceptance_primary_safety_action_start_endpoint: fieldAcceptancePacket.primary_safety_confirm_ready_action_start_endpoint,
    field_acceptance_primary_safety_action_stop_endpoint: fieldAcceptancePrimarySafetyAction?.stop_endpoint ?? "none",
    field_acceptance_primary_safety_action_acceptance_endpoints: fieldAcceptancePrimarySafetyAction?.acceptance_endpoints ?? [],
    field_acceptance_primary_safety_action_ready_for_safety_confirm: fieldAcceptancePacket.primary_safety_confirm_ready_action_id !== "none",
    field_acceptance_primary_safety_action_minimal_precheck_safety_only: fieldAcceptancePrimarySafetyAction?.minimal_precheck_safety_only ?? false,
    field_acceptance_primary_safety_action_operator_report_preflight_required: fieldAcceptancePrimarySafetyAction?.operator_report_preflight_required ?? false,
    field_acceptance_primary_safety_action_requires_safety_confirm: fieldAcceptancePacket.primary_safety_confirm_ready_action_requires_safety_confirm,
    field_acceptance_primary_safety_action_sends_motion: fieldAcceptancePacket.primary_safety_confirm_ready_action_sends_motion,
    ...
    current_motion_action_ready_for_safety_confirm: fieldAcceptancePacket.primary_safety_confirm_ready_action_id !== "none",
    ...
    current_motion_action_operator_report_preflight_required: fieldAcceptancePrimarySafetyAction?.operator_report_preflight_required ?? false,
    ...
    current_motion_action_starts_nav2: fieldAcceptancePacket.primary_safety_confirm_ready_action_id === "run_nav2_route",
    ...
    current_trip_execution_pack_safety_confirm_required: nav2RouteAcceptancePacket.requires_safety_confirm,
    ...
    // Nav2 完整行程短别名：复用 current_trip_execution_pack，方便现场一条 summary 判断发车入口和执行后复验顺序。
    nav2_route_run_status: currentTripExecutionPackStatus,
    nav2_route_run_plain: currentTripExecutionPackNextActionPlain,
    nav2_route_run_action_id: nav2RouteAcceptancePacket.action_id,
    nav2_route_run_display_label: nav2RouteAcceptancePacket.display_label,
    nav2_route_run_start_endpoint: nav2RouteAcceptancePacket.start_endpoint,
    nav2_route_run_stop_endpoint: nav2RouteAcceptancePacket.stop_endpoint,
    nav2_route_run_readback_endpoints: nav2RouteAcceptancePacket.readback_endpoints,
    nav2_route_run_required_success_markers: nav2RouteAcceptancePacket.required_success_markers,
    nav2_route_run_missing_evidence: nav2RouteAcceptancePacket.missing_evidence,
    nav2_route_run_missing_evidence_labels: nav2RouteAcceptancePacket.missing_evidence.map(currentControlPackEvidenceLabel),
    nav2_route_run_route_ready_on_map: nav2RouteAcceptancePacket.route_ready_on_map,
    nav2_route_run_goal_succeeded: nav2RouteAcceptancePacket.nav2_goal_succeeded,
    nav2_route_run_same_window_wheel_lr_nonzero: nav2RouteAcceptancePacket.same_window_wheel_lr_nonzero,
    nav2_route_run_delivery_success: nav2RouteAcceptancePacket.delivery_success,
    nav2_route_run_needs_same_window_wheel_rerun: nav2RouteAcceptancePacket.needs_same_window_wheel_rerun,
    nav2_route_run_delivery_success_required: nav2RouteAcceptancePacket.delivery_success_required,
    nav2_route_run_current_gap_plain: nav2RouteAcceptancePacket.current_gap_plain,
    nav2_route_run_delivery_next_action_plain: nav2RouteAcceptancePacket.delivery_next_action_plain,
    nav2_route_run_requires_safety_confirm: nav2RouteAcceptancePacket.requires_safety_confirm,
    nav2_route_run_minimal_precheck_safety_only: nav2RouteAcceptancePacket.minimal_precheck_safety_only,
    nav2_route_run_camera_preflight_required: false,
    nav2_route_run_radar_preflight_required: false,
    nav2_route_run_route_wysiwyg_preflight_required: false,
    nav2_route_run_sends_motion_when_clicked: false,
    nav2_route_run_sends_motion_when_executed: true,
    nav2_route_run_starts_nav2_when_clicked: false,
    nav2_route_run_starts_nav2_when_executed: true,
    nav2_route_run_starts_manual_when_clicked: false,
    nav2_route_run_starts_keyboard_when_clicked: false,
    nav2_route_run_starts_free_roam_when_clicked: false,
    nav2_route_run_starts_map_runtime_when_clicked: false,
    nav2_route_run_submits_delivery_when_clicked: false,
    nav2_route_run_stops_motion_when_clicked: false,
    nav2_route_run_readback_sends_motion: false,
    nav2_route_run_readback_starts_nav2: false,
    nav2_route_run_readback_starts_manual: false,
    nav2_route_run_readback_starts_keyboard: false,
    nav2_route_run_readback_starts_free_roam: false,
    ...
    current_motion_verification_pack_requires_safety_confirm: false,
    ...
    current_safety_confirm_queue_status: currentMotionVerificationPackStatus,
    current_safety_confirm_queue_plain: currentMotionVerificationPackActionIds.length > 0
      ? `打开即用动作队列：按顺序手动执行 ${currentMotionVerificationPackActionDisplayLabels.join("、")}；现场默认安全，不把相机、雷达或路线 WYSIWYG 当作额外发车前置；队列卡本身只读不发车，执行后按读回端点复验证据。`
      : "打开即用动作队列暂为空；先补齐当前运动入口或上车连接。",
    current_safety_confirm_queue_action_ids: currentMotionVerificationPackActionIds,
    current_safety_confirm_queue_action_display_labels: currentMotionVerificationPackActionDisplayLabels,
    current_safety_confirm_queue_action_start_endpoints: currentMotionVerificationPackActionStartEndpoints,
    ...
    current_safety_confirm_queue_next_action_plain: currentMotionVerificationPackPrimaryAction
      ? `下一步先处理：${currentMotionVerificationPackPrimaryAction.display_label ?? currentMotionVerificationPackPrimaryAction.label}；点击队列按钮只跳到对应卡片，动作按钮可直接执行。`
      : "当前打开即用队列没有可执行项；先刷新小车状态或补齐入口。",
    current_safety_confirm_queue_action_count: currentMotionVerificationPackActionIds.length,
    current_safety_confirm_queue_requires_safety_confirm: false,
    current_safety_confirm_queue_minimal_precheck_safety_only: currentMotionVerificationPackMinimalPrecheckSafetyOnly,
    current_safety_confirm_queue_camera_preflight_required: currentMotionVerificationPackCameraPreflightRequired,
    current_safety_confirm_queue_radar_preflight_required: currentMotionVerificationPackRadarPreflightRequired,
    current_safety_confirm_queue_operator_report_preflight_required: currentMotionVerificationPackOperatorReportPreflightRequired,
    current_safety_confirm_queue_route_wysiwyg_preflight_required: currentMotionVerificationPackRouteWysiwygPreflightRequired,
    current_safety_confirm_queue_requires_manual_action_per_step: true,
    current_safety_confirm_queue_auto_runs: false,
    current_safety_confirm_queue_sends_motion_when_clicked: false,
    current_safety_confirm_queue_sends_motion_when_executed: currentMotionVerificationPackActionIds.length > 0,
    current_safety_confirm_queue_readback_sends_motion: false,
    current_safety_confirm_queue_starts_nav2_when_clicked: false,
    current_safety_confirm_queue_starts_manual_when_clicked: false,
    current_safety_confirm_queue_starts_keyboard_when_clicked: false,
    current_safety_confirm_queue_starts_free_roam_when_clicked: false,
    ...
    // “可先动”短别名复用安全确认队列，普通 PC 和脚本不必在目标总览、runbook、队列之间拼字段。
    current_move_now_status: currentMotionVerificationPackActionIds.length > 0 ? "ready_to_use" : "no_ready_motion",
    current_move_now_plain: goalSummary.move_now_status_plain,
    current_move_now_action_ids: currentMotionVerificationPackActionIds,
    current_move_now_action_display_labels: currentMotionVerificationPackActionDisplayLabels,
    current_move_now_action_start_endpoints: currentMotionVerificationPackActionStartEndpoints,
    current_move_now_action_stop_endpoints: currentMotionVerificationPackActionStopEndpoints,
    current_move_now_primary_action_id: currentMotionVerificationPackPrimaryAction?.id ?? "none",
    current_move_now_primary_action_display_label: currentMotionVerificationPackPrimaryAction?.display_label
      ?? currentMotionVerificationPackPrimaryAction?.label
      ?? "无待执行动作",
    current_move_now_primary_focus_source_card_id: currentSafetyConfirmQueuePrimaryFocusSourceCardId,
    current_move_now_primary_focus_kind: currentSafetyConfirmQueuePrimaryFocusKind,
    current_move_now_primary_focus_button_label: currentSafetyConfirmQueuePrimaryFocusButtonLabel,
    current_move_now_mapping_blockers_plain: goalSummary.mapping_blockers_plain,
    current_move_now_free_move_allowed_while_mapping_blocked: currentGoalFreeMoveAllowedWhileMappingBlocked,
    current_move_now_requires_safety_confirm: false,
    current_move_now_minimal_precheck_safety_only: currentMotionVerificationPackMinimalPrecheckSafetyOnly,
    current_move_now_camera_preflight_required: currentMotionVerificationPackCameraPreflightRequired,
    current_move_now_radar_preflight_required: currentMotionVerificationPackRadarPreflightRequired,
    current_move_now_route_wysiwyg_preflight_required: currentMotionVerificationPackRouteWysiwygPreflightRequired,
    current_move_now_sends_motion_when_clicked: false,
    current_move_now_sends_motion_when_executed: currentMotionVerificationPackActionIds.length > 0,
    current_move_now_starts_nav2_when_clicked: false,
    current_move_now_starts_manual_when_clicked: false,
    current_move_now_starts_keyboard_when_clicked: false,
    current_move_now_starts_free_roam_when_clicked: false,
    ...
    current_minimal_precheck_pack_requires_safety_confirm: false,
    current_minimal_precheck_pack_safety_confirm_required: false,
    ...
    current_keyboard_action_ready_for_safety_confirm: keyboardRunbookItem?.ready ?? liveClosureSummary.keyboard_continuous_ready,
    current_keyboard_action_requires_safety_confirm: false,
    ...
    current_keyboard_action_post_hold_readback_sequence_labels: keyboardPostHoldReadbackSequenceLabels,
    ...
    current_keyboard_control_pack_requires_safety_confirm: false,
    current_keyboard_control_pack_safety_confirm_required: false,
    ...
    // 键盘连续手控短别名：只收拢当前键盘包，方便现场脚本判断“启用不发车、按住才动、松开后读回”。
    keyboard_hold_status: currentKeyboardControlPackStatus,
    keyboard_hold_plain: currentKeyboardControlPackNextActionPlain,
    keyboard_hold_action_id: keyboardRunbookItem?.id ?? "hold_keyboard",
    keyboard_hold_display_label: keyboardRunbookItem?.display_label ?? "键盘连续手控",
    keyboard_hold_ready: keyboardRunbookItem?.ready ?? liveClosureSummary.keyboard_continuous_ready,
    keyboard_hold_requires_safety_confirm: false,
    keyboard_hold_minimal_precheck_safety_only: keyboardRunbookItem?.minimal_precheck_safety_only ?? liveClosureSummary.keyboard_continuous_minimal_precheck_safety_only,
    keyboard_hold_start_endpoint: keyboardActionStartEndpoint,
    keyboard_hold_stop_endpoint: keyboardActionStopEndpoint,
    keyboard_hold_readback_endpoints: keyboardActionAcceptanceEndpoints,
    keyboard_hold_post_hold_readback_endpoints: keyboardPostHoldReadbackEndpoints,
    keyboard_hold_post_hold_readback_sequence_labels: keyboardPostHoldReadbackSequenceLabels,
    keyboard_hold_missing_evidence: keyboardActionMissingEvidence,
    keyboard_hold_missing_evidence_labels: keyboardActionMissingEvidence.map(currentControlPackEvidenceLabel),
    keyboard_hold_pulse_interval_ms: liveClosureSummary.keyboard_continuous_pulse_interval_ms,
    keyboard_hold_pulse_duration_ms: liveClosureSummary.keyboard_continuous_pulse_duration_ms,
    keyboard_hold_stop_triggers: liveClosureSummary.keyboard_continuous_stop_triggers,
    keyboard_hold_wheel_feedback_acceptance: liveClosureSummary.keyboard_continuous_wheel_feedback_acceptance,
    ...
    current_free_move_action_acceptance_plain: freeMoveRunbookItem?.acceptance_plain ?? "启动后读取 free-roam latest、地图预览和 summary；相机、雷达不作为自由移动发车前置。",
    current_free_move_action_ready_for_safety_confirm: freeMoveRunbookItem?.ready ?? liveClosureSummary.free_move_start_ready,
    current_free_move_action_requires_safety_confirm: false,
    ...
    current_free_move_control_pack_requires_safety_confirm: false,
    current_free_move_control_pack_safety_confirm_required: false,
    ...
    // 自由移动启动短别名：只收拢当前 free move 包，方便现场脚本判断“可先自由动，建图另等相机/雷达”。
    free_move_start_status: currentFreeMoveControlPackStatus,
    free_move_start_plain: currentFreeMoveControlPackNextActionPlain,
    free_move_start_action_id: freeMoveRunbookItem?.id ?? "start_free_move",
    free_move_start_display_label: freeMoveRunbookItem?.display_label ?? "自由自助移动",
    free_move_start_start_endpoint: freeMoveActionStartEndpoint,
    free_move_start_stop_endpoint: freeMoveActionStopEndpoint,
    free_move_start_latest_endpoint: liveClosureSummary.fixed_free_roam_latest_endpoint,
    free_move_start_readback_endpoints: freeMoveActionAcceptanceEndpoints,
    free_move_start_post_start_readback_endpoints: freeMoveActionAcceptanceEndpoints,
    free_move_start_post_start_readback_sequence_labels: freeMovePostStartReadbackSequenceLabels,
    free_move_start_required_success_markers: freeMoveActionRequiredSuccessMarkers,
    free_move_start_missing_evidence: freeMoveActionMissingEvidence,
    free_move_start_missing_evidence_labels: freeMoveActionMissingEvidence.map(currentControlPackEvidenceLabel),
    free_move_start_proof_status: freeMoveRunbookItem?.proof_status ?? "blocked",
    free_move_start_is_ready: freeMoveRunbookItem?.ready ?? liveClosureSummary.free_move_start_ready,
    free_move_start_is_running: liveClosureSummary.free_roam_motion_ready,
    free_move_start_is_complete: freeMoveComplete,
    free_move_start_requires_safety_confirm: false,
    free_move_start_minimal_precheck_safety_only: freeMoveRunbookItem?.minimal_precheck_safety_only ?? liveClosureSummary.free_move_minimal_precheck_safety_only,
    free_move_start_camera_preflight_required: liveClosureSummary.free_move_camera_preflight_required,
    free_move_start_radar_preflight_required: liveClosureSummary.free_move_radar_preflight_required,
    free_move_start_without_camera_allowed: liveClosureSummary.free_move_without_camera_allowed,
    free_move_start_without_radar_allowed: liveClosureSummary.free_roam_motion_without_radar_allowed,
    free_move_start_blocked_by_camera_wysiwyg: liveClosureSummary.free_move_blocked_by_camera_wysiwyg,
    free_move_start_blocked_by_radar_wysiwyg: liveClosureSummary.free_move_blocked_by_radar_wysiwyg,
    free_move_start_mapping_start_ready: liveClosureSummary.mapping_start_ready,
    free_move_start_mapping_start_missing_reasons: liveClosureSummary.mapping_start_missing_reasons,
    free_move_start_post_start_readback_refreshes_latest: freeMovePostStartReadbackRefreshesLatest,
    free_move_start_post_start_readback_refreshes_map_preview: freeMovePostStartReadbackRefreshesMapPreview,
    free_move_start_post_start_readback_refreshes_summary: freeMovePostStartReadbackRefreshesSummary,
    free_move_start_sends_motion_when_clicked: false,
    free_move_start_sends_motion_when_executed: true,
    free_move_start_starts_nav2_when_clicked: false,
    free_move_start_starts_manual_when_clicked: false,
    free_move_start_starts_keyboard_when_clicked: false,
    free_move_start_starts_free_roam_when_clicked: false,
    free_move_start_starts_free_roam_when_executed: true,
    free_move_start_starts_map_runtime_when_clicked: false,
    free_move_start_submits_delivery_when_clicked: false,
    free_move_start_stops_motion_when_clicked: false,
    free_move_start_readback_sends_motion: false,
    free_move_start_readback_starts_nav2: false,
    free_move_start_readback_starts_manual: false,
    free_move_start_readback_starts_keyboard: false,
    free_move_start_readback_starts_free_roam: false,
    free_move_start_readback_starts_map_runtime: false,
    free_move_start_readback_submits_delivery: false,
    free_move_start_readback_stops_motion: false,
    ...
    // 建图启动门禁短别名：强调摄像头+雷达都 ready 才能建图，自由移动不被建图缺口挡住。
    mapping_start_gate_status: currentMappingControlPackStatus,
    mapping_start_gate_plain: currentMappingControlPackNextActionPlain,
    mapping_start_gate_action_id: mappingRunbookItem?.id ?? "start_mapping_when_sensors_ready",
    mapping_start_gate_display_label: mappingRunbookItem?.display_label ?? "传感器就绪后建图",
    mapping_start_gate_start_endpoint: mappingActionStartEndpoint,
    mapping_start_gate_stop_endpoint: mappingActionStopEndpoint,
    mapping_start_gate_preview_endpoint: liveClosureSummary.fixed_mapping_preview_endpoint,
    mapping_start_gate_readback_endpoints: mappingActionAcceptanceEndpoints,
    mapping_start_gate_post_start_readback_endpoints: mappingActionAcceptanceEndpoints,
    mapping_start_gate_post_start_readback_sequence_labels: mappingPostStartReadbackSequenceLabels,
    mapping_start_gate_required_success_markers: mappingActionRequiredSuccessMarkers,
    mapping_start_gate_missing_evidence: mappingActionMissingEvidence,
    mapping_start_gate_missing_evidence_labels: mappingActionMissingEvidence.map(currentControlPackEvidenceLabel),
    mapping_start_gate_proof_status: mappingRunbookItem?.proof_status ?? "blocked",
    mapping_start_gate_is_ready: mappingRunbookItem?.ready ?? liveClosureSummary.mapping_start_ready,
    mapping_start_gate_requires_safety_confirm: mappingRunbookItem?.safety_confirm_required ?? liveClosureSummary.mapping_start_ready,
    mapping_start_gate_safety_confirm_required_when_executed: true,
    mapping_start_gate_minimal_precheck_safety_only: mappingRunbookItem?.minimal_precheck_safety_only ?? true,
    mapping_start_gate_camera_required: liveClosureSummary.mapping_start_requires_camera_first_frame,
    mapping_start_gate_radar_required: liveClosureSummary.mapping_start_requires_lidar_fresh,
    mapping_start_gate_camera_ready: !liveClosureSummary.mapping_camera_blocks_start,
    mapping_start_gate_radar_ready: !liveClosureSummary.mapping_lidar_blocks_start,
    mapping_start_gate_camera_blocks_start: liveClosureSummary.mapping_camera_blocks_start,
    mapping_start_gate_radar_blocks_start: liveClosureSummary.mapping_lidar_blocks_start,
    mapping_start_gate_only_camera_missing: mappingStartOnlyCameraMissing,
    mapping_start_gate_radar_overlay_wysiwyg_complete: radarOverlayWysiwygComplete,
    mapping_start_gate_camera_hardware_action_required: liveClosureSummary.camera_hardware_action_required,
    mapping_start_gate_camera_hardware_action_label: liveClosureSummary.camera_hardware_action_label,
    mapping_start_gate_camera_recovery_next_action_plain: liveClosureSummary.camera_recovery_next_action_plain,
    mapping_start_gate_blocks_free_move: false,
    mapping_start_gate_free_move_allowed_while_blocked: liveClosureSummary.free_move_start_ready && !liveClosureSummary.mapping_start_ready,
    mapping_start_gate_post_start_readback_refreshes_free_roam_latest: mappingPostStartReadbackRefreshesFreeRoamLatest,
    mapping_start_gate_post_start_readback_refreshes_map_preview: mappingPostStartReadbackRefreshesMapPreview,
    mapping_start_gate_post_start_readback_refreshes_summary: mappingPostStartReadbackRefreshesSummary,
    ...
    // 相机首帧恢复短别名：把硬件处理、复测链路和 no-motion 边界压成现场脚本可直接读取的一组字段。
    camera_first_frame_fix_status: currentCameraWysiwygPackStatus,
    camera_first_frame_fix_plain: liveClosureSummary.live_wysiwyg_camera_recovery_next_action_plain,
    camera_first_frame_fix_missing_evidence: currentCameraWysiwygPackMissingEvidence,
    camera_first_frame_fix_missing_evidence_labels: currentCameraWysiwygPackMissingEvidenceLabels,
    camera_first_frame_fix_visible: liveClosureSummary.live_wysiwyg_camera_visible,
    camera_first_frame_fix_first_frame_ready: liveClosureSummary.camera_current_visible,
    camera_first_frame_fix_shared_preview_current_frame_visible: liveClosureSummary.camera_shared_preview_current_frame_visible,
    camera_first_frame_fix_shared_preview_everyone_can_join: liveClosureSummary.camera_shared_preview_everyone_can_join,
    camera_first_frame_fix_source_diagnosis_status: liveClosureSummary.camera_source_diagnosis_status,
    camera_first_frame_fix_source_diagnosis_not_exclusive: liveClosureSummary.camera_source_diagnosis_not_exclusive,
    camera_first_frame_fix_probe_status: liveClosureSummary.camera_first_frame_probe_status,
    camera_first_frame_fix_failure_reason: liveClosureSummary.camera_first_frame_failure_reason,
    camera_first_frame_fix_low_bandwidth_fallback_attempted: readbackSummary.camera.first_frame_probe_low_bandwidth_fallback_attempted,
    camera_first_frame_fix_low_bandwidth_fallback_min_size: readbackSummary.camera.first_frame_probe_low_bandwidth_fallback_min_size,
    camera_first_frame_fix_software_fallback_exhausted: currentCameraWysiwygPackSoftwareFallbackExhausted,
    camera_first_frame_fix_requires_physical_usb_fix: currentCameraWysiwygPackRequiresPhysicalUsbFix,
    camera_first_frame_fix_physical_fix_label: currentCameraWysiwygPackPhysicalFixLabel,
    camera_first_frame_fix_hardware_action_required: liveClosureSummary.camera_hardware_action_required,
    camera_first_frame_fix_hardware_action_label: liveClosureSummary.camera_hardware_action_label,
    camera_first_frame_fix_usb_full_speed_detected: liveClosureSummary.camera_usb_full_speed_detected,
    camera_first_frame_fix_usb_speed: liveClosureSummary.camera_usb_speed,
    camera_first_frame_fix_sequence: liveClosureSummary.live_wysiwyg_camera_recovery_sequence,
    camera_first_frame_fix_sequence_labels: liveClosureSummary.live_wysiwyg_camera_recovery_sequence_labels,
    camera_first_frame_fix_probe_endpoint: liveClosureSummary.fixed_camera_probe_endpoint,
    camera_first_frame_fix_mjpeg_status_endpoint: liveClosureSummary.fixed_camera_mjpeg_status_endpoint,
    camera_first_frame_fix_summary_endpoint: liveClosureSummary.fixed_objective_audit_summary_endpoint,
    camera_first_frame_fix_blocks_wysiwyg: !currentCameraWysiwygVisible,
    camera_first_frame_fix_blocks_mapping_start: liveClosureSummary.camera_blocks_mapping_start,
    camera_first_frame_fix_blocks_free_move: false,
    camera_first_frame_fix_readback_only: true,
    camera_first_frame_fix_no_motion_refresh: true,
    camera_first_frame_fix_sends_motion_when_clicked: false,
    camera_first_frame_fix_starts_camera_exclusive_capture: false,
    camera_first_frame_fix_starts_nav2: false,
    camera_first_frame_fix_starts_manual: false,
    camera_first_frame_fix_starts_keyboard: false,
    camera_first_frame_fix_starts_free_roam: false,
    camera_first_frame_fix_starts_map_runtime: false,
    camera_first_frame_fix_submits_delivery: false,
    camera_first_frame_fix_stops_motion: false,
    ...
    current_radar_map_wysiwyg_pack_source_point_count: liveClosureSummary.radar_overlay_source_point_count,
    current_radar_map_wysiwyg_pack_current_vs_source_plain: liveClosureSummary.radar_overlay_current_vs_source_plain,
    current_radar_map_wysiwyg_pack_next_action_plain: liveClosureSummary.radar_overlay_refresh_next_action_plain,
    current_radar_map_wysiwyg_pack_sequence: liveClosureSummary.radar_overlay_recovery_sequence,
    current_radar_map_wysiwyg_pack_sequence_labels: liveClosureSummary.live_wysiwyg_radar_map_refresh_sequence_labels,
    current_radar_map_wysiwyg_pack_refresh_endpoint: liveClosureSummary.fixed_radar_overlay_refresh_endpoint,
    current_radar_map_wysiwyg_pack_status_endpoint: liveClosureSummary.fixed_live_wysiwyg_radar_status_endpoint,
    current_radar_map_wysiwyg_pack_map_preview_endpoint: liveClosureSummary.fixed_radar_overlay_map_preview_endpoint,
    current_radar_map_wysiwyg_pack_summary_endpoint: liveClosureSummary.fixed_objective_audit_summary_endpoint,
    current_radar_map_wysiwyg_pack_loaded: radarOverlayWysiwygComplete,
    current_radar_map_wysiwyg_pack_needs_refresh: liveClosureSummary.radar_overlay_needs_refresh,
    current_radar_map_wysiwyg_pack_blocks_wysiwyg: liveClosureSummary.radar_overlay_blocks_wysiwyg,
    current_radar_map_wysiwyg_pack_blocks_free_move: false,
    current_radar_map_wysiwyg_pack_readback_only: true,
    current_radar_map_wysiwyg_pack_no_motion_refresh: true,
    current_radar_map_wysiwyg_pack_sends_motion_when_clicked: false,
    current_radar_map_wysiwyg_pack_starts_radar_lifecycle: false,
    current_radar_map_wysiwyg_pack_starts_nav2: false,
    current_radar_map_wysiwyg_pack_starts_manual: false,
    current_radar_map_wysiwyg_pack_starts_keyboard: false,
    current_radar_map_wysiwyg_pack_starts_free_roam: false,
    current_radar_map_wysiwyg_pack_starts_map_runtime: false,
    current_radar_map_wysiwyg_pack_submits_delivery: false,
    current_radar_map_wysiwyg_pack_stops_motion: false,
    ...
    free_move_acceptance_plain: freeMoveRunbookItem?.acceptance_plain ?? "启动后读取 free-roam latest、地图预览和 summary；相机、雷达不作为自由移动发车前置。",
    ...
    map_preview_status: readbackSummary.map.map_current_visible === "true" ? "loaded" : readbackSummary.map.status,
    path_preview_point_count: readbackSummary.map.path_preview_point_count,
    route_target_visible: readbackSummary.map.route_target_visible,
    route_target_current_visible: readbackSummary.map.route_target_visible,
    route_target_source: readbackSummary.map.route_target_source,
    route_target_state: readbackSummary.map.route_target_state,
    robot_pose_status: readbackSummary.map.robot_pose_status,
    ...
    camera_input_signal_check_required: liveClosureSummary.camera_input_signal_check_required,
    camera_input_signal_check_label: liveClosureSummary.camera_input_signal_check_label,
    camera_input_signal_check_plain: liveClosureSummary.camera_input_signal_check_plain,
    ...
    radar_map_points_current_visible: liveClosureSummary.radar_map_points_visible,
    ...
    radar_overlay_next_action_plain: liveClosureSummary.radar_overlay_refresh_next_action_plain,
    ...
    radar_overlay_refresh_sequence: liveClosureSummary.radar_overlay_recovery_sequence,
    radar_overlay_refresh_sequence_labels: liveClosureSummary.live_wysiwyg_radar_map_refresh_sequence_labels,
    ...
    radar_overlay_refresh_starts_nav2: liveClosureSummary.radar_overlay_refresh_starts_nav2,
    radar_overlay_refresh_starts_manual: liveClosureSummary.radar_overlay_refresh_starts_manual,
    radar_overlay_refresh_starts_keyboard: liveClosureSummary.radar_overlay_refresh_starts_keyboard,
    radar_overlay_refresh_starts_free_roam: liveClosureSummary.radar_overlay_refresh_starts_free_roam,
    radar_overlay_refresh_starts_map_runtime: liveClosureSummary.radar_overlay_refresh_starts_map_runtime,
    radar_overlay_refresh_submits_delivery: liveClosureSummary.radar_overlay_refresh_submits_delivery,
    ...
    keyboard_post_hold_readback_sequence_labels: keyboardPostHoldReadbackSequenceLabels,
    ...
    base_summary: readbackSummary.base,
    base_motion_signal_observed: readbackSummary.base.motion_signal_observed,
    base_motion_signal_source: readbackSummary.base.motion_signal_source,
    base_imu_attitude_delta_observed: readbackSummary.base.imu_attitude_delta_observed,
    base_wheel_feedback_lr_nonzero_proven: readbackSummary.base.wheel_feedback_lr_nonzero_proven,
    base_wheel_feedback_nonzero_observed: readbackSummary.base.wheel_feedback_nonzero_observed,
    base_wheel_feedback_latest_raw_left: readbackSummary.base.wheel_feedback_latest_raw_left ?? readbackSummary.base.wheel_feedback_latest_left_speed,
    base_wheel_feedback_latest_raw_right: readbackSummary.base.wheel_feedback_latest_raw_right ?? readbackSummary.base.wheel_feedback_latest_right_speed,
    motion_signal_observed: readbackSummary.base.motion_signal_observed,
    motion_signal_source: readbackSummary.base.motion_signal_source,
    imu_attitude_delta_observed: readbackSummary.base.imu_attitude_delta_observed,
    wheel_feedback_lr_nonzero_proven: readbackSummary.base.wheel_feedback_lr_nonzero_proven,
    wheel_feedback_nonzero_observed: readbackSummary.base.wheel_feedback_nonzero_observed,
    wheel_feedback_latest_raw_left: readbackSummary.base.wheel_feedback_latest_raw_left ?? readbackSummary.base.wheel_feedback_latest_left_speed,
    wheel_feedback_latest_raw_right: readbackSummary.base.wheel_feedback_latest_raw_right ?? readbackSummary.base.wheel_feedback_latest_right_speed,
