...
function buildLiveClosureSummary(
  ...
  const mapDisplayTooSmallNextActionPlain = "PC 首页现在让地图独占首行，真实地图按画布高度优先放大，宽图横向滚动，默认用 1600% 大图显示地图、路线、小车、雷达和目标；图传和 WASD 放到地图下方；需要全局总览点“完整态势”回到 100%，需要局部排障再点“细节放大”到 4800%，或点“进入地图大屏”打开 /map；/map 默认也是 1600% 大图，工具条悬浮在地图上，不再占画布高度，只保留缩放、只读刷新和工程观察入口；建图、保存和其他卡片都会收起；不需要先开 RViz2。";
