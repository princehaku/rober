...
function buildLiveClosureSummary(
  ...
  const mapDisplayTooSmallNextActionPlain = "PC 首页现在让地图独占首行，默认用 100% 完整态势显示真实地图、路线、小车、雷达和目标，图传和 WASD 放到地图下方；需要局部排障再点“细节放大”到 1200%，或点“进入地图大屏”打开 /map；/map 默认也是 100% 完整态势，只保留缩放、只读刷新和工程观察入口；建图、保存和其他卡片都会收起；不需要先开 RViz2。";
