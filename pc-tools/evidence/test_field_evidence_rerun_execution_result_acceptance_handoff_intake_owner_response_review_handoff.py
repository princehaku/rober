#!/usr/bin/env python3
"""acceptance handoff intake owner response review-handoff gate 的围栏测试。"""
...
import field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff as handoff  # noqa: E402


# 测试约束 01：fixture 只表达 safe owner response review decision 与 reviewer handoff metadata。
...
class FieldEvidenceRerunExecutionResultAcceptanceHandoffIntakeReviewHandoffTest(unittest.TestCase):
    ...
    def owner_response_review_decision_summary(self, evidence_ref: str, ready: bool = True) -> dict:
        # 样本沿用上一轮 owner response review decision summary 的安全字段，不夹带 raw artifact。
        status = handoff.READY_SOURCE_DECISION if ready else "review_needs_owner_rework"
        return {
            "schema": "trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_decision_summary.v1",
            "schema_version": 1,
            ...
            "status": status,
            "owner_response_review_decision": status,
    ...
    def handoff_packet(self, evidence_ref: str, materials: list[str] | None = None) -> dict:
        ...
        return {
            "schema": "trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff_packet.v1",
            "schema_version": 1,
            "source": "software_proof",
            "safe_evidence_ref": evidence_ref,
            "evidence_ref": evidence_ref,
            ...
            "next_required_evidence": [
                "handoff owner response review decision metadata only",
                "keep PR #5 hardware material pending",
            ],
            "not_proven": "not_proven",
            "safe_to_control": False,
            "delivery_success": False,
    ...
    def build(self, root: Path, decision_payload: dict | str, packet_payload: dict | str, evidence_ref: str = "review-handoff-001") -> tuple[dict, dict, int]:
        ...
        decision_path = self.write_json(root, "owner_response_review_decision.json", decision_payload)
        packet_path = self.write_json(root, "handoff_packet.json", packet_payload)
        return handoff.build_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff(
            str(decision_path),
            str(packet_path),
            evidence_ref,
        )
    ...
    def test_ready_acceptance_review_handoff_not_proven(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            artifact, summary, exit_code = self.build(
                root,
                {"payload": {"summary": self.owner_response_review_decision_summary("review-handoff-001")}},
                {"payload": {"handoff_packet": self.handoff_packet("review-handoff-001")}},
            )

        self.assertEqual(exit_code, 0)
        self.assertEqual(artifact["schema"], "trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff.v1")
        self.assertEqual(summary["schema"], "trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff_summary.v1")
        self.assertEqual(
        ...
        self.assertEqual(artifact["handoff_status"], "ready_for_owner_response_review_handoff_not_proven")
        ...
        self.assertIn("blocked_missing_owner_response_review_decision", artifact["allowed_handoff_states"])
        ...
        self.assertEqual(summary["owner_support_reviewer_routing"]["owner"], "field-owner")
        self.assertEqual(summary["owner_support_reviewer_routing"]["support"], "support")
        self.assertEqual(summary["owner_support_reviewer_routing"]["reviewer"], "reviewer")
    ...
    def test_owner_rework_when_handoff_packet_missing_material(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            artifact, summary, exit_code = self.build(
                root,
                self.owner_response_review_decision_summary("review-handoff-002"),
                self.handoff_packet("review-handoff-002", [item for item in handoff.HANDOFF_CHECKLIST if item != "true phone/browser evidence"]),
                "review-handoff-002",
            )
    ...
    def test_missing_handoff_fields_need_owner_rework(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            packet = self.handoff_packet("review-handoff-002b")
            packet.pop("support")
            packet.pop("next_required_evidence")
            artifact, summary, exit_code = self.build(
                root,
                self.owner_response_review_decision_summary("review-handoff-002b"),
                packet,
                "review-handoff-002b",
            )
        ...
        self.assertEqual(artifact["handoff_status"], "handoff_needs_owner_rework")
        self.assertNotEqual(exit_code, 0)
        self.assertIn("missing_handoff_support", artifact["material_status"]["owner_rework_reasons"])
        self.assertIn("missing_handoff_next_required_evidence", summary["material_status"]["owner_rework_reasons"])
    ...
    def test_evidence_ref_mismatch_is_blocked_nonzero(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            artifact, summary, exit_code = self.build(
                root,
                self.owner_response_review_decision_summary("review-handoff-003"),
    ...
    def test_unsafe_and_success_claims_reject_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            artifact, _summary, exit_code = self.build(
                root,
                self.owner_response_review_decision_summary("review-handoff-004"),
                unsafe,
                "review-handoff-004",
            )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            success_artifact, success_summary, success_exit = self.build(
                root,
                self.owner_response_review_decision_summary("review-handoff-005"),
                success,
                "review-handoff-005",
            )
    ...
    def test_missing_owner_response_review_decision_is_blocked_nonzero(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            packet_path = self.write_json(root, "handoff_packet.json", self.handoff_packet("review-handoff-006"))
            artifact, summary, exit_code = handoff.build_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff(
                str(root / "missing_owner_response_review_decision.json"),
                str(packet_path),
                "review-handoff-006",
            )

        self.assertEqual(artifact["handoff_status"], "blocked_missing_owner_response_review_decision")
        self.assertNotEqual(exit_code, 0)
        self.assertIn("owner_response_review_decision_json_missing", artifact["handoff_reasons"])
        self.assertFalse(artifact["delivery_success"])
        self.assertFalse(summary["primary_actions_enabled"])
