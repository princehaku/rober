#!/usr/bin/env python3
"""生成 acceptance handoff intake owner response review-handoff。

该 PC-only gate 接在
`field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_decision`
之后，只读取上一轮 owner response review decision artifact / summary / Robot diagnostics safe
alias 或 wrapper/nested JSON，并读取 field owner/support/reviewer 的 safe handoff
packet。输出只表示 owner response review decision metadata 可交给下一步人工复核交接，不读取真实
...
import field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_decision as owner_response_review_decision
import route_task_field_retest_material_pack as material_pack


HANDOFF_SCHEMA = "trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff.v1"
HANDOFF_SUMMARY_SCHEMA = "trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff_summary.v1"
HANDOFF_PACKET_SCHEMA = "trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff_packet.v1"
SCHEMA_VERSION = 1
HANDOFF_BOUNDARY = "software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff_gate"

# 上游必须是上一轮 owner-response review-decision，不能跳过人工复核决策阶段。
SOURCE_SCHEMAS = {
    owner_response_review_decision.SCHEMA,
    owner_response_review_decision.SUMMARY_SCHEMA,
    owner_response_review_decision.ROBOT_ALIAS,
    f"trashbot.{owner_response_review_decision.ROBOT_ALIAS}.v1",
}
SOURCE_BOUNDARIES = {owner_response_review_decision.EVIDENCE_BOUNDARY}
READY_SOURCE_DECISION = owner_response_review_decision.READY

READY_HANDOFF = "ready_for_owner_response_review_handoff_not_proven"
...
BLOCKED_MISSING_REVIEW_DECISION = "blocked_missing_owner_response_review_decision"
...
HANDOFF_CHECKLIST = owner_response_review_decision.REQUIRED_REVIEW_MATERIALS
NOT_PROVEN = (
...
BOUNDARY_NOTE = (
    ...
    "ready_for_owner_response_review_handoff_not_proven; handoff_needs_owner_rework; "
    "handoff_evidence_ref_mismatch; handoff_unsafe_rejected; blocked_missing_owner_response_review_decision"
)

# 设计约束 01：本 gate 只消费上一轮 owner response review decision safe output。
...
# 设计约束 23：上游 owner response review decision 未 ready 时不能从 packet 推导 ready。
...
def _source_candidates(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for key in (
        ...
        "acceptance_handoff_intake_owner_response_review_decision",
        "acceptance_handoff_intake_owner_response_review_decision_summary",
...
def _packet_candidates(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    candidates = [payload]
    for key in (
        "field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff_packet",
...
def _source_owner_response_review_decision(source: dict[str, Any]) -> str:
    ...
    return material_pack._safe_text(
        _first_text(
            source.get("owner_response_review_decision"),
            source.get("status"),
            robot.get("owner_response_review_decision"),
            robot.get("status"),
            mobile.get("owner_response_review_decision"),
            mobile.get("status"),
            safe_copy.get("owner_response_review_decision"),
...
def _has_forbidden_proof_claim(value: Any) -> bool:
    ...
    for term in HANDOFF_CHECKLIST:
        encoded = encoded.replace(term, "<allowed_handoff_material_category>")
...
def _has_success_or_control_claim(value: Any) -> bool:
    # 先擦除允许的 checklist 类别名，避免 "true route/elevator field pass" 作为待补类别被误判。
    return _has_forbidden_proof_claim(value)
...
def _packet_material_status(packet: dict[str, Any]) -> dict[str, Any]:
    ...
    safe_copy = _dict(packet, "safe_copy")
    owner = material_pack._safe_text(_first_text(packet.get("owner"), packet.get("field_owner"), safe_copy.get("owner"), default=""))
    support = material_pack._safe_text(_first_text(packet.get("support"), packet.get("support_owner"), safe_copy.get("support"), default=""))
    reviewer = material_pack._safe_text(_first_text(packet.get("reviewer"), packet.get("review_owner"), safe_copy.get("reviewer"), default=""))
    accepted = []
    for key in (
        "accepted_materials",
    ...
    if not owner:
        rework.append("missing_handoff_owner")
    if not support:
        rework.append("missing_handoff_support")
    if not reviewer:
        rework.append("missing_handoff_reviewer")
    if not _safe_list(packet.get("next_required_evidence") or safe_copy.get("next_required_evidence")):
        rework.append("missing_handoff_next_required_evidence")
    return {
...
def _source_lineage(source: dict[str, Any], packet: dict[str, Any]) -> dict[str, str]:
    ...
    lineage = {
        "source_owner_response_review_decision_schema": material_pack._safe_text(source.get("schema", "")),
        "source_owner_response_review_decision_status": _source_owner_response_review_decision(source),
    ...
    if isinstance(safe_lineage, dict):
        for key, value in safe_lineage.items():
            ...
            if text:
                lineage[f"owner_response_review_decision_{material_pack._safe_text(key)}"] = text
    return lineage
...
def _handoff_decision(
    ...
    if source_state["schema_status"] != "supported":
        return BLOCKED_MISSING_REVIEW_DECISION, ["unsupported_owner_response_review_decision_schema_or_boundary"], 2
    ...
    if source_review_status != READY_SOURCE_DECISION:
        return HANDOFF_NEEDS_OWNER_REWORK, ["owner_response_review_decision_not_ready_for_review_handoff"], 3
    ...
    return READY_HANDOFF, ["owner_response_review_decision_and_owner_support_reviewer_handoff_packet_ready_not_proven"], 0
...
def _next_required_evidence(state: str, evidence_ref: str, material_status: dict[str, Any], reasons: list[str]) -> list[str]:
    ...
    if state == HANDOFF_NEEDS_OWNER_REWORK:
        ...
        return required or [f"rerun owner/support/reviewer handoff packet after previous owner response review decision emits {READY_SOURCE_DECISION} for evidence_ref={ref}", *reasons]
    if state == HANDOFF_EVIDENCE_REF_MISMATCH:
        return [f"rerun owner response review decision and handoff packet so all summaries share evidence_ref={ref}"]
    ...
    return [f"provide supported acceptance handoff intake owner response review decision artifact or summary for evidence_ref={ref}", *reasons]
...
def _owner_handoff(state: str, evidence_ref: str, checklist: list[dict[str, Any]], next_required_evidence: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "reviewer_boundary": "acceptance_handoff_intake_owner_response_review_handoff_only_not_proven",
...
def _owner_support_reviewer_routing(state: str, evidence_ref: str, material_status: dict[str, Any], next_required_evidence: list[str]) -> dict[str, Any]:
    # routing 只保留 owner/support/reviewer 的短标识，便于后续接 Robot/mobile safe summary。
    packet_routing = material_status.get("routing") if isinstance(material_status.get("routing"), dict) else {}
    return {
        "owner": material_pack._safe_text(packet_routing.get("owner") or "field owner"),
        "support": material_pack._safe_text(packet_routing.get("support") or "support"),
        "reviewer": material_pack._safe_text(packet_routing.get("reviewer") or "reviewer"),
        "handoff_status": state,
        "safe_evidence_ref": evidence_ref or "<same_evidence_ref>",
        "evidence_ref": evidence_ref or "<same_evidence_ref>",
        "next_required_evidence": next_required_evidence,
...
def _rerun_commands(evidence_ref: str) -> list[str]:
    ...
    return [
        ...
        f"python3 pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff.py --owner-response-review-decision-json <owner_response_review_decision.json> --handoff-packet-json <handoff_packet.json> --evidence-ref {ref}",
...
def _safe_copy(
    ...
    return {
        ...
        "owner_support_reviewer_routing": routing,
        "next_required_evidence": next_required_evidence,
...
def build_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff(
    owner_response_review_decision_json: str,
    handoff_packet_json: str,
    evidence_ref: str = "",
) -> tuple[dict[str, Any], dict[str, Any], int]:
    """读取 owner response review decision 与 handoff packet，生成 fail-closed review handoff。"""
    source_payload, source_load_issue = _load_json(owner_response_review_decision_json, "owner_response_review_decision")
    ...
    source_review_status = _source_owner_response_review_decision(source) if source else "missing"
    ...
    success_or_control_claim = bool(source_payload or packet_payload) and (
        ...
        or _has_success_or_control_claim(packet)
    ...
    routing = _owner_support_reviewer_routing(state, effective_ref, material_status, next_required_evidence)
    ...
    source_summary = {
        **source_state,
        ...
        "owner_response_review_decision": source_review_status,
    ...
    artifact = {
        ...
        "rerun_commands": rerun_commands,
        "safe_copy": safe_copy,
        "field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff_summary": summary,
    ...
    if material_pack._has_forbidden_copy(artifact) or material_pack._has_forbidden_copy(summary):
        ...
        artifact["field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff_summary"] = summary
        exit_code = 5
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Generate a field evidence rerun acceptance handoff intake owner response review-handoff artifact")
    parser.add_argument("--owner-response-review-decision-json", required=True, help="acceptance handoff intake owner response review decision artifact, summary, or wrapper JSON")
    ...
    artifact, summary, exit_code = build_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff(
    ...
    if args.once_json or not (args.output or args.summary_output):
        ...
        print(f"field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff: artifact_file:{material_pack._safe_ref(args.output)}")
        if args.summary_output:
            print(f"acceptance_handoff_intake_owner_response_review_handoff_summary_file: {material_pack._safe_ref(args.summary_output)}")
