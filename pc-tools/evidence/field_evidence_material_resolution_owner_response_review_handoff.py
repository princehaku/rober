#!/usr/bin/env python3
"""生成 owner response review handoff 的 PC-only evidence gate。

该 gate 只消费上一轮
`field_evidence_material_resolution_owner_response_review_decision` 的 safe
artifact、summary、Robot alias 或常见 wrapper。输出给 support、field owner 与
reviewer 的 handoff package，但仍固定为 Docker-only software proof，不证明真实
cloud、phone、field pass、HIL、delivery success、PR resolution 或 OKR lift。
"""

from __future__ import annotations

# 设计约束 01：只读取上一轮 review-decision safe surface，不读取 raw 现场材料。
# 设计约束 02：handoff ready 只表示人工复核包可转交，不表示 owner 接受或现场完成。
# 设计约束 03：source=software_proof、not_proven 和三个 false flag 必须逐层固定。
# 设计约束 04：safe evidence_ref 是跨 owner-response 链路的唯一串联主键。
# 设计约束 05：缺输入、坏 JSON、unsupported schema 或 wrong boundary 一律 blocked。
# 设计约束 06：输入出现 success/control/cloud/phone/HIL/PR resolution claim 必须 rejected。
...
import field_evidence_material_resolution_owner_response_review_decision as decision_gate


SCHEMA = "trashbot.field_evidence_material_resolution_owner_response_review_handoff.v1"
SUMMARY_SCHEMA = "trashbot.field_evidence_material_resolution_owner_response_review_handoff_summary.v1"
ROBOT_ALIAS = "robot_diagnostics_field_evidence_material_resolution_owner_response_review_handoff_summary"
SCHEMA_VERSION = 1
...
HANDOFF_READY = "ready_for_material_review_handoff_not_proven"
HANDOFF_NEEDS_MORE = "needs_owner_more_evidence_handoff_not_proven"
HANDOFF_REJECTED = "rejected_unsafe_owner_response_handoff_not_proven"
HANDOFF_BLOCKED = "blocked_missing_owner_response_intake_handoff_not_proven"
HANDOFF_STATUSES = (HANDOFF_READY, HANDOFF_NEEDS_MORE, HANDOFF_REJECTED, HANDOFF_BLOCKED)
...
NOT_PROVEN_ITEMS = (
    ...
    "real_field_route_or_elevator_pass",
    "verified_terminal_result",
...
def _read_json(path: str) -> tuple[dict[str, Any], str]:
    ...
    if not path:
        return {}, "owner_response_review_decision_input_not_provided"
    try:
        payload = json.loads(Path(path).expanduser().read_text(encoding="utf-8"))
    except FileNotFoundError:
        return {}, "owner_response_review_decision_input_missing"
    except json.JSONDecodeError:
        return {}, "owner_response_review_decision_input_bad_json"
    except (OSError, UnicodeDecodeError):
        return {}, "owner_response_review_decision_input_read_error"
    if not isinstance(payload, dict):
        return {}, "owner_response_review_decision_input_not_object"
    return payload, ""
...
def _find_source(payload: dict[str, Any]) -> dict[str, Any]:
    # 优先选择 schema/capability 命中的 owner-response review-decision surface。
...
def _schema_supported(normalized: dict[str, Any]) -> bool:
    # schema 或 capability 命中，并且边界必须是上一轮 owner-response decision gate。
...
def _normalize_source(payload: dict[str, Any], read_issue: str) -> dict[str, Any]:
    ...
    source_decision = _safe_text(source.get("review_decision") or source.get("decision") or source.get("field_evidence_material_resolution_owner_response_review_decision"))
    return {
        ...
        "accepted_materials": _safe_list(source.get("accepted_materials")),
        "missing_required_materials": _safe_list(source.get("missing_materials") or source.get("missing_required_materials")),
        "rejected_unsafe_materials": _safe_list(source.get("rejected_materials")) + _safe_list(source.get("unsafe_materials")),
        "decision_reasons": _safe_list(source.get("decision_reasons") or source.get("handoff_reasons")),
...
def _handoff_status(normalized: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    if not _schema_supported(normalized):
        return HANDOFF_BLOCKED, ["unsupported_owner_response_review_decision_schema_or_boundary"]
    ...
    if source_decision == decision_gate.ACCEPTED:
        return HANDOFF_READY, ["owner_response_review_decision_ready_for_material_review_handoff_only"]
    if source_decision == decision_gate.NEEDS_MORE:
        return HANDOFF_NEEDS_MORE, ["owner_response_review_decision_requires_more_evidence"]
    if source_decision == decision_gate.REJECTED_UNSAFE:
        return HANDOFF_REJECTED, ["owner_response_review_decision_rejected_unsafe_material"]
    if source_decision == decision_gate.BLOCKED_MISSING_INTAKE:
        return HANDOFF_BLOCKED, ["owner_response_review_decision_blocked_missing_intake"]
    return HANDOFF_REJECTED, ["unknown_owner_response_review_decision"]
...
def _next_required(normalized: dict[str, Any], handoff_status: str, reasons: list[str]) -> list[str]:
    ...
    if handoff_status == HANDOFF_READY:
        return [
            "support packages the sanitized owner-response review handoff under the same safe evidence_ref",
            "field owner and reviewer check material readiness without marking field/cloud/phone/HIL/delivery success",
        ]
    if handoff_status == HANDOFF_NEEDS_MORE:
        return normalized["missing_required_materials"] or normalized["next_required_evidence"] or [
            "backfill missing owner-response material under the same safe evidence_ref"
        ]
    if handoff_status == HANDOFF_REJECTED:
        return normalized["rejected_unsafe_materials"] or reasons
    return ["provide_supported_field_evidence_material_resolution_owner_response_review_decision_safe_artifact_summary_or_robot_alias"]


def _field_owner_handoff(normalized: dict[str, Any], handoff_status: str, reasons: list[str], next_required: list[str]) -> dict[str, Any]:
    # field owner 只拿补证/重提交流程，不拿 raw material 或控制入口。
    if handoff_status == HANDOFF_READY:
        action = "confirm_material_review_queue_under_same_safe_evidence_ref_without_success_claim"
    elif handoff_status == HANDOFF_NEEDS_MORE:
        action = "backfill_missing_owner_response_material_under_same_safe_evidence_ref"
    elif handoff_status == HANDOFF_REJECTED:
        action = "resubmit_sanitized_owner_response_without_unsafe_success_control_raw_or_resolution_claims"
    else:
        action = "provide_owner_response_review_decision_safe_summary_before_handoff"
    return {
        **_safe_flags(),
        "role": "field owner",
...
def _reviewer_handoff(normalized: dict[str, Any], handoff_status: str, reasons: list[str], next_required: list[str]) -> dict[str, Any]:
    ...
    if handoff_status == HANDOFF_READY:
        action = "review_material_readiness_later_without_marking_pr_or_delivery_resolved"
    elif handoff_status == HANDOFF_NEEDS_MORE:
        action = "wait_for_owner_backfill_before_material_review"
    elif handoff_status == HANDOFF_REJECTED:
        action = "reject_unsafe_owner_response_until_sanitized"
    else:
        action = "wait_for_supported_safe_review_decision_source"
    return {
        **_safe_flags(),
        "role": "reviewer",
        "handoff_status": handoff_status,
...
def build_field_evidence_material_resolution_owner_response_review_handoff(
    owner_response_review_decision_json: str,
) -> tuple[dict[str, Any], dict[str, Any], int]:
    """构建 owner-response review handoff；ready 也只是 not_proven。"""
    payload, read_issue = _read_json(owner_response_review_decision_json)
    ...
    common = {
        **_safe_flags(),
        ...
        "rejected_unsafe_materials": normalized["rejected_unsafe_materials"],
        "next_required_evidence": next_required,
    ...
    artifact["field_evidence_material_resolution_owner_response_review_handoff_summary"] = summary
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build trashbot.field_evidence_material_resolution_owner_response_review_handoff.v1 from a sanitized "
            "field_evidence_material_resolution_owner_response_review_decision artifact, summary, or Robot alias. Keeps "
            "source=software_proof, not_proven, delivery_success=false, primary_actions_enabled=false, "
            "safe_to_control=false."
        )
    )
    parser.add_argument("--input", default="", help="sanitized owner response review decision artifact, summary, or Robot alias JSON")
    ...
    artifact, summary, exit_code = build_field_evidence_material_resolution_owner_response_review_handoff(args.input)
    if args.output_dir:
        _write_json(args.output_dir / "field_evidence_material_resolution_owner_response_review_handoff.json", artifact)
        _write_json(args.output_dir / "field_evidence_material_resolution_owner_response_review_handoff_summary.json", summary)
    if args.output:
