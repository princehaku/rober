#!/usr/bin/env python3
"""生成 route/task field retest acceptance execution rerun queue artifact。

该 PC-only gate 只读取上一轮 acceptance execution handoff intake artifact、
summary 或 wrapper/nested JSON，并可选读取 queue request JSON。输出仅代表
Docker/local software proof 的受控复跑排队材料，不读取现场材料、不触发复跑。
"""
...
# 本 gate 位于 handoff intake 之后，schema/status 必须与上一轮 intake 区分。
QUEUE_SCHEMA = "trashbot.route_task_field_retest_acceptance_execution_rerun_queue.v1"
QUEUE_SUMMARY_SCHEMA = "trashbot.route_task_field_retest_acceptance_execution_rerun_queue_summary.v1"
SCHEMA_VERSION = 1
QUEUE_BOUNDARY = "software_proof_docker_route_task_field_retest_acceptance_execution_rerun_queue_gate"

# 上游只接受 handoff intake artifact 或 summary，避免跳过 owner acknowledgement 层。
SOURCE_SCHEMAS = {
...
SOURCE_BOUNDARY = "software_proof_docker_route_task_field_retest_acceptance_execution_handoff_intake_gate"
READY_SOURCE_STATUS = "ready_for_controlled_field_rerun_queue"

# queue request 是可选输入；ack 只代表 owner 允许进入队列，不代表现场通过。
...
BOUNDARY_NOTE = (
    ...
    "queued_for_controlled_field_rerun_not_proven; needs_owner_ack_before_queue; "
    "needs_acceptance_execution_rerun_queue_backfill; evidence_ref_mismatch_rerun_queue; "
    "blocked_unsafe_rerun_queue; blocked_unsupported_handoff_intake; "
    "not_proven; delivery_success=false; primary_actions_enabled=false"
)

# 设计约束 01：本 gate 只消费 handoff intake，不读取现场材料目录。
# 设计约束 02：队列状态只表达受控复跑候选，不证明复跑已执行。
# 设计约束 03：queued 状态仍保持 not_proven 和动作禁用。
# 设计约束 04：source schema/boundary 不匹配必须 fail closed。
# 设计约束 05：缺 safe evidence_ref 不能进入受控复跑队列。
# 设计约束 06：same_evidence_ref_required 固定为 true，保护证据链主键。
# 设计约束 07：queue request 与 source evidence_ref 不一致必须重跑。
# 设计约束 08：owner ack 可来自上游 intake 或 queue request，但不能代表现场结果。
# 设计约束 09：unsafe copy、凭证、runtime topic、硬件 transport 和路径必须阻断。
# 设计约束 10：success/control claim 必须阻断，不能进入 Robot/mobile。
# 设计约束 11：summary 是下游白名单，不复制完整 source 或 queue request。
# 设计约束 12：wrapper/nested JSON 只递归白名单 key，避免 raw payload 误采信。
# 设计约束 13：source 非 ready 时进入 rerun queue backfill。
# 设计约束 14：queue request 只允许 owner-safe metadata 和 requested reason。
# 设计约束 15：safe rerun hint 只给 PC operator，不访问 ROS graph 或硬件。
# 设计约束 16：输出递归脱敏，blocked artifact 也不泄漏 raw material。
# 设计约束 17：exit code 保持 0，让 blocked queue 也能落盘审计。
...
# 设计约束 22：queued 只代表排队材料可复核，不代表 owner 已完成现场复跑。
# 设计约束 23：backfill 状态必须说明是 handoff intake 材料不足。
...
# 成功或动作授权不能通过 queue request 进入下游。
...
def _load_json(path: str, label: str) -> tuple[dict[str, Any], str]:
    # queue request 是可选输入；handoff intake 缺失则 fail closed。
    if not path:
...
def _find_source(payload: dict[str, Any]) -> dict[str, Any]:
    # 优先选择 schema 命中的 handoff intake 对象；否则保留顶层用于 unsupported 解释。
    keys = (
...
def _find_queue_request(payload: dict[str, Any]) -> dict[str, Any]:
    # queue request 支持常见 wrapper，但只提取 owner-safe 字段。
    keys = (
        "queue_request",
        "rerun_queue_request",
        "owner_queue_request",
...
def _source_status(source: dict[str, Any]) -> str:
    # handoff_intake_status 是上游主键；status 只用于兼容 summary。
    safe_copy = _dict(source, "safe_copy")
    return _safe_text(_first_text(source.get("handoff_intake_status"), source.get("status"), safe_copy.get("handoff_intake_status"), default=""))
...
def _source_ack_state(source: dict[str, Any]) -> str:
    # 上游 handoff intake summary 的 ack 是首选 owner 确认来源。
    safe_copy = _dict(source, "safe_copy")
    return _owner_ack_state(source) if _owner_ack_state(source) != "missing" else _owner_ack_state(safe_copy)
...
def _request_reason(request: dict[str, Any]) -> str:
    ...
    reason = _first_text(request.get("requested_rerun_reason"), request.get("rerun_reason"), request.get("reason"), default="")
    return _safe_text(reason)[:240]


def _request_next_required(request: dict[str, Any]) -> list[str]:
    # queue request 可补充 next-required evidence，但仍只输出扁平白名单文本。
    safe_copy = _dict(request, "safe_copy")
...
def _queue_request_summary(request: dict[str, Any], ack_state: str, evidence_ref: str) -> dict[str, Any]:
    # queue request summary 不复制完整 request，只保留 owner-safe 元数据。
    return {
...
def _queue_status(
    ...
    if request_load_issue and request_load_issue != "queue_request_not_provided":
        reasons.append(request_load_issue)
    if success_claim:
        return "blocked_unsafe_rerun_queue", reasons + ["success_or_primary_action_claim_detected"]
    if unsafe:
        return "blocked_unsafe_rerun_queue", reasons + ["unsafe_copy_detected"]
    if source_load_issue or not _schema_supported(source):
        return "blocked_unsupported_handoff_intake", reasons + ["unsupported_acceptance_execution_handoff_intake_schema_or_boundary"]
    if source.get("same_evidence_ref_required") is not True:
        return "evidence_ref_mismatch_rerun_queue", reasons + ["same_evidence_ref_required_not_true"]
    if not source_ref:
        return "evidence_ref_mismatch_rerun_queue", reasons + ["safe_evidence_ref_missing"]
    if requested_ref and requested_ref != source_ref:
        return "evidence_ref_mismatch_rerun_queue", reasons + [f"requested_ref:{requested_ref}!={source_ref}"]
    if request_ref and request_ref != source_ref:
        return "evidence_ref_mismatch_rerun_queue", reasons + [f"queue_request_ref:{request_ref}!={source_ref}"]
    if source_status != READY_SOURCE_STATUS:
        return "needs_acceptance_execution_rerun_queue_backfill", reasons + [f"source_handoff_intake_status:{source_status or 'missing'}"]
    if source_ack != "acknowledged" and request_ack != "acknowledged":
        return "needs_owner_ack_before_queue", reasons + ["owner_acknowledgement_missing_before_queue"]
...
def _next_required_evidence(status: str, evidence_ref: str, upstream_next: list[str], request_next: list[str]) -> list[str]:
    ...
    if status == "queued_for_controlled_field_rerun_not_proven":
        return request_next or [f"Keep controlled field rerun queue entry pending real materials for evidence_ref={ref}."]
    if status == "needs_owner_ack_before_queue":
        return [f"Collect owner queue acknowledgement JSON for evidence_ref={ref} before accepting the queue entry."]
    if status == "needs_acceptance_execution_rerun_queue_backfill":
        return upstream_next or [f"Backfill ready handoff intake package for evidence_ref={ref} before queueing rerun."]
    if status == "evidence_ref_mismatch_rerun_queue":
        return [f"Regenerate handoff intake and queue request with one evidence_ref={ref}."]
    if status == "blocked_unsupported_handoff_intake":
        return [f"Provide a supported handoff intake artifact or summary for evidence_ref={ref}."]
    return ["Regenerate rerun queue input without raw paths, credentials, runtime topics, hardware transport detail, unsafe material identifiers, control claims, or success claims."]
...
def build_route_task_field_retest_acceptance_execution_rerun_queue(
    handoff_intake_json: str,
    queue_request_json: str = "",
    evidence_ref: str = "",
) -> tuple[dict[str, Any], dict[str, Any], int]:
    """读取 handoff intake 和可选 queue request，生成 rerun queue artifact。"""
    source_payload, source_load_issue = _load_json(handoff_intake_json, "handoff_intake_json")
    request_payload, request_load_issue = _load_json(queue_request_json, "queue_request")
    ...
    source_ack = _source_ack_state(source) if source else "missing"
    request_ack = _owner_ack_state(request) if request else "missing"
    ack_state = "acknowledged" if "acknowledged" in {source_ack, request_ack} else "missing"
    ...
    unsafe = (bool(source_payload) and _has_forbidden_copy(source)) or (bool(request_payload) and _has_forbidden_copy(request))
    success_claim = (bool(source_payload) and _has_success_claim(source)) or (bool(request_payload) and _has_success_claim(request))
    status, status_reasons = _queue_status(
        source_load_issue,
        request_load_issue,
        source,
        requested_ref,
    ...
    queue_request_summary = _queue_request_summary(request, ack_state, evidence_ref_out)
    owner_handoff = [
        {
            "owner": "Autonomy Algorithm Engineer",
            "supporting_owners": ["Robot Platform Engineer", "User Touchpoint Full-Stack Engineer"],
            "action": "hold_queue_until_real_controlled_field_materials_arrive" if status == "queued_for_controlled_field_rerun_not_proven" else "repair_rerun_queue_inputs_before_queueing",
    ...
    summary = {
        ...
        "source_handoff_intake_status": source_status,
        "owner_acknowledgement_state": ack_state,
        "queue_request": queue_request_summary,
        ...
        "fail_closed_notes": [
            ...
            "Queue acknowledgement is not real route/elevator field validation, HIL, delivery_result, or delivery_success.",
    ...
    artifact = {
        ...
        "evidence_ref": evidence_ref_out,
        "same_evidence_ref_required": True,
        "source_acceptance_execution_handoff_intake": {
            ...
            "owner_acknowledgement_state": source_ack,
        ...
        "queue_request_summary": queue_request_summary,
        "owner_handoff": owner_handoff,
        "next_required_evidence": next_required,
        "safe_rerun_hint": safe_rerun_hint,
        "safe_copy": safe_copy,
        "route_task_field_retest_acceptance_execution_rerun_queue_summary": summary,
    ...
    if _has_success_claim(artifact) or _has_success_claim(summary):
        ...
        artifact["status"] = "blocked_unsafe_rerun_queue"
        artifact["rerun_queue_status"] = "blocked_unsafe_rerun_queue"
        summary["status"] = "blocked_unsafe_rerun_queue"
        summary["rerun_queue_status"] = "blocked_unsafe_rerun_queue"
        artifact["route_task_field_retest_acceptance_execution_rerun_queue_summary"] = summary
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Generate a route/task field retest acceptance execution rerun queue artifact")
    ...
    artifact, summary, exit_code = build_route_task_field_retest_acceptance_execution_rerun_queue(
    ...
    if args.once_json or not (args.output or args.summary_output):
        ...
        print(f"route_task_field_retest_acceptance_execution_rerun_queue: artifact_file:{_safe_ref(args.output)}")
        if args.summary_output:
            print(f"acceptance_execution_rerun_queue_summary_file: {_safe_ref(args.summary_output)}")
        print(f"rerun_queue_status: {artifact['rerun_queue_status']}")
    return exit_code
