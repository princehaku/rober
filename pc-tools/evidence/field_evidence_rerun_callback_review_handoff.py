#!/usr/bin/env python3
"""生成 field_evidence_rerun_callback_review_handoff 的 fail-closed PC gate。

该 gate 只读上一轮 field_evidence_rerun_callback_review_decision artifact、
...
HANDOFF_SCHEMA = "trashbot.field_evidence_rerun_callback_review_handoff.v1"
HANDOFF_SUMMARY_SCHEMA = "trashbot.field_evidence_rerun_callback_review_handoff_summary.v1"
SCHEMA_VERSION = 1
HANDOFF_BOUNDARY = "software_proof_docker_field_evidence_rerun_callback_review_handoff_gate"
...
SOURCE_BOUNDARY = "software_proof_docker_field_evidence_rerun_callback_review_decision_gate"

REVIEW_DECISIONS = ("accepted", "missing", "rejected", "blocked")
READY_STATUS = "ready_for_field_evidence_rerun_callback_review_handoff"

DECISION_TO_HANDOFF = {
    "accepted": READY_STATUS,
    "missing": "needs_owner_follow_up",
    "rejected": "needs_owner_follow_up",
    "blocked": "needs_field_evidence_rerun_callback_rerun",
}
...
# 设计约束 03：accepted 只映射为 handoff ready，不映射为 field pass。
...
# 设计约束 42：needs_field_evidence_rerun_callback_rerun 保留上游修复路径。
...
def _handoff_status(
    ...
    if load_issue or not _schema_supported(source):
        return "needs_field_evidence_rerun_callback_rerun", reasons + ["unsupported_callback_review_decision_schema_or_boundary"]
    if not _source_is_software_not_proven(source):
        return "needs_field_evidence_rerun_callback_rerun", reasons + ["source_not_software_proof_or_missing_not_proven"]
    ...
    if not source_decision:
        return "needs_field_evidence_rerun_callback_rerun", reasons + ["source_review_decision_missing"]
    if source_decision not in DECISION_TO_HANDOFF:
        return "needs_field_evidence_rerun_callback_rerun", reasons + [f"unsupported_source_review_decision:{source_decision}"]
...
def _owner_handoff(handoff_status: str, evidence_ref: str, source_decision: str, source_owner: dict[str, Any]) -> dict[str, Any]:
    ...
    if handoff_status == READY_STATUS:
        ...
        action = "regenerate_supported_callback_review_decision"
    return {
...
def _next_required_evidence(handoff_status: str, evidence_ref: str, source_next: list[str]) -> list[str]:
    ...
    ref = evidence_ref or "<same_evidence_ref>"
    if handoff_status == READY_STATUS:
        return source_next or [f"Hand off sanitized callback review decision for evidence_ref={ref} to Product closeout."]
    if handoff_status == "needs_owner_follow_up":
        return source_next or [f"Backfill or correct callback review materials for evidence_ref={ref} before closeout."]
    ...
    if handoff_status == "blocked_unsafe_review_handoff":
        return ["Regenerate callback review decision without raw paths, credentials, ROS topics, serial/UART detail, checksums, control claims, or success claims."]
    return [f"Regenerate supported field evidence rerun callback review decision for evidence_ref={ref}."]
...
def build_field_evidence_rerun_callback_review_handoff(
    ...
    if _has_success_claim(artifact) or _has_success_claim(summary):
        ...
        artifact["field_evidence_rerun_callback_review_handoff_summary"] = summary
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Generate a field evidence rerun callback review handoff artifact")
    parser.add_argument("--callback-review-decision-json", required=True, help="callback review decision artifact, summary, or wrapper/nested JSON")
    ...
    artifact, summary, exit_code = build_field_evidence_rerun_callback_review_handoff(
    ...
    if args.once_json or not (args.output or args.summary_output):
        ...
        print(f"field_evidence_rerun_callback_review_handoff: artifact_file:{_safe_ref(args.output)}")
        if args.summary_output:
            print(f"callback_review_handoff_summary_file: {_safe_ref(args.summary_output)}")
