...
import field_evidence_material_resolution_reviewer_ack_followup_escalation_status as gate  # noqa: E402
import field_evidence_material_resolution_reviewer_ack_review_handoff as handoff_gate  # noqa: E402
...
# 测试约束 04：missing source 必须 blocked_missing_reviewer_ack_handoff_not_proven。
# 测试约束 05：source handoff 非 ready 必须 blocked_missing_required_materials_not_proven。
...
class FieldEvidenceMaterialResolutionReviewerAckFollowupEscalationStatusTest(unittest.TestCase):
    ...
    def _handoff_summary(
        ...
        return {
            ...
            "field_evidence_material_resolution_reviewer_ack_review_handoff": handoff_status,
    ...
    def _build(
        ...
        return gate.build_field_evidence_material_resolution_reviewer_ack_followup_escalation_status(
            str(source_path),
            due_status,
            evidence_ref,
        )

    def test_ready_handoff_pending_maps_to_owner_response_pending(self) -> None:
        ...
        self.assertEqual(artifact["followup_status"], gate.OWNER_RESPONSE_PENDING)
    ...
    def test_ready_handoff_overdue_maps_to_escalate_status(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            artifact, summary, exit_code = self._build(
                Path(tmp),
        ...
        self.assertEqual(exit_code, 0)
        self.assertEqual(artifact["followup_status"], gate.OWNER_RESPONSE_OVERDUE)
    ...
    def test_ready_handoff_accepted_maps_to_owner_response_intake(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            artifact, summary, exit_code = self._build(
                Path(tmp),
        ...
        self.assertEqual(exit_code, 0)
        self.assertEqual(artifact["followup_status"], gate.ACCEPTED_FOR_OWNER_RESPONSE_INTAKE)
        self.assertEqual(summary["missing_required_evidence"], [])
        self.assertIn("owner response intake", summary["next_required_evidence"][0])
    ...
    def test_non_ready_source_handoff_blocks_missing_required_materials(self) -> None:
        ...
        self.assertEqual(artifact["followup_status"], gate.BLOCKED_MISSING_REQUIRED_MATERIALS)
    ...
    def test_unsafe_claims_fail_closed_without_raw_copy(self) -> None:
        ...
        self.assertEqual(exit_code, 2)
        self.assertEqual(artifact["followup_status"], gate.BLOCKED_UNSAFE_MATERIAL_CLAIMS)
    ...
    def test_missing_handoff_maps_to_blocked_not_proven(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            artifact, summary, exit_code = gate.build_field_evidence_material_resolution_reviewer_ack_followup_escalation_status(
                str(Path(tmp) / "missing-reviewer-ack-handoff.json")
            )

        self.assertEqual(exit_code, 2)
        self.assertEqual(artifact["followup_status"], gate.BLOCKED_MISSING_REVIEWER_ACK_HANDOFF)
    ...
    def test_cli_prints_overdue_status_for_safe_handoff(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    str(EVIDENCE_DIR / "field_evidence_material_resolution_reviewer_ack_followup_escalation_status.py"),
                    "--input",
                    str(source_path),
                    "--due-status",
                    "overdue",
        ...
        self.assertIn(gate.OWNER_RESPONSE_OVERDUE, result.stdout)
        self.assertIn("software_proof_docker_field_evidence_material_resolution_reviewer_ack_followup_escalation_status_gate", result.stdout)
    ...
    def test_output_preserves_required_boundary_literals_and_no_raw_copy(self) -> None:
        ...
        encoded = json.dumps({"artifact": artifact, "summary": summary}, ensure_ascii=False)
        self.assertIn("field_evidence_material_resolution_reviewer_ack_followup_escalation_status", encoded)
        self.assertIn("software_proof_docker_field_evidence_material_resolution_reviewer_ack_followup_escalation_status_gate", encoded)
        ...
        self.assertIn(gate.OWNER_RESPONSE_PENDING, encoded)
        self.assertIn(gate.OWNER_RESPONSE_OVERDUE, encoded)
        self.assertIn(gate.BLOCKED_MISSING_REQUIRED_MATERIALS, encoded)
        self.assertIn(gate.BLOCKED_UNSAFE_MATERIAL_CLAIMS, encoded)
        self.assertIn(gate.ACCEPTED_FOR_OWNER_RESPONSE_INTAKE, encoded)
        self.assertIn(gate.BLOCKED_MISSING_REVIEWER_ACK_HANDOFF, encoded)
