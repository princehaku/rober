...
import field_evidence_material_resolution_reviewer_ack_intake as intake  # noqa: E402
import field_evidence_material_resolution_reviewer_ack_review_decision as gate  # noqa: E402


# 测试约束 01：fixture 只表达 reviewer ACK intake safe summary。
# 测试约束 02：accepted 只表示进入材料复核，不代表 reviewer resolution。
...
class FieldEvidenceMaterialResolutionReviewerAckReviewDecisionTest(unittest.TestCase):
    ...
    def _intake_summary(
        ...
        return {
            ...
            "source_handoff_status": "ready_for_material_review_handoff_not_proven",
            "evidence_boundary": intake.EVIDENCE_BOUNDARY,
            "safe_evidence_ref": evidence_ref,
            "evidence_ref": evidence_ref,
            "same_evidence_ref_required": True,
            "ack_reasons": ack_reasons or ["reviewer ACK received for later material review"],
    ...
    def _build(self, root: Path, payload: dict[str, object], evidence_ref: str) -> tuple[dict[str, object], dict[str, object], int]:
        ...
        return gate.build_field_evidence_material_resolution_reviewer_ack_review_decision(
            str(input_path),
            evidence_ref,
        )

    def test_acknowledged_intake_maps_to_accepted_for_material_review(self) -> None:
    ...
    def test_missing_or_bad_intake_maps_to_blocked_missing_intake(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            missing, missing_summary, missing_exit = gate.build_field_evidence_material_resolution_reviewer_ack_review_decision(
                str(root / "missing-intake.json"),
                "reviewer-ack-review-905",
            )
            bad_path = self._write_json(root, "unsupported.json", {"schema": "unsupported", "source": "software_proof"})
            bad, _, bad_exit = gate.build_field_evidence_material_resolution_reviewer_ack_review_decision(
                str(bad_path),
    ...
    def test_robot_alias_wrapper_is_supported(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            payload = {intake.ROBOT_ALIAS: self._intake_summary("reviewer-ack-review-907")}
            artifact, summary, exit_code = self._build(Path(tmp), payload, "reviewer-ack-review-907")

        self.assertEqual(exit_code, 0)
        self.assertEqual(artifact["review_decision"], gate.ACCEPTED)
        self.assertEqual(artifact["safe_evidence_ref"], "reviewer-ack-review-907")
    ...
    def test_cli_prints_accepted_for_safe_ack_intake(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            input_path = self._write_json(root, "reviewer_ack_intake.json", self._intake_summary("reviewer-ack-review-908"))
            result = subprocess.run(
                [
                    sys.executable,
                    str(EVIDENCE_DIR / "field_evidence_material_resolution_reviewer_ack_review_decision.py"),
                    "--reviewer-ack-intake-json",
                    str(input_path),
                    "--evidence-ref",
                    "reviewer-ack-review-908",
                ],
                check=False,
                text=True,
                capture_output=True,
        ...
        self.assertIn("software_proof_docker_field_evidence_material_resolution_reviewer_ack_review_decision_gate", result.stdout)

    def test_output_preserves_required_boundary_literals_and_no_raw_copy(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            artifact, summary, _ = self._build(
                Path(tmp),
                self._intake_summary("reviewer-ack-review-909"),
                "reviewer-ack-review-909",
            )

        encoded = json.dumps({"artifact": artifact, "summary": summary}, ensure_ascii=False)
        self.assertIn("field_evidence_material_resolution_reviewer_ack_review_decision", encoded)
        self.assertIn("software_proof_docker_field_evidence_material_resolution_reviewer_ack_review_decision_gate", encoded)
