...
该 gate 只消费上一轮
`field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_handoff` 的 safe
...
import field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_handoff as handoff_gate


SCHEMA = "trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status.v1"
SUMMARY_SCHEMA = "trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status_summary.v1"
ROBOT_ALIAS = "robot_diagnostics_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status_summary"
SCHEMA_VERSION = 1
...
PENDING_REVIEWER_ACK_FOLLOWUP = "pending_reviewer_ack_followup_not_proven"
OVERDUE_REVIEWER_ACK_FOLLOWUP = "overdue_reviewer_ack_followup_not_proven"
ESCALATED_MISSING_REAL_MATERIAL = "escalated_missing_real_material_not_proven"
BLOCKED_MISSING_REVIEWER_ACK_REVIEW_HANDOFF = "blocked_missing_reviewer_ack_review_handoff_not_proven"
READY_FOR_REAL_MATERIAL_REVIEWER_FOLLOWUP = "ready_for_real_material_reviewer_followup_not_proven"
FOLLOWUP_STATUSES = (
    PENDING_REVIEWER_ACK_FOLLOWUP,
    OVERDUE_REVIEWER_ACK_FOLLOWUP,
    ESCALATED_MISSING_REAL_MATERIAL,
    BLOCKED_MISSING_REVIEWER_ACK_REVIEW_HANDOFF,
    READY_FOR_REAL_MATERIAL_REVIEWER_FOLLOWUP,
)
...
def _normalize_source(payload: dict[str, Any], read_issue: str, expected_ref: str) -> dict[str, Any]:
    ...
    source_handoff_status = _safe_text(
        ...
        or source.get("field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_handoff")
...
def _followup_status(normalized: dict[str, Any], due_status: str) -> tuple[str, list[str]]:
    ...
    if normalized["read_issue"]:
        return BLOCKED_MISSING_REVIEWER_ACK_REVIEW_HANDOFF, [normalized["read_issue"]]
    if not _schema_supported(normalized):
        return BLOCKED_MISSING_REVIEWER_ACK_REVIEW_HANDOFF, ["unsupported_reviewer_ack_review_handoff_schema_or_boundary"]
    if normalized["unsafe_reasons"]:
        return ESCALATED_MISSING_REAL_MATERIAL, normalized["unsafe_reasons"]
    if not normalized["source_is_software_not_proven"]:
        return ESCALATED_MISSING_REAL_MATERIAL, ["source_not_software_proof_not_proven_or_false_flags_changed"]
    if normalized["ref_errors"] or not normalized["safe_evidence_ref"] or normalized["same_evidence_ref_required"] is not True:
        return BLOCKED_MISSING_REVIEWER_ACK_REVIEW_HANDOFF, normalized["ref_errors"] or ["missing_or_weak_same_evidence_ref"]
    if normalized["source_handoff_status"] == handoff_gate.HANDOFF_REJECTED:
        return ESCALATED_MISSING_REAL_MATERIAL, ["source_reviewer_ack_handoff_rejected_unsafe"]
    if normalized["source_handoff_status"] != handoff_gate.HANDOFF_READY:
        return ESCALATED_MISSING_REAL_MATERIAL, ["source_reviewer_ack_handoff_not_ready_for_followup"]
    if due_status == "pending":
        return PENDING_REVIEWER_ACK_FOLLOWUP, ["reviewer_ack_handoff_sent_owner_response_pending"]
    if due_status == "overdue":
        return OVERDUE_REVIEWER_ACK_FOLLOWUP, ["reviewer_ack_handoff_sent_owner_response_overdue_escalate"]
    if due_status == "accepted":
        return READY_FOR_REAL_MATERIAL_REVIEWER_FOLLOWUP, ["reviewer_ack_handoff_ready_for_real_material_reviewer_followup_only"]
    return ESCALATED_MISSING_REAL_MATERIAL, ["unsupported_due_status"]


def _next_required(status: str, normalized: dict[str, Any]) -> list[str]:
    # next_required_evidence 只描述人工补证动作，不描述机器人命令。
    if status == READY_FOR_REAL_MATERIAL_REVIEWER_FOLLOWUP:
        return ["prepare real-material reviewer followup under the same safe evidence_ref without success/control claims"] + list(
            DEFAULT_MISSING_REQUIRED_EVIDENCE
        )
    if status == PENDING_REVIEWER_ACK_FOLLOWUP:
        return ["wait for reviewer ACK followup under the same safe evidence_ref"] + list(DEFAULT_MISSING_REQUIRED_EVIDENCE)
    if status == OVERDUE_REVIEWER_ACK_FOLLOWUP:
        return ["escalate overdue reviewer ACK followup to support escalation owner"] + list(DEFAULT_MISSING_REQUIRED_EVIDENCE)
    if status == ESCALATED_MISSING_REAL_MATERIAL:
        return ["resubmit sanitized reviewer ACK review handoff without unsafe raw, control, credential, or success claims"]
    if status == BLOCKED_MISSING_REVIEWER_ACK_REVIEW_HANDOFF:
...
def _field_owner_handoff(normalized: dict[str, Any], followup_status: str, due_status: str, next_required: list[str]) -> dict[str, Any]:
    ...
    if followup_status == READY_FOR_REAL_MATERIAL_REVIEWER_FOLLOWUP:
        action = "prepare_real_material_reviewer_followup_under_same_safe_evidence_ref"
    elif followup_status == OVERDUE_REVIEWER_ACK_FOLLOWUP:
        action = "respond_to_overdue_escalation_with_missing_required_evidence"
    elif followup_status == PENDING_REVIEWER_ACK_FOLLOWUP:
        action = "collect_reviewer_ack_followup_without_success_or_control_claims"
    else:
        action = "repair_reviewer_ack_review_handoff_or_missing_real_materials_before_followup"
    return {
        **_safe_flags(),
        "role": "field owner",
...
def _support_escalation(normalized: dict[str, Any], followup_status: str, due_status: str, reasons: list[str], next_required: list[str]) -> dict[str, Any]:
    ...
    return {
        **_safe_flags(),
        ...
        "safe_evidence_ref": normalized["safe_evidence_ref"],
        "evidence_ref": normalized["safe_evidence_ref"],
        "escalate": followup_status in {OVERDUE_REVIEWER_ACK_FOLLOWUP, ESCALATED_MISSING_REAL_MATERIAL},
        "blocked_or_rejected_reasons": reasons
        if followup_status in {BLOCKED_MISSING_REVIEWER_ACK_REVIEW_HANDOFF, ESCALATED_MISSING_REAL_MATERIAL}
        else [],
        "next_required_evidence": next_required,
    }
...
def build_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status(
    ...
    missing_required = next_required
    ...
    common = {
        **_safe_flags(),
        ...
        "due_status": normalized_due_status
        if followup_status != BLOCKED_MISSING_REVIEWER_ACK_REVIEW_HANDOFF
        else "blocked_missing_reviewer_ack_review_handoff",
        "source_handoff_status": normalized["source_handoff_status"],
        "reviewer_ack_status": normalized["reviewer_ack_state"] or "missing",
        ...
        "rejected_or_unsafe_reasons": reasons if followup_status == ESCALATED_MISSING_REAL_MATERIAL else [],
        "next_required_evidence": next_required,
        "field_owner_handoff": field_owner_handoff,
    ...
    artifact = {
        ...
        "field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status": followup_status,
        **common,
    ...
    artifact["field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status_summary"] = summary
    ...
    return (
        ...
        0
        if followup_status
        in {PENDING_REVIEWER_ACK_FOLLOWUP, OVERDUE_REVIEWER_ACK_FOLLOWUP, READY_FOR_REAL_MATERIAL_REVIEWER_FOLLOWUP}
        else 2,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status.v1 from a "
            "sanitized field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_handoff artifact, summary, or Robot alias. "
    ...
    artifact, summary, exit_code = build_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status(
        args.input,
        args.due_status,
        args.evidence_ref,
    )
    if args.output_dir:
        _write_json(args.output_dir / "field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status.json", artifact)
        _write_json(args.output_dir / "field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status_summary.json", summary)
    if args.output:
