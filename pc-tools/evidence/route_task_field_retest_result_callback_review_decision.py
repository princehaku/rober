#!/usr/bin/env python3
"""生成 route/task field retest result callback review decision artifact。

该 PC-only gate 只读取上一轮 result callback intake 的 artifact/summary/wrapper
JSON，把 accepted/missing/rejected updates 转成下一步复核决策。它不读取真实
材料目录、不访问 ROS graph、Nav2 runtime、serial/UART、WAVE ROVER、外部云、
...
# 本 gate 是 result callback intake 后的新契约，不能复用上一层 schema。
DECISION_SCHEMA = "trashbot.route_task_field_retest_result_callback_review_decision.v1"
DECISION_SUMMARY_SCHEMA = "trashbot.route_task_field_retest_result_callback_review_decision_summary.v1"
SCHEMA_VERSION = 1
DECISION_BOUNDARY = "software_proof_docker_route_task_field_retest_result_callback_review_decision_gate"

# 只接受上一轮 result callback intake，避免跳过回执摄取直接解释现场材料。
SOURCE_SCHEMAS = {
...
SOURCE_BOUNDARY = "software_proof_docker_route_task_field_retest_result_callback_intake_gate"
READY_SOURCE_STATUS = "ready_for_field_retest_result_callback_intake_not_proven"
...
BOUNDARY_NOTE = (
    ...
    "ready_for_result_review; needs_material_backfill; evidence_ref_mismatch_rerun"
)
...
def _find_source(payload: dict[str, Any]) -> dict[str, Any]:
    # 优先选择 schema 命中的 result callback intake 对象；否则保留顶层解释失败。
...
def _list_items(value: Any, limit: int = 48) -> list[Any]:
    # update list 只接受 list，避免弱类型字符串被当成已复核材料。
    if not isinstance(value, list):
        return []
    return _safe_value(value[:limit])


def _updates_from_source(source: dict[str, Any], key: str) -> list[Any]:
    # accepted/missing/rejected 可能在顶层、summary 或 safe_copy，统一白名单提取。
    ...
    for candidate in (source, robot, mobile, safe_copy):
        updates = _list_items(candidate.get(key))
        if updates:
            return updates
    return []
...
def _review_decision(
    ...
    if load_issue or not _schema_supported(source):
        return "needs_callback_rerun", [load_issue or "unsupported_result_callback_intake_schema_or_boundary"]
    ...
    if source_status != READY_SOURCE_STATUS:
        return "needs_callback_rerun", [f"source_intake_status:{source_status or 'missing'}"]
    if rejected:
        return "needs_callback_rerun", ["rejected_updates_present"]
    if missing:
        return "needs_material_backfill", ["missing_updates_present"]
    if not accepted:
        return "needs_callback_rerun", ["accepted_updates_empty"]
    return "ready_for_result_review", []


def _review_readiness(decision: str) -> dict[str, Any]:
    # readiness 是只读判断，不会触发 result review、Robot action 或 mobile action。
    return {
        "ready": decision == "ready_for_result_review",
        "state": "ready_for_result_review" if decision == "ready_for_result_review" else "not_ready",
        "blocked_by": "" if decision == "ready_for_result_review" else decision,
        "delivery_success": False,
        "primary_actions_enabled": False,
    }


def _update_keys(items: list[Any]) -> list[str]:
    # 下游只需要可读摘要键，不复制完整材料内容。
    keys: list[str] = []
    for item in items[:24]:
        if isinstance(item, dict):
            key = _first_text(item.get("key"), item.get("kind"), item.get("action"), item.get("safe_note"), default="")
            keys.append(_safe_text(key or "update"))
        else:
...
def _next_required_evidence(decision: str, evidence_ref: str, missing: list[Any], rejected: list[Any]) -> list[str]:
    ...
    if decision == "ready_for_result_review":
        return [f"Run result review for sanitized callback updates under evidence_ref={ref}."]
    if decision == "needs_material_backfill":
        missing_keys = _update_keys(missing) or ["missing_callback_updates"]
        return [f"Backfill missing result callback metadata for evidence_ref={ref}: " + ", ".join(missing_keys[:8])]
    if decision == "needs_callback_rerun":
        rejected_keys = _update_keys(rejected)
        if rejected_keys:
            return [f"Repair rejected result callback updates for evidence_ref={ref}: " + ", ".join(rejected_keys[:8])]
        return [f"Regenerate supported result callback intake for evidence_ref={ref}."]
    if decision == "evidence_ref_mismatch_rerun":
        return [f"Regenerate result callback intake with one evidence_ref={ref}.", "Rerun this review decision after refs match."]
...
def _owner_handoff(decision: str, evidence_ref: str, missing: list[Any], rejected: list[Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "handoff_status": "ready_for_result_review_not_proven" if decision == "ready_for_result_review" else "callback_repair_required_not_proven",
        "handoff_note": f"Review result callback intake for evidence_ref={evidence_ref or '<same_evidence_ref>'}; keep Robot/mobile read-only and primary actions disabled.",
        "missing_update_keys": _update_keys(missing),
        "rejected_update_keys": _update_keys(rejected),
        "delivery_success": False,
        "primary_actions_enabled": False,
    }
...
def _rerun_commands(evidence_ref: str) -> list[str]:
    ...
    return [
        ...
        f"python3 pc-tools/evidence/route_task_field_retest_result_callback_review_decision.py --callback-intake-json <callback_intake.json> --evidence-ref {ref} --once-json",
    ]


def _safe_copy(decision: str, evidence_ref: str, source_status: str, accepted: list[Any], missing: list[Any], rejected: list[Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "accepted_update_keys": _update_keys(accepted),
        "missing_update_keys": _update_keys(missing),
        "rejected_update_keys": _update_keys(rejected),
...
def _summary_payload(
    ...
    return {
        ...
        "missing_update_keys": _update_keys(missing),
        "rejected_update_keys": _update_keys(rejected),
        "result_review_readiness": _review_readiness(decision),
...
def build_route_task_field_retest_result_callback_review_decision(
    callback_intake_json: str,
    evidence_ref: str = "",
) -> tuple[dict[str, Any], dict[str, Any], int]:
    """读取 result callback intake，生成 metadata-only review decision artifact。"""
    ...
    accepted = _updates_from_source(source, "accepted_updates") if source else []
    missing = _updates_from_source(source, "missing_updates") if source else []
    rejected = _updates_from_source(source, "rejected_updates") if source else []
    ...
    safe_copy = _safe_copy(decision, evidence_ref_out, source_status, accepted, missing, rejected)
    ...
    artifact = {
        ...
        "safe_copy": safe_copy,
        "route_task_field_retest_result_callback_review_decision_summary": summary,
    ...
    if _unsafe_copy(artifact) or _unsafe_copy(summary) or _success_or_control_claim(artifact) or _success_or_control_claim(summary):
        ...
        artifact["route_task_field_retest_result_callback_review_decision_summary"] = summary
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Generate a route/task field retest result callback review decision artifact")
    parser.add_argument("--callback-intake-json", required=True, help="result callback intake artifact, summary, or wrapper/nested JSON")
    ...
    artifact, summary, exit_code = build_route_task_field_retest_result_callback_review_decision(args.callback_intake_json, args.evidence_ref)
    write_json(artifact, args.output)
    write_json(summary, args.summary_output)
    if args.once_json or not (args.output or args.summary_output):
        print(json.dumps(artifact, ensure_ascii=False, indent=2, sort_keys=True))
    else:
        print(f"route_task_field_retest_result_callback_review_decision: artifact_file:{_safe_ref(args.output)}")
        if args.summary_output:
            print(f"result_callback_review_decision_summary_file: {_safe_ref(args.summary_output)}")
        print(f"result_callback_review_decision: {artifact['review_decision']}")
    return exit_code
