...
该 gate 只消费上一轮
`field_evidence_material_resolution_reviewer_ack_review_decision` 的 safe
...
reviewer resolution、field pass、cloud、phone、HIL、delivery success、PR #5 closure
或 OKR lift。
"""
...
# 设计约束 04：safe evidence_ref 是 ACK review-decision 到 handoff 的唯一串联主键。
...
import field_evidence_material_resolution_reviewer_ack_review_decision as decision_gate


SCHEMA = "trashbot.field_evidence_material_resolution_reviewer_ack_review_handoff.v1"
SUMMARY_SCHEMA = "trashbot.field_evidence_material_resolution_reviewer_ack_review_handoff_summary.v1"
ROBOT_ALIAS = "robot_diagnostics_field_evidence_material_resolution_reviewer_ack_review_handoff_summary"
SCHEMA_VERSION = 1
...
HANDOFF_READY = "ready_for_reviewer_ack_material_review_handoff_not_proven"
HANDOFF_REASSIGNMENT = "needs_reassignment_handoff_not_proven"
HANDOFF_FIELD_OWNER_SUPPLEMENT = "needs_field_owner_supplement_handoff_not_proven"
HANDOFF_REJECTED = "rejected_unsafe_reviewer_ack_handoff_not_proven"
HANDOFF_BLOCKED = "blocked_missing_reviewer_ack_review_decision_handoff_not_proven"
HANDOFF_STATUSES = (
    HANDOFF_READY,
...
def _handoff_status(normalized: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    if source_decision == decision_gate.ACCEPTED:
        return HANDOFF_READY, ["reviewer_ack_review_decision_ready_for_material_handoff_only"]
    if source_decision == decision_gate.NEEDS_REASSIGNMENT:
        return HANDOFF_REASSIGNMENT, ["reviewer_ack_review_decision_needs_reassignment"]
    if source_decision == decision_gate.NEEDS_FIELD_OWNER_SUPPLEMENT:
        return HANDOFF_FIELD_OWNER_SUPPLEMENT, ["reviewer_ack_review_decision_needs_field_owner_supplement"]
    ...
    if source_decision == decision_gate.BLOCKED_MISSING_INTAKE:
        return HANDOFF_BLOCKED, ["reviewer_ack_review_decision_blocked_missing_intake"]
...
def _next_required(normalized: dict[str, Any], handoff_status: str, reasons: list[str]) -> list[str]:
    ...
    if handoff_status == HANDOFF_READY:
        return [
            "support packages the sanitized reviewer ACK review handoff under the same safe evidence_ref",
    ...
    if handoff_status == HANDOFF_REASSIGNMENT:
        target = normalized["reassignment_target"] or "reassigned owner"
        return [f"{target} provides reassigned owner ACK under the same safe evidence_ref"]
    if handoff_status == HANDOFF_FIELD_OWNER_SUPPLEMENT:
        return normalized["next_required_evidence"] or [
            "field owner backfills supplement material under the same safe evidence_ref"
        ]
    if handoff_status == HANDOFF_REJECTED:
        return normalized["decision_reasons"] or reasons
    return ["provide_supported_field_evidence_material_resolution_reviewer_ack_review_decision_safe_artifact_summary_or_robot_alias"]
...
def _field_owner_handoff(normalized: dict[str, Any], handoff_status: str, reasons: list[str], next_required: list[str]) -> dict[str, Any]:
    ...
    if handoff_status == HANDOFF_READY:
        action = "queue_material_review_under_same_safe_evidence_ref_without_success_claim"
        ...
        action = "backfill_field_owner_supplement_under_same_safe_evidence_ref"
...
def _reviewer_handoff(normalized: dict[str, Any], handoff_status: str, reasons: list[str], next_required: list[str]) -> dict[str, Any]:
    ...
    if handoff_status == HANDOFF_READY:
        ...
        action = "wait_for_reassigned_owner_ack_before_material_review"
    elif handoff_status == HANDOFF_FIELD_OWNER_SUPPLEMENT:
        action = "wait_for_field_owner_supplement_before_material_review"
    elif handoff_status == HANDOFF_REJECTED:
...
def build_field_evidence_material_resolution_reviewer_ack_review_handoff(
    ...
    artifact["field_evidence_material_resolution_reviewer_ack_review_handoff_summary"] = summary
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build trashbot.field_evidence_material_resolution_reviewer_ack_review_handoff.v1 from a sanitized "
            "field_evidence_material_resolution_reviewer_ack_review_decision artifact, summary, or Robot alias. Keeps "
            "source=software_proof, not_proven, delivery_success=false, primary_actions_enabled=false, "
            "safe_to_control=false."
        )
    )
    parser.add_argument("--input", default="", help="sanitized reviewer ACK review decision artifact, summary, or Robot alias JSON")
    ...
    artifact, summary, exit_code = build_field_evidence_material_resolution_reviewer_ack_review_handoff(args.input)
    if args.output_dir:
        _write_json(args.output_dir / "field_evidence_material_resolution_reviewer_ack_review_handoff.json", artifact)
        _write_json(args.output_dir / "field_evidence_material_resolution_reviewer_ack_review_handoff_summary.json", summary)
    if args.output:
