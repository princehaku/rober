#!/usr/bin/env python3
"""生成 route/task field retest result review intake artifact。

该 PC-only gate 只读取上一轮 result callback review handoff 的 artifact、
summary 或 wrapper/nested JSON，把 handoff 状态转成 review 前的
owner follow-up、review-intake package 和 rerun package。它不读取真实材料目录，
...
# 本 gate 是 result callback review handoff 后的新契约，不能复用旧 intake schema。
INTAKE_SCHEMA = "trashbot.route_task_field_retest_result_review_intake.v1"
INTAKE_SUMMARY_SCHEMA = "trashbot.route_task_field_retest_result_review_intake_summary.v1"
SCHEMA_VERSION = 1
INTAKE_BOUNDARY = "software_proof_docker_route_task_field_retest_result_review_intake_gate"

# 只接受上一轮 result callback review handoff，避免跳过决策层直接包装 callback。
SOURCE_SCHEMAS = {
...
SOURCE_BOUNDARY = "software_proof_docker_route_task_field_retest_result_callback_review_handoff_gate"

# result review intake 仍需要这几类证据说明；这里只生成 checklist，不读取材料。
REVIEW_INTAKE_REQUIREMENTS = (
    "supported_callback_review_handoff",
    "same_evidence_ref_match",
    "review_ready_package_present",
    "owner_follow_up_preserved",
    "safe_copy_only",
...
BOUNDARY_NOTE = (
    ...
    "needs_handoff_rerun; evidence_ref_mismatch_rerun; blocked_unsafe_review_intake"
)

# 设计约束 01：本 gate 只消费 result callback review handoff，不读材料目录。
# 设计约束 02：intake_status 只表达 result review 前交接状态，不证明现场结果。
# 设计约束 03：ready 分支仍保持 not_proven，不能改变 Robot/mobile action。
# 设计约束 04：source handoff 是唯一映射来源，不重新解释材料内容。
# 设计约束 05：unsupported schema/boundary 或缺 handoff_status 必须 fail closed。
...
# 设计约束 16：exit code 保持 0，让 blocked intake 也能作为证据落盘。
# 设计约束 17：文档和单测同步覆盖所有 intake status mapping。
...
# 设计约束 23：needs_handoff_rerun 保留复跑路径，避免人工猜测 callback 内容。
# 设计约束 24：evidence_ref_mismatch_rerun 优先保护同一证据链主键。
# 设计约束 25：blocked_unsafe_review_intake 统一覆盖敏感信息和控制声明。
...
# 设计约束 29：缺 handoff_status 时不从 review_ready_package 推导结论。
...
# 设计约束 36：source_callback_review_handoff 只保留 schema/boundary/ref 摘要。
...
# 设计约束 54：_source_handoff_status 兼容 safe_copy，但仍要求至少一个位置存在。
# 设计约束 55：_same_ref_required_ok 要求真布尔，字符串 true 不能通过。
# 设计约束 56：intake mapping 与文档一一对应，新增状态必须补测试。
...
# 设计约束 72：unsupported handoff 进入 rerun，而不是 unsafe，便于修 schema。
# 设计约束 73：blocked_unsafe_review_handoff 进入 blocked，保留安全边界。
...
# 设计约束 89：review_ready_package 的 ready 布尔只由 intake_status 推导。
# 设计约束 90：rerun_package 的 required 布尔只由 intake_status 推导。
...
HANDOFF_TO_INTAKE = {
    "ready_for_result_review_handoff": "ready_for_result_review_intake",
...
# 成功或动作授权不能通过 result review 前置 intake 进入下游。
...
def _load_json(path: str) -> tuple[dict[str, Any], str]:
    # 缺输入、坏 JSON、非 object 都输出 rerun intake，便于留痕。
    if not path:
        return {}, "callback_review_handoff_not_provided"
    try:
        with Path(path).expanduser().open("r", encoding="utf-8") as f:
            payload = json.load(f)
    except FileNotFoundError:
        return {}, "callback_review_handoff_missing"
    except json.JSONDecodeError:
        return {}, "callback_review_handoff_bad_json"
    except (OSError, UnicodeDecodeError):
        return {}, "callback_review_handoff_read_error"
    if not isinstance(payload, dict):
        return {}, "callback_review_handoff_not_object"
    return payload, ""
...
def _intake_status(
    ...
    if success_claim:
        return "blocked_unsafe_review_intake", reasons + ["success_or_primary_action_claim_detected"]
    if unsafe:
        return "blocked_unsafe_review_intake", reasons + ["unsafe_copy_detected"]
    if load_issue or not _schema_supported(source):
        return "needs_handoff_rerun", reasons + ["unsupported_result_callback_review_handoff_schema_or_boundary"]
    ...
    if not source_handoff_status:
        return "needs_handoff_rerun", reasons + ["source_handoff_status_missing"]
    if source_handoff_status not in HANDOFF_TO_INTAKE:
        return "needs_handoff_rerun", reasons + [f"unsupported_source_handoff_status:{source_handoff_status}"]
    return HANDOFF_TO_INTAKE[source_handoff_status], reasons


def _result_review_intake_package(intake_status: str, evidence_ref: str, source_handoff_status: str) -> dict[str, Any]:
    # result review intake package 是只读验收入口，不触发真正 result review。
    ...
    return {
        ...
        "requirements": list(REVIEW_INTAKE_REQUIREMENTS),
        "review_entry": "result_review_intake_metadata_ready" if ready else "result_review_intake_blocked_until_follow_up",
...
def _owner_follow_up(intake_status: str, evidence_ref: str, source_handoff_status: str, upstream_follow_up: list[Any]) -> list[dict[str, Any]]:
    # owner follow-up 只分派材料责任，不授权 Robot/mobile 任何动作。
    ref = evidence_ref or "<same_evidence_ref>"
    if intake_status == "ready_for_result_review_intake":
        action = "register_metadata_intake_before_result_review"
    elif intake_status == "needs_owner_follow_up":
        action = "backfill_owner_material_before_result_review"
    elif intake_status == "needs_handoff_rerun":
        action = "repair_callback_review_handoff_and_rerun_intake"
    elif intake_status == "evidence_ref_mismatch_rerun":
        action = "rerun_callback_review_handoff_with_same_evidence_ref"
    else:
...
def _rerun_package(intake_status: str, evidence_ref: str) -> dict[str, Any]:
    # rerun package 只给 PC operator 复跑，不访问 ROS graph、Nav2 或硬件。
    ref = evidence_ref or "<same_evidence_ref>"
    return {
        "required": intake_status != "ready_for_result_review_intake",
        "status": intake_status,
        "safe_evidence_ref": ref,
        "commands": [
...
def _next_required_evidence(intake_status: str, evidence_ref: str, upstream_next: list[str]) -> list[str]:
    # 下一步只列证据动作，不写现场通过或完成措辞。
    ref = evidence_ref or "<same_evidence_ref>"
    if intake_status == "ready_for_result_review_intake":
        return upstream_next or [f"Prepare result review packet for evidence_ref={ref} from sanitized callback review handoff."]
    if intake_status == "needs_owner_follow_up":
        return upstream_next or [f"Backfill owner follow-up items for evidence_ref={ref} before result review."]
    if intake_status == "needs_handoff_rerun":
        return [f"Regenerate supported result callback review handoff for evidence_ref={ref}."]
    if intake_status == "evidence_ref_mismatch_rerun":
        return [f"Regenerate callback review handoff and intake with one evidence_ref={ref}."]
    return ["Regenerate callback review handoff without raw paths, credentials, ROS topics, serial/UART, WAVE ROVER detail, checksums, control claims, or success claims."]
...
def _safe_copy(
    ...
    evidence_ref: str,
    source_handoff_status: str,
    result_review_intake_package: dict[str, Any],
    ...
    return {
        ...
        "result_review_intake_ready": bool(result_review_intake_package.get("ready")),
...
def build_route_task_field_retest_result_review_intake(
    callback_review_handoff_json: str,
    evidence_ref: str = "",
) -> tuple[dict[str, Any], dict[str, Any], int]:
    """读取 result callback review handoff，生成 result review 前 intake artifact。"""
    payload, load_issue = _load_json(callback_review_handoff_json)
    ...
    source_handoff_status = _source_handoff_status(source) if source else ""
    ...
    result_review_intake_package = _result_review_intake_package(intake_status, evidence_ref_out, source_handoff_status)
    owner_follow_up = _owner_follow_up(intake_status, evidence_ref_out, source_handoff_status, upstream_follow_up)
    rerun_package = _rerun_package(intake_status, evidence_ref_out)
    next_required = _next_required_evidence(intake_status, evidence_ref_out, upstream_next)
    safe_copy = _safe_copy(intake_status, evidence_ref_out, source_handoff_status, result_review_intake_package, next_required)
    summary = {
        ...
        "result_review_intake_package": result_review_intake_package,
    ...
    artifact = {
        ...
        "next_required_evidence": next_required,
        "safe_copy": safe_copy,
        "route_task_field_retest_result_review_intake_summary": summary,
    ...
    if _has_success_claim(artifact) or _has_success_claim(summary):
        ...
        artifact["status"] = "blocked_unsafe_review_intake"
        artifact["intake_status"] = "blocked_unsafe_review_intake"
        summary["status"] = "blocked_unsafe_review_intake"
        summary["intake_status"] = "blocked_unsafe_review_intake"
        artifact["route_task_field_retest_result_review_intake_summary"] = summary
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Generate a route/task field retest result review intake artifact")
    parser.add_argument("--callback-review-handoff-json", required=True, help="result callback review handoff artifact, summary, or wrapper/nested JSON")
    parser.add_argument("--evidence-ref", default="", help="expected same evidence_ref for this review intake")
    ...
    artifact, summary, exit_code = build_route_task_field_retest_result_review_intake(
    ...
    if args.once_json or not (args.output or args.summary_output):
        ...
        print(f"route_task_field_retest_result_review_intake: artifact_file:{_safe_ref(args.output)}")
        if args.summary_output:
            print(f"result_review_intake_summary_file: {_safe_ref(args.summary_output)}")
