...
该 gate 只消费
`field_evidence_material_resolution_reviewer_ack_intake` 的 safe artifact、
...
import field_evidence_material_resolution_reviewer_ack_intake as intake


SCHEMA = "trashbot.field_evidence_material_resolution_reviewer_ack_review_decision.v1"
SUMMARY_SCHEMA = "trashbot.field_evidence_material_resolution_reviewer_ack_review_decision_summary.v1"
ROBOT_ALIAS = "robot_diagnostics_field_evidence_material_resolution_reviewer_ack_review_decision_summary"
SCHEMA_VERSION = 1
...
NEEDS_REASSIGNMENT = "needs_reassignment_not_proven"
NEEDS_FIELD_OWNER_SUPPLEMENT = "needs_field_owner_supplement_not_proven"
REJECTED_UNSAFE = "rejected_unsafe_ack_not_proven"
...
# 设计约束 02：accepted 只表示可进入后续材料复核，不表示 reviewer 通过。
...
def _review_decision(source: dict[str, Any], requested_ref: str) -> tuple[str, list[str]]:
    ...
    if source["reviewer_ack_state"] == intake.ACK_BLOCKED_MISSING_HANDOFF:
    ...
    if source["reviewer_ack_state"] == intake.ACK_NEEDS_REASSIGNMENT:
        return NEEDS_REASSIGNMENT, ["reviewer requested safe reassignment before material review"]
    if source["reviewer_ack_state"] == intake.ACK_ACKNOWLEDGED and _needs_field_owner_supplement(source):
        return NEEDS_FIELD_OWNER_SUPPLEMENT, ["field owner supplement required before material review"]
    if source["reviewer_ack_state"] == intake.ACK_ACKNOWLEDGED:
        return ACCEPTED, ["reviewer ACK accepted for later material review only"]
...
def _next_required_evidence(decision: str, source: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        return [
            {
                ...
                "action": "queue_material_review_without_claiming_field_cloud_phone_hil_or_delivery_success",
    ...
    return [
        {
            "owner": "reviewer",
            "action": "provide_supported_field_evidence_material_resolution_reviewer_ack_intake_safe_artifact_or_summary",
...
def _owner_action(decision: str) -> str:
    ...
    if decision == ACCEPTED:
        return "queue material review; keep not_proven and no OKR lift"
    if decision == NEEDS_REASSIGNMENT:
...
def _safe_phone_copy(decision: str, evidence_ref: str, reasons: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "message": "accepted 只表示 reviewer ACK 可进入后续材料复核，仍不是现场通过、云端通过、手机通过、HIL 或交付成功。",
...
def build_field_evidence_material_resolution_reviewer_ack_review_decision(
    ...
    artifact = {
        ...
        **_safe_flags(),
        ...
        "review_handoff_recommendation": _review_handoff_recommendation(decision),
        "safe_copy": safe_copy,
        "field_evidence_material_resolution_reviewer_ack_review_decision_summary": summary,
        ROBOT_ALIAS: summary,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build trashbot.field_evidence_material_resolution_reviewer_ack_review_decision.v1 from a sanitized "
            "field_evidence_material_resolution_reviewer_ack_intake artifact/summary/Robot alias. Keeps "
    ...
    artifact, summary, exit_code = build_field_evidence_material_resolution_reviewer_ack_review_decision(
    ...
    if args.once_json or not (args.output or args.summary_output):
        ...
        print(f"field_evidence_material_resolution_reviewer_ack_review_decision: artifact_file:{args.output}")
        if args.summary_output:
