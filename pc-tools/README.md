...
2026-07-01 CST 起，普通首屏地图继续按普通用户“大地图优先”处理；当前合同为默认 `2400%` 缩放、最高 `2400%`，`/map` 与 `?view=map` 直达地图大屏默认使用同一上限。ROS2 配套口径保持分层：RViz2 / `nav2_rviz_plugins` 是本地工程调试地图、雷达、TF、规划轨迹和定位的标准工具；Foxglove / `foxglove_bridge` 是浏览器远程观察配套；普通用户默认仍留在 PC 简易工作站超大地图里看路线、小车位置和雷达贴图。该变化只改显示和只读 DOM 合同，不启动 RViz2/Foxglove/ROS2 runtime，不执行 Nav2，不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
...
2026-07-01 CST 起，普通首屏地图默认缩放提升到 `2400%`，`/map` 和 `?view=map` 直达地图大屏默认也使用 `2400%` 上限，并保持地图、路线、小车位置、雷达贴图在同一张 WYSIWYG 画布。“适配”仍可回到 `100%` 看全图。ROS2 配套按用途分层：本地工程调试用 RViz2；需要浏览器远程观察时，在上车侧部署 bridge 后用 Foxglove Studio；普通用户默认仍使用 PC 简易工作站和“打开地图大屏”。该变化只改显示合同，不启动 ROS2/RViz2/Foxglove/Nav2/建图 runtime，不发送任何运动命令。
