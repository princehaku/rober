...
O7 Previews 现在把 `O7 consumer read primary path` 作为历史回放主入口。operator 先用 `Load consumer task list` 调用 PC 后端 `GET /api/o7/consumer-read/tasks?baseUrl=<loopback>`，再选择 task 并用 `Load consumer task detail` 调用 `GET /api/o7/consumer-read/tasks/<task_id>?baseUrl=<loopback>`；PC 后端固定请求 O6 `view=summary` 和 detail `include=trajectory,events,evidence,labeling,inference,tunnel`。主路径 route replay player 只消费 consumer detail 里的 `trajectory.sample_frames`、`events.sample_events`、`evidence.sample_evidence`、`labeling.sample_items`、`inference.sample_results` 和 `tunnel_status` 摘要，支持 Play/Pause、Previous、Next、Reset 和 range cursor，但所有 cursor/playback 都只在浏览器内存中变化，不写后端、不重新下发机器人命令、不证明真实云 archive 或真实机器人运动。缺 detail、unknown task、task id mismatch、轨迹缺失、blocked/not_proven/error/cancel 状态或危险 true 字段都会显示 `blocked_not_proven` / fail-closed reason；页面固定展示 `safe_to_control=false`、`primary_actions_enabled=false`、`robot_control_executed=false`。
