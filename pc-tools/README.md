...
2026-06-30 15:49 CST 起，Robot Control summary 新增只读 `live_closure_summary`，普通首屏在“现在可以做什么”和“当前所见”之间展示 `plain-live-closure-summary`。它把当前路线是否 ready、Nav2 action 是否成功、同窗口 wheel raw L/R 是否非零、delivery success 是否已记录、画面/地图/雷达点是否 WYSIWYG、自由移动和建图入口是否 ready、发车前是否仍为最小安全确认压成一块“当前卡点”。DOM 同步暴露 `data-route-ready`、`data-nav2-goal-succeeded`、`data-wheel-lr-nonzero-proven`、`data-needs-wheel-rerun`、`data-delivery-success`、`data-camera-current-visible`、`data-map-current-visible`、`data-radar-map-points-visible`、`data-free-move-start-ready`、`data-mapping-start-ready`、`data-minimal-precheck-safety-only` 和 `data-sends-motion-when-clicked=false`。该变化只读，不自动执行 Nav2、不提交 delivery、不发送 manual、keyboard、free-roam、stop 或 `/cmd_vel`。

2026-06-30 15:58 CST 起，普通首屏 `plain-wysiwyg-evidence-refresh` 从轻量只读刷新升级为“刷新当前所见（含雷达贴图）”：点击后会调用固定 `POST /api/robot-control/radar/scan-proof/refresh` 刷新 no-motion 雷达扫描 proof，再读取 `/api/robot-control/radar/status`、`/api/robot-control/map/preview` 和 `/api/robot-control/camera/mjpeg/status`，让雷达开始后的地图标记尽量用同轮新扫描贴图。按钮暴露 `data-fixed-radar-refresh-endpoint=/api/robot-control/radar/scan-proof/refresh`、`data-refreshes-radar-scan-proof=true`、`data-refreshes-map-after-radar=true` 和 `data-sends-motion-when-clicked=false`。该动作不启动/停止雷达 lifecycle、不执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
