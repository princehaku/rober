...
本目录是 `rober` 的 **PC 端 dev tool 集合**：路径学习、视觉样本标注/复核、证据交叉检查、模型训练等工作流，统一收口在这里。
...
是 PC-only terminal-result 材料 owner response review-decision gate，能力名
...
`--source` 是 `--input` 的别名；`--evidence-ref` 可选但必须与 source safe
`evidence_ref` 一致。输入支持上一轮
`verified_terminal_result_material_owner_response_intake` artifact、summary、Robot safe
alias `robot_diagnostics_verified_terminal_result_material_owner_response_intake_summary`
以及常见 wrapper/nested JSON。输出写入
`verified_terminal_result_material_owner_response_review_decision.json` 和
`verified_terminal_result_material_owner_response_review_decision_summary.json`，schema
固定为 `trashbot.verified_terminal_result_material_owner_response_review_decision.v1`
和
...
`blocked_missing_terminal_result_owner_response_intake_not_proven` 和
`blocked_evidence_ref_mismatch_not_proven`。该 gate 输出 source owner response
status、safe `evidence_ref`、safe `command_id`、`terminal_result_type`、
`field_owner`、`support_owner`、`reviewer_route`、accepted/missing/rejected/unsafe
materials、`blocked_reason`、`next_required_evidence` 和 `safe_copy`。它拒绝 raw
artifacts、完整 JSON dump、raw owner response body、raw terminal material、凭证、URL、
DB/queue、OSS、本机路径、traceback、ROS topic、`/cmd_vel`、serial/UART、WAVE ROVER、
ACK/cursor/replay/resubmit hints、reviewer-resolution claims 和 success/control
overclaim；accepted 只表示 owner response intake safe metadata 可进入后续 review
handoff，不证明真实 terminal result、delivery success、真实手机/browser、HIL、O5
external proof、PR #5 closure 或机器人控制权限。PR #5 thread `PRRT_kwDOSWB9286CJ3tX`
继续保持 unresolved / hardware_material_pending。
...
是 PC-only terminal-result 材料 owner response review handoff gate，能力名
...
以及常见 wrapper/nested JSON。输出写入
`verified_terminal_result_material_owner_response_review_handoff.json` 和
`verified_terminal_result_material_owner_response_review_handoff_summary.json`，schema
固定为 `trashbot.verified_terminal_result_material_owner_response_review_handoff.v1`
和
...
`handoff_status` 只允许
`accepted_terminal_result_material_owner_response_review_handoff_not_proven`、
`missing_terminal_result_material_owner_response_review_handoff_not_proven`、
`rejected_terminal_result_material_owner_response_review_handoff_not_proven`、
`unsafe_terminal_result_material_owner_response_review_handoff_not_proven`、
`blocked_missing_terminal_result_owner_response_review_decision_not_proven` 和
`blocked_evidence_ref_mismatch_not_proven`。该 gate 输出 source review decision、
safe `evidence_ref`、safe `command_id`、`terminal_result_type`、`field_owner`、
`support_owner`、`reviewer_route`、`owner_handoff`、`support_handoff`、
`reviewer_handoff`、accepted/missing/rejected/unsafe materials、`blocked_reason`、
`next_required_evidence` 和 `safe_copy`。它拒绝 raw artifacts、完整 JSON dump、
raw owner response body、raw terminal material、凭证、URL、DB/queue、OSS、本机路径、
traceback、ROS topic、`/cmd_vel`、serial/UART、WAVE ROVER、ACK/cursor/replay/resubmit
hints、reviewer-resolution claims 和 success/control overclaim；accepted 只表示
脱敏 owner/support/reviewer handoff packet 可交接，不证明真实 terminal result、
delivery success、真实手机/browser、public HTTPS/TLS、4G/SIM、OSS/CDN live traffic、
production DB/queue、worker/cutover、route/elevator field pass、HIL、WAVE ROVER/UART
...
`pc-tools/evidence/verified_terminal_result_material_owner_response_reviewer_ack_intake.py`
是 PC-only reviewer ACK intake gate，能力名
...
输入必须是上一节 `verified_terminal_result_material_owner_response_review_handoff`
artifact、summary、Robot safe alias 或 wrapper/nested safe JSON，再加一个脱敏 reviewer
ACK packet。输出写入
`verified_terminal_result_material_owner_response_reviewer_ack_intake.json` 和
`verified_terminal_result_material_owner_response_reviewer_ack_intake_summary.json`，schema
固定为 `trashbot.verified_terminal_result_material_owner_response_reviewer_ack_intake.v1`
和
...
reviewer ACK packet schema 为
`trashbot.verified_terminal_result_material_owner_response_reviewer_ack_packet.v1`，证据边界固定为
...
`reviewer_ack_state` 只允许 `reviewer_acknowledged_not_proven`、
`reviewer_ack_needs_reassignment`、`reviewer_ack_evidence_ref_mismatch`、
`reviewer_ack_rejected_unsafe` 和
`blocked_missing_terminal_result_owner_response_review_handoff`。该 gate 输出上一跳
handoff status、safe `evidence_ref`、safe `command_id`、`terminal_result_type`、
reviewer ACK role/label/reason、三方 next step、`next_required_evidence`、
`safe_copy`、`pr5_thread` 和 fail-closed diagnostics。ACK acknowledged 只表示同一 safe
`evidence_ref` 下 reviewer 已收到上一跳 safe metadata；它不是真实 terminal result、
delivery/dropoff/cancel success、真实 phone/browser proof、public HTTPS/TLS、4G/SIM、
OSS/CDN live traffic、production DB/queue、route/elevator field pass、HIL、WAVE ROVER/UART
proof、O5 external proof、PR #5 closure 或机器人控制权限。PR #5 thread
`PRRT_kwDOSWB9286CJ3tX` 继续保持 unresolved / hardware_material_pending。
...
是 PC-only reviewer ACK review-handoff gate，能力名
...
`missing_material_not_proven`、`reassignment_required_not_proven`、
`rejected_unsafe_not_proven`、`blocked_missing_source_review_decision_not_proven`
和 `evidence_ref_mismatch_not_proven`。该 gate 只从上一跳 safe review-decision
metadata 派生 source review decision、safe `evidence_ref`、safe `command_id`、
`terminal_result_type`、handoff reasons、`next_required_evidence`、`handoff_action`、
`reviewer_handoff`、`pr5_thread` 和 `safe_copy`；不读取真实材料目录，不触发 GitHub
mutation，不访问 ROS graph、Nav2、serial/UART、WAVE ROVER、真实电梯、外部云、
OSS/CDN、DB/queue、4G 或真实手机/browser。ready 只表示同一 safe `evidence_ref`
下脱敏 review-decision metadata 可交给真实材料 reviewer；missing material /
reassignment / mismatch 都是 fail-closed 人工处理状态，不证明真实 terminal result、
delivery/dropoff/cancel success、真实 phone/browser proof、public HTTPS/TLS、4G/SIM、
OSS/CDN live traffic、production DB/queue、route/elevator field pass、HIL、
...
## hardware sensor HIL-entry callback review decision

`pc-tools/evidence/hardware_sensor_hil_entry_callback_review_decision_gate.py`
是 `hardware_sensor_hil_entry_callback_intake` 之后的 PC-only fail-closed
复核决策 gate：

```bash
python3 pc-tools/evidence/hardware_sensor_hil_entry_callback_review_decision_gate.py \
  --callback-intake-json /tmp/hardware_sensor_hil_entry_callback_intake_summary.json \
  --summary-output /tmp/hardware_sensor_hil_entry_callback_review_decision_summary.json
```

输出 artifact 使用
`schema=trashbot.hardware_sensor_hil_entry_callback_review_decision.v1`，
summary 使用
`schema=trashbot.hardware_sensor_hil_entry_callback_review_decision_summary.v1`，
证据边界固定为
`software_proof_docker_hardware_sensor_hil_entry_callback_review_decision_gate`。
该 gate 只读取上一轮 callback intake artifact、summary 或 nested wrapper，
把 `accepted_callback_materials`、`missing_required_materials` 和
`rejected_callback_materials` 转成 `review_decision`、`decision_reasons`、
`owner_handoff`、`next_required_evidence`、safe rerun command 和 `safe_copy`。

所有输出都固定保持 `source=software_proof`、`hardware_material_pending`、
`not_proven`、`delivery_success=false`、`primary_actions_enabled=false` 和
`safe_to_control=false`。缺 callback intake、unsupported schema/boundary、
`evidence_ref` mismatch、弱 bool/source 合同、raw credentials、raw serial/UART、
完整本机路径、complete artifacts、checksums、HIL pass / field pass /
delivery success copy、`delivery_success=true`、`primary_actions_enabled=true`
或 `safe_to_control=true` 都会 fail closed。

vendor/source boundary 只引用 `docs/vendor/VENDOR_INDEX.md` 及其列出的本地
WAVE ROVER / UART JSON / firmware/vendor-app 资料：`base_ctrl.py`、
`config.yaml`、`json_cmd.h` 和 `uart_ctrl.h`。该 gate 不访问 ROS graph、
Nav2 runtime、serial/UART、WAVE ROVER、真实 2D LiDAR / ToF、HIL rig、真实手机、
外部云、OSS/CDN、DB/queue 或 4G；ready 状态只表示脱敏 callback 材料可进入
Hardware owner 人工复核，不是 HIL pass、PRRT_kwDOSWB9286CJ3tX 关闭、field pass、
Objective 5 external proof 或 delivery success。

## hardware sensor HIL-entry callback review handoff

`pc-tools/evidence/hardware_sensor_hil_entry_callback_review_handoff.py`
是 `hardware_sensor_hil_entry_callback_review_decision` 之后的 PC-only
fail-closed owner handoff gate：

```bash
python3 pc-tools/evidence/hardware_sensor_hil_entry_callback_review_handoff.py \
  --callback-review-decision-json /tmp/hardware_sensor_hil_entry_callback_review_decision_summary.json \
  --summary-output /tmp/hardware_sensor_hil_entry_callback_review_handoff_summary.json
```
...
该 gate 只读取上一轮 callback review decision artifact、summary 或 nested
safe wrapper，把 accepted/ready、missing、rejected、unsafe、evidence-ref
mismatch、unsupported schema 和 malformed input 转成明确 handoff status。

summary 只暴露 `handoff_status`、`source_review_decision_status`、safe
`evidence_ref`、`owner_handoff`、`missing_required_materials`、
`next_required_evidence`、`evidence_boundary` 和 non-claim flags。所有输出都固定
保持 `source=software_proof`、`not_proven`、`safe_to_control=false`、
`delivery_success=false` 和 `primary_actions_enabled=false`。

vendor/source boundary 只引用 `docs/vendor/VENDOR_INDEX.md` 及其列出的本地
WAVE ROVER / UART JSON / firmware/vendor-app 资料：`base_ctrl.py`、
`config.yaml`、`json_cmd.h`、`uart_ctrl.h` 和 `movtion_module.h`。该 gate 只证明
source-boundary context，不证明真实 SKU、安装、接线、电源、标定、
...
### field evidence rerun callback review decision

`pc-tools/evidence/field_evidence_rerun_callback_review_decision.py` 只读上一节 `field_evidence_rerun_callback_intake` artifact、summary 或 wrapper/nested JSON，把 callback-intake 的 `accepted`、`missing`、`rejected`、`blocked` material groups 转成 PC review decision：

```bash
python3 pc-tools/evidence/field_evidence_rerun_callback_review_decision.py \
  --callback-intake-json /tmp/field_evidence_rerun_callback_intake_summary.json \
  --evidence-ref /tmp/same_evidence_ref.json \
  --output /tmp/field_evidence_rerun_callback_review_decision.json \
  --summary-output /tmp/field_evidence_rerun_callback_review_decision_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_callback_review_decision.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_callback_review_decision_summary.v1`，证据边界固定为 `software_proof_docker_field_evidence_rerun_callback_review_decision_gate`。核心字段包括 `review_decision`、safe `evidence_ref`、`owner_handoff`、`next_required_evidence`、`rerun_guidance`、`blocker_summary`、`same_evidence_ref_status`、`accepted_materials`、`missing_materials`、`rejected_materials`、`blocked_materials`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。

`review_decision` 允许值限定为 `accepted`、`missing`、`rejected` 和 `blocked`。material classes 固定覆盖 `real route completion signal`、`real field task record`、`real Nav2/fixed-route runtime log`、`real elevator door summary`、`real target floor / floor arrival summary`、`real human-assistance summary`、`real dropoff completion`、`real cancel completion`、`real delivery result` 和 `real phone/browser evidence`。缺 callback-intake 输入、缺 summary、bad JSON、unsupported schema/boundary、same safe `evidence_ref` 不一致、unsafe copy、raw path、credential、ROS topic、serial/UART/WAVE ROVER detail、checksum、完整 artifact、traceback、success/control claim、`safe_to_control=true`、`delivery_success=true` 或 `primary_actions_enabled=true` 都会 fail closed 到 `review_decision=blocked`。

该 gate 只证明本地 callback review decision software proof 可复跑，不读取真实材料目录，不访问 ROS graph、Nav2/fixed-route runtime、serial/UART、WAVE ROVER、真实电梯、外部云、OSS/CDN、DB/queue、4G 或真实手机/browser。`accepted` 只表示 callback-intake 的十类 safe material groups 已被复核为可交给后续人工 closeout，不是真实 route/elevator field pass、真实 dropoff/cancel completion、delivery success、HIL、O5 external proof、PR #5 resolved 或真实 phone/browser 证据。

### field evidence rerun callback review handoff

`pc-tools/evidence/field_evidence_rerun_callback_review_handoff.py` 只读上一节 `field_evidence_rerun_callback_review_decision` artifact、summary 或 wrapper/nested JSON，把 review decision、owner handoff、next required evidence、rerun guidance 和 blocker summary 打包成 metadata-only handoff artifact / summary：

```bash
python3 pc-tools/evidence/field_evidence_rerun_callback_review_handoff.py \
  --callback-review-decision-json /tmp/field_evidence_rerun_callback_review_decision_summary.json \
  --evidence-ref /tmp/same_evidence_ref.json \
  --output /tmp/field_evidence_rerun_callback_review_handoff.json \
  --summary-output /tmp/field_evidence_rerun_callback_review_handoff_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_callback_review_handoff.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_callback_review_handoff_summary.v1`，证据边界固定为 `software_proof_docker_field_evidence_rerun_callback_review_handoff_gate`。核心字段包括 `handoff_status`、source `review_decision`、safe `evidence_ref`、`owner_handoff`、`handoff_package`、`next_required_evidence`、`rerun_guidance`、`blocker_summary`、`safe_copy`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。

`handoff_status` 固定包含 `ready_for_field_evidence_rerun_callback_review_handoff`、`needs_owner_follow_up`、`needs_field_evidence_rerun_callback_rerun`、`evidence_ref_mismatch_rerun` 和 `blocked_unsafe_review_handoff`。顶层验收可用 `python3 -m unittest tests.test_field_evidence_rerun_callback_review_handoff` 复用离线围栏测试；该入口只证明本地 callback-review-handoff gate 的 fail-closed contract 可复跑，仍属于 Docker/local software proof。

该 gate 不读取真实材料目录、不触发现场复跑、不访问 ROS graph、Nav2/fixed-route runtime、serial/UART、WAVE ROVER、真实电梯、外部云、OSS/CDN、DB/queue、4G 或真实手机/browser，也不执行任何机器人动作。缺 review-decision 输入、bad JSON、unsupported schema/boundary、source 不是 `source=software_proof` 或缺 `not_proven`、证据号不一致、弱类型 `same_evidence_ref_required`、unsupported `review_decision`、unsafe copy、raw path、credential、ROS topic、serial/UART/WAVE ROVER detail、checksum、完整 artifact、traceback、success/control claim、`safe_to_control=true`、`delivery_success=true` 或 `primary_actions_enabled=true` 都会 fail closed。`ready_for_field_evidence_rerun_callback_review_handoff` 只表示 Docker/local `software_proof_docker_field_evidence_rerun_callback_review_handoff_gate` 已把上一轮 review decision 交接成只读 handoff package，不是真实 route/elevator field pass、真实 Nav2/fixed-route proof、dropoff/cancel completion、delivery success、HIL、O5 external proof、PR #5 resolved 或真实 phone/browser 证据。

## field evidence rerun handoff intake

`pc-tools/evidence/field_evidence_rerun_handoff_intake.py` 只读上一节 `field_evidence_rerun_callback_review_handoff` artifact/summary/wrapper JSON，以及现场 owner 回填的 owner-safe handoff intake packet，把“交接包已被安全接收”复账成 metadata-only artifact / summary：

```bash
python3 pc-tools/evidence/field_evidence_rerun_handoff_intake.py \
  --callback-review-handoff-json /tmp/field_evidence_rerun_callback_review_handoff_summary.json \
  --handoff-intake-packet-json /tmp/field_evidence_rerun_handoff_intake_packet.json \
  --evidence-ref ev-field-intake-001 \
  --output /tmp/field_evidence_rerun_handoff_intake.json \
  --summary-output /tmp/field_evidence_rerun_handoff_intake_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_handoff_intake.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_handoff_intake_summary.v1`，证据边界固定为 `software_proof_docker_field_evidence_rerun_handoff_intake_gate`。核心字段包括 `intake_status`、source `handoff_status`、safe `evidence_ref`、`owner_intake`、`next_required_evidence`、`rerun_guidance`、`blocker_summary`、`safe_copy`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。

`intake_status` 固定包含 `ready_for_field_evidence_rerun_handoff_intake_not_proven`、`blocked_missing_field_evidence_rerun_handoff_intake_packet`、`blocked_unsupported_field_evidence_rerun_handoff_intake_source`、`blocked_field_evidence_rerun_review_handoff_not_ready`、`evidence_ref_mismatch_field_evidence_rerun_handoff_intake_blocked` 和 `blocked_unsafe_field_evidence_rerun_handoff_intake_copy`。顶层验收可用 `python3 -m unittest tests.test_field_evidence_rerun_handoff_intake` 复用离线围栏测试；该入口只证明本地 handoff-intake gate 的 fail-closed contract 可复跑，仍属于 Docker/local software proof。
...
## field evidence rerun execution callback review decision

`pc-tools/evidence/field_evidence_rerun_execution_callback_review_decision.py` 只读上一节 `field_evidence_rerun_execution_callback_intake` artifact、summary、Robot safe alias 或 wrapper/nested JSON，把 execution callback intake 的 `accepted`、`missing`、`rejected`、`blocked` material groups 转成 canonical review-decision artifact / summary：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_callback_review_decision.py \
  --callback-intake-json /tmp/field_evidence_rerun_execution_callback_intake_summary.json \
  --evidence-ref ev-field-queue-001 \
  --output /tmp/field_evidence_rerun_execution_callback_review_decision.json \
  --summary-output /tmp/field_evidence_rerun_execution_callback_review_decision_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_callback_review_decision.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_callback_review_decision_summary.v1`，Robot safe alias 为 `robot_diagnostics_field_evidence_rerun_execution_callback_review_decision_summary`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_callback_review_decision_gate`。核心字段包括 `review_decision`、`decision_reasons`、safe `evidence_ref`、`source_callback_intake_status`、`same_evidence_ref_status`、`accepted_materials`、`missing_materials`、`rejected_materials`、`blocked_materials`、`owner_handoff`、`next_required_evidence`、`rerun_guidance`、`safe_copy`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。

`review_decision` 固定包含 `ready`、`missing`、`rejected`、`blocked`、`unsupported`、`unsafe`、`evidence_ref_mismatch` 和 `source_not_ready`。顶层 acceptance command 可用 `python3 -m unittest tests.test_field_evidence_rerun_execution_callback_review_decision` 运行离线围栏测试；该入口只证明本地 callback-review-decision gate 的 fail-closed contract 可复跑，仍属于 Docker/local software proof。

该 gate 不读取真实材料目录、不触发现场复跑、不访问 ROS graph、Nav2/fixed-route runtime、serial/UART、WAVE ROVER、真实电梯、外部云、OSS/CDN、DB/queue、4G 或真实手机/browser，也不执行任何机器人动作。缺 callback-intake 输入、bad JSON、unsupported schema/boundary、source 不是 `source=software_proof` 或缺 `not_proven`、source callback intake 不是 `ready_for_field_evidence_rerun_execution_callback_intake_not_proven`、证据号不一致、弱类型 `same_evidence_ref_required`、`same_evidence_ref_status` 非 matched/ready、unknown category、跨组重复 category、unsafe copy、raw path、credential、ROS topic、`/cmd_vel`、serial/UART/WAVE ROVER detail、checksum、完整/raw artifact、traceback、success/control wording、`safe_to_control=true`、`delivery_success=true` 或 `primary_actions_enabled=true` 都会 fail closed。`ready` 只表示 Docker/local `software_proof_docker_field_evidence_rerun_execution_callback_review_decision_gate` 已把同一 safe `evidence_ref` 的 execution callback intake 复核为只读 review decision，不是真实现场复跑、真实 Nav2/fixed-route proof、真实 route/elevator field pass、task record/completion signal、dropoff/cancel completion、delivery success、HIL、O5 external proof、PR #5 resolved 或真实 phone/browser 证据。

## field evidence rerun execution callback review handoff

`pc-tools/evidence/field_evidence_rerun_execution_callback_review_handoff.py` 只读上一节 `field_evidence_rerun_execution_callback_review_decision` artifact、summary、Robot safe alias 或 wrapper/nested JSON，把 execution callback review decision、owner handoff、next required evidence、rerun guidance 和 blocker summary 打包成 metadata-only handoff artifact / summary：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_callback_review_handoff.py \
  --execution-callback-decision-json /tmp/field_evidence_rerun_execution_callback_review_decision_summary.json \
  --evidence-ref ev-field-queue-001 \
  --output /tmp/field_evidence_rerun_execution_callback_review_handoff.json \
  --summary-output /tmp/field_evidence_rerun_execution_callback_review_handoff_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_callback_review_handoff.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_callback_review_handoff_summary.v1`，Robot safe alias 为 `robot_diagnostics_field_evidence_rerun_execution_callback_review_handoff_summary`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_callback_review_handoff_gate`。核心字段包括 `handoff_status`、source `review_decision`、safe `evidence_ref`、`same_evidence_ref_status`、`owner_handoff`、`handoff_package`、`next_required_evidence`、`rerun_guidance`、`blocker_summary`、`safe_copy`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。

`handoff_status` 固定包含 `ready_for_field_evidence_rerun_execution_callback_review_handoff`、`needs_owner_follow_up`、`needs_field_evidence_rerun_execution_callback_rerun`、`evidence_ref_mismatch_rerun` 和 `blocked_unsafe_review_handoff`。顶层 acceptance command 可用 `python3 -m unittest tests.test_field_evidence_rerun_execution_callback_review_handoff` 运行离线围栏测试；该入口只证明本地 execution-callback-review-handoff gate 的 fail-closed contract 可复跑，仍属于 Docker/local software proof。

该 gate 不读取真实材料目录、不触发现场复跑、不访问 ROS graph、Nav2/fixed-route runtime、serial/UART、WAVE ROVER、真实电梯、外部云、OSS/CDN、DB/queue、4G 或真实手机/browser，也不执行任何机器人动作。缺 execution callback review decision 输入、bad JSON、unsupported schema/boundary、source 不是 `source=software_proof` 或缺 `not_proven`、source review decision 不是 `ready/missing/rejected/blocked`、证据号不一致、弱类型 `same_evidence_ref_required`、`same_evidence_ref_status` 非 matched/ready、unsafe copy、raw path、credential、ROS topic、`/cmd_vel`、serial/UART/WAVE ROVER detail、checksum、完整/raw artifact、traceback、success/control wording、true field pass、O5 external proof、PR #5 resolution claim、`safe_to_control=true`、`delivery_success=true` 或 `primary_actions_enabled=true` 都会 fail closed。`ready_for_field_evidence_rerun_execution_callback_review_handoff` 只表示 Docker/local `software_proof_docker_field_evidence_rerun_execution_callback_review_handoff_gate` 已把上一轮 review decision 交接成只读 handoff package，不是真实现场复跑、真实 Nav2/fixed-route proof、真实 route/elevator field pass、task record/completion signal、dropoff/cancel completion、delivery success、HIL、O5 external proof、PR #5 resolved 或真实 phone/browser 证据。

## field evidence rerun execution result intake

`pc-tools/evidence/field_evidence_rerun_execution_result_intake.py` 只读上一节 `field_evidence_rerun_execution_callback_review_handoff` artifact、summary、Robot safe alias 或 wrapper/nested JSON，并可选读取现场 owner-safe execution result packet，把 packet 状态收敛成 `missing` / `accepted` / `rejected` / `blocked`：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_intake.py \
  --execution-callback-handoff-json /tmp/field_evidence_rerun_execution_callback_review_handoff_summary.json \
  --result-packet-json /tmp/field_evidence_rerun_execution_result_packet.json \
  --evidence-ref ev-field-queue-001 \
  --output /tmp/field_evidence_rerun_execution_result_intake.json \
  --summary-output /tmp/field_evidence_rerun_execution_result_intake_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_intake.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_intake_summary.v1`，Robot safe alias 为 `robot_diagnostics_field_evidence_rerun_execution_result_intake_summary`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_result_intake_gate`。核心字段包括 `result_intake_status`、source `handoff_status`、`result_packet_status`、safe `evidence_ref`、`same_evidence_ref_status`、`material_summary`、`owner_handoff`、`next_required_evidence`、`reconciliation_hint`、`safe_copy`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。

`result_intake_status` 只能是 `missing`、`accepted`、`rejected` 或 `blocked`。缺 result packet 或 packet 标记 `missing` 输出 `missing`；packet 标记 `accepted` 输出 `accepted`，但只表示 accepted for review intake，不是真实 field pass；packet 标记 `rejected` 或 `blocked` 原样保留短状态。顶层 acceptance command 可用 `python3 -m unittest tests.test_field_evidence_rerun_execution_result_intake` 运行离线围栏测试；该入口只证明本地 result-intake gate 的 fail-closed contract 可复跑，仍属于 Docker/local software proof。

该 gate 不读取真实材料目录、不触发现场复跑、不访问 ROS graph、Nav2/fixed-route runtime、serial/UART、WAVE ROVER、真实电梯、外部云、OSS/CDN、DB/queue、4G 或真实手机/browser，也不执行任何机器人动作。缺 handoff 输入、bad JSON、unsupported schema/boundary、source 不是 `source=software_proof` 或缺 `not_proven`、source handoff 不是 `ready_for_field_evidence_rerun_execution_callback_review_handoff`、result packet 非 owner-safe schema、packet 不是 `source=software_proof` 或缺 `not_proven`、packet 状态不在 `missing/accepted/rejected/blocked`、证据号不一致、弱类型 `same_evidence_ref_required`、`same_evidence_ref_status` 非 matched/ready、unsafe copy、raw path、credential、ROS topic、`/cmd_vel`、serial/UART/WAVE ROVER detail、checksum、完整/raw artifact、traceback、success/control wording、true field pass、O5 external proof、PR #5 resolution claim、PR #6 runtime proof、`safe_to_control=true`、`delivery_success=true` 或 `primary_actions_enabled=true` 都会 fail closed 到 `blocked`。`accepted` 只表示 Docker/local `software_proof_docker_field_evidence_rerun_execution_result_intake_gate` 已接收可进入下一轮 review intake 的安全结果包，不是真实现场复跑、真实 Nav2/fixed-route proof、真实 route/elevator field pass、task record/completion signal、dropoff/cancel completion、delivery success、HIL、O5 external proof、PR #5 resolved、PR #6 runtime proof 或真实 phone/browser 证据。

`pc-tools/evidence/field_evidence_rerun_execution_result_review_decision.py` 只读上一节 `field_evidence_rerun_execution_result_intake` artifact、summary、Robot safe alias 或 wrapper/nested JSON，把 result-intake 状态转成 metadata-only review-decision artifact / summary：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_review_decision.py \
  --result-intake-json /tmp/field_evidence_rerun_execution_result_intake_summary.json \
  --evidence-ref ev-field-result-review-001 \
  --output /tmp/field_evidence_rerun_execution_result_review_decision.json \
  --summary-output /tmp/field_evidence_rerun_execution_result_review_decision_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_review_decision.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_review_decision_summary.v1`，Robot safe alias 为 `robot_diagnostics_field_evidence_rerun_execution_result_review_decision_summary`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_result_review_decision_gate`。核心字段包括 `review_decision`、`decision_reasons`、safe `evidence_ref`、`source_result_intake_status`、`same_evidence_ref_status`、`owner_handoff`、`next_required_evidence`、`reconciliation_hint`、`blocker_summary`、`safe_copy`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。

`review_decision` 固定包含 `accepted_for_review`、`needs_material_backfill`、`rejected` 和 `blocked`。顶层 acceptance command 可用 `python3 -m unittest tests.test_field_evidence_rerun_execution_result_review_decision` 运行离线围栏测试；该入口只证明本地 result-review-decision gate 的 fail-closed contract 可复跑，仍属于 Docker/local software proof。
...
`pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_packet.py` 只读上一节 `field_evidence_rerun_execution_result_review_handoff` artifact、summary、Robot safe alias 或 wrapper/nested JSON，并读取 field owner 提交的脱敏 acceptance packet，把同一 safe `evidence_ref` 下的现场执行结果材料整理成 metadata-only 验收 readiness artifact / summary：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_packet.py \
  --review-handoff-json /tmp/field_evidence_rerun_execution_result_review_handoff_summary.json \
  --acceptance-packet-json /tmp/field_evidence_rerun_execution_result_acceptance_packet_input.json \
  --evidence-ref ev-field-result-review-001 \
  --output /tmp/field_evidence_rerun_execution_result_acceptance_packet.json \
  --summary-output /tmp/field_evidence_rerun_execution_result_acceptance_packet_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_packet.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_packet_summary.v1`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_packet_gate`。核心字段包括 `acceptance_verdict`、safe `evidence_ref`、`same_evidence_ref_status`、`required_materials`、`accepted_materials`、`missing_materials`、`rejected_materials`、`acceptance_gap_summary`、`owner_handoff`、`next_required_evidence`、`safe_copy`、`source=software_proof`、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false` 和 `safe_to_control=false`。

`required_materials` 固定包含 `task_record`、`nav2_fixed_route_runtime_log`、`route_completion_signal`、`elevator_evidence`、`dropoff_cancel_completion`、`delivery_result`、`true_phone_browser_evidence` 和 `diagnostics_mobile_safe_summary`。顶层 acceptance command 可用 `python3 -m unittest pc-tools/evidence/test_field_evidence_rerun_execution_result_acceptance_packet.py` 运行离线围栏测试；该入口只证明本地 acceptance packet gate 的 fail-closed contract 可复跑，仍属于 Docker/local software proof。

该 gate 不读取真实 task record、Nav2/fixed-route runtime log、route completion signal、elevator evidence、dropoff/cancel completion、delivery result、true phone/browser evidence 或 raw diagnostics，不访问 ROS graph、serial/UART、WAVE ROVER、真实电梯、外部云、OSS/CDN、DB/queue、4G 或真实手机/browser，也不执行任何机器人动作。缺 review handoff、缺 acceptance packet、bad JSON、unsupported schema/boundary、source 不是 `source=software_proof` 或缺 `not_proven`、source handoff 不是 `ready_for_field_evidence_rerun_execution_result_review_owner_handoff_not_proven`、证据号不一致、弱类型 `same_evidence_ref_required`、缺任一 `required_materials`、unsafe copy、raw path、credential、ROS topic、`/cmd_vel`、serial/UART/WAVE ROVER detail、checksum、完整/raw artifact、traceback、success/control wording、`safe_to_control=true`、`delivery_success=true` 或 `primary_actions_enabled=true` 都会 fail closed 到 `blocked_missing_acceptance_packet`、`needs_material_backfill`、`blocked_evidence_ref_mismatch`、`blocked_unsafe_material` 或 `rejected_success_claim`。`ready_for_field_owner_acceptance_review_not_proven` 只表示 Docker/local `software_proof_docker_field_evidence_rerun_execution_result_acceptance_packet_gate` 已把八类材料索引整理到同一 safe `evidence_ref` 下可供人工验收复核，不是真实现场复跑、真实 Nav2/fixed-route proof、真实 route/elevator field pass、真实 task record、真实 route completion signal、真实 dropoff/cancel completion、delivery result、delivery success、HIL、O5 external proof、PR #5 resolved、PR #6 runtime proof 或真实 phone/browser 证据。

`pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_backfill.py` 只读上一节 `field_evidence_rerun_execution_result_acceptance_packet` artifact、summary、Robot safe alias 或 wrapper/nested JSON，并读取 `--material-dir` 中现场 owner 提交的脱敏回填材料，把 packet status 与 material completeness、same-`evidence_ref` alignment、missing/rejected material categories、owner handoff、rerun commands 和 decision inputs 汇总成 PC acceptance backfill artifact / summary：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_backfill.py \
  --acceptance-packet-json /tmp/field_evidence_rerun_execution_result_acceptance_packet_summary.json \
  --material-dir /tmp/field_evidence_rerun_execution_result_acceptance_backfill_materials \
  --evidence-ref ev-field-result-review-001 \
  --output /tmp/field_evidence_rerun_execution_result_acceptance_backfill.json \
  --summary-output /tmp/field_evidence_rerun_execution_result_acceptance_backfill_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_backfill.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_backfill_summary.v1`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_backfill_gate`。核心字段包括 safe `evidence_ref`、`source_acceptance_packet`、safe lineage、`required_materials`、`material_states`、`material_completeness`、`same_evidence_ref_alignment`、`acceptance_backfill_gap_summary`、`owner_handoff`、`rerun_commands`、`decision_inputs`、`safe_copy`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。

acceptance backfill 固定检查八类回填材料：`task_record`、`nav2_fixed_route_runtime_log`、`route_completion_signal`、`elevator_evidence`、`dropoff_cancel_completion`、`delivery_result`、`true_phone_browser_evidence` 和 `diagnostics_mobile_safe_summary`。顶层 acceptance command 可用 `python3 -m unittest pc-tools/evidence/test_field_evidence_rerun_execution_result_acceptance_backfill.py` 运行离线围栏测试；该入口只证明本地 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_backfill_gate` 的 fail-closed contract 可复跑，仍属于 Docker/local software proof。
...
`pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_review_handoff.py` 只读上一节 `field_evidence_rerun_execution_result_acceptance_backfill_review_decision` artifact、summary、Robot diagnostics safe alias 或 wrapper/nested JSON，把 safe review-decision 转成 sanitized owner/support/reviewer handoff package：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_review_handoff.py \
  --review-decision-json /tmp/field_evidence_rerun_execution_result_acceptance_backfill_review_decision_summary.json \
  --evidence-ref ev-field-result-review-001 \
  --output /tmp/field_evidence_rerun_execution_result_acceptance_review_handoff.json \
  --summary-output /tmp/field_evidence_rerun_execution_result_acceptance_review_handoff_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_review_handoff.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_review_handoff_summary.v1`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_review_handoff_gate`。核心字段包括 `handoff_status`、`allowed_handoff_states`、`handoff_reasons`、safe `evidence_ref`、`source_review_decision`、`material_status`、`handoff_checklist`、`owner_handoff`、`next_required_evidence`、`rerun_commands`、`safe_copy`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。

`handoff_status` 只允许 `ready_for_field_owner_support_reviewer_handoff_not_proven`、`handoff_needs_more_material`、`handoff_evidence_ref_mismatch`、`handoff_unsafe_rejected` 和 `blocked_missing_review_decision`。handoff checklist 固定列出 `true task record`、`true Nav2/fixed-route runtime log`、`route completion signal`、`true elevator door state`、`target floor confirmation`、`human assistance record`、`dropoff/cancel completion or delivery result` 和 `true phone/browser evidence`；顶层 acceptance command 可用 `python3 -m unittest pc-tools/evidence/test_field_evidence_rerun_execution_result_acceptance_review_handoff.py` 运行离线围栏测试。

该 gate 不读取真实 task record、Nav2/fixed-route runtime log、route completion signal、电梯门/目标楼层证据、人工协助记录、dropoff/cancel completion、delivery result、true phone/browser evidence 或 raw diagnostics，不访问 ROS graph、serial/UART、WAVE ROVER、真实电梯、外部云、真实手机/browser，也不执行任何机器人动作。缺 source、坏 JSON、unsupported schema/boundary、source 不是 `source=software_proof` 或缺 `not_proven`、上游不是 `ready_for_field_rerun_result_acceptance_review_handoff`、上游 boundary 不是 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_backfill_review_decision_gate`、缺 safe `evidence_ref`、证据号不一致、弱类型 `same_evidence_ref_required`、缺/拒绝任一回填材料、unsafe copy、sensitive copy、raw/local paths、ROS topics、serial/UART/WAVE ROVER details、credentials、external proof/HIL/PR #5 resolution claims、success/control claim、`safe_to_control=true`、`delivery_success=true` 或 `primary_actions_enabled=true` 都会 fail closed。`ready_for_field_owner_support_reviewer_handoff_not_proven` 只表示 Docker/local `software_proof_docker_field_evidence_rerun_execution_result_acceptance_review_handoff_gate` 已把上一轮 review-decision 安全转成交接包，不是真实 field rerun、真实 route/elevator pass、真实 Nav2/fixed-route、verified terminal result、dropoff/cancel completion、delivery result、delivery success、HIL、真实手机/browser、PR #5 resolution 或 Objective 5 external proof。

`pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake.py` 只读上一节 `field_evidence_rerun_execution_result_acceptance_review_handoff` artifact、summary、Robot diagnostics safe alias 或 wrapper/nested JSON，并读取 field owner/support 提交的脱敏 intake packet，把上一轮 handoff 与 owner/support acknowledgement 复账成 sanitized intake artifact / summary：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake.py \
  --review-handoff-json /tmp/field_evidence_rerun_execution_result_acceptance_review_handoff_summary.json \
  --owner-intake-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_owner_intake.json \
  --evidence-ref ev-field-result-review-001 \
  --output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake.json \
  --summary-output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_summary.v1`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_gate`。核心字段包括 `intake_status`、`allowed_intake_states`、`intake_reasons`、safe `evidence_ref`、`source_review_handoff`、`owner_support_packet`、`required_materials`、`material_status`、`intake_checklist`、`owner_intake`、`next_required_evidence`、`rerun_commands`、`safe_copy`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。

`intake_status` 只允许 `ready_for_acceptance_handoff_owner_intake_not_proven`、`intake_needs_more_material`、`intake_evidence_ref_mismatch`、`intake_unsafe_rejected` 和 `blocked_missing_review_handoff`。owner/support intake checklist 固定列出 `true task record`、`true Nav2/fixed-route runtime log`、`route completion signal`、`true elevator door state`、`target floor confirmation`、`human assistance record`、`dropoff/cancel completion or delivery result` 和 `true phone/browser evidence`；顶层 acceptance command 可用 `python3 -m unittest pc-tools/evidence/test_field_evidence_rerun_execution_result_acceptance_handoff_intake.py` 运行离线围栏测试。CLI 仅在 `ready_for_acceptance_handoff_owner_intake_not_proven` 时返回 0；缺材料、证据号不一致、unsafe/rejected 或缺 handoff 均返回非 0。

该 gate 不读取真实 task record、Nav2/fixed-route runtime log、route completion signal、电梯门/目标楼层证据、人工协助记录、dropoff/cancel completion、delivery result、true phone/browser evidence 或 raw diagnostics，不访问 ROS graph、serial/UART、WAVE ROVER、真实电梯、外部云、真实手机/browser，也不执行任何机器人动作。缺 review handoff、缺 owner/support packet、坏 JSON、unsupported schema/boundary、source 或 owner packet 不是 `source=software_proof` 或缺 `not_proven`、上游不是 `ready_for_field_owner_support_reviewer_handoff_not_proven`、上游 boundary 不是 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_review_handoff_gate`、缺 safe `evidence_ref`、证据号不一致、弱类型 `same_evidence_ref_required`、缺任一 required checklist acknowledgement、rejected/unsafe material refs、unsafe copy、sensitive copy、raw/local paths、ROS topics、serial/UART/WAVE ROVER details、credentials、raw/complete artifact、checksum、traceback、true phone/browser proof、real route/elevator field pass、verified terminal result、Objective 5 external proof、HIL/PR #5 resolution claims、success/control claim、`safe_to_control=true`、`delivery_success=true` 或 `primary_actions_enabled=true` 都会 fail closed。`ready_for_acceptance_handoff_owner_intake_not_proven` 只表示 Docker/local `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_gate` 已把上一轮 review handoff 和 owner/support intake packet 安全复账，不是真实 field rerun、真实 route/elevator pass、真实 Nav2/fixed-route、verified terminal result、dropoff/cancel completion、delivery result、delivery success、HIL、真实手机/browser、PR #5 resolution 或 Objective 5 external proof。

`pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision.py` 只读上一节 `field_evidence_rerun_execution_result_acceptance_handoff_intake` artifact、summary、Robot diagnostics safe alias 或 wrapper/nested JSON，并读取 field owner/support 提交的脱敏 review packet，把 owner/support intake 转成 sanitized review-decision artifact / summary：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision.py \
  --handoff-intake-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_summary.json \
  --review-packet-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_review_packet.json \
  --evidence-ref ev-field-result-review-001 \
  --output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision.json \
  --summary-output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision_summary.v1`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision_gate`。核心字段包括 `review_decision`、`allowed_review_decisions`、`decision_reasons`、safe `evidence_ref`、`source_handoff_intake`、`owner_support_review_packet`、`required_materials`、`material_status`、`review_checklist`、`owner_handoff`、`next_required_evidence`、`rerun_commands`、`safe_copy`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。

`review_decision` 只允许 `ready_for_acceptance_handoff_review_handoff_not_proven`、`review_needs_owner_rework`、`review_evidence_ref_mismatch`、`review_unsafe_rejected` 和 `blocked_missing_handoff_intake`。owner/support review checklist 固定列出 `true task record`、`true Nav2/fixed-route runtime log`、`route completion signal`、`true elevator door state`、`target floor confirmation`、`human assistance record`、`dropoff/cancel completion or delivery result` 和 `true phone/browser evidence`；顶层 acceptance command 可用 `python3 -m unittest pc-tools/evidence/test_field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision.py` 运行离线围栏测试。CLI 仅在 `ready_for_acceptance_handoff_review_handoff_not_proven` 时返回 0；缺材料、证据号不一致、unsafe/rejected 或缺 handoff intake 均返回非 0。

该 gate 不读取真实 task record、Nav2/fixed-route runtime log、route completion signal、电梯门/目标楼层证据、人工协助记录、dropoff/cancel completion、delivery result、true phone/browser evidence 或 raw diagnostics，不访问 ROS graph、serial/UART、WAVE ROVER、真实电梯、外部云、真实手机/browser，也不执行任何机器人动作。缺 handoff intake、缺 owner/support review packet、坏 JSON、unsupported schema/boundary、source 或 review packet 不是 `source=software_proof` 或缺 `not_proven`、上游不是 `ready_for_acceptance_handoff_owner_intake_not_proven`、上游 boundary 不是 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_gate`、缺 safe `evidence_ref`、证据号不一致、弱类型 `same_evidence_ref_required`、缺任一 required checklist review、rejected/unsafe material refs、unsafe copy、sensitive copy、raw/local paths、ROS topics、serial/UART/WAVE ROVER details、credentials、raw/complete artifact、checksum、traceback、true phone/browser proof、real route/elevator field pass、verified terminal result、Objective 5 external proof、HIL/PR #5 resolution claims、success/control claim、`safe_to_control=true`、`delivery_success=true` 或 `primary_actions_enabled=true` 都会 fail closed。`ready_for_acceptance_handoff_review_handoff_not_proven` 只表示 Docker/local `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision_gate` 已把上一轮 owner/support intake 和 review packet 安全复核成可进入下一步 handoff 的只读决策，不是真实 field rerun、真实 route/elevator pass、真实 Nav2/fixed-route、verified terminal result、dropoff/cancel completion、delivery result、delivery success、HIL、真实手机/browser、PR #5 resolution 或 Objective 5 external proof。

`pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_review_handoff.py` 只读上一节 `field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision` artifact、summary、Robot diagnostics safe alias 或 wrapper/nested JSON，并读取 field owner/support/reviewer 提交的脱敏 handoff packet，把 review decision 转成 sanitized review-handoff artifact / summary：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_review_handoff.py \
  --review-decision-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision_summary.json \
  --handoff-packet-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_review_handoff_packet.json \
  --evidence-ref ev-field-result-review-001 \
  --output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_review_handoff.json \
  --summary-output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_review_handoff_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_review_handoff.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_review_handoff_summary.v1`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_review_handoff_gate`。核心字段包括 `handoff_status`、`allowed_handoff_states`、`handoff_reasons`、safe `evidence_ref`、`source_review_decision`、`owner_support_reviewer_handoff_packet`、`required_materials`、`material_status`、`handoff_checklist`、`owner_handoff`、`next_required_evidence`、`rerun_commands`、`safe_copy`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。

`handoff_status` 只允许 `ready_for_acceptance_review_handoff_not_proven`、`handoff_needs_owner_rework`、`handoff_evidence_ref_mismatch`、`handoff_unsafe_rejected` 和 `blocked_missing_review_decision`。owner/support/reviewer handoff checklist 固定列出 `true task record`、`true Nav2/fixed-route runtime log`、`route completion signal`、`true elevator door state`、`target floor confirmation`、`human assistance record`、`dropoff/cancel completion or delivery result` 和 `true phone/browser evidence`；顶层 acceptance command 可用 `python3 -m unittest pc-tools/evidence/test_field_evidence_rerun_execution_result_acceptance_handoff_intake_review_handoff.py` 运行离线围栏测试。CLI 仅在 `ready_for_acceptance_review_handoff_not_proven` 时返回 0；缺材料、证据号不一致、unsafe/rejected 或缺 review decision 均返回非 0。

该 gate 不读取真实 task record、Nav2/fixed-route runtime log、route completion signal、电梯门/目标楼层证据、人工协助记录、dropoff/cancel completion、delivery result、true phone/browser evidence 或 raw diagnostics，不访问 ROS graph、serial/UART、WAVE ROVER、真实电梯、外部云、真实手机/browser，也不执行任何机器人动作。缺 review decision、缺 owner/support/reviewer handoff packet、坏 JSON、unsupported schema/boundary、source 或 handoff packet 不是 `source=software_proof` 或缺 `not_proven`、上游不是 `ready_for_acceptance_handoff_review_handoff_not_proven`、上游 boundary 不是 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision_gate`、缺 safe `evidence_ref`、证据号不一致、弱类型 `same_evidence_ref_required`、缺任一 required checklist handoff、rejected/unsafe material refs、unsafe copy、sensitive copy、raw/local paths、ROS topics、serial/UART/WAVE ROVER details、credentials、raw/complete artifact、checksum、traceback、true phone/browser proof、real route/elevator field pass、verified terminal result、Objective 5 external proof、HIL/PR #5 resolution claims、success/control claim、`safe_to_control=true`、`delivery_success=true` 或 `primary_actions_enabled=true` 都会 fail closed。`ready_for_acceptance_review_handoff_not_proven` 只表示 Docker/local `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_review_handoff_gate` 已把上一轮 review decision 和 handoff packet 安全复核成可交给 field owner/support/reviewer 的只读交接包，不是真实 field rerun、真实 route/elevator pass、真实 Nav2/fixed-route、verified terminal result、dropoff/cancel completion、delivery result、delivery success、HIL、真实手机/browser、PR #5 resolution 或 Objective 5 external proof。

`pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_followup_escalation_status.py` 只读上一节 `field_evidence_rerun_execution_result_acceptance_handoff_intake_review_handoff` artifact、summary、Robot diagnostics safe alias 或 wrapper/nested JSON，并读取 owner/support/reviewer 提交的脱敏 follow-up policy，把同一 safe `evidence_ref` 下的跟进到期状态整理成 PC follow-up escalation artifact / summary：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_followup_escalation_status.py \
  --review-handoff-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_review_handoff_summary.json \
  --followup-policy-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_followup_policy.json \
  --evidence-ref ev-field-result-review-001 \
  --output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_followup_escalation_status.json \
  --summary-output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_followup_escalation_status_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_followup_escalation_status.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_followup_escalation_status_summary.v1`，Robot safe alias 为 `robot_diagnostics_field_evidence_rerun_execution_result_acceptance_handoff_intake_followup_escalation_status_summary`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_followup_escalation_status_gate`。核心字段包括 `due_state`、`followup_status`、`allowed_due_states`、`followup_reasons`、safe `evidence_ref`、`source_review_handoff`、`safe_followup_policy`、`required_materials`、`material_status`、`owner_escalation`、`next_required_evidence`、`rerun_commands`、`safe_copy`、`source=software_proof`、`software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。

`due_state` / `followup_status` 只允许 `pending`、`overdue`、`escalated` 和 `blocked`。follow-up policy 必须在同一 safe `evidence_ref` 下确认十类安全材料类别已纳入跟进：`true task record`、`true Nav2/fixed-route runtime log`、`route completion signal`、`true elevator door state`、`target floor confirmation`、`human assistance record`、`dropoff/cancel completion or delivery result`、`true route/elevator field pass`、`true phone/browser evidence` 和 `PR #5 hardware material remains pending unless PRRT_kwDOSWB9286CJ3tX is live resolved`；顶层 acceptance command 可用 `python3 -m unittest pc-tools/evidence/test_field_evidence_rerun_execution_result_acceptance_handoff_intake_followup_escalation_status.py` 运行离线围栏测试。CLI 在安全的 `pending`、`overdue` 或 `escalated` 状态返回 0；缺 source、缺 policy、缺材料类别、证据号不一致、unsupported due state 或 unsafe/success/control claim 均 fail closed 到 `blocked` 并返回非 0。

该 gate 不读取真实 task record、Nav2/fixed-route runtime log、route completion signal、电梯门/目标楼层证据、人工协助记录、dropoff/cancel completion、delivery result、true route/elevator field pass、true phone/browser evidence 或 raw diagnostics，不访问 ROS graph、serial/UART、WAVE ROVER、真实电梯、外部云、真实手机/browser，也不执行任何机器人动作。缺 review handoff、缺 follow-up policy、坏 JSON、unsupported schema/boundary、source 或 policy 不是 `source=software_proof` 或缺 `not_proven`、上游不是 `ready_for_acceptance_review_handoff_not_proven`、上游 boundary 不是 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_review_handoff_gate`、缺 safe `evidence_ref`、证据号不一致、弱类型 `same_evidence_ref_required`、缺任一 required material acknowledgement、rejected/unsafe material refs、unsafe copy、sensitive copy、raw/local paths、ROS topics、serial/UART/WAVE ROVER details、credentials、raw/complete artifact、checksum、traceback、true phone/browser proof、real route/elevator field pass、real Nav2/fixed-route pass、verified terminal result、Objective 5 external proof、HIL/PR #5 resolution claims、success/control claim、`safe_to_control=true`、`delivery_success=true` 或 `primary_actions_enabled=true` 都会 fail closed。`pending`、`overdue` 和 `escalated` 只表示 Docker/local `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_followup_escalation_status_gate` 已把上一轮 safe review-handoff 和 follow-up policy 安全复账为人工跟进状态，不是真实 field rerun、真实 route/elevator pass、真实 Nav2/fixed-route、verified terminal result、dropoff/cancel completion、delivery result、delivery success、HIL、真实手机/browser、PR #5 resolution 或 Objective 5 external proof。

`pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_intake.py` 只读上一节 `field_evidence_rerun_execution_result_acceptance_handoff_intake_followup_escalation_status` artifact、summary、Robot diagnostics safe alias 或 wrapper/nested JSON，也可安全桥接上一链路 `field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status` 的 sanitized artifact、summary、Robot alias 或 wrapper source，并读取 field owner 提交的脱敏 owner response packet，把同一 safe `evidence_ref` 下的 owner response 分类为 `accepted`、`missing`、`rejected` 或 `blocked`：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_intake.py \
  --followup-status-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_followup_escalation_status_summary.json \
...
输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_intake.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_intake_summary.v1`，Robot safe alias 为 `robot_diagnostics_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_intake_summary`。普通 source 的证据边界为 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_intake_gate`；reviewer-ACK followup bridge source 的输出证据边界为 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_owner_response_intake_bridge_gate`，并输出 `bridge_capability=field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_owner_response_intake_bridge`、`source_bridge=field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status`。核心字段包括 `owner_response_status`、`allowed_owner_response_statuses`、`owner_response_reasons`、safe `evidence_ref`、`previous_followup_reference`、`accepted_materials`、`missing_materials`、`rejected_materials`、`blocked_materials`、`required_owner_response_materials`、`material_response_details`、`next_required_evidence`、`rerun_commands`、`safe_copy`、`source=software_proof`、`software_proof`、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false` 和 `no OKR percentage lift`。

`owner_response_status` 只允许 `accepted`、`missing`、`rejected` 和 `blocked`。Required owner response checklist 固定列出 `real task record`、`Nav2/fixed-route runtime log`、`route completion signal`、`dropoff/cancel completion`、`elevator door status`、`floor confirmation`、`human assistance note`、`delivery result`、`route/elevator field pass`、`true phone/browser evidence` 和 `PR #5 hardware material remains pending unless PRRT_kwDOSWB9286CJ3tX is live resolved by reviewer`；顶层 acceptance command 可用 `python3 -m unittest pc-tools/evidence/test_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_intake.py` 运行离线围栏测试。CLI 仅在 `accepted` 时返回 0；缺上一环、缺 owner response、required materials 缺失、证据号不一致、rejected material、missing/unsafe bridge、unsafe/raw artifact paths/credential/ROS topic/`/cmd_vel`/serial/UART details、ACK/cursor mutation、GitHub mutation、upload/review action、robot command hints、success/control claim、O5 external proof claim、O1 HIL claim 或 PR #5 resolution claim 均返回非 0。

该 gate 不读取真实 task record、Nav2/fixed-route runtime log、route completion signal、电梯门/目标楼层证据、人工协助记录、dropoff/cancel completion、delivery result、route/elevator field pass、true phone/browser evidence 或 raw diagnostics，不访问 ROS graph、serial/UART、WAVE ROVER、真实电梯、外部云、真实手机/browser，也不执行任何机器人动作。`accepted` 只表示 Docker/local owner response metadata 已在同一 safe `evidence_ref` 下复账；当 source 来自 reviewer-ACK followup 时，`field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_owner_response_intake_bridge` 只把 safe followup summary 接回既有 owner-response intake，不创建新的 owner-response mainline。它不是真实 field rerun、真实 route/elevator pass、真实 Nav2/fixed-route、verified terminal result、dropoff/cancel completion、delivery result、delivery success、HIL、真实手机/browser、PR #5 resolution、Objective 5 external proof 或 OKR 完成度提升。

`pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_decision.py` 只读上一节 `field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_intake` artifact、summary、Robot safe alias 或 wrapper/nested JSON，并读取脱敏 owner response review packet，把同一 safe `evidence_ref` 下的 review packet 转成五值 review decision：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_decision.py \
  --owner-response-intake-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_intake_summary.json \
  --owner-response-review-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_packet.json \
  --evidence-ref field-rerun-owner-review-001 \
  --output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_decision.json \
  --summary-output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_decision_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_decision.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_decision_summary.v1`，Robot safe alias 为 `robot_diagnostics_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_decision_summary`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_decision_gate`。核心字段包括 `capability=field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_decision`、safe `evidence_ref`、`previous_owner_response_intake_reference`、`owner_response_review_decision`、`allowed_owner_response_review_decision_states`、`decision_reasons`、`accepted_materials`、`missing_materials`、`rejected_materials`、`blocked_materials`、`review_confirmed_materials`、`review_needs_rework_materials`、`review_ref_mismatch_materials`、`review_unsafe_rejected_materials`、`next_required_evidence`、`safe_copy`、`source=software_proof`、`software_proof`、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false` 和 `no OKR percentage lift`。

`owner_response_review_decision` 只允许 `ready_for_owner_response_review_handoff_not_proven`、`review_needs_owner_rework`、`review_evidence_ref_mismatch`、`review_unsafe_rejected` 和 `blocked_missing_owner_response_intake`。Review packet 必须在同一 safe `evidence_ref` 下确认 `true task record`、`true Nav2/fixed-route runtime log`、`route completion signal`、`true elevator door state`、`target floor confirmation`、`human assistance record`、`dropoff/cancel completion`、`delivery result`、`true route/elevator field pass`、`true phone/browser evidence` 和 `PR #5 hardware material remains pending unless PRRT_kwDOSWB9286CJ3tX is live resolved by reviewer` 这些类别，但不得复制 raw material body。CLI 仅在 `ready_for_owner_response_review_handoff_not_proven` 时返回 0；缺上一环 intake、缺 review packet、required materials 缺失、证据号不一致、unsafe/raw/local path/credential/ROS topic/serial/WAVE ROVER wording、success/control claim、Objective 5 external proof claim、O1 HIL claim 或 PR #5 resolution claim 均返回非 0。

该 gate 不读取真实 task record、Nav2/fixed-route runtime log、route completion signal、电梯门/目标楼层证据、人工协助记录、dropoff/cancel completion、delivery result、true route/elevator field pass、true phone/browser evidence 或 raw diagnostics，不访问 ROS graph、serial/UART、WAVE ROVER、真实电梯、外部云、真实手机/browser，也不执行任何机器人动作。`ready_for_owner_response_review_handoff_not_proven` 只表示 Docker/local `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_decision_gate` 已把上一轮 owner response intake 和 review packet 安全复账为后续 handoff metadata；它不是真实 field rerun、真实 route/elevator pass、真实 Nav2/fixed-route runtime pass、verified terminal result、dropoff/cancel completion、delivery result、delivery success、HIL、WAVE ROVER/UART proof、真实手机/browser、PR #5 resolution、Objective 5 external proof 或 OKR 完成度提升。

`pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff.py` 只读上一节 `field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_decision` artifact、summary、Robot diagnostics safe alias 或 wrapper/nested JSON，并读取脱敏 owner/support/reviewer handoff packet，把同一 safe `evidence_ref` 下的 handoff packet 转成五值 review handoff：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff.py \
  --owner-response-review-decision-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_decision_summary.json \
  --handoff-packet-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff_packet.json \
  --evidence-ref field-rerun-owner-review-001 \
  --output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff.json \
  --summary-output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff_summary.v1`，handoff packet 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff_packet.v1`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff_gate`。核心字段包括 `capability=field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff`、safe `evidence_ref`、`source_owner_response_review_decision`、`owner_support_reviewer_handoff_packet`、`owner_support_reviewer_routing`、`handoff_status`、`allowed_handoff_states`、`handoff_reasons`、`material_status`、`handoff_checklist`、`owner_handoff`、`next_required_evidence`、`rerun_commands`、`safe_copy`、`source=software_proof`、`software_proof`、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false` 和 `no OKR percentage lift`。

`handoff_status` 只允许 `ready_for_owner_response_review_handoff_not_proven`、`handoff_needs_owner_rework`、`handoff_evidence_ref_mismatch`、`handoff_unsafe_rejected` 和 `blocked_missing_owner_response_review_decision`。Handoff packet 必须在同一 safe `evidence_ref` 下提供 owner/support/reviewer routing、`next_required_evidence`，并确认 `true task record`、`true Nav2/fixed-route runtime log`、`route completion signal`、`true elevator door state`、`target floor confirmation`、`human assistance record`、`dropoff/cancel completion`、`delivery result`、`true route/elevator field pass`、`true phone/browser evidence` 和 `PR #5 hardware material remains pending unless PRRT_kwDOSWB9286CJ3tX is live resolved by reviewer` 这些类别，但不得复制 raw material body。CLI 仅在 `ready_for_owner_response_review_handoff_not_proven` 时返回 0；缺上一环 decision、缺 handoff packet、缺 owner/support/reviewer routing、缺 `next_required_evidence`、required materials 缺失、证据号不一致、unsafe/raw/local path/credential/ROS topic/serial/WAVE ROVER wording、success/control claim、Objective 5 external proof claim、O1 HIL claim 或 PR #5 resolution claim 均返回非 0。

该 gate 不读取真实 task record、Nav2/fixed-route runtime log、route completion signal、电梯门/目标楼层证据、人工协助记录、dropoff/cancel completion、delivery result、true route/elevator field pass、true phone/browser evidence 或 raw diagnostics，不访问 ROS graph、serial/UART、WAVE ROVER、真实电梯、外部云、真实手机/browser，也不执行任何机器人动作。`ready_for_owner_response_review_handoff_not_proven` 只表示 Docker/local `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff_gate` 已把上一轮 owner response review decision 和 handoff packet 安全复账为后续 handoff metadata；它不是真实 field rerun、真实 route/elevator pass、真实 Nav2/fixed-route runtime pass、verified terminal result、dropoff/cancel completion、delivery result、delivery success、HIL、WAVE ROVER/UART proof、真实手机/browser、PR #5 resolution、Objective 5 external proof 或 OKR 完成度提升。

`pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_intake.py` 只读上一节 `field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff` artifact、summary、Robot diagnostics safe alias 或 wrapper/nested JSON，并读取脱敏 reviewer ACK packet，把同一 safe `evidence_ref` 下的 reviewer ACK 转成五值 intake：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_intake.py \
  --owner-response-review-handoff-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff_summary.json \
  --reviewer-ack-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_packet.json \
  --evidence-ref field-rerun-owner-review-ack-001 \
  --output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_intake.json \
  --summary-output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_intake_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_intake.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_intake_summary.v1`，Robot safe alias 为 `robot_diagnostics_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_intake_summary`，reviewer ACK packet 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_packet.v1`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_intake_gate`。核心字段包括 `capability=field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_intake`、上一跳 `source_capability=field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_review_handoff`、safe `evidence_ref`、`reviewer_ack_state`、`allowed_reviewer_ack_states`、`ack_reasons`、`source_owner_response_review_handoff`、`reviewer_acknowledgement`、`next_required_evidence`、`safe_copy`、`source=software_proof`、`software_proof`、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false` 和 `no OKR percentage lift`。

`reviewer_ack_state` 只允许 `reviewer_acknowledged_not_proven`、`reviewer_ack_needs_reassignment`、`reviewer_ack_evidence_ref_mismatch`、`reviewer_ack_rejected_unsafe` 和 `blocked_missing_owner_response_review_handoff`。Reviewer ACK packet 必须确认 reviewer role、reviewer identity label、ACK reason、owner/support/reviewer next step 和同一 safe `evidence_ref` 下的 `next_required_evidence`，但不得复制 raw material body。CLI 仅在 `reviewer_acknowledged_not_proven` 或 `reviewer_ack_needs_reassignment` 且所有必填 ACK 字段安全时返回 0；缺上一环 handoff、缺 ACK packet、上一跳未包含 previous capability、上一跳 boundary 不匹配、上一跳不是 `ready_for_owner_response_review_handoff_not_proven`、缺 ACK 字段、证据号不一致、unsafe/raw/local path/credential/ROS topic/serial/WAVE ROVER wording、success/control claim、Objective 5 external proof claim、O1 HIL claim 或 PR #5 resolution claim 均返回非 0。

该 gate 不读取真实 task record、Nav2/fixed-route runtime log、route completion signal、电梯门/目标楼层证据、人工协助记录、dropoff/cancel completion、delivery result、true route/elevator field pass、true phone/browser evidence、reviewer raw ACK body 或 raw diagnostics，不访问 ROS graph、serial/UART、WAVE ROVER、真实电梯、外部云、真实手机/browser，也不执行任何机器人动作。`reviewer_acknowledged_not_proven` 只表示 Docker/local `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_intake_gate` 已把上一轮 owner response review handoff 和 reviewer ACK packet 安全复账为 reviewer ACK metadata；它不是真实 field rerun、真实 route/elevator pass、真实 Nav2/fixed-route runtime pass、verified terminal result、dropoff/cancel completion、delivery result、delivery success、HIL、WAVE ROVER/UART proof、真实手机/browser、PR #5 resolution、Objective 5 external proof 或 OKR 完成度提升。

`pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_decision.py` 只读上一节 `field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_intake` artifact、summary、Robot safe alias 或 wrapper/nested JSON，把 reviewer ACK intake 的 ACK 状态转换成五值 structured review decision：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_decision.py \
  --reviewer-ack-intake-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_intake_summary.json \
  --evidence-ref field-rerun-owner-review-ack-001 \
  --output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_decision.json \
  --summary-output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_decision_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_decision.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_decision_summary.v1`，建议 Robot safe alias 为 `robot_diagnostics_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_decision_summary`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_decision_gate`。核心字段包括 `capability=field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_decision`、safe `evidence_ref`、`previous_intake_reference`、`reviewer_ack_state`、`review_decision`、`decision_reasons`、`ack_owner`、`acknowledged_at`、`reassignment_target`、`next_required_evidence`、`owner_action`、`review_handoff_recommendation`、`safe_copy`、`source=software_proof`、`software_proof`、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false` 和 `no OKR percentage lift`。

`review_decision` 枚举固定为 `accepted_for_reviewer_ack_review_not_proven`、`needs_reviewer_reassignment_not_proven`、`needs_field_owner_supplement_not_proven`、`rejected_unsafe_reviewer_ack_not_proven` 和 `blocked_missing_reviewer_ack_intake_not_proven`。accepted 只表示 reviewer ACK 可进入后续 reviewer ACK 复核，不表示真实 reviewer resolution、OKR movement、真实 field pass、verified terminal result、dropoff/cancel completion、delivery success、HIL、O5 external proof、真实 phone/browser 证据或 PR #5 resolved。缺 intake source、unsupported schema、缺上一环 proof boundary、证据号不一致或 source 不是 `source=software_proof` / `not_proven` 会 fail closed；raw ROS topics、`/cmd_vel`、serial/UART 路径、本机路径、credentials、DB/queue URLs、complete artifacts、checksums、tracebacks、HIL/pass、delivery success、dropoff/cancel completion、true phone/browser、PR #5 resolved 等 claim 会输出 `rejected_unsafe_reviewer_ack_not_proven` 或 blocked safe state，且永远保持 `delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false`。

`pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_handoff.py` 只读上一节 `field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_decision` artifact、summary、Robot safe alias 或 wrapper/nested JSON，把 reviewer ACK review decision 转成五值 sanitized handoff：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_handoff.py \
  --reviewer-ack-review-decision-json /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_decision_summary.json \
  --output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_handoff.json \
  --summary-output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_handoff_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_handoff.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_handoff_summary.v1`，建议 Robot safe alias 为 `robot_diagnostics_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_handoff_summary`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_handoff_gate`。核心字段包括 `capability=field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_handoff`、safe `evidence_ref`、`source_review_decision`、`reviewer_ack_state`、`handoff_status`、`allowed_handoff_statuses`、`handoff_reasons`、`field_owner_handoff`、`support_handoff`、`reviewer_handoff`、`next_required_evidence`、`safe_copy`、`source=software_proof`、`software_proof`、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false` 和 `no OKR percentage lift`。

`handoff_status` 枚举固定为 `ready_for_field_owner_reviewer_ack_followup_not_proven`、`needs_reviewer_handoff_reassignment_not_proven`、`needs_field_owner_ack_material_supplement_not_proven`、`rejected_unsafe_reviewer_ack_handoff_not_proven` 和 `blocked_missing_reviewer_ack_review_decision_not_proven`。ready 只表示 reviewer ACK review decision 可进入 field owner / reviewer ACK follow-up handoff，不表示真实 reviewer resolution、OKR movement、真实 field pass、verified terminal result、dropoff/cancel completion、delivery success、HIL、O5 external proof、真实 phone/browser 证据或 PR #5 resolved。缺 review-decision source、unsupported schema、缺上一环 proof boundary、同一 safe `evidence_ref` 不一致、source 不是 `source=software_proof` / `not_proven`，或出现 raw ROS topics、`/cmd_vel`、serial/UART 路径、本机路径、credentials、DB/queue URLs、complete artifacts、checksums、tracebacks、HIL/pass、delivery success、dropoff/cancel completion、true phone/browser、route/elevator field-pass、PR #5 resolved 等 claim，都会 fail closed 到 rejected 或 blocked safe state，且永远保持 `delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false`。

`pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status.py` 只读上一节 `field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_handoff` artifact、summary、Robot safe alias 或 wrapper/nested JSON，把 reviewer ACK review-handoff 后续转成 support escalation / real-material reviewer follow-up 的 PC-only status：

```bash
python3 pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status.py \
  --input /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_review_handoff_summary.json \
  --due-status overdue \
  --output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status.json \
  --summary-output /tmp/field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status.v1`，summary 使用 `schema=trashbot.field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status_summary.v1`，Robot safe alias 为 `robot_diagnostics_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status_summary`，证据边界固定为 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status_gate`。核心字段包括 `capability=field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_followup_escalation_status`、safe `evidence_ref`、`followup_status`、`due_status`、`source_handoff_status`、`reviewer_ack_status`、`field_owner_handoff`、`support_escalation_owner`、`missing_required_evidence`、`rejected_or_unsafe_reasons`、`next_required_evidence`、`phone_safe_copy`、`source=software_proof`、`software_proof`、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false` 和 `safe_to_control=false`。

`followup_status` 枚举固定为 `pending_reviewer_ack_followup_not_proven`、`overdue_reviewer_ack_followup_not_proven`、`escalated_missing_real_material_not_proven`、`blocked_missing_reviewer_ack_review_handoff_not_proven` 和 `ready_for_real_material_reviewer_followup_not_proven`。ready 只表示可用同一 safe `evidence_ref` 继续 real-material reviewer follow-up，仍不是 PR #5 resolution、delivery success、真实 route/elevator field pass、verified terminal result、dropoff/cancel completion、O5 external cloud/4G/OSS/CDN/DB/queue proof、真实 phone/browser proof、HIL 或 OKR movement。缺 source handoff、bad JSON、unsupported schema、wrong proof boundary、safe `evidence_ref` 不一致或缺失会 fail closed 到 `blocked_missing_reviewer_ack_review_handoff_not_proven`；非 ready handoff、unsafe raw/control/credential/hardware/success copy、Objective 5 external-proof claim、`delivery_success=true`、`primary_actions_enabled=true` 或 `safe_to_control=true` 会输出 `escalated_missing_real_material_not_proven`，同时继续要求真实材料补齐。
...
```bash
python3 pc-tools/evidence/field_evidence_real_material_request_dispatch.py \
  --acceptance-backfill-json /tmp/field_evidence_rerun_execution_result_acceptance_backfill_summary.json \
  --evidence-ref ev-field-result-review-001 \
  --output /tmp/field_evidence_real_material_request_dispatch.json \
  --summary-output /tmp/field_evidence_real_material_request_dispatch_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_real_material_request_dispatch.v1`，summary 使用 `schema=trashbot.field_evidence_real_material_request_dispatch_summary.v1`，证据边界固定为 `software_proof_docker_field_evidence_real_material_request_dispatch_gate`。核心字段包括 `request_dispatch_status`、safe `evidence_ref`、`source_acceptance_backfill`、`required_materials`、`request_items`、`owner_handoff`、`next_steps`、`blocked_claims`、`safe_copy`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。

real material request dispatch 固定请求九类同一 safe `evidence_ref` 材料：`task_record`、`nav2_fixed_route_runtime_log`、`route_completion_signal`、`elevator_door_floor_evidence`、`human_assistance_note`、`dropoff_cancel_completion`、`delivery_result`、`true_phone_browser_evidence` 和 `diagnostics_mobile_safe_summary`。顶层 acceptance command 可用 `python3 -m unittest pc-tools/evidence/test_field_evidence_real_material_request_dispatch.py` 运行离线围栏测试；该入口只证明本地 `software_proof_docker_field_evidence_real_material_request_dispatch_gate` 的 fail-closed request contract 可复跑，仍属于 Docker/local software proof。

该 gate 不读取真实 task record、Nav2/fixed-route runtime log、route completion signal、电梯门/楼层材料、人工协助记录、dropoff/cancel completion、delivery result、true phone/browser evidence、raw diagnostics、ROS graph、serial/UART、WAVE ROVER、真实电梯、外部云、OSS/CDN、DB/queue、4G 或真实手机/browser，也不执行任何机器人动作。缺 source、坏 JSON、unsupported schema/boundary、source 不是 `source=software_proof` 或缺 `not_proven`、source backfill 不是 `ready_for_field_evidence_rerun_execution_result_acceptance_backfill_not_proven`、缺 safe `evidence_ref`、证据号不一致、弱类型 `same_evidence_ref_required`、unsafe copy、sensitive copy、raw path/credential/ROS topic/serial/UART/WAVE ROVER detail、checksum、traceback、完整/raw artifact、success/control claim、`safe_to_control=true`、`delivery_success=true` 或 `primary_actions_enabled=true` 都会 fail closed。`ready_for_field_owner_real_material_request_not_proven` 只表示 Docker/local gate 已把下一步 field-owner 真实材料请求清单派发清楚，不是真实 field rerun、true phone/browser proof、Nav2/fixed-route proof、route/elevator field pass、HIL、O5 external proof、PR #5 resolved、delivery result 或 delivery success。

`pc-tools/evidence/field_evidence_real_material_owner_ack_intake.py` 只读上一节 `field_evidence_real_material_followup_escalation_status` artifact、summary、Robot safe alias 或 wrapper/nested JSON，并读取现场 owner 提交的脱敏 acknowledgement packet，把 owner ack 整理成 metadata-only intake artifact / summary：

```bash
python3 pc-tools/evidence/field_evidence_real_material_owner_ack_intake.py \
  --followup-json /tmp/field_evidence_real_material_followup_escalation_status_summary.json \
  --owner-ack-json /tmp/field_evidence_real_material_owner_ack_packet.json \
  --evidence-ref ev-field-owner-ack-001 \
  --output /tmp/field_evidence_real_material_owner_ack_intake.json \
  --summary-output /tmp/field_evidence_real_material_owner_ack_intake_summary.json
```

输出 artifact 使用 `schema=trashbot.field_evidence_real_material_owner_ack_intake.v1`，summary 使用 `schema=trashbot.field_evidence_real_material_owner_ack_intake_summary.v1`，Robot safe alias 为 `robot_diagnostics_field_evidence_real_material_owner_ack_intake_summary`，证据边界固定为 `software_proof_docker_field_evidence_real_material_owner_ack_intake_gate`。核心字段包括 `owner_ack_intake_status`、safe `evidence_ref`、`source_followup`、`owner_acknowledgement`、`material_categories`、`owner_next_steps`、`safe_copy`、`source=software_proof`、`not_proven`、`safe_to_control=false`、`delivery_success=false` 和 `primary_actions_enabled=false`。

`material_categories` 固定输出 `accepted`、`missing`、`rejected` 和 `blocked` 四类，只表示 owner-safe acknowledgement packet 中材料类别的接收状态，不表示真实验收。顶层 acceptance command 可用 `python3 -m unittest pc-tools.evidence.test_field_evidence_real_material_owner_ack_intake` 运行离线围栏测试；该入口只证明本地 `software_proof_docker_field_evidence_real_material_owner_ack_intake_gate` 的 fail-closed contract 可复跑，仍属于 Docker/local software proof。
...
## field evidence material resolution reviewer ACK review handoff

`pc-tools/evidence/field_evidence_material_resolution_reviewer_ack_review_handoff.py`
只读上一节 `field_evidence_material_resolution_reviewer_ack_review_decision`
artifact、summary、Robot safe alias 或 wrapper/nested JSON，把 reviewer ACK
review decision 转成 support / field owner / reviewer handoff package：

```bash
python3 pc-tools/evidence/field_evidence_material_resolution_reviewer_ack_review_handoff.py \
  --input /tmp/field_evidence_material_resolution_reviewer_ack_review_decision_summary.json \
  --output /tmp/field_evidence_material_resolution_reviewer_ack_review_handoff.json \
  --summary-output /tmp/field_evidence_material_resolution_reviewer_ack_review_handoff_summary.json
```

输出 artifact 使用
`schema=trashbot.field_evidence_material_resolution_reviewer_ack_review_handoff.v1`，
summary 使用
`schema=trashbot.field_evidence_material_resolution_reviewer_ack_review_handoff_summary.v1`，
Robot safe alias 为
...
核心字段包括
`capability=field_evidence_material_resolution_reviewer_ack_review_handoff`、safe
`evidence_ref`、`handoff_status`、`source_review_decision`、
`reviewer_ack_state`、`ack_owner`、`reassignment_target`、
`field_owner_handoff`、`support_handoff`、`reviewer_handoff`、
`decision_reasons`、`next_required_evidence`、`proof_flags`、`source=software_proof`、
`not_proven`、`primary_actions_enabled=false`、`delivery_success=false` 和
`safe_to_control=false`。
...
`rejected_unsafe_reviewer_ack_handoff_not_proven` 和
`blocked_missing_reviewer_ack_review_decision_handoff_not_proven`。ready 只表示
support、field owner 与 reviewer 可以拿到脱敏 reviewer ACK handoff package，仍不是真实
material accepted、OKR movement、真实 field pass、verified terminal result、
delivery success、HIL、O5 external proof、真实 phone/browser 证据或 PR #5 resolved。
缺 input、bad JSON、unsupported schema、缺上一环 proof boundary 或缺 safe
`evidence_ref` 会 fail closed；success/control/reviewer-resolution claim、raw artifact、
credential/local path、ROS/control detail、field/cloud/phone/HIL proof claim 会输出
`rejected_unsafe_reviewer_ack_handoff_not_proven`。该入口只证明本地
`software_proof_docker_field_evidence_material_resolution_reviewer_ack_review_handoff_gate`
的 fail-closed contract 可复跑，仍属于 Docker/local software proof。
