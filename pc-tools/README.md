...
2026-07-06 02:13 CST 起，用户反馈“PC 地图太小 / ROS2 有无配套”后，PC 首页和 `/map`
默认缩放先收敛为 `200%` 可读大图，`细节放大` 为 `1200%`，`完整态势` 仍一键回到 `100%` 全局。
...
2026-07-06 02:40 CST 起，历史默认值曾升级为：PC 首页和 `/map` 默认 `300%` 可读大图，
`完整态势` 回到 `100%`，`细节放大` 仍为 `1200%`。这是普通用户默认入口；RViz2、Nav2 RViz 插件和
Foxglove bridge 仍只是工程观察配套，不替代 PC 简易控制台，也不启动 ROS2/RViz2/Foxglove/Nav2/建图 runtime，
...
2026-07-06 05:07 CST 起，历史默认值曾升级为：PC 首页和 `/map` 默认 `1600%` 可读大图，
...
2026-07-06 05:36 CST 起，地图显示曾收敛为“首页先完整态势，细节再放大”：普通首页的地图卡
强制排在图传、WASD 和连接状态之前，默认 `100%` 时真实地图按画布宽度铺满，机器人位置、Nav2
路线、雷达点和目标点同屏可见；`细节放大` 仍可逐级到 `4800%`。`/map` 直达页保持满屏只看地图，
...
2026-07-06 08:01 CST 起，PC fixed first-frame probe 会把显式 `backendSmoke=1` 的深度矩阵结论直接平铺到
probe 回包顶层：`software_capture_exhausted`、`known_good_uvc_required` 和
`camera_input_signal_check_required`。真实 7001 复验返回 `backend_smoke_status=backend_no_frame_observed`、
`backend_attempts=11`、`software_capture_exhausted=true`、`known_good_uvc_required=true` 和
`camera_input_signal_check_required=true`；`GET /api/robot-control/camera/mjpeg/status` 同步显示这三项为
...
2026-07-06 04:05 CST 起，USB recovery 结束时会把本次解绑过的 DV20 audio 复合接口恢复 bind，并把
`audio_rebind_ok`、`audio_bind_status_after_rebind` 和 `topology_after_audio_rebind` 透传到 PC 代理回包；
普通页 DOM 同步暴露 `data-auto-usb-recovery-audio-rebind-ok`。真实上位机复验中，`3-1:1.2/3-1:1.3`
最终均回到 `snd-usb-audio`，`trashbot-local-webrtc-camera.service` 为 `active`，`uvcvideo`
保持 `quirks=0,nodrop=0,timeout=5000`；但 DV20 仍是 `streamon_success_zero_byte_no_frame`，
所以实时画面仍需处理输入信号、线/接口/供电或 known-good UVC。
...
同轮继续按 Linux UVC quirk 常用位复测 `quirks=16/128/256/4/2/144/400/20/272`，并分别抓
...
`/map` 直达页继续填满当前 viewport，默认同样是 `100%` 完整态势，最高 `4800%`。需要局部排障时点
`细节放大` 到 `4800%`，点 `完整态势` 回到 `100%`；地图、Nav2 路线、小车位置、雷达点和目标点仍在同一张 WYSIWYG 画布。ROS2 配套结论保持分层：本地工程调试用 RViz2 /
...
2026-07-04 01:10 CST 起，历史版本曾把真实地图 PNG 的 CSS 从“宽度优先”修正为“高度优先”：
`.plain-map-layer.has-real-map .plain-map-overlay-frame` 当时按地图画布高度撑满，宽地图通过横向滚动查看全图，
...
`map_pose_observed`。地图太小/ROS2 配套的当前答案仍是：普通用户用 PC 首页大地图和 `/map`，默认 `800%` 大地图，
最高 `4800%`，点 `细节放大` 看局部，点 `完整态势` 回到 `100%` 全局；工程调试才用 RViz2，远程浏览器观察用 Foxglove bridge + Foxglove Web。
...
2026-07-04 CST 起，PC 地图曾按“先完整可读、再局部排障”处理：普通用户点 `进入地图大屏` 打开 `/map`，
当时默认 `100%` 完整态势，最高 `4800%` 只用于局部排障，点 `完整态势` 回到 `100%`，summary/DOM 同步暴露
...
同轮补强：`current_free_move_control_pack_next_action_plain` 和
...
2026-07-02 CST 起，`GET /api/robot-control/camera/mjpeg/status` 与 summary 同步暴露首帧失败 alias：
`first_frame_probe_status`、`first_frame_probe_failure_reason`、`first_frame_failure_reason`、
`camera_first_frame_probe_status` 和 `camera_first_frame_failure_reason`。现场脚本单看 MJPEG status 即可读到
`source_first_frame_failed / first_frame_total_timeout` 这类首帧结论，不需要绕到 summary；该端点仍只读共享预览和上车
camera health，不新建独占采集、不启动 Nav2、manual、keyboard、free-roam、建图、delivery、stop 或 `/cmd_vel`。
...
`field_acceptance_primary_missing_action_id`、`field_acceptance_primary_missing_action_label`、
`field_acceptance_primary_missing_action_start_endpoint`、`field_acceptance_primary_missing_action_stop_endpoint`、
...
`field_acceptance_primary_missing_action_requires_safety_confirm`、
`field_acceptance_primary_missing_action_minimal_precheck_safety_only`、
...
`field_acceptance_primary_requires_motion_before_readback`、
...
2026-07-01 22:20 CST 起，地图卡的 ROS2 配套入口从泛化的“工程观察”改为“工程观察：RViz2 / Foxglove”，并同步到 `GET /api/robot-control/summary` 的 `map_display_engineering_tools_action_label`。当时普通用户解决“地图太小”仍优先点 `/map` 进入 PC 内置地图大屏；当时默认 `100%` 完整态势、最高 `4800%` 细节放大，底图、路线、小车位置和雷达点仍共用同一张 WYSIWYG 画布；RViz2 只用于本地工程调试 `/map`、`/scan`、TF、路径、定位和 costmap，Foxglove 只用于部署 `foxglove_bridge` 后的浏览器远程观察。该变化只改显示和只读 DOM/API 合同，不启动 RViz2/Foxglove/ROS2 runtime，不执行 Nav2，不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。

2026-07-01 19:07 CST 起，普通首屏地图继续按普通用户“大地图优先”处理；历史合同曾为默认 `600%` 现场大图、最高 `4800%`，当前合同已收敛为默认 `100%` 完整态势、最高 `4800%`，需要看局部时点“细节放大”。ROS2 配套口径保持分层：RViz2 / `nav2_rviz_plugins` 是本地工程调试地图、雷达、TF、规划轨迹和定位的标准工具；Foxglove / `foxglove_bridge` 是浏览器观察配套；普通用户默认仍留在 PC 简易工作站超大地图里看路线、小车位置和雷达贴图。该变化只改显示和只读 DOM 合同，不启动 RViz2/Foxglove/ROS2 runtime，不执行 Nav2，不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
...
O7 Previews 现在把 `O7 consumer read primary path` 作为历史回放和标注队列检查主入口。operator 先用 `Load consumer task list` 调用 PC 后端 `GET /api/o7/consumer-read/tasks?baseUrl=<loopback>`，再选择 task 并用 `Load consumer task detail` 调用 `GET /api/o7/consumer-read/tasks/<task_id>?baseUrl=<loopback>&fieldEvidenceManifestJson=<local-json>`；PC 后端固定请求 O6 `view=summary` 和 detail `include=trajectory,events,evidence,field_evidence,labeling,inference,tunnel,artifact_access_probe,offline_artifact_seed_smoke,route_root_seed_gate,route_bag_evidence,route_bag_payload_replay,route_bag_semantic_replay,route_bag_full_semantic_decode_matrix,route_bag_pose_progress_replay,nav2_goal_execution_evidence,delivery_result_evidence,route_execution_result_delivery_readiness,route_delivery_closure_packet,same_task_mission_evidence_gate`。若 O6 list/detail 已有 `task_origin`、`field_evidence_source` 或新的 `field_evidence` wrapper，PC adapter 会先验证 `source_contract`，再从 wrapper 内的 `manifest` / `field_evidence_manifest` / `field_evidence_consumer_ingest` 提取 `trashbot.field_evidence_manifest.v1` 安全摘要，并在 UI 展示 `source_origin`、`task_origin`、`manifest_run_id`、artifact health、present/missing artifacts。旧的顶层 `field_evidence_manifest` 和 `trashbot.pc_tools_workstation.o7_field_evidence_consumer_ingest.v1` 仍兼容；只有远端缺失 field evidence 且本地 `fieldEvidenceManifestJson` 是合法 manifest 时，才用本地 manifest 补齐 `field_evidence`。`artifact_access_probe` 二级消费会从 top-level、`field_evidence`、`artifact_bundle` 或相关 ingest wrapper 读取 O6 `trashbot.o6.artifact_access_probe.v1`，只展示 counts、basename refs、`detected_type`、`size_bytes`、sha256 prefix、blocked reasons 和 `real_or_offline_artifact_access_probe_for_selected_task`，不展示 allowlist root 或原始路径。`route_bag_semantic_replay` 二级消费会从 top-level、`field_evidence`、`field_motion_evidence_packet`、`artifact_bundle`、`artifact_bundle_consumer_ingest` 或 `artifact_bundle_readiness` 读取 O6 已脱敏摘要，只展示 `semantic_decode_status`、`semantic_topic_types`、LaserScan/Image/TF/Odometry/DiagnosticArray 白名单统计、blocked reasons 和 `next_required_evidence`，不展示 raw payload、base64、图片内容、绝对路径或 token。`route_bag_full_semantic_decode_matrix` 二级消费会从 direct、`field_evidence`、`field_motion_evidence_packet`、`artifact_bundle`、`artifact_bundle_consumer_ingest` 或 `artifact_bundle_readiness` 读取 `trashbot.route_bag_full_semantic_decode_matrix.v1` / `trashbot.o6.route_bag_full_semantic_decode_matrix.v1`，只展示 decoded/unsupported/failed counts、coverage ratio、限量 sample topic/type、blocked reasons、next evidence 和固定 false safety fields；DiagnosticArray 进入 decoded 时显示 `/diagnostics`、`diagnostic_msgs/msg/DiagnosticArray`、`decode_status=decoded`、`decoder_name=decode_diagnostic_array_payload`，coverage ratio 可随 fixture 从 0.75 提升到 1，但这仍只表示本地/离线 semantic coverage 可读，`ready_not_route_execution_proof` 不表示 route execution success 或 delivery success。`route_execution_result_delivery_readiness` 二级消费会从 top-level、`field_evidence`、`field_motion_evidence_packet`、`artifact_bundle`、`artifact_bundle_consumer_ingest` 或 `artifact_bundle_readiness` 读取 O6 `trashbot.o6.route_execution_result_delivery_readiness.v1`，只展示 `route_execution_result_status/source`、`delivery_result_readiness_status/source`、`operator_confirmation_readiness_status/source`、`nav2_goal_execution_ready`、`delivery_result_ready`、`operator_confirmation_ready`、blocked reasons 和 `next_required_evidence`，不把 ready 外推成真实 route execution 或真实 delivery success；`route_delivery_closure_packet` 会沿同样的 direct / field_evidence / field_motion / artifact bundle / readiness 路径读取 O6 `trashbot.o6.route_delivery_closure_packet.v1`，只展示 closure status、linked evidence flags、blocked reasons、next required evidence 和固定 false safety fields，不启用提交、控制、发车、delivery_success 或 production connected。本地 manifest 缺失、坏 JSON、顶层非 object、schema mismatch、unsafe copy、unsafe probe ref、外网 base URL、credentials/query/hash 或 `safe_to_control=true` / `primary_actions_enabled=true` / `delivery_success=true` / `media_access_proven=true` / `real_oss_connected=true` / `real_cdn_connected=true` 等危险声明都会 fail closed。同一份 consumer detail 既驱动 route replay player，也驱动 `Consumer-detail labeling queue primary path`：前者只消费 `trajectory.sample_frames`、`events.sample_events`、`evidence.sample_evidence`、`labeling.sample_items`、`inference.sample_results` 和 `tunnel_status` 摘要，后者只读检查 `labeling/evidence/events/trajectory` 的白名单短摘要。两条主路径都支持本地 cursor / 只读浏览，但所有 cursor/playback 都只在浏览器内存中变化，不写后端、不重新下发机器人命令、不证明真实云 archive 或真实机器人运动；submit/export/rollback 继续关闭。缺 detail、unknown task、task id mismatch、probe 缺失、blocked/not_proven/error/cancel 状态或危险 true 字段都会显示 `blocked_not_proven` / fail-closed reason；页面固定展示 `safe_to_control=false`、`primary_actions_enabled=false`、`delivery_success=false`、`robot_control_executed=false`、`media_access_proven=false`、`real_oss_connected=false`、`real_cdn_connected=false`。

2026-07-10 起，O7 Previews 会在同一 detail 和 `artifact_bundle_readiness` 中展示 `same_task_mission_evidence_gate`：operator 可以直接看到同一 `task_id` 下 `cloud_command_terminal_result` 来源、`trashbot.cloud_command_terminal_result.v1` schema、route execution materials status、linked flags、blocked reasons 和 next required evidence。`same_task_mission_gate_ready_not_success_proof` 只表示 terminal result 与 route materials 配对可读，仍固定 `delivery_success=false`、`safe_to_control=false`、`connects_cloud_production=false`，不证明真实送达成功或真实 production cloud。

2026-07-09 起，`Consumer-detail labeling queue primary path` 允许从 PC 后端触发 O6 local/mock 标注提交和 task-level 数据集导出：浏览器只调用 `POST /api/o7/consumer-read/tasks/<task_id>/annotations/submit?baseUrl=<loopback>` 和 `GET /api/o7/consumer-read/tasks/<task_id>/annotations/export?baseUrl=<loopback>&format=jsonl`，再由 PC adapter 调用 O6 `POST /api/o6/archive/labels` 和 `GET /api/o6/archive/labels/<task_id>/export?format=jsonl`。adapter 只接受本机 HTTP 回环 base URL，拒绝 credentials、query/hash、非回环、空 task id、schema mismatch、危险 true 字段和 token/password/secret 等敏感文本。结果 schema 分别为 `trashbot.pc_tools_workstation.o7_annotation_submit_result.v1` 与 `trashbot.pc_tools_workstation.o7_annotation_dataset_export_result.v1`；UI 只展示 local/mock receipt/export result 和 `proof_status=not_proven`，并固定 `submit_enabled=false`、`dataset_export_available=false`、`real_annotation_api_connected=false`、`real_dataset_export_connected=false`、`cloud_write_executed=false`、`connects_cloud_production=false`、`safe_to_control=false`、`robot_control_executed=false`。这不是真实 annotation API、真实训练集导出、生产云写入、真实 rollback 或 O7 完成证明；缺 detail、blocked task 或 labeling MVP 不完整时按钮禁用并显示 blocker。

O7 Previews 还新增了 `Field evidence consumer ingest` 主入口。它从 `trashbot.field_evidence_manifest.v1` 出发，把 `route replay` 和 `labeling` 两条只读 preview 绑成同一份 `trashbot.pc_tools_workstation.o7_field_evidence_consumer_ingest.v1` 摘要，供本地/mock manifest、route replay fixture 和 labeling fixture 共享同一输出结构。该入口只接受本地文件路径，不会把 UI 直接升级成真实回放、真实标注提交或控制面；`delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false` 始终关闭。manifest 缺失、schema 不匹配、unsafe copy、preview 缺文件或 SSH 不可达时都会 fail closed，并显式暴露 blocked reason 与 next required evidence。2026-07-09 起，consumer read primary path 还会把 `artifact_bundle`、`artifact_bundle_consumer_ingest`、`artifact_access_probe` 和 `artifact_bundle_readiness` 作为优先消费面；route/replay/keyframe/evidence refs、probe counts、sha256 prefix、route bag evidence、route bag payload replay、route bag semantic replay、route bag full semantic decode matrix 的 decode blockers 和 next required evidence 先从 bundle / preflight / probe 读，旧 fallback 只做兼容兜底。
...
地图太小时，普通用户优先打开 PC 首页大地图或 `/map`，当前默认 `800%` 大地图，`完整态势` 回到 `100%`，局部最高 `4800%`。
