...
2026-06-30 17:02 CST 起，普通首屏地图继续按普通用户“大地图优先”处理；2026-06-30 17:50 CST 后当前合同为默认 `1200%` 缩放、最高 `1600%`，`?view=map` 直达地图大屏默认使用最高 `1600%`。ROS2 配套口径保持分层：RViz2 / `nav2_rviz_plugins` 是本地工程调试地图、雷达、TF、规划轨迹和定位的标准工具；Foxglove / `foxglove_bridge` 是浏览器远程观察配套；普通用户默认仍留在 PC 简易工作站大地图里看路线、小车位置和雷达贴图。该变化只改显示和只读 DOM 合同，不启动 RViz2/Foxglove/ROS2 runtime，不执行 Nav2，不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
...
2026-06-30 17:50 CST 起，普通首屏地图默认缩放从 `800%` 提升到 `1200%`，`?view=map` 直达地图大屏默认使用 `1600%` 上限，并保持地图、路线、小车位置、雷达贴图在同一张 WYSIWYG 画布。ROS2 配套按用途分层：本地工程调试用 RViz2；需要浏览器远程观察时，在上车侧部署 bridge 后用 Foxglove Studio；普通用户默认仍使用 PC 简易工作站和“打开地图大屏”。该变化只改显示合同，不启动 ROS2/RViz2/Foxglove/Nav2/建图 runtime，不发送任何运动命令。
...
2026-06-30 17:50 CST 起，普通首屏地图继续按“PC 主视图”处理：工作站外壳保持 `min(2600px, calc(100% - 2px))`，默认地图缩放为 `1200%`，最高缩放为 `1600%`，大图高度为 `clamp(1040px, calc(100vh - 4px), 2600px)`，全屏地图高度为 `100vh`，只看地图模式高度保持 `calc(100vh - 42px)`，按钮文案保持普通用户可理解的“只看地图”。ROS2 配套仍按两层处理：工程调试首选 RViz2 / `nav2_rviz_plugins` 看 `/map`、`/scan`、TF、规划轨迹和定位；浏览器远程观察接 Foxglove / `foxglove_bridge`；普通用户默认留在 PC 简易工作站近整屏大地图里看路线、小车位置和雷达贴图。该变化只改显示和只读合同，不启动 RViz2/Foxglove/ROS2 runtime，不执行 Nav2，不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
