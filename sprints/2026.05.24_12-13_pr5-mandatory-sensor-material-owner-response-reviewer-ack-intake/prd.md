...
| KR | Owner | 用户价值 | 验收口径 |
| --- | --- | --- | --- |
| KR-A Hardware reviewer ACK intake gate | `hardware-engineer` | reviewer ACK 状态能从上一轮 handoff 被安全接入和分类。 | PC gate 输出 artifact/summary，包含 `PRRT_kwDOSWB9286CJ3tX`、`hardware_material_pending`、proof boundary、next evidence 和 false-state flags；focused unit tests pass。 |
...
- `hardware-engineer`：主责 PC gate 和硬件材料 evidence contract；涉及 vendor/source attribution 时必须先读 `docs/vendor/VENDOR_INDEX.md` 及其指向文件。
