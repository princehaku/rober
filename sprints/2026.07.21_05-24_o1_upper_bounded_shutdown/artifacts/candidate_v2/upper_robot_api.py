...
def _ensure_ros_cmd_vel_context() -> dict[str, Any]:
    ...
    with _ROS_CMD_VEL_LOCK:
        if _ROS_CMD_VEL_CONTEXT.get("status") in {"ready", "unavailable", "shutting_down", "shutdown"}:
        ...
        try:
            if not rclpy.ok():
                # SIGTERM 必须由 asyncio server owner 接管；否则 rclpy 只关 context，不会结束永久 sleep 的 HTTP loop。
                try:
                    from rclpy.signals import SignalHandlerOptions  # type: ignore[import-not-found]

                    rclpy.init(args=None, signal_handler_options=SignalHandlerOptions.NO)
                except (ImportError, TypeError):
                    # 旧测试 fake 或不支持该关键字的 rclpy 仍可初始化；部署环境 Humble 支持显式 NO。
    ...
    *,
    ...
    with _ROS_CMD_VEL_LOCK:
        if context.get("status") != "ready":
            return {
                **result,
                "rclpy_context_status": context.get("status", "shutdown_fail_closed"),
                "frames_published": 0,
                "fallback_required": False,
                "error": {"type": "rclpy_shutdown_fail_closed", "message": "rclpy context is shutting down"},
            }
        ...
        if context.get("status") != "ready":
            return {
                **result,
                ...
                "fallback_required": False,
                "cmd_vel_first_publish_mono_ns": None,
                "error": {"type": "rclpy_shutdown_fail_closed", "message": "rclpy context stopped before first publish"},
            }
...
def build_stop_payload(port: str, baudrate: int) -> dict[str, Any]:
    ...
    # release stop 与 watchdog/runtime stop 必须共用同一 owner 锁，避免 ROS/串口零命令交叉。
    # 该锁只覆盖停车事务，不进入 nonzero keydown 热路径，也不改变 vendor stop 顺序。
    # 锁内仍执行原有 ROS-first、serial-all-surfaces 合同，响应字段保持兼容。
    with _MANUAL_HOLD_STOP_LOCK:
...
class UpperRobotApi:
    ...
    def __init__(
        ...
        # signal owner 在第一次 await 前置位；之后排队到达的 manual 请求必须零 publish fail-closed。
        # 该标志是单进程终态 gate，不用于跨进程状态同步，也不在重启前清零。
        # stop endpoint 不受此 gate 限制，确保退出期间仍可执行显式零速收口。
        ...
        try:
            while True:
                ...
                if delay_s > 0:
                    ...
                    continue
                # 在交给 worker 前先原子认领 stop；shutdown coroutine 看到 inactive 后不得再补第二次零速。
                # 这三次字典写在 event-loop 线程连续完成，中间没有 await，因此 shutdown 无法读到半状态。
                # to_thread 即使随后被 task cancel 也会继续完成已认领的零速 worker。
                # stop owner 留在 state 中，日志可区分 watchdog 已承担收口而非 runtime 静默跳过。
                state["active"] = False
                state["stop_owner"] = "watchdog"
                state["stop_started_at_ms"] = now_ms()
    ...
    async def manual_control(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        if self._runtime_shutdown_started:
            # stop endpoint 仍可独立收口；新的 motion/manual 请求在 signal 后一律零执行拒绝。
            # 这里位于任何 direction backend、watchdog schedule 或 serial/ROS helper 之前。
            # latency trace 仍可安全回传白名单字段，便于 PC 解释请求为何未执行。
            # failure response 固定所有控制/交付证明为 false，不把 shutdown 当 command success。
            return {
                "schema": f"{SCHEMA}.base_manual_result",
                "generated_at_ms": now_ms(),
                "accepted": False,
                "status": "blocked_runtime_shutting_down",
                "failure_reason": "upper_runtime_shutdown_started",
                "latency_trace": latency_trace,
                "command_result": None,
                "stop_result": None,
                "manual_command_executed": False,
...
async def run_server(
    ...
    *,
    ...
    """启动 aiohttp API；SIGTERM 只唤醒 stop event，所有 owner 在 finally 中有界收口。"""
    # run_server 自己创建并持有 event、handler、runner 和 ROS prewarm 的生命周期。
    # 外部注入 event 只用于离线测试；生产路径仍由 asyncio loop 安装 SIGTERM/SIGINT。
    # runner 在 setup 成功后才写入 owner 变量，startup 半途失败也能安全进入 finally。
    # started 日志明确 signal owner，便于首次迁移时区分旧版 rclpy handler 进程。
    # stopped 日志包含分层结果，后续 v3 可据此拒绝不完整 cleanup。
    # 永久 sleep 被 event.wait 取代，使 systemd SIGTERM 能实际推动 Python coroutine 退出。
    from aiohttp import web
    ...
    installed_handlers = install_upper_shutdown_signal_handlers(loop, shutdown_event) if install_signal_handlers else []
    ...
    try:
            ...
            json.dumps(
                {
                    ...
                    "shutdown_signal_owner": "asyncio_event",
                },
                ensure_ascii=False,
            ),
            flush=True,
        ...
        shutdown_result = await shutdown_upper_runtime(api, runner)
        remove_upper_shutdown_signal_handlers(loop, installed_handlers)
        print(
            json.dumps({"event": "upper_robot_api_stopped", "shutdown": shutdown_result}, ensure_ascii=False),
            flush=True,
        )
