#!/usr/bin/env python3
"""在已验证 adadb0 latency-only Upper 上叠加 reconciled bounded shutdown，输出可从 85ba 应用的 v3。"""
...
# 这些已有 latency symbols 仍是 85ba -> candidate v3 总白名单的一部分。
...
# v3 只替换与 teardown 串行、fail-closed 和 active-hold stop ownership 直接相关的既有定义。
...
# 这些字符串只属于 c8 Nav2/sensor-owned 合同，candidate v3 必须继续为零。
FORBIDDEN_SENTINELS = (
    "sensor_owned_scan",
...
def main() -> int:
    ...
    incremental_patch = unified_patch(
        previous,
        candidate,
        # 增量 patch 与 full patch 都绑定真实部署目标；输入 SHA 负责区分所需基线。
        before_label="a/onboard/scripts/upper_robot_api.py",
        after_label="b/onboard/scripts/upper_robot_api.py",
    )
    ...
    manifest = {
        "schema": "trashbot.o1.latency_only_upper_candidate_v3.v1",
        ...
        "candidate_v3_sha256": sha256_text(candidate),
