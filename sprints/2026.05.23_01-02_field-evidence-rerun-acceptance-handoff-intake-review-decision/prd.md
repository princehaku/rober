# Field Evidence Rerun Acceptance Handoff Intake Review Decision PRD
...
本轮产品价值：在上一轮 owner/support intake 后，新增一个 review decision 层，把“收到了什么材料”变成“这些材料是否足够进入下一步 handoff/rework”。这样 support 不会把 owner acknowledgement 误当真实 delivery，也不会在材料缺失、evidence_ref 不一致或出现成功/控制 claims 时继续推进。

## 问题定义

上一轮 `field_evidence_rerun_execution_result_acceptance_handoff_intake` 证明了 Docker/local 环境下 owner/support acknowledgement intake metadata 可被 PC/Robot/mobile 安全消费。但 intake 只回答“回执是否进入系统”，不回答“回执是否足够让下一步 handoff 继续”。如果没有 review decision，后续现场 owner、support 和 mobile panel 容易把 acknowledged/missing/rejected 材料状态混成同一个结论。
...
- Objective 2/3/4：本轮只推进 field-evidence rerun acceptance handoff 的 software-proof review decision readiness，作为后续真实 route/elevator/phone/browser 材料回填前的安全判定层。

## KR 拆解或更新

本轮不改 OKR/KR，只对现有 KR 做 sprint-level 拆解：

- KR-A：PC gate 给出 review decision：ready for next handoff、needs rework、evidence_ref mismatch、unsafe rejected、blocked missing intake。
...
- Capability 名称：`field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision`。
- Evidence boundary：`software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision_gate`。
- PC-only review-decision artifact and summary。
- Robot diagnostics safe alias。
- mobile/web read-only review decision panel and fixture。
...
- 不新增机器人控制 endpoint、material upload route、ACK/cursor route、handoff route 或 hidden primary-action enablement。

## 用户/支持人员流程

1. Field owner/support 提供上一轮 intake 的 safe summary 和同一 safe `evidence_ref` 的 review packet。
2. PC-only gate 判断材料是否足够进入下一步 handoff/rework。
3. Robot diagnostics 暴露 safe alias，support 可在 diagnostics 中看到决策状态和缺口。
...
- 输出中包含 `field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision`。
- 输出中包含 `software_proof_docker_field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision_gate`。
...
- Evidence-ref mismatch、missing required material、unsafe wording、success/control wording、external-proof/HIL/PR-resolution claim 均 fail closed。
- `mobile/web` Start Delivery、Confirm Dropoff、Cancel 在 fixture 下保持禁用。
- Product closeout 不提高 OKR 百分比，除非真实材料出现。

## 责任 Engineer

- `autonomy-engineer`：PC-only review-decision gate + tests + evidence docs。
...
- 如果 owner/support packet 包含 raw logs、local paths、credentials、serial/UART details、ROS topics 或 complete artifacts，PC/Robot/mobile 必须拒绝或过滤。
