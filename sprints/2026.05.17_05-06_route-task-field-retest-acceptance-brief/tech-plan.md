# Sprint 2026.05.17_05-06 Route Task Field Retest Acceptance Brief - Tech Plan

sprint_type: epic

## 1. 总体方案

实现 `route_task_field_retest_acceptance_brief`，作为 `route_task_field_retest_drill_console` 后的 PC/Robot/mobile software-proof 现场验收简报层。该层不读取真实材料目录，不访问真实机器人或外部云，只把 drill console 的 command labels、safe checklist、missing prompts 和 callback checklist 转成现场支持可执行的 acceptance brief / handoff packet。
...
本轮文件范围由 3 个并行 owner 拆分，互不重叠。Product closeout 后续由 Product Manager / OKR Owner 处理。
...
- `pc-tools/evidence/route_task_field_retest_acceptance_brief.py`
- `pc-tools/evidence/test_route_task_field_retest_acceptance_brief.py`
- `pc-tools/README.md`
- `docs/navigation/fixed_route_workflow.md`

实现要求：

- 新增 dependency-free PC CLI，建议参数为 `--drill-console-json`、`--evidence-ref`、`--output`、`--summary-output`、`--once-json`。
- 兼容 `route_task_field_retest_drill_console` artifact、summary、wrapper / nested JSON。
- 输出 `trashbot.route_task_field_retest_acceptance_brief.v1` 与 `trashbot.route_task_field_retest_acceptance_brief_summary.v1`。
- Summary 必须包含 acceptance status、safe `evidence_ref`、field entry prerequisites、execution checklist、pass/fail criteria、required evidence packet、owner handoff、rerun notes、not-proven 列表和 evidence boundary。
...
```bash
PYTHONDONTWRITEBYTECODE=1 python3 -m py_compile pc-tools/evidence/route_task_field_retest_acceptance_brief.py
PYTHONDONTWRITEBYTECODE=1 python3 -m unittest pc-tools/evidence/test_route_task_field_retest_acceptance_brief.py
python3 pc-tools/evidence/route_task_field_retest_acceptance_brief.py --help
rg -n "route_task_field_retest_acceptance_brief|software_proof_docker_route_task_field_retest_acceptance_brief_gate|trashbot.route_task_field_retest_acceptance_brief.v1|not_proven|delivery_success=false|primary_actions_enabled=false" pc-tools/evidence/route_task_field_retest_acceptance_brief.py pc-tools/evidence/test_route_task_field_retest_acceptance_brief.py pc-tools/README.md docs/navigation/fixed_route_workflow.md
git diff --check -- pc-tools/evidence/route_task_field_retest_acceptance_brief.py pc-tools/evidence/test_route_task_field_retest_acceptance_brief.py pc-tools/README.md docs/navigation/fixed_route_workflow.md
```
...
- 支持 acceptance brief artifact、summary、Robot-compatible summary 和 nested diagnostics summary。
- 只暴露 safe summary、safe `evidence_ref`、acceptance status、pass/fail criteria、required evidence packet、owner handoff、boundary、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`。
...
- 新增只读“现场复测验收简报” panel，消费 `route_task_field_retest_acceptance_brief` artifact / summary / Robot diagnostics compatible summary。
- 展示 acceptance status、safe `evidence_ref`、pass/fail criteria、required evidence packet、owner handoff、safe copy、boundary、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`。
...
- 明确本轮由 `route_task_field_retest_drill_console` 继续推进，不是 Objective 5 external proof，也不是 O1 HIL-entry 或硬件材料真实完成。
- 明确本轮不是真实 route/elevator field pass、HIL、真实手机/browser、production app、真实投放、dropoff/cancel completion 或 delivery success。

验收命令：

```bash
rg -n "route_task_field_retest_acceptance_brief|software_proof_docker_route_task_field_retest_acceptance_brief_gate|Objective 2|Objective 3|Objective 4|Objective 5|Docker-only|not_proven|delivery_success=false|primary_actions_enabled=false|PR #5" sprints/2026.05.17_05-06_route-task-field-retest-acceptance-brief OKR.md docs/process/okr_progress_log.md
git diff --check -- sprints/2026.05.17_05-06_route-task-field-retest-acceptance-brief/tech-done.md sprints/2026.05.17_05-06_route-task-field-retest-acceptance-brief/side2side_check.md sprints/2026.05.17_05-06_route-task-field-retest-acceptance-brief/final.md OKR.md docs/process/okr_progress_log.md
```

## 6. 接口影响

- PC 新增 `trashbot.route_task_field_retest_acceptance_brief.v1` artifact 和 `trashbot.route_task_field_retest_acceptance_brief_summary.v1` summary。
...
- 为什么不转向 Objective 1 硬件材料：最近 O1 hardware chain 已有 readiness review 和 execution pack，下一步需要真实 2D LiDAR / ToF / WAVE ROVER / UART / HIL-entry 证据；继续本地包装会重复消费硬件 blocker。
...
```bash
rg -n "sprint_type: epic|route_task_field_retest_acceptance_brief|software_proof_docker_route_task_field_retest_acceptance_brief_gate|OKR 最低优先级核对|Objective 5|Objective 2|Objective 3|Docker-only|not_proven|delivery_success=false|primary_actions_enabled=false" sprints/2026.05.17_05-06_route-task-field-retest-acceptance-brief
git diff --check -- sprints/2026.05.17_05-06_route-task-field-retest-acceptance-brief/pre_start.md sprints/2026.05.17_05-06_route-task-field-retest-acceptance-brief/prd.md sprints/2026.05.17_05-06_route-task-field-retest-acceptance-brief/tech-plan.md
```
