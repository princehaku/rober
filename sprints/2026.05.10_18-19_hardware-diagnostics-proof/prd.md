...
由 `robot-hardware-engineer` 在 `src/ros2_trashbot_hardware/` 中新增一个不打开串口、不启动 ROS2 node 的 diagnostics/proof 模块，复用 `esp32_bridge.py` 的纯函数：
...
- `robot-hardware-engineer`：实现、测试、修复和 `tech-done.md`。
