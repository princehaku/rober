...
- 本轮实现、测试、修复：`robot-hardware-engineer`。
- 后续真实 WAVE ROVER HIL：`robot-hardware-engineer`。
...
来自 `robot-hardware-engineer` 的本轮证据：
...
1. 有车环境下由 `robot-hardware-engineer` 跑 no-motion、low-speed `T=1`、`T=13` 专项 HIL recipe。
