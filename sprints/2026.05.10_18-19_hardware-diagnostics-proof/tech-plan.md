...
- 执行方式：1 owner 单线闭环，必须由 `robot-hardware-engineer` 子 agent 实现、验证并更新 `tech-done.md`。Coordinator/Product Owner 不直接写产品代码、测试代码或硬件配置。
...
下阶段 implementation 应派发 1 个 `robot-hardware-engineer` worker，prompt 必须包含：

- 角色 System Prompt：从 `.codex/agents/robot-hardware-engineer.toml` 的 `prompt` 字段完整复制。
