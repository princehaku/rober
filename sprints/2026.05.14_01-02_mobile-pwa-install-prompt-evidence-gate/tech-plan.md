# Sprint 2026.05.14_01-02 Mobile PWA Install Prompt Evidence Gate - Tech Plan

## 总体方案

本 sprint 是 Epic，采用 A/B 并行、C 收口：

- Task A Full-stack 在 `mobile/web/` 增加 PWA install prompt evidence UI、fixture、targeted mobile unittest，并更新 `docs/product/mobile_user_flow.md`。
- Task B Robot compatibility fence 在 remote bridge/protocol tests 和 `docs/interfaces/ros_contracts.md` 证明 PWA install prompt evidence metadata 不进入 command/ACK/cursor/robot execution。
- Task C Product 等 A/B 返回后更新 `OKR.md`、progress log 和本 sprint 收口文档。

统一证据边界：`software_proof_docker_mobile_pwa_install_prompt_evidence_gate`。本边界只证明 Docker/local mobile software proof 与 robot metadata-only fence，不证明真实手机设备、production app、真实 PWA install prompt、O5 外部材料或真实 delivery。
...
3. 不针对 Objective 5 的具体理由：`OKR.md` 第 6 节和最近两轮 final 明确，Objective 5 completion 需要至少一种真实外部材料，包括真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue connectivity 或 production worker/migration。当前本机没有真实云、真实 4G/SIM、OSS/CDN live traffic、production DB/queue 或 worker/migration 材料，继续推进本地 O5 metadata depth 会重复消费同一 blocker。
4. 转向 Objective 4 的证据依据：`sprints/2026.05.14_00-01_mobile-device-handoff-session-gate/final.md` 和 `sprints/2026.05.13_23-24_mobile-device-evidence-capture-gate/final.md` 都指出可执行下一步是采集真实手机、production app 或真实 PWA install prompt 证据。本轮在 Docker-only host 上先建立 `software_proof_docker_mobile_pwa_install_prompt_evidence_gate`，服务后续真实设备/PWA 验收。
...
- `install_prompt_user_outcome`
- `display_mode`
- `manifest_present`
- `service_worker_status`
- `offline_shell_status`
- `production_app_ready`
- `safe_to_control`
...
这些字段只能作为 phone/support metadata。它们不是 robot command、remote ACK、cursor instruction、delivery result、production readiness、HIL、真实 phone acceptance proof、真实 PWA install prompt proof 或 Start/Confirm/Cancel 放行依据。metadata 出现在 valid command envelope 外时必须被 robot bridge 忽略；metadata 与 valid command envelope 混合时，只能按 valid command envelope 执行，metadata 不得进入 ACK/status 编码。
...
1. 在 `mobile/web/` 首屏或现有手机验收区域新增 “PWA 安装提示证据” 面板。
2. 面板必须展示 install prompt capture status、user outcome、display-mode/installability/offline shell、manifest/service worker、production app readiness、safe-to-control 和 `not_proven`。
3. 面板必须提供复制 phone-safe install prompt evidence package 的入口；复制内容必须过滤敏感字段和 robot/raw 技术字段。
4. 面板可复用或引用已有 `mobile_device_handoff_session`、`mobile_device_evidence_capture`、`mobile_browser_acceptance_bundle`，但不得把这些 package 表述为真实 PWA install prompt 通过。
5. 缺真实手机/browser、production app、真实 PWA install prompt 时，Start Delivery、Confirm Dropoff、Cancel 继续 fail closed。
6. 保持现有 Start confirmation、operation log、action feedback、terminal action confirmation、device evidence capture、handoff session、browser acceptance bundle 语义不倒退。
7. `docs/product/mobile_user_flow.md` 与 `mobile/README.md` 必须同步写清 install prompt evidence 的字段、步骤、边界和 not-proven 范围。
...
1. 增加 `mobile_pwa_install_prompt_evidence` / summary / package metadata-only tests。
...
3. 证明 metadata 与 valid command envelope 混合时，只执行 command envelope，不把 install prompt evidence metadata 编入 ACK/status。
4. `docs/interfaces/ros_contracts.md` 必须写清这些字段是 phone/support metadata，不是 command、ACK、cursor、delivery result、production readiness、HIL、真实 device proof、真实 PWA install prompt proof 或 Start/Confirm/Cancel 放行依据。
...
Task C 只能在 Task A 和 Task B 都返回后执行。Product 必须核对 A/B 文件范围、验证输出、证据边界、phone-safe copy 和 `not_proven` 文案，再决定 Objective 4 是否谨慎上调。没有真实外部 O5 材料时，Objective 5 不上调。
...
本 sprint 不证明真实 iPhone/Android device behavior、production app、真实 PWA install prompt、真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue、production worker/migration、Nav2/fixed-route、WAVE ROVER、HIL、真实 dropoff completion、真实 cancel completion 或真实 delivery。ACK、HTTP accepted、receipt、evidence package、handoff session、install prompt evidence 和 terminal confirmation 仍只是 accepted/processing/support evidence，不是 delivery success。
