...
- 前置文档：`prd.md`。
- 前置 gate：PRD 已完成 OKR 映射、范围内/外、验收口径、估时里程碑和依赖对齐。
- 当前阶段：TECH PLAN。
- 本阶段完成条件：模块方案、P7 DEV handoff、验证计划、风险边界全部明确。
- 下一文档：只有本阶段完成后，才允许进入 DEV 并创建或生效 `tech-done.md`。
- 如果本文件早于 PRD gate 被预创建，则在 gate 完成前只能视为 invalid draft。
