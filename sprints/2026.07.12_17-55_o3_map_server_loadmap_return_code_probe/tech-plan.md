# Tech Plan - O3 Map Server LoadMap Return Code Probe
...
- Planned proof boundary: `software_proof_o3_o1_strict_no_motion_map_server_loadmap_return_code_probe_only`
...
用户价值是让真实上位机 fixed-route/nav 链路向同 run path generation 和 route execution 迈进。技术目标是解除或继续收窄 `/map_server` lifecycle 上游 blocker，避免继续停留在 16:55 的 `map_server_on_configure_return_false_after_valid_map_io_deferred_completion` 标签。

P0 技术目标：

- 优先证明 `/map_server active=true`；或
- 如果仍 blocked，证明比 `map_server_on_configure_return_false_after_valid_map_io_deferred_completion` 更窄的 `loadMapResponseFromYaml` return code、error string、response status、`on_configure` return path、executor/log ordering 或 lifecycle manager ChangeState response handling root cause。

## 现有 Root Cause Baseline

16:55 accepted baseline：
...
- `sprints/2026.07.12_17-55_o3_map_server_loadmap_return_code_probe/tech-done.md`
...
- 可新增 proof JSON 字段，用于表达 `loadMapResponseFromYaml` return code、error string、response status、`on_configure` return path、map IO ordering、executor/log ordering 或 lifecycle manager ChangeState response handling。
...
1. 复用 16:55 true-board artifact 和 runtime log ordering，建立 baseline 断言。
2. Inspect Nav2 map_server `loadMapResponseFromYaml` 调用点，提取 return code、error string、response status 或 equivalent runtime evidence。
3. Inspect `on_configure` return path：区分 callback exception、parameter validation、YAML/image/map mode、map response assembly、map IO completion 和 lifecycle response。
4. 扩展 helper/proof/parser，输出更窄字段，例如 `load_map_response_return_code`、`load_map_response_error`、`on_configure_return_path`、`executor_log_ordering_summary` 或 `lifecycle_changestate_response_handling`。
5. 若找到小修路径，优先修到 `/map_server active=true`，再记录 strict no-motion artifact。
6. 若无法修复，生成 true-board strict no-motion artifact，证明 root cause 比 16:55 更窄，且下一轮可直接按字段修复。
7. 更新 tests 和 docs，最后更新 `tech-done.md`，不得把 blocked proof 包装成 OKR 增量。
...
git diff --check -- \
  ...
  docs/navigation/field_route_evidence_preflight.md \
  docs/navigation/fixed_route_workflow.md \
  sprints/2026.07.12_17-55_o3_map_server_loadmap_return_code_probe
```
...
```bash
rg -n "sprint_type: epic|OKR 最低优先级核对|map_server_on_configure_return_false_after_valid_map_io_deferred_completion|loadMapResponseFromYaml|robot-software-engineer|strict no-motion|/cmd_vel|NavigateToPose|WAVE ROVER" sprints/2026.07.12_17-55_o3_map_server_loadmap_return_code_probe
```

```bash
git diff --check -- sprints/2026.07.12_17-55_o3_map_server_loadmap_return_code_probe
```
...
- 安全字段 fail-closed。
- `/map_server active=true`，或 blocked root cause 比 16:55 更窄。
- 如果仍 blocked，必须明确 `loadMapResponseFromYaml` return code / error string / response status、`on_configure` return path、executor/log ordering 或 lifecycle manager ChangeState response handling 中至少一项。
- 不调整 OKR 百分比，不归档 KR，除非新增证据真正越过 path/route/delivery/production gate。

Product 拒收条件：

- 没有证明 `/map_server active`，也没有比 16:55 更窄的 root cause。
...
- 核心任务：inspect/fix/narrow Nav2 map_server `loadMapResponseFromYaml` return code 与 `on_configure` return path，解释 valid map inputs 下为什么 lifecycle manager 仍收到 failure。
