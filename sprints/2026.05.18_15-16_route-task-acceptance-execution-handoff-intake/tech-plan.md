# Sprint 2026.05.18_15-16 Route Task Acceptance Execution Handoff Intake - Tech Plan

## 1. 目标和架构

本轮实现 `route_task_field_retest_acceptance_execution_handoff_intake`：在上一轮 `route_task_field_retest_acceptance_execution_callback_review_handoff` 之后增加 owner handoff intake / acknowledgement gate，消费 handoff summary 和可选 owner callback/intake JSON，输出 PC gate artifact/summary、Robot diagnostics safe alias 和 mobile/web 只读“现场交接回执入口” panel。

架构原则:

- Autonomy 是主 artifact 生产者，读取上一轮 handoff artifact/summary 和可选 owner callback/intake JSON，输出 handoff intake artifact/summary。
...
- 不针对理由: Objective 5 的下一步真实进展需要真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live traffic、production DB/queue、worker/cutover 或真实手机/browser 外部材料；当前本机只有 Docker，O5 stop rule 成立。Objective 1 约 81%，但近期 WAVE ROVER feedback replay、HIL packet intake、review decision、execution pack 等 sprint 已重复消费同一真实 WAVE ROVER/UART/HIL blocker，本机无真实硬件，不继续本地包装同一 blocker。PR #4 route/elevator callback review handoff 已完成，当前最可执行且不重复消费 blocker 的抓手是把 handoff package 转成 owner handoff intake / acknowledgement gate。
- final.md 收口时需复核: O5 外部材料或 O1 真实硬件材料是否在本轮期间出现；若出现，下一轮应重新排序。若没有，本轮仍按 O2/O3 route/elevator handoff intake 收口。
...
- Create: `pc-tools/evidence/route_task_field_retest_acceptance_execution_handoff_intake.py`
- Create: `tests/test_route_task_field_retest_acceptance_execution_handoff_intake.py`

### 需要做什么

- 新增 PC gate `route_task_field_retest_acceptance_execution_handoff_intake.py`。
- 读取上一轮 handoff artifact/summary，支持 artifact 文件输入和 summary 文件输入。
- 可选读取 owner callback/intake JSON；若未提供 owner ack，状态必须 fail-closed 为 `needs_owner_ack`。
...
- 输出 handoff intake artifact/summary，schema 建议:
...
- Summary 至少包含 safe `evidence_ref`、source handoff schema/status、owner acknowledgement state、owner handoff、next required evidence、safe rerun hint、boundary、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`。

### 验收命令

```bash
python3 -m py_compile pc-tools/evidence/route_task_field_retest_acceptance_execution_handoff_intake.py tests/test_route_task_field_retest_acceptance_execution_handoff_intake.py
python3 -m unittest tests.test_route_task_field_retest_acceptance_execution_handoff_intake
python3 pc-tools/evidence/route_task_field_retest_acceptance_execution_handoff_intake.py --help
rg -n "route_task_field_retest_acceptance_execution_handoff_intake|software_proof_docker_route_task_field_retest_acceptance_execution_handoff_intake_gate|ready_for_controlled_field_rerun_queue|needs_owner_ack|needs_acceptance_execution_handoff_backfill|evidence_ref_mismatch_rerun|blocked_unsafe_handoff_intake|not_proven|delivery_success=false|primary_actions_enabled=false" pc-tools/evidence/route_task_field_retest_acceptance_execution_handoff_intake.py tests/test_route_task_field_retest_acceptance_execution_handoff_intake.py
git diff --check -- pc-tools/evidence/route_task_field_retest_acceptance_execution_handoff_intake.py tests/test_route_task_field_retest_acceptance_execution_handoff_intake.py
```

### 接口边界

- 不读取真实硬件、串口、网络、Nav2 runtime 或 mobile runtime。
- 不把 owner ack 写成 field pass。
...
- diagnostics test 覆盖:
  ...
  - alias 保留 boundary/status/evidence_ref/owner acknowledgement/next evidence flags。
...
rg -n "robot_diagnostics_route_task_field_retest_acceptance_execution_handoff_intake_summary|route_task_field_retest_acceptance_execution_handoff_intake|software_proof_docker_route_task_field_retest_acceptance_execution_handoff_intake_gate|not_proven|delivery_success=false|primary_actions_enabled=false" onboard/src/ros2_trashbot_behavior/ros2_trashbot_behavior/operator_gateway_diagnostics.py onboard/src/ros2_trashbot_behavior/test/test_operator_gateway_diagnostics.py
...
- 在 `mobile/web/app.js` 新增只读“现场交接回执入口” panel。
- Panel 消费主 summary 或 Robot safe alias:
  ...
  - `robot_diagnostics_route_task_field_retest_acceptance_execution_handoff_intake_summary`
- 展示字段只限 phone-safe metadata: handoff intake status、source handoff status、safe `evidence_ref`、owner acknowledgement state、owner handoff、next required evidence、safe rerun hint、boundary、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`。
- fixture 增加 handoff intake summary 样例。
...
rg -n "现场交接回执入口|route_task_field_retest_acceptance_execution_handoff_intake|robot_diagnostics_route_task_field_retest_acceptance_execution_handoff_intake_summary|software_proof_docker_route_task_field_retest_acceptance_execution_handoff_intake_gate|not_proven|delivery_success=false|primary_actions_enabled=false" mobile/web/app.js mobile/fixtures/mobile_web_status.fixture.json mobile/web/test_mobile_web_entrypoint.py
...
- Create/Modify: `sprints/2026.05.18_15-16_route-task-acceptance-execution-handoff-intake/tech-done.md`
- Create/Modify: `sprints/2026.05.18_15-16_route-task-acceptance-execution-handoff-intake/side2side_check.md`
- Create/Modify: `sprints/2026.05.18_15-16_route-task-acceptance-execution-handoff-intake/final.md`
- Modify: `OKR.md`
...
- 写清本轮 `software_proof_docker_route_task_field_retest_acceptance_execution_handoff_intake_gate` 只证明 owner handoff intake layer 可复核。

### 验收命令

```bash
test -f sprints/2026.05.18_15-16_route-task-acceptance-execution-handoff-intake/tech-done.md && test -f sprints/2026.05.18_15-16_route-task-acceptance-execution-handoff-intake/side2side_check.md && test -f sprints/2026.05.18_15-16_route-task-acceptance-execution-handoff-intake/final.md
rg -n "route_task_field_retest_acceptance_execution_handoff_intake|robot_diagnostics_route_task_field_retest_acceptance_execution_handoff_intake_summary|software_proof_docker_route_task_field_retest_acceptance_execution_handoff_intake_gate|Objective 5|Objective 1|PR #4|PR #5|not_proven|delivery_success=false|primary_actions_enabled=false" OKR.md docs/process/okr_progress_log.md sprints/2026.05.18_15-16_route-task-acceptance-execution-handoff-intake pc-tools tests onboard/src/ros2_trashbot_behavior mobile/web mobile/fixtures docs
git diff --check -- OKR.md docs/process/okr_progress_log.md sprints/2026.05.18_15-16_route-task-acceptance-execution-handoff-intake pc-tools tests onboard/src/ros2_trashbot_behavior mobile/web mobile/fixtures docs
```
...
- 本轮计划阶段只创建 planning docs；后续实现完成后由 closeout 统一提交最终功能与 OKR/docs 更新。
...
node --check mobile/web/app.js
python3 -m unittest mobile/web/test_mobile_web_entrypoint.py
rg -n "route_task_field_retest_acceptance_execution_handoff_intake|robot_diagnostics_route_task_field_retest_acceptance_execution_handoff_intake_summary|software_proof_docker_route_task_field_retest_acceptance_execution_handoff_intake_gate|Objective 5|Objective 1|PR #4|PR #5|not_proven|delivery_success=false|primary_actions_enabled=false" OKR.md docs/process/okr_progress_log.md pc-tools tests onboard/src/ros2_trashbot_behavior mobile/web mobile/fixtures docs sprints/2026.05.18_15-16_route-task-acceptance-execution-handoff-intake
git diff --check -- OKR.md docs/process/okr_progress_log.md sprints/2026.05.18_15-16_route-task-acceptance-execution-handoff-intake pc-tools tests onboard/src/ros2_trashbot_behavior mobile/web mobile/fixtures docs
```
...
- 是否没有把 handoff intake、owner ack、diagnostics/mobile summary 写成真实现场通过或 delivery success。
