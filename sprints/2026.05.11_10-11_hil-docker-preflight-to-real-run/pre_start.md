...
本轮只创建下一轮 sprint 入口文档，不修改 OKR、不把软件证据升级为实机通过。当前本机条件只有 docker，无真实 WAVE ROVER/ESP32/Orange Pi 串口设备；因此本轮目标是为下一位 `robot-hardware-engineer` 建立可执行、可追责、可复跑的准入链路。
...
| `robot-hardware-engineer` | 下一阶段主责：在 docker 中完成 preflight，并在真实串口环境执行 WAVE ROVER move-test |
