...
- 目标 owner：`robot-hardware-engineer`
...
主抓手是 O1 Docker/Humble preflight unblock。`robot-hardware-engineer` 应从失败证据倒推最小工程改动：让脚本在 Docker registry 返回 HTML、metadata 解包失败、host cache 异常或代理问题时输出明确诊断建议，并尽量恢复 `ros-rbs-humble:dev` image build。
...
- `robot-hardware-engineer`：主责实现、验证、修复和 `tech-done.md` 更新。
