...
核心抓手是定义并交付下一阶段的 engineering plan：`robot-hardware-engineer` 实现一个 Docker-only 可运行的 HIL evidence packet gate。该 gate 不依赖当前机器有硬件，但必须严格防止假阳性 `hil_pass`。
...
- `robot-hardware-engineer`：主责实现 gate、fixtures、围栏验证和 `tech-done.md`。
