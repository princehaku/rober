...
本轮核心抓手是 Product 定义 sprint：把 O1 从“反复等待硬件”推进到“先把 HIL evidence packet 合规 gate 做出来”。实现阶段由 `robot-hardware-engineer` 主责，`robot-software-engineer` 只在 gate 输出需要对接 task record 或 ROS topic sample 语义时做接口咨询，`autonomy-engineer` 只在 route replay crosscheck 入口需要事实补充时介入。
...
| P1 | 实现 owner 明确 | `robot-hardware-engineer` 主责 gate 实现和验证；其他 owner 仅按接口边界咨询。 |
...
- `robot-hardware-engineer`：下一阶段主责实现 HIL evidence packet gate、Docker-only 判定和 `tech-done.md`。
