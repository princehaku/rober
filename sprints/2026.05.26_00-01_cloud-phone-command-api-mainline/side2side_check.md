...
- 现在：command store 写入失败时 relay fail closed，返回 `503 command_store_unavailable`，不返回 queued receipt，避免“未入队却显示已排队”的假阳性。
...
- Store unavailable 固定走 `command_store_unavailable` 错误语义，不允许生成 queued receipt。
...
- Robot focused validation：通过，`Ran 4 tests in 2.769s OK`，覆盖 store unavailable fail-closed。
