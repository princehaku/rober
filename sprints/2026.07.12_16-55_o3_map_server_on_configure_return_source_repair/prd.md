# PRD - O3 Map Server On-Configure Return Source Repair
...
本 PRD 的产品目标不是“再写一个诊断 wrapper”，而是把 `/map_server` configure failure 从 15:54 的 timing 现象推进到可修复的 return source，或直接修到 `/map_server active`。
...
- 比 `map_server_changestate_response_false_before_map_io_completion` 更窄的 `on_configure` return false source proof；或
- 明确的参数、异常、map IO ordering、executor/service future 或 lifecycle manager response handling root cause。

## 4. Problem Statement

15:54 sprint 已接受的事实是：

- `/map_server` configure callback 已进入。
- YAML/image load 已开始。
- lifecycle manager 在 map IO 完成前收到 ChangeState response false。
- map read 随后仍完成。
- 没有 service timeout 或 RPC error log。
- `/map_server active=false`。

这仍不足以让 Algorithm 介入路径生成，因为 blocker 仍停在 lifecycle 上游。下一步必须回答：`on_configure` 为什么 return false，或者 lifecycle manager 为什么把这个窗口判成 transition failure。

## 5. In Scope

- 读取并分析 Nav2 map_server configure/runtime log。
- 检查 helper/proof/parser 是否能提取 `on_configure` return source、异常和参数字段。
- 对 map IO sync/async ordering、service future、executor timing、lifecycle manager response handling 建立更窄分类。
...
- 若仍 blocked，输出比 15:54 更窄的 primary root cause，至少落到 `on_configure` return false source、参数、异常、executor/service future、map IO sync/async ordering 或 lifecycle manager response handling 之一。

硬性失败条件：

- 只重复 `map_server_changestate_response_false_before_map_io_completion`，没有新增字段、代码路径、参数、异常或时序解释。
- 没有 true-board 或同等 strict no-motion artifact。
...
- Nav2 map_server 源码或运行参数不在当前 helper 可观测范围内，需要先扩展 proof 字段。
- runtime log 可能混有 LiDAR serial/runtime 背景噪声，但本轮不能在未成为 primary root cause 前转硬件。
- 如果只加分类不加修复，Product 只接受“比 15:54 更窄”的 root cause，不接受同名 blocker 重复消费。
