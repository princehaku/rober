...
> 当前增量：sprint `2026.05.14_17-18_mobile-pwa-install-prompt-event-capture` 在同一 panel 内新增 `beforeinstallprompt`、`userChoice`、`appinstalled` 浏览器事件监听和 `mobile_pwa_install_prompt_event_capture*` whitelist-only 复制包。schema 是 `trashbot.mobile_pwa_install_prompt_event_capture.v1`，证据边界是 `software_proof_docker_mobile_pwa_install_prompt_event_capture_gate`，只证明 Docker/local mobile software proof 与真实浏览器事件捕获准备；不等于真实手机验收、production app、O5 external proof、HIL、delivery success 或控制授权。
>
> 当前增量：sprint `2026.05.14_18-19_mobile-pwa-install-prompt-evidence-export` 将安装提示 panel 升级为 `mobile_pwa_install_prompt_evidence_export*` 导出面。schema 是 `trashbot.mobile_pwa_install_prompt_evidence_export.v1`、summary schema 是 `trashbot.mobile_pwa_install_prompt_evidence_export_summary.v1`、copy schema 是 `trashbot.mobile_pwa_install_prompt_evidence_export_copy.v1`，证据边界是 `software_proof_docker_mobile_pwa_install_prompt_evidence_export_gate`。复制和下载使用同一份 whitelist-only phone-safe 现场验收 metadata，`safe_to_control=false` 且 `ack_semantics=accepted_processing_only_not_delivery_success`；不等于真实手机验收、production app、O5 external proof、HIL、delivery success 或控制授权。
>
...
>
> 当前增量：sprint `2026.05.14_11-12_mobile-field-trial-runbook-execution-gate` 在首屏新增“现场试跑执行清单”panel。证据边界是 `software_proof_docker_mobile_real_device_field_trial_runbook_execution_gate`，只证明 Docker/local `mobile/web` 能从 `mobile_real_device_field_trial_review*` 派生或消费 `mobile_real_device_field_trial_runbook_execution`、`mobile_real_device_field_trial_runbook_execution_summary` 和 whitelist-only `mobile_real_device_field_trial_runbook_execution_copy`，覆盖下一次现场试跑所需 real device、production app、PWA install prompt、user choice、offline、touch、visual 和 material redaction 执行清单；`safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`、`not_proven` 必须保留，不等于真实手机验收、production app、真实 PWA install prompt/user choice、O5 外部 proof、HIL、dropoff/cancel completion 或 delivery success。
>
> 当前增量：sprint `2026.05.14_12-13_mobile-field-trial-evidence-recorder` 在首屏新增“现场证据记录”panel。证据边界是 `software_proof_docker_mobile_real_device_field_trial_evidence_record_gate`，只证明 Docker/local `mobile/web` 能从 `mobile_real_device_field_trial_runbook_execution*` 派生或消费 `mobile_real_device_field_trial_evidence_record`、`mobile_real_device_field_trial_evidence_record_summary`、whitelist-only `mobile_real_device_field_trial_evidence_record_copy` 和 `mobile_real_device_field_trial_evidence_record_archive`，覆盖 real device、production app、PWA install prompt、user choice、offline、touch、visual、material redaction、operator note 和 support note；`safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`、`not_proven` 必须保留，不等于真实手机验收、production app、真实 PWA install prompt/user choice、O5 外部 proof、HIL、dropoff/cancel completion 或 delivery success。
>
> 当前增量：sprint `2026.05.14_13-14_mobile-field-trial-evidence-verdict` 在首屏新增“现场证据 verdict”panel。证据边界是 `software_proof_docker_mobile_real_device_field_trial_evidence_verdict_gate`，只证明 Docker/local `mobile/web` 能从 `mobile_real_device_field_trial_evidence_record*` 或本地 archive 派生/消费 `mobile_real_device_field_trial_evidence_verdict`、`mobile_real_device_field_trial_evidence_verdict_summary` 和 whitelist-only `mobile_real_device_field_trial_evidence_verdict_copy`，输出 missing evidence、verdict、retest request 和 next material request；`safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`、`not_proven` 必须保留，不等于真实手机验收、production app、真实 PWA install prompt/user choice、O5 外部 proof、HIL、dropoff/cancel completion 或 delivery success。
>
> 当前增量：sprint `2026.05.14_14-15_mobile-field-trial-retest-execution-gate` 在首屏新增“现场复测执行”panel。证据边界是 `software_proof_docker_mobile_real_device_field_trial_retest_execution_gate`，只证明 Docker/local `mobile/web` 能从 `mobile_real_device_field_trial_evidence_verdict*` 的 `retest_request` / `material_request` 派生或消费 `mobile_real_device_field_trial_retest_execution`、`mobile_real_device_field_trial_retest_execution_summary` 和 whitelist-only `mobile_real_device_field_trial_retest_execution_copy`，输出复测执行状态、材料请求、缺口摘要和执行 checklist；`safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`、`not_proven` 必须保留，不等于真实手机验收、production app、真实 PWA install prompt/user choice、真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live、production DB/queue、HIL 或 delivery success。
>
> 当前增量：sprint `2026.05.14_15-16_mobile-current-pwa-field-trial-browser-proof` 刷新当前 `mobile/web` PWA 的本地 Chromium-family browser proof。证据边界是 `software_proof_docker_mobile_current_pwa_field_trial_browser_proof_gate`，只证明本机 Chromium-family 浏览器可渲染当前 PWA，并看到 `mobile_real_device_field_trial_package*`、`mobile_real_device_field_trial_review*`、`mobile_real_device_field_trial_runbook_execution*`、`mobile_real_device_field_trial_evidence_record*`、`mobile_real_device_field_trial_evidence_verdict*` 和 `mobile_real_device_field_trial_retest_execution*` 的可见/summary/copy 边界；`safe_to_control=false`、`accepted_processing_only_not_delivery_success`、`not_proven`、whitelist-only copy 和 Start/Confirm/Cancel fail closed 必须保留。不等于真实 iPhone/Android、production app、真实 PWA install prompt/user choice、O5 外部材料、HIL、dropoff/cancel completion 或 delivery success。
>
> 当前增量：sprint `2026.05.14_16-17_mobile-field-trial-acceptance-session` 在首屏新增“现场验收会话”panel。证据边界是 `software_proof_docker_mobile_real_device_field_trial_acceptance_session_gate`，只证明 Docker/local `mobile/web` 能按优先级消费 `mobile_real_device_field_trial_acceptance_session*`、`mobile_real_device_field_trial_retest_execution*`、`mobile_real_device_field_trial_evidence_verdict*` 或 `mobile_current_pwa_field_trial_browser_proof*`，并派生 whitelist-only `mobile_real_device_field_trial_acceptance_session_copy`；`safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`、`not_proven` 和 Start/Confirm/Cancel fail closed 必须保留。不等于真实手机验收、production app、真实 PWA prompt/user choice、O5 external proof、HIL 或 delivery success。
...
- 可选：`mobile_real_device_field_trial_runbook_execution`、`mobile_real_device_field_trial_runbook_execution_summary`、`mobile_real_device_field_trial_runbook_execution_copy` 或 `/api/status.phone_readiness.mobile_real_device_field_trial_runbook_execution*` 作为现场试跑执行清单；它只组织下一次真实手机现场试跑采证步骤，覆盖 real device、production app、PWA install prompt、user choice、offline、touch、visual 和 material redaction，`safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`、`not_proven` 必须保留
- 可选：`mobile_real_device_field_trial_evidence_record`、`mobile_real_device_field_trial_evidence_record_summary`、`mobile_real_device_field_trial_evidence_record_copy`、`mobile_real_device_field_trial_evidence_record_archive` 或 `/api/status.phone_readiness.mobile_real_device_field_trial_evidence_record*` 作为现场证据记录；它只归档 real device、production app、PWA install prompt、user choice、offline、touch、visual、material redaction、operator note 和 support note 的 phone-safe 摘要，`safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`、`not_proven` 必须保留
- 可选：`mobile_real_device_field_trial_evidence_verdict`、`mobile_real_device_field_trial_evidence_verdict_summary`、`mobile_real_device_field_trial_evidence_verdict_copy` 或 `/api/status.phone_readiness.mobile_real_device_field_trial_evidence_verdict*` 作为现场证据 verdict；它只从 record/archive 输出 missing evidence、retest request 和 material request，`safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`、`not_proven` 必须保留
- 可选：`mobile_real_device_field_trial_retest_execution`、`mobile_real_device_field_trial_retest_execution_summary`、`mobile_real_device_field_trial_retest_execution_copy` 或 `/api/status.phone_readiness.mobile_real_device_field_trial_retest_execution*` 作为现场复测执行包；它只从 evidence verdict 的 `retest_request` / `material_request` 派生复测执行 checklist、材料缺口和 source boundary，`safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`、`not_proven` 必须保留
- 可选：`mobile_real_device_field_trial_acceptance_session`、`mobile_real_device_field_trial_acceptance_session_summary`、`mobile_real_device_field_trial_acceptance_session_copy` 或 `/api/status.phone_readiness.mobile_real_device_field_trial_acceptance_session*` 作为现场验收会话包；输入优先级为显式 acceptance session、field-trial retest execution、field-trial evidence verdict、current PWA field-trial browser proof，缺失时派生 blocked-by-design session。它只组织 real device、production app、PWA prompt、install/user choice、offline reload、touch、visual、redaction 和 operator/support note 的会话 checklist，`safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`、`not_proven` 必须保留
...
- 新增事件捕获字段为 `mobile_pwa_install_prompt_event_capture`、`mobile_pwa_install_prompt_event_capture_summary`、`mobile_pwa_install_prompt_event_capture_copy`，并兼容这些字段出现在 `phone_readiness` 或 `/api/diagnostics`。
- 新增导出字段为 `mobile_pwa_install_prompt_evidence_export`、`mobile_pwa_install_prompt_evidence_export_summary`、`mobile_pwa_install_prompt_evidence_export_copy`，并兼容这些字段出现在 `phone_readiness` 或 `/api/diagnostics`。
...
- 事件捕获 schema 为 `trashbot.mobile_pwa_install_prompt_event_capture.v1`，summary/copy schema 为 `trashbot.mobile_pwa_install_prompt_event_capture_summary.v1` 和 `trashbot.mobile_pwa_install_prompt_event_capture_copy.v1`，本地证据边界统一为 `software_proof_docker_mobile_pwa_install_prompt_event_capture_gate`。
- 导出 schema 为 `trashbot.mobile_pwa_install_prompt_evidence_export.v1`，summary/copy schema 为 `trashbot.mobile_pwa_install_prompt_evidence_export_summary.v1` 和 `trashbot.mobile_pwa_install_prompt_evidence_export_copy.v1`，本地证据边界统一为 `software_proof_docker_mobile_pwa_install_prompt_evidence_export_gate`。
- 静态入口监听 `beforeinstallprompt`、`appinstalled`，并只从 `beforeinstallprompt.userChoice` 派生 allowlisted outcome：`accepted`、`dismissed`、`unknown`、`not_proven`。deferred prompt 只保存在 runtime，不进入 copy package。
...
- Event capture panel/copy 必须展示 beforeinstallprompt observed/missing/blocked、client timestamp、display-mode、platform/browser summary、prompt availability、userChoice outcome、choice timestamp、appinstalled observed/timestamp、`safe_to_control=false`、`accepted_processing_only_not_delivery_success` 和 `not_proven`。
- Export 输入优先级为显式 `mobile_pwa_install_prompt_evidence_export*`，其次 `mobile_pwa_install_prompt_event_capture*`，再其次既有 `mobile_pwa_install_prompt_evidence*`、`mobile_device_handoff_session*`、`mobile_device_evidence_capture*`、`mobile_browser_acceptance_bundle*`；全部缺失时派生 blocked-by-design export。
- Export copy 只允许 `schema=trashbot.mobile_pwa_install_prompt_evidence_export_copy.v1`、`schema_version=1`、`source=mobile_web`、`overall_status`、`install_prompt_capture_status`、`install_prompt_user_choice`、`appinstalled_status`、`display_mode`、`manifest_present`、`service_worker_status`、`client_reference`、`client_timestamp`、`safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`、`evidence_boundary=software_proof_docker_mobile_pwa_install_prompt_evidence_export_gate`、`not_proven` 和 `safe_phone_copy`。
...
- 事件捕获复制包是 whitelist-only，只允许 schema/version、source、overall status、事件状态和时间、display-mode、platform/browser summary、prompt availability、production_app_ready=false、safe_to_control=false、safe copy、recovery hint、ACK 语义、evidence boundary 和 `not_proven`。
- 复制/导出包不得包含 token、Authorization、OSS AK/SK、root password、DB/queue URL、credential-bearing URL、raw ROS topic、`/cmd_vel`、serial/UART、baudrate、WAVE ROVER、本地路径、traceback、checksum、完整 artifact、raw robot response、raw event payload 或 robot/internal 技术字段。
...
- 首屏“真实设备现场试跑包”panel 消费或派生 `mobile_real_device_field_trial_package`、`mobile_real_device_field_trial_package_summary`、`mobile_real_device_field_trial_package_copy`；缺少后端字段时从当前浏览器 runtime metadata 和人工 observation fields 生成 blocked software-proof package。
- schema 为 `trashbot.mobile_real_device_field_trial_package.v1`，summary schema 为 `trashbot.mobile_real_device_field_trial_package_summary.v1`，copy schema 为 `trashbot.mobile_real_device_field_trial_package_copy.v1`，本地证据边界为 `software_proof_docker_mobile_real_device_field_trial_package_gate`。
- 自动采集白名单 runtime metadata：viewport CSS width/height、device pixel ratio、orientation、touch capability、max touch points、coarse pointer、display-mode、manifest link presence、service worker support/registration hint、offline shell hint、client timestamp 和去 query/hash 的 entry URL summary。
- 人工 observation fields 只允许记录 device type、OS/browser、production app observed、PWA install prompt observed、user choice、offline reload observed、touch target issue、visual issue、operator note、support note；输入必须经过 phone-safe 过滤。
- copy package 必须固定 `safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`，并保留 `not_proven`：真实 iPhone/Android device behavior、production app、真实 PWA install prompt/user choice、Objective 5 外部 proof、HIL 和 delivery success。
...
- 首屏“现场试跑执行清单”panel 消费或派生 `mobile_real_device_field_trial_runbook_execution`、`mobile_real_device_field_trial_runbook_execution_summary`、`mobile_real_device_field_trial_runbook_execution_copy`，输入来源是上一轮 `mobile_real_device_field_trial_review` / summary / copy。
- schema 为 `trashbot.mobile_real_device_field_trial_runbook_execution.v1`，summary schema 为 `trashbot.mobile_real_device_field_trial_runbook_execution_summary.v1`，copy schema 为 `trashbot.mobile_real_device_field_trial_runbook_execution_copy.v1`，本地证据边界为 `software_proof_docker_mobile_real_device_field_trial_runbook_execution_gate`。
- Execution checklist 必须覆盖 real device、production app、PWA install prompt、user choice、offline、touch、visual 和 material redaction；缺失时必须显示 open item 或 `not_proven`。
- `mobile_real_device_field_trial_runbook_execution_copy` 是 whitelist-only support metadata，必须固定 `safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`，并完整保留 `not_proven`。
- 复制包不得包含 token、Authorization、OSS AK/SK、DB/queue URL、credential-bearing URL、raw ROS topic、`/cmd_vel`、serial/UART、baudrate、WAVE ROVER 参数、本地路径、traceback、checksum、complete artifacts、raw robot responses、raw intake JSON 或 robot/internal technical fields。
- 本 gate 只指导下一次真实手机现场试跑执行和采证，不证明真实手机验收、production app readiness、真实 PWA install prompt/user choice、Objective 5 外部 proof、HIL、真实 dropoff/cancel completion 或 delivery success，也不新增控制放行条件。

现场证据记录规则：

- 首屏“现场证据记录”panel 消费或派生 `mobile_real_device_field_trial_evidence_record`、`mobile_real_device_field_trial_evidence_record_summary`、`mobile_real_device_field_trial_evidence_record_copy` 和 `mobile_real_device_field_trial_evidence_record_archive`，输入来源是上一轮 `mobile_real_device_field_trial_runbook_execution` / summary / copy。
- schema 为 `trashbot.mobile_real_device_field_trial_evidence_record.v1`，summary schema 为 `trashbot.mobile_real_device_field_trial_evidence_record_summary.v1`，copy schema 为 `trashbot.mobile_real_device_field_trial_evidence_record_copy.v1`，archive schema 为 `trashbot.mobile_real_device_field_trial_evidence_record_archive.v1`，本地证据边界为 `software_proof_docker_mobile_real_device_field_trial_evidence_record_gate`。
- Record fields 必须覆盖 real device、production app、PWA install prompt、user choice、offline、touch、visual、material redaction、operator note 和 support note；缺失时必须显示 open item 或 `not_proven`。
- `mobile_real_device_field_trial_evidence_record_copy` 与 `mobile_real_device_field_trial_evidence_record_archive` 都是 whitelist-only support metadata，必须固定 `safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`，并完整保留 `not_proven`。
- 复制/归档包不得包含 token、Authorization、OSS AK/SK、DB/queue URL、credential-bearing URL、raw ROS topic、`/cmd_vel`、serial/UART、baudrate、WAVE ROVER 参数、本地路径、traceback、checksum、complete artifacts、raw robot responses、raw intake JSON 或 robot/internal technical fields。
- 本 gate 只记录和归档 phone-safe 现场观察摘要，不证明真实手机验收、production app readiness、真实 PWA install prompt/user choice、Objective 5 外部 proof、HIL、真实 dropoff/cancel completion 或 delivery success，也不新增控制放行条件。

现场证据 verdict 规则：

- 首屏“现场证据 verdict”panel 消费或派生 `mobile_real_device_field_trial_evidence_verdict`、`mobile_real_device_field_trial_evidence_verdict_summary` 和 `mobile_real_device_field_trial_evidence_verdict_copy`，输入来源是上一轮 `mobile_real_device_field_trial_evidence_record` / summary / copy / archive。
- schema 为 `trashbot.mobile_real_device_field_trial_evidence_verdict.v1`，summary schema 为 `trashbot.mobile_real_device_field_trial_evidence_verdict_summary.v1`，copy schema 为 `trashbot.mobile_real_device_field_trial_evidence_verdict_copy.v1`，本地证据边界为 `software_proof_docker_mobile_real_device_field_trial_evidence_verdict_gate`。
- Verdict 必须展示字段缺口、复核结论、下一步 `retest_request` 和 `next_material_request`；缺失材料继续保持 `not_proven`。
- `mobile_real_device_field_trial_evidence_verdict_copy` 是 whitelist-only support metadata，必须固定 `safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`，并完整保留 `not_proven`。
- 复制包不得包含 token、Authorization、OSS AK/SK、DB/queue URL、credential-bearing URL、raw ROS topic、`/cmd_vel`、serial/UART、baudrate、WAVE ROVER 参数、本地路径、traceback、checksum、complete artifacts、raw robot responses、raw intake JSON、record/archive 原文或 robot/internal technical fields。
- 本 gate 只复核 field trial evidence record/archive 并安排下一轮 retest/material request，不证明真实手机验收、production app readiness、真实 PWA install prompt/user choice、Objective 5 外部 proof、HIL、真实 dropoff/cancel completion 或 delivery success，也不新增控制放行条件。

现场复测执行规则：

- 首屏“现场复测执行”panel 消费或派生 `mobile_real_device_field_trial_retest_execution`、`mobile_real_device_field_trial_retest_execution_summary` 和 `mobile_real_device_field_trial_retest_execution_copy`，输入来源是上一轮 `mobile_real_device_field_trial_evidence_verdict` / summary / copy。
- schema 为 `trashbot.mobile_real_device_field_trial_retest_execution.v1`，summary schema 为 `trashbot.mobile_real_device_field_trial_retest_execution_summary.v1`，copy schema 为 `trashbot.mobile_real_device_field_trial_retest_execution_copy.v1`，本地证据边界为 `software_proof_docker_mobile_real_device_field_trial_retest_execution_gate`。
- Retest execution 必须把 verdict 的 `retest_request` / `material_request` 落成执行 checklist，并展示 missing evidence summary、source boundary、ACK-not-delivery 和 `not_proven`。
- `mobile_real_device_field_trial_retest_execution_copy` 是 whitelist-only support metadata，必须固定 `safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`，并完整保留 `not_proven`。
- 复制包不得包含 token、Authorization、OSS AK/SK、DB/queue URL、credential-bearing URL、raw ROS topic、`/cmd_vel`、serial/UART、baudrate、WAVE ROVER 参数、本地路径、traceback、checksum、complete artifacts、raw robot responses、raw intake JSON、verdict 原文/附件或 robot/internal technical fields。
- 本 gate 只把 verdict 的复测和材料请求推进成下一轮真实手机现场执行 checklist，不证明真实手机验收、production app readiness、真实 PWA install prompt/user choice、真实公网 HTTPS/TLS、4G/SIM、OSS/CDN live、production DB/queue、HIL、真实 dropoff/cancel completion 或 delivery success，也不新增控制放行条件。

现场验收会话规则：

- 首屏“现场验收会话”panel 消费或派生 `mobile_real_device_field_trial_acceptance_session`、`mobile_real_device_field_trial_acceptance_session_summary` 和 `mobile_real_device_field_trial_acceptance_session_copy`。
- 输入优先级固定为显式 `mobile_real_device_field_trial_acceptance_session*`，然后是 `mobile_real_device_field_trial_retest_execution*`、`mobile_real_device_field_trial_evidence_verdict*`、`mobile_current_pwa_field_trial_browser_proof*`，全部缺失时只派生 blocked-by-design session。
- schema 为 `trashbot.mobile_real_device_field_trial_acceptance_session.v1`，summary schema 为 `trashbot.mobile_real_device_field_trial_acceptance_session_summary.v1`，copy schema 为 `trashbot.mobile_real_device_field_trial_acceptance_session_copy.v1`，本地证据边界为 `software_proof_docker_mobile_real_device_field_trial_acceptance_session_gate`。
- Checklist 必须覆盖 real device observed、production app observed、PWA install prompt observed、install/user choice observed、offline reload observed、touch target issue、visual issue、material redaction status 和 operator/support note。
- `mobile_real_device_field_trial_acceptance_session_copy` 是 whitelist-only support metadata，必须固定 `safe_to_control=false`、`ack_semantics=accepted_processing_only_not_delivery_success`，并完整保留 `not_proven`。
- 复制包不得包含 token、Authorization、OSS AK/SK、DB/queue URL、credential-bearing URL、raw ROS topic、`/cmd_vel`、serial/UART、baudrate、WAVE ROVER 参数、本地路径、traceback、checksum、complete artifacts、raw robot responses、raw intake JSON、完整材料或 robot/internal technical fields。
- 本 gate 只把 retest/verdict/current PWA browser proof 转成可执行现场验收会话，不证明真实手机验收、production app、真实 PWA prompt/user choice、O5 external proof、HIL、真实 dropoff/cancel completion 或 delivery success，也不新增控制放行条件。
...
| 当前 mobile-field-trial-runbook-execution gate | 首屏现场试跑执行清单、八项 execution checklist、whitelist-only execution copy package 和非控制放行边界 |
| 当前 mobile-field-trial-evidence-recorder gate | 首屏现场证据记录、十项 record fields、whitelist-only copy/archive package 和非控制放行边界 |
| 当前 mobile-field-trial-evidence-verdict gate | 首屏现场证据 verdict、missing evidence、retest/material request、whitelist-only verdict copy package 和非控制放行边界 |
