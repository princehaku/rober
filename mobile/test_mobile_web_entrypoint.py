...
class MobileWebEntrypointTest(unittest.TestCase):
    ...
    def test_browser_acceptance_gate_targets_current_mobile_web_pwa(self):
        ...
        self.assertIn('"routeElevatorFieldSessionHandoffTitle"', script)
        self.assertIn('"routeElevatorFieldSessionHandoffBoundary"', script)
        self.assertIn('"routeElevatorFieldSessionHandoffControls"', script)
        self.assertIn('"routeElevatorFieldSessionHandoffNotProven"', script)
        self.assertIn('"routeElevatorFieldSessionHandoffHint"', script)
        self.assertIn('"fieldEvidenceMaterialResolutionIntakeTitle"', script)
        self.assertIn('"fieldEvidenceMaterialResolutionReviewDecisionTitle"', script)
        self.assertIn('"fieldEvidenceMaterialResolutionReviewHandoffTitle"', script)
        self.assertIn('"fieldEvidenceMaterialResolutionFollowupEscalationStatusTitle"', script)
        self.assertIn('"fieldEvidenceMaterialResolutionOwnerResponseIntakeTitle"', script)
        ...
        self.assertIn('"PWA 安装提示导出"', script)
        ...
        self.assertIn("route_elevator_field_session_handoff_visible", script)
        self.assertIn("route_elevator_field_session_handoff_fail_closed", script)
        self.assertIn("mobile_route_elevator_handoff_browser", script)
        ...
        self.assertIn("--evidence-boundary", script)
    ...
    def test_current_browser_gate_field_trial_boundary_keeps_compatibility(self):
        ...
        self.assertIn("software_proof_docker_field_evidence_material_resolution_review_decision_gate", script)
        self.assertIn("software_proof_docker_field_evidence_material_resolution_review_handoff_gate", script)
        self.assertIn("software_proof_docker_field_evidence_material_resolution_followup_escalation_status_gate", script)
        self.assertIn("software_proof_docker_field_evidence_material_resolution_owner_response_intake_gate", script)
    ...
    def test_static_shell_files_exist_and_reference_phone_safe_schema(self):
        ...
        self.assertIn("mobile_field_material_review_decision", app)
        self.assertIn("mobile_field_material_review_decision_summary", app)
        self.assertIn("mobileFieldMaterialReviewDecisionTitle", index)
        self.assertIn("copyMobileFieldMaterialReviewDecisionButton", index)
    ...
    def test_mobile_field_material_review_decision_panel_is_read_only_and_copyable(self):
        app = self.read("app.js")
        index = self.read("index.html")
        fixture_text = json.dumps(json.loads(FIXTURE.read_text(encoding="utf-8")), ensure_ascii=False)

        # review decision 只消费 phone-safe 摘要，不能读取 raw artifact 或改变动作 gating。
        self.assertIn("mobileFieldMaterialReviewDecisionTitle", index)
        self.assertIn("现场材料评审决策", index)
        self.assertIn("mobileFieldMaterialReviewDecisionState", index)
        self.assertIn("mobileFieldMaterialReviewDecisionBlocker", index)
        self.assertIn("mobileFieldMaterialReviewDecisionNextEvidence", index)
        self.assertIn("mobileFieldMaterialReviewDecisionOwnerHandoff", index)
        self.assertIn("mobileFieldMaterialReviewDecisionEvidenceRef", index)
        self.assertIn("mobileFieldMaterialReviewDecisionSameRef", index)
        self.assertIn("delivery_success=false / primary_actions_enabled=false", index)
        self.assertIn("software_proof_docker_mobile_field_material_review_decision_gate", index)
        self.assertIn("mobileFieldMaterialReviewDecisionCandidate", app)
        self.assertIn("mobileFieldMaterialReviewDecisionFromStatus", app)
        self.assertIn("mobile_field_material_review_decision", app)
        self.assertIn("mobile_field_material_review_decision_summary", app)
        self.assertIn("diagnosticsSummary.mobile_field_material_review_decision", app)
        self.assertIn("nestedDiagnosticsSummary.mobile_field_material_review_decision", app)
        self.assertIn("statusDiagnosticsSummary.mobile_field_material_review_decision", app)
        ...
        self.assertIn("UNSAFE_MOBILE_FIELD_MATERIAL_REVIEW_DECISION_TEXT", app)
        self.assertIn("trashbot.mobile_field_material_review_decision_copy.v1", app)
        self.assertIn("copyMobileFieldMaterialReviewDecisionButton", app)
        self.assertIn("downloadMobileFieldMaterialReviewDecisionButton", app)
        self.assertIn("blocked_missing_route_elevator_field_materials", fixture_text)
