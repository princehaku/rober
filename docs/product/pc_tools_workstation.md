...
- 2026-07-02 CST 现场复核历史证据：历史版本曾把 `/map` 缩放推进到 `3200%`、`8000%/9600%`，会让浏览器形成超宽滚动画布，不再作为当前有效口径。当前“PC 地图太小”的默认处理是使用 PC 大地图、访问 `http://<PC>:7001/map`，默认 `100%` 完整态势同屏看路线/车位/雷达，或在同一页面点 `细节放大` 到最高 `4800%`；ROS2 配套只推荐工程观察：本机 RViz2 看 `/map`、`/scan`、TF、路径、定位和 costmap，远程多人观察用 Foxglove bridge + Foxglove Web，不替代普通用户 PC 大地图，也不作为发车、自由移动或建图前置。
...
- 2026-07-01 12:40 CST 起，普通首屏地图必须继续保持普通用户“大地图优先”；历史默认曾使用 `400%` 现场大图，当前默认已改为 `100%` 完整态势、最高细节放大为 `4800%`；`/map` 是推荐直达地图大屏入口，`?view=map` 保留兼容，用户需要看局部时点“细节放大”。ROS2 配套只作旁路：RViz2 / `nav2_rviz_plugins` 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位，Foxglove / `foxglove_bridge` 用于浏览器观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-01 12:40 CST 起，普通首屏地图必须保持可适配全图；历史默认曾使用 `400%` 现场大图，当前默认使用 `100%` 完整态势，`/map` 直达地图大屏也默认使用 `100%`，`?view=map` 继续兼容，并继续保留同一张 WYSIWYG 画布里的地图、路线、小车位置和雷达贴图。“细节放大”按钮一键到 `4800%`，`+/-` 支持逐级缩放，“完整态势”保持回到 `100%` 全图。ROS2 配套只作旁路：RViz2 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位；Foxglove 用于部署 bridge 后的浏览器远程观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-01 08:35 CST 起，普通首屏地图必须继续按 PC 主视图处理：历史默认曾为 `400%` 现场大图，当前默认为 `100%` 完整态势、最高 `4800%` 细节放大、桌面外壳接近全宽、大图高度贴近整屏，只看地图模式使用普通用户文案并保留路线、小车位置和雷达贴图；`完整态势` 可回到 `100%` 全图。ROS2 配套分层为 RViz2 / `nav2_rviz_plugins` 做工程调试，Foxglove / `foxglove_bridge` 做浏览器远程观察；普通用户默认不离开简易 PC 工作站。
...
- 2026-06-30 06:05 CST 起，普通首屏地图默认缩放已提升到 `400%`，当前缩放档位支持到 `1200%`，大图高度按当前 PC 主画布口径执行，并暴露 `data-default-map-zoom-percent=400%`。普通操作继续留在 PC 工作站大地图；ROS2 工程观察首选 RViz2，远程浏览器观察可接 Foxglove Bridge/Studio，但 PC 页面不会自动启动这些 ROS2 runtime。
...
默认仍使用 PC 大地图，当前缩放合同为 `100%` 完整态势、最高 `4800%` 细节放大，地图、路线、小车位置和雷达贴图共享同一张 WYSIWYG 画布。
...
2026-07-04 00:16 CST 起，普通 PC 首页地图默认使用 100% 完整态势视角：首屏驾驶台保持左侧地图、
...
`radar_map_points_visible=true`、`camera_current_visible=false`、`map_display_default_zoom_percent=1600%`（该现场证据属于当时 1600% 版本；当前有效默认值已在 2026-07-06 05:36 调整为 `100%`）。
