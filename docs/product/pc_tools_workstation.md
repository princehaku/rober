...
- 2026-07-02 CST 起，地图太小的当前有效口径为：普通用户默认先进 PC `/map` 大屏，地图缩放默认 `1600%`、最高 `4800%`，`map_display_direct_map_viewport_priority=fullscreen_map_canvas`，`map_display_direct_map_canvas_height_mode=viewport_dominant_full_height`。地图卡工具行新增 `ROS2观察` 展开按钮，但它只展开 RViz2/Foxglove 观察说明和命令，固定 `data-sends-motion-when-clicked=false`、`data-starts-ros2=false`、`data-starts-rviz2=false`、`data-starts-foxglove=false`、`data-starts-nav2=false` 和 `data-starts-map-runtime=false`。ROS2 配套答案保持分层：本地工程调试用 `ros2 launch ros2_trashbot_bringup rviz.launch.py`；远程浏览器观察用 `ros2 launch ros2_trashbot_bringup foxglove_bridge.launch.py` + Foxglove Web 连接 `ws://192.168.1.11:8765`；普通用户界面不被替代。
- 2026-07-02 CST 起，现场验收包的 `no_motion_readback_actions[]` 必须暴露完整只读链路：每个 action 都包含 `sequence_endpoints`、`sequence_labels`、`refreshes_summary`、`refreshes_radar_scan_proof`、`refreshes_camera_first_frame_probe`、`refreshes_map_preview`、`refreshes_radar_status` 和 `refreshes_camera_mjpeg_status`。顶层同步暴露 `field_acceptance_no_motion_readback_action_sequences`、`field_acceptance_no_motion_readback_action_sequence_labels` 和 primary action 的 sequence/refresh flags。雷达贴图 primary 链路固定为 `/api/robot-control/radar/scan-proof/refresh -> /api/robot-control/summary -> /api/robot-control/radar/status -> /api/robot-control/map/preview`，用于确保“雷达开始后地图标记所见即所得”的只读复验不再停在单个 proof endpoint；PC 首屏“只读复验全部”必须按 `readback_all.sequence_endpoints` 的白名单顺序执行，白名单外 endpoint 直接跳过。这些字段和点击路径都只描述/执行读回，不启动 Nav2/manual/keyboard/free-roam/建图/雷达 lifecycle，不提交送达，不发送 stop 或 `/cmd_vel`。
- 2026-07-02 CST 起，summary 顶层必须保留现场脚本短 alias：`field_acceptance_primary_missing_id`、`field_acceptance_primary_missing_label`、`field_acceptance_primary_missing_action_id`、`field_acceptance_primary_readback_endpoint`、`field_acceptance_primary_readback_method`、`field_acceptance_primary_requires_motion_before_readback`、`field_acceptance_primary_requires_safety_confirm_before_motion`、`field_acceptance_primary_blocks_field_acceptance`、`live_wysiwyg_missing_reasons` 和 `mapping_start_missing_evidence`。普通首屏 `plain-field-acceptance-packet` DOM 必须同步暴露对应 `data-field-acceptance-primary-*`、`data-live-wysiwyg-missing-reasons` 和 `data-mapping-start-missing-evidence`。这些字段必须与 `field_acceptance_primary_missing_evidence_*`、`live_wysiwyg_missing_surface_ids`、`mapping_start_missing_reasons` 同源；它们只服务 `curl | jq`、DOM smoke 和现场验收读回，不新增判断逻辑，不启动 Nav2/manual/keyboard/free-roam/建图/雷达 lifecycle，不提交送达，不发送 stop 或 `/cmd_vel`。
...
- 2026-07-01 20:25 CST 起，普通首屏地图和 `/map` 直达大屏默认缩放提升为 `1600%` 现场大图，地图大面板继续占据首屏主视图；`适配` 按钮仍固定回到 `100%` 全图，最高细节放大保持 `4800%`。这是普通用户解决“地图太小”的默认路径；ROS2 配套仍只放在默认折叠的“工程观察”里，RViz2/Foxglove 不作为普通用户发车入口，也不得自动启动 ROS2、Nav2、建图 runtime、manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-01 16:45 CST 起，`GET /api/robot-control/summary` 顶层还必须直接暴露雷达贴图恢复 alias：`radar_overlay_primary_blocked_reason`、`radar_overlay_current_vs_source_plain`、`radar_overlay_refresh_next_action_plain`、`radar_overlay_needs_refresh`、`radar_overlay_blocks_wysiwyg`、`radar_overlay_blocks_free_move=false`、`radar_overlay_recovery_sequence`、`fixed_radar_overlay_refresh_endpoint=/api/robot-control/radar/scan-proof/refresh`、`fixed_radar_overlay_map_preview_endpoint=/api/robot-control/map/preview`、`radar_overlay_refresh_sends_motion=false` 和 `radar_overlay_refresh_starts_radar_lifecycle=false`。这些字段与 `live_closure_summary` 同源，用于现场一条 `curl | jq` 判断旧雷达点是否已抑制、是否只阻塞 WYSIWYG、以及下一步是否只需手动刷新雷达扫描读数再刷新地图画面；不得启动雷达 lifecycle、Nav2、manual/keyboard/free-roam/建图/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-01 13:26 CST 起，普通首屏地图和 `/map` 直达大屏在 `100%`/`2400%` 放大时必须保证整张画布四周都可滚动到：`.plain-map-layer` 的滚动原点固定为左上完整画布，DOM 暴露 `data-scroll-origin=top_left_full_canvas` 和 `data-auto-center-on-zoom=true`；进入 `/map`、全屏、只看地图、刷新地图画面或调整缩放后，PC 端只自动居中当前视口，不改变地图、路线、小车位置或雷达点坐标。该修正只解决大地图负方向溢出导致边缘不可达的问题，不启动 ROS2/RViz2/Foxglove、Nav2、建图 runtime，不发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
- 2026-07-01 12:40 CST 起，普通首屏地图必须继续保持普通用户“大地图优先”；2026-07-01 22:35 CST 后当前默认使用 `300%` 现场大图，最高细节放大仍为 `2400%`；`/map` 是推荐直达地图大屏入口，`?view=map` 保留兼容，两者默认同样使用 `300%`，用户需要看完整全图时点“适配”回到 `100%`。ROS2 配套只作旁路：RViz2 / `nav2_rviz_plugins` 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位，Foxglove / `foxglove_bridge` 用于浏览器观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-01 12:40 CST 起，普通首屏地图必须保持可适配全图；2026-07-01 22:35 CST 后默认使用 `300%` 现场大图，`/map` 直达地图大屏也默认使用 `300%`，`?view=map` 继续兼容，并继续保留同一张 WYSIWYG 画布里的地图、路线、小车位置和雷达贴图。“细节放大”按钮一键到 `2400%`，`+/-` 支持逐级缩放，“适配”保持回到 `100%` 全图。ROS2 配套只作旁路：RViz2 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位；Foxglove 用于部署 bridge 后的浏览器远程观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-01 08:35 CST 起，普通首屏地图必须继续按 PC 主视图处理：当前默认 `300%` 现场大图、最高 `2400%` 细节放大、桌面外壳接近全宽、大图高度贴近整屏，只看地图模式使用普通用户文案并保留路线、小车位置和雷达贴图；`适配` 可回到 `100%` 全图。ROS2 配套分层为 RViz2 / `nav2_rviz_plugins` 做工程调试，Foxglove / `foxglove_bridge` 做浏览器远程观察；普通用户默认不离开简易 PC 工作站。
...
- 2026-06-30 13:17 CST 起，`/map` 与兼容 `?view=map` 直达地图大屏必须切换 App 页面壳到 map-only：隐藏普通顶栏和高级工具，`.shell` 使用 `100vw` / `100vh`，地图组件进入 full + 只看地图状态；2026-07-01 22:35 CST 后当前默认使用 `300%` 现场大图，最高 `2400%` 细节放大，“适配”回到 `100%` 全图；同时继续暴露 `data-direct-map-view-behavior=page_shell_map_only` 与地图卡 `page_fixed_fullscreen_map_only`。该模式只为第二屏/PC 大屏观察地图、路线、小车位置和雷达贴图，不启动 RViz2/Foxglove/ROS2 runtime，不执行 Nav2，不发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-06-30 06:05 CST 起，普通首屏地图默认缩放已提升到 `300%`，缩放档位继续支持到 `2400%`，大图高度提升到 `clamp(840px, calc(100vh - 132px), 1400px)`，全屏地图高度提升到 `calc(100vh - 96px)`，并暴露 `data-default-map-zoom-percent=300%`。普通操作继续留在 PC 工作站大地图；ROS2 工程观察首选 RViz2，远程浏览器观察可接 Foxglove Bridge/Studio，但 PC 页面不会自动启动这些 ROS2 runtime。
...
WYSIWYG 地图画布和 4800% 缩放上限；ROS2 本机工程调试使用 `ros2 launch ros2_trashbot_bringup rviz.launch.py`
