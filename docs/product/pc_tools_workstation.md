...
- `o7ConsumerReadAdapter.ts` 只允许本机 HTTP 回环 base URL，把 O6 `GET /api/o6/consumer/tasks` 和 `GET /api/o6/consumer/tasks/<task_id>` 压成 PC 端 `trashbot.pc_tools_workstation.o7_consumer_task_list.v1` / `trashbot.pc_tools_workstation.o7_consumer_task_detail.v1` 摘要；固定 `view=summary`，detail 默认 include `trajectory,events,evidence,field_evidence,labeling,inference,tunnel,artifact_access_probe,offline_artifact_seed_smoke,route_root_seed_gate,route_bag_evidence,route_bag_payload_replay,route_bag_semantic_replay,route_bag_full_semantic_decode_matrix,route_bag_pose_progress_replay,nav2_goal_execution_evidence,delivery_result_evidence,route_execution_result_delivery_readiness,route_delivery_closure_packet,same_task_field_material_packet,current_field_evidence_material,clean_baseline_nav2_path_material,localization_path_material_readback,same_task_route_execution_material_packet,same_task_mission_evidence_gate,field_operator_confirmation_material`。坏 URL、非回环、credentials/query/hash、schema mismatch、fetch 失败、非 object、危险 true、unsafe ref、unsafe text、proof scope mismatch、`route_bag_required=true` 或 `route_bag_present=true` 都保持 fail-closed，不展示 token、绝对路径、base64、allowlist root、控制字段、原始 payload 或原始路径。只有远端缺失 field evidence 且可选 query `fieldEvidenceManifestJson=<local-json>` 指向合法 manifest 时，才用本地 manifest 补齐 `field_evidence`；`trajectory/events/evidence/labeling/inference/tunnel/artifact_access_probe/offline_artifact_seed_smoke/route_root_seed_gate/route_bag_evidence/route_bag_payload_replay/route_bag_semantic_replay/nav2_goal_execution_evidence/delivery_result_evidence/route_execution_result_delivery_readiness/route_delivery_closure_packet/same_task_field_material_packet/current_field_evidence_material/clean_baseline_nav2_path_material/localization_path_material_readback/same_task_route_execution_material_packet/same_task_mission_evidence_gate/field_operator_confirmation_material` 仍全部来自远端 O6 detail。adapter 会把 `source_origin`、`task_origin`、`manifest_run_id`、`artifact_health_summary`、present/missing artifacts、`manifest_gate`、`artifact_status`、`not_proven`、`safe_to_control`、`delivery_success`、`primary_actions_enabled` 显式带到 O7 页面。
...
- O6 Consumer Read API：`GET /api/o6/consumer/tasks` 和 `GET /api/o6/consumer/tasks/<task_id>` 是当前推荐给 PC/后续手机消费的统一查询面。它在 relay 后端复用同一个 `TRASHBOT_O6_CLOUD_ARCHIVE_STATE`，把 archive task、events、evidence refs、labels、`model_inference.*` timeline、tunnel latest known snapshot、可选 `artifact_access_probe`、`offline_artifact_seed_smoke`、`route_root_seed_gate`、`route_bag_evidence`、`route_bag_payload_replay`、`field_motion_evidence_packet` 和 `nav2_goal_execution_evidence` 聚合成 `trashbot.o6.consumer_read.v1` 读模型，支持 `view=summary` 与 `include=trajectory,events,evidence,field_evidence,labeling,inference,tunnel,artifact_access_probe,offline_artifact_seed_smoke,route_root_seed_gate,route_bag_evidence,route_bag_payload_replay,route_bag_semantic_replay,route_bag_full_semantic_decode_matrix,route_bag_pose_progress_replay,nav2_goal_execution_evidence,delivery_result_evidence,route_execution_result_delivery_readiness,route_delivery_closure_packet,same_task_field_material_packet,current_field_evidence_material,clean_baseline_nav2_path_material,same_task_route_execution_material_packet,same_task_mission_evidence_gate,field_operator_confirmation_material`。PC 读历史任务、详情、轨迹、事件、打标状态、tunnel 在线态、artifact access probe、offline seed smoke、route-root seed gate、route bag evidence / payload replay、field motion evidence packet、Nav2 goal evidence 和 delivery result evidence 时应优先走这两个 endpoint，而不是在前端重复 join 多条 `/api/o6/archive/*`、`/api/o6/tunnel/*`。该接口仍固定 `proof_status=not_proven`、`safe_to_control=false`、`connects_cloud_production=false`、`robot_control_executed=false`；`selected` 只是 store 单选标记，不等于 UI 用户选择；`tunnel_status` 明确是 latest known robot snapshot，不是 task 内历史事实；没有 labels / inference / tunnel / probe / route-root seed gate / route bag payload replay / field motion packet / Nav2 goal evidence 时必须显式返回 `pending`、`absent`、`blocked_not_proven`、`artifact_access_probe_missing`、`route_root_seed_gate_missing`、`route_bag_payload_replay_missing`、`field_motion_evidence_packet_missing` 或 `nav2_goal_execution_evidence_missing` 等 fail-closed 摘要，不得伪造“真实没有异常”。
- O7 Consumer Detail Route Replay：O7 Previews 的 `O7 consumer read primary path` 现在把历史回放主路径切到 PC 后端 `GET /api/o7/consumer-read/tasks` 和 `GET /api/o7/consumer-read/tasks/<task_id>` adapter。operator 加载 task list 后选择 task，可选填写 `Local field evidence manifest JSON`，再加载 detail；UI 直接展示 `trajectory / events / evidence / labeling / inference / tunnel_status` 摘要，以及 local manifest query、field evidence contract/input/fail-closed reason、O6 wrapper `source_origin`、`task_origin`、manifest run 和 artifact health。本地 manifest fallback 只补 `field_evidence`，不得覆盖远端 trajectory/events/evidence/labeling/inference/tunnel；远端已有合法 field evidence 时优先远端。detail 现在同时带 `route_bag_evidence` 与 `route_bag_payload_replay` 两层只读摘要：前者保留 DB3 topic/message/timestamp 结构，后者再展示 topic/message/timestamp 摘要、payload size/hash prefix、blocked reasons、next evidence 和 false safety fields；`same_task_field_material_packet`、`current_field_evidence_material` 与 `same_task_route_execution_material_packet` 也各自作为只读摘要分段展示，其中 `current_field_evidence_material` 只说明 current field status、present/missing materials、camera/radar/map/nav2/manual gate booleans 和 support-only blocked 语义，不表示 route execution success。三者都不能打开 submit/control/action。`Consumer-detail route replay player` 的 Play/Pause、Previous/Next、Reset 和 range cursor 只改浏览器内存，不调用 API、不写后端、不发送机器人命令；缺 detail、unknown task、task id mismatch、轨迹缺失、blocked/not_proven/error/cancel 状态或 adapter fail-closed 时禁用导航并显示 blocker。`Consumer-detail trajectory minimap` 只使用 detail 样本中的有限数值型 `x_m/y_m` 归一化到固定 SVG viewBox；没有有效点时显示 `blocked_not_proven`，单点只显示 `readonly_consumer_detail_trajectory_single_point`，不声明真实地图或真实机器人位置。旧 `Cloud Archive Tasks` fixture route replay player 仍保留，但其 cursor 与 consumer 主路径 cursor 隔离，只作为次路径 / debug fallback。
...
- O7 Previews 新增 `O7 consumer read primary path` 区块，作为 O7 任务列表/详情与标注队列检查的首选读取入口。operator 只需提供本机回环 relay base URL，点击 `Load consumer task list` 后，workstation 后端固定请求 `GET /api/o6/consumer/tasks?view=summary&limit=50`；点击 `Load consumer task detail` 后，workstation 后端固定请求 `GET /api/o6/consumer/tasks/<task_id>?view=default&include=trajectory,events,evidence,field_evidence,labeling,inference,tunnel,artifact_access_probe,offline_artifact_seed_smoke,route_root_seed_gate,route_bag_evidence,route_bag_payload_replay,route_bag_semantic_replay,route_bag_full_semantic_decode_matrix,route_bag_pose_progress_replay,nav2_goal_execution_evidence,delivery_result_evidence,route_execution_result_delivery_readiness,route_delivery_closure_packet,same_task_field_material_packet,current_field_evidence_material,clean_baseline_nav2_path_material,same_task_route_execution_material_packet,same_task_mission_evidence_gate,field_operator_confirmation_material`，并可附带 `fieldEvidenceManifestJson=<local-json>` 作为远端缺 field evidence 时的本地只读补齐输入。UI 明确展示 `view=summary`、detail `include=` 策略、`artifact_access_probe` counts / sample refs / sha256 prefix、`route_root_seed_gate` 的 route-root seed status、`route_bag_required=false`、`route_bag_present=false`、frame/ref counts、blocked reasons、next required evidence，`field_motion_evidence_packet` 的 route frame count、nonzero displacement、displacement meters、motion log sources、route_bag/live log present gap、blocked reasons 和 `next_required_evidence`，以及 `nav2_goal_execution_evidence` 的 goal/result/base command 摘要、blocked reasons 和 next required evidence；`safe_to_control=false`、`primary_actions_enabled=false`、`delivery_success=false`、`connects_cloud_production=false`、`robot_control_executed=false`、`media_access_proven=false`、`real_oss_connected=false`、`real_cdn_connected=false`、blocked reasons、not proven、样本计数和 `tunnel_status.temporal_alignment=latest_known_robot_snapshot_not_task_aligned` 持续可见。consumer-detail labeling primary path 与 route replay primary path 都会带上同一 `task_id` 的 field motion summary 和 Nav2 goal summary；该区块不绕过 relay、不连公网云、不发控制命令，也不把缺 labels / inference / tunnel / probe / route-root seed gate / field motion packet / Nav2 goal evidence 解释成真实“无异常”。
...
- `view=summary` 适用于移动端或列表内嵌只读快照；PC 需要逐项排障时可加 `include=trajectory,events,evidence,field_evidence,labeling,inference,tunnel,artifact_access_probe,offline_artifact_seed_smoke,route_root_seed_gate,route_bag_evidence,route_bag_payload_replay,route_bag_semantic_replay,route_bag_full_semantic_decode_matrix,route_bag_pose_progress_replay,nav2_goal_execution_evidence,delivery_result_evidence,route_execution_result_delivery_readiness,route_delivery_closure_packet,same_task_field_material_packet,current_field_evidence_material,clean_baseline_nav2_path_material,same_task_route_execution_material_packet,same_task_mission_evidence_gate,field_operator_confirmation_material`。O7 adapter 现在还会优先消费同一条 O6 detail 主路径上的 `artifact_media_preflight`、`artifact_access_probe`、`offline_artifact_seed_smoke`、`route_root_seed_gate`、`route_bag_evidence`、`route_bag_payload_replay`、`route_bag_semantic_replay`、`route_bag_full_semantic_decode_matrix`、`field_motion_evidence_packet`、`nav2_goal_execution_evidence` 和 `delivery_result_evidence`，并在 consumer detail 中额外派生 `artifact_bundle`、`artifact_bundle_consumer_ingest` 和 `artifact_bundle_readiness`；主路径优先从 O6 top-level `route_root_seed_gate` / `route_bag_evidence` / `route_bag_payload_replay` / `route_bag_semantic_replay` / `route_bag_full_semantic_decode_matrix` / `field_motion_evidence_packet` / `nav2_goal_execution_evidence` / `delivery_result_evidence` / `artifact_bundle` / `artifact_bundle_consumer_ingest` / `field_evidence.artifact_bundle` / `field_evidence_consumer_ingest.artifact_bundle` 读取 route-root、route bag、payload replay、semantic replay、full semantic decode matrix、field motion、Nav2 goal、delivery result 和 route/replay/keyframe/evidence refs，旧 fallback 只做兼容兜底。`artifact_access_probe` 可从 top-level、`field_evidence`、`artifact_bundle` 或相关 ingest wrapper 读取；`offline_artifact_seed_smoke`、`route_root_seed_gate`、`route_bag_evidence`、`route_bag_payload_replay`、`route_bag_semantic_replay`、`route_bag_full_semantic_decode_matrix`、`field_motion_evidence_packet`、`nav2_goal_execution_evidence` 和 `delivery_result_evidence` 可从 top-level、`field_evidence`、`field_motion_evidence_packet`、`field_evidence_consumer_ingest` 或相关 wrapper 读取，只展示 software proof readiness、counts、basename sample refs、motion log sources、route bag topic/message/timestamp 摘要、payload size/hash prefix 摘要、LaserScan/Image/TF/Odometry 语义摘要、topic/type decode coverage 摘要、goal/result/base command 摘要、delivery record/operator confirmation 摘要、blocked reasons 和 next required evidence，不证明真实媒体、真实生产云、真实 annotation API、真实 dataset export、真实 route_bag/live log、真实机器人运动、真实 Nav2 live run 或真实送达。若 O6 暂时缺该 section，workstation 会把对应 section 保持 `blocked_not_proven`，继续把真实媒体可访问性、OSS/CDN 连通性、生产云能力和真实送达能力锁死为 not proven。
