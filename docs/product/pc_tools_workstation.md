...
- `o7ConsumerReadAdapter.ts` 只允许本机 HTTP 回环 base URL，把 O6 `GET /api/o6/consumer/tasks` 和 `GET /api/o6/consumer/tasks/<task_id>` 压成 PC 端 `trashbot.pc_tools_workstation.o7_consumer_task_list.v1` / `trashbot.pc_tools_workstation.o7_consumer_task_detail.v1` 摘要；固定 `view=summary`、detail `include=trajectory,events,evidence,field_evidence,labeling,inference,tunnel,artifact_access_probe,offline_artifact_seed_smoke,route_root_seed_gate,route_bag_evidence,route_bag_payload_replay,route_bag_semantic_replay,route_bag_full_semantic_decode_matrix,route_bag_pose_progress_replay,nav2_goal_execution_evidence,delivery_result_evidence,route_execution_result_delivery_readiness,route_delivery_closure_packet,same_task_field_material_packet,same_task_mission_evidence_gate`，递归扫描危险 true 字段，坏 URL、非回环、credentials/query/hash、schema mismatch、fetch 失败或危险 true 字段均 fail-closed。list 会只读展示 `task_origin`、`field_evidence_source` 和 `field_evidence_artifact_status`；detail 主路径优先使用 O6 新 `field_evidence` wrapper，并验证 wrapper 的 `source_contract` 只能是 `trashbot.field_evidence_manifest.v1` 或 `trashbot.pc_tools_workstation.o7_field_evidence_consumer_ingest.v1`。旧的顶层 `field_evidence_manifest` / `field_evidence_consumer_ingest` 仍兼容；只有远端缺失 field evidence 且可选 query `fieldEvidenceManifestJson=<local-json>` 指向合法 manifest 时，才用本地 manifest 补齐 `field_evidence`，同时保持 `trajectory/events/evidence/labeling/inference/tunnel/artifact_access_probe/offline_artifact_seed_smoke/route_root_seed_gate/route_bag_evidence/route_bag_payload_replay/route_bag_semantic_replay/nav2_goal_execution_evidence/delivery_result_evidence/route_execution_result_delivery_readiness/route_delivery_closure_packet/same_task_field_material_packet/same_task_mission_evidence_gate` 全部来自远端 O6 detail。新增 `route_root_seed_gate` 只展示 O6 `trashbot.o6.route_root_seed_gate.v1` 的安全摘要：`route_root_seed_status`、`route_bag_required=false`、`route_bag_present=false`、frame/ref counts、basename refs、blocked reasons 和 next required evidence；新增 `route_bag_payload_replay` 只展示 O6 `trashbot.o6.route_bag_payload_replay.v1` / `trashbot.route_bag_payload_replay.v1` 的安全摘要：`route_bag_source/status`、topic/message/timestamp 摘要、payload size/hash prefix 摘要、blocked reasons、next required evidence 和 false safety fields；新增 `route_bag_semantic_replay` 只展示 O6 `trashbot.o6.route_bag_semantic_replay.v1` / `trashbot.route_bag_semantic_replay.v1` 的安全摘要：`semantic_decode_status`、`semantic_topic_types`、LaserScan/Image/TF/Odometry 白名单统计、blocked reasons、next required evidence 和 false safety fields；新增 `field_motion_evidence_packet` 只展示 O6 `trashbot.o6.field_motion_evidence_packet.v1` 或源 `trashbot.field_motion_evidence_packet.v1` 的安全摘要：`route_summary.frame_count`、`nonzero_displacement_observed`、`displacement_m`、`motion_log_summary.live_motion_evidence_present`、`motion_log_summary.evidence_sources`、`route_bag_or_live_nav2_log.present/source/route_bag_present/live_motion_log_present`、blocked reasons 和 next required evidence；新增 `route_execution_result_delivery_readiness` 只展示 O6 `trashbot.o6.route_execution_result_delivery_readiness.v1` 的统一结果链安全摘要：`route_execution_result_status/source`、`delivery_result_readiness_status/source`、`operator_confirmation_readiness_status/source`、`nav2_goal_execution_ready`、`delivery_result_ready`、`operator_confirmation_ready`、blocked reasons、next required evidence 和 false safety fields；新增 `route_delivery_closure_packet` 只展示 O6 `trashbot.o6.route_delivery_closure_packet.v1` 的 closure status、linked evidence flags、blocked reasons、next required evidence 和固定 false safety fields；新增 `same_task_field_material_packet` 只展示 O6 `trashbot.o6.same_task_field_material_packet.v1` / `trashbot.same_task_field_material_packet.v1` 的 same-task material status、present/missing materials、basename sample refs、blocked reasons、next required evidence 和固定 false safety fields，不启用提交、控制、发车或 success 标记。上述摘要的 schema mismatch、缺字段、危险 true、unsafe ref、unsafe text、proof scope mismatch、`route_bag_required=true` 或 `route_bag_present=true` 都保持 fail-closed，不展示 token、绝对路径、base64、allowlist root、控制字段、原始 payload 或原始路径。本地 manifest 缺失、坏 JSON、顶层非 object、schema mismatch、unsafe copy 或危险 true claim 均 `fail_closed`。adapter 会把 `source_origin`、`task_origin`、`manifest_run_id`、`artifact_health_summary`、present/missing artifacts、`manifest_gate`、`artifact_status`、`not_proven`、`safe_to_control`、`delivery_success`、`primary_actions_enabled` 显式带到 O7 页面。
