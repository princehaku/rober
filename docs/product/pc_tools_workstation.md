...
pc-tools/workstation/
  ...
  src/server/o7VoiceRuntimePreflight.ts # O7 voice runtime preflight 本地配置检查
...
- 2026-07-13 21:21 CST 起，O7 consumer detail 增加 selected-task `bounded_route_execution_gate_material` intake 入口：浏览器只调用 PC 本机 `POST /api/o7/consumer-read/tasks/<task_id>/bounded-route-gate/intake?baseUrl=<local-loopback-url>`，Node adapter 固定向本机 O6 `POST /api/o6/archive/field-evidence` 写入 `trashbot.o6.bounded_route_execution_gate_material.v1`，再读回 `trashbot.pc_tools_workstation.o7_bounded_route_gate_intake_result.v1` receipt。该入口只允许 `task_id=task_o3_28_pose_fixed_route_consumer_20260713_0402`、`packet_id=packet_o3_28_pose_same_task_replay_7d57826142b0c79c`、`route_intent_id=route_intent_20260713_0402_from_20260713_0300_28_pose_structured_path`、`route_csv_row_count=28`、`path_structured_pose_count=28`、`segment_count=27`、`execution_plan_status=blocked_pending_live_safety_gate` 的安全摘要；非回环 URL、task mismatch、raw local path、raw command body、`/cmd_vel`、`/api/base/manual`、NavigateToPose、serial/UART、WAVE ROVER 或 route/delivery/HIL/control true 字段必须 fail-closed。产品边界固定为 `software_proof_o6_o7_bounded_route_gate_material_intake_only`，并保持 `safe_to_control=false`、`delivery_success=false`、`route_execution_success=false`、`hil_pass=false`、`robot_control_executed=false`、`connects_cloud_production=false`；它只提升 PC/O7 可复现的 O6 local/mock material receipt，不声明 route execution、delivery、HIL、safe-to-control 或 control capability。
- 2026-07-14 01:24 CST 起，O7 consumer detail 增加 selected-task `bounded_route_terminal_result_material` intake 入口：浏览器只调用 PC 本机 `POST /api/o7/consumer-read/tasks/<task_id>/bounded-route-terminal-result/intake?baseUrl=<local-loopback-url>`，Node adapter 固定向本机 O6 `POST /api/o6/archive/field-evidence` 写入 `trashbot.o6.bounded_route_terminal_result_material.v1`，再读回 `trashbot.pc_tools_workstation.o7_bounded_route_terminal_result_intake_result.v1` receipt。该入口只允许 `task_id=task_o3_28_pose_fixed_route_consumer_20260713_0402`、`packet_id=packet_o3_28_pose_same_task_replay_7d57826142b0c79c`、`route_intent_id=route_intent_20260713_0402_from_20260713_0300_28_pose_structured_path`、`result_code=mock_route_execution_completed_not_live_delivery`、`terminal_result_state=terminal_result_recorded`、`reconciliation_state=terminal_result_recorded`、`route_csv_row_count=28`、`path_structured_pose_count=28`、`segment_count=27`、`source_schema=trashbot.o5.bounded_route_terminal_result_bridge.v1`、`source_proof_boundary=software_proof_o5_bounded_route_terminal_result_bridge_only` 和 basename 级 `safe_evidence_ref` 的安全摘要；非回环 URL、非 selected task、task mismatch、raw local path、raw command body、`/cmd_vel`、`/api/base/manual`、NavigateToPose、serial/UART、WAVE ROVER 或 route/delivery/HIL/control true 字段必须 fail-closed。产品边界固定为 `software_proof_o6_o7_bounded_route_terminal_result_intake_only`，并保持 `delivery_success=false`、`route_execution_success=false`、`safe_to_control=false`、`hil_pass=false`、`robot_control_executed=false`、`connects_cloud_production=false`；它只提升 PC/O7 可复现的 O6 local/mock terminal-result receipt，不声明 route execution、delivery、HIL、safe-to-control、production cloud 或 control capability。
- 2026-07-14 06:27 CST 起，O7 consumer detail 增加 selected-task voice/TTS draft event-write 入口：浏览器只调用 PC 本机 `POST /api/o7/consumer-read/tasks/<task_id>/voice/tts-draft/request?baseUrl=<local-loopback-url>`，Node adapter 固定向本机 O6 `POST /api/o6/archive/events` 写入 `event_type=voice.tts_draft`，再读回 `trashbot.pc_tools_workstation.o7_voice_tts_draft_request_result.v1` receipt。该入口只允许同一 `task_id` 的短 `draft_text`、`event_id`、basename 级 `evidence_ref`、`occurred_at_ms`、可选 `voice_profile`、`locale` 和 primitive metadata；非回环 URL、task mismatch、unknown field、unsafe ref、URL、绝对路径、credential/raw/base64-like 内容、音频 payload、真实 voice API 文案、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART 或任何真实能力 true 字段必须 fail-closed。产品边界固定为 `software_proof_o6_o7_voice_tts_draft_event_write_only`，并保持 `tts_send_enabled=false`、`speaker_dispatch_enabled=false`、`real_voice_api_connected=false`、`real_asr_tts_runtime_connected=false`、`safe_to_control=false`、`delivery_success=false`、`robot_control_executed=false`、`connects_cloud_production=false`；它只提升 selected-task voice/TTS 草稿可追溯事件，不声明真实 TTS 发送、音频播放、喇叭调度、production cloud、delivery、HIL 或 safe-to-control。
- 2026-07-14 08:31 CST 起，O7 consumer detail 增加 selected-task operator dropoff action capture 入口：浏览器只调用 PC 本机 `POST /api/o7/consumer-read/tasks/<task_id>/operator/dropoff-acceptance/request?baseUrl=<local-loopback-url>`，Node adapter 固定向本机 O6 `POST /api/o6/archive/events` 写入 `event_type=operator.dropoff_acceptance`，再读回 `trashbot.pc_tools_workstation.o7_operator_dropoff_action_capture_result.v1` receipt。该入口只允许同一 `task_id` 的 `event_id`、`occurred_at_ms`、basename 级 `evidence_ref`、可选 `operator_action_id`、`operator_display_name`、summary 和 primitive metadata；非回环 URL、task mismatch、caller 覆盖 `event_type`、unknown field、unsafe ref、URL、绝对路径、credential/raw/base64-like 内容、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART 或任何真实能力 true 字段必须 fail-closed。产品边界固定为 `software_proof_o6_o7_operator_dropoff_action_capture_only`，并保持 `real_operator_action_proven=false`、`delivery_success=false`、`route_execution_success=false`、`safe_to_control=false`、`hil_pass=false`、`robot_control_executed=false`、`connects_cloud_production=false`；它只证明 selected-task operator action request construction、O6 local archive event write/readback receipt 和 O7 UI receipt 展示，不声明 real operator action、delivery success、route execution、HIL、safe-to-control、production cloud 或 robot control。
- 2026-07-14 09:33 CST 起，该 operator dropoff UI 流程新增 sprint-scoped local browser/DOM artifact：`sprints/2026.07.14_09-33_o7_operator_dropoff_browser_artifact/artifacts/o7_operator_dropoff_browser_artifact.json`，schema 为 `trashbot.pc_tools_workstation.o7_operator_dropoff_browser_artifact.v1`，proof boundary 固定为 `software_proof_o7_operator_dropoff_browser_artifact_only`。artifact 只保存 selected `task_id`、`记录 operator dropoff capture` 按钮文案、endpoint path、receipt schema/status、`event_type=operator.dropoff_acceptance`、固定 `real_operator_action_proven=false`、`delivery_success=false`、`route_execution_success=false`、`safe_to_control=false`、`hil_pass=false`、`robot_control_executed=false`、`connects_cloud_production=false` 和 `not_proven` 列表；它不保存 raw DOM/截图/credential，也不证明真实手机、production cloud/DB/queue/OSS/CDN/4G/SIM、real operator action、delivery success、route execution、HIL、safe-to-control 或机器人运动。
- 2026-07-14 11:35 CST 起，O7 工作站新增 `GET /api/o7/voice-runtime/preflight`：浏览器只调用 PC 本机 Node endpoint，返回 `trashbot.pc_tools_workstation.o7_voice_runtime_preflight_result.v1` 和 `software_proof_o7_voice_runtime_preflight_only`。该 preflight 只读取安全的本地/离线 mode（`offline_stub`、`local_stub`、`disabled_local`）或 schema 为 `trashbot.pc_tools_workstation.o7_voice_runtime_preflight_config.v1` 的本地配置 JSON；它不连接生产云、不访问 voice provider、不打开麦克风/喇叭、不发送 TTS、不播放音频、不写 O6 archive events、不控制机器人。缺 config/mode 返回 `blocked_missing_voice_runtime_config`；安全 local/offline 配置返回 `ready_for_configured_runtime_check_only`；任何 URL/凭证/音频 payload/设备路径/`/cmd_vel`/`/api/base/manual`/NavigateToPose/WAVE ROVER/UART 或 dangerous true claim 必须 `fail_closed`。固定字段保持 `real_voice_api_connected=false`、`real_asr_tts_runtime_connected=false`、`tts_send_enabled=false`、`speaker_dispatch_enabled=false`、`safe_to_control=false`、`delivery_success=false`、`robot_control_executed=false`、`connects_cloud_production=false`；该入口不重复 voice/TTS draft event-write 或 speaker ACK/failure event-write，也不声明真实 ASR/TTS runtime、real speaker ACK、delivery、HIL、safe-to-control 或 O5 production/cloud 证据。
- 2026-07-14 12:35 CST 起，O7 工作站新增 `GET /api/o7/voice-runtime/offline-smoke`：浏览器只调用 PC 本机 Node endpoint，返回 `trashbot.pc_tools_workstation.o7_voice_runtime_offline_smoke_result.v1` 和 `software_proof_o7_voice_runtime_offline_smoke_only`。该 offline smoke 消费 safe local/offline mode（`offline_stub`、`local_stub`、`disabled_local`）或 schema 为 `trashbot.pc_tools_workstation.o7_voice_runtime_offline_smoke_fixture.v1` 的本地 JSON fixture，并把 selected task identity、preflight-derived status、deterministic `preflight_config_checked` / `offline_asr_stub_loaded` / `tts_draft_trace_prepared` / `speaker_ack_pending_not_real` trace events、blocked reasons、not-proven list 和 next required evidence 展示给 operator；它不连接生产云、不访问 voice provider、不打开麦克风/喇叭、不发送 TTS、不播放音频、不写 O6 archive events、不控制机器人。任何 unsupported mode、task mismatch、URL/凭证/音频 payload/设备路径/`/cmd_vel`/`/api/base/manual`/NavigateToPose/WAVE ROVER/UART 或 dangerous true claim 必须 `fail_closed`。固定字段保持 `real_voice_api_connected=false`、`real_asr_tts_runtime_connected=false`、`tts_send_enabled=false`、`speaker_dispatch_enabled=false`、`real_speaker_ack_proven=false`、`microphone_opened=false`、`speaker_playback_opened=false`、`safe_to_control=false`、`delivery_success=false`、`robot_control_executed=false`、`connects_cloud_production=false`、`route_execution_success=false`、`hil_pass=false`；该入口只证明 PC/Node deterministic local/offline smoke trace，不证明 real ASR/TTS runtime、真实麦克风/喇叭、speaker dispatch、real speaker ACK、production cloud、route execution、delivery、HIL、safe-to-control 或机器人控制。
- 2026-07-14 15:38 CST 起，O7 工作站新增 live relay browser smoke artifact 命令：`cd pc-tools/workstation && npm run smoke:o7-live-relay-browser -- --artifact ../../sprints/2026.07.14_15-38_o7_live_relay_browser_smoke_artifact/artifacts/o7_live_relay_browser_smoke_artifact.json`。该命令启动本机 `127.0.0.1:17001` 或下一可用 loopback Express server，经真实 HTTP socket 访问 `/api/health`、`/api/o7/operator-console` 和 `/api/o7/cloud-operator-console-probe?baseUrl=<same-live-loopback-server>`，写出 `trashbot.pc_tools_workstation.o7_live_relay_browser_smoke_artifact.v1`。artifact 的 proof boundary 固定为 `software_proof_o7_live_relay_browser_smoke_artifact_only`，`endpoint_transport=live_loopback_http_socket`、`server_started=true`、`http_smoke_executed=true`；当前项目未配置可复验浏览器 runtime 时记录 `browser_smoke_status=not_run_http_only_minimum`，只声明 live HTTP minimum。固定 false 字段保持 `delivery_success=false`、`safe_to_control=false`、`route_execution_success=false`、`hil_pass=false`、`robot_control_executed=false`、`connects_cloud_production=false`；它不证明 production cloud、真实手机/browser 生产路径、4G/SIM、DB/queue、OSS/CDN、route execution、delivery/operator acceptance、HIL、safe-to-control 或任何机器人控制副作用。
- 2026-07-14 16:40 CST 起，O7 工作站新增真实 headless Chrome live relay smoke 命令：`cd pc-tools/workstation && npm run smoke:o7-live-relay-headless-browser -- --artifact ../../sprints/2026.07.14_16-40_o7_live_relay_headless_browser_smoke/artifacts/o7_live_relay_headless_browser_smoke.json`。该命令启动本机 `127.0.0.1:17002` 或下一可用 loopback Express server，再用本机 headless Chrome `--headless=new --dump-dom` 加载 `/api/health`、`/api/o7/operator-console` 和 `/api/o7/cloud-operator-console-probe?baseUrl=<same-live-loopback-server>`，从 Chrome DOM 中解析 JSON schema/status/false-field contract 并写出 `trashbot.pc_tools_workstation.o7_live_relay_headless_browser_smoke.v1`。artifact anchor 固定为 `proof_boundary=software_proof_o7_live_relay_headless_browser_smoke_only`、`artifact_status=headless_browser_smoke_ready_not_delivery_proof`、`endpoint_transport=live_loopback_http_socket`、`browser_runtime=headless_chrome`、`browser_smoke_status=live_headless_chrome_executed`、`server_started=true`、`http_smoke_executed=true`、`headless_browser_smoke_executed=true`；固定 false 字段保持 `delivery_success=false`、`safe_to_control=false`、`route_execution_success=false`、`hil_pass=false`、`robot_control_executed=false`、`connects_cloud_production=false`。若本机 Chrome 缺失或无法 headless 运行，该命令必须 fail closed，不能回退成 HTTP-only smoke；它仍不证明 production cloud、真实手机/browser production proof、delivery、route execution、HIL、safe-to-control 或 robot control。
- 2026-07-06 02:40 CST 起，PC 地图“太小”的历史默认值曾升级为：普通首页和 `/map` 默认 `300%` 可读大图，
  `完整态势` 回到 `100%`，`细节放大` 到 `1200%`。普通用户打开先看更大的地图、Nav2 路线、
  小车位置、雷达点和目标点；ROS2 配套仍分层：本地工程调试用 RViz2/Nav2 RViz 配置；远程浏览器观察用
  `foxglove_bridge` + Foxglove Web。工程观察只看 `/map`、`/scan`、TF、路径、定位和 costmap，
...
- 2026-07-06 05:07 CST 起，PC 地图“仍太小”的历史默认值曾升级为：普通首页和 `/map` 默认
...
- 2026-07-06 06:40 CST 起，地图显示曾改为“默认可读大图 + 一键完整态势”：普通首页和 `/map`
...
- 2026-07-06 07:26 CST 起，当前有效地图显示继续按现场“地图太小”反馈放大：普通首页和 `/map`
  默认 `800%` 大地图，`完整态势` 按钮回到 `100%` 全局视角，`细节放大` 最高 `4800%`。summary/live-summary
  合同同步为 `map_display_default_zoom_percent=800%`、`map_display_direct_map_default_zoom_percent=800%`、
  `map_display_fit_zoom_percent=100%`、`map_display_max_zoom_percent=4800%`。普通用户仍先用 PC 简易控制台和
...
- 2026-07-06 03:36 CST 起，固定相机 USB recovery 的产品状态不再把所有 0 字节无帧统称为
  `streamon_failed`。上车脚本会解析 `VIDIOC_STREAMON returned 0 (Success)`、`select timeout` 和输出字节数；
  PC 代理同步透传 `streamon_success_observed`、`select_timeout_observed`、`zero_byte_no_frame_observed`、
  `stream_status_summary`、`software_capture_exhausted`、`known_good_uvc_required` 和
  `camera_input_signal_check_required`。真实 7001 代理复验结果为
  `status=streamon_success_zero_byte_no_frame`、`usb_video_speed=480M`、
  `stream_failure_class=high_speed_zero_byte_no_frame`，YUYV/MJPG 都 STREAMON 成功但没有任何视频 buffer。
...
- 2026-07-06 02:29 CST 起，`live-summary` 直接平铺 WASD/手控 raw 命令证据：
  ...
  `streamon_success_zero_byte_no_frame/high_speed_zero_byte_no_frame`，`YUYV@320x240@20` 与
  `MJPG@480x320@30` 均 STREAMON 成功但 0 字节。
...
- 2026-07-04 04:58 CST 起，PC 相机共享预览诊断以最近一次真实 MJPEG 失败为准：如果 `/api/robot-control/camera/mjpeg`
  ...
  `source_first_frame_failed / uvc_no_frame_not_exclusive`，并显示 `camera_hardware_action_required=true`、
  `camera_hardware_action_label=检查摄像头输入/供电后复测`、`camera_input_signal_check_required=true`。这只修正普通 PC 页面
  对“没图”的解释，不启动独占采集、不启动 ROS2/Nav2/建图 runtime，不发送 manual、keyboard、free-roam、delivery、stop
  或 `/cmd_vel`。真实现场复验仍为 DV20 `/dev/video1`、USB `480M`、无人外部独占，但 MJPEG 首帧超时；因此 PC 端已准确提示
  输入信号/供电/线材/接口方向，实时图传是否真正出帧仍依赖摄像头硬件输出。
- 2026-07-04 05:25 CST 现场继续复核相机：停掉 `trashbot-local-webrtc-camera.service` 后，
  `/dev/video1` / `/dev/video2` 没有 owner；`media-ctl -d /dev/media1 -p` 显示 DV20 UVC 拓扑完整，
  `v4l2-ctl --all` 的 `Video input` 为 `ok`。直接用 `v4l2-ctl` 与 `ffmpeg` 逐一尝试
  `MJPG@640x480/1280x720/480x320/1920x1080@30`、`YUYV@320x240@20/25` 和 `YUYV@640x480@22`，
  再用 GStreamer 分别抓 `MJPG@640x480@30` 与 `YUYV@320x240@20`，全部能打开设备或协商 caps，
  但输出均为 `0` 字节/`0` 帧。随后运行 `onboard/scripts/camera_usb_recovery_smoke.py`，确认 USB 为
  `480M`、已 reauthorize、关闭 autosuspend、解绑同复合设备 audio 接口、`uvcvideo quirks=0`，但
  `YUYV@320x240@20` 和 `MJPG@480x320@30` STREAMON 仍超时 0 字节，返回
  `stream_failure_class=high_speed_zero_byte_no_frame`。当前结论是 PC 页面、多浏览器共享预览、OpenCV、
  MJPEG 编码、低带宽格式和 USB 恢复都已排除；剩余动作是检查 DV20 上游视频输入、摄像头/采集卡、
  视频线、接口或供电，或换 known-good UVC 后再复测。
- 2026-07-04 05:36 CST 起，WAVE ROVER 主类型口径按本地 vendor 资料纠正为
  `main_type=1,module_type=0`：`docs/vendor/waveshare_wave_rover/WAVE_ROVER_V0.9/json_cmd.h`
  写明 `1-WAVE ROVER, 2-UGV02, 3-UGV01`，`ugv_config.h` 默认 `mainType=1`。当前现场仍能看到历史手工进程参数
  `main_type:=2,module_type:=0`，这只解释旧 A/B 复测背景，不再作为推荐运行状态；下一次重启/deploy
...
- 2026-07-04 05:58 CST 起，自由移动/自动扫图 runtime 不再用节点进程年龄判断本次会话时长。
  `free_roam_autonomy_node` 在 PC start 写入 `operator_confirmed=true` 或 `mapping_active=true` 且没有外部
  stop 时重置会话计时，stop/locked 时 `elapsed_s=0`，避免常驻节点一启动就因为超过 `max_runtime_s`
  直接 `completed` 并只发零速。现场已清掉两个历史同名 `/free_roam_autonomy` 节点，只保留一个更新后的节点；
  PC start 读回 `autonomy_forwarded`、`start_runtime_wait.ok=true`、`decision_state=avoiding`、
  `cmd_vel_publish_enabled=true`、`motion_ready=true`、`motion_without_radar_allowed=true`。当前雷达近障碍约
  `0.04m`，策略按避障原地换向；WAVE ROVER command debug 写出非零 `T=11 L=164,R=-164` /
  `L=-164,R=164`，依据 `docs/vendor/VENDOR_INDEX.md` 中 WAVE ROVER `T=11` PWM 指令口径。feedback-samples
  仍为 vendor `T=1001 L/R=0/0`，但 `imu_attitude_delta_observed=true`，因此 wheel raw 继续作为反馈闭环风险单列，
  不把 IMU 或命令 raw 冒充 wheel 非零。
- 2026-07-04 06:14 CST 起，PC `live-summary` 的普通用户状态不再让 wheel raw `T=1001 L/R=0/0`
  独占主卡点：如果同轮 readback 已证明 `command_raw_lr_nonzero_proven=true` 且
  `motion_signal_observed=true`，或键盘/自由移动 motion 已证明，summary 返回 `ready_for_motion`，下一步提示直接使用
  WASD、自由移动或图上路线继续跑。`needs_same_window_wheel_rerun=true` 与 `wheel_lr_nonzero_proven=false`
  仍保留为反馈诊断风险，不能宣称 wheel raw 非零、完整 Nav2 route execution 或 delivery success。
  地图太小的现场答案保持 05:19 的 `800%` PC 大地图和 `/map` 满屏；ROS2 配套为本地 RViz2/Nav2 RViz 配置、
  远程 Foxglove bridge + Foxglove Web，只观察 `/map`、`/scan`、TF、路径、定位和 costmap，不替代普通简易控制台。
  相机本轮直接复核仍是 DV20 `/dev/video1`、USB `480M`、无人独占但多格式 0 字节无帧；图传缺口不重新阻塞移动入口。
- 2026-07-04 07:05 CST 起，上位机开机恢复链路改为 systemd 管理：`trashbot-esp32-bridge.service`
  负责 `/cmd_vel -> esp32_bridge -> WAVE ROVER HTTP`，`trashbot-lidar-lifecycle.service` 负责 `/scan`
  雷达流。bridge wrapper 启动前会清理脱管同名进程，避免两个 `/esp32_bridge` 同时订阅 `/cmd_vel`；
  参数来源写明采用 `docs/vendor/VENDOR_INDEX.md` 指向的 WAVE ROVER `json_cmd.h` 与 `ugv_config.h`，
  即 `main_type=1,module_type=0` 和 `T=11` PWM164。现场复验两个 service 均 `enabled/active`，
  ROS 图中只有一个 bridge 订阅 `/cmd_vel`，`/scan` 可读 LaserScan，PC live-summary 仍为
  `ready_for_motion` 且地图、Nav2 路线、目标点、小车位置和雷达贴图可见。重启后 CMA 已恢复为
  `cma_available_no_recent_failure`，但 DV20 仍 `first_frame_total_timeout / uvc_no_frame_not_exclusive`；
  这说明当前图传 blocker 已从内存不足下沉为输入信号、供电、线材、接口或采集卡/摄像头本体。
...
- 2026-07-04 02:18 CST 起，`/api/status` 返回的内嵌 `operator_report.structured_hil_claims.delivery_success=true`
  也按人工材料处理，不再让 `GET /api/robot-control/summary` 进入 `blocked`；该豁免只限
  `operator_report.structured_hil_claims.delivery_success` 和 `latest_result.operator_report.structured_hil_claims.delivery_success`。
  顶层 `delivery_success=true`、非 operator report 的 `structured_hil_claims.delivery_success=true`、`hil_pass=true`
  等仍按危险 true 字段阻断。现场复验为：PC 7001 summary `connection=readable`、`blocked=0`，
  `delivery/complete` 与 `delivery/latest` 均返回 `delivery_success=true`；route acceptance packet 显示
  `delivery_success=true`，但全局 summary 顶层继续 fail-closed 为 `delivery_success=false`。
- 2026-07-04 02:35 CST 起，PC 首页和 `/map` 直达大屏默认缩放改为 `150%` 可读大图；
  `map_display_default_zoom_percent=150%`、`map_display_direct_map_default_zoom_percent=150%`，
...
- 2026-07-04 03:20 CST 起，PC 地图“太小”的历史口径曾升级为默认 `200%` 大图：
  `map_display_default_zoom_percent=200%`、`map_display_direct_map_default_zoom_percent=200%`，
  `map_display_fit_zoom_percent=100%`、`map_display_max_zoom_percent=1200%`。普通首页地图面板进一步扩大为
  `height: clamp(1080px, calc(100vh + 96px), 2000px)`，大图画布最小高度为
  `clamp(900px, calc(100vh - 56px), 1700px)`，顶栏入口显示 `地图大屏 /map`。ROS2 配套答案仍分层：
  ...
  不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-07-04 04:05 CST 起，PC 地图“太小”的历史默认值继续提升为 `300%`：
  `map_display_default_zoom_percent=300%`、`map_display_direct_map_default_zoom_percent=300%`，
...
- 2026-07-04 03:34 CST 现场复核当前 PC 三主功能：地图链路已读到
  ...
  `radar_overlay_status=loaded / radar_overlay_current_point_count=70`。WASD/方向键当前可运行链路为 PC 固定
  `/api/robot-control/base/manual` 的 `command_mode=ros`，上车 `/cmd_vel` 由 `esp32_bridge` 映射到
  `command_transport=http`、`command_mode=pwm`、ESP32 `http://192.168.1.3`、vendor `T=11 L/R=±255`；
  PC 前进/后退短脉冲均返回 `command_forwarded`、`command_result_ok=true`、`stop_result_ok=true`、
  `command_raw_lr_nonzero_proven=true`、`motion_signal_observed=true`，live-summary 同轮为
  `keyboard_motion_verified=true`、`keyboard_stop_settled_after_pulse=true`。但 `T=1001 L/R` 仍为 `0/0`，
  `keyboard_wheel_feedback_lr_nonzero_proven=false`，不能把 IMU/姿态扰动升级成 wheel raw 非零。
  相机链路经 PC MJPEG、首帧探针、USB recovery、停止服务后的独占 `v4l2-ctl` 30 秒直采复核仍无帧：
  DV20 位于 480M USB、`/dev/video1` 是 UVC Video Capture、`/dev/video2` 是 metadata、无人占用；
  `YUYV@320x240` 与 `MJPG@640x480` 独占 30 秒均 0 字节，PC status 为
  `source_first_frame_failed / uvc_no_frame_not_exclusive`。结论是 PC 页面、多人共享预览、短超时和浏览器独占已排除，
  剩余动作是检查 DV20 输入信号/摄像头/线材/供电或换 known-good UVC 后复测。
...
- 2026-07-06 09:21 CST 起，相机 USB 恢复代理新增 `reload_uvc_module` 布尔白名单。PC 7001 只接受 `/dev/videoN` 与固定布尔开关，上位机 8787 只在该开关为 `true` 时追加 `--reload-uvc-module`，恢复脚本内部固定执行 `modprobe -r uvcvideo` 与 `modprobe uvcvideo quirks=0 nodrop=0 timeout=5000`，并回传 `uvc_module_reload_requested/attempted/ok`、`uvc_module_parameters_after_reload` 和完整 `uvc_module_reload` 证据；代理超时放宽到 96 秒以容纳模块重载和六组 STREAMON。真实 PC 复验 `POST /api/robot-control/camera/usb-recovery` 返回 `uvc_module_reload_ok=true`、模块参数 `quirks=0/nodrop=0/timeout=5000`，但仍 `status=streamon_success_zero_byte_no_frame`、`frame_observed=false`、USB `480M`、`software_capture_exhausted=true`、`known_good_uvc_required=true`。结论是页面独占、低速 USB、quirk、V4L2 控制项、mmap/userptr/no-query 和 uvcvideo 模块状态漂移均已排除，实时图传剩余风险继续指向 DV20 输入信号、视频线/接口/供电、采集设备本体或 known-good UVC 对照；该恢复入口仍固定 `publishes_cmd_vel=false`、`opens_base_uart=false`、`sends_motion_commands=false`。
- 2026-07-07 02:18 CST 起，相机 USB 恢复代理新增 `usbreset_device` 布尔白名单，并把普通页面的一次性自动 USB recovery 改为携带该开关。上位机只从 sysfs 当前相机 USB 设备读取 `busnum/devnum` 后执行固定 `usbreset BBB/DDD`，PC body 不能指定 bus/dev、VID/PID、产品名或 shell 参数；回包顶层暴露 `usbreset_requested`、`usbreset_attempted`、`usbreset_ok` 和完整 `usbreset` 证据。真实 PC 复验中，`usbreset_device=true` 后 recovery 返回 `frame_observed=true`、`stream_failure_class=none`、`software_capture_exhausted=false`、`known_good_uvc_required=false`；随后 PC MJPEG 入口 8 秒收到 `4056284` 字节 multipart JPEG/JFIF，status 为 `streaming / first_frame_observed / shared_preview_current_frame_visible=true`。当前结论修正为：DV20 曾进入 USB 设备级卡死状态，设备级 reset 可恢复真实图传；普通用户无需额外勾选或手工 SSH，打开页面后若遇到同类 UVC 无帧会自动做一次相机-only usbreset，仍固定不发 `/cmd_vel`、不打开 WAVE ROVER UART、不启动 Nav2/manual/keyboard/free-roam/delivery。
- 2026-07-03 22:41 CST 起，PC 相机读回额外暴露输入信号处理合同：`camera_input_signal_check_required`、`camera_input_signal_check_label` 和 `camera_input_signal_check_plain`。该字段只在已排除页面独占、USB 非 `12M` 低速、没有当前 UVC 传输错误、但共享 MJPEG/V4L2 仍没有首帧时点亮，现场 7001 当前值为 `true / 检查摄像头输入信号/供电后复测`。同轮停掉相机服务后直连 `/dev/video1`：`v4l2-ctl` 抓 `MJPG@640x480` 与 `YUYV@320x240` 均 `select timeout` 且输出 0 字节；`ffmpeg` 抓 MJPG/YUYV 10 秒也没有任何帧。结论继续是摄像头/采集卡输入信号、视频线、接口或供电方向，而不是 PC 页面、共享预览、OpenCV 单点或低速 USB；该字段不阻塞地图、WASD、自由移动或 Nav2 控制入口。
- 2026-07-03 22:33 CST 起，上车共享相机和 PC `GET /api/robot-control/camera/mjpeg/status` 把 `opencv_capture_not_opened` 也纳入首帧失败集合；即使 `/api/camera/health` 暂时仍显示 `ready/source_selected_not_probed/source_failure_reason=none`，只要 `media_diagnostics.last_offer_error.failure_reason=opencv_capture_not_opened`，summary 和 MJPEG status 都必须提升为 `source_first_frame_failed`、`source_diagnosis_status=uvc_no_frame_not_exclusive`、`source_diagnosis_not_exclusive=true`、`camera_hardware_action_required=true`、`camera_hardware_action_label=检查摄像头输入/供电后复测`。现场 7001 读回确认当前 DV20 UVC 仍无真实首帧，USB 为 `480M`、`source_usage_owner_count=0`、`exclusive_camera_claim=false`，所以缺口继续是摄像头输入/线/口/供电或 known-good UVC 复测；该状态不阻塞地图、WASD、自由移动或 Nav2 控制入口。
- 2026-07-03 23:25 CST 起，普通首屏把“打开即用、无可见安全勾选框”写成可验收 DOM 合同：`.simple-user-console` 暴露 `data-open-page-motion-ready=true`、`data-open-page-no-visible-safety-checkbox=true`、`data-open-page-safety-mode=site_safe_by_default` 和 `data-visible-safety-checkbox-count=0`，移动/导航卡同步暴露 `data-open-page-motion-ready=true`。用户不需要再勾选任何 safety checkbox；旧兼容 input 仍为隐藏且固定 checked，取消时会恢复为 true。该变化只减少普通 PC 操作摩擦，不放宽 PC Node 固定代理里的速度/时长/目标范围/dangerous true 字段护栏。
- 2026-07-03 22:07 CST 起，PC `GET /api/robot-control/summary` 顶层直接暴露当前地图画面的关键 WYSIWYG alias：`map_preview_status`、`path_preview_point_count`、`route_target_visible`、`route_target_source`、`route_target_state` 和 `robot_pose_status`，并与 `readback_summary.map` 同源。现场一条 summary now 能直接看到地图画面 loaded、路线点数、目标点来源和小车位姿，不必再额外钻嵌套字段或单独调 map preview。该 alias 只读当前 map preview/readback，不启动雷达 lifecycle、Nav2、manual、keyboard、free-roam、建图 runtime、delivery、stop 或 `/cmd_vel`。
- 2026-07-03 20:57 CST 起，PC `GET /api/robot-control/camera/mjpeg/status` 在保留 `selected_path` / `selected_name` 的同时新增普通 alias `selected_device` / `selected_device_label`。现场当前值为 `/dev/video1` 与 `USB Composite Device: DV20 USB (usb-5310000.usb-1)`，并继续返回 `source_diagnosis_status=uvc_transport_error_not_exclusive`、`source_failure_reason=first_frame_total_timeout`、`source_usage_owner_count=0`、`exclusive_camera_claim=false` 和 `camera_hardware_action_required=true`。这只让打开页面和现场脚本直接知道正在看的摄像头源，不启动独占采集、不发送 free-roam/Nav2/建图/delivery 或 `/cmd_vel`；实时图传是否可见仍以真实 MJPEG 首帧为准。本轮为验证 PC 前进/后退目标执行了短 manual `forward` / `back` 脉冲并 stop 收口，二者均返回 command/stop OK 和 IMU 运动信号。
- 2026-07-03 21:55 CST 起，PC `POST /api/robot-control/camera/first-frame/probe` 除 `probe_key_values` 外，还顶层返回压缩后的 `probe_payload`、`fallback_attempts`、`auto_format_fallback`、`low_bandwidth_fallback_attempted` 和 `low_bandwidth_fallback_min_size`。这些字段只来自上位机白名单探针，不接受浏览器任意设备或命令；用途是让现场一条 curl 直接确认 320x240/160x120 低带宽是否已经跑过。当前实板 USB recovery 后确认 DV20 在 `480M` 高速 USB，但 `YUYV@320x240@20` 与 `MJPG@480x320@30` 仍 0 字节无帧，所以 PC 必须继续显示“不是页面独占，检查摄像头输入/线/供电或换 known-good UVC 后复测”，同时不阻塞地图、WASD、自由移动和 Nav2 运动入口。
- 2026-07-03 19:56 CST 现场复核当前 PC 功能状态：本机 PC Node 继续监听 `0.0.0.0:7001`，默认上位机为 `http://192.168.1.11:8787`；上位机 `trashbot-upper-robot-api.service` 与 `trashbot-local-webrtc-camera.service` 均 active。地图大屏读回已满足当前四层显示：`map_current_visible=true`、`path_preview_point_count=18`、`robot_pose` 来自 `/amcl_pose`、`radar_overlay_status=loaded` 且当前雷达点 `191` 个、`route_target_visible=true` 且目标点为 path 末点 `(0.8,0.05,map)`。PC 固定 manual 代理短脉冲验证 `forward` 与 `back` 均 `command_forwarded`、`base_command_mode=ros`、`command_result_ok=true`、`stop_result_ok=true`、`motion_signal_observed=true`，松开 stop 代理也 `command_forwarded`；但 `wheel_feedback_lr_nonzero_proven=false`、`wheel_feedback_latest_raw_left/right=0/0` 仍不能宣称 wheel raw 非零。相机 `/mjpeg` 和 USB recovery 后仍为 `first_frame_total_timeout`、`high_speed_zero_byte_no_frame`，当前图传缺口不阻塞地图、键盘手控、自由移动或 Nav2 路线执行。
- 2026-07-03 19:46 CST 起，PC summary 接受当前浏览器键盘按住窗口的本地证据 query：`keyboard_best_continuous_pulse_count`、`keyboard_verified_min_forwarded_pulses`、`keyboard_stop_settled_after_pulse`、`keyboard_current_direction` 等字段。Vue 每次刷新 `GET /api/robot-control/summary` 时带上本地 WASD/方向键 hold 证据；Node 只在连续转发 pulse 数达到阈值、松开后的 stop 已转发成功，且同轮后端读回或最近 manual 回包已有运动信号时，才把 `keyboard_motion_verified=true`、`keyboard_stop_settled_after_pulse=true` 和 `keyboard_continuous_motion_verified=true` 写入 live closure。没有本地证据或只有单独 stop 时仍保持 false，不能用旧只读材料冒充“松开即停”完成。该链路只修正 summary 验收读数，不启动 manual/keyboard/free-roam/Nav2/建图 runtime/delivery/stop 或 `/cmd_vel`。
- 2026-07-03 19:36 CST 现场复核相机：停止 `trashbot-local-webrtc-camera.service` 后确认 `/dev/video1`/`/dev/video2` 无 owner，直连 `v4l2-ctl` 对 DV20 `MJPG@640x480@30` 与 `YUYV@320x240` 能 `STREAMON` 但 10 秒内 0 字节无帧；恢复 8088 后 `/api/camera/mjpeg/status` 仍为 `source_first_frame_failed`、`source_diagnosis_status=uvc_no_frame_not_exclusive`、`exclusive_camera_claim=false`、`source_usage_owner_count=0`。因此当前图传缺口继续归因为 DV20/UVC 源头无视频 buffer，不是 PC 页面独占、多人预览独占或服务未重启；剩余动作是检查摄像头输入/线/接口/供电或换 known-good UVC 后复测。该结论不阻塞地图、键盘手控、自由移动或 Nav2 路线执行。
- 2026-07-04 02:35 CST 起，地图太小的历史口径曾为：PC 首页和 `/map` 直达大屏默认使用 `150%` 可读大图显示真实地图、Nav2 路线、小车位置、雷达点和目标点；当时有效口径曾收敛为默认 `100%` 完整态势、`细节放大` 到 `4800%`。`/map` 直达页必须使用整屏 flex 地图布局，只保留缩放、`刷新地图画面`、雷达贴图只读刷新和 `工程观察`，并收起建图、保存、地图列表、普通说明和非地图卡片；DOM 必须暴露 `data-direct-map-view-visible-controls=zoom,map_refresh,radar_refresh,ros2_observe_toggle`、`data-direct-map-view-hides-map-lifecycle-actions=true` 和 `data-direct-map-view-hides-non-map-cards=true`。地图卡工具行保留 `工程观察` 展开按钮，但它只展开 RViz2/Foxglove 观察说明和命令，固定 `data-sends-motion-when-clicked=false`、`data-starts-ros2=false`、`data-starts-rviz2=false`、`data-starts-foxglove=false`、`data-starts-nav2=false` 和 `data-starts-map-runtime=false`。ROS2 配套答案保持分层：本地工程调试用 `ros2 launch ros2_trashbot_bringup rviz.launch.py`；远程浏览器观察用 `ros2 launch ros2_trashbot_bringup foxglove_bridge.launch.py` + Foxglove Web 连接 `ws://192.168.1.11:8765`；普通用户界面不被替代。
- 2026-07-03 16:14 CST 起，普通首页再按现场反馈收紧：`1600px` 及以下 viewport 不再保留右侧固定栏，`.robot-console-grid[data-layout="visual-first"]` 改为地图整行优先；`900px-1600px` 时第二行把图传和 WASD 并排，`900px` 以下纵向排列。这样 1440x900 PC 首页地图画布从约 `1018x756` 提升到接近整页宽度，仍保留 `/map` 纯大屏、共享图传、连续键盘手控和 `工程观察`；该断点只改 CSS 显示，不启动 ROS2/RViz2/Foxglove/Nav2/建图 runtime，不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-07-03 16:57 CST 起，PC summary 把 `source_first_frame_failed + uvc_no_frame_not_exclusive` 提升为普通用户可理解的相机处理动作：`camera_hardware_action_required=true`、`camera_hardware_action_label=检查摄像头输入/供电后复测`，但同时固定 `camera_blocks_free_move=false` 和 `free_move_without_camera_allowed=true`。现场复测已确认当前 DV20 UVC 位于 `480M` USB 高速口、owner_count=0、共享预览 `exclusive_camera_claim=false`，短首帧探针仍 `probe_total_timeout`，所以 PC 不再把问题说成浏览器独占，也不再只提示反复点“复测相机首帧”。普通首屏文案必须避免“硬件/速度/时长”等工程词，改用“设备/画面处理、速率、持续”等口径；高级 data 字段名保持兼容。该变化只改只读诊断和 UI 文案，不启动独占相机、不启动 ROS2/RViz2/Foxglove/Nav2/建图 runtime，不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-07-03 17:45 CST 起，PC 增加固定相机 USB 恢复代理 `POST /api/robot-control/camera/usb-recovery`，上位机对应 `POST /api/camera/usb-recovery`，内部只运行 `onboard/scripts/camera_usb_recovery_smoke.py` 的白名单恢复流程：重启 `trashbot-local-webrtc-camera.service`、按需 USB reauthorize、解绑同一复合设备的 USB audio 接口，再做 `YUYV@320x240@20` 与 `MJPG@480x320@30` 直接 STREAMON smoke。浏览器 body 只能选择 `/dev/videoN` 与三个 skip 开关，不能传任意命令；回包固定 `sends_motion_when_clicked=false`、`publishes_cmd_vel=false`、`opens_base_uart=false`、`starts_nav2=false`、`starts_keyboard=false` 和 `robot_control_executed=false`。现场 PC 代理 live 结果为 `status=streamon_failed`、`usb_video_speed=480M`、`stream_failure_class=high_speed_zero_byte_no_frame`、两种格式均 0 字节无帧，说明问题已从“页面独占/低速 USB”收窄到摄像头输入、线/口/供电或设备本体；下一步是换线/换口/供电 Hub 或 known-good UVC 后再复测。
- 2026-07-03 17:56 CST 起，上位机共享 MJPEG `/api/camera/mjpeg` 的自动首帧预算收紧到约 `5s`：单次格式尝试 `0.75s`，第一段关键格式 `3.8s`，index/V4L2 打开兜底 `1.2s`。该入口定位为普通 PC 首屏实时预览的短路通道；没有真实帧时必须快速返回结构化 503/PC 502，并把 `first_frame_total_timeout`、`uvc_no_frame_not_exclusive` 和 “检查 USB、摄像头输入或供电，必要时换 known-good UVC 复测” 写回 `/api/robot-control/camera/mjpeg/status`。完整相机排障仍走 `POST /api/robot-control/camera/first-frame/probe` 或 `POST /api/robot-control/camera/usb-recovery`，不允许 MJPEG `<img>` 长时间悬空，也不发送黑帧或 placeholder。
- 2026-07-03 18:16 CST 起，上位机共享 MJPEG 的首帧尝试补齐最低带宽兜底：`MJPG@160x120@30` 和 `YUYV@160x120@20` 必须排在 `default@current`、index 和 CAP_V4L2 兜底之前，避免真实摄像头只在极小帧下吐首帧时被软件跳过。现场已把更新部署到 `trashbot-local-webrtc-camera.service`，PC 7001 读回的 `last_first_frame_format_attempts_summary` 包含 `MJPG@160x120@30(/dev/video1)` 与 `YUYV@160x120@20(/dev/video1)`，但 DV20 仍返回 `first_frame_total_timeout`、所有 STREAMON 0 字节；因此共享预览软件链路已经尽力尝试低带宽真帧，剩余动作仍是检查摄像头输入、USB 线/接口/供电或换 known-good UVC 复测。该兜底只读 camera，不启动独占采集、ROS2/RViz2/Foxglove/Nav2、manual、keyboard、free-roam、建图 runtime、delivery、stop 或 `/cmd_vel`。
- 2026-07-03 18:29 CST 起，PC `/api/robot-control/camera/mjpeg/status` 与相机首帧 probe 代理把 `uvc_no_frame_not_exclusive` 也升级为硬件处理动作：当 USB 已是 `480M`、`source_usage_not_exclusive=true`、但共享 MJPEG / V4L2 / ffmpeg / GStreamer 都没有视频 buffer 时，直连 status 返回 `camera_hardware_action_required=true`、`camera_hardware_action_label=检查摄像头输入/供电后复测`、`camera_reprobe_after_hardware_action_required=true`，并继续固定 `camera_blocks_free_move=false`。现场同轮把 `uvcvideo quirks` 从临时 `4294967295` 复位到 `0` 后仍 0 字节，说明问题不只是 quirk 参数；普通页不得再把这类状态只显示成“复测相机首帧”。低速手控复验继续读到 PC -> `/cmd_vel` -> bridge -> HTTP -> WAVE ROVER `T=11,L/R=255` 与 stop，协议依据仍来自 `docs/vendor/VENDOR_INDEX.md` 指向的 WAVE ROVER JSON `T=11` PWM 命令。
- 2026-07-03 19:03 CST 起，`GET /api/robot-control/summary` 的 `readback_summary.camera` 也同步暴露 `camera_hardware_action_required`、`camera_hardware_action_label` 和 `camera_reprobe_after_hardware_action_required`。现场脚本现在只看 camera readback 就能得到同一个结论：共享 MJPEG 是单上游多人入口、`exclusive_camera_claim=false`，当前 DV20 UVC 首帧失败需要检查摄像头输入/供电后复测；不用再把 summary 顶层、live closure 和 MJPEG status 三处字段手工拼起来。该变化只补只读诊断和高级 DOM，不启动相机独占采集、不启动 ROS2/RViz2/Foxglove/Nav2/manual/keyboard/free-roam/建图 runtime/delivery/stop，也不发送 `/cmd_vel`。
- 2026-07-03 19:12 CST 起，PC 普通首页和高级区保留的旧安全确认 input 只作为测试/现场脚本兼容节点存在，统一使用 `.plain-hidden-safety-input { display: none !important; }` 从页面布局中移除。现场安全默认已确认，用户不再看到也不需要勾选 safety checkbox；这些 input 仍保持 checked/aria-hidden，取消勾选会被页面逻辑恢复为 true，且不会触发 Nav2/manual/keyboard/free-roam/建图 runtime/delivery/stop 或 `/cmd_vel`。
- 2026-07-03 19:22 CST 起，上位机 8088 共享 MJPEG 首屏首帧短预算补入当前 DV20 native 模式：`MJPG@1280x720@30` 排在 `MJPG@640x480@30` 后、低带宽兜底前；单次尝试收紧为 `0.6s`，保证 `640 MJPG -> 1280 native MJPG -> 480 MJPG -> 320 YUYV -> 160 MJPG -> 160 YUYV` 能在主预算内完成。现场已部署到 `trashbot-local-webrtc-camera.service` 并复测 `/api/camera/mjpeg`：USB 仍为 `480M`、`owner_count=0`、`exclusive_camera_claim=false`，上述六种模式全部无首帧，`POST /api/robot-control/camera/usb-recovery` 后仍 `streamon_failed/high_speed_zero_byte_no_frame`。因此当前实时图传缺口不再是页面独占、未试 native 格式或服务未重启；剩余动作仍是检查摄像头输入/供电、线/接口，或换 known-good UVC 后复测。该变化只打开相机读真实帧，不发送 manual、keyboard、free-roam、Nav2、delivery、stop 或 `/cmd_vel`。
- 2026-07-03 18:36 CST 起，PC 手控代理 `/api/robot-control/base/manual` 和 `/api/robot-control/base/first-jog` 把本次点动窗口的关键事实提升到顶层：`base_command_mode`、`feedback_mode`、`command_result_ok`、`stop_result_ok`、`wheel_feedback_lr_nonzero_proven`、`wheel_feedback_latest_raw_left/right`、`imu_attitude_delta_observed`、`motion_signal_observed`、`motion_signal_source` 和 T1001 帧数。`remote_motion_key_values` 仍保留完整材料；顶层 alias 只减少现场脚本解析成本，不改变 `safe_to_control=false`、`delivery_success=false` 或 `robot_control_executed=false`。现场 live 后退脉冲已返回 `base_command_mode=ros`、`feedback_mode=realtime`、`command_result_ok=true`、`stop_result_ok=true`、`motion_signal_observed=true`、`wheel_feedback_lr_nonzero_proven=false`，同时上位机命令日志新增 vendor `T=11,L=-255,R=-255` 与 stop `T=11,L=0,R=0`。
- 2026-07-03 15:23 CST 起，普通首页地图再按“大画布优先”收紧：`.robot-console-grid[data-layout="visual-first"] .plain-map-panel` 使用 `height: clamp(760px, calc(100vh - 24px), 1280px)` 与 `min-height: 760px`，内层 `plain-map-viewport[data-size=large]` / `plain-map-layer` 使用 `min-height: clamp(620px, calc(100vh - 176px), 980px)`。这只是把 PC 首页地图从“预览卡”抬成主画布；`/map` 直达页、缩放、RViz2/Foxglove 只读观察入口和 no-motion 边界保持不变。
- 2026-07-02 CST 补强：`/map` 直达页默认 `observer` 模式不能把 `plain-map-ros2-tool-note` 一并隐藏；样式必须显式恢复 `.shell[data-direct-map-view-requested="true"] .plain-map-panel[data-observer-mode="true"] > .plain-map-ros2-tool-note { display: block; }`，确保现场点击 `工程观察` 后能看到 RViz2/Foxglove 只读观察说明。该入口仍不启动 ROS2/RViz2/Foxglove/Nav2/建图 runtime，不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-07-02 CST 起，PC 键盘连续手控面板必须明确普通用户焦点合同：页面打开、summary 到达或窗口重新获得焦点后自动准备键盘，但准备本身不发车；DOM 暴露 `data-keyboard-event-scope=page_non_editable`、`data-keyboard-auto-arm-on-load=true`、`data-keyboard-click-to-arm-required=false`、`data-keyboard-auto-focus-after-arm=true`、`data-keyboard-editable-fields-block-motion=true`、`data-keyboard-global-listener-owner-required=true` 和 `data-keyboard-input-fields-safe=true`；可见说明固定为“键盘生效区：打开页面后自动准备；本页非输入区按住 W/A/S/D 或方向键才动；输入框里按键只输入文字，不发车。”`safe_command_boundary.keyboard_control_next_action`、`readback_summary.keyboard.next_action_plain` 和 `current_keyboard_control_pack_next_action_plain` 必须同源输出“页面自动准备/准备本身不发车/按住才移动”口径。备用启用按钮仍固定 `data-sends-motion-when-clicked=false`，只有按住方向键/WASD 才通过固定 `/api/robot-control/base/manual` 发 ROS `/cmd_vel` 低速 pulse，松开/失焦/切页/进入输入框都必须停止或解除键盘所有权。
- 2026-07-03 01:35 CST 起，PC 键盘和低速点动的用户控制面仍固定为 `command_mode=ros`：PC Node 代理在省略模式时写入 `ros`，上车端优先使用进程内 `rclpy` publisher 向 `/cmd_vel` 连续 burst 发布，等待至少一个订阅者；只有 ROS Python 环境不可用或发布失败时才回退到带 `--wait-matching-subscriptions` 的 `ros2 topic pub` burst。底层 `esp32_bridge` 当前默认改为 `command_mode=pwm`、`pwm_min_abs/max_abs=164`，即继续订阅 `/cmd_vel`，但落到 WAVE ROVER 时用 vendor `T=11` PWM164；该口径来自 `docs/vendor/VENDOR_INDEX.md` 指向的 `json_cmd.h`：`T=13` 标注 `(Not for the products without encoders)`，且 2026-07-03 真机 smoke 已观察到 `T=13` 路径轮速 `L/R=0/0`，而 `T=11/PWM164` 同窗口产生 IMU 姿态变化。PC summary 因此可把 `motion_signal_observed=true` / `imu_attitude_delta_observed=true` 作为键盘“确有运动信号”的验收材料，同时继续单独展示 wheel raw `L/R=0/0` 与 `wheel_feedback_lr_nonzero_proven=false`，不把 IMU 姿态变化伪装成编码器轮速。
- 2026-07-03 04:05 CST 起，`esp32_bridge` 支持运行中用 ROS 参数调 `command_mode`、`pwm_min_abs`、`pwm_max_abs`、`max_wheel_speed_mps` 等映射参数，非法 PWM 范围会被拒绝，合法调参会写入 `wave_rover_command_debug.jsonl` 且不发送运动命令。现场已上车验证：`pwm_min_abs/max_abs=220`、`255` 时 PC first-jog 均能把 `/cmd_vel` 映射成 vendor `T=11,L/R=220` 或 `255` 并自动 stop；切到 `speed` 时映射为 `T=1,L/R=0.061538`，切到 `ros` 时映射为 `T=13,X=0.08,Z=0`。四组对照均成功到达 UART，但 `T=1001.L/R` 仍保持 `0/0`、wheel 非零未证明。当前最新结论是：PC/WASD/Robot API/ROS `/cmd_vel`/UART JSON 链路已通，底盘不动或 wheel raw 不回填的主嫌疑转为电机使能、底盘模式、下位机固件状态或反馈链路；PC 不得把这些命令到达证据升级成 delivery success、Nav2 成功或 wheel L/R 非零。
- 2026-07-03 04:22 CST 起，上位机 `/api/base/manual` 会把 fresh `esp32_bridge` feedback debug JSONL 中的 vendor `T=1001` 帧作为本次 ROS 手控 `feedback_during_motion` 返回给 PC，PC Node `remote_motion_key_values` 也必须优先读取这个顶层字段，旧 `serial_motion_transaction.feedback_during_motion` 只作为兼容 fallback。PC first-jog/WASD 因此能直接显示执行窗口 T1001 帧数、latest raw `L/R` 和 IMU 姿态变化，而不是在串口由 bridge 持有时误显示 `feedback_during_motion_t1001_frame_count=0`。该变化只修正读回证据链：`wheel_feedback_lr_nonzero_proven` 仍必须由同一 T1001 帧内 `L/R` 非零决定，`delivery_success=false`、`safe_to_control=false`、`hil_pass=false` 不变。
- 2026-07-03 15:23 CST 起，PC Node `baseManualMotionKeyValues()` 必须把上位机 `/api/base/manual` 顶层 `motion_signal_observed`、`motion_signal_source`、`imu_attitude_delta_observed` 和 `manual_imu_attitude_delta_summary.max_abs_roll_delta/max_abs_pitch_delta` 抬到 `remote_motion_key_values`。`buildMotionEvidenceGaps()` 在这些同窗口 IMU/车身运动信号为 true 时，不再保留 `physical_motion_lidar_delta_not_proven`；但 `wheel_feedback_lr_nonzero_not_proven` 仍只由 vendor `T=1001 L/R` 非零清除。该变化用于回答“小车动不动不依赖雷达”，不得把 IMU 姿态变化伪装成 wheel raw L/R、Nav2 完整路线执行或 delivery success。
- 2026-07-03 17:27 CST 起，PC Node `baseManualMotionKeyValues()` 还必须把上位机 `/api/base/manual` 顶层 `base_command_mode`、`feedback_mode`、`command_result.ok` 和 `stop_result.ok` 抬到 `remote_motion_key_values`。这样 PC WASD/低速点动回包可直接显示本次走 `ros`、`speed` 或 `pwm` 链路，以及 manual command 与 auto-stop 是否被上车端确认；现场仍必须把 vendor `T=1001 L/R` 非零、Nav2 完整路线和 delivery success 分开验收。协议依据采用 `docs/vendor/VENDOR_INDEX.md` 指向的 WAVE ROVER UART JSON：`T=11` 为 direct PWM input，`T=13` 为 ROS control 且需硬件确认，当前 PC 默认仍用 ROS `/cmd_vel` 入口，由上车 bridge 再映射到底盘兼容链路。
- 2026-07-03 06:27 CST 起，PC Nav2 图上路线执行也按“打开即用”口径收口：`POST /api/robot-control/nav2/goal/execute` 会在固定代理内自动写入上车兼容 `confirm_navigation_execution=true`，普通用户和现场脚本不再需要传确认字段，首屏 blocking requirements 只保留 `goal_limits` 与 `hard_dangerous_true_fields`。现场复测已手动激活 `bt_navigator`、`behavior_server`、`waypoint_follower`、`velocity_smoother`，随后通过 PC 代理执行目标 `(0.8,0.05,0)`，返回 `goal_accepted=true`、`result_received=true`、`result_status=succeeded`，并由 PC stop 代理收口。该次 route succeeded 没有观察到非零底盘命令，不能替代 wheel raw 非零、真实移动 HIL 或 delivery success。
- 2026-07-03 06:27 CST 现场硬件复测结论：相机停服务、USB `6-1` reauthorize、最低带宽 `YUYV 320x240@20` 直采仍 `VIDIOC_STREAMON Input/output error`，设备继续挂在 USB `12M` full-speed；WASD/手控经 bridge 已观察到 `T=11 L/R=±255`，临时 `speed/T=1` 也观察到 `L/R=±0.092308` 和强对照 `L/R=±0.5`，但 `T=1001 L/R` 始终 `0/0`。因此 PC/API/ROS/UART 命令链路已通，剩余风险转为摄像头物理 USB 链路和 WAVE ROVER 电机使能、底盘模式、下位机固件或反馈链路。
- 2026-07-03 07:20 CST 现场直连复核进一步收窄：相机在停用共享预览服务后，`MJPG 640x480@30`、`MJPG 480x320@30`、`YUYV 320x240@20/25`、`YUYV 640x480@22` 直采均返回 `VIDIOC_STREAMON Input/output error` 且 0 字节，继续挂在 USB `12M` full-speed，页面独占已排除。底盘在停用 bridge 后直连 `/dev/ttyS5 @115200` 可持续读到 ESP32 `T=1001`，但写入 `T=143` 回显、`T=139` 查询、`T=900` 配置和 `T=11 L/R=255` 后均没有命令回显、查询响应或非零 `T=1001 L/R`；PC/上车服务现在必须把 `serial_write_returned=true` 与 `wheel_feedback_lr_nonzero_proven=false` 分开展示。最新风险从“PC 页面或 ROS2 未发命令”下沉为上位机 TX 到 ESP32 RX 接线/引脚/固件 UART 接收、电机使能、底盘模式和电机电源；硬件协议依据仍采用 `docs/vendor/VENDOR_INDEX.md` 指向的 WAVE ROVER `json_cmd.h`、`uart_ctrl.h`、`movtion_module.h` 与 `ugv_rpi/config.yaml`。
- 2026-07-03 07:30 CST 起，现场恢复动作工具化：`onboard/scripts/camera_usb_recovery_smoke.py` 会停相机服务、关闭目标 USB autosuspend、按需 reauthorize、解绑同一复合设备的 `snd-usb-audio` 接口，再用 `YUYV@320x240@20` 和 `MJPG@480x320@30` 做直接 STREAMON smoke，最后重启服务；`onboard/scripts/wave_rover_uart_tx_probe.py` 默认只在底盘串口空闲时发送非运动 `T=143/T=139/T=900/T=131` 查询，用于确认上位机 TX 是否真的被 ESP32 解析，只有显式 `--motion-test` 才发送短 `T=11` 脉冲并自动 stop。真机结果：相机 smoke 仍 `streamon_failed`、两路输出 0 字节；UART probe 停 bridge 后仍能读到 `T=1001`，但 `esp32_receive_confirmed=false`。这两个脚本用于换 USB/改接线后的快速复测，不改变 PC 普通用户首屏，也不把硬件 blocker 包装成已完成。
...
- 2026-07-06 02:13 CST 起，地图太小/ROS2 配套口径曾再次收敛：PC 首页和 `/map` 默认缩放为 `200%` 可读大图，`完整态势` 回 `100%`，`细节放大` 到 `1200%`；后续 02:40 已按现场“仍太小”的反馈升级为 `300%` 默认值。ROS2 配套仍为工程观察：RViz2/Nav2 RViz 配置用于本地看 `/map`、`/scan`、TF、路径、定位和 costmap；Foxglove Bridge + Foxglove Web 用于远程浏览器观察。这些入口不替代 PC 简易控制台，不启动 ROS2/RViz2/Foxglove/Nav2/建图 runtime，不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。同轮 summary/live-summary 还会在最近 first-frame probe 已证明 `uvc_no_frame_not_exclusive` 时压过 health 中由 UVC XU 控制查询产生的 transport 日志，避免现场误判为 USB 传输主因。
...
- 2026-07-02 CST 现场复核历史证据：历史版本曾把 `/map` 缩放推进到 `3200%`、`8000%/9600%`，会让浏览器形成超宽滚动画布，不再作为当前有效口径。当时“PC 地图太小”的默认处理是使用 PC 大地图、访问 `http://<PC>:7001/map`，默认 `100%` 完整态势同屏看路线/车位/雷达，或在同一页面点 `细节放大` 到最高 `4800%`；ROS2 配套只推荐工程观察：本机 RViz2 看 `/map`、`/scan`、TF、路径、定位和 costmap，远程多人观察用 Foxglove bridge + Foxglove Web，不替代普通用户 PC 大地图，也不作为发车、自由移动或建图前置。
- 2026-07-02 CST 起，PC 普通首屏必须打开即维持实时观察面：根节点暴露 `data-open-page-live-map-refresh=true`、`data-open-page-live-camera-preview=true` 和 `data-open-page-keyboard-auto-ready=true`；地图卡固定每 `2500ms` 读取 `/api/robot-control/map/preview` 与 radar status，并每 `5000ms` 低频刷新 `/api/robot-control/radar/scan-proof/refresh` 后回写地图 overlay，DOM 暴露 `data-live-map-refreshes=map_preview,radar_status,radar_scan_proof` 和 `data-live-map-keeps-overlays=robot_pose,nav2_route,radar_points,goal`；相机卡每 `2000ms` 只读刷新共享 MJPEG status，DOM 暴露 `data-live-camera-auto-joins-shared-mjpeg=true`。这些定时刷新都不得启动雷达 lifecycle、Nav2、manual、keyboard、free-roam、建图 runtime、delivery、stop 或 `/cmd_vel`。
- 2026-07-03 CST 起，PC 普通首屏加载必须先并行预热真实地图和共享图传状态：组件挂载后立即请求 `GET /api/robot-control/map/preview` 与 `GET /api/robot-control/camera/mjpeg/status`，不等待完整 `GET /api/robot-control/summary` 返回；summary 到达后再自动接管 WASD gate、目标闭环和完整诊断。Node summary 中用于 MJPEG relay 的 camera overlay 只给 `600ms` 辅助预算，避免 USB full-speed/首帧失败的相机 health 慢响应把整页首包拖住；权威相机诊断仍来自 summary builder 的只读 readback 与独立 MJPEG status。该预热只读，不启动相机独占采集、雷达 lifecycle、Nav2、manual、keyboard、free-roam、建图 runtime、delivery、stop 或 `/cmd_vel`。
- 2026-07-02 CST 起，`GET /api/robot-control/nav2/goal/execution/latest` 和 `GET /api/robot-control/base/feedback-samples` 的直连响应也必须显式暴露只读边界。Nav2 latest 固定返回 `readback_only=true`、`nav2_goal_execution_latest_readback_only=true`、`sends_motion_when_clicked=false`、`starts_camera_exclusive_capture=false`、`starts_radar_lifecycle=false`、`starts_nav2=false`、`starts_manual=false`、`starts_keyboard=false`、`starts_free_roam=false`、`starts_map_runtime=false`、`submits_delivery=false`、`stops_motion=false` 和 `robot_control_executed=false`；轮速 samples latest 固定返回同组 no-motion 字段与 `base_feedback_samples_readback_only=true`。这些字段只证明读最近行程和读轮速样本不会发车，不会启动 Nav2/manual/keyboard/free-roam/建图/雷达 lifecycle，不提交送达、不发送 stop 或 `/cmd_vel`；真正执行路线、键盘或自由移动仍必须走对应动作入口和现场安全确认。
- 2026-07-02 CST 起，`GET /api/robot-control/summary` 与 `GET /api/robot-control/live-summary` 顶层必须显式暴露只读执行边界：`readback_only=true`、`robot_control_executed=false`、`publishes_cmd_vel=false`、`sends_motion_when_clicked=false`、`starts_nav2=false`、`starts_manual=false`、`starts_keyboard=false`、`starts_free_roam=false`、`starts_map_runtime=false`、`submits_delivery=false` 和 `stops_motion=false`。现场脚本只读这两个总览端点时，不得再把缺失字段读成 `null` 后误判总览可能会发车或发布 `/cmd_vel`。
- 2026-07-02 CST 起，`GET /api/robot-control/summary` 顶层必须暴露当前目标短字段：`current_goal_missing_ids`、`current_goal_missing_labels`、`current_goal_blocked_ids`、`current_goal_blocked_labels`、`current_goal_ready_action_labels`、`current_goal_mapping_blocked_only_by_camera`、`current_goal_free_move_allowed_while_mapping_blocked` 和 `current_goal_camera_only_blocks_mapping_plain`。这些字段与 `goal_checklist`、`goal_checklist_summary` 和 `live_closure_summary` 同源，只用于现场 curl/PC 首屏判断“建图是否只被相机挡住、自由移动是否仍可先做”；不得自动勾安全确认、不得执行 Nav2/manual/keyboard/free-roam/建图/delivery/stop 或 `/cmd_vel`。
- 2026-07-02 CST 起，普通首屏 `plain-objective-overview` 也必须同步暴露上述当前目标短字段：`data-current-goal-missing-ids`、`data-current-goal-missing-labels`、`data-current-goal-blocked-ids`、`data-current-goal-blocked-labels`、`data-current-goal-ready-action-labels`、`data-current-goal-move-now-status-plain`、`data-current-goal-mapping-blockers-plain`、`data-current-goal-mapping-blocked-only-by-camera`、`data-current-goal-free-move-allowed-while-mapping-blocked` 和 `data-current-goal-camera-only-blocks-mapping-plain`。该 DOM 只读，固定 `data-current-goal-sends-motion-when-clicked=false`，用于现场 smoke 一眼确认“可先移动、建图还卡画面/雷达”，不启动任何控制入口。
- 2026-07-02 CST 起，`GET /api/robot-control/map/preview` 顶层必须直接暴露 `radar_overlay_wysiwyg_complete`，与 `radar_overlay_status`、`radar_overlay_current_point_count`、`radar_overlay_source_point_count`、`radar_overlay_blocks_wysiwyg` 和 `radar_overlay_current_vs_source_plain` 同源，现场脚本不需要解析嵌套 overlay 就能判断“雷达开始后地图上的标记是否所见即所得”。`POST /api/robot-control/radar/scan-proof/refresh` 也必须暴露 `next_action_plain`，刷新后只指向固定读回链路 `radar/status -> map/preview`，不得暗示该 refresh 会启动雷达 lifecycle、Nav2、建图 runtime、manual/keyboard/free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-07-02 CST 起，`GET /api/robot-control/free-roam/autonomy/latest` 必须直接暴露自由移动 latest 的只读边界：`readback_only=true`、`free_roam_latest_readback_only=true`、`sends_motion_when_clicked=false`、`starts_camera_exclusive_capture=false`、`starts_radar_lifecycle=false`、`starts_nav2=false`、`starts_manual=false`、`starts_keyboard=false`、`starts_free_roam=false`、`starts_map_runtime=false`、`submits_delivery=false` 和 `stops_motion=false`。该接口只读上车 `/api/free-roam/autonomy/latest` runtime artifact 和门禁摘要，不清停止请求、不启动自由移动、不启动建图、不发 Nav2/manual/keyboard/delivery/stop 或 `/cmd_vel`；现场发车仍必须走单独 start endpoint 和现场安全确认。
- 2026-07-02 CST 起，summary 顶层和普通首屏 DOM 必须暴露 `current_radar_map_wysiwyg_pack_*`，直接说明当前地图雷达贴图是 `loaded` 还是 `needs_readback_refresh`、当前点数/来源点数、只读恢复顺序 `/api/robot-control/radar/scan-proof/refresh -> /api/robot-control/radar/status -> /api/robot-control/map/preview -> /api/robot-control/summary` 以及完整 no-motion 边界。该包用于配合 `/map` 大屏和 RViz2/Foxglove 工程观察判断“雷达点有没有贴到当前地图”，不启动雷达 lifecycle、Nav2、manual、keyboard、free-roam、建图 runtime、delivery 或 stop，也不发送 `/cmd_vel`。
- 2026-07-02 CST 起，`current_radar_map_wysiwyg_pack_missing_evidence` 与 `current_radar_map_wysiwyg_pack_missing_evidence_labels` 必须在 summary 顶层显式输出：雷达地图贴图 loaded 时为空数组 `[]`，未贴到当前地图时分别为 `["radar_map_points"]` 和 `["雷达地图标记"]`。普通首屏 `plain-current-radar-map-wysiwyg-pack` 必须同步暴露 `data-missing-evidence` 与 `data-missing-evidence-labels`，空数组显示为 `none`；这些字段只说明当前地图标记缺口，不启动雷达 lifecycle、Nav2、manual、keyboard、free-roam、建图 runtime、delivery 或 stop，也不发送 `/cmd_vel`。
- 2026-07-02 CST 起，summary 顶层和普通首屏 DOM 必须暴露 `current_camera_wysiwyg_pack_*`，直接说明当前画面 WYSIWYG 是 `visible` 还是 `needs_first_frame`、共享预览是否已有当前帧、是否多人可加入、相机首帧 probe 状态、USB full-speed/硬件动作、只读恢复顺序 `/api/robot-control/camera/first-frame/probe -> /api/robot-control/camera/mjpeg/status -> /api/robot-control/summary` 和完整 no-motion 边界。2026-07-02 CST 起还必须暴露 `current_camera_wysiwyg_pack_missing_evidence=["camera_first_frame"]` 与 `current_camera_wysiwyg_pack_missing_evidence_labels=["画面首帧"]`，普通首屏 `plain-current-camera-wysiwyg-pack` 同步输出 `data-missing-evidence` 和 `data-missing-evidence-labels`。该包用于回答“摄像头无法看到效果/是不是独占”：共享预览是单上游多人观看，画面首帧缺口阻塞建图启动但不阻塞自由移动；复测不启动独占采集、Nav2、manual、keyboard、free-roam、建图 runtime、delivery 或 stop，也不发送 `/cmd_vel`。
- 2026-07-02 CST 起，`GET /api/robot-control/camera/mjpeg/status` 必须与 summary 同源暴露首帧失败直连 alias：`first_frame_probe_status`、`first_frame_probe_failure_reason`、`first_frame_failure_reason`、`camera_first_frame_probe_status` 和 `camera_first_frame_failure_reason`。当共享预览未出帧且 health/source diagnosis 已证明 `first_frame_total_timeout`、`capture_read_returned_false` 等原因时，直连 status 不得再返回 null；它仍只读本机共享预览和上车 camera health，不新建独占采集、不启动建图 runtime、Nav2、manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-07-02 CST 起，`GET /api/robot-control/camera/mjpeg/status` 还必须在既有 `camera_usb_speed`、`camera_usb_full_speed_detected`、`camera_hardware_action_required`、`camera_hardware_action_label` 之外同步暴露短 alias：`usb_speed`、`usb_full_speed_detected`、`hardware_action_required` 和 `hardware_action_label`。当诊断为 `uvc_full_speed_usb_not_exclusive` 时，现场 `curl | jq` 必须能直接读到 `usb_speed=12M`、`usb_full_speed_detected=true`、`hardware_action_required=true` 和 `hardware_action_label=换高速USB后复测`，避免只看直连 status 时误判“没有硬件动作”。这些字段只读，不打开独占相机、不启动建图/运动链路。
- 2026-07-04 CST 起，`GET /api/robot-control/camera/mjpeg/status` 与 `summary/live-summary`
  还必须识别 `uvc_cma_alloc_failed_not_exclusive`。当上车 health/diagnostics 显示 UVC 没有页面独占、
  USB 为 `480M` high-speed，但内核近期出现 CMA 连续内存分配失败时，PC status 顶层必须暴露
  `cma_memory_diagnostics_status`、`cma_memory_diagnostics_cma_total_kb`、
  `cma_memory_diagnostics_cma_free_kb`、`cma_memory_diagnostics_failure_count`、
  `cma_memory_diagnostics_latest_failure`，并把 `hardware_action_required=true`、
  `hardware_action_label=释放内存/重启后复测`、`camera_blocks_mapping_start=true` 和
  `camera_blocks_free_move=false` 同步输出。该 status 仍只读本机共享预览和上车 health，不创建独占采集、
  不启动建图/雷达 lifecycle/运动链路。
- 2026-07-02 CST 起，上位机共享 MJPEG 默认预览首帧采用两段式兜底：第一段优先按低带宽 DV20/UVC 格式矩阵在默认 `/dev/video*` path 上取帧；失败后第二段用剩余预算只尝试 OpenCV index 与 CAP_V4L2 打开方式。普通首帧 probe 在不开 backend smoke 时总预算固定为 `10s`，确保 PC 代理能在 `12s` 请求超时前拿到格式 fallback 或总超时摘要；backend smoke 仍保留长预算。这样普通用户多人预览更容易在 path 能 open 但无帧时恢复画面，同时不把整个首屏预算耗在同一格式的 fallback 上。该逻辑仍只读 camera，不独占浏览器页面，不启动 Nav2/manual/keyboard/free-roam/建图 runtime、delivery 或 stop，也不发送 `/cmd_vel`。
- 2026-07-02 CST 起，summary 顶层和普通首屏 DOM 必须暴露 `current_trip_execution_pack_*`，把完整 Nav2 图上行程的当前验收压成一个现场包：状态、执行端点、stop 端点、读回端点、路线 ready、Nav2 到点、同窗口轮速 L/R、送达确认、缺口、最小安全确认和边界都必须同源于 `nav2_route_acceptance_packet`。该包本身只读，固定 `current_trip_execution_pack_sends_motion_when_clicked=false`；只有用户进入行程卡、勾现场安全确认并点击执行时才会按 `current_trip_execution_pack_sends_motion_when_executed=true` / `current_trip_execution_pack_starts_nav2_when_executed=true` 执行图上路线，执行后的读回仍不启动 Nav2/manual/keyboard/free-roam/建图 runtime、不提交 delivery、不 stop。
- 2026-07-02 CST 起，summary 顶层和普通首屏 DOM 必须暴露 `current_minimal_precheck_pack_*`，直接回答发车前预检是否已经精简到只需勾现场安全确认。当 `current_minimal_precheck_pack_status=safety_confirm_only` 时，Nav2 图上行程、键盘连续手控和自由自助移动这些 ready action 不再把相机、雷达、现场报告或路线 WYSIWYG 当作额外发车前置；相机和雷达 ready 仍只影响建图启动/验收。该包只读，必须固定 `current_minimal_precheck_pack_sends_motion_when_clicked=false`、`current_minimal_precheck_pack_starts_nav2_when_clicked=false`、`current_minimal_precheck_pack_starts_manual_when_clicked=false`、`current_minimal_precheck_pack_starts_keyboard_when_clicked=false`、`current_minimal_precheck_pack_starts_free_roam_when_clicked=false`、`current_minimal_precheck_pack_starts_map_runtime_when_clicked=false`、`current_minimal_precheck_pack_submits_delivery_when_clicked=false` 和 `current_minimal_precheck_pack_stops_motion_when_clicked=false`。
- 2026-07-02 CST 起，`current_minimal_precheck_pack_missing_evidence` 与 `current_minimal_precheck_pack_missing_evidence_labels` 必须显式输出：当 `current_minimal_precheck_pack_status=safety_confirm_only` 或 `complete` 时为空数组 `[]`，不得返回 `null`；当 status 为 `blocked` 时必须列出仍未收敛的发车前置，例如 `camera_preflight`、`radar_preflight`、`operator_report_preflight`、`route_wysiwyg_preflight` 或 `minimal_precheck_safety_only`。普通首屏 `plain-current-minimal-precheck-pack` 同步暴露 `data-missing-evidence` 与 `data-missing-evidence-labels`，空数组显示为 `none`；字段只读，不自动勾安全确认、不执行任何运动或建图入口。
- 2026-07-02 CST 起，完整 Nav2 行程、PC 键盘连续手控和自由自助移动三个当前控制包必须额外暴露统一命名 alias：`current_trip_execution_pack_safety_confirm_required`、`current_keyboard_control_pack_safety_confirm_required` 和 `current_free_move_control_pack_safety_confirm_required`。这些字段分别与既有 `current_trip_execution_pack_requires_safety_confirm`、`current_keyboard_control_pack_requires_safety_confirm`、`current_free_move_control_pack_requires_safety_confirm` 同源；普通首屏 DOM 对应包节点必须暴露 `data-safety-confirm-required=true`。用途是现场脚本无需记两套命名即可确认“发车前预检只剩安全确认”，字段只读，不执行 Nav2、manual、keyboard、free-roam、建图、delivery、stop 或 `/cmd_vel`。
- 2026-07-02 CST 起，summary 顶层和普通首屏必须额外暴露 `current_safety_confirm_queue_*` / `plain-current-safety-confirm-queue`，把安全确认后可做的三个动作压成一个短队列：`run_nav2_route -> hold_keyboard -> start_free_move`。队列必须输出 action ids、显示名、start/stop、每项 acceptance endpoints、去重 readback endpoints、primary action、动作数量、`requires_safety_confirm=true`、`minimal_precheck_safety_only=true`、相机/雷达/现场报告/路线 WYSIWYG preflight 均为 false、`requires_manual_action_per_step=true` 和 `auto_runs=false`。队列还必须暴露 `current_safety_confirm_queue_primary_focus_source_card_id`、`current_safety_confirm_queue_primary_focus_kind`、`current_safety_confirm_queue_primary_focus_button_label` 和 `current_safety_confirm_queue_next_action_plain`，普通首屏按钮只跳到第一项动作卡片。2026-07-02 CST 起，队列还必须暴露 `current_safety_confirm_queue_readback_sequence_labels`、`current_safety_confirm_queue_readback_button_label` 和 `current_safety_confirm_queue_readback_refreshes_*`；普通首屏 `plain-current-safety-confirm-queue-readback` 按钮只按队列 readback endpoints 刷新 map preview、Nav2 latest、wheel feedback、delivery latest、summary 和 free-roam latest。该队列只说明“勾现场安全确认后按顺序手动执行并读回复验”，节点本身固定不发车、不自动执行 Nav2/manual/keyboard/free-roam/建图/delivery/stop 或 `/cmd_vel`。
- 2026-07-02 CST 起，普通首屏 `plain-current-safety-confirm-queue` 还必须把后端原始焦点和浏览器本地有效焦点分开暴露：`data-backend-primary-focus-kind` / `data-backend-primary-focus-button-label` 保留 summary 原值，`data-effective-primary-focus-kind` / `data-effective-primary-focus-button-label` 随本地安全确认 checkbox 即时变化。当 primary action 是 `run_nav2_route` 且本地已勾安全确认时，有效焦点必须从 `trip_safety_confirm` 切到 `trip_execute_button`，按钮文案从 `去勾行程安全确认` 切到 `去执行图上行程`；按钮仍固定 `data-focus-only=true`、`data-sends-motion-when-clicked=false`，不执行 Nav2/manual/keyboard/free-roam/建图/delivery/stop 或 `/cmd_vel`。
- 2026-07-02 CST 起，自由移动和建图当前控制包必须额外暴露下一步白话：`current_free_move_control_pack_next_action_plain`、`current_mapping_control_pack_next_action_plain`，普通首屏 `plain-current-free-move-control-pack` 和 `plain-current-mapping-control-pack` 同步暴露 `data-next-action-plain`。自由移动 ready 时必须直接写“勾现场安全确认后启动自由自助移动”；建图 blocked 时必须直接写相机/雷达下一步，并明确低速自由移动不受影响。字段只读，不自动勾安全确认、不启动 free-roam/建图/Nav2/manual/keyboard/delivery/stop 或 `/cmd_vel`。
- 2026-07-02 CST 起，完整 Nav2 行程和 PC 键盘连续手控当前控制包也必须暴露下一步白话：`current_trip_execution_pack_next_action_plain`、`current_keyboard_control_pack_next_action_plain`，普通首屏 `plain-current-trip-execution-pack` 和 `plain-current-keyboard-control-pack` 同步暴露 `data-next-action-plain`。Nav2 ready 时必须直接写“勾现场安全确认后执行图上 Nav2 行程”；键盘 ready 时必须直接写“页面自动准备不发车，按住 W/A/S/D 或方向键才连续低速移动”。字段只读，不自动执行 Nav2、不发送 manual/free-roam/建图/delivery/stop 或 `/cmd_vel`。
- 2026-07-02 CST 起，RViz2 工程观察入口 `ros2 launch ros2_trashbot_bringup rviz.launch.py` 同步显示 `/camera/image_raw`，与 `/map`、`/scan`、TF、Nav2 路径、定位和 costmap 一起组成只读现场观察面。该入口只用于工程排障“画面是否有真实帧、地图/雷达/路线是否同屏可见”，不得加入 GoalTool、SetInitialPose 或任何控制入口；普通用户仍优先使用 PC `/map` 大地图和共享相机预览。
...
- 2026-07-02 CST 起，自由移动/建图卡必须在主按钮前暴露 `plain-free-roam-primary-button-summary`，直接说明当前点击是 `只自由移动` 还是 `先建图再移动`；当画面或雷达未就绪时文本必须说明“点击只启动低速自由移动，不启动建图记录；传感器补齐后再建图”，并暴露固定 free-roam start、mapping start 和 post-start readback endpoints。该说明只读，不触发任何运动。
- 2026-07-02 CST 起，建图启动成功后的自动复验链路必须额外暴露 `mapping_post_start_readback_endpoints`、`mapping_post_start_readback_sequence_labels`、`mapping_post_start_readback_refreshes_free_roam_latest`、`mapping_post_start_readback_refreshes_map_preview`、`mapping_post_start_readback_refreshes_summary` 和同源 `current_mapping_action_post_start_readback_*` 字段。固定顺序为 `/api/robot-control/free-roam/autonomy/latest -> /api/robot-control/map/preview -> /api/robot-control/summary`，标签为“读取自由移动状态、刷新地图画面、刷新总览”；只读边界固定 `*_sends_motion=false`、`*_starts_nav2=false`、`*_starts_manual=false`、`*_starts_keyboard=false`、`*_starts_free_roam=false`、`*_starts_map_runtime=false`、`*_submits_delivery=false` 和 `*_stops_motion=false`。这些字段只描述 `/api/robot-control/map/start` 成功后的验收读回链，不替用户再次启动自由移动或建图 runtime，也不把 post-start readback 当成发车动作。
- 2026-07-02 CST 起，summary 顶层和普通首屏 DOM 必须暴露 `current_mapping_action_*` 短字段，与 runbook 的 `start_mapping_when_sensors_ready` 同源：`current_mapping_action_id=start_mapping_when_sensors_ready`、`current_mapping_action_start_endpoint=/api/robot-control/map/start`、`current_mapping_action_stop_endpoint=/api/robot-control/free-roam/autonomy/stop`、`current_mapping_action_preview_endpoint=/api/robot-control/map/preview`、`current_mapping_action_acceptance_endpoints`、`current_mapping_action_readback_endpoints`、`current_mapping_action_missing_evidence`、`current_mapping_action_camera_ready`、`current_mapping_action_radar_ready`、`current_mapping_action_only_camera_missing`、`current_mapping_action_radar_overlay_wysiwyg_complete`、`current_mapping_action_blocks_free_move=false` 和 `current_mapping_action_safety_confirm_required_when_executed=true`。普通首屏 `plain-current-mapping-action` 必须用一句话说明当前建图动作是可启动、只差画面还是待补条件；如果只剩相机首帧，它必须明确雷达/地图贴图已满足、自由移动不受影响。若相机诊断显示 USB full-speed 或硬件动作必需，该行还必须暴露并展示 `current_mapping_action_camera_hardware_action_required`、`current_mapping_action_camera_hardware_action_label`、`current_mapping_action_camera_usb_full_speed_detected`、`current_mapping_action_camera_usb_speed`、`current_mapping_action_camera_source_diagnosis_status`、`current_mapping_action_camera_source_diagnosis_not_exclusive` 和 `current_mapping_action_camera_recovery_next_action_plain`，让现场直接看到“不是页面独占、换高速 USB/带供电 Hub 后复测”。执行该动作的边界必须明示为 `current_mapping_action_starts_map_runtime_when_executed=true`，同时 `current_mapping_action_starts_nav2=false`、`current_mapping_action_starts_manual=false`、`current_mapping_action_starts_keyboard=false`、`current_mapping_action_starts_free_roam=false`、`current_mapping_action_submits_delivery=false` 和 `current_mapping_action_stops_motion=false`；DOM 只读行本身不自动勾安全确认、不启动 free-roam/Nav2/manual/keyboard/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-02 CST 起，catalog 验证必须对上述三个 proof refresh 的直接回包逐项断言完整只读边界，尤其是 `map/proof/refresh` 和 `nav2/proof/refresh` 也要覆盖 `starts_radar_lifecycle=false`、manual、keyboard、free-roam、map runtime、delivery 和 stop，避免只测 radar refresh 导致现场 curl map/nav2 proof 回包时边界退化无人发现。
...
- 2026-07-02 CST 起，`GET /api/robot-control/radar/status` 在读取固定 `/api/radar/status` 后，会额外读取固定 `/api/map/preview` 并合并只读贴图 alias：`radar_status_map_preview_*`、`radar_overlay_status`、`radar_overlay_current_point_count`、`radar_overlay_source_point_count`、`radar_overlay_wysiwyg_complete`、`radar_overlay_primary_blocked_reason` 和 `radar_overlay_current_vs_source_plain`。这样现场脚本单看 radar status 回包也能知道“雷达本体是否 ready”以及“地图上当前雷达点是否已 WYSIWYG”。map preview 聚合失败不得让 radar status 退化为发车或控制入口；只允许返回未加载 alias，不启动雷达 lifecycle、Nav2、manual、keyboard、free-roam、建图 runtime、delivery、stop 或 `/cmd_vel`。
...
- 2026-07-02 CST 起，普通首屏 `连接/刷新` 也必须按雷达 WYSIWYG 顺序刷新：先 `POST /api/robot-control/radar/scan-proof/refresh`，再读 `/api/robot-control/radar/status`、`/api/robot-control/map/preview` 和共享相机状态，最后让 summary 与地图层回到同轮事实。按钮 DOM 必须暴露 `data-refreshes-radar-scan-proof=true`、`data-refreshes-map-preview=true`、`data-refreshes-radar-status=true`、`data-refreshes-camera-mjpeg-status=true` 以及完整 no-motion 边界；该动作不启动雷达 lifecycle、Nav2、manual、keyboard、free-roam、建图 runtime、delivery 或 stop，也不发送 `/cmd_vel`。
...
- 2026-07-02 CST 起，summary 顶层和 `plain-current-wysiwyg-action` DOM 必须暴露 `current_wysiwyg_next_action_*` 短 alias。若雷达贴图已完成且唯一缺口是相机，`current_wysiwyg_next_action_status` 必须为 `only_camera_hardware_action` 或 `camera_readback_only`，`current_wysiwyg_next_action_plain` 必须直接写明“雷达贴图已完成，只剩相机；自由移动不受相机阻塞，建图仍等待相机首帧”。当相机诊断要求硬件处理时，同步暴露 `current_wysiwyg_next_action_hardware_action_required=true`、`current_wysiwyg_next_action_hardware_action_label=换高速USB后复测`、`current_wysiwyg_next_action_blocks_mapping_start=true` 和 `current_wysiwyg_next_action_allows_free_move=true`。这些字段只服务现场读回和 DOM smoke，不启动 Nav2/manual/keyboard/free-roam/建图/雷达 lifecycle，不提交送达，不发送 stop 或 `/cmd_vel`。
- 2026-07-02 CST 起，summary 顶层和 `plain-current-motion-verification-pack` DOM 必须暴露 `current_motion_verification_pack_*` 短 alias，把当前可现场验证的运动动作压成一个普通用户动作包。典型值为 `current_motion_verification_pack_status=ready_for_safety_confirm`、`current_motion_verification_pack_action_ids=[run_nav2_route,hold_keyboard,start_free_move]`、`current_motion_verification_pack_primary_action_id=run_nav2_route`、`current_motion_verification_pack_ready_action_count=3`。文案必须说明“勾一次现场安全确认后，按顺序验证重跑图上行程并复验轮速、键盘连续手控、自由自助移动；发车前预检只保留安全确认，不把相机、雷达或现场报告作为额外前置”。该包节点本身只读，固定 `current_motion_verification_pack_sends_motion_when_clicked=false`、`current_motion_verification_pack_readback_sends_motion=false`、`current_motion_verification_pack_starts_nav2_when_clicked=false`、`current_motion_verification_pack_starts_manual_when_clicked=false`、`current_motion_verification_pack_starts_keyboard_when_clicked=false`、`current_motion_verification_pack_starts_free_roam_when_clicked=false`、`current_motion_verification_pack_starts_map_runtime_when_clicked=false`、`current_motion_verification_pack_submits_delivery_when_clicked=false` 和 `current_motion_verification_pack_stops_motion_when_clicked=false`；真正发车仍只能由各动作按钮在现场安全确认后触发。
...
- 2026-07-02 CST 起，雷达贴图只读刷新还必须补齐完整安全边界 alias：`radar_overlay_refresh_sends_motion=false`、`radar_overlay_refresh_starts_radar_lifecycle=false`、`radar_overlay_refresh_starts_nav2=false`、`radar_overlay_refresh_starts_manual=false`、`radar_overlay_refresh_starts_keyboard=false`、`radar_overlay_refresh_starts_free_roam=false`、`radar_overlay_refresh_starts_map_runtime=false`、`radar_overlay_refresh_submits_delivery=false` 和 `radar_overlay_refresh_stops_motion=false`，并同步暴露在 summary 顶层、`live_closure_summary`、`plain-live-closure-summary` 和 `plain-live-closure-wysiwyg-diagnostics` DOM。现场验收脚本可据此确认“刷新雷达扫描 -> 雷达状态 -> 地图预览 -> 总览”只是贴图读回链路，不会误触发 Nav2、手控、键盘、自由移动、建图 runtime、送达或急停。
...
- 2026-07-02 CST 起，summary 顶层 `nav2_goal_succeeded` 与 `nav2_goal_execution_proven` 必须拆开：`nav2_goal_succeeded=true` 只表示 Nav2 action / latest 已到点成功；`nav2_goal_execution_proven=true` 必须继续要求同窗口 wheel L/R 非零，不能在 `current_motion_action_missing_evidence` 或 `trip_execution_missing_evidence` 仍包含 `same_window_wheel_lr_nonzero` 时为 true。这样 `GET /api/robot-control/summary` 与 `GET /api/robot-control/nav2/goal/execution/latest` 不会一个说 proven、一个说 not proven，现场脚本也不会把“需要轮速复验”误读成完整路线已闭环。
- 2026-07-02 CST 起，`GET /api/health` 必须暴露 PC 工作站运行态标识：`workstation_host=0.0.0.0`、`workstation_port=7001`、`workstation_listen_address=http://0.0.0.0:7001`、`default_robot_api_base_url=http://192.168.1.11:8787`、`server_started_at_ms` 和 `server_started_at_iso`。该 health 仍固定 `health_readback_only=true`、`robot_api_probe_executed=false`、`sends_motion_when_clicked=false`、`starts_nav2=false`、`starts_manual=false`、`starts_keyboard=false`、`starts_free_roam=false`、`starts_map_runtime=false`、`submits_delivery=false`、`stops_motion=false`，只用于确认 7001 是否已从当前代码重启，不证明机器人在线或路线/画面/雷达/建图完成。
- 2026-07-02 CST 起，`current_motion_action_*` 也必须同步暴露完整行程当前读回细项：`current_motion_action_route_ready_on_map`、`current_motion_action_nav2_goal_succeeded`、`current_motion_action_same_window_wheel_lr_nonzero`、`current_motion_action_delivery_success`、`current_motion_action_needs_same_window_wheel_rerun`、`current_motion_action_delivery_success_required`、`current_motion_action_latest_raw_left/right`、`current_motion_action_feedback_sample_count`、`current_motion_action_feedback_nonzero_sample_count`、`current_motion_action_current_gap_plain`、`current_motion_action_no_extra_precheck_plain` 和 `current_motion_action_delivery_next_action_plain`。这些字段必须与 `nav2_route_acceptance_packet` 同源，并在普通首屏 `plain-trip-current-motion-action` 与执行按钮 DOM 上暴露；用途是在安全确认后重跑图上行程、读回轮速和送达确认时，不用解析嵌套 packet 也能判断当前 motion 目标是否完成。当前动作边界必须同时暴露 `current_motion_action_sends_motion=true`、`current_motion_action_starts_nav2=true`、`current_motion_action_starts_manual=false`、`current_motion_action_starts_keyboard=false`、`current_motion_action_starts_free_roam=false`、`current_motion_action_starts_map_runtime=false`、`current_motion_action_submits_delivery=false` 和 `current_motion_action_stops_motion=false`；DOM 行本身只读，只有用户勾现场安全确认并点击执行按钮才会调用 Nav2 路线执行入口。
...
- 2026-07-02 CST 起，完整图上路线执行成功后的自动复验链路必须额外暴露 `nav2_post_execute_readback_endpoints`、`nav2_post_execute_readback_sequence_labels`、`nav2_post_execute_readback_refreshes_map_preview`、`nav2_post_execute_readback_refreshes_nav2_latest`、`nav2_post_execute_readback_refreshes_wheel_feedback`、`nav2_post_execute_readback_refreshes_delivery_latest`、`nav2_post_execute_readback_refreshes_summary` 和只读边界 `nav2_post_execute_readback_sends_motion=false`、`nav2_post_execute_readback_starts_nav2=false`、`nav2_post_execute_readback_starts_manual=false`、`nav2_post_execute_readback_starts_keyboard=false`、`nav2_post_execute_readback_starts_free_roam=false`、`nav2_post_execute_readback_starts_map_runtime=false`、`nav2_post_execute_readback_submits_delivery=false`、`nav2_post_execute_readback_stops_motion=false`。这些字段与 `trip_execution_readback_endpoints` 同源，说明 PC 在 `/api/robot-control/nav2/goal/execute` 成功转发后会自动刷新地图画面、读取最近行程、复验轮速采样、读取送达确认并刷新总览；该复验链本身不再发车、不提交送达、不启动 Nav2/manual/keyboard/free-roam/建图或 stop。
...
- 2026-07-02 CST 起，键盘 post-hold 复验链路必须同步暴露 summary 顶层和普通首屏 DOM alias：`keyboard_post_hold_readback_endpoints=["/api/robot-control/base/feedback-samples","/api/robot-control/summary"]`、`keyboard_post_hold_readback_sequence_labels=["复验键盘轮速采样","刷新总览"]`、`keyboard_post_hold_feedback_readback_required=true` 和 `keyboard_post_hold_summary_refresh_required=true`。2026-07-02 CST 起还必须同步暴露只读边界 `keyboard_post_hold_readback_sends_motion=false`、`keyboard_post_hold_readback_starts_nav2=false`、`keyboard_post_hold_readback_starts_manual=false`、`keyboard_post_hold_readback_starts_keyboard=false`、`keyboard_post_hold_readback_starts_free_roam=false`、`keyboard_post_hold_readback_starts_map_runtime=false`、`keyboard_post_hold_readback_submits_delivery=false` 和 `keyboard_post_hold_readback_stops_motion=false`，以及同源 `current_keyboard_action_post_hold_readback_*` 字段。这些 alias 与 `live_closure_summary.fixed_keyboard_*` 同源，只描述松开/停止后的只读验收链，不发送 manual pulse、stop、Nav2/free-roam/建图/delivery 或 `/cmd_vel`。
...
- 2026-07-04 02:35 CST 起，普通首屏地图和 `/map` 直达大屏默认缩放历史值曾为 `150%` 可读大图；当时有效值为 `100%` 完整态势，地图大面板继续占据首屏主视图，`细节放大` 最高为 `4800%`。这是普通用户解决“地图太小”的默认路径；ROS2 配套仍只放在默认折叠的“工程观察”里，RViz2/Foxglove 不作为普通用户发车入口，也不得自动启动 ROS2、Nav2、建图 runtime、manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-01 16:45 CST 起，`GET /api/robot-control/summary` 顶层还必须直接暴露雷达贴图恢复 alias：`radar_overlay_primary_blocked_reason`、`radar_overlay_current_vs_source_plain`、`radar_overlay_next_action_plain`、`radar_overlay_refresh_next_action_plain`、`radar_overlay_needs_refresh`、`radar_overlay_blocks_wysiwyg`、`radar_overlay_blocks_free_move=false`、`radar_overlay_readback_endpoint=/api/robot-control/radar/scan-proof/refresh`、`radar_overlay_refresh_endpoint=/api/robot-control/radar/scan-proof/refresh`、`radar_overlay_status_endpoint=/api/robot-control/radar/status`、`radar_overlay_preview_endpoint=/api/robot-control/map/preview`、`radar_overlay_summary_endpoint=/api/robot-control/summary`、`radar_overlay_refresh_sequence`、`radar_overlay_refresh_sequence_labels`、`radar_overlay_recovery_sequence`、`fixed_radar_overlay_refresh_endpoint=/api/robot-control/radar/scan-proof/refresh`、`fixed_radar_overlay_map_preview_endpoint=/api/robot-control/map/preview`、`radar_overlay_refresh_sends_motion=false` 和 `radar_overlay_refresh_starts_radar_lifecycle=false`。这些字段与 `live_closure_summary` 同源，用于现场一条 `curl | jq` 判断旧雷达点是否已抑制、是否只阻塞 WYSIWYG、以及下一步是否只需手动刷新雷达扫描读数再刷新地图画面；不得启动雷达 lifecycle、Nav2、manual/keyboard/free-roam/建图/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-01 13:26 CST 起，普通首屏地图和 `/map` 直达大屏在 `100%`/`1200%` 放大时必须保证整张画布四周都可滚动到：`.plain-map-layer` 的滚动原点固定为左上完整画布，DOM 暴露 `data-scroll-origin=top_left_full_canvas` 和 `data-auto-center-on-zoom=true`；进入 `/map`、全屏、只看地图、刷新地图画面或调整缩放后，PC 端只自动居中当前视口，不改变地图、路线、小车位置或雷达点坐标。该修正只解决大地图负方向溢出导致边缘不可达的问题，不启动 ROS2/RViz2/Foxglove、Nav2、建图 runtime，不发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
- 2026-07-01 12:40 CST 起，普通首屏地图必须继续保持普通用户“大地图优先”；历史默认曾使用 `400%` 现场大图，当时默认曾改为 `100%` 完整态势、最高细节放大为 `4800%`；`/map` 是推荐直达地图大屏入口，`?view=map` 保留兼容，用户需要看局部时点“细节放大”。ROS2 配套只作旁路：RViz2 / `nav2_rviz_plugins` 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位，Foxglove / `foxglove_bridge` 用于浏览器观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-02 CST 起，共享相机预览必须额外暴露“任何页面可加入同一条上游流”的短合同。`GET /api/robot-control/summary`、`live_closure_summary`、`plain-live-closure-summary` 和 `plain-live-camera-recovery-readback` 必须同步暴露 `camera_shared_preview_everyone_can_join=true`、`camera_shared_preview_current_frame_visible`、`camera_shared_preview_gap_plain`、`camera_shared_preview_readback_only=true`、`camera_shared_preview_starts_camera_exclusive_capture=false` 和 `camera_shared_preview_sends_motion=false`；2026-07-02 CST 起还必须补齐 `camera_shared_preview_starts_nav2=false`、`camera_shared_preview_starts_manual=false`、`camera_shared_preview_starts_keyboard=false`、`camera_shared_preview_starts_free_roam=false`、`camera_shared_preview_starts_map_runtime=false`、`camera_shared_preview_submits_delivery=false` 和 `camera_shared_preview_stops_motion=false`。`GET /api/robot-control/camera/mjpeg/status` 必须暴露同源 `shared_preview_everyone_can_join=true`、`shared_preview_current_frame_visible`、`shared_preview_gap_plain`、`shared_preview_readback_only=true`、`shared_preview_starts_camera_exclusive_capture=false`、`shared_preview_sends_motion=false`、`shared_preview_starts_nav2=false`、`shared_preview_starts_manual=false`、`shared_preview_starts_keyboard=false`、`shared_preview_starts_free_roam=false`、`shared_preview_starts_map_runtime=false`、`shared_preview_submits_delivery=false` 和 `shared_preview_stops_motion=false`。这些字段只证明 PC Node 共享 relay 是单上游、多页面、非独占、只读；如果 `current_frame_visible=false`，仍必须如实提示相机源未出首帧或 USB full-speed 等硬件缺口，不得把可加入共享入口冒充成真实画面 WYSIWYG 已完成。
...
- 2026-07-02 CST 起，`objective_audit_items[]` 每个目标还必须输出机器可读缺口证据：`missing_evidence_ids`、`missing_evidence_labels`、`readback_endpoints` 和 `next_action_requires_safety_confirm`。普通首屏 `plain-objective-overview-*` 行必须同步暴露 `data-missing-evidence-ids`、`data-missing-evidence-labels`、`data-readback-endpoints` 和 `data-next-action-requires-safety-confirm`，空缺口显示为 `none`。这些字段用于现场脚本逐项判断完整行程/键盘/自由移动、WYSIWYG、最小预检和建图缺什么证据，不解析中文文案；它们只读，不执行 Nav2/manual/keyboard/free-roam/建图/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-01 12:40 CST 起，普通首屏地图必须保持可适配全图；历史默认曾使用 `400%` 现场大图，当时默认使用 `100%` 完整态势，`/map` 直达地图大屏也默认使用 `100%`，`?view=map` 继续兼容，并继续保留同一张 WYSIWYG 画布里的地图、路线、小车位置和雷达贴图。“细节放大”按钮一键到 `4800%`，`+/-` 支持逐级缩放，“完整态势”保持回到 `100%` 全图。ROS2 配套只作旁路：RViz2 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位；Foxglove 用于部署 bridge 后的浏览器远程观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
- 2026-06-30 21:15 CST 起，`/map` 直达地图页必须隐藏普通连接、相机、雷达、自由移动、行程等非地图卡片，只保留地图面板、缩放、刷新和 ROS2 配套说明，使 PC 第二屏真正成为近全屏地图观察页；`?view=map` 同样保持兼容。该模式仍只改变 CSS/DOM 显示，不启动 ROS2/RViz2/Foxglove，不执行 Nav2，不发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。ROS2 现成配套继续采用 RViz2：`ros2 launch ros2_trashbot_bringup rviz.launch.py`，用于工程观察 `/map`、`/scan`、`/camera/image_raw`、TF、`/plan`、`/local_plan`、`/amcl_pose` 和 costmap；普通用户不需要离开 PC `7001/map` 地图大屏。
...
- 2026-07-01 08:35 CST 起，普通首屏地图必须继续按 PC 主视图处理：历史默认曾为 `400%` 现场大图，当时默认为 `100%` 完整态势、最高 `4800%` 细节放大、桌面外壳接近全宽、大图高度贴近整屏，只看地图模式使用普通用户文案并保留路线、小车位置和雷达贴图；`完整态势` 可回到 `100%` 全图。ROS2 配套分层为 RViz2 / `nav2_rviz_plugins` 做工程调试，Foxglove / `foxglove_bridge` 做浏览器远程观察；普通用户默认不离开简易 PC 工作站。
- 2026-07-03 03:05 CST 起，ROS2 地图配套入口必须在上车环境真实可用：`ros2_trashbot_bringup/package.xml` 明确声明 `foxglove_bridge` 运行依赖，`ros2 launch ros2_trashbot_bringup foxglove_bridge.launch.py --show-args` 必须能在上位机返回 `address=0.0.0.0`、`port=8765`、`use_sim_time=false`、`sysinfo=true`。PC 普通用户仍默认使用 `/map` 大地图；Foxglove 只作为浏览器远程观察桥，topic 白名单限定地图、雷达、TF、路线、定位、costmap、相机和 sysinfo，不开放发布、service 或参数能力。
...
- 2026-06-30 13:17 CST 起，`/map` 与兼容 `?view=map` 直达地图大屏必须切换 App 页面壳到 map-only：隐藏普通顶栏和高级工具，`.shell` 使用 `100vw` / `100vh`，地图组件进入 full + 只看地图状态；当时默认使用 `100%` 完整态势，最高 `4800%` 细节放大，“完整态势”回到 `100%` 全图；同时继续暴露 `data-direct-map-view-behavior=page_shell_map_only` 与地图卡 `page_fixed_fullscreen_map_only`。该模式只为第二屏/PC 大屏观察地图、路线、小车位置和雷达贴图，不启动 RViz2/Foxglove/ROS2 runtime，不执行 Nav2，不发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-06-30 08:00 CST 起，普通首屏地图默认缩放提升到 `400%`，仍保留 `适配` 回到 `100%` 和最高 `400%`；地图卡继续暴露 RViz2/Foxglove 配套入口，工程调试用 RViz2 看 `/map`、`/scan`、TF、规划轨迹和定位，普通用户仍在 PC 工作站大地图完成操作。移动/导航卡新增 `plain-trip-execution-gauge` 行程仪表，结构化暴露图上行程点数、主按钮点击语义、是否会执行当前地图行程、托管 runtime、同窗口轮速 L/R、送达 success 是否匹配当前行程、固定执行代理 `/api/robot-control/nav2/goal/execute` 和固定送达代理 `/api/robot-control/delivery/complete`。仪表本身固定 `data-sends-motion-when-clicked=false`，只作为只读验收合同，不新增运动入口。
...
- 2026-06-30 06:05 CST 起，普通首屏地图默认缩放已提升到 `400%`，当时缩放档位支持到 `1200%`，大图高度按当前 PC 主画布口径执行，并暴露 `data-default-map-zoom-percent=400%`。普通操作继续留在 PC 工作站大地图；ROS2 工程观察首选 RViz2，远程浏览器观察可接 Foxglove Bridge/Studio，但 PC 页面不会自动启动这些 ROS2 runtime。
...
- 2026-06-30 05:32 CST 起，普通首屏地图新增内置缩放控件，默认 `125%`，支持 `+`、`-` 和 `适配`。缩放作用在同一个地图 overlay frame 上，底图、路线、小车位置、雷达点和目标点一起放大；DOM 暴露 `data-map-zoom-scale`、`data-map-zoom-percent` 和 `data-map-zoom-affects=image-route-robot-radar-target`，用于验收地图看大后仍然所见即所得。该变化只改 PC Web 只读显示，不启动 RViz2 或 ROS2 runtime，不执行 Nav2，不发送 manual/keyboard/free-roam/stop 或 `/cmd_vel`。
...
- `o7ConsumerReadAdapter.ts` 只允许本机 HTTP 回环 base URL，把 O6 `GET /api/o6/consumer/tasks` 和 `GET /api/o6/consumer/tasks/<task_id>` 压成 PC 端 `trashbot.pc_tools_workstation.o7_consumer_task_list.v1` / `trashbot.pc_tools_workstation.o7_consumer_task_detail.v1` 摘要；固定 `view=summary`，detail 默认 include `trajectory,events,evidence,field_evidence,labeling,inference,tunnel,artifact_access_probe,offline_artifact_seed_smoke,route_root_seed_gate,route_bag_evidence,route_bag_payload_replay,route_bag_semantic_replay,route_bag_full_semantic_decode_matrix,route_bag_pose_progress_replay,nav2_goal_execution_evidence,delivery_result_evidence,route_execution_result_delivery_readiness,route_delivery_closure_packet,same_task_field_material_packet,same_task_replay_packet_readback,bounded_route_execution_gate_material,bounded_route_terminal_result_material,current_field_evidence_material,pc_live_nav2_execution_material,clean_baseline_nav2_path_material,localization_path_material_readback,same_task_route_execution_material_packet,same_task_mission_evidence_gate,field_operator_confirmation_material,phone_browser_terminal_material`。坏 URL、非回环、credentials/query/hash、schema mismatch、fetch 失败、非 object、危险 true、unsafe ref、unsafe text、proof scope mismatch、`route_bag_required=true` 或 `route_bag_present=true` 都保持 fail-closed，不展示 token、绝对路径、base64、allowlist root、控制字段、原始 payload 或原始路径。只有远端缺失 field evidence 且可选 query `fieldEvidenceManifestJson=<local-json>` 指向合法 manifest 时，才用本地 manifest 补齐 `field_evidence`；`trajectory/events/evidence/labeling/inference/tunnel/artifact_access_probe/offline_artifact_seed_smoke/route_root_seed_gate/route_bag_evidence/route_bag_payload_replay/route_bag_semantic_replay/nav2_goal_execution_evidence/delivery_result_evidence/route_execution_result_delivery_readiness/route_delivery_closure_packet/same_task_field_material_packet/same_task_replay_packet_readback/bounded_route_execution_gate_material/bounded_route_terminal_result_material/current_field_evidence_material/pc_live_nav2_execution_material/clean_baseline_nav2_path_material/localization_path_material_readback/same_task_route_execution_material_packet/same_task_mission_evidence_gate/field_operator_confirmation_material/phone_browser_terminal_material` 仍全部来自远端 O6 detail。新增 `pc_live_nav2_execution_material` 只展示 `source_sprint`、`goal_accepted`、`goal_result_status`、UART/base command/IMU 摘要、`base_feedback_lr_nonzero_proven=false`、blocked reasons 和 next required evidence，不把现场 live Nav2 摘要解释成真实 route execution success、delivery success 或控制准入；字段读取优先 canonical `goal_accepted` / `goal_result_status`，并兼容 `nav2_goal_accepted`、`result_status`、`nav2_terminal_status`、`terminal_status` alias。adapter 会把 `source_origin`、`task_origin`、`manifest_run_id`、`artifact_health_summary`、present/missing artifacts、`manifest_gate`、`artifact_status`、`not_proven`、`safe_to_control`、`delivery_success`、`primary_actions_enabled` 显式带到 O7 页面。
- 同一个 `o7ConsumerReadAdapter.ts` 还提供 `POST /api/o7/consumer-read/tasks/<task_id>/inference/request?baseUrl=<local-loopback-url>`。该 action 只接受小型白名单 body：`robot_id`、可选且必须匹配 path 的 `task_id`、可选 `inference_id`、`model_family`、`requested_outputs` 和 `inputs`；requested outputs 仅限 `elevator_door_state` / `floor_recognition`，input 仅保留 `input_id`、`input_type`、`evidence_ref`、`captured_at_ms` 与 primitive metadata。adapter 固定转发 O6 `POST /api/o6/archive/inference`，拒绝 unknown fields、数组/标量错配、unsafe refs、credential/raw/base64-like 内容、`/cmd_vel`、`/api/base/manual`、serial path、`baudrate`、traceback、token/password/secret/bearer 和任何真实模型/楼层/电梯识别 true claim。成功只接受 O6 `schema=trashbot.o6.model_inference.v1`、`source=local_mock_inference`、`proof_status=not_proven`、`archive_event_written=true`、`write_status=created|updated` 且固定 false fields 全部为 false 的响应，返回 `trashbot.pc_tools_workstation.o7_consumer_inference_request_result.v1` receipt；该 receipt 固定 `real_model_inference_success=false`、`real_floor_recognition_proven=false`、`real_elevator_door_state_proven=false`、`safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`、`connects_cloud_production=false`、`robot_control_executed=false`。
- 同一个 `o7ConsumerReadAdapter.ts` 还提供 `POST /api/o7/consumer-read/tasks/<task_id>/delivery-result/intake?baseUrl=<local-loopback-url>`。该 action 只接受 selected-task delivery result 白名单 body：`robot_id`、必须匹配 path 的 `task_id`、`record_status`、`delivery_result_claimed`、basename 级 `evidence_ref`、`dropoff_confirmation_type`、`completed_at_utc`、可选短 `notes` 与 primitive metadata。adapter 固定转发 O6 `POST /api/o6/archive/field-evidence`，把请求包成最小 `trashbot.field_evidence_manifest.v1` gate 和 `trashbot.delivery_result_evidence.v1` additive section；拒绝 unknown fields、缺失/不一致身份、unsafe ref、URL、路径、credential/raw/base64-like 内容、ROS topic/control 字符串、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART 和任何真实能力 true claim。成功只接受 O6 `schema=trashbot.o6.field_evidence_archive.v1`、`source=local_mock_field_evidence_archive`、`proof_status=not_proven`、`archive_status=local_mock_field_evidence_ready`、`field_evidence_written=true`、`write_status=created|updated`、task/robot identity、delivery result evidence summary 和固定 false fields 全部匹配的响应，返回 `trashbot.pc_tools_workstation.o7_consumer_delivery_result_intake_result.v1` receipt；该 receipt 固定 `proof_scope=software_proof_o7_o6_consumer_delivery_result_intake_only`、`safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`、`connects_cloud_production=false`、`robot_control_executed=false`、`route_execution_success=false`、`hil_pass=false`、`real_cloud_db_connected=false`、`real_oss_connected=false`。
- 同一个 `o7ConsumerReadAdapter.ts` 还提供 `POST /api/o7/consumer-read/tasks/<task_id>/phone-browser-proof/intake?baseUrl=<local-loopback-url>`。该 action 只接受 selected-task phone/browser terminal material 白名单 body：`robot_id`、必须匹配 path 的 `task_id`、basename/token 级 `safe_evidence_ref`、`terminal_result_type`、`accepted_materials`、可选 `missing_materials` / `rejected_materials`、`captured_at_utc` 与 primitive metadata。adapter 固定转发 O6 `POST /api/o6/archive/field-evidence`，把请求包成最小 `trashbot.field_evidence_manifest.v1` gate 和 `trashbot.o6.phone_browser_terminal_material.v1` additive section；拒绝 unknown fields、缺失/不一致身份、unsafe ref、完整 URL、路径、credential/raw body/token/local path/base64-like 内容、ROS topic/control 字符串、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART 和任何真实能力 true claim。成功只接受 O6 `schema=trashbot.o6.field_evidence_archive.v1`、`source=local_mock_field_evidence_archive`、`proof_status=not_proven`、`archive_status=local_mock_field_evidence_ready`、`field_evidence_written=true`、`write_status=created|updated`、task/robot identity、`phone_browser_terminal_material_written=true`、`phone_browser_terminal_material_readback=true`、`same_task_id_consumed=true` 和固定 false fields 全部匹配的响应，返回 `trashbot.pc_tools_workstation.o7_phone_browser_proof_intake_result.v1` receipt。UI 展示 `phone_browser_terminal_material_written/readback`、`same_task_id_consumed`、`safe_evidence_ref`、accepted/missing/rejected materials 和固定 false fields；proof boundary 固定 `software_proof_o6_o7_phone_browser_terminal_material_intake_only`，并固定 `safe_to_control=false`、`delivery_success=false`、`route_execution_success=false`、`hil_pass=false`、`connects_cloud_production=false`、`robot_control_executed=false`。该入口只证明 phone/browser terminal material 摘要进入 O6/O7 readback，不证明真实手机/浏览器验收、production cloud、route execution、delivery success、HIL 或安全控制。
- 同一个 `o7ConsumerReadAdapter.ts` 还提供 `POST /api/o7/consumer-read/tasks/<task_id>/bounded-route-terminal-result/intake?baseUrl=<local-loopback-url>`。该 action 只接受 selected-task terminal result 白名单 body：`robot_id`、必须匹配 path 的 `task_id`、固定 `packet_id`、固定 `route_intent_id`、固定 `result_code=mock_route_execution_completed_not_live_delivery`、固定 `terminal_result_state=terminal_result_recorded`、固定 `reconciliation_state=terminal_result_recorded`、固定 `source_schema=trashbot.o5.bounded_route_terminal_result_bridge.v1`、固定 `source_proof_boundary=software_proof_o5_bounded_route_terminal_result_bridge_only`、28/28/27 计数、basename 级 `safe_evidence_ref` 与 primitive metadata。adapter 固定转发 O6 `POST /api/o6/archive/field-evidence`，把请求包成最小 `trashbot.field_evidence_manifest.v1` gate 和 `trashbot.o6.bounded_route_terminal_result_material.v1` additive section；拒绝 unknown fields、非 selected task、缺失/不一致身份、unsafe ref、完整 URL、路径、credential/raw body/token/local path/base64-like 内容、ROS topic/control 字符串、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART 和任何真实能力 true claim。成功只接受 O6 `schema=trashbot.o6.field_evidence_archive.v1`、`source=local_mock_field_evidence_archive`、`proof_status=not_proven`、`archive_status=local_mock_field_evidence_ready`、`field_evidence_written=true`、`write_status=created|updated`、task/robot identity、`bounded_route_terminal_result_material_written=true`、`bounded_route_terminal_result_material_readback=true`、`same_task_id_consumed=true` 和固定 false fields 全部匹配的响应，返回 `trashbot.pc_tools_workstation.o7_bounded_route_terminal_result_intake_result.v1` receipt。UI 展示 `bounded_route_terminal_result_material`、result/terminal/reconciliation state、same-task identity、write/readback 状态、`proof_scope=software_proof_o6_o7_bounded_route_terminal_result_intake_only` 和固定 `delivery_success=false`、`route_execution_success=false`、`safe_to_control=false`、`hil_pass=false`、`robot_control_executed=false`、`connects_cloud_production=false`。该入口只证明 O5 mock terminal-result 摘要进入 O6/O7 readback，不证明真实 route execution、delivery success、HIL、production cloud 或安全控制。
- 同一个 `o7ConsumerReadAdapter.ts` 还提供 `POST /api/o7/consumer-read/tasks/<task_id>/events/append?baseUrl=<local-loopback-url>`。该 action 只接受单条 selected-task mission event 白名单 body：`robot_id`、必须匹配 path 的 `task_id`、`event_id`、O6 allowed `event_type`、`occurred_at_ms`、至少一个安全 `evidence_ref`/`evidence_refs`、可选 `summary` / `severity` / primitive `metadata`。adapter 固定转发 O6 `POST /api/o6/archive/events`，拒绝 unknown fields、缺失/不一致身份、unsupported event type、无效时间戳、过大 metadata、unsafe ref、credential/raw/base64-like 内容、完整 URL、绝对路径、ROS topic/control 字符串、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART 和任何真实能力 true claim。成功只接受 O6 `schema=trashbot.o6.archive_events.v1`、`source=local_mock_event_archive`、`proof_status=not_proven`、`archive_event_written=true`、`write_status=created|updated`、task/robot/event identity、`event_summary` 和固定 false fields 全部匹配的响应，返回 `trashbot.pc_tools_workstation.o7_consumer_mission_event_append_result.v1` receipt；该 receipt 固定 `safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`、`connects_cloud_production=false`、`robot_control_executed=false`、`route_execution_success=false`、`hil_pass=false`、`real_cloud_db_connected=false`、`real_oss_connected=false`。
- 同一个 `o7ConsumerReadAdapter.ts` 还提供 `POST /api/o7/consumer-read/tasks/<task_id>/operator/dropoff-acceptance/request?baseUrl=<local-loopback-url>`。该 action 只接受 selected-task operator dropoff capture 白名单 body：`robot_id`、必须匹配 path 的 `task_id`、`event_id`、`occurred_at_ms`、至少一个安全 `evidence_ref`/`evidence_refs`、可选 `operator_action_id` / `operator_display_name` / `summary` / primitive `metadata`。adapter 固定转发 O6 `POST /api/o6/archive/events` 并强制写入 `event_type=operator.dropoff_acceptance`，拒绝 caller 覆盖 `event_type`、unknown fields、缺失/不一致身份、无效时间戳、unsafe ref、credential/raw/base64-like 内容、完整 URL、绝对路径、ROS topic/control 字符串、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART 和任何真实能力 true claim。成功只接受 O6 `schema=trashbot.o6.archive_events.v1`、`source=local_mock_event_archive`、`proof_status=not_proven`、`archive_event_written=true`、`write_status=created|updated`、task/robot/event identity、`event_type=operator.dropoff_acceptance`、`event_summary` 和固定 false fields 全部匹配的响应，返回 `trashbot.pc_tools_workstation.o7_operator_dropoff_action_capture_result.v1` receipt；该 receipt 固定 `proof_boundary=software_proof_o6_o7_operator_dropoff_action_capture_only`、`real_operator_action_proven=false`、`delivery_success=false`、`route_execution_success=false`、`safe_to_control=false`、`hil_pass=false`、`robot_control_executed=false`、`connects_cloud_production=false`。
- 对应的 `o7_operator_dropoff_browser_artifact.json` 由 `pc-tools/workstation/test/App.test.ts` 的 local DOM/Vitest 流程生成，只复用上述 action receipt，不新增 adapter/server endpoint。artifact schema 为 `trashbot.pc_tools_workstation.o7_operator_dropoff_browser_artifact.v1`，proof boundary 为 `software_proof_o7_operator_dropoff_browser_artifact_only`；它只证明浏览器 UI 能触发 selected-task local/mock action 并渲染 receipt/fixed false fields，不证明真实手机、production cloud/DB/queue/OSS/CDN/4G/SIM、real operator action、delivery success、route execution、HIL、safe-to-control、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART 或 robot movement。
- 同一个 `o7ConsumerReadAdapter.ts` 还提供 `POST /api/o7/consumer-read/tasks/<task_id>/voice/tts-draft/request?baseUrl=<local-loopback-url>`。该 action 只接受单条 selected-task voice/TTS 草稿事件白名单 body：`robot_id`、必须匹配 path 的 `task_id`、`event_id`、basename 级 `evidence_ref`、`occurred_at_ms`、短 `draft_text`、可选 `voice_profile` / `locale` / primitive `metadata`。adapter 固定转发 O6 `POST /api/o6/archive/events` 并强制写入 `event_type=voice.tts_draft`，拒绝 unknown fields、缺失/不一致身份、空/过长 draft、unsafe ref、credential/raw/base64-like 内容、完整 URL、绝对路径、音频 payload、真实 voice API 文案、ROS topic/control 字符串、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART 和任何真实能力 true claim。成功只接受 O6 `schema=trashbot.o6.archive_events.v1`、`source=local_mock_event_archive`、`proof_status=not_proven`、`archive_event_written=true`、`write_status=created|updated`、task/robot/event identity、`event_type=voice.tts_draft` 和固定 false fields 全部匹配的响应，返回 `trashbot.pc_tools_workstation.o7_voice_tts_draft_request_result.v1` receipt；该 receipt 固定 `proof_boundary=software_proof_o6_o7_voice_tts_draft_event_write_only`、`tts_send_enabled=false`、`speaker_dispatch_enabled=false`、`real_voice_api_connected=false`、`real_asr_tts_runtime_connected=false`、`safe_to_control=false`、`delivery_success=false`、`robot_control_executed=false`、`connects_cloud_production=false`。
- 同一个 `o7ConsumerReadAdapter.ts` 还提供 `GET /api/o7/consumer-read/tasks/<task_id>/mission-evidence/export?baseUrl=<local-loopback-url>&format=json`。该 action 只在 UI 已选中并加载同一个 O6 consumer detail 后可点；PC adapter 固定重新读取 selected task 的 O6 detail，把 mission events、field evidence、same-task replay packet/readiness、delivery result/readiness、route/closure/material sections 聚合成 `trashbot.pc_tools_workstation.o7_mission_evidence_bundle_export_result.v1` receipt，其中 `counts.material_section_count` 显式覆盖 `bounded_route_execution_gate_material` 与 `bounded_route_terminal_result_material`。receipt 只展示 mission evidence bundle 的 schema/status/proof scope、数量、basename refs、blocked reasons、not_proven、same-task identity 和固定 false fields；不回显 raw artifact body、真实路径、完整 URL、token 或真实 dataset。坏 URL、非回环、credentials/query/hash、非 `format=json`、task mismatch、schema mismatch、危险 true 字段、unsafe refs/content 全部 fail-closed。成功状态为 `local_mock_mission_evidence_bundle_ready`，proof scope 固定 `software_proof_o7_o6_mission_evidence_bundle_export_only`，并继续展示 `safe_to_control=false`、`delivery_success=false`、`route_execution_success=false`、`hil_pass=false`、`connects_cloud_production=false`、`robot_control_executed=false`、`real_cloud_db_connected=false`、`real_oss_connected=false`；该 receipt 不归档，不证明 production cloud、route execution、delivery success、HIL、safe-to-control、real dataset export 或 O5 external evidence。
- `same_task_mission_evidence_gate` 是 O7 consumer detail 的只读 mission 配对摘要，展示同一 `task_id` 下 O5 terminal/cloud source、`trashbot.cloud_command_terminal_result.v1` source schema、route execution materials status、linked flags、blocked reasons 和 next required evidence。即使 gate 为 `same_task_mission_gate_ready_not_success_proof`，页面仍展示 `delivery_success=false`、`safe_to_control=false`、`connects_cloud_production=false`，不把配对证据解释成真实送达成功。
- `field_operator_confirmation_material` 是 O7 consumer detail 的只读现场 operator 材料摘要，schema 为 `trashbot.pc_tools_workstation.o7_field_operator_confirmation_material.v1`。它从 O6 top-level、`field_evidence`、`field_motion_evidence_packet`、`artifact_bundle`、`artifact_bundle_consumer_ingest` 或 `artifact_bundle_readiness` 读取并归一 operator report/confirmation present 与 status、安全预检布尔位、observed motion/stop、same-task consumed、linked route/delivery material、blocked reasons、next required evidence 和 material summaries。ready status 固定为 `field_operator_confirmation_material_ready_not_delivery_proof`，proof scope 固定为 `software_proof_field_operator_confirmation_material_only`；缺失、task mismatch、dangerous true、unsafe text/list 或 proof scope mismatch 都 fail-closed。页面只读展示这些字段，不新增控制按钮，并固定 `delivery_success=false`、`safe_to_control=false`、`primary_actions_enabled=false`、`robot_control_executed=false`、`route_execution_success=false`、`hil_pass=false`、`connects_cloud_production=false`。
- `same_task_field_material_packet` 是 O7 consumer detail 的只读准现场材料摘要，展示同一 `task_id` 下 `map_yaml`、`route_csv`、keyframes、route bag / rosbag、replay JSONL 是否已被消费，包含 `same_task_id_consumed`、`live_or_field_material_consumed`、present/missing materials、top-level `sample_refs`，以及 per-material `basename`、`size_bytes`、`sha256_prefix`、`sample_refs`、`count`、`present` 摘要。它兼容上游 `material_summaries`、`material_sample_refs`、`sample_ref_summaries`，也兼容旧 O6 dict-shaped `sample_refs`；`map_yaml` 缺失只记 optional gap，不阻断 route/keyframe/route bag/replay 的材料展示。即使 packet 为 `ready_not_delivery_proof`，页面仍展示 `delivery_success=false`、`safe_to_control=false`、`connects_cloud_production=false`，不把材料可读解释成真实 route execution 或真实送达成功。
- `same_task_replay_packet_readback` 是 O7 consumer detail 的只读 replay packet identity/count 摘要，schema 为 `trashbot.pc_tools_workstation.o7_same_task_replay_packet_readback.v1`。它从 O6 top-level、`field_evidence`、`field_motion_evidence_packet`、`artifact_bundle`、`artifact_bundle_consumer_ingest` 或 `artifact_bundle_readiness` 读取 05:02 O3/O6 same-task replay packet，只展示 exact `packet_id`、`task_id`、`route_intent_id`、`route_csv_row_count=28`、`replay_jsonl_event_count=28`、`path_structured_pose_count=28`、`same_task_replay_packet_ready=true`、basename refs、sha256 prefix、blocked reasons、next required evidence 和固定 false fields。它独立于 `same_task_route_execution_material_packet`，即使 ready 也仍展示 `route_execution_success=false`、`delivery_success=false`、`hil_pass=false`、`safe_to_control=false`、`robot_control_executed=false`、`primary_actions_enabled=false`、`publishes_cmd_vel=false`、`calls_base_manual=false`、`uses_base_uart=false`、`connects_cloud_production=false`。
- `same_task_route_execution_material_packet` 是 O7 consumer detail 的只读路线执行材料摘要，消费 O6 `trashbot.o6.same_task_route_execution_material_packet.v1`，独立展示 O6 顶层 status、same-task identity、present/missing route execution materials、route execution result summary、pose progress/replay timeline summary、blocked reasons、next required evidence 和固定 false flags。`2026-07-10 12:20 CST` 起，O7 还会把 `live_or_field_command_evidence_present`、`delivery_or_operator_material_consumed`、`route_execution_credit_candidate`、`credit_support_only_reason`、`credit_required_evidence` 作为 route execution credit material summary 单独展示，并把 `route_execution_credit_candidate=false` 明确展示为 support-only/blocked。即使 `route_execution_credit_candidate=true`，页面也不会把它升级成 `delivery_success=true`、`safe_to_control=true` 或任何可点击主动作。Checklist 只能引用这个 packet，不替代 packet 自身验收；O7 不从 child readiness 推导 `delivery_success=true`、`safe_to_control=true` 或 primary action enabled。
- `clean_baseline_nav2_path_material` 是 O7 consumer detail 的只读 clean-baseline no-motion Nav2 path 预检材料摘要，消费 O6 `trashbot.o6.clean_baseline_nav2_path_material.v1`，独立展示 first failure、retry status、path_point_count、planner/AMCL/initialpose/no-motion cleanup 状态、blocked reasons、next required evidence 和固定 false flags。它只证明 clean-baseline path preflight 材料已被同一 `task_id` 主路径消费，不证明真实 route execution、delivery success、safe_to_control=true 或任何可点击主动作。
- `localization_path_material_readback` 是 O7 consumer detail 的只读 same-run localization/path 材料摘要，schema 为 `trashbot.pc_tools_workstation.o7_localization_path_material_readback.v1`。它从 O6 top-level、`field_evidence`、`field_motion_evidence_packet`、`artifact_bundle`、`artifact_bundle_consumer_ingest` 或 `artifact_bundle_readiness` 读取并归一 same-run map/localization/TF/planner/path 请求字段，以及 cross-run clean-baseline comparator boundary。`2026-07-10 21:27` 的 O7 返工同时兼容 O6 初版 `status=localization_path_material_readback_ready_not_route_execution_proof` 与最终 `status=localization_path_material_ready_not_route_execution_proof`，也兼容旧 TF aliases `same_run_tf_map_to_odom_observed` / `same_run_tf_map_to_base_link_observed`；若 `localization_path_material_bridge_present` 缺失，但 payload 命中 ready status 且 `same_run_localization_material_present=true`，bridge 仍按兼容 alias 视为 true，避免把真实 O6 payload 误降级。O7 固定把 `same_run_path_generation_succeeded=false`、`same_run_path_generated=false`、`same_run_path_point_count=0`、`same_run_path_proven=false` 和 `nav2_route_execution_success=false` 展示为当前结论；cross-run comparator 只允许显示历史 `path_point_count` 等对照字段，不能覆盖 same-run false 结论。缺失、task mismatch、dangerous true、unsafe text/list、unsupported schema/proof scope 或任何 same-run path success claim 都 fail-closed。
...
- O6 Consumer Read API：`GET /api/o6/consumer/tasks` 和 `GET /api/o6/consumer/tasks/<task_id>` 是当前推荐给 PC/后续手机消费的统一查询面。它在 relay 后端复用同一个 `TRASHBOT_O6_CLOUD_ARCHIVE_STATE`，把 archive task、events、evidence refs、labels、`model_inference.*` timeline、tunnel latest known snapshot、可选 `artifact_access_probe`、`offline_artifact_seed_smoke`、`route_root_seed_gate`、`route_bag_evidence`、`route_bag_payload_replay`、`field_motion_evidence_packet` 和 `nav2_goal_execution_evidence` 聚合成 `trashbot.o6.consumer_read.v1` 读模型，支持 `view=summary` 与 `include=trajectory,events,evidence,field_evidence,labeling,inference,tunnel,artifact_access_probe,offline_artifact_seed_smoke,route_root_seed_gate,route_bag_evidence,route_bag_payload_replay,route_bag_semantic_replay,route_bag_full_semantic_decode_matrix,route_bag_pose_progress_replay,nav2_goal_execution_evidence,delivery_result_evidence,route_execution_result_delivery_readiness,route_delivery_closure_packet,same_task_field_material_packet,same_task_replay_packet_readback,current_field_evidence_material,pc_live_nav2_execution_material,clean_baseline_nav2_path_material,same_task_route_execution_material_packet,same_task_mission_evidence_gate,field_operator_confirmation_material,phone_browser_terminal_material,bounded_route_terminal_result_material`。PC 读历史任务、详情、轨迹、事件、打标状态、tunnel 在线态、artifact access probe、offline seed smoke、route-root seed gate、route bag evidence / payload replay、field motion evidence packet、Nav2 goal evidence、delivery result evidence、phone/browser terminal material、bounded route terminal result material 和 same-task replay packet readback 时应优先走这两个 endpoint，而不是在前端重复 join 多条 `/api/o6/archive/*`、`/api/o6/tunnel/*`。该接口仍固定 `proof_status=not_proven`、`safe_to_control=false`、`connects_cloud_production=false`、`robot_control_executed=false`；`selected` 只是 store 单选标记，不等于 UI 用户选择；`tunnel_status` 明确是 latest known robot snapshot，不是 task 内历史事实；没有 labels / inference / tunnel / probe / route-root seed gate / route bag payload replay / field motion packet / Nav2 goal evidence / delivery result evidence / phone-browser terminal material / bounded route terminal result material / same-task replay packet readback 时必须显式返回 `pending`、`absent`、`blocked_not_proven`、`artifact_access_probe_missing`、`route_root_seed_gate_missing`、`route_bag_payload_replay_missing`、`field_motion_evidence_packet_missing`、`nav2_goal_execution_evidence_missing`、`phone_browser_terminal_material_missing` 或 `same_task_replay_packet_readback_not_available` 等 fail-closed 摘要，不得伪造“真实没有异常”。
- O7 Consumer Detail Route Replay：O7 Previews 的 `O7 consumer read primary path` 现在把历史回放主路径切到 PC 后端 `GET /api/o7/consumer-read/tasks` 和 `GET /api/o7/consumer-read/tasks/<task_id>` adapter。operator 加载 task list 后选择 task，可选填写 `Local field evidence manifest JSON`，再加载 detail；UI 直接展示 `trajectory / events / evidence / labeling / inference / tunnel_status` 摘要，以及 local manifest query、field evidence contract/input/fail-closed reason、O6 wrapper `source_origin`、`task_origin`、manifest run 和 artifact health。本地 manifest fallback 只补 `field_evidence`，不得覆盖远端 trajectory/events/evidence/labeling/inference/tunnel；远端已有合法 field evidence 时优先远端。detail 现在同时带 `route_bag_evidence` 与 `route_bag_payload_replay` 两层只读摘要：前者保留 DB3 topic/message/timestamp 结构，后者再展示 topic/message/timestamp 摘要、payload size/hash prefix、blocked reasons、next evidence 和 false safety fields；`same_task_field_material_packet`、`same_task_replay_packet_readback`、`current_field_evidence_material` 与 `same_task_route_execution_material_packet` 也各自作为只读摘要分段展示，其中 `same_task_replay_packet_readback` 只说明 05:02 replay packet exact identity/count/readback，`current_field_evidence_material` 只说明 current field status、present/missing materials、camera/radar/map/nav2/manual gate booleans 和 support-only blocked 语义，不表示 route execution success。四者都不能打开 submit/control/action。`Consumer-detail route replay player` 的 Play/Pause、Previous/Next、Reset 和 range cursor 只改浏览器内存，不调用 API、不写后端、不发送机器人命令；缺 detail、unknown task、task id mismatch、轨迹缺失、blocked/not_proven/error/cancel 状态或 adapter fail-closed 时禁用导航并显示 blocker。`Consumer-detail trajectory minimap` 只使用 detail 样本中的有限数值型 `x_m/y_m` 归一化到固定 SVG viewBox；没有有效点时显示 `blocked_not_proven`，单点只显示 `readonly_consumer_detail_trajectory_single_point`，不声明真实地图或真实机器人位置。旧 `Cloud Archive Tasks` fixture route replay player 仍保留，但其 cursor 与 consumer 主路径 cursor 隔离，只作为次路径 / debug fallback。
...
- O7 Previews 新增 `O7 consumer read primary path` 区块，作为 O7 任务列表/详情与标注队列检查的首选读取入口。operator 只需提供本机回环 relay base URL，点击 `Load consumer task list` 后，workstation 后端固定请求 `GET /api/o6/consumer/tasks?view=summary&limit=50`；点击 `Load consumer task detail` 后，workstation 后端固定请求 `GET /api/o6/consumer/tasks/<task_id>?view=default&include=trajectory,events,evidence,field_evidence,labeling,inference,tunnel,artifact_access_probe,offline_artifact_seed_smoke,route_root_seed_gate,route_bag_evidence,route_bag_payload_replay,route_bag_semantic_replay,route_bag_full_semantic_decode_matrix,route_bag_pose_progress_replay,nav2_goal_execution_evidence,delivery_result_evidence,route_execution_result_delivery_readiness,route_delivery_closure_packet,same_task_field_material_packet,same_task_replay_packet_readback,current_field_evidence_material,pc_live_nav2_execution_material,clean_baseline_nav2_path_material,same_task_route_execution_material_packet,same_task_mission_evidence_gate,field_operator_confirmation_material,phone_browser_terminal_material,bounded_route_terminal_result_material`，并可附带 `fieldEvidenceManifestJson=<local-json>` 作为远端缺 field evidence 时的本地只读补齐输入。UI 明确展示 `view=summary`、detail `include=` 策略、`artifact_access_probe` counts / sample refs / sha256 prefix、`route_root_seed_gate` 的 route-root seed status、`route_bag_required=false`、`route_bag_present=false`、frame/ref counts、blocked reasons、next required evidence，`field_motion_evidence_packet` 的 route frame count、nonzero displacement、displacement meters、motion log sources、route_bag/live log present gap、blocked reasons 和 `next_required_evidence`，`same_task_replay_packet_readback` 的 exact packet/task/route intent identity、28/28/28 counts 与固定 false fields，以及 `nav2_goal_execution_evidence` 的 goal/result/base command 摘要、blocked reasons 和 next required evidence；`safe_to_control=false`、`primary_actions_enabled=false`、`delivery_success=false`、`route_execution_success=false`、`hil_pass=false`、`connects_cloud_production=false`、`robot_control_executed=false`、`media_access_proven=false`、`real_oss_connected=false`、`real_cdn_connected=false`、blocked reasons、not proven、样本计数和 `tunnel_status.temporal_alignment=latest_known_robot_snapshot_not_task_aligned` 持续可见。consumer-detail inference request 控件在加载 selected task detail 后才启用，允许 operator 选择 `elevator_door_state` / `floor_recognition`、填写安全 input/ref/timestamp，并通过 PC adapter 请求 O6 local/mock inference write；receipt 展示 `request_status`、`write_status`、created/updated count、`archive_event_written`、O6 schema/source 和固定 false fields。consumer-detail delivery result intake 控件同样只在 selected task detail 已加载后启用，允许 operator 针对同一 `task_id` 提交 local/mock `delivery_result_evidence` 请求，并通过 PC adapter 请求 O6 local/mock `field-evidence` write；receipt 展示 `intake_status`、`record_status`、`delivery_result_claimed`、`operator_confirmation_present`、`field_evidence_written`、write status、O6 schema/source、`proof_scope=software_proof_o7_o6_consumer_delivery_result_intake_only` 和固定 false fields。consumer-detail phone/browser proof intake 控件同样只在 selected task detail 已加载后启用，允许 operator 针对同一 `task_id` 提交 local/mock `phone_browser_terminal_material` 安全摘要，并通过 PC adapter 请求 O6 local/mock `field-evidence` write/readback；receipt 展示 `phone_browser_terminal_material_written`、`phone_browser_terminal_material_readback`、`same_task_id_consumed`、`safe_evidence_ref`、accepted/missing/rejected materials、`proof_scope=software_proof_o6_o7_phone_browser_terminal_material_intake_only` 和固定 `safe_to_control=false`、`delivery_success=false`、`route_execution_success=false`、`hil_pass=false`、`connects_cloud_production=false`、`robot_control_executed=false`。consumer-detail mission event append 控件同样只在 selected task detail 已加载后启用，允许 operator 填写 O6 白名单事件类型、安全 evidence ref、timestamp、summary/severity，并通过 PC adapter 请求 O6 local/mock `archive/events` write；receipt 展示 `append_status`、`event_id`、`event_type`、`archive_event_written`、write status、created/updated count、O6 schema/source 和 `safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`、`connects_cloud_production=false`、`robot_control_executed=false`、`route_execution_success=false`、`hil_pass=false`、`real_cloud_db_connected=false`、`real_oss_connected=false`。consumer-detail labeling primary path 与 route replay primary path 都会带上同一 `task_id` 的 field motion summary、same-task replay packet readback 和 Nav2 goal summary；该区块不绕过 relay、不连公网云、不发控制命令，也不把缺 labels / inference / delivery result intake receipt / phone-browser proof intake receipt / bounded route terminal result intake receipt / mission event append receipt / tunnel / probe / route-root seed gate / field motion packet / same-task replay packet readback / Nav2 goal evidence 解释成真实“无异常”。
- O7 consumer read primary path 现在还提供 selected-task `voice/speaker-ack/request` action。operator 必须先加载同一 task detail，选择 `ack_status=ack` 或 `ack_status=failure` 后，PC adapter 才会固定请求 `POST /api/o7/consumer-read/tasks/<task_id>/voice/speaker-ack/request?baseUrl=<local-loopback-url>`，再由 Node 端转写 O6 `POST /api/o6/archive/events`。`ack_status=ack` 只写 `voice.speaker_ack`，`ack_status=failure` 只写 `voice.speaker_failure`；receipt schema 固定为 `trashbot.pc_tools_workstation.o7_voice_speaker_ack_event_result.v1`，proof boundary 固定为 `software_proof_o6_o7_voice_speaker_ack_event_write_only`，并展示 `speaker_ack_event_written` / `speaker_failure_event_written`、O6 schema/source、write status、evidence refs、`speaker_dispatch_enabled=false`、`real_speaker_ack_proven=false`、`tts_send_enabled=false`、`real_voice_api_connected=false`、`real_asr_tts_runtime_connected=false`、`safe_to_control=false`、`delivery_success=false`、`robot_control_executed=false`、`connects_cloud_production=false`。它只改善 operator 对 speaker ACK/failure event-write 的复盘入口，不证明真实音频播放、真实 speaker ACK、真实 voice API、真实 ASR/TTS runtime、production cloud、route execution、delivery、HIL 或 safe-to-control。
...
- `view=summary` 适用于移动端或列表内嵌只读快照；PC 需要逐项排障时可加 `include=trajectory,events,evidence,field_evidence,labeling,inference,tunnel,artifact_access_probe,offline_artifact_seed_smoke,route_root_seed_gate,route_bag_evidence,route_bag_payload_replay,route_bag_semantic_replay,route_bag_full_semantic_decode_matrix,route_bag_pose_progress_replay,nav2_goal_execution_evidence,delivery_result_evidence,route_execution_result_delivery_readiness,route_delivery_closure_packet,same_task_field_material_packet,same_task_replay_packet_readback,current_field_evidence_material,pc_live_nav2_execution_material,clean_baseline_nav2_path_material,same_task_route_execution_material_packet,same_task_mission_evidence_gate,field_operator_confirmation_material,phone_browser_terminal_material,bounded_route_terminal_result_material`。O7 adapter 现在还会优先消费同一条 O6 detail 主路径上的 `artifact_media_preflight`、`artifact_access_probe`、`offline_artifact_seed_smoke`、`route_root_seed_gate`、`route_bag_evidence`、`route_bag_payload_replay`、`route_bag_semantic_replay`、`route_bag_full_semantic_decode_matrix`、`field_motion_evidence_packet`、`same_task_replay_packet_readback`、`nav2_goal_execution_evidence`、`delivery_result_evidence` 和 `phone_browser_terminal_material`、`bounded_route_terminal_result_material`，并在 consumer detail 中额外派生 `artifact_bundle`、`artifact_bundle_consumer_ingest` 和 `artifact_bundle_readiness`；主路径优先从 O6 top-level `route_root_seed_gate` / `route_bag_evidence` / `route_bag_payload_replay` / `route_bag_semantic_replay` / `route_bag_full_semantic_decode_matrix` / `field_motion_evidence_packet` / `same_task_replay_packet_readback` / `nav2_goal_execution_evidence` / `delivery_result_evidence` / `phone_browser_terminal_material` / `bounded_route_terminal_result_material` / `artifact_bundle` / `artifact_bundle_consumer_ingest` / `field_evidence.artifact_bundle` / `field_evidence_consumer_ingest.artifact_bundle` 读取 route-root、route bag、payload replay、semantic replay、full semantic decode matrix、field motion、same-task replay packet readback、Nav2 goal、delivery result、phone/browser terminal material、bounded route terminal result material 和 route/replay/keyframe/evidence refs，旧 fallback 只做兼容兜底。`artifact_access_probe` 可从 top-level、`field_evidence`、`artifact_bundle` 或相关 ingest wrapper 读取；`offline_artifact_seed_smoke`、`route_root_seed_gate`、`route_bag_evidence`、`route_bag_payload_replay`、`route_bag_semantic_replay`、`route_bag_full_semantic_decode_matrix`、`field_motion_evidence_packet`、`same_task_replay_packet_readback`、`nav2_goal_execution_evidence`、`delivery_result_evidence` 、`phone_browser_terminal_material` 和 `bounded_route_terminal_result_material` 可从 top-level、`field_evidence`、`field_motion_evidence_packet`、`field_evidence_consumer_ingest` 或相关 wrapper 读取，只展示 software proof readiness、counts、basename sample refs、motion log sources、route bag topic/message/timestamp 摘要、payload size/hash prefix 摘要、LaserScan/Image/TF/Odometry 语义摘要、topic/type decode coverage 摘要、same-task replay packet identity/count、goal/result/base command 摘要、delivery record/operator confirmation 摘要、phone/browser terminal material write/readback 摘要、bounded route terminal result material terminal/reconciliation 摘要、blocked reasons 和 next required evidence，不证明真实媒体、真实生产云、真实 annotation API、真实 dataset export、真实 route_bag/live log、真实机器人运动、真实 Nav2 live run、真实 route execution 或真实送达。若 O6 暂时缺该 section，workstation 会把对应 section 保持 `blocked_not_proven`，继续把真实媒体可访问性、OSS/CDN 连通性、生产云能力和真实送达能力锁死为 not proven。
- 2026-07-09 起，PC `GET /api/o7/consumer-read/tasks/<task_id>` 会在 O6 detail 上派生两个 O7 MVP 字段：
  - `route_replay_mvp`：schema 为 `trashbot.pc_tools_workstation.o7_consumer_route_replay_mvp.v1`，围绕同一 `task_id` 输出 trajectory frame count、current frame、pose/velocity summary、events timeline、evidence refs、keyframe refs 和本地 cursor contract。`previous_frame`、`next_frame`、`reset_cursor`、`toggle_playing` 只代表浏览器本地 cursor 能力；`playback_available=false`、`safe_to_play=false`、`sends_to_robot=false` 固定不变。
  - `labeling_mvp`：schema 为 `trashbot.pc_tools_workstation.o7_consumer_labeling_mvp.v1`，围绕同一 `task_id` 输出 review item、media/evidence ref、current labels、draft labels、label schema、allowed label types，以及 `submit_receipt.status=submit_blocked_fail_closed`。`submit_enabled=false`、`rollback_enabled=false`、`dataset_export_available=false`、`real_annotation_api_connected=false`、`cloud_write_executed=false` 固定不变。
  - `artifact_media_preflight`：schema 为 `trashbot.o6.artifact_media_preflight.v1` 或 workstation 派生的 `derived_blocked_not_proven` 摘要。它把 route replay / labeling 共用的媒体依赖集中到 `route_replay_dependency` 与 `labeling_dependency`，只保留 `route_ref`、`replay_ref`、`keyframe_ref`、`sample_refs`、`review_item_media_refs`、blocked reasons 和 `next_required_evidence`；任何 evidence_ref 字符串都不能被 UI 解释成“媒体已可访问”。
- 2026-07-09 同轮新增 PC-only local/mock action adapter，浏览器不得直连 O6：
  - `POST /api/o7/consumer-read/tasks/<task_id>/annotations/submit?baseUrl=<loopback>` 由 Node adapter 转发到 O6 `POST /api/o6/archive/labels`，只接受当前 consumer detail labeling MVP 能证明的 task/review item/draft label，并返回 `trashbot.pc_tools_workstation.o7_annotation_submit_result.v1`。
  - `GET /api/o7/consumer-read/tasks/<task_id>/annotations/export?baseUrl=<loopback>&format=jsonl` 由 Node adapter 转发到 O6 `GET /api/o6/archive/labels/<task_id>/export?format=jsonl`，返回 `trashbot.pc_tools_workstation.o7_annotation_dataset_export_result.v1`。
  - 两个 adapter 都只允许本机 HTTP 回环 base URL，拒绝 credentials、query/hash、非 HTTP、非回环、空 task id、schema mismatch、O6 危险 true 字段、token/password/secret/credential/bearer 等敏感文本和未知 submit body 字段。
  - 两个结果都只说明 local/mock receipt 或 local/mock jsonl export 可被 PC 展示；`proof_status=not_proven`、`submit_enabled=false`、`dataset_export_available=false`、`real_annotation_api_connected=false`、`real_dataset_export_connected=false`、`cloud_write_executed=false`、`connects_cloud_production=false`、`safe_to_control=false`、`robot_control_executed=false` 必须固定。
- `artifact_bundle`：schema 为 `trashbot.o6.artifact_bundle.v1`，优先从 O6 主路径上的 bundle alias / wrapper 读取，输出同一 `task_id` 的 route/replay/keyframe/evidence refs、安全 request summary、固定 false fields 和只读 preflight 占位。任何危险 true 字段、unsafe ref、schema mismatch 或空 refs 都必须 fail closed。
- `artifact_bundle_consumer_ingest`：schema 为 `trashbot.o6.artifact_bundle_consumer_ingest.v1`，只补充 bundle ingest 的来源、run id、manifest gate 和 field evidence summary，不扩展真实写入能力边界。
- `artifact_bundle_readiness`：schema 为 `trashbot.pc_tools_workstation.o7_consumer_artifact_bundle_readiness.v1`，围绕同一 `task_id` 汇总 route/replay/keyframe/evidence/review item/sample/ref 计数、`artifact_access_probe` counts、route bag topic/message/timestamp 摘要、route bag payload replay 的 topic/message/timestamp、payload size/hash prefix 摘要、route bag pose progress replay 的 sample/decode/frame/pose/displacement 摘要、样本 refs、blocked reasons 和 next required evidence；route replay / labeling 的阻塞原因与样本媒体 refs 优先来自 bundle / preflight，route bag blocked reasons、pose progress blocked reasons 与 probe blocked reasons 会合并进 readiness，旧 fallback 只做兼容兜底。
- `same_task_replay_packet_readback`：schema 为 `trashbot.pc_tools_workstation.o7_same_task_replay_packet_readback.v1`，只消费 O6 `trashbot.o6.same_task_replay_packet_readback.v1` 或 O3 `trashbot.o3.same_task_route_replay_packet.v1` 白名单摘要，展示 exact source identity、28/28/28 counts、basename source refs、sha256 prefix、blocked reasons、next required evidence 和固定 false fields。缺 summary、task mismatch、bad schema/proof scope、危险 true、unsafe text/list、path/url/token/raw/base64 或任何控制/执行成功宣称全部 fail-closed；ready 只表示 readback 可用，不表示 route execution、delivery、HIL、safe-to-control、`/cmd_vel`、`/api/base/manual`、WAVE ROVER UART 或 production cloud 可用。
- `route_root_seed_gate`：schema 为 `trashbot.o6.route_root_seed_gate.v1`，只消费 O6 consumer detail 已脱敏摘要，展示 `route_root_seed_status`、`route_bag_required=false`、`route_bag_present=false`、route frame count、derived replay frame count、manifest / route / replay / keyframe / evidence ref counts、basename sample refs、blocked reasons 和 next required evidence。缺 summary、缺必填字段、schema mismatch、危险 true、unsafe refs / unsafe text、`route_bag_required=true` 或 `route_bag_present=true` 全部 fail-closed；O7 不读取 route root 本地路径，不暴露 token、绝对路径、base64、allowlist root 或控制字段，且继续固定 `safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`、`robot_control_executed=false`。
- `route_bag_evidence`：schema 为 `trashbot.route_bag_evidence.v1` 或 `trashbot.o6.route_bag_evidence.v1`，只消费 O6 consumer detail / `field_evidence` / `field_motion_evidence_packet` / artifact bundle wrapper 里的白名单脱敏摘要，展示 `source_label`、metadata/db3 读取状态、topic/message count、首末 timestamp、限量 topic 名称、blocked reasons 和 next required evidence。该 section 的 `proof_scope` 固定为 `software_proof_route_bag_evidence_intake_only`；缺 summary、缺必填字段、schema mismatch、proof scope mismatch、危险 true、unsafe text、URL、绝对路径、base64、raw/path/root、credential URL、串口路径、完整 DB3 内容或 `/cmd_vel` topic 全部 fail-closed。即使 topic/message/timestamp 摘要 ready，O7 也只把它当作只读 route bag evidence intake，不解锁真实控制、不宣称真实 route execution 或真实送达，且继续固定 `safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`、`robot_control_executed=false`。
- `route_bag_payload_replay`：schema 为 `trashbot.route_bag_payload_replay.v1` 或 `trashbot.o6.route_bag_payload_replay.v1`，只消费 O6 consumer detail / `field_evidence` / `field_motion_evidence_packet` / artifact bundle wrapper 里的白名单脱敏摘要，展示 `route_bag_source`、`source_label`、metadata/db3 读取状态、topic/message/timestamp 摘要、`payload_sample_count`、`payload_size_min_bytes/max_bytes/avg_bytes`、`payload_sha256_prefix_samples`、blocked reasons 和 next required evidence。该 section 的 `proof_scope` 固定为 `software_proof_route_bag_payload_replay_only`；缺 summary、缺必填字段、schema mismatch、proof scope mismatch、危险 true、unsafe text、unsafe topic、URL、绝对路径、base64、raw/content/path/root、credential URL、串口路径、完整 hash 或 `/cmd_vel` topic 全部 fail-closed。即使 payload replay 摘要 ready，O7 也只把它当作只读 payload replay intake，不解锁真实控制、不宣称真实 route execution 或真实送达，且继续固定 `safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`、`robot_control_executed=false`。
- `route_bag_semantic_replay`：schema 为 `trashbot.route_bag_semantic_replay.v1` 或 `trashbot.o6.route_bag_semantic_replay.v1`，只消费 O6 consumer detail / `field_evidence` / `field_motion_evidence_packet` / artifact bundle wrapper 里的白名单脱敏摘要，展示 `semantic_decode_status`、`semantic_topic_types`、`laser_scan_summary`、`image_summary`、`tf_summary`、Odometry/DiagnosticArray 已解码 topic/type，以及 blocked reasons 和 next required evidence。该 section 的 `proof_scope` 固定为 `software_proof_route_bag_semantic_replay_only`；缺 summary、缺必填字段、schema mismatch、proof scope mismatch、危险 true、unsafe text、unsafe topic、URL、绝对路径、base64、raw/content/path/root、credential URL、串口路径、完整 hash 或 `/cmd_vel` topic 全部 fail-closed。即使 semantic replay 摘要 ready，O7 也只把它当作只读语义回放 intake，不解锁真实控制、不宣称真实 route execution、真实 robot motion 或真实送达，且继续固定 `safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`、`robot_control_executed=false`。
- `route_bag_full_semantic_decode_matrix`：schema 为 `trashbot.route_bag_full_semantic_decode_matrix.v1` 或 `trashbot.o6.route_bag_full_semantic_decode_matrix.v1`，只消费 O6 consumer detail / `field_evidence` / `field_motion_evidence_packet` / `artifact_bundle` / `artifact_bundle_consumer_ingest` / `artifact_bundle_readiness` 里的白名单脱敏摘要，展示 decoded/unsupported/failed topic type counts、`coverage_ratio`、限量 topic/type sample、blocked reasons、next evidence 和固定 false safety fields。当前 fixture/test 证明 `diagnostic_msgs/msg/DiagnosticArray` 的 `/diagnostics` matrix item 可显示为 `decode_status=decoded`、`decoder_name=decode_diagnostic_array_payload`，`nav_msgs/msg/Odometry` 仍显示 `decode_status=decoded`、`decoder_name=decode_odometry_payload`，coverage ratio 从 0.75 提升到 1，但这仍只是 local/offline semantic coverage。该 section 的 `proof_scope` 固定为 `software_proof_route_bag_full_semantic_decode_matrix_only`；缺 summary、坏 schema、proof scope mismatch、危险 true、unsafe 文本、控制 topic 或 unsafe topic type 全部 fail-closed。即使 `status=ready_not_route_execution_proof`，O7 也只把它当作本地/离线 semantic coverage 可读，不解锁真实控制、不宣称真实 route execution、真实 delivery success 或真实 robot motion。
- `field_motion_evidence_packet`：schema 为 `trashbot.o6.field_motion_evidence_packet.v1` 或 `trashbot.field_motion_evidence_packet.v1`，只消费 O6 consumer detail 已脱敏摘要，展示 `route_summary.frame_count`、`nonzero_displacement_observed`、`displacement_m`、`motion_log_summary.live_motion_evidence_present`、`motion_log_summary.evidence_sources`、`route_bag_or_live_nav2_log.present/source/route_bag_present/live_motion_log_present`、blocked reasons 和 next required evidence。缺 summary、缺必填字段、schema mismatch、proof scope mismatch、危险 true、unsafe text、token、绝对路径、base64、allowlist root 或 raw/path/root 回显全部 fail-closed；O7 不读取 route bag/live log 本地路径，且继续固定 `safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`、`robot_control_executed=false`。
- `nav2_goal_execution_evidence`：schema 为 `trashbot.nav2_goal_execution_evidence.v1`，只消费 O6 consumer detail / `field_evidence` / `field_motion_evidence_packet` / artifact bundle wrapper 里的白名单摘要，展示 `goal_requested`、`goal_sent`、`goal_accepted`、`result_received`、`goal_result_status`、`result_status_code`、`nav2_goal_execution_proven`、`base_motion_command_nonzero_proven`、`requested_base_command_mode`、`base_command_mode`、pose/base feedback/base command 摘要、blocked reasons 和 next required evidence。该 section 的 `proof_scope` 固定为 `software_proof_nav2_goal_execution_evidence_only`；缺 summary、缺必填字段、schema mismatch、proof scope mismatch、危险 true 或 unsafe text 全部 fail-closed，不展示 token、URL、绝对路径、base64、raw payload、allowlist root、串口路径或 `/cmd_vel` 文本。即使 `goal_result_status=succeeded`，O7 也只把它当作只读 Nav2 goal/result evidence，不解锁真实控制、不宣称真实送达，且继续固定 `safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`、`robot_control_executed=false`。
- `delivery_result_evidence`：schema 为 `trashbot.delivery_result_evidence.v1`，只消费 O6 consumer detail / `field_evidence` / `field_motion_evidence_packet` / artifact bundle wrapper 里的白名单摘要，展示 `record_present`、`record_read_ok`、`record_status`、`delivery_result_claimed`、`operator_confirmation_present`、`dropoff_confirmation_type`、`completed_at_utc`、`linked_nav2_goal_execution_proven`、blocked reasons 和 next required evidence。该 section 的 `proof_scope` 固定为 `software_proof_delivery_result_evidence_only`；缺 summary、缺必填字段、schema mismatch、proof scope mismatch、危险 true、unsafe text、token、URL、绝对路径、base64、raw/path/root、credential URL 或 allowlist root 回显全部 fail-closed，不展示原始 delivery record。即使上游记录宣称完成或人工已确认，O7 也只把它当作只读 delivery result evidence，不解锁真实控制、不宣称真实送达，且继续固定 `safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`、`robot_control_executed=false`。
- `artifact_media_preflight`：schema 为 `trashbot.o6.artifact_media_preflight.v1` 或 workstation 派生的 `derived_blocked_not_proven` 摘要。它把 route replay / labeling 共用的媒体依赖集中到 `route_replay_dependency` 与 `labeling_dependency`，只保留 `route_ref`、`replay_ref`、`keyframe_ref`、`sample_refs`、`review_item_media_refs`、blocked reasons 和 `next_required_evidence`；任何 evidence_ref 字符串都不能被 UI 解释成“媒体已可访问”。
- `O7FixturePreviewPanel.vue` 的 consumer-detail route replay、artifact bundle readiness 和 labeling primary path 优先消费上述字段；route replay 侧额外显示 route/replay/keyframe/sample refs、field motion summary 与 blocked reasons，bundle readiness 侧额外显示 counts、refs、route bag summary、field motion summary、Nav2 goal summary、delivery result summary、blocked reasons 和 `next_required_evidence`，labeling 侧额外显示 review item media refs、route bag readiness、field motion summary、Nav2 goal readiness、delivery result readiness、blocked reasons 和 `next_required_evidence`。这些区域只读，不探测真实媒体，不发 OSS/CDN 请求，不透传 token/query/hash/raw path/DB3 raw payload，不把 route bag topic/message、Nav2 result、pose progress 或 delivery record claim 解读成真实送达。旧 `Cloud Archive Tasks` archive fixture player / labeling review panel 只保留为 debug fallback，不能覆盖 consumer detail 主路径。
- 容错原则：
  ...
  - O6 detail 的 `field_evidence` wrapper 只被当成来源摘要；`gate_pass=true` 仍不等于 `delivery_success=true`，PC 端继续固定 `safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`。
- `route_replay_mvp` / `labeling_mvp` 任一 ready 状态也只表示 O6 detail 可被 PC 工作站压成安全摘要；local/mock annotation submit/export ready 也只表示 O6 本地/mock archive API 被 PC adapter 消费成功。它不证明真实生产云、真实 OSS/CDN、真实地图叠加、真实关键帧媒体可访问、真实 annotation API、真实训练集导出、真实机器人运动或真实送达。
...
`data-refreshes-radar-status=true`。两个按钮共同声明 `data-refresh-affects=map-image-route-robot-radar-target`、
`data-sends-motion-when-clicked=false`、`data-starts-map-runtime=false`、`data-starts-nav2=false`。
这让验收脚本能直接确认：地图刷新只更新当前画面、路线、目标点、小车位置和雷达层的显示材料，不会启动建图 runtime、不会执行 Nav2，也不会发任何底盘命令。
...
默认仍使用 PC 大地图，当时缩放合同为 `100%` 完整态势、最高 `4800%` 细节放大，地图、路线、小车位置和雷达贴图共享同一张 WYSIWYG 画布。
...
`data-current-motion-action-id/start-endpoint/stop-endpoint/acceptance-endpoints/readback-endpoints/required-success-markers/missing-evidence/proof-status/requires-safety-confirm/minimal-precheck-safety-only/*-preflight-required/sends-motion/starts-nav2/starts-manual/starts-keyboard/starts-free-roam/starts-map-runtime/submits-delivery/stops-motion`。
...
2026-07-02 14:25 CST 起，`/map` 直达页继续按“地图太小先解决画布”收敛：直达页默认已经是满屏只看地图，
因此隐藏“收起地图 / 退出全屏 / 退出只看”这类视图切换按钮，只保留地图刷新、雷达贴图只读刷新、缩放和
`工程观察`；`plain-map-wysiwyg-layer-strip` 改成地图画布内浮层，不再占用垂直布局高度，地图层按紧凑工具条
...
2026-07-02 19:05 CST 起，summary 顶层新增 `nav2_route_run_*` 短字段，直接回答完整 Nav2 图上行程是否可执行、
当前缺哪些验收证据、执行端点、停止端点、执行后只读复验端点，以及点击/执行/读回边界。普通 PC 的
`plain-current-trip-execution-pack` 同步暴露 `data-nav2-route-run-*`。这些字段与
...
2026-07-03 00:45 CST 曾短暂验证 PC 低速试动和键盘连续手控走上位机 `command_mode=pwm` +
`feedback_mode=realtime` 的直接串口路径；2026-07-03 01:35 CST 后当前有效口径改为 PC 仍发送
`command_mode=ros` 并发布 `/cmd_vel`，由已经持有 UART 的 `esp32_bridge` 默认映射成 vendor `T=11/PWM164`。
这样既避免 PC 绕过 bridge 抢 `/dev/ttyS5`，又避开当前 `T=13` 在无编码器/反馈状态下持续 `L/R=0/0` 的问题。
当前现场事实是 `T=1001` 的 wheel raw `L/R` 仍可能保持 `0/0`，因此 PC 可展示
`motion_signal_observed` / `imu_attitude_delta_observed` 作为“确有运动信号”的材料，wheel raw 非零仍保留为未完成硬件/固件风险并单独展示。
...
2026-07-02 23:30 CST 起，PC 普通首屏真实地图历史上曾改为高度优先撑满主画布；当前有效实现已收敛为宽度优先完整态势：`.plain-map-layer.has-real-map`
按画布宽度设置 overlay frame，保留地图比例，`data-real-map-fit-mode` 固定为
`width-first-preserve-aspect-full-overview`。普通用户入口仍是 PC 首屏和 `/map` 大屏；ROS2 配套工具只作为工程观察：
...
雷达独立卡、连接状态、自由移动和长验收说明下沉到详情区。首页地图仍默认按 400% 细节观察，`/map` 直达页仍是整屏地图；
...
2026-07-04 00:16 CST 起，普通 PC 首页地图当时默认使用 100% 完整态势视角：首屏驾驶台保持左侧地图、
右侧实时图传和 WASD/方向键连续手控，地图默认完整显示真实地图、Nav2 路线、小车位置、雷达点和目标点。
`细节放大` 仍可一键到 1200% 排障，点 `完整态势` 回到 100% 全局视角，`/map` 直达页同样使用 100% 完整态势默认值。
首页地图卡会默认收起长说明、地图生命周期按钮和工程说明正文，把首屏高度让给实时地图；点击 `工程观察` 才展开 RViz2/Foxglove 只读观察入口。
...
`clamp(820px, calc(100vh - 60px), 1500px)`，默认缩放为 `400%`，最高为 `1200%`。ROS2 配套仍是
RViz2 本地工程调试和 Foxglove 远程浏览器观察，不替代普通用户 PC 大地图。

2026-07-03 02:41 CST 起，普通 PC 首页地图再次放大：驾驶台布局从约 `1.65fr/0.9fr` 调整为
`4fr/0.75fr`，右列保留实时图传和 WASD，但最小宽度收紧到 `300px`；首页大地图卡改为
`clamp(760px, calc(100vh - 12px), 1400px)`，卡内地图画布改为
`clamp(720px, calc(100vh - 84px), 1280px)`，并把“当前画布”图层条改成地图内横向浮层，避免五层状态文字
...
2026-07-04 00:16 CST 起，PC 地图按现场反馈改为默认完整态势：普通首页和 `/map` 直达页默认缩放使用
`100%` 完整态势，`细节放大` 到 `1200%` 只用于局部排障，点 `完整态势` 回到 `100%` 全局视角。首页地图卡的覆盖规则收敛为
...
2026-07-04 01:10 CST 起，PC 首页和 `/map` 的真实地图 PNG 渲染曾从宽度优先改为高度优先；当前有效实现改回宽度优先完整态势：
`.plain-map-layer.has-real-map .plain-map-overlay-frame` 现在用画布宽度计算 overlay frame，
保留比例并优先同屏显示路线、小车、雷达和目标，`data-real-map-fit-mode=width-first-preserve-aspect-full-overview` 与真实 CSS 行为一致。
这修正了“地图卡很高但全局态势被局部放大裁掉”的体验问题；PC 仍默认 `100%` 完整态势，`细节放大`
到 `4800%` 用于局部排障。ROS2 配套仍只是工程观察：本地 RViz2、远程 Foxglove bridge + Foxglove Web
...
`radar_map_points_visible=true`、`camera_current_visible=false`、`map_display_default_zoom_percent=1600%`（该现场证据属于当时 1600% 版本；当前有效默认值已在 2026-07-06 07:26 调整为 `800%`）。
...
默认 `800%` 大地图，`完整态势` 回 `100%`，局部最高 `4800%`；ROS2 配套只作为工程观察。本地工程调试用 RViz2/Nav2 RViz 配置看
...
2026-07-06 01:53 CST 继续查 DV20 输入/驱动可能性：`lsusb -v -d 4c4a:4a55` 显示 DV20 为 Jieli
UVC 1.00 复合设备、bus powered `400mA`，VideoControl processing unit 有 `Descriptor too short` 警告，
并存在 vendor extension unit `{28f03370-6311-4a2e-ba2c-6890eb334016}`、`bNumControls=8`。标准
`v4l2-ctl --list-ctrls-menus` 只暴露亮度、曝光、白平衡等普通控制项，没有可直接切 HDMI/CVBS/输入源的标准控制。
`media-ctl -p -d /dev/media1` 确认 graph 为 `Input 1 -> Processing 2 -> Extension 3 -> /dev/video1`，
链路 enabled/immutable。GStreamer 直采也失败：`image/jpeg 640x480@30`、`image/jpeg 1280x720@30`、
`YUY2 320x240@25`、`YUY2 640x480@22` 均能协商 caps，但输出全为 `0 bytes`。

同轮 PC 7001 复验：first-frame probe 仍为 `probe_total_timeout`、`frame_observed=false`、
`source_diagnosis_status=uvc_no_frame_not_exclusive`；雷达只读刷新后 live-summary 为
`status=ready_for_motion`、`map_current_visible=true`、`path_current_visible=true`、
