...
- 2026-07-01 08:35 CST 起，普通首屏地图必须保持普通用户“大地图优先”，默认使用 `150%` 细节视图，最高细节放大 `2400%`；`/map` 是推荐直达地图大屏入口，`?view=map` 保留兼容，两者默认同样使用 `150%`，用户需要看全图时点“适配”回到 `100%`。ROS2 配套只作旁路：RViz2 / `nav2_rviz_plugins` 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位，Foxglove / `foxglove_bridge` 用于浏览器观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-01 08:35 CST 起，普通首屏地图必须默认使用 `150%` 细节视图，`/map` 直达地图大屏也默认使用 `150%`，`?view=map` 继续兼容，并继续保留同一张 WYSIWYG 画布里的地图、路线、小车位置和雷达贴图。“细节放大”按钮一键到 `2400%`，`+/-` 支持逐级缩放，“适配”回到 `100%` 全图。ROS2 配套只作旁路：RViz2 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位；Foxglove 用于部署 bridge 后的浏览器远程观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-01 08:35 CST 起，普通首屏地图必须继续按 PC 主视图处理：默认 `150%` 细节视图、最高 `2400%` 细节放大、桌面外壳接近全宽、大图高度贴近整屏，只看地图模式使用普通用户文案并保留路线、小车位置和雷达贴图。ROS2 配套分层为 RViz2 / `nav2_rviz_plugins` 做工程调试，Foxglove / `foxglove_bridge` 做浏览器远程观察；普通用户默认不离开简易 PC 工作站。
...
- 2026-06-30 13:17 CST 起，`/map` 与兼容 `?view=map` 直达地图大屏必须切换 App 页面壳到 map-only：隐藏普通顶栏和高级工具，`.shell` 使用 `100vw` / `100vh`，地图组件进入 full + 只看地图状态；2026-07-01 08:35 CST 后当前默认使用 `150%` 细节视图，最高 `2400%` 细节放大，“适配”回到 `100%` 全图；同时继续暴露 `data-direct-map-view-behavior=page_shell_map_only` 与地图卡 `page_fixed_fullscreen_map_only`。该模式只为第二屏/PC 大屏观察地图、路线、小车位置和雷达贴图，不启动 RViz2/Foxglove/ROS2 runtime，不执行 Nav2，不发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-01 08:38 CST 起，普通首屏当前卡点区新增 `plain-live-trip-closure-readback`，镜像行程卡的完整行程闭环三事实：到点是否已读到、同窗口 wheel L/R 是否非零、送达确认是否完成。该行让现场不用滚到行程卡也能直接看出完整 Nav2 路线执行还差哪一项；按钮 `plain-live-trip-closure-readback-refresh` 固定只读回 `/api/robot-control/nav2/goal/execution/latest`、`/api/robot-control/base/feedback-samples`、`/api/robot-control/summary` 和 `/api/robot-control/delivery/latest`，并暴露 `data-readback-only=true`、`data-sends-motion-when-clicked=false`、`data-starts-nav2=false`、`data-starts-manual=false`、`data-starts-keyboard=false`、`data-starts-free-roam=false`、`data-starts-map-runtime=false`、`data-submits-delivery=false`、`data-stops-motion=false`；它只做验收读回，不执行路线、不发手控、不启动自由移动/建图、不提交送达、不停止底盘。
