...
- 2026-06-30 02:12 CST 起，普通首屏雷达卡片新增 `地图雷达事实` 行；2026-06-29 07:58 CST 起，该行优先消费 `readback_summary.radar.plain_hint`，只显示“地图雷达点当前显示几个、是否需要同轮地图预览/新扫描、下一步是什么”，不再把 `雷达 marker` 或 `overlay` 这种高级诊断词放到首屏。旧后端缺少 `plain_hint` 时才回退 `radar_overlay_wysiwyg_status_plain`；该行只读 summary，不启动雷达、不刷新地图、不调用 manual、Nav2、free-roam、stop 或 `/cmd_vel`。
