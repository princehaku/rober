...
- 2026-07-03 19:40 CST 起，地图太小的当前有效口径为：PC 首页和 `/map` 直达大屏默认使用 `400%` 现场细节视角显示真实地图、Nav2 路线、小车位置、雷达点和目标点；需要一键完整态势时点 `适配` 回到 `45%`，需要局部排障时点 `细节放大` 到 `1200%`。`/map` 直达页必须使用整屏 flex 地图布局，只保留缩放、`刷新地图画面`、雷达贴图只读刷新和 `工程观察`，并收起建图、保存、地图列表、普通说明和非地图卡片；DOM 必须暴露 `data-direct-map-view-visible-controls=zoom,map_refresh,radar_refresh,ros2_observe_toggle`、`data-direct-map-view-hides-map-lifecycle-actions=true` 和 `data-direct-map-view-hides-non-map-cards=true`。地图卡工具行新增 `工程观察` 展开按钮，但它只展开 RViz2/Foxglove 观察说明和命令，固定 `data-sends-motion-when-clicked=false`、`data-starts-ros2=false`、`data-starts-rviz2=false`、`data-starts-foxglove=false`、`data-starts-nav2=false` 和 `data-starts-map-runtime=false`。ROS2 配套答案保持分层：本地工程调试用 `ros2 launch ros2_trashbot_bringup rviz.launch.py`；远程浏览器观察用 `ros2 launch ros2_trashbot_bringup foxglove_bridge.launch.py` + Foxglove Web 连接 `ws://192.168.1.11:8765`；普通用户界面不被替代。
