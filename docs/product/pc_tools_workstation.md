...
- 2026-06-30 01:16 CST 起，Robot Control summary 新增 `readback_summary.radar`，把雷达本体 lifecycle/fresh 状态和地图同轮 `radar_overlay_*` marker 读数合成一个只读入口。现场脚本可直接读取 `radar_overlay_point_count` / `map_marker_point_count` 判断地图上实际显示的雷达 marker 数；该字段只派生已有 summary，不启动雷达、不刷新地图、不调用 Nav2、manual、free-roam、stop 或 `/cmd_vel`。
...
- 2026-06-29 07:43 CST 起，Robot Control summary 的顶层 `current_fact_plain` 雷达段优先消费 `readback_summary.radar.plain_hint`，让普通首屏当前事实直接显示“地图雷达点 / 旧来源点只作诊断 / 下一步先启动雷达并等待新扫描”，不再把高级诊断里的 `雷达 marker` 或 `overlay` 文案带到用户主句。拆分字段仍保留 marker/overlay 细节给高级脚本；该变化只改只读 summary 合成，不启动雷达、不刷新地图、不执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-06-29 07:49 CST 起，Robot Control summary 的 `readback_summary.map.plain_hint` 和 `GET /api/robot-control/map/preview` 顶层 `plain_hint` 也改用普通地图雷达口径：只说“地图雷达点当前显示几个、旧来源点是否只作诊断、原因是什么”，不再在普通 `plain_hint` 里暴露 `雷达 marker` 或 `overlay`。`radar_overlay_wysiwyg_status_plain`、`radar_overlay_*` 数值和嵌套对象仍保留原精确诊断给高级脚本；该变化只改只读 summary/map preview 合成，不启动雷达、不刷新地图、不执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
...
- 2026-06-30 02:12 CST 起，普通首屏雷达卡片新增 `地图雷达事实` 行，直接消费 `readback_summary.radar.radar_overlay_wysiwyg_status_plain` 与下一步文案。它把“地图上实际 marker 显示几个点”和“旧来源点只作诊断”放到雷达卡片首屏；该行只读 summary，不启动雷达、不刷新地图、不调用 manual、Nav2、free-roam、stop 或 `/cmd_vel`。
- 2026-06-30 02:30 CST 起，普通首屏行程卡片新增 `行程复验事实` 行，直接消费 `readback_summary.nav2.route_execution_precheck_plain`、`route_execution_readiness_plain` 和 wheel raw L/R 复验字段。它把“只需勾选现场安全确认”和“重跑完整图上路线后确认同窗口 wheel raw L/R 非零”放到行程卡片首屏；该行只读 summary，不触发 Nav2 execute、不启动 runtime、不调用 manual、free-roam、stop 或 `/cmd_vel`。
- 2026-06-29 02:54 CST 起，Robot Control summary 的 `readback_summary.nav2` 新增 `goal_execution_wheel_raw_lr_status_plain` 和 `goal_execution_wheel_raw_lr_next_action_plain`。它们把完整路线验收最关键的同窗口 `wheel raw L/R` 结论从长状态句里拆出来：路线 action 已成功但 L/R 仍为 `0/0` 时，字段会单独说明同窗口轮速未非零、已观察到的非零底盘命令或 IMU 姿态变化，以及下一步用 ROS 模式重跑图上路线复验。该字段只消费只读 latest/status/readback，不自动执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
...
- 2026-06-29 03:08 CST 起，Robot Control summary 的 `readback_summary.map` 新增 `robot_pose_status`、`radar_overlay_point_count`、`radar_overlay_source_point_count` 和 `radar_overlay_frame_id` 短别名。短别名与既有雷达 overlay 长字段同源，目的是让普通脚本/面板直接读取当前地图是否真正画出小车和雷达 marker；旧雷达来源点不贴图时 `radar_overlay_point_count=0`，source count 仍保留诊断值。该字段只消费只读 map/radar overlay，不启动雷达、不刷新地图、不执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-06-29 03:31 CST 起，Robot Control summary 的 `readback_summary.map` 新增 `radar_overlay_wysiwyg_status_plain` 和 `radar_overlay_wysiwyg_next_action_plain`。它们把“当前地图上实际显示几个雷达 marker”和“旧雷达来源点是否只作诊断”合成一句白话：例如 live 形态会显示当前 marker 为 0，旧来源点 81 个不贴到当前地图。普通首屏雷达点口径和地图下一步优先消费这两个字段，避免把旧点误画成实时雷达 marker。该字段只消费只读 map/radar overlay，不启动雷达、不刷新地图、不执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-06-29 07:00 CST 起，Robot Control summary 的 `readback_summary.map` 新增 `plain_hint`，把地图画面、图上路线和雷达 marker WYSIWYG 结论合成一条。外部脚本只读 summary 时，不必拼 `map_wysiwyg_status_plain`、`path_wysiwyg_status_plain` 和 `radar_overlay_wysiwyg_status_plain`，也能直接看到旧雷达来源点是否只作诊断、当前 marker 实际显示几个点，以及下一步是否要启动雷达/刷新地图。该字段只读 summary，不刷新地图、不启动雷达、不执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-06-30 00:12 CST 起，Robot Control summary 的 `current_fact_plain` 会把地图主句和雷达 overlay 主句分层拼接：地图主句只说地图/路线/小车位置，雷达 overlay 主句单独说明当前地图上实际 marker 数和旧来源点诊断数。这样 live 中“旧雷达来源点存在但当前不贴图”的原因只出现一次，避免普通首屏把旧来源点误读成当前 marker。该字段仍只消费只读 summary，不启动雷达、不刷新地图、不执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-06-29 04:05 CST 起，`GET /api/robot-control/radar/status` 顶层新增雷达本体和地图 marker 验收白话字段：`continuous_scan_status`、`lifecycle_running`、`lifecycle_state`、`latest_scan_proof_fresh`、`scan_point_count`、`latest_scan_age_ms`、`radar_status_plain`、`radar_next_action_plain`、`radar_overlay_point_count`、`radar_overlay_source_point_count`、`radar_overlay_wysiwyg_status_plain` 和 `radar_overlay_wysiwyg_next_action_plain`。独立 radar status 现在能直接说明雷达是否运行/fresh，并明确地图 marker 是否所见即所得仍以 `GET /api/robot-control/map/preview` 的 overlay 字段为准。该字段只消费只读 radar status，不启动雷达、不刷新地图、不执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-06-29 05:02 CST 起，`GET /api/robot-control/radar/status` 顶层新增 `plain_hint` 和 `next_action_plain`，分别对齐 `radar_status_plain` 与 `radar_next_action_plain`。现场脚本只看顶层字段时，也能直接知道雷达是否 fresh、旧扫描不能当作地图 marker，以及下一步要启动雷达后再刷新地图画面；该字段只消费只读 radar status，不启动雷达、不刷新地图、不执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-06-29 03:36 CST 起，`GET /api/robot-control/map/preview` 的 `radar_overlay` 嵌套对象和顶层 alias 同步返回 `wysiwyg_status_plain`、`wysiwyg_next_action_plain`、`radar_overlay_wysiwyg_status_plain` 和 `radar_overlay_wysiwyg_next_action_plain`。地图预览响应本身现在能证明雷达 marker 当前显示数量、旧来源点诊断数量和下一步动作，不必再旁路读取 summary 才能确认雷达贴图是否所见即所得。该字段只消费同一轮只读 map/radar/localization overlay，不启动雷达、不刷新地图、不执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-06-29 04:57 CST 起，`GET /api/robot-control/map/preview` 顶层新增 `plain_hint`，直接对齐 `map_wysiwyg_status_plain`。现场脚本和普通页面只看顶层字段时，也能直接知道当前地图画面是否已读到、路线/小车/雷达 marker 是否真的显示在当前地图上；该字段只消费同一轮只读 map preview 和 overlay，不启动雷达、不刷新地图、不执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-06-30 00:28 CST 起，`GET /api/robot-control/map/preview` 顶层新增 `map_plain_hint` 和 `map_next_action_plain`，并让 `plain_hint` 使用地图主句 + 雷达 overlay 主句的去重合成。`next_action_plain` 现在对齐地图 WYSIWYG 下一步，优先暴露“先启动雷达/刷新地图 marker”等地图验收动作；路线执行下一步仍保留在 `path_preview_next_action_plain`、`path_wysiwyg_next_action_plain` 和 `nav2_route_overlay_next_action_plain`。该入口仍只读 map preview/overlay，不启动雷达、不刷新地图、不执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
...
- 2026-06-29 06:48 CST 起，Robot Control summary 的 `readback_summary.nav2` 新增 `plain_hint`，把 `execution_status_plain` 和 `next_action_plain` 合成一条自动驾驶事实。外部脚本或普通页面只看一个字段时，也能直接看到“路线是否已经证明、若 action 成功但 wheel raw L/R 未非零，下一步要勾安全确认后用指定模式重跑并复验同窗口轮速”。该字段只读 summary，不准备路线、不执行 Nav2、不启动 runtime、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-06-29 07:27 CST 起，Robot Control summary 的 `readback_summary.nav2.plain_hint` 改为普通用户口径：`wheel raw L/R` 在总事实里显示为 `执行窗口轮速 L/R`，`Nav2 planner/controller` 显示为 `规划服务/控制服务`。拆分诊断字段如 `goal_execution_wheel_raw_lr_status_plain` 仍保留原始术语，方便高级排障；该变化只改只读 summary 文案，不准备路线、不执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
...
- 2026-06-29 03:53 CST 起，`GET /api/robot-control/nav2/goal/execution/latest` 顶层同步返回 `route_execution_readiness_plain`、`route_execution_precheck_plain`、`goal_execution_wheel_raw_lr_status_plain` 和 `goal_execution_wheel_raw_lr_next_action_plain`。独立 latest endpoint 现在不用依赖 summary，也能直接说明完整路线是否已证明、发车前只需勾选安全确认，以及 action 成功但 wheel raw L/R 未非零时下一步用 ROS 模式重跑复验。该字段只消费只读 latest artifact，不执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-06-29 23:58 CST 起，`GET /api/robot-control/nav2/goal/execution/latest` 顶层新增 `plain_hint`、`base_command_mode`、`goal_execution_base_command_mode`、`next_execution_base_command_mode`、`goal_execution_base_command_nonzero_observed/count`、`goal_execution_base_command_mode_counts`、`goal_execution_base_feedback_lr_nonzero_proven` 和 `goal_execution_base_feedback_imu_attitude_delta_observed`。现场脚本只看 latest 顶层时，也能直接分辨“上次 PWM action 成功但 wheel raw L/R=0/0 未非零，已看到非零命令/IMU，下一次要 ROS 模式重跑复验”，避免误判成雷达、相机或控制服务阻塞。该入口仍只读 latest artifact，不执行 Nav2、不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
...
- 2026-06-29 04:37 CST 起，`GET /api/robot-control/summary` 顶层新增 `current_fact_plain`。该字段把本轮只读 summary 内的画面可见性、地图/路线/雷达 marker 所见即所得、Nav2 路线复验、键盘连续手控、自由移动和建图 readiness 合成一段普通话事实；连接失败时也会直接返回“当前事实未读到”的下一步。该变化只消费同一轮 readback，不额外请求上位机、不准备或执行 Nav2、不启用键盘、不启动 free-roam、不发送 manual、delivery、stop 或 `/cmd_vel`。
