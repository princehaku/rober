...
- 2026-06-22 15:27 起，Robot Control summary 的 `readback_summary.base` 增加最新只读 `wheel_feedback_latest_left_speed/right_speed`，普通首屏“轮速记录”也会显示“当前只读轮速是 L/R=0/0；这还不是非零证据，需要现场试动窗口”。该提示只消费 summary 或高级只读采样结果，不调用 `/api/base/manual`、first-jog、Nav2 或 `/cmd_vel`，也不把 `T=1001` 静态反馈外推成 wheel raw L/R 非零。
