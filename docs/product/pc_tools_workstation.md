...
- 2026-07-03 CST 本轮补强：PC 首页不再把地图当普通卡片，但地图层也不得制造超长滚动框；large 模式固定为 `--plain-map-large-min-height=840px`、目标高度 `calc(100vh - 24px)`、上限 `1400px`，`/map` 直达页默认填满当前 viewport。地图标题短句固定为 `普通看 PC 大地图；工程看 RViz2 / Foxglove`。summary 的 `map_display_too_small_next_action_plain` 必须说明 PC 首页默认 `300%` 细节大图、可点 `适配` 回到 `100%` 全图或进 `/map`，`map_display_ros2_companion_answer_plain` 必须说明普通用户仍用 PC 大地图和 `/map`；RViz2/Foxglove 仍只作为观察配套，不启动 ROS2/RViz2/Foxglove/Nav2/建图 runtime，不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
