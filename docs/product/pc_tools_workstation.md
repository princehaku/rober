...
- 2026-06-23 13:45 起，Node API public 启动前会先探测监听地址；当 `0.0.0.0:7071` 被 Clash Verge 或其他进程占用时，直接输出 `address already in use`、`lsof/netstat` 排查命令和 `PORT=<free-port> npm run api:public` 兜底命令，不再先打印“listening”再抛 Node 栈。该启动诊断只影响 PC 工作站可访问性，不会调用 Nav2、manual、delivery complete、keyboard pulse 或 `/cmd_vel`。
