...
- 2026-06-30 23:11 CST 起，普通首屏地图必须默认使用 `1600%` 缩放，`/map` 直达地图大屏也默认使用 `1600%` 上限，`?view=map` 继续兼容，并继续保留同一张 WYSIWYG 画布里的地图、路线、小车位置和雷达贴图。ROS2 配套只作旁路：RViz2 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位；Foxglove 用于部署 bridge 后的浏览器远程观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-06-30 23:11 CST 起，普通首屏地图必须继续按 PC 主视图处理：默认 `1600%` 缩放、最高 `1600%`、桌面外壳接近全宽、大图高度贴近整屏，只看地图模式使用普通用户文案并保留路线、小车位置和雷达贴图。ROS2 配套分层为 RViz2 / `nav2_rviz_plugins` 做工程调试，Foxglove / `foxglove_bridge` 做浏览器远程观察；普通用户默认不离开简易 PC 工作站。
