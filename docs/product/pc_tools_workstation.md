...
- 2026-07-03 CST 本轮补强：PC 首页不再把地图当普通卡片，而是普通用户主画布；large 模式固定为 `--plain-map-large-min-height=960px`、目标高度 `calc(100vh - 8px)`、上限 `1800px`，桌面外壳为 `width: min(3200px, 100%)`，驾驶台布局为 `grid-template-columns: minmax(0, 4fr) minmax(300px, 0.75fr)`，首页地图卡为 `height: clamp(760px, calc(100vh - 12px), 1400px)`，地图画布为 `height: clamp(720px, calc(100vh - 84px), 1280px)`；`/map` 直达页默认填满当前 viewport。地图标题短句固定为 `普通看 PC 大地图；工程看 RViz2 / Foxglove`。summary 的 `map_display_too_small_next_action_plain` 必须说明 PC 首页默认 `300%` 细节大图、可点 `适配` 回到 `100%` 全图或进 `/map`，`map_display_ros2_companion_answer_plain` 必须说明普通用户仍用 PC 大地图和 `/map`；RViz2/Foxglove 仍只作为观察配套，不启动 ROS2/RViz2/Foxglove/Nav2/建图 runtime，不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
...
- 2026-07-02 CST 现场复核历史证据：历史版本曾把 `/map` 缩放推进到 `3200%`、`8000%/9600%`，会让浏览器形成超宽滚动画布，不再作为当前有效口径。当前“PC 地图太小”的默认处理是使用 PC 大地图、访问 `http://<PC>:7001/map`，或在同一页面点 `细节放大` 到最高 `800%`；ROS2 配套只推荐工程观察：本机 RViz2 看 `/map`、`/scan`、TF、路径、定位和 costmap，远程多人观察用 Foxglove bridge + Foxglove Web，不替代普通用户 PC 大地图，也不作为发车、自由移动或建图前置。
...
- 2026-07-03 00:45 CST 起，普通首屏地图和 `/map` 直达大屏默认缩放当前有效值为 `300%` 细节大图，地图大面板继续占据首屏主视图；`适配` 回到 `100%` 全图，`细节放大` 继续放大局部，最高细节放大为 `800%`。这是普通用户解决“地图太小”的默认路径；ROS2 配套仍只放在默认折叠的“工程观察”里，RViz2/Foxglove 不作为普通用户发车入口，也不得自动启动 ROS2、Nav2、建图 runtime、manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-01 13:26 CST 起，普通首屏地图和 `/map` 直达大屏在 `100%`/`800%` 放大时必须保证整张画布四周都可滚动到：`.plain-map-layer` 的滚动原点固定为左上完整画布，DOM 暴露 `data-scroll-origin=top_left_full_canvas` 和 `data-auto-center-on-zoom=true`；进入 `/map`、全屏、只看地图、刷新地图画面或调整缩放后，PC 端只自动居中当前视口，不改变地图、路线、小车位置或雷达点坐标。该修正只解决大地图负方向溢出导致边缘不可达的问题，不启动 ROS2/RViz2/Foxglove、Nav2、建图 runtime，不发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
- 2026-07-01 12:40 CST 起，普通首屏地图必须继续保持普通用户“大地图优先”；当前默认使用 `300%` 现场大图，最高细节放大为 `800%`；`/map` 是推荐直达地图大屏入口，`?view=map` 保留兼容，两者默认同样使用 `300%`，用户需要看完整全图时点“适配”回到 `100%`。ROS2 配套只作旁路：RViz2 / `nav2_rviz_plugins` 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位，Foxglove / `foxglove_bridge` 用于浏览器观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-01 12:40 CST 起，普通首屏地图必须保持可适配全图；当前默认使用 `300%` 现场大图，`/map` 直达地图大屏也默认使用 `300%`，`?view=map` 继续兼容，并继续保留同一张 WYSIWYG 画布里的地图、路线、小车位置和雷达贴图。“细节放大”按钮一键到 `800%`，`+/-` 支持逐级缩放，“适配”保持回到 `100%` 全图。ROS2 配套只作旁路：RViz2 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位；Foxglove 用于部署 bridge 后的浏览器远程观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-07-01 08:35 CST 起，普通首屏地图必须继续按 PC 主视图处理：当前默认 `300%` 现场大图、最高 `800%` 细节放大、桌面外壳接近全宽、大图高度贴近整屏，只看地图模式使用普通用户文案并保留路线、小车位置和雷达贴图；`适配` 可回到 `100%` 全图。ROS2 配套分层为 RViz2 / `nav2_rviz_plugins` 做工程调试，Foxglove / `foxglove_bridge` 做浏览器远程观察；普通用户默认不离开简易 PC 工作站。
...
- 2026-06-30 13:17 CST 起，`/map` 与兼容 `?view=map` 直达地图大屏必须切换 App 页面壳到 map-only：隐藏普通顶栏和高级工具，`.shell` 使用 `100vw` / `100vh`，地图组件进入 full + 只看地图状态；当前默认使用 `300%` 现场大图，最高 `800%` 细节放大，“适配”回到 `100%` 全图；同时继续暴露 `data-direct-map-view-behavior=page_shell_map_only` 与地图卡 `page_fixed_fullscreen_map_only`。该模式只为第二屏/PC 大屏观察地图、路线、小车位置和雷达贴图，不启动 RViz2/Foxglove/ROS2 runtime，不执行 Nav2，不发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-06-30 06:05 CST 起，普通首屏地图默认缩放已提升到 `300%`，当前缩放档位支持到 `800%`，大图高度按当前 PC 主画布口径执行，并暴露 `data-default-map-zoom-percent=300%`。普通操作继续留在 PC 工作站大地图；ROS2 工程观察首选 RViz2，远程浏览器观察可接 Foxglove Bridge/Studio，但 PC 页面不会自动启动这些 ROS2 runtime。
...
默认仍使用 PC 大地图，当前缩放合同为 `300%` 现场大图、最高 `800%` 细节放大，地图、路线、小车位置和雷达贴图共享同一张 WYSIWYG 画布。
