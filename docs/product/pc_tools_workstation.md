...
- O7 Previews 现在还包含 `Cloud operator console probe`、`Cloud archive tasks probe`、`Realtime/elevator cloud probe` 和 `Cloud Archive Tasks` 区块。三个 probe 区块默认只填本机回环示例 URL，不自动发起请求；operator 点击对应 Probe 按钮后才调用 PC 后端 probe API，且浏览器不直接访问 relay。Archive 区块默认不读取本地路径；operator 输入 archive fixture 路径并点击 `Load archive tasks` 后才调用 `GET /api/o7/cloud-archive/tasks?archiveJson=<local-json>`。UI 只展示 probe/archive status、input status、task list 数量、selected/latest task、inspector 状态、安全摘要、route replay inspector、labeling queue inspector、voice ASR/TTS inspector、realtime/elevator snapshot 摘要、dangerous true fields、fixed false fields、blocked reasons 和 not proven。route inspector 区域用表格展示 selected task、map frame、frame count、sample frames、event timeline、keyframe refs 和 cursor false fields；同一区域提供 PC-only 本地 route replay player，`Previous frame`、`Next frame`、`Reset cursor` 和 range cursor 只改变浏览器本地 sample frame 下标，不调用 API、不写后端、不发送机器人命令，并在未加载 archive、无 selected task、无 sample frames、inspector blocked 或显式 `playback_available=false` 时显示 `blocked_not_proven` 且禁用 navigation。该 player 只用于检查 fixture sample frame 的 timestamp、pose、velocity、state 和 evidence ref，不等于真实云历史路线回放、真实地图叠加、真实机器人运动或真实控制。labeling inspector 区域用表格展示 sample review items，并展示 label schema、allowed label types、draft labels、dataset export gaps 和标注 false fields；voice inspector 区域用表格展示 ASR event sample，并展示 latest partial/final、TTS draft summary、speaker dispatch summary、media preflight dependency 和语音 false fields；realtime/elevator probe 区域展示 map ref/frame、pose freshness、route membership false fields、电梯状态、楼层证据和人工接管缺口，仍不提供自动播放、提交、导出、发送、控制、停止、取消或恢复类动作按钮。
