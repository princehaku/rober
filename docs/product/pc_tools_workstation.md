...
- 2026-07-03 CST 起，PC 键盘和低速点动默认控制链固定为 `command_mode=ros`：PC Node 代理在省略模式时也写入 `ros`，上车端优先使用进程内 `rclpy` publisher 向 `/cmd_vel` 连续 burst 发布，等待至少一个订阅者；只有 ROS Python 环境不可用或发布失败时才回退到带 `--wait-matching-subscriptions` 的 `ros2 topic pub` burst。该路径避免每次 WASD 都冷启动两个 `ros2 topic pub` 进程，也避免 PC 绕过 `esp32_bridge` 抢 `/dev/ttyS5`；轮速验收仍必须以同窗口 WAVE ROVER T1001 L/R 非零为准，IMU 姿态变化不能替代 wheel raw L/R。
...
2026-07-03 00:45 CST 起，PC 普通低速试动和键盘连续手控默认改走上位机 `command_mode=pwm` +
`feedback_mode=realtime`：前端和 PC 代理统一发送 vendor `T=11` PWM 短脉冲，由上位机只写 `/dev/ttyS5`
并自动发送 `T=11/T=1/T=13` 多模式停车；该实时路径不抢 UART 读反馈、不覆盖 `manual_feedback_samples_latest`，
松开键盘后再通过固定 `/api/robot-control/base/feedback-samples` 与 `/api/robot-control/summary` 做只读复验。
`feedback_mode=bridge_debug` 保留给首动复验和诊断按钮，用于需要同窗口材料时读取 `esp32_bridge` 新鲜 debug log；当前现场事实是 `T=1001` 的 wheel raw
`L/R` 仍可能保持 `0/0`，因此 PC 可展示 IMU 姿态变化作为“确有运动信号”的次级证据，wheel raw 非零仍保留为未完成硬件/固件风险。
