...
- 2026-06-28 19:45 CST 起，上述 execute pending 的普通文案进一步收口为“行程请求已发送，等待结果返回”：地图 marker、路线 caption、`当前事实`、行程状态和进度都不再把 PC 请求未返回的窗口直接说成“正在执行图上路线”。这避免把网络/上位机 pending 窗口误当作完整 Nav2 路线执行已开始或已证明；停止兜底仍显示 `行程停止（随时可点）` 并继续复用固定 base stop，不新增 Nav2 cancel、manual、keyboard、delivery、free-roam 或 `/cmd_vel` 调用。
