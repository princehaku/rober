...
- 2026-06-23 00:14 起，普通首屏 `启用键盘` 在连接、键盘合同、移动前检查和现场画面都已满足，但键盘 gate 仍缺轮速材料时显示 `启用键盘（先补轮速）`。按钮仍保持禁用，不 arm 键盘、不发送 keyboard pulse、manual、stop、Nav2、delivery complete 或 `/cmd_vel`。
