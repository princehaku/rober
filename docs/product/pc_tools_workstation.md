...
- 2026-06-30 17:50 CST 起，普通首屏地图必须默认使用 `1200%` 缩放，`?view=map` 直达地图大屏必须默认使用 `1600%` 上限，并继续保留同一张 WYSIWYG 画布里的地图、路线、小车位置和雷达贴图。ROS2 配套只作旁路：RViz2 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位；Foxglove 用于部署 bridge 后的浏览器远程观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
- 2026-06-30 21:15 CST 起，`?view=map` 直达地图页必须隐藏普通连接、相机、雷达、自由移动、行程等非地图卡片，只保留地图面板、缩放、刷新和 ROS2 配套说明，使 PC 第二屏真正成为近全屏地图观察页。该模式仍只改变 CSS/DOM 显示，不启动 ROS2/RViz2/Foxglove，不执行 Nav2，不发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。ROS2 现成配套继续采用 RViz2：`ros2 launch ros2_trashbot_bringup rviz.launch.py`，用于工程观察 `/map`、`/scan`、TF、`/plan`、`/local_plan`、`/amcl_pose` 和 costmap；普通用户不需要离开 PC `7001` 地图大屏。
...
- 2026-06-30 11:21 CST 起，普通首屏地图必须提供 `?view=map` 直达地图大屏入口：地图卡显示“打开地图大屏”，新窗口打开后页面内自动进入全屏只看地图状态，继续使用同一套地图、路线、小车位置和雷达 overlay WYSIWYG 画布。该入口必须暴露 `data-direct-map-view-requested`、`data-direct-map-view-url="?view=map"` 和 `data-direct-map-view-behavior=page_fixed_fullscreen_map_only`，并固定声明不启动 ROS2/RViz2/Foxglove/Nav2/建图 runtime、不发送任何运动命令。RViz2/Foxglove 仍只是工程观察配套，不替代普通用户地图大屏。
...
- 2026-06-30 17:50 CST 起，普通首屏地图必须继续按 PC 主视图处理：默认 `1200%` 缩放、最高 `1600%`、桌面外壳接近全宽、大图高度贴近整屏，只看地图模式使用普通用户文案并保留路线、小车位置和雷达贴图。ROS2 配套分层为 RViz2 / `nav2_rviz_plugins` 做工程调试，Foxglove / `foxglove_bridge` 做浏览器远程观察；普通用户默认不离开简易 PC 工作站。
...
- 2026-06-30 13:17 CST 起，`?view=map` 直达地图大屏必须切换 App 页面壳到 map-only：隐藏普通顶栏和高级工具，`.shell` 使用 `100vw` / `100vh`，地图组件进入 full + 只看地图状态；2026-06-30 17:50 CST 后当前默认使用最高 `1600%` 缩放；同时继续暴露 `data-direct-map-view-behavior=page_shell_map_only` 与地图卡 `page_fixed_fullscreen_map_only`。该模式只为第二屏/PC 大屏观察地图、路线、小车位置和雷达贴图，不启动 RViz2/Foxglove/ROS2 runtime，不执行 Nav2，不发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
