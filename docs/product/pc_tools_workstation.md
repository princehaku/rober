...
- 2026-06-30 17:02 CST 起，普通首屏地图必须保持普通用户“大地图优先”；2026-06-30 22:35 CST 后当前合同为默认 `1200%` 缩放、最高缩放 `1600%`，`/map` 是推荐直达地图大屏入口，`?view=map` 保留兼容，两者都默认进入最高 `1600%`。ROS2 配套只作旁路：RViz2 / `nav2_rviz_plugins` 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位，Foxglove / `foxglove_bridge` 用于浏览器观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-06-30 17:50 CST 起，普通首屏地图必须默认使用 `1200%` 缩放，`/map` 直达地图大屏必须默认使用 `1600%` 上限，`?view=map` 继续兼容，并继续保留同一张 WYSIWYG 画布里的地图、路线、小车位置和雷达贴图。ROS2 配套只作旁路：RViz2 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位；Foxglove 用于部署 bridge 后的浏览器远程观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-06-30 13:17 CST 起，`/map` 与兼容 `?view=map` 直达地图大屏必须切换 App 页面壳到 map-only：隐藏普通顶栏和高级工具，`.shell` 使用 `100vw` / `100vh`，地图组件进入 full + 只看地图状态；2026-06-30 17:50 CST 后当前默认使用最高 `1600%` 缩放；同时继续暴露 `data-direct-map-view-behavior=page_shell_map_only` 与地图卡 `page_fixed_fullscreen_map_only`。该模式只为第二屏/PC 大屏观察地图、路线、小车位置和雷达贴图，不启动 RViz2/Foxglove/ROS2 runtime，不执行 Nav2，不发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
