...
- 2026-07-01 03:24 CST 起，普通首屏地图必须保持普通用户“大地图优先”，但默认改为 `100%` 整图铺满大画布，最高细节放大 `2400%`；`/map` 是推荐直达地图大屏入口，`?view=map` 保留兼容，两者默认先看全图，用户需要看细节时点“细节放大”。ROS2 配套只作旁路：RViz2 / `nav2_rviz_plugins` 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位，Foxglove / `foxglove_bridge` 用于浏览器观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-06-30 15:37 CST 起，普通首屏地图标题行必须把“打开地图大屏”作为第一个主入口，避免现场还要记 query 参数或先理解 RViz2/Foxglove；2026-06-30 22:35 CST 起该入口指向 `/map`，`?view=map` 只作旧书签兼容。该入口必须暴露 `data-user-facing-primary-map-action=true`、`data-ordinary-user-map-entry=true`、`data-opens-new-window=true`、`data-ros2-companion-required=false`；`plain-map-display-proof` 必须同步暴露 `data-primary-map-action-testid=plain-map-direct-view-link`、`data-primary-map-action-label=打开地图大屏`、`data-primary-map-action-opens-new-window=true`。普通用户优先使用 PC 地图大屏；RViz2 只作本地工程调试，Foxglove 只作 bridge 部署后的浏览器远程观察。
...
- 2026-07-01 03:24 CST 起，普通首屏地图必须默认使用 `100%` 整图视图，`/map` 直达地图大屏也默认使用 `100%`，`?view=map` 继续兼容，并继续保留同一张 WYSIWYG 画布里的地图、路线、小车位置和雷达贴图。“细节放大”按钮一键到 `2400%`，`+/-` 支持逐级缩放，“适配”回到 `100%` 全图。ROS2 配套只作旁路：RViz2 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位；Foxglove 用于部署 bridge 后的浏览器远程观察；普通用户默认不离开 PC 简易工作站大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-06-30 11:46 CST 起，普通首屏行程最小预检必须提供机器可读合同：`plain-trip-minimal-precheck` 暴露 `data-minimal-precheck-safety-only=true`，相机、雷达、operator report、路线读回、定位读回和 Nav2 status 均为 `*-preflight-required=false`；PC Node 的 Nav2 preflight / execute 响应也必须返回同一组字段。真正会阻断发车的是现场安全确认、固定目标白名单和危险 true 字段；地图路线所见即所得用于执行对象绑定，轮速 L/R 和送达材料用于执行后复验。
...
- 2026-06-30 11:21 CST 起，普通首屏地图必须提供直达地图大屏入口；2026-06-30 22:35 CST 起推荐入口为 `/map`，`?view=map` 保留兼容。地图卡显示“打开地图大屏”，新窗口打开后页面内自动进入全屏只看地图状态，继续使用同一套地图、路线、小车位置和雷达 overlay WYSIWYG 画布。该入口必须暴露 `data-direct-map-view-requested`、`data-direct-map-view-url="/map"`、`data-direct-map-view-legacy-url="?view=map"` 和 `data-direct-map-view-behavior=page_fixed_fullscreen_map_only`，并固定声明不启动 ROS2/RViz2/Foxglove/Nav2/建图 runtime、不发送任何运动命令。RViz2/Foxglove 仍只是工程观察配套，不替代普通用户地图大屏。
...
- 2026-07-01 03:24 CST 起，普通首屏地图必须继续按 PC 主视图处理：默认 `100%` 整图、最高 `2400%` 细节放大、桌面外壳接近全宽、大图高度贴近整屏，只看地图模式使用普通用户文案并保留路线、小车位置和雷达贴图。ROS2 配套分层为 RViz2 / `nav2_rviz_plugins` 做工程调试，Foxglove / `foxglove_bridge` 做浏览器远程观察；普通用户默认不离开简易 PC 工作站。
...
- 2026-06-30 13:17 CST 起，`/map` 与兼容 `?view=map` 直达地图大屏必须切换 App 页面壳到 map-only：隐藏普通顶栏和高级工具，`.shell` 使用 `100vw` / `100vh`，地图组件进入 full + 只看地图状态；2026-07-01 03:24 CST 后当前默认使用 `100%` 整图，最高 `2400%` 细节放大；同时继续暴露 `data-direct-map-view-behavior=page_shell_map_only` 与地图卡 `page_fixed_fullscreen_map_only`。该模式只为第二屏/PC 大屏观察地图、路线、小车位置和雷达贴图，不启动 RViz2/Foxglove/ROS2 runtime，不执行 Nav2，不发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-06-30 09:36 CST 起，普通首屏 summary 聚合不得被聚合 status、相机设备枚举或 fresh 底盘状态慢读拖住：HTTP 首屏 route 对 `status`、`camera_health`、`camera_devices`、`base_status` 和 `base_feedback_samples_latest` 使用 2400ms 上限，超时只作为分项读取较慢/轮速未知提示。地图、画面、雷达、Nav2 路线和自由移动状态必须先可见。
...
- 2026-06-30 06:05 CST 起，普通首屏地图默认缩放覆盖为 `150%`，缩放档位扩展到 `300%`，大图高度提升到 `clamp(840px, calc(100vh - 132px), 1400px)`，全屏地图高度提升到 `calc(100vh - 96px)`，并暴露 `data-default-map-zoom-percent=150%`。普通操作继续留在 PC 工作站大地图；ROS2 工程观察首选 RViz2，远程浏览器观察可接 Foxglove Bridge/Studio，但 PC 页面不会自动启动这些 ROS2 runtime。
