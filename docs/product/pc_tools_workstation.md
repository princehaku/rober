...
- 2026-06-27 04:46 起，Robot Control summary 对同一 IMU-only 现场材料采用一致口径：即使上位机 artifact 的 `hil_pass=false`、`nav2_goal_execution_proven=false`，只要同一 `latest_result` 同时满足 `goal_succeeded/result_status=succeeded`、`robot_control_executed=true`、`sends_base_motion_commands=true`、`uses_base_uart` 未否定、反馈样本存在且 `base_feedback_summary.imu_attitude_delta_observed=true`，PC summary 会把 `readback_summary.nav2.goal_execution_proven` 推导为 `true`，让普通首屏显示路线已执行并有车身响应；同时继续保留 `goal_execution_hil_pass=false`、`goal_execution_base_feedback_lr_nonzero_proven=false` 和 latest L/R=`0/0`，用于提示 wheel raw L/R 仍需同窗口复验。该口径不把 IMU 姿态变化当作轮速 HIL，不自动确认送达，不放开 `safe_to_control`、`primary_actions_enabled`，也不自动发送 Nav2、manual、keyboard、delivery、stop 或 `/cmd_vel`。
