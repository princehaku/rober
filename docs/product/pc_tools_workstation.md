...
- O7 Previews 现在还包含 `Cloud Archive Tasks` 区块。该区块默认不读取本地路径；operator 输入 archive fixture 路径并点击 `Load archive tasks` 后才调用 `GET /api/o7/cloud-archive/tasks?archiveJson=<local-json>`。UI 只展示 archive status、input status、task list 数量、selected/latest task、safe summaries、route replay inspector、labeling queue inspector、fixed false fields、blocked reasons 和 not proven。route inspector 区域用表格展示 selected task、map frame、frame count、sample frames、event timeline、keyframe refs 和 cursor false fields；labeling inspector 区域用表格展示 sample review items，并展示 label schema、allowed label types、draft labels、dataset export gaps 和标注 false fields，仍不提供播放、提交、导出、发送、控制、停止、取消或恢复类动作按钮。
