...
- 2026-06-29 17:10 CST 起，如果 Robot Control summary 已读到地图/Nav2 等主体事实，但 `base_status` 或 `base_feedback_samples_latest` 只读端点超时，普通首屏 `当前事实` 会单独显示“当前底盘反馈读取超时；旧 L/R 不能当当前轮速结论”。这避免把旧 Nav2 执行窗口 `L/R=0/0` 或历史 samples 误当成当前轮速读回；该提示只消费 summary 的只读连接状态，不发送 manual、keyboard、Nav2、delivery、free-roam、stop 或 `/cmd_vel`。
