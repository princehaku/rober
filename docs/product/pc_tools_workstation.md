...
- 2026-07-02 CST 本轮补强：PC 首页不再把地图当普通卡片，`.shell` 放宽到 `min(2800px, 100%)`，大地图层默认 `min-height=640px`、large 模式 `--plain-map-large-min-height=1680px`、目标高度 `calc(100vh + 720px)`；`/map` 直达页的地图层必须使用 `height: calc(100vh - 88px)` 和同值 `min-height`，避免 flex/grid 百分比高度退回小画布。地图标题短句固定为 `普通看 PC 特大图；工程看 RViz2 / Foxglove`。summary 的 `map_display_too_small_next_action_plain` 必须说明 PC 首页默认就是特大地图，`map_display_ros2_companion_answer_plain` 必须说明普通用户仍用 PC 特大地图和 `/map`；RViz2/Foxglove 仍只作为观察配套，不启动 ROS2/RViz2/Foxglove/Nav2/建图 runtime，不发送 manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
- 2026-07-02 CST 现场复核历史证据：当时 `0.0.0.0:7001` 服务的 `/map` 路由返回 HTTP 200，Chrome DOM smoke 在 `1440x1000` viewport 下测得普通首页地图面板约 `1418x2278`、地图视窗约 `1388x1886`，`/map` 直达页地图面板约 `1440x1000`、地图视窗约 `1432x906`，两处当时为 `data-map-zoom-percent=3200%`；后续当前有效缩放以 `8000%/9600%` 合同为准。因此“PC 地图太小”的默认处理仍是使用 PC 特大地图或访问 `http://<PC>:7001/map`；ROS2 配套只推荐工程观察：本机 RViz2 看 `/map`、`/scan`、TF、路径、定位和 costmap，远程多人观察用 Foxglove bridge + Foxglove Web，不替代普通用户 PC 特大地图，也不作为发车、自由移动或建图前置。
...
- 2026-07-02 CST 起，上位机共享 MJPEG 默认预览首帧采用两段式兜底：第一段优先按低带宽 DV20/UVC 格式矩阵在默认 `/dev/video*` path 上取帧；失败后第二段用剩余预算只尝试 OpenCV index 与 CAP_V4L2 打开方式。这样普通用户多人预览更容易在 path 能 open 但无帧时恢复画面，同时不把整个首屏预算耗在同一格式的 fallback 上。该逻辑仍只读 camera，不独占浏览器页面，不启动 Nav2/manual/keyboard/free-roam/建图 runtime、delivery 或 stop，也不发送 `/cmd_vel`。
...
- 2026-07-02 12:00 CST 起，普通首屏地图和 `/map` 直达大屏默认缩放提升为 `8000%` 现场大图，地图大面板继续占据首屏主视图；`适配` 按钮仍固定回到 `100%` 全图，最高细节放大保持 `9600%`。这是普通用户解决“地图太小”的默认路径；ROS2 配套仍只放在默认折叠的“工程观察”里，RViz2/Foxglove 不作为普通用户发车入口，也不得自动启动 ROS2、Nav2、建图 runtime、manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
