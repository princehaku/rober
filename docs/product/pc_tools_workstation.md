...
- 2026-06-30 15:49 CST 起，普通首屏必须展示 `plain-live-closure-summary` 当前卡点汇总，由 `GET /api/robot-control/summary` 的 `live_closure_summary` 只读字段驱动。该汇总必须同时暴露路线 ready、Nav2 action 成功、同窗口 wheel raw L/R 非零、delivery success、画面/地图/雷达 WYSIWYG、自由移动 ready、建图 ready 和最小安全确认状态；DOM 字段至少包含 `data-route-ready`、`data-nav2-goal-succeeded`、`data-wheel-lr-nonzero-proven`、`data-needs-wheel-rerun`、`data-delivery-success`、`data-camera-current-visible`、`data-map-current-visible`、`data-radar-map-points-visible`、`data-free-move-start-ready`、`data-mapping-start-ready`、`data-minimal-precheck-safety-only` 和 `data-sends-motion-when-clicked=false`。这块只做可读验收和下一步提示，不新增任何控制入口。
- 2026-06-30 15:58 CST 起，普通首屏 `plain-wysiwyg-evidence-refresh` 必须把“当前所见”刷新和雷达贴图刷新串起来：点击后固定调用 no-motion `POST /api/robot-control/radar/scan-proof/refresh`，随后刷新 radar status、map preview 和 camera MJPEG status。按钮必须暴露 `data-fixed-radar-refresh-endpoint=/api/robot-control/radar/scan-proof/refresh`、`data-refreshes-radar-scan-proof=true`、`data-refreshes-map-after-radar=true`、`data-sends-motion-when-clicked=false`；该按钮不得启动/停止雷达 lifecycle，不得调用 Nav2 execute、manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
...
- 2026-06-30 13:17 CST 起，`?view=map` 直达地图大屏必须切换 App 页面壳到 map-only：隐藏普通顶栏和高级工具，`.shell` 使用 `100vw` / `100vh`，地图组件进入 full + 只看地图状态，并默认使用最高 `800%` 缩放；同时继续暴露 `data-direct-map-view-behavior=page_shell_map_only` 与地图卡 `page_fixed_fullscreen_map_only`。该模式只为第二屏/PC 大屏观察地图、路线、小车位置和雷达贴图，不启动 RViz2/Foxglove/ROS2 runtime，不执行 Nav2，不发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
