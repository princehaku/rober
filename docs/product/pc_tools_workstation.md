...
- 2026-06-26 只读 latest 查询也复用固定小车默认地址：`GET /api/robot-control/nav2/goal/execution/latest` 和 `GET /api/robot-control/delivery/latest` 在缺省 `baseUrl` 时默认读取 `http://192.168.1.11:8787`，与 summary 首屏一致，避免现场反复手填。会动作的 manual、first-jog、Nav2 execute、delivery complete、operator report、map/radar lifecycle 等 POST 仍保持显式 baseUrl、确认项和原有 fail-closed gate，不因默认地址自动执行。
