...
- 2026-07-02 12:00 CST 起，普通首屏地图和 `/map` 直达大屏默认缩放提升为 `4800%` 现场大图，地图大面板继续占据首屏主视图；`适配` 按钮仍固定回到 `100%` 全图，最高细节放大保持 `9600%`。这是普通用户解决“地图太小”的默认路径；ROS2 配套仍只放在默认折叠的“工程观察”里，RViz2/Foxglove 不作为普通用户发车入口，也不得自动启动 ROS2、Nav2、建图 runtime、manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
