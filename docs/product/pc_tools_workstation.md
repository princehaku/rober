...
- 2026-06-23 00:25 起，普通首屏最终 `确认送达` 按钮在已有送达草稿但 checklist 未齐时不再只显示缺项数量，而是按当前第一组缺口显示 `确认送达（先勾选安全）`、`确认送达（先确认到达）`、`确认送达（先核对材料）` 或 `确认送达（先确认投放）`。按钮禁用状态和 gate 不变；这些文案只引导 operator 勾选本地确认项，不提交 operator report、不调用 delivery complete、不发送 Nav2、manual 或 `/cmd_vel`。
