...
- 2026-07-01 CST 起，普通首屏地图必须保持普通用户“大地图优先”；当前合同为默认 `2400%` 缩放、最高缩放 `2400%`，`/map` 是推荐直达地图大屏入口，`?view=map` 保留兼容，两者都默认进入最高 `2400%`。ROS2 配套只作旁路：RViz2 / `nav2_rviz_plugins` 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位，Foxglove / `foxglove_bridge` 用于浏览器观察；普通用户默认不离开 PC 简易工作站超大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
- 2026-07-01 02:38 CST 起，`live_closure_summary` API 和普通首屏 `plain-live-closure-summary` 必须结构化暴露地图显示/ROS2 配套合同：普通用户主工具固定为 `pc_big_map`，主入口 `/map`，兼容入口 `?view=map`，默认/最高缩放 `2400%`，WYSIWYG overlay 为 `image,route,robot,radar`；ROS2 配套工具固定为 `rviz2,foxglove` 且 `map_display_ros2_companion_required=false`，RViz2 命令为 `ros2 launch ros2_trashbot_bringup rviz.launch.py`，Foxglove bridge 命令为 `ros2 launch foxglove_bridge foxglove_bridge_launch.xml`。这些字段只用于显示、脚本验收和现场解释，不启动 ROS2/RViz2/Foxglove/Nav2/建图 runtime，不发送任何运动命令。窄 PC 窗口下地图媒体查询也必须保持主视图高度，不得把大地图压回最高 520px 的小卡片。
...
- 2026-07-01 CST 起，普通首屏地图必须默认使用 `2400%` 缩放，`/map` 直达地图大屏也默认使用 `2400%` 上限，`?view=map` 继续兼容，并继续保留同一张 WYSIWYG 画布里的地图、路线、小车位置和雷达贴图。“适配”按钮必须保留 `100%` 回退，方便查看全图。ROS2 配套只作旁路：RViz2 用于本地工程调试 `/map`、`/scan`、TF、规划轨迹和定位；Foxglove 用于部署 bridge 后的浏览器远程观察；普通用户默认不离开 PC 简易工作站超大地图。该合同只影响显示和只读 DOM，不得启动 ROS2/RViz2/Foxglove、不得执行 Nav2、不得发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
...
- 2026-06-30 23:30 CST 起，PC 地图 ROS2 配套说明必须把工程观察命令结构化暴露到 `plain-map-panel`、`plain-map-display-proof` 和 `plain-map-ros2-tool-note`：`data-rviz-launch-command="ros2 launch ros2_trashbot_bringup rviz.launch.py"`、`data-foxglove-bridge-package=foxglove_bridge`、`data-foxglove-bridge-launch-command="ros2 launch foxglove_bridge foxglove_bridge_launch.xml"`。这些字段只作为现场手工启动/脚本读取提示，不表示 PC 页已经启动 RViz2/Foxglove，不改变普通用户优先打开 `/map` 大地图的路径，也不得触发 ROS2 runtime、Nav2、建图、manual、keyboard、free-roam、delivery、stop 或 `/cmd_vel`。
...
- 2026-07-01 CST 起，普通首屏地图必须继续按 PC 主视图处理：默认 `2400%` 缩放、最高 `2400%`、桌面外壳接近全宽、大图高度贴近整屏，只看地图模式使用普通用户文案并保留路线、小车位置和雷达贴图。ROS2 配套分层为 RViz2 / `nav2_rviz_plugins` 做工程调试，Foxglove / `foxglove_bridge` 做浏览器远程观察；普通用户默认不离开简易 PC 工作站。
...
- 2026-06-30 13:17 CST 起，`/map` 与兼容 `?view=map` 直达地图大屏必须切换 App 页面壳到 map-only：隐藏普通顶栏和高级工具，`.shell` 使用 `100vw` / `100vh`，地图组件进入 full + 只看地图状态；2026-07-01 CST 后当前默认使用最高 `2400%` 缩放；同时继续暴露 `data-direct-map-view-behavior=page_shell_map_only` 与地图卡 `page_fixed_fullscreen_map_only`。该模式只为第二屏/PC 大屏观察地图、路线、小车位置和雷达贴图，不启动 RViz2/Foxglove/ROS2 runtime，不执行 Nav2，不发送 manual/keyboard/free-roam/delivery/stop 或 `/cmd_vel`。
