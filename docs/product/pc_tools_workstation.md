...
`ROS2观察`；`plain-map-wysiwyg-layer-strip` 改成地图画布内浮层，不再占用垂直布局高度，地图层按紧凑工具条
...
首页地图卡会默认收起长说明、地图生命周期按钮和工程说明正文，把首屏高度让给实时地图；点击 `ROS2观察` 才展开 RViz2/Foxglove 只读观察入口。
