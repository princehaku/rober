...
- 2026-06-23 23:55 起，Vite 开发入口也同步为默认 `0.0.0.0:7071`；直接运行 `npm run dev` 就能在同局域网访问前端开发页，仍可用 `HOST=<host> PORT=<port>` 覆盖。`npm run api` 继续是 Node/Express 构建后入口，两个入口不能同时占用 7071；这只改变 PC 工作站 HTTP 可达性，不自动执行 Nav2、manual、delivery complete、keyboard pulse、stop 或 `/cmd_vel`。
