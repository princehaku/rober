...
本轮 `2026.07.10_17-22_o5_production_cutover_readiness_packet` 新增 `trashbot.cloud_production_cutover_readiness_packet.v1`，证据边界是 `software_proof_cloud_production_cutover_readiness_packet_only`。该 packet 只聚合 existing relay 的生产 cutover 相关 artifact 摘要：cloud deployment readiness、cloud external probe、public ingress/TLS、DB/queue external probe、worker migration rehearsal、worker cutover drain、OSS/CDN live probe 和 external evidence intake。CLI 可用 `--write-cloud-production-cutover-readiness-packet-artifact <packet.json>` 写出，`--preflight --cloud-production-cutover-readiness-packet-artifact <packet.json>` 可消费。packet 只输出短状态、counts、安全 basename/hash prefix、blocked reasons、next required evidence 和 `next_live_command`；不回显 artifact 路径、URL、Authorization/bearer、token、连接串、base64/raw、response body、traceback、串口、ROS topic 或 `/cmd_vel`。即使 packet 有效，也必须固定 `production_ready=false`、`okr_credit_allowed=false`、`support_only_reason=no_real_production_external_evidence`、`proof_scope_class=software_proof_support_only`、`connects_cloud_production=false`、`delivery_success=false`、`safe_to_control=false`、`primary_actions_enabled=false`、`robot_control_executed=false`。它只证明 O5 cutover readiness 合同可机读和 preflight 可消费，不证明真实公网 HTTPS/TLS、真实 4G/SIM、production DB/queue、production worker/cutover、OSS/CDN live traffic、真实手机/browser 或真实 delivery success。
