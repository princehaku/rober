...
本轮 `2026.07.10_17-22_o5_production_cutover_readiness_packet` 新增 `trashbot.cloud_production_cutover_readiness_packet.v1`，证据边界是 `software_proof_cloud_production_cutover_readiness_packet_only`。该 packet 只聚合 existing relay 的生产 cutover 相关 artifact 摘要：cloud deployment readiness、cloud external probe、public ingress/TLS、DB/queue external probe、worker migration rehearsal、worker cutover drain、OSS/CDN live probe、CDN/TLS external evidence、external evidence intake 和 cloud external evidence review decision。CLI 可用 `--write-cloud-production-cutover-readiness-packet-artifact <packet.json>` 写出，`--preflight --cloud-production-cutover-readiness-packet-artifact <packet.json>` 可消费；`TRASHBOT_REMOTE_CLOUD_CDN_TLS_EXTERNAL_EVIDENCE_ARTIFACT` 或 `--cdn-tls-external-evidence-artifact` 会把 `trashbot.o5.cdn_tls_external_evidence.v1` 作为独立 `cdn_tls_external_evidence` source slot 消费；`TRASHBOT_REMOTE_CLOUD_EXTERNAL_EVIDENCE_REVIEW_DECISION_ARTIFACT` 或 `--cloud-external-evidence-review-decision-artifact` 会把 `trashbot.cloud_external_evidence_review_decision.v1` 作为独立 `cloud_external_evidence_review_decision` source slot 消费。13:13 的 `blocked_http_status_not_success_class` / `http_status_class=4xx` artifact 会被读取并保留 TLS/cert observation，但 section 仍必须是 `blocked_not_proven`；未来 success-class artifact 最多让该 section 进入 `software_proof_ready`，packet 本身仍是 support-only。packet 只输出短状态、counts、安全 basename/hash prefix、blocked reasons、next required evidence 和 `next_live_command`；不回显 artifact 路径、URL、Authorization/bearer、token、连接串、base64/raw、response body、traceback、串口、ROS topic 或 `/cmd_vel`。即使 packet 有效，也必须固定 `production_ready=false`、`okr_credit_allowed=false`、`support_only_reason=no_real_production_external_evidence`、`proof_scope_class=software_proof_support_only`、`connects_cloud_production=false`、`delivery_success=false`、`safe_to_control=false`、`primary_actions_enabled=false`、`robot_control_executed=false`。它只证明 O5 cutover readiness 合同可机读和 preflight 可消费，不证明真实公网 HTTPS/TLS、真实 4G/SIM、production DB/queue、production worker/cutover、OSS/CDN live traffic、真实手机/browser 或真实 delivery success。

本轮 `2026.05.24_07-08_cloud-command-lifecycle-acceptance-http-export` 在 independent relay HTTP server 上新增 support-safe GET export：`GET /api/support/cloud-command-lifecycle-replay-acceptance-packet-export`。响应 capability 为 `cloud_command_lifecycle_replay_acceptance_packet_http_export`，证据边界为 `software_proof_docker_cloud_command_lifecycle_replay_acceptance_packet_http_export_gate`，并复用 CLI export helper，保留 `software_proof_docker_cloud_command_lifecycle_replay_acceptance_packet_cli_export_gate`、源 packet boundary `software_proof_docker_cloud_command_lifecycle_replay_acceptance_packet_gate`、`accepted_processing_only_not_delivery_success`、`terminal_result_pending`、`owner_handoff`、`next_required_evidence`、`not_proven`、`delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false` 和 `redaction_status=passed`。该 route 和 `/api/status` 一样不要求 bearer auth，因为它只读且 phone/support-safe；它不 replay/resubmit command、不 post ACK、不 mutate cursor/state、不上传材料、不触发 GitHub action、不控制 Nav2/机器人、不写 delivery success。它仍是 Docker/local software proof：not delivery success、not true phone/browser proof、not real external cloud proof、not public HTTPS/TLS、not 4G/SIM、not OSS/CDN live traffic、not production DB/queue、not worker/cutover、not HIL、not PR #5 resolution；no OKR percentage lift。

本轮 `2026.07.14_14-38_o5_command_lifecycle_cli_export_refresh` 刷新同一 CLI 导出链路，产物为 `sprints/2026.07.14_14-38_o5_command_lifecycle_cli_export_refresh/artifacts/o5_command_lifecycle_cli_export.json`。artifact schema 继续是 `trashbot.cloud_command_lifecycle_replay_acceptance_packet_cli_export.v1`，capability 继续是 `cloud_command_lifecycle_replay_acceptance_packet_cli_export`，证据边界继续是 `software_proof_docker_cloud_command_lifecycle_replay_acceptance_packet_cli_export_gate`，状态为 `export_ready_for_field_owner_review_not_proven`。该刷新只给 support/field-owner review 提供 phone-safe JSON，继续固定 `accepted_processing_only_not_delivery_success`、`terminal_result_pending`、`delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false`；O5 仍约 `85%`，KR `不归档`。它不证明 production cloud、production DB/queue、4G/SIM、OSS/CDN live traffic、真实 phone/browser、route execution、delivery、HIL、safe-to-control 或机器人控制。

本轮 `2026.05.26_00-01_cloud-phone-command-api-mainline` 在 independent relay 上新增 bearer-gated phone command API：`POST /api/commands/collect`、`POST /api/commands/confirm-dropoff`、`POST /api/commands/cancel`。这些入口只接受任务级 `robot_id`、可选 `command_id` 或 `idempotency_key` 以及任务 `payload`，再规范化为既有 command store 的 `collect`、`confirm_dropoff`、`cancel` 命令并复用 `submit_command()`；机器人侧 `/robots/{robot_id}/commands/next`、status 和 ACK 语义不变。响应 capability 为 `cloud_phone_command_api`，证据边界为 `software_proof_docker_cloud_phone_command_api_gate`，并强制保留 `ack_semantics=queued_not_delivery_success`、`delivery_success=false`、`primary_actions_enabled=false`、`safe_to_control=false`。command store 不可用时必须返回 phone-safe `503 command_store_unavailable`，不能返回 queued receipt。该 API 只证明 Docker/local 同源任务提交和队列入队 gate，不证明真实公网 HTTPS/TLS、真实 4G/SIM、production DB/queue、Nav2/fixed-route、WAVE ROVER/HIL、机器人实际执行或送达成功；错误响应不得暴露 Authorization、bearer token、raw state path、ROS topic、`/cmd_vel`、serial/UART 或 WAVE ROVER 字段。

本轮 `2026.05.26_08-09_cloud-command-result-reconciliation` 在同一 independent relay 上新增 bearer-gated、只读的同源结果对账 route：`GET /api/commands/{command_id}/result?robot_id=<robot_id>`。它复用现有 command store、最近 status 和 terminal ACK envelope，返回 schema `trashbot.cloud_command_result_reconciliation.v1`、capability `cloud_command_result_reconciliation`、证据边界 `software_proof_docker_cloud_command_result_reconciliation_gate`，并把 lifecycle 压成 `queued`、`processing`、`terminal_result_pending`、`missing_or_expired` 或 `store_unavailable`。该 route 不 enqueue/replay/cancel、不推进 ACK cursor、不修改 status、不触发 ROS 或机器人控制、不绕过 robot outbound polling；`terminal_result_pending` 只表示云端收到 terminal ACK envelope，仍不是 delivery/dropoff/cancel success。所有响应必须保持 `delivery_success=false`、`safe_to_control=false`、`primary_actions_enabled=false`，并只输出 `safe_copy`、`next_required_evidence` 等 phone-safe 字段；不得暴露 Authorization、bearer token、raw state path、DB/queue URL、ROS topic、`/cmd_vel`、serial/UART、WAVE ROVER、完整 artifact、checksum 或 traceback。

本轮 `2026.05.26_07-08_cloud-command-terminal-result-mainline` 在 independent relay 上新增 robot-facing terminal result 主路径：`POST /robots/{robot_id}/commands/{command_id}/terminal-result`。写入 schema 为 `trashbot.cloud_command_terminal_result.v1`，capability 为 `cloud_command_terminal_result`，证据边界为 `software_proof_docker_cloud_command_terminal_result_gate`；query schema 升级为 `trashbot.cloud_command_result_reconciliation.v2`。写入前必须确认同一 `robot_id + command_id` 的 command 已存在，且 body 与路径匹配；不存在时返回 `terminal_result_missing`，不同结果重复写入返回 `terminal_result_conflict` 且不覆盖，完全相同结果重复写入返回幂等 `terminal_result_recorded`。file-backed 和 SQLite-backed proof store 都持久化 terminal result，随后 `GET /api/commands/{command_id}/result?robot_id=<robot_id>` 可返回 `terminal_result_recorded`、`terminal_result_type`、`task_terminal_state`、`result_code` 和 safe refs。ACK terminal 但没有 terminal result 时仍保持 `terminal_result_pending`。所有响应继续固定 `delivery_success=false`、`safe_to_control=false`、`primary_actions_enabled=false`、`real_world_delivery_proven=false`；不得暴露 Authorization、bearer token、raw state path、DB/queue URL、ROS topic、`/cmd_vel`、serial/UART、WAVE ROVER、完整 artifact、checksum 或 traceback。该能力只证明 Docker/local software terminal result 主路径，不证明公网 HTTPS/TLS、真实 4G/SIM、production DB/queue、OSS/CDN live traffic、HIL、Nav2/fixed-route 或真实送达。

为避免 O5 / O6 / O7 再次各自维护一套本地串联脚本，[`/Users/m1/apps/rober/onboard/scripts/o5_same_task_mission_archive_smoke.py`](/Users/m1/apps/rober/onboard/scripts/o5_same_task_mission_archive_smoke.py) 继续串起 `POST /api/commands/confirm-dropoff`、`POST /robots/{robot_id}/commands/{command_id}/terminal-result`、`GET /api/commands/{command_id}/result?robot_id=...` 的 reconciliation v2、Algorithm manifest、`POST /api/o6/archive/field-evidence` 和 `GET /api/o6/consumer/tasks/<task_id>?include=same_task_mission_evidence_gate,cloud_external_probe,cloud_db_queue_external_probe`。脚本默认 `--state-backend file` 保持上一轮兼容；`sqlite` shadow 模式继续使用 `build_server(..., state_backend="sqlite")` 写入 terminal result 后关闭 relay，再用同一 SQLite state path 重启 relay 并读回 reconciliation。新加的 probe readback 复用既有 `cloud_external_probe` / `cloud_db_queue_external_probe` artifact summary 逻辑：前者只验证本地 relay 的 `/healthz`、`/readyz`、`/preflightz` 合同可被安全摘要消费，后者只把 DB/queue probe 的枚举化 blocked 状态矩阵并入同一 `task_id`。本轮 summary 固定输出 `same_task_mission_gate_ready_not_success_proof`、`cloud_external_probe_ready_not_production_proof`、`cloud_db_queue_external_probe_ready_not_production_proof`，以及全 false 的 `connects_cloud_production`、`delivery_success`、`safe_to_control`、`primary_actions_enabled`、`robot_control_executed`。这条 smoke 只证明 `software_proof_o5_o6_live_endpoint_probe_readback_only`：同一轮本地/mock 材料可从 O5 relay readback 进入 O6 archive/readback，并安全携带 live endpoint probe 摘要；它不证明真实公网 HTTPS/TLS、真实 4G/SIM、真实 production cloud、production DB、queue、多实例一致性、真实 live Nav2 route execution、真实 operator confirmation、真实 delivery success、真实 OSS/CDN 或真实手机/browser。

本轮 `2026.07.13_20-20_o6_o7_phone_browser_proof_intake` 在 O6 local/mock archive 中新增 `phone_browser_terminal_material` 安全摘要 intake/readback。该 section 只接受同一 `task_id` 的 phone/browser terminal material 枚举、terminal result 类型和 basename `safe_evidence_ref`，proof boundary 固定为 `software_proof_o6_o7_phone_browser_terminal_material_intake_only`，并在 archive task、`field_evidence`、`artifact_bundle`、consumer detail alias 与 `include=phone_browser_terminal_material` 中回读。输出必须保持 `safe_to_control=false`、`delivery_success=false`、`route_execution_success=false`、`hil_pass=false`、`connects_cloud_production=false`、`robot_control_executed=false`；raw URL、cookie、Authorization、token、本地路径、screenshot body、DOM dump、traceback、`/cmd_vel`、serial/UART、WAVE ROVER 或 dangerous true field 都 fail-closed。该材料只证明 O6/O7 可安全消费同 task 的本地/mock 摘要，不证明真实手机/browser 操作、生产公网 HTTPS/TLS、真实 4G/SIM、production DB/queue、OSS/CDN live traffic、路线执行、送达或 HIL。

本轮 `2026.07.13_21-21_o6_o7_bounded_route_gate_intake` 在 O6 local/mock archive 中新增 `bounded_route_execution_gate_material` 安全摘要 intake/readback。该 section 只接收 07:07 controlled route execution gate 与 08:09 bounded route command plan 的同 task 摘要，proof boundary 固定为 `software_proof_o6_o7_bounded_route_gate_material_intake_only`，ready status 统一为 `bounded_route_execution_gate_material_ready_not_route_execution_proof`，并回读 exact `packet_id=packet_o3_28_pose_same_task_replay_7d57826142b0c79c`、`task_id=task_o3_28_pose_fixed_route_consumer_20260713_0402`、`route_intent_id=route_intent_20260713_0402_from_20260713_0300_28_pose_structured_path`、`route_csv_row_count=28`、`path_structured_pose_count=28`、`segment_count=27`、`execution_plan_status=blocked_pending_live_safety_gate`。输出必须保持 `safe_to_control=false`、`delivery_success=false`、`route_execution_success=false`、`hil_pass=false`、`robot_control_executed=false`、`connects_cloud_production=false`；raw local path、raw command body、`/cmd_vel`、`/api/base/manual`、NavigateToPose、serial/UART、WAVE ROVER 或任何 route/delivery/HIL/control true field 都 fail-closed。该材料只证明 O6/O7 可安全消费 bounded gate 摘要，不证明 route execution、delivery、HIL、safe-to-control、control capability、真实 4G/SIM 或 production cloud。

本轮 `2026.07.14_00-24_o5_bounded_route_terminal_result_bridge` 新增本地/mock O5 `bounded_route_terminal_result_bridge` CLI，证据边界固定为 `software_proof_o5_bounded_route_terminal_result_bridge_only`。该 bridge 只消费 23:23 O3 `trashbot.o3.bounded_route_mock_execution.v1` summary 的 safe basename、same-task identity 和计数字段，然后通过既有 relay HTTP 主路径依次执行 `POST /api/commands/collect`、`POST /robots/{robot_id}/commands/{command_id}/terminal-result`、`GET /api/commands/{command_id}/result?robot_id=...`。写入的 terminal result 使用 `result_code=mock_route_execution_completed_not_live_delivery`，对账必须回读 `terminal_result_recorded`，capability 覆盖 `cloud_phone_command_api`、`cloud_command_terminal_result` 和 `cloud_command_result_reconciliation`。summary artifact schema 为 `trashbot.o5.bounded_route_terminal_result_bridge.v1`，固定 `delivery_success=false`、`route_execution_success=false`、`safe_to_control=false`、`hil_pass=false`、`robot_control_executed=false`、`connects_cloud_production=false`、`uses_base_uart=false`、`publishes_cmd_vel=false`、`calls_base_manual=false`。它只证明本地 relay command/result/reconciliation 软件链路可串通，不证明 production cloud、公网 HTTPS/TLS、4G/SIM、production DB/queue、OSS/CDN live traffic、真实 route execution、delivery/operator acceptance、HIL 或 safe-to-control。

本轮 `2026.07.14_01-24_o6_o7_bounded_route_terminal_result_intake` 在 O6 archive/consumer readback 中新增 `bounded_route_terminal_result_material` additive section，证据边界固定为 `software_proof_o6_o7_bounded_route_terminal_result_intake_only`。该 section 只消费上一段 O5 summary 的安全摘要，source schema 必须是 `trashbot.o5.bounded_route_terminal_result_bridge.v1`，source boundary 必须是 `software_proof_o5_bounded_route_terminal_result_bridge_only`，O6 schema 固定为 `trashbot.o6.bounded_route_terminal_result_material.v1`，ready status 固定为 `bounded_route_terminal_result_material_ready_not_delivery_proof`。archive 和 consumer detail 只回显 exact `task_id=task_o3_28_pose_fixed_route_consumer_20260713_0402`、`packet_id=packet_o3_28_pose_same_task_replay_7d57826142b0c79c`、`route_intent_id=route_intent_20260713_0402_from_20260713_0300_28_pose_structured_path`、route/pose/segment counts、`result_code=mock_route_execution_completed_not_live_delivery`、`terminal_result_state=terminal_result_recorded`、`reconciliation_state=terminal_result_recorded`、安全 basename ref、blocked reasons 和 next evidence；同时固定 `delivery_success=false`、`route_execution_success=false`、`safe_to_control=false`、`hil_pass=false`、`robot_control_executed=false`、`connects_cloud_production=false`。raw path、raw command、URL/token、`/cmd_vel`、`/api/base/manual`、NavigateToPose、serial/UART、WAVE ROVER 或危险 true 会使该 section fail-closed 为 `blocked_not_proven`。它只证明 O6/O7 能安全读回 terminal result bridge 摘要，不证明 production cloud、4G/SIM、真实 route execution、delivery、HIL 或 safe-to-control。

本轮 `2026.07.14_04-27_o5_delivery_state_terminal_reconciliation` 新增 `DeliveryStateMachine` 离线 terminal-result reconcile 和 `trashbot.o5.delivery_state_terminal_reconciliation.v1` summary。该入口只读 00:24 O5 bridge summary，校验 `schema=trashbot.o5.bounded_route_terminal_result_bridge.v1`、`proof_boundary=software_proof_o5_bounded_route_terminal_result_bridge_only`、`result_code=mock_route_execution_completed_not_live_delivery`、同 task identity、`terminal_result_state=terminal_result_recorded`、`reconciliation_state=terminal_result_recorded` 和所有安全 false 字段；任何 schema 漂移、缺少 identity、dangerous true field 或 live/success state 都 fail closed 且不写 artifact。有效输入也只能输出 `terminal_result_accepted_for_delivery=false`、`delivery_success=false`、`route_execution_success=false`、`safe_to_control=false`、`hil_pass=false`、`final_state=error` 和 `reconciliation_status=fail_closed_mock_terminal_result_not_delivery`。这只是状态机可读解释层：mock terminal result 不是真实 delivery、dropoff success、route execution、operator acceptance、HIL、safe-to-control、production cloud、4G/SIM 或 OSS/CDN live traffic 证据。

本轮 `2026.07.14_05-28_o5_delivery_state_live_success_gate` 在 `DeliveryStateMachine` 中新增 `delivery_state_live_success_gate` 合同，artifact schema 固定为 `trashbot.o5.delivery_state_live_success_gate.v1`，proof boundary 固定为 `software_proof_o5_delivery_state_live_success_gate_only`。该合同只定义未来什么证据可以进入状态机 delivery success：source mode 必须是真实 live，task/robot/packet/route/terminal result identity 必须同任务一致，且 live route execution success、operator/dropoff acceptance、HIL pass、safe-to-control、terminal result record、fresh/same-window evidence 必须同时满足。本轮 CLI 只生成 synthetic-current-live 形状 summary，固定 `live_success_gate_contract_ready=true`、`current_live_evidence_observed=false`、`delivery_success_claimed_by_this_run=false`、`real_world_delivery_proven=false`、`safe_to_control=false`、`hil_pass=false` 和 `delivery_success_accepted_for_state_machine=false`；它不是 delivery、dropoff success、真实 route execution、operator acceptance、HIL、safe-to-control、production cloud、真实 4G/SIM、真实 phone/browser 或 OSS/CDN live traffic 证明。

本轮 `2026.07.14_07-29_o5_operator_dropoff_acceptance_gate` 新增 `operator_dropoff_acceptance` 独立证据 gate，artifact schema 固定为 `trashbot.o5.operator_dropoff_acceptance_gate.v1`，proof boundary 固定为 `software_proof_o5_operator_dropoff_acceptance_gate_only`。该 gate 只把 operator/user 的 dropoff acceptance 整理成 live-success gate 可消费的必要输入；它本身不充分证明 delivery success。正向 acceptance 必须同时满足 `source_mode=live`、同 task terminal result recorded、live route execution success、同 task `operator_dropoff_acceptance`、HIL pass、`safe_to_control=true` 和 same-window freshness；任一缺失、identity mismatch、unsafe evidence ref、stale evidence 或非 live source 携带 dangerous true 都输出 `acceptance_decision=blocked_missing_live_success_evidence`。当前 synthetic artifact 固定 `delivery_success=false`、`route_execution_success=false`、`safe_to_control=false`、`hil_pass=false` 和 `operator_dropoff_acceptance_gate_accepted=false`，只证明状态机有可复核的 operator acceptance 收件门，不证明真实手机/browser、production cloud、4G/SIM、route execution、delivery、HIL 或 safe-to-control。
...
- `TRASHBOT_REMOTE_CLOUD_CDN_TLS_EXTERNAL_EVIDENCE_ARTIFACT`：可选的 sanitized CDN/TLS external evidence artifact 路径，只供 preflight 和 `cloud_production_cutover_readiness_packet` 的 `cdn_tls_external_evidence` slot 消费。`blocked_http_status_not_success_class` / 4xx artifact 只记录 TLS/cert observation 并保持 fail-closed；success-class artifact 也只能进入 `software_proof_o5_cdn_tls_external_evidence_readiness_packet_consumption_only`，不得作为 production cloud ready、OSS/CDN 回源、4G/SIM、delivery 或 safe-to-control 证据。
...
python3 pc-tools/evidence/cloud_external_evidence_review_decision.py \
  ...
  --evidence-ref external_evidence_ref_20260524_0001 \
