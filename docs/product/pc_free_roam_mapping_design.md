...
8. 如果“自动扫图准备”显示 `刷新雷达后开始`，先按它跳到 `刷新雷达` 并刷新 LiDAR proof；只有准备状态变为 `已就绪` 后，才可点击“开始自动扫图（低速）”启动上车端状态机。PC 会在地图上显示 `自动扫图已启动`，并立即刷新一次雷达和地图画面，`下一步` 会带到 `停止自动扫图`，继续负责地图/雷达监看和停止兜底。
