...
6. 松开按键或点击“停止”收口。
7. 点击“保存当前地图”，保存完成后 PC 会自动刷新地图画面，再检查 free cell、地图尺寸、覆盖提示和可导航状态。
8. 如果“自动扫图准备”已显示 `自动扫图 / 已就绪`，可点击“自动扫图”启动上车端状态机；PC 会在地图上显示 `自动扫图已启动`，并立即刷新一次雷达和地图画面，继续负责地图/雷达监看和停止兜底。
