...
- 2026-07-04 08:08 起，上述缓存不会被 PC 代理层深度探针超时误覆盖：`backendSmoke` 请求本身
...
- 2026-07-06 02:13 起，当前地图显示口径为 PC 首页和 `/map` 默认 `200%` 可读大图，
  `完整态势` 回 `100%`，`细节放大` 到 `1200%`；`/map` 工具条和图层状态悬浮在地图上，不再占画布高度。
...
- 2026-07-04 00:16 起，`/map` 直达页的内部地图层跟随可见地图 viewport，不再继承通用 fullscreen 的
  ...
  当时默认用 100% 完整态势显示真实地图、路线、小车、雷达和目标点；2026-07-04 02:35 起曾改为 150% 可读大图，点“完整态势”仍回到 100% 全局视角，不启动 RViz2/Foxglove/ROS2 runtime，也不发送任何运动指令。
- 2026-07-04 01:10 起，真实地图 PNG 的 PC 渲染曾改为高度优先：`.plain-map-layer.has-real-map`
  当时按画布高度设置 overlay frame，宽图横向滚动，避免宽地图在大画布里只贴住上半屏。该变化只放大普通用户可见地图，
...
- 2026-07-04 03:20 起，当时默认缩放再提升到 `200%` 大图，`/map` 同步使用
...
- 2026-07-04 04:05 起，当时默认缩放再提升到 `300%` 大图，`/map` 同步使用
...
- 2026-07-04 04:40 起，当时默认缩放继续提升到 `400%` 大图，`/map` 同步使用
...
- 2026-07-04 05:19 起，当时默认缩放继续提升到 `800%` 大图，`/map` 同步使用
  ...
  普通用户仍先用 PC 大地图和 `/map`。
- 2026-07-06 05:07 历史复核实现：代码、summary/live-summary 与 7001 运行态曾统一为 PC 首页和
...
- 2026-07-06 05:36 当前有效显示口径：普通首页地图卡排在图传、WASD 和连接状态之前，默认回到
  `100%` 完整态势，真实地图按画布宽度铺满；机器人位置、Nav2 路线、雷达点和目标点需要同屏可见。
  `细节放大` 仍可到 `4800%`，只用于局部排障。`/map` 直达页继续满屏只看地图，RViz2/Foxglove
  仍只作为工程观察，不替代 PC 简易控制台，也不发送任何运动或建图命令。
...
完整态势，最高 `4800%` 只作为局部排障放大；点“完整态势”回到 `100%` 全局视角。扫图向导继续把 PC 大地图作为普通入口，
...
地图太小的操作口径继续固定：普通用户先用 PC 首页全宽大地图或 `/map` 直达页，当前默认 `100%` 完整态势，
