...
`owner_handoff` 固定把真实采集交给 robot-hardware-engineer，Robot 只读消费 review-decision summary，Full-stack 保持 mobile execution pack 只读且 `primary_actions_enabled=false`。
