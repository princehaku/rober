...
generates the PC-only canonical handoff gate after
`field_evidence_rerun_execution_callback_review_decision.py`. It consumes only
a supported execution callback review-decision artifact/summary/wrapper JSON
or Robot safe alias; it does not read field material directories, execute
field reruns, schedule robot actions, or expose raw callback material.
...
- Allowed source inputs:
  `trashbot.field_evidence_rerun_execution_callback_review_decision.v1`,
  `trashbot.field_evidence_rerun_execution_callback_review_decision_summary.v1`,
  and `robot_diagnostics_field_evidence_rerun_execution_callback_review_decision_summary`
  under `software_proof_docker_field_evidence_rerun_execution_callback_review_decision_gate`.
- Handoff statuses:
  `ready_for_field_evidence_rerun_execution_callback_review_handoff`,
  `needs_owner_follow_up`,
  `needs_field_evidence_rerun_execution_callback_rerun`,
  `evidence_ref_mismatch_rerun`, and
  `blocked_unsafe_review_handoff`.
...
`review_decision=ready`, matched safe `evidence_ref`,
`same_evidence_ref_required=true`, `same_evidence_ref_status=matched`,
`source=software_proof`, `not_proven`, and all action flags false maps to
`ready_for_field_evidence_rerun_execution_callback_review_handoff`. Source
`missing` or `rejected` maps to `needs_owner_follow_up`; source `blocked`
maps to `needs_field_evidence_rerun_execution_callback_rerun`. Mismatched
evidence refs, missing safe evidence_ref, weak same-ref typing, or non-matched
same-ref status map to `evidence_ref_mismatch_rerun`. Missing input, bad JSON,
unsupported schema or boundary, missing software-proof/not-proven fields, or
unsupported review decisions map to `needs_field_evidence_rerun_execution_callback_rerun`.
Unsafe copy, raw paths, credentials, ROS topic text, `/cmd_vel`, serial/UART or
WAVE ROVER text, checksum text, complete/raw artifact text, traceback text,
success/control wording, true field pass wording, Objective 5 external proof
claims, PR #5 resolution claims, `safe_to_control=true`,
`delivery_success=true`, or `primary_actions_enabled=true` map to
`blocked_unsafe_review_handoff`.
...
generates the PC-only canonical result-intake gate after
`field_evidence_rerun_execution_callback_review_handoff.py`. It consumes only
a supported execution callback review-handoff artifact/summary/wrapper JSON
or Robot safe alias, plus an optional owner-safe execution result packet; it
does not read field material directories, execute field reruns, schedule robot
...
- Allowed source inputs:
  `trashbot.field_evidence_rerun_execution_callback_review_handoff.v1`,
  `trashbot.field_evidence_rerun_execution_callback_review_handoff_summary.v1`,
  and `robot_diagnostics_field_evidence_rerun_execution_callback_review_handoff_summary`
  under
  `software_proof_docker_field_evidence_rerun_execution_callback_review_handoff_gate`.
- Optional result packet inputs:
  `trashbot.field_evidence_rerun_execution_result_packet.v1`,
  `trashbot.field_evidence_rerun_execution_result_packet_summary.v1`, or an
  owner-safe object with `source=software_proof`, `not_proven`, same safe
  `evidence_ref`, `packet_status`, sanitized `material_summary`, and all action
  flags false.
- Result intake status values:
  `missing`, `accepted`, `rejected`, and `blocked`.

The output always includes `source=software_proof`, `result_intake_status`,
source `handoff_status`, `result_packet_status`, safe `evidence_ref`,
`same_evidence_ref_required=true`, `same_evidence_ref_status`,
...
Result-intake mapping is fail-closed. A supported, safe handoff source with
`handoff_status=ready_for_field_evidence_rerun_execution_callback_review_handoff`,
matched safe `evidence_ref`, `same_evidence_ref_required=true`,
`same_evidence_ref_status=matched`, `source=software_proof`, `not_proven`, and
all action flags false can map to `accepted` only when the optional result
packet is supported, safe, uses the same evidence ref, and declares
`packet_status=accepted`. Missing result packet or `packet_status=missing` maps
to `missing`. `packet_status=rejected` maps to `rejected`.
`packet_status=blocked` maps to `blocked`. Non-ready handoff sources, missing
or unsupported source input, bad JSON, unsupported packet schema, missing
software-proof/not-proven fields, mismatched evidence refs, weak same-ref
typing, or non-matched same-ref status map to `blocked`. Unsafe copy, raw
paths, credentials, ROS topic text, `/cmd_vel`, serial/UART or WAVE ROVER text,
checksum text, complete/raw artifact text, traceback text, success/control
wording, true field pass wording, Objective 5 external proof claims, PR #5
resolution claims, PR #6 runtime-proof claims, `safe_to_control=true`,
`delivery_success=true`, or `primary_actions_enabled=true` also map to
`blocked`.

This contract is software proof only. `accepted` means accepted for review
intake only; it does not prove a real field rerun, real route/elevator field
pass, real Nav2/fixed-route execution, real field task record, real route
completion signal, real elevator door/floor material, real human assistance,
real dropoff or cancel completion, real delivery result, real phone/browser
...
`pc-tools/evidence/field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision.py`
generates the PC-only review-decision gate after
`field_evidence_rerun_execution_result_acceptance_handoff_intake.py`. It
consumes only a safe handoff-intake artifact/summary, Robot diagnostics safe
alias, or wrapper/nested JSON plus a field owner/support safe review packet. It
does not scan a material directory and does not read real Nav2 runtime, ROS graph
state, serial/UART or WAVE ROVER data, real elevator systems, external cloud
evidence, real phone/browser runtime state, or raw field artifacts.
...
`review_unsafe_rejected`, and `blocked_missing_handoff_intake`. The ready state
requires the previous safe intake
`ready_for_acceptance_handoff_owner_intake_not_proven`,
`source=software_proof`, `not_proven`, safe `evidence_ref`,
`same_evidence_ref_required=true`, a safe owner/support review packet accepting
all required checklist categories, and no unsafe or success/control claim. The
CLI returns zero only for the ready state; missing material, evidence-ref
mismatch, unsafe/rejected content, or missing handoff intake returns nonzero.

The owner/support review checklist is fixed to true task record, true
Nav2/fixed-route runtime log, route completion signal, true elevator door state,
target floor confirmation, human assistance record, dropoff/cancel completion or
delivery result, and true phone/browser evidence. These are review checklist
category names only; this gate does not validate raw task records, runtime logs,
route completion signals, elevator/floor records, human assistance records,
dropoff/cancel completion, delivery results, or true phone/browser runtime.
...
The output always includes `source=software_proof`, `review_decision`,
`allowed_review_decisions`, `decision_reasons`, safe `evidence_ref`,
`source_handoff_intake`, `owner_support_review_packet`, `required_materials`,
`material_status`, `review_checklist`, `owner_handoff`,
`next_required_evidence`, `rerun_commands`, `safe_copy`, `not_proven`,
`safe_to_control=false`, `delivery_success=false`,
`primary_actions_enabled=false`, and
...
Missing handoff intake, missing owner/support review packet, bad JSON,
unsupported schema/boundary, source or review packet not marked as
`source=software_proof` with `not_proven`, source intake not
`ready_for_acceptance_handoff_owner_intake_not_proven`, missing safe
`evidence_ref`, evidence_ref mismatch, weak `same_evidence_ref_required`,
missing checklist review, rejected or unsafe material refs, unsafe copy, raw or
local paths, credentials, ROS topics, `/cmd_vel`, serial/UART/WAVE ROVER details,
raw/complete artifacts, checksums, tracebacks, true phone/browser proof claims,
real route/elevator field pass claims, verified terminal result claims,
Objective 5 external proof claims, HIL claims, PR #5 resolution claims,
success/control claims, `safe_to_control=true`, `delivery_success=true`, or
`primary_actions_enabled=true` all fail closed to one of the allowed review
decision states.

This contract is software proof only. A ready review decision means the
metadata-only handoff intake and owner/support review packet can move to the next
review handoff while remaining fail-closed. It is not real field rerun proof, not
real route/elevator field pass, not real Nav2/fixed-route execution, not true
task record validation, not route completion signal validation, not
dropoff/cancel completion, not verified terminal result, not delivery result,
not delivery success, not HIL, not WAVE ROVER/UART feedback, not true
...
generates the PC-only review-handoff gate after
`field_evidence_rerun_execution_result_acceptance_handoff_intake_review_decision.py`.
It consumes only a safe review-decision artifact/summary, Robot diagnostics safe
alias, or wrapper/nested JSON plus a field owner/support/reviewer safe handoff
packet. It does not scan a material directory and does not read real Nav2
...
`handoff_unsafe_rejected`, and `blocked_missing_review_decision`. The ready
state requires the previous safe review decision
`ready_for_acceptance_handoff_review_handoff_not_proven`,
`source=software_proof`, `not_proven`, safe `evidence_ref`,
`same_evidence_ref_required=true`, a safe owner/support/reviewer handoff packet
accepting all required checklist categories, and no unsafe or success/control
claim. The CLI returns zero only for the ready state; missing material,
evidence-ref mismatch, unsafe/rejected content, or missing review decision
returns nonzero.

The owner/support/reviewer handoff checklist is fixed to true task record, true
Nav2/fixed-route runtime log, route completion signal, true elevator door state,
target floor confirmation, human assistance record, dropoff/cancel completion or
delivery result, and true phone/browser evidence. These are handoff checklist
category names only; this gate does not validate raw task records, runtime logs,
route completion signals, elevator/floor records, human assistance records,
dropoff/cancel completion, delivery results, or true phone/browser runtime.
...
The output always includes `source=software_proof`, `handoff_status`,
`allowed_handoff_states`, `handoff_reasons`, safe `evidence_ref`,
`source_review_decision`, `owner_support_reviewer_handoff_packet`,
`required_materials`, `material_status`, `handoff_checklist`, `owner_handoff`,
`next_required_evidence`, `rerun_commands`, `safe_copy`, `not_proven`,
...
Missing review decision, missing owner/support/reviewer handoff packet, bad JSON,
unsupported schema/boundary, source or handoff packet not marked as
`source=software_proof` with `not_proven`, source review decision not
`ready_for_acceptance_handoff_review_handoff_not_proven`, missing safe
`evidence_ref`, evidence_ref mismatch, weak `same_evidence_ref_required`,
missing checklist handoff, rejected or unsafe material refs, unsafe copy, raw or
local paths, credentials, ROS topics, `/cmd_vel`, serial/UART/WAVE ROVER details,
raw/complete artifacts, checksums, tracebacks, true phone/browser proof claims,
real route/elevator field pass claims, verified terminal result claims,
Objective 5 external proof claims, HIL claims, PR #5 resolution claims,
...
This contract is software proof only. A ready review handoff means the
metadata-only review decision and owner/support/reviewer handoff packet can move
to Product/Robot/mobile read-only consumption while remaining fail-closed. It is
not real field rerun proof, not real route/elevator field pass, not real
Nav2/fixed-route execution, not true task record validation, not route completion
signal validation, not dropoff/cancel completion, not verified terminal result,
not delivery result, not delivery success, not HIL, not WAVE ROVER/UART feedback,
...
`field_evidence_rerun_execution_result_acceptance_handoff_intake_followup_escalation_status.py`.
It consumes only a safe follow-up escalation status artifact/summary, Robot
diagnostics safe alias, or wrapper/nested JSON plus a sanitized owner response
packet. It does not scan a material directory and does not read real Nav2
runtime, ROS graph state, serial/UART or WAVE ROVER data, real elevator systems,
external cloud evidence, real phone/browser runtime state, hardware/HIL
evidence, or raw field artifacts.
...
Allowed `owner_response_status` values are exactly `accepted`, `missing`,
`rejected`, and `blocked`. The accepted state requires the previous follow-up
status to be `pending`, `overdue`, or `escalated`, `source=software_proof`,
`software_proof`, `not_proven`, safe `evidence_ref`,
`same_evidence_ref_required=true`, a sanitized owner response packet accepting
all required owner response checklist categories, and no unsafe or
success/control claim. The CLI returns zero only for `accepted`; `missing`,
`rejected`, and `blocked` return nonzero.

The required owner response checklist is fixed to true task record, true
Nav2/fixed-route runtime log, route completion signal, true elevator door state,
...
`PRRT_kwDOSWB9286CJ3tX` is live resolved by reviewer. These are owner response
category names only; this gate does not validate raw task records, runtime logs,
route completion signals, elevator/floor records, human assistance records,
...
The output always includes `source=software_proof`, `software_proof`,
`owner_response_status`, `allowed_owner_response_statuses`,
`owner_response_reasons`, safe `evidence_ref`, `previous_followup_reference`,
`accepted_materials`, `missing_materials`, `rejected_materials`,
`blocked_materials`, `required_owner_response_materials`,
`material_response_details`, `next_required_evidence`, `rerun_commands`,
`safe_copy`, `not_proven`, `non_access_scope`, `delivery_success=false`,
...
Missing follow-up status, missing owner response packet, bad JSON, unsupported
schema/boundary, source or owner response not marked as `source=software_proof`
with `not_proven`, previous follow-up status not `pending`, `overdue`, or
`escalated`, missing safe `evidence_ref`, evidence_ref mismatch, weak
`same_evidence_ref_required`, missing required materials, rejected owner
response materials, unsafe copy, raw or local paths, credentials, ROS topics,
`/cmd_vel`, serial/UART/WAVE ROVER wording, raw/complete artifacts, checksums,
tracebacks, true phone/browser proof claims, real route/elevator field pass
claims, real Nav2/fixed-route pass claims, verified terminal result claims,
Objective 5 external proof claims, O1 HIL claims, PR #5 resolution claims,
success/control claims, `safe_to_control=true`, `delivery_success=true`, or
`primary_actions_enabled=true` all fail closed to `missing`, `rejected`, or
`blocked` with nonzero CLI exit.

This contract is software proof only. `accepted` means the metadata-only
follow-up escalation status and owner response packet can move to later review
while remaining fail-closed. It is not real field rerun proof, not real
route/elevator field pass, not real Nav2/fixed-route execution, not true task
record validation, not route completion signal validation, not dropoff/cancel
completion, not delivery result, not delivery success, not HIL, not WAVE
ROVER/UART feedback, not true phone/browser proof, not PR #5 resolution, not
...
generates the PC-only owner response review decision gate after
`field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_intake.py`.
It consumes only a safe owner response intake artifact/summary, Robot
diagnostics safe alias, or wrapper/nested JSON plus a sanitized owner response
review packet. It does not scan raw material bodies and does not read real Nav2
runtime, ROS graph state, serial/UART or WAVE ROVER data, real elevator systems,
external cloud evidence, real phone/browser runtime state, hardware/HIL
evidence, or raw field artifacts.
...
Allowed `owner_response_review_decision` values are exactly
`ready_for_owner_response_review_handoff_not_proven`,
`review_needs_owner_rework`, `review_evidence_ref_mismatch`,
`review_unsafe_rejected`, and `blocked_missing_owner_response_intake`. The
ready state requires the previous owner response intake to be `accepted`,
`source=software_proof`, `software_proof`, `not_proven`, safe `evidence_ref`,
`same_evidence_ref_required=true`, a sanitized review packet confirming all
required review material categories, and no unsafe or success/control claim.
The CLI returns zero only for
`ready_for_owner_response_review_handoff_not_proven`; all other states return
nonzero.

The required review checklist is fixed to true task record, true
Nav2/fixed-route runtime log, route completion signal, true elevator door state,
target floor confirmation, human assistance record, dropoff/cancel completion,
delivery result, true route/elevator field pass, true phone/browser evidence,
and PR #5 hardware material remaining pending unless
`PRRT_kwDOSWB9286CJ3tX` is live resolved by reviewer. These are review category
names only; this gate does not validate raw task records, runtime logs, route
completion signals, elevator/floor records, human assistance records,
dropoff/cancel completion, delivery results, field passes, phone/browser
runtime, hardware/HIL state, or GitHub review state.
...
`allowed_owner_response_review_decision_states`, `decision_reasons`, safe
`evidence_ref`, `previous_owner_response_intake_reference`,
`accepted_materials`, `missing_materials`, `rejected_materials`,
`blocked_materials`, `required_review_materials`, `review_material_details`,
`review_confirmed_materials`, `review_needs_rework_materials`,
`review_ref_mismatch_materials`, `review_unsafe_rejected_materials`,
`next_required_evidence`, `rerun_commands`, `safe_copy`, `not_proven`,
`non_access_scope`, `delivery_success=false`, `primary_actions_enabled=false`,
`safe_to_control=false`, `no OKR percentage lift`, and
...
Missing owner response intake, missing review packet, bad JSON, unsupported
schema/boundary, source or review packet not marked as `source=software_proof`
with `not_proven`, previous owner response intake not `accepted`, missing safe
`evidence_ref`, evidence_ref mismatch, weak `same_evidence_ref_required`,
missing required review materials, rejected or unsafe review materials, unsafe
copy, raw or local paths, credentials, ROS topics, `/cmd_vel`,
...
owner response intake and review packet can move to later handoff while
remaining fail-closed. It is not real field rerun proof, not real route/elevator
field pass, not real Nav2/fixed-route execution, not true task record
validation, not route completion signal validation, not dropoff/cancel
completion, not delivery result, not delivery success, not HIL, not WAVE
ROVER/UART feedback, not true phone/browser proof, not PR #5 resolution, not
Objective 5 external cloud/4G/OSS/CDN/DB/queue proof, not an OKR percentage
lift, and not any primary robot action being enabled.
...
generates the PC-only reviewer ACK review-decision gate after
`field_evidence_rerun_execution_result_acceptance_handoff_intake_owner_response_reviewer_ack_intake.py`.
...
claims, true phone/browser proof, PR #5 resolved claims, or raw field artifacts.
...
`previous_intake_reference`, `reviewer_ack_state`, `review_decision`,
`decision_reasons`, `ack_owner`, `acknowledged_at`, `reassignment_target`,
`next_required_evidence`, `owner_action`, `review_handoff_recommendation`,
`safe_copy`, and
...
`rejected_unsafe_reviewer_ack_not_proven`, and
`blocked_missing_reviewer_ack_intake_not_proven`. A sanitized acknowledged ACK
maps only to `accepted_for_reviewer_ack_review_not_proven`; this is later
reviewer ACK review readiness only, not real reviewer resolution, not real
owner acceptance, not real route/elevator field pass, not verified terminal
result, not dropoff/cancel completion, not delivery success, not HIL, not
Objective 5 external cloud/4G/OSS/CDN/DB/queue proof, not true phone/browser
proof, not PR #5 resolution, not an OKR percentage lift, and not any primary
robot action being enabled.
...
completion claims, true phone/browser claims, or PR #5 resolved claims all fail
closed to one of the five allowed states. The summary and Robot alias remain
...
`field_evidence_material_resolution_review_decision.py`.
...
- Allowed source inputs:
  `trashbot.field_evidence_material_resolution_review_decision.v1`,
  `trashbot.field_evidence_material_resolution_review_decision_summary.v1`, the
...
- Handoff status values:
  ...
  `rejected_unsafe_resolution_owner_handoff_not_proven`, and
  `blocked_missing_review_decision_handoff_not_proven`.

The output always includes `handoff_status`, safe `evidence_ref`,
`accepted_material_refs`, `rejected_material_refs`,
`missing_required_materials`, `blocked_categories`, `owner_handoff`,
`next_required_evidence`, `handoff_reasons`, `safe_copy`,
...
Decision mapping is fail closed. A supported safe review decision with
`accepted_for_owner_review_not_proven`, the same safe evidence ref, and
preserved false-state flags maps to
`ready_for_owner_handoff_not_proven`. A source decision of
`needs_more_evidence_not_proven` maps to
`needs_more_evidence_owner_handoff_not_proven` while preserving missing required
materials. Unsafe copy, rejected upstream material, success/control wording,
credentials, raw fields, local paths, ROS/control details, serial/UART or WAVE
ROVER details, truthy `delivery_success`, truthy `primary_actions_enabled`, or
truthy `safe_to_control` maps to
`rejected_unsafe_resolution_owner_handoff_not_proven`. Missing input, bad JSON,
unsupported schema/boundary, weak same-evidence-ref state, or evidence-ref
mismatch maps to `blocked_missing_review_decision_handoff_not_proven`.

The handoff also carries blocked categories for external cloud, verified
terminal result, phone/browser, field route/elevator, hardware/HIL, and PR #5
`PRRT_kwDOSWB9286CJ3tX`. These categories are owner routing hints only. This
contract is software proof only: `ready_for_owner_handoff_not_proven` does not
prove real external cloud, real phone/browser, real route/elevator field pass,
...
generates the PC-only owner-response review-decision gate after
`field_evidence_material_resolution_owner_response_intake.py`.
...
`same_evidence_ref_required=true`, `source=software_proof`, `not_proven`,
`safe_to_control=false`, `delivery_success=false`,
`primary_actions_enabled=false`, `previous_intake_reference`,
`owner_response_material_status`, `review_decision`, `decision_reasons`,
`accepted_materials`, `missing_materials`, `rejected_materials`,
`unsafe_materials`, `next_required_evidence`, `owner_action`,
`ceo_escalation_recommendation`, `review_handoff_recommendation`,
`pr5_thread`, `safe_copy`, and
...
`blocked_missing_owner_response_intake_not_proven`. Fully accepted sanitized
owner response material maps only to
`accepted_for_material_review_not_proven`; this is later material-review
readiness only, not real owner acceptance, not OKR movement, not delivery
success, and not reviewer resolution. Missing owner response material maps to
`needs_more_evidence_not_proven`. Rejected or unsafe material categories map to
`rejected_unsafe_material_response_not_proven`. Missing JSON, bad JSON,
unsupported intake schema, or wrong intake proof boundary maps to
`blocked_missing_owner_response_intake_not_proven`.

Decision mapping is fail closed. Missing safe `evidence_ref`, evidence-ref
mismatch, source not preserving `source=software_proof` / `not_proven` /
false-state flags, unsafe raw artifacts, local paths, credentials,
ROS/control details, reviewer resolution claims, enabled primary actions,
delivery-success claims, or field/cloud/phone/HIL proof claims are rejected
without copying raw material into the artifact. The gate keeps PR #5 thread
`PRRT_kwDOSWB9286CJ3tX` unresolved / `hardware_material_pending` and comment
...
generates the PC-only reviewer ACK review-decision gate after
`field_evidence_material_resolution_reviewer_ack_intake.py`.
...
`safe_to_control=false`, `delivery_success=false`,
`primary_actions_enabled=false`, `previous_intake_reference`,
`reviewer_ack_state`, `review_decision`, `decision_reasons`, `ack_owner`,
`acknowledged_at`, `reassignment_target`, `next_required_evidence`,
`owner_action`, `review_handoff_recommendation`, `safe_copy`, and
...
`blocked_missing_reviewer_ack_intake_not_proven`. A sanitized acknowledged ACK
maps only to `accepted_for_material_review_not_proven`; this is later material
review readiness only, not real reviewer resolution, not real owner acceptance,
not OKR movement, not delivery success, and not PR #5 resolution.
`needs_reassignment` upstream ACK maps to
`needs_reassignment_not_proven`. ACK reasons or next required evidence that ask
for field-owner supplement map to
`needs_field_owner_supplement_not_proven`. Missing JSON, bad JSON, unsupported
intake schema, or wrong intake proof boundary maps to
`blocked_missing_reviewer_ack_intake_not_proven`.

Decision mapping is fail closed. Missing safe `evidence_ref`, evidence-ref
mismatch, source not preserving `source=software_proof` / `not_proven` /
false-state flags, unsafe raw artifacts, local paths, credentials, checksums,
ROS/control details, reviewer resolution claims, enabled primary actions,
delivery-success claims, or field/cloud/phone/HIL proof claims are rejected
without copying raw material into the artifact.
