...
Operator diagnostics may also expose
`field_evidence_rerun_material_dispatch`, the alias
`field_evidence_rerun_material_dispatch_summary`, and the Robot safe alias
`robot_diagnostics_field_evidence_rerun_material_dispatch_summary` from an
explicit `field_evidence_rerun_material_dispatch_ref`,
`TRASHBOT_FIELD_EVIDENCE_RERUN_MATERIAL_DISPATCH`,
`TRASHBOT_FIELD_EVIDENCE_RERUN_MATERIAL_DISPATCH_SUMMARY`, top-level status
fields, an artifact nested summary, status diagnostics summary, or diagnostics
nested summary. The source JSON must use
`schema=trashbot.field_evidence_rerun_material_dispatch.v1` or
`schema=trashbot.field_evidence_rerun_material_dispatch_summary.v1` and
...
evidence rerun material dispatch summary: it may expose only dispatch status,
safe `evidence_ref`, owner work orders, required material groups, rerun
commands, callback packet requirements, `same_evidence_ref_required=true`,
safe copy, boundary flags, `source=software_proof`, `not_proven`,
`safe_to_control=false`, `delivery_success=false`, and
`primary_actions_enabled=false`. Missing sanitized summary, unreadable input,
unsupported schema or boundary, unsafe copy, raw artifact consumption,
control-entrypoint enablement, same `evidence_ref` mismatch, success/control
claims, `safe_to_control=true`, `delivery_success=true`, or
`primary_actions_enabled=true` fail closed as blocked/not_proven. This alias is
not control authorization, collect/dropoff/cancel, ACK, cursor, ROS runtime,
Nav2/fixed-route runtime, serial/UART, WAVE ROVER, HIL, delivery result,
dropoff/cancel completion, real phone/browser proof, Objective 5 external
...
Operator diagnostics may also expose
`field_evidence_rerun_callback_review_decision`, the alias
`field_evidence_rerun_callback_review_decision_summary`, and the Robot safe
alias `robot_diagnostics_field_evidence_rerun_callback_review_decision_summary`
from an explicit `field_evidence_rerun_callback_review_decision_ref`,
`TRASHBOT_FIELD_EVIDENCE_RERUN_CALLBACK_REVIEW_DECISION`,
`TRASHBOT_FIELD_EVIDENCE_RERUN_CALLBACK_REVIEW_DECISION_SUMMARY`, top-level
status fields, an artifact nested summary, status diagnostics summary, or
diagnostics nested summary. The source JSON must use
`schema=trashbot.field_evidence_rerun_callback_review_decision.v1` or
`schema=trashbot.field_evidence_rerun_callback_review_decision_summary.v1` and
`evidence_boundary=software_proof_docker_field_evidence_rerun_callback_review_decision_gate`.
This alias is Robot diagnostics read-only consumption of Autonomy's safe field
evidence rerun callback review decision summary: it may expose only
`review_decision` (`accepted`, `missing`, `rejected`, or `blocked`), safe
`evidence_ref`, `owner_handoff`, `next_required_evidence`, `rerun_guidance`,
`blocker_summary`, same-`evidence_ref` status, safe copy, proof boundary,
`source=software_proof`, `not_proven`, `safe_to_control=false`,
`delivery_success=false`, and `primary_actions_enabled=false`. Missing
sanitized summary, unreadable input, unsupported schema or boundary, unsafe
copy, raw artifact consumption, control-entrypoint enablement, same
...
| `mobile_real_device_field_trial_acceptance_execution_handoff_intake` / `mobile_real_device_field_trial_acceptance_execution_handoff_intake_summary` / `robot_diagnostics_mobile_real_device_field_trial_acceptance_execution_handoff_intake_summary` | Robot diagnostics metadata-only consumer for `trashbot.mobile_real_device_field_trial_acceptance_execution_handoff_intake.v1` / `trashbot.mobile_real_device_field_trial_acceptance_execution_handoff_intake_summary.v1` under `software_proof_docker_mobile_real_device_field_trial_acceptance_execution_handoff_intake_gate`. It may expose only source handoff status, owner ack status, missing evidence, next owner, owner handoff, next required evidence, rerun guidance, blocker summary, the same safe evidence ref, evidence boundary, `source=software_proof`, `software_proof`, `not_proven`, `delivery_success=false`, `primary_actions_enabled=false`, and `safe_to_control=false`; missing source handoff, missing owner ack, missing safe evidence ref, schema or boundary mismatch, unsafe copy, raw artifacts, cursor/ACK payloads, local paths, credentials, complete artifacts, checksums, success/pass/control copy, `delivery_success=true`, `primary_actions_enabled=true`, or `safe_to_control=true` fail closed as blocked/not_proven. It never triggers ACK, cursor updates, Start Delivery, Confirm Dropoff, Cancel, diagnostics fetch, robot command, ROS control, HIL, real phone/browser proof, Objective 5 external proof, Objective 1 HIL/material closure, route/elevator field pass, dropoff/cancel completion, or delivery success. |
| `mobile_real_device_field_trial_acceptance_execution_handoff_review_decision` / `mobile_real_device_field_trial_acceptance_execution_handoff_review_decision_summary` / `robot_diagnostics_mobile_real_device_field_trial_acceptance_execution_handoff_review_decision_summary` | Robot diagnostics metadata-only consumer for `trashbot.mobile_real_device_field_trial_acceptance_execution_handoff_review_decision.v1` / `trashbot.mobile_real_device_field_trial_acceptance_execution_handoff_review_decision_summary.v1` under `software_proof_docker_mobile_real_device_field_trial_acceptance_execution_handoff_review_decision_gate`, derived only from the sanitized `mobile_real_device_field_trial_acceptance_execution_handoff_intake` safe summary. It may expose only source handoff intake status, review decision values `accepted`, `missing`, `rejected`, or `blocked`, accepted material summary, missing material summary, rejected material summary, blocked reason, next owner, rerun guidance, the same safe evidence ref, evidence boundary, `source=software_proof`, `software_proof`, `not_proven`, `delivery_success=false`, `primary_actions_enabled=false`, and `safe_to_control=false`; missing source intake, missing safe evidence ref, schema mismatch, unsafe copy, raw artifact, cursor/ACK payload, local path, credential, complete artifact, checksum, success/pass/control copy, `delivery_success=true`, `primary_actions_enabled=true`, or `safe_to_control=true` fail closed as `blocked` or `rejected`. It never triggers ACK, cursor updates, Start Delivery, Confirm Dropoff, Cancel, diagnostics fetch, robot command, ROS control, HIL, real phone/browser proof, Objective 5 external proof, Objective 1 hardware proof, route/elevator field pass, dropoff/cancel completion, or delivery success. |
| `mobile_real_device_field_trial_acceptance_execution_handoff_review_handoff` / `mobile_real_device_field_trial_acceptance_execution_handoff_review_handoff_summary` / `robot_diagnostics_mobile_real_device_field_trial_acceptance_execution_handoff_review_handoff_summary` | Robot diagnostics metadata-only consumer for `trashbot.mobile_real_device_field_trial_acceptance_execution_handoff_review_handoff.v1` / `trashbot.mobile_real_device_field_trial_acceptance_execution_handoff_review_handoff_summary.v1` under `software_proof_docker_mobile_real_device_field_trial_acceptance_execution_handoff_review_handoff_gate`, derived only from the sanitized `mobile_real_device_field_trial_acceptance_execution_handoff_review_decision` safe summary. It may expose only `current_decision`, `handoff_owner`, `handoff_reason`, accepted/missing/rejected/blocked summaries, `next_required_evidence`, `rerun_guidance`, the same `safe_evidence_ref`, `same_evidence_ref_required=true`, evidence boundary, `source=software_proof`, `software_proof`, `not_proven`, `delivery_success=false`, `primary_actions_enabled=false`, and `safe_to_control=false`; missing source decision, missing safe evidence ref, schema or boundary mismatch, unsafe/raw fields, raw artifact, local path, credential, ACK/cursor payload, complete artifact/checksum, success/pass/HIL/control/delivery-success copy, or truthy control flags fail closed as `blocked`. It never proves a real phone/browser run, Objective 5 external proof, Objective 1 hardware/HIL proof, route/elevator field pass, dropoff/cancel completion, delivery success, or any robot-control permission. |
...
| `field_evidence_rerun_material_dispatch` / `field_evidence_rerun_material_dispatch_summary` / `robot_diagnostics_field_evidence_rerun_material_dispatch_summary` | Robot diagnostics metadata-only consumer for `trashbot.field_evidence_rerun_material_dispatch.v1` / `trashbot.field_evidence_rerun_material_dispatch_summary.v1` under `software_proof_docker_field_evidence_rerun_material_dispatch_gate`. It may summarize dispatch status, safe evidence ref, owner work orders, required material groups, rerun commands, callback packet requirements, `same_evidence_ref_required=true`, boundary flags, `source=software_proof`, `not_proven`, `safe_to_control=false`, `delivery_success=false`, and `primary_actions_enabled=false`, but it must fail closed on missing summary, unsupported schema or boundary, unsafe raw fields or copy, raw artifact consumption, evidence-ref mismatch, enabled actions, success claims, or control claims. It is not a command, ACK, cursor, ROS runtime, ROS control, Nav2/fixed-route proof, serial/UART, WAVE ROVER, HIL pass, Start/Confirm/Cancel enablement source, real phone/browser proof, Objective 5 external proof, delivery result, dropoff/cancel completion, or delivery success. |
| `field_evidence_rerun_callback_review_decision` / `field_evidence_rerun_callback_review_decision_summary` / `robot_diagnostics_field_evidence_rerun_callback_review_decision_summary` | Robot diagnostics metadata-only consumer for `trashbot.field_evidence_rerun_callback_review_decision.v1` / `trashbot.field_evidence_rerun_callback_review_decision_summary.v1` under `software_proof_docker_field_evidence_rerun_callback_review_decision_gate`. It may summarize review decision values `accepted`, `missing`, `rejected`, or `blocked`, safe evidence ref, owner handoff, next required evidence, rerun guidance, blocker summary, same-evidence-ref status, safe copy, boundary flags, `source=software_proof`, `not_proven`, `safe_to_control=false`, `delivery_success=false`, and `primary_actions_enabled=false`, but it must fail closed on missing summary, unsupported schema or boundary, unsafe raw fields or copy, raw artifact consumption, evidence-ref mismatch, enabled actions, success claims, or control claims. It is not a command, collect/dropoff/cancel, ACK, cursor, ROS runtime, ROS control, Nav2/fixed-route proof, serial/UART, WAVE ROVER, HIL pass, Start/Confirm/Cancel enablement source, real phone/browser proof, Objective 5 external proof, field pass, delivery result, dropoff/cancel completion, or delivery success. |
| `field_evidence_material_resolution_intake` / `field_evidence_material_resolution_intake_summary` / `robot_diagnostics_field_evidence_material_resolution_intake_summary` | Robot diagnostics metadata-only consumer for `trashbot.field_evidence_material_resolution_intake.v1` / `trashbot.field_evidence_material_resolution_intake_summary.v1` under `software_proof_docker_field_evidence_material_resolution_intake_gate`. It may expose only decision values `accepted`, `missing`, `rejected`, or `blocked`, safe evidence ref, accepted/missing/rejected/blocked summaries, next required evidence, owner handoff, evidence boundary, `source=software_proof`, `not_proven`, `delivery_success=false`, `primary_actions_enabled=false`, and `safe_to_control=false`, but it must fail closed on missing summary, unsupported schema or boundary, unsafe raw fields or copy, raw artifact bodies, local paths, credentials, ACK/cursor payloads, complete artifacts, checksums, success/pass/control copy, `delivery_success=true`, `primary_actions_enabled=true`, or `safe_to_control=true`. It is not a command, collect/dropoff/cancel, ACK, cursor, ROS runtime, ROS control, Nav2/fixed-route proof, serial/UART, WAVE ROVER, HIL pass, Start/Confirm/Cancel enablement source, real phone/browser proof, Objective 5 external proof, PR #5 reviewer resolution, field pass, delivery result, dropoff/cancel completion, or delivery success. |
| `field_evidence_material_resolution_review_decision` / `field_evidence_material_resolution_review_decision_summary` / `robot_diagnostics_field_evidence_material_resolution_review_decision_summary` | Robot diagnostics metadata-only consumer for `trashbot.field_evidence_material_resolution_review_decision.v1` / `trashbot.field_evidence_material_resolution_review_decision_summary.v1` under `software_proof_docker_field_evidence_material_resolution_review_decision_gate`. It may expose only decision values `accepted_for_owner_review_not_proven`, `needs_more_evidence_not_proven`, `rejected_unsafe_resolution_not_proven`, or `blocked_missing_resolution_intake_not_proven`, safe evidence ref, reason, next required evidence, owner review handoff, evidence boundary, `source=software_proof`, `not_proven`, `delivery_success=false`, `primary_actions_enabled=false`, and `safe_to_control=false`, but it must fail closed on missing summary, unsupported schema or boundary, unsafe copy, raw artifact bodies, local paths, credentials, ACK/cursor payloads, complete artifacts, checksums, success/pass/control copy, `delivery_success=true`, `primary_actions_enabled=true`, or `safe_to_control=true`. It is not a command, collect/dropoff/cancel, ACK, cursor, ROS runtime, ROS control, Nav2/fixed-route proof, serial/UART, WAVE ROVER, HIL pass, Start/Confirm/Cancel enablement source, real phone/browser proof, Objective 5 external proof, PR #5 reviewer resolution, field result, delivery result, dropoff/cancel completion, or delivery success. |
| `field_evidence_material_resolution_review_handoff` / `field_evidence_material_resolution_review_handoff_summary` / `robot_diagnostics_field_evidence_material_resolution_review_handoff_summary` | Robot diagnostics metadata-only consumer for `trashbot.field_evidence_material_resolution_review_handoff.v1` / `trashbot.field_evidence_material_resolution_review_handoff_summary.v1` under `software_proof_docker_field_evidence_material_resolution_review_handoff_gate`. It may expose only handoff statuses `ready_for_owner_handoff_not_proven`, `needs_more_evidence_not_proven`, `blocked_missing_review_decision_not_proven`, or `blocked_unsafe_handoff_not_proven`, safe evidence ref, previous review decision ref, previous review decision, accepted material refs, rejected material refs, missing required materials, owner handoff role, owner next action, next required real evidence, blocked categories `external_cloud`, `terminal_result`, `phone_browser`, `field_route_elevator`, `hardware_hil`, and `pr5`, evidence boundary, `source=software_proof`, `not_proven`, `delivery_success=false`, `primary_actions_enabled=false`, and `safe_to_control=false`, but it must fail closed on missing previous review decision, unsupported schema or boundary, unsafe copy, raw artifact bodies, local paths, credentials, ACK/cursor payloads, complete artifacts, checksums, readiness/success/pass/control copy, `delivery_success=true`, `primary_actions_enabled=true`, or `safe_to_control=true`. Handoff status is not readiness or success. It is not a command, collect/dropoff/cancel, ACK, cursor, ROS runtime, ROS control, Nav2/fixed-route proof, serial/UART, WAVE ROVER, HIL pass, Start/Confirm/Cancel enablement source, real phone/browser proof, Objective 5 external proof, PR #5 reviewer resolution, field result, delivery result, dropoff/cancel completion, or delivery success. |
| `field_evidence_material_resolution_followup_escalation_status` / `field_evidence_material_resolution_followup_escalation_status_summary` / `robot_diagnostics_field_evidence_material_resolution_followup_escalation_status_summary` | Robot diagnostics metadata-only consumer for `trashbot.field_evidence_material_resolution_followup_escalation_status.v1` / `trashbot.field_evidence_material_resolution_followup_escalation_status_summary.v1` under `software_proof_docker_field_evidence_material_resolution_followup_escalation_status_gate`. It may expose only follow-up statuses `pending_owner_response_not_proven`, `overdue_owner_response_not_proven`, or `escalated_for_owner_action_not_proven`, owner response material states `missing`, `pending`, or `escalate`, safe evidence ref, previous handoff ref, previous review decision ref, due status, blocked reason, next required evidence, owner action, CEO escalation recommendation, PR #5 `PRRT_kwDOSWB9286CJ3tX` unresolved state, comment `3269642220` as not reviewer resolution, `hardware_material_pending`, evidence boundary, `source=software_proof`, `not_proven`, `delivery_success=false`, `primary_actions_enabled=false`, and `safe_to_control=false`, but it must fail closed on missing summary, unsupported schema or boundary, unsafe copy, raw artifacts, raw GitHub payloads, local paths, credentials, ACK/cursor payloads, complete artifacts, checksums, ROS topics, `/cmd_vel`, serial/UART details, WAVE ROVER parameters, readiness/success/pass/control copy, reviewer-resolution claims, field/cloud/phone/HIL proof claims, `delivery_success=true`, `primary_actions_enabled=true`, or `safe_to_control=true`. Follow-up escalation status is not readiness, material completion, PR #5 reviewer resolution, field result, delivery result, dropoff/cancel completion, or delivery success. It is not a command, collect/dropoff/cancel, ACK, cursor, ROS runtime, ROS control, Nav2/fixed-route proof, serial/UART, WAVE ROVER, HIL pass, Start/Confirm/Cancel enablement source, real phone/browser proof, or Objective 5 external proof. |
...
| `hardware_sensor_hil_entry_callback_intake` / `hardware_sensor_hil_entry_callback_intake_summary` | Robot diagnostics metadata-only consumer for sanitized Hardware callback-intake summary `trashbot.hardware_sensor_hil_entry_callback_intake_summary.v1` under `software_proof_docker_hardware_sensor_hil_entry_callback_intake_gate`, with source artifact contract `trashbot.hardware_sensor_hil_entry_callback_intake.v1`. It may expose only summary status, safe evidence ref, accepted/missing/rejected material labels, `next_required_evidence`, `owner_handoff`, `rerun_commands`, `source=software_proof`, `hardware_material_status=hardware_material_pending`, `evidence_status=not_proven`, `not_proven`, `delivery_success=false`, and `primary_actions_enabled=false`; missing sanitized summary, unsupported schema or boundary, weak evidence ref, unsafe copy/raw fields, local paths, credentials, serial/UART details, ROS graph/control fields, success/HIL/route/elevator/field-pass claims, `delivery_success=true`, or `primary_actions_enabled=true` must fail closed as `blocked_missing_hardware_sensor_hil_entry_callback_intake`, `blocked_unsupported_hardware_sensor_hil_entry_callback_intake`, or `blocked_unsafe_hardware_sensor_hil_entry_callback_intake_copy`. It is not collect, dropoff, cancel, ACK, cursor, Nav2, HIL pass, real sensor proof, Objective 5 external proof, route/elevator field pass, dropoff/cancel completion, or delivery success. |
...
| `pr5_vendor_source_review_packet` / `pr5_vendor_source_review_packet_summary` / `robot_diagnostics_pr5_vendor_source_review_packet_summary` | Robot diagnostics metadata-only consumer for Hardware gate output `trashbot.pr5_vendor_source_review_packet.v1` / `trashbot.pr5_vendor_source_review_packet_summary.v1` under `software_proof_docker_pr5_vendor_source_review_packet_gate`. It may expose only sanitized `thread_id=PRRT_kwDOSWB9286CJ3tX`, `source=software_proof`, `proof_boundary`, `vendor_source_boundary`, missing materials, `next_required_evidence`, safe copy, `not_proven`, `delivery_success=false`, and `primary_actions_enabled=false`. Missing sanitized summary, unsupported schema or boundary, unsafe copy, raw artifact body/review body, credentials, local paths, serial/UART paths, ROS topics, ACK/cursor/command/control fields, success/HIL/field-pass wording, `delivery_success=true`, or `primary_actions_enabled=true` must fail closed. It is not a command, ACK, cursor, ROS control, `/cmd_vel`, Start Delivery, Confirm Dropoff, Cancel, Nav2, WAVE ROVER, HIL pass, real 2D LiDAR/ToF SKU/vendor/source/receipt/procurement/install/wiring/power/calibration proof, Objective 5 external proof, dropoff/cancel completion, or delivery success. |
| `pr5_vendor_source_review_reply_dispatch` / `pr5_vendor_source_review_reply_dispatch_summary` / `robot_diagnostics_pr5_vendor_source_review_reply_dispatch_summary` | Robot diagnostics metadata-only consumer for Hardware sanitized reply-dispatch output `trashbot.pr5_vendor_source_review_reply_dispatch.v1` / `trashbot.pr5_vendor_source_review_reply_dispatch_summary.v1` under `software_proof_docker_pr5_vendor_source_review_reply_dispatch_gate`. It may expose only sanitized `thread_id=PRRT_kwDOSWB9286CJ3tX`, `source=software_proof`, `proof_boundary`, reply-dispatch status, missing materials, `next_required_evidence`, owner handoff, safe copy, `not_proven`, `hardware_material_pending`, `delivery_success=false`, `primary_actions_enabled=false`, and `safe_to_control=false`. Missing sanitized summary, unsupported schema or boundary, raw body fields, token/credential material, serial/UART details, ROS/control fields, ACK/cursor/command fields, success/HIL/field-pass wording, `delivery_success=true`, `primary_actions_enabled=true`, or `safe_to_control=true` must fail closed. It is not a command, ACK, cursor, ROS control, `/cmd_vel`, Start Delivery, Confirm Dropoff, Cancel, Nav2, WAVE ROVER, HIL pass, real 2D LiDAR/ToF SKU/vendor/source/receipt/procurement/install/wiring/power/calibration proof, Objective 5 external proof, dropoff/cancel completion, route/elevator field pass, PR #5 material closure, or delivery success. |
...
Real-material evidence intake diagnostics may expose
`real_material_evidence_intake`, `real_material_evidence_intake_summary`, and
the Robot safe alias
`robot_diagnostics_real_material_evidence_intake_summary` from an explicit
`real_material_evidence_intake_ref`,
`TRASHBOT_REAL_MATERIAL_EVIDENCE_INTAKE`,
`TRASHBOT_REAL_MATERIAL_EVIDENCE_INTAKE_SUMMARY`, top-level status fields, or
an already sanitized nested diagnostics summary source. The source JSON must
use `schema=trashbot.real_material_evidence_intake_summary.v1`; an artifact
with `schema=trashbot.real_material_evidence_intake.v1` is accepted only when
it contains a sanitized summary wrapper. The evidence boundary must remain
`software_proof_docker_real_material_evidence_intake_gate`. This field is
