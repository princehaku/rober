...
`pr5_mandatory_sensor_material_owner_response_review_handoff` is a PC-only
software-proof gate for unresolved PR #5 review thread `PRRT_kwDOSWB9286CJ3tX`.
It consumes only the sanitized
`pr5_mandatory_sensor_material_owner_response_review_decision` artifact/summary
fields and emits a fail-closed owner/support/reviewer handoff summary for
...
- `evidence_boundary=software_proof_docker_pr5_mandatory_sensor_material_owner_response_review_decision_gate`
...
- sanitized `review_decision` and `material_status`
...
Every status preserves `source=software_proof`, `hardware_material_pending`,
`not_proven`, `delivery_success=false`, `primary_actions_enabled=false`, and
`safe_to_control=false`.

`ready_for_owner_response_review_handoff_not_proven` means only that the
sanitized review-decision metadata can be handed to a human owner/support/
reviewer route. It does not prove real sensor material, does not resolve
`PRRT_kwDOSWB9286CJ3tX`, and does not upgrade comment `3269642220` beyond a
software-proof reply publication.

## Fail-Closed Rules

The gate emits a non-ready state when any of these are true:
...
- Source does not preserve `software_proof`, `hardware_material_pending`,
  ...
  `safe_to_control=false`.
- Source review decision is not accepted, needs-more, rejected/unsafe, or
  blocked evidence-ref mismatch.
...
- `handoff_reasons`
- safe `evidence_ref`
- `source_owner_response_review_decision`
- `material_status`
- `review_handoff`
...
`ready_for_owner_response_review_handoff_not_proven`. It returns non-zero for
`needs_more_material_before_owner_response_review_handoff_not_proven`,
`rejected_unsafe_owner_response_review_handoff_not_proven`,
`blocked_missing_owner_response_review_decision_not_proven`, or
`blocked_evidence_ref_mismatch_not_proven`.
