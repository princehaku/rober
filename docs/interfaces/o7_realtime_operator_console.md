...
- `tasks[]` 可包含 `task_id`、`robot_id`、`route_id`、`status`、`started_at_ms`、`updated_at_ms`、`evidence_ref`、`trajectory_frames[]`、`events[]`、`labels[]`、`asr_events[]`、`tts_drafts[]`、`commands[]`
...
该接口的 `fixture_archive_ready` 只表示本地 archive JSON 被压缩成安全摘要；`route_replay_inspector.status=fixture_inspector_ready` 也只表示 selected task 的本地轨迹 fixture 可被限量查看。它不证明 O6 cloud archive、真实历史任务列表、真实轨迹回放、真实标注 API、真实语音 API、真实 command API、真实 robot ACK、真实机器人运动或真实 delivery success。
