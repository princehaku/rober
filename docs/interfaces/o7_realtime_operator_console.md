...
- PC O7 consumer annotation adapter：`POST /api/o7/consumer-read/tasks/<task_id>/annotations/submit?baseUrl=<local-loopback-url>` 和 `GET /api/o7/consumer-read/tasks/<task_id>/annotations/export?baseUrl=<local-loopback-url>&format=jsonl`
- PC O7 consumer inference adapter：`POST /api/o7/consumer-read/tasks/<task_id>/inference/request?baseUrl=<local-loopback-url>`，固定转发 O6 `POST /api/o6/archive/inference`
- PC O7 consumer delivery result intake adapter：`POST /api/o7/consumer-read/tasks/<task_id>/delivery-result/intake?baseUrl=<local-loopback-url>`，固定转发 O6 `POST /api/o6/archive/field-evidence`
- PC O7 consumer phone/browser proof intake adapter：`POST /api/o7/consumer-read/tasks/<task_id>/phone-browser-proof/intake?baseUrl=<local-loopback-url>`，固定转发 O6 `POST /api/o6/archive/field-evidence` 并回读 `phone_browser_terminal_material`
- PC O7 consumer bounded route gate intake adapter：`POST /api/o7/consumer-read/tasks/<task_id>/bounded-route-gate/intake?baseUrl=<local-loopback-url>`，固定转发 O6 `POST /api/o6/archive/field-evidence` 并回读 `bounded_route_execution_gate_material`
- PC O7 consumer bounded route terminal result intake adapter：`POST /api/o7/consumer-read/tasks/<task_id>/bounded-route-terminal-result/intake?baseUrl=<local-loopback-url>`，固定转发 O6 `POST /api/o6/archive/field-evidence` 并回读 `bounded_route_terminal_result_material`
- PC O7 consumer mission event append adapter：`POST /api/o7/consumer-read/tasks/<task_id>/events/append?baseUrl=<local-loopback-url>`，固定转发 O6 `POST /api/o6/archive/events`
- PC O7 selected-task operator dropoff action capture adapter：`POST /api/o7/consumer-read/tasks/<task_id>/operator/dropoff-acceptance/request?baseUrl=<local-loopback-url>`，固定转发 O6 `POST /api/o6/archive/events` 并写入 `operator.dropoff_acceptance`
- PC O7 consumer voice TTS draft event-write adapter：`POST /api/o7/consumer-read/tasks/<task_id>/voice/tts-draft/request?baseUrl=<local-loopback-url>`，固定转发 O6 `POST /api/o6/archive/events` 并写入 `voice.tts_draft`
- PC O7 consumer voice speaker ACK/failure event-write adapter：`POST /api/o7/consumer-read/tasks/<task_id>/voice/speaker-ack/request?baseUrl=<local-loopback-url>`，固定转发 O6 `POST /api/o6/archive/events`，`ack_status=ack` 写入 `voice.speaker_ack`，`ack_status=failure` 写入 `voice.speaker_failure`
- PC O7 consumer mission evidence bundle export adapter：`GET /api/o7/consumer-read/tasks/<task_id>/mission-evidence/export?baseUrl=<local-loopback-url>&format=json`，固定读取 O6 selected-task detail 并返回 `trashbot.pc_tools_workstation.o7_mission_evidence_bundle_export_result.v1`
...
同一 tab 还包含 `O7 consumer read primary path` 区块，用于把 O6 consumer read detail 作为 O7-KR3 历史回放和 O7-KR4 标注队列检查主路径。该区块只有一个本机回环 relay base URL 输入、`Load consumer task list`、`Load consumer task detail` 和 selected task ID 输入；默认不自动请求。PC 后端 adapter 固定调用 O6 `GET /api/o6/consumer/tasks?view=summary&limit=50` 以及 `GET /api/o6/consumer/tasks/<task_id>?view=default&include=trajectory,events,evidence,field_evidence,labeling,inference,tunnel,artifact_access_probe,offline_artifact_seed_smoke,route_root_seed_gate,route_bag_evidence,route_bag_payload_replay,route_bag_semantic_replay,route_bag_full_semantic_decode_matrix,route_bag_pose_progress_replay,nav2_goal_execution_evidence,delivery_result_evidence,route_execution_result_delivery_readiness,route_delivery_closure_packet,same_task_field_material_packet,same_task_replay_packet_readback,bounded_route_execution_gate_material,bounded_route_terminal_result_material,current_field_evidence_material,pc_live_nav2_execution_material,clean_baseline_nav2_path_material,same_task_route_execution_material_packet,same_task_mission_evidence_gate,field_operator_confirmation_material,phone_browser_terminal_material`，返回 `trashbot.pc_tools_workstation.o7_consumer_task_list.v1` / `trashbot.pc_tools_workstation.o7_consumer_task_detail.v1`。adapter 只允许本机 HTTP 回环 base URL，拒绝非 HTTP、非回环、credentials、query/hash、空 task id、schema mismatch、fetch 失败、非 object 响应和危险 true 字段；`safe_to_control`、`delivery_success`、`primary_actions_enabled`、`connects_cloud_production`、`robot_control_executed`、`submit_enabled`、`submit_available`、`rollback_enabled`、`dataset_export_available`、`playback_available`、`safe_to_play`、`media_access_proven`、`real_cloud_archive_connected`、`real_annotation_api_connected`、`real_command_api_connected`、`real_oss_connected`、`real_cdn_connected`、`allowlist_root_echoed` 或 `offline_artifact_seed_smoke.allowlist_root_echoed` 任何一项为 true 都必须 fail-closed。list 摘要展示 `task_origin`、`field_evidence_source`、`field_evidence_artifact_status`。detail 优先消费 O6 新 `field_evidence` wrapper，wrapper 的 `source_contract` 只能是 `trashbot.field_evidence_manifest.v1` 或 `trashbot.pc_tools_workstation.o7_field_evidence_consumer_ingest.v1`；UI 展示 `source_origin`、`task_origin`、`manifest_run_id`、artifact health 和 present/missing artifacts。2026-07-09 起，consumer detail 还会派生 `artifact_bundle`、`artifact_bundle_consumer_ingest`、`artifact_access_probe`、`offline_artifact_seed_smoke` 和 `artifact_bundle_readiness`；主路径优先从 O6 top-level `artifact_bundle` / `artifact_bundle_consumer_ingest` / `field_evidence.artifact_bundle` / `field_evidence_consumer_ingest.artifact_bundle` 读取同一 task_id 的 route/replay/keyframe/evidence refs、样本计数、blocked reasons 和 next required evidence；`same_task_field_material_packet` 则从 top-level、`field_evidence`、`field_motion_evidence_packet`、`artifact_bundle`、`artifact_bundle_consumer_ingest` 或 `artifact_bundle_readiness` 读取 `trashbot.o6.same_task_field_material_packet.v1` / `trashbot.same_task_field_material_packet.v1`，兼容 top-level `sample_refs` list、`material_summaries` / `material_sample_refs` / `sample_ref_summaries`，以及旧 O6 dict-shaped `sample_refs`，只展示材料布尔位、present/missing material labels、basename sample refs、per-material basename/size/hash/count 摘要、blocked reasons 和 next required evidence；`map_yaml` 缺失时只记录 optional gap，不阻断 route CSV、keyframes、route bag / rosbag、replay JSONL 的消费展示。`current_field_evidence_material` 则从 top-level、`field_evidence`、`field_motion_evidence_packet`、`field_evidence_consumer_ingest`、`artifact_bundle`、`artifact_bundle_consumer_ingest` 或 `artifact_bundle_readiness` 读取 `trashbot.o6.current_field_evidence_material.v1` / `trashbot.current_field_evidence_material.v1`，只展示 current field status、present/missing materials、camera/radar/map/nav2/manual gate booleans、blocked reasons 和 next required evidence；即使该 section ready，也只说明 support-only/current evidence 可读，不表示 route execution success、delivery success 或可控。`pc_live_nav2_execution_material` 则从 top-level、`field_evidence`、`field_motion_evidence_packet`、`field_evidence_consumer_ingest`、`artifact_bundle`、`artifact_bundle_consumer_ingest` 或 `artifact_bundle_readiness` 读取 `trashbot.o6.pc_live_nav2_execution_material.v1` / `trashbot.pc_live_nav2_execution_material.v1`，`goal_accepted` 优先读 canonical `goal_accepted`，fallback `nav2_goal_accepted`；`goal_result_status` 优先读 canonical `goal_result_status`，fallback `result_status`、`nav2_terminal_status`、`terminal_status`。UI 只展示 `source_sprint`、`goal_accepted`、`goal_result_status`、`uses_base_uart`、`base_command_nonzero_observed`、`base_command_nonzero_count`、`base_feedback_sample_count`、`base_feedback_imu_attitude_delta_observed`、`base_feedback_imu_pitch_delta`、`base_feedback_lr_nonzero_proven=false`、blocked reasons 和 next required evidence；即使该 section ready，也只说明 PC 侧 live Nav2 安全摘要已被消费，不表示真实 route execution success、delivery success、wheel L/R 非零或可控。`clean_baseline_nav2_path_material` 则从 top-level、`field_evidence`、`field_motion_evidence_packet`、`field_evidence_consumer_ingest`、`artifact_bundle`、`artifact_bundle_consumer_ingest` 或 `artifact_bundle_readiness` 读取 `trashbot.o6.clean_baseline_nav2_path_material.v1` / `trashbot.clean_baseline_nav2_path_material.v1`，只展示 first failure、retry status、path_point_count、planner/AMCL/initialpose/no-motion cleanup 状态、blocked reasons 和 next required evidence；即使该 section ready，也只说明 clean-baseline no-motion Nav2 path preflight 材料可读，不表示 route execution success、delivery success、safe_to_control=true 或可点击主动作。`same_task_route_execution_material_packet` 从 top-level、`field_evidence`、`field_motion_evidence_packet`、`artifact_bundle`、`artifact_bundle_consumer_ingest` 或 `artifact_bundle_readiness` 消费 `trashbot.o6.same_task_route_execution_material_packet.v1`，UI 必须独立展示 O6 顶层 status、same-task identity、present/missing route execution materials、route execution result summary、pose progress/replay timeline summary、blocked reasons、next required evidence 和固定 false flags；从 `2026.07.10_12-20_o6_o7_route_execution_credit_material` 起，还必须原样展示 `live_or_field_command_evidence_present`、`delivery_or_operator_material_consumed`、`route_execution_credit_candidate`、`credit_support_only_reason`、`credit_required_evidence`。这五个字段任一缺失、形状错误、包含 unsafe text/list，或 packet 自己出现 schema mismatch、task mismatch、proof scope mismatch、dangerous true 时，detail 必须 fail-closed。即使 `route_execution_credit_candidate=true`，O7 也只能把它解释成“same-task route execution credit 材料候选已出现”，不得推出 `delivery_success=true`、`safe_to_control=true` 或启用任何 primary action。checklist 只能引用该 packet，不得替代 packet 自身验收，也不得从 child readiness 推导 `delivery_success=true`、`safe_to_control=true` 或 primary action enabled。`artifact_access_probe` 则从 top-level `artifact_access_probe`、`field_evidence.artifact_access_probe`、`artifact_bundle.artifact_access_probe` 或相关 ingest wrapper 读取 `trashbot.o6.artifact_access_probe.v1`，只展示 basename refs、`detected_type`、`size_bytes`、sha256 prefix、counts、blocked reasons 和 `real_or_offline_artifact_access_probe_for_selected_task`，不得展示 allowlist root 或原始路径。`offline_artifact_seed_smoke` 只展示 route/replay/keyframe/evidence 的 software proof readiness、counts、sample refs、sha256 prefix、blocked reasons 和 `real_or_offline_artifact_seed_smoke_for_selected_task`，不证明真实媒体、真实生产云、真实 annotation API、真实 dataset export 或真实机器人运动。`bounded_route_execution_gate_material` 只展示 selected-task route gate 的 same-task identity、execution plan status、计数、write/readback 状态和固定 false fields，不证明 route execution、delivery success 或 HIL。`bounded_route_terminal_result_material` 只展示 O5 mock terminal result 的 same-task identity、terminal/reconciliation state、write/readback 状态和固定 false fields，不证明 route execution、delivery success 或 HIL。旧 fallback 只做兼容兜底。旧顶层 manifest / ingest 仍兼容，本地 manifest 仅在远端缺 field evidence 时补缺。

2026-07-13 起，`O7 consumer read primary path` 的 list 入口支持只读查询过滤。PC UI / adapter 可在本机回环 `baseUrl` 外附加 `robot_id`、`task_id`、`date=YYYY-MM-DD`、`status=all|completed_mock|failed_mock|in_progress_mock|unknown_not_proven`、`limit` 和可选 `before_started_at_ms`，由 workstation 后端先做白名单校验，再转发给 O6 `GET /api/o6/consumer/tasks?view=summary&limit=<n>`。所有过滤按 AND 语义应用，`limit` 在 O6 consumer list 中保持限量语义，O7 响应回显 `applied_filters`、`filter_semantics=and`、`filtered_result_count`、`o7_consumer_read_query_filters_ready_not_production_proof=true` 和 `o7_consumer_read_query_filters_proof_scope=software_proof_o7_consumer_read_query_filters_only`。unknown key、重复/数组 query、非法日期、非法 status、非法 limit、非法 `before_started_at_ms`、URL/path/credential/raw-like filter 值必须在 O7 adapter 内 fail-closed，错误只暴露 `invalid_o7_consumer_read_query_filter:<field>`，不得把危险原文转发到 O6 或展示到 UI。该能力仍固定只读、本机回环和 fail-closed：不启用控制、播放、提交、导出、production cloud、真实机器人数据、route execution、delivery、HIL 或 safe-to-control。

同日新增 `POST /api/o7/consumer-read/tasks/<task_id>/inference/request?baseUrl=<local-loopback-url>`。UI 必须先选中并加载 consumer detail；按钮只构造 `robot_id`、可选匹配的 `task_id`、`inference_id`、`model_family`、`requested_outputs=elevator_door_state|floor_recognition` 和小型 `inputs[]`，再由 PC Node adapter 固定转发 O6 `POST /api/o6/archive/inference`。adapter 只允许本机 HTTP 回环 base URL，拒绝 path/body task mismatch、unknown fields、数组/标量错配、过大 body、unsafe ref、credential/raw/base64-like 内容、`/cmd_vel`、`/api/base/manual`、serial path、`baudrate`、traceback、token/password/secret/bearer、真实模型/楼层/电梯识别 true claim。O7 只接受 O6 `schema=trashbot.o6.model_inference.v1`、`source=local_mock_inference`、`proof_status=not_proven`、`archive_event_written=true`、`write_status=created|updated` 且固定 false 字段全部为 false 的响应，并返回 `trashbot.pc_tools_workstation.o7_consumer_inference_request_result.v1` receipt。该 receipt 只证明 local/mock request construction、O6 archive write 和安全 readback，不证明真实模型、真实电梯门识别、真实楼层识别、production cloud、route execution、delivery、HIL 或 safe-to-control。

同日新增 `POST /api/o7/consumer-read/tasks/<task_id>/delivery-result/intake?baseUrl=<local-loopback-url>`。UI 必须先选中并加载 consumer detail；按钮只构造 `robot_id`、必须匹配 path 的 `task_id`、`record_status`、`delivery_result_claimed`、basename 级 `evidence_ref`、`dropoff_confirmation_type`、`completed_at_utc`、可选短 `notes` 和小型 primitive `metadata`，再由 PC Node adapter 固定转发 O6 `POST /api/o6/archive/field-evidence`。adapter 会把请求包成最小 `trashbot.field_evidence_manifest.v1` gate + `trashbot.delivery_result_evidence.v1` additive section；只允许本机 HTTP 回环 base URL，拒绝 path/body task mismatch、unknown fields、unsafe ref、URL、路径、credential/raw/base64-like 内容、ROS topic/control 字符串、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART 和任何真实能力 true claim。O7 只接受 O6 `schema=trashbot.o6.field_evidence_archive.v1`、`source=local_mock_field_evidence_archive`、`proof_status=not_proven`、`archive_status=local_mock_field_evidence_ready`、`field_evidence_written=true`、`write_status=created|updated`、task/robot identity 和 delivery result summary 全部匹配且固定 false 字段为 false 的响应，并返回 `trashbot.pc_tools_workstation.o7_consumer_delivery_result_intake_result.v1` receipt。该 receipt 只证明 selected-task local/mock delivery result evidence request 已进入 O6 field-evidence store；proof boundary 固定为 `software_proof_o7_o6_consumer_delivery_result_intake_only`，不证明 production cloud、真实 robot data、route execution、delivery success、operator acceptance、HIL、safe-to-control、`/cmd_vel`、`/api/base/manual`、NavigateToPose 或 WAVE ROVER UART。

同日新增 `POST /api/o7/consumer-read/tasks/<task_id>/phone-browser-proof/intake?baseUrl=<local-loopback-url>`。UI 必须先选中并加载 consumer detail；按钮只构造 `robot_id`、必须匹配 path 的 `task_id`、basename/token 级 `safe_evidence_ref`、`terminal_result_type`、`accepted_materials`、可选 `missing_materials` / `rejected_materials`、`captured_at_utc` 和小型 primitive `metadata`，再由 PC Node adapter 固定转发 O6 `POST /api/o6/archive/field-evidence`。adapter 会把请求包成最小 `trashbot.field_evidence_manifest.v1` gate + `trashbot.o6.phone_browser_terminal_material.v1` additive section，只写 safe terminal material 摘要，不上传截图、DOM、raw body、URL、token、本地路径或完整终端内容。只允许本机 HTTP 回环 base URL，拒绝非回环 URL、path/body task mismatch、unknown fields、unsafe ref、URL、路径、credential/raw/base64-like 内容、ROS topic/control 字符串、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART 和任何真实能力 true claim。O7 只接受 O6 `schema=trashbot.o6.field_evidence_archive.v1`、`source=local_mock_field_evidence_archive`、`proof_status=not_proven`、`archive_status=local_mock_field_evidence_ready`、`field_evidence_written=true`、`write_status=created|updated`、task/robot identity 和 `phone_browser_terminal_material` readback 全部匹配且固定 false 字段为 false 的响应，并返回 `trashbot.pc_tools_workstation.o7_phone_browser_proof_intake_result.v1` receipt。UI/receipt 必须显示 `phone_browser_terminal_material_written`、`phone_browser_terminal_material_readback`、`same_task_id_consumed`、`safe_evidence_ref`、accepted/missing/rejected materials 和固定 false 字段。该 receipt 的 proof boundary 固定为 `software_proof_o6_o7_phone_browser_terminal_material_intake_only`，且固定 `safe_to_control=false`、`delivery_success=false`、`route_execution_success=false`、`hil_pass=false`、`connects_cloud_production=false`、`robot_control_executed=false`；它只证明 selected-task phone/browser terminal material 摘要进入 O6/O7 readback，不证明 production cloud、真实 phone/browser 验收、route execution、delivery success、operator acceptance、HIL、safe-to-control、`/cmd_vel`、`/api/base/manual`、NavigateToPose 或 WAVE ROVER UART。

同日新增 `POST /api/o7/consumer-read/tasks/<task_id>/bounded-route-gate/intake?baseUrl=<local-loopback-url>`。UI 必须先选中并加载 `task_o3_28_pose_fixed_route_consumer_20260713_0402` 的 consumer detail；按钮只构造 `robot_id`、必须匹配 path 的 `task_id`、`packet_id=packet_o3_28_pose_same_task_replay_7d57826142b0c79c`、`route_intent_id=route_intent_20260713_0402_from_20260713_0300_28_pose_structured_path`、`route_csv_row_count=28`、`path_structured_pose_count=28`、`segment_count=27`、`execution_plan_status=blocked_pending_live_safety_gate` 和 basename/token 级 safe refs。PC Node adapter 固定转发 O6 `POST /api/o6/archive/field-evidence`，写入 `trashbot.o6.bounded_route_execution_gate_material.v1` 的 `bounded_route_execution_gate_material`，并只接受 O6 `trashbot.o6.field_evidence_archive.v1` readback 中同 task、同 packet、同 route intent、同计数、同 blocked status 的响应。O7 receipt schema 为 `trashbot.pc_tools_workstation.o7_bounded_route_gate_intake_result.v1`，proof boundary 固定为 `software_proof_o6_o7_bounded_route_gate_material_intake_only`；固定 `safe_to_control=false`、`delivery_success=false`、`route_execution_success=false`、`hil_pass=false`、`robot_control_executed=false`、`connects_cloud_production=false`。adapter 拒绝非回环 URL、task mismatch、raw local path、raw command body、`/cmd_vel`、`/api/base/manual`、NavigateToPose、serial/UART、WAVE ROVER，以及任何 route/delivery/HIL/control true 字段。该 receipt 只证明 selected-task bounded route gate 安全摘要进入本机 O6/O7 readback，不证明 route execution、delivery、HIL、safe-to-control 或 control capability。

同日新增 `POST /api/o7/consumer-read/tasks/<task_id>/bounded-route-terminal-result/intake?baseUrl=<local-loopback-url>`。UI 必须先选中并加载 `task_o3_28_pose_fixed_route_consumer_20260713_0402` 的 consumer detail；按钮只构造 `robot_id`、必须匹配 path 的 `task_id`、`packet_id=packet_o3_28_pose_same_task_replay_7d57826142b0c79c`、`route_intent_id=route_intent_20260713_0402_from_20260713_0300_28_pose_structured_path`、`result_code=mock_route_execution_completed_not_live_delivery`、`terminal_result_state=terminal_result_recorded`、`reconciliation_state=terminal_result_recorded`、`source_schema=trashbot.o5.bounded_route_terminal_result_bridge.v1`、`source_proof_boundary=software_proof_o5_bounded_route_terminal_result_bridge_only`、`route_csv_row_count=28`、`path_structured_pose_count=28`、`segment_count=27` 和 basename 级 `safe_evidence_ref`。PC Node adapter 固定转发 O6 `POST /api/o6/archive/field-evidence`，写入 `trashbot.o6.bounded_route_terminal_result_material.v1` 的 `bounded_route_terminal_result_material`，并只接受 O6 `trashbot.o6.field_evidence_archive.v1` readback 中同 task、同 packet、同 route intent、同 result code、同 terminal/reconciliation state、同计数、同 safe ref 的响应。O7 receipt schema 为 `trashbot.pc_tools_workstation.o7_bounded_route_terminal_result_intake_result.v1`，proof boundary 固定为 `software_proof_o6_o7_bounded_route_terminal_result_intake_only`；固定 `delivery_success=false`、`route_execution_success=false`、`safe_to_control=false`、`hil_pass=false`、`robot_control_executed=false`、`connects_cloud_production=false`。adapter 拒绝非回环 URL、非 selected task、path/body task mismatch、unknown fields、unsafe ref、raw local path、raw command body、`/cmd_vel`、`/api/base/manual`、NavigateToPose、serial/UART、WAVE ROVER，以及任何 route/delivery/HIL/control true 字段。该 receipt 只证明 O5 mock terminal-result 安全摘要进入本机 O6/O7 readback，不证明 route execution、delivery、HIL、safe-to-control、production cloud 或 control capability。

同日新增 `POST /api/o7/consumer-read/tasks/<task_id>/events/append?baseUrl=<local-loopback-url>`。UI 必须先选中并加载 consumer detail；按钮只构造 `robot_id`、必须匹配 path 的 `task_id`、`event_id`、O6 白名单 `event_type`、`occurred_at_ms`、至少一个安全 `evidence_ref`/`evidence_refs`，以及可选 `summary`、`severity` 和小型 primitive `metadata`，再由 PC Node adapter 固定转发 O6 `POST /api/o6/archive/events`。adapter 只允许本机 HTTP 回环 base URL，拒绝 path/body task mismatch、unknown fields、缺少身份字段、unsupported event type、无效时间戳、过大 metadata、unsafe ref、credential/raw/base64-like 内容、完整 URL、绝对路径、ROS topic/control 字符串、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART 和任何真实能力 true claim。O7 只接受 O6 `schema=trashbot.o6.archive_events.v1`、`source=local_mock_event_archive`、`proof_status=not_proven`、`archive_event_written=true`、`write_status=created|updated`、task/robot/event identity 和 `event_summary` 全部匹配且固定 false 字段为 false 的响应，并返回 `trashbot.pc_tools_workstation.o7_consumer_mission_event_append_result.v1` receipt。该 receipt 只证明 selected-task local/mock mission event append software proof，不证明 production cloud、真实 robot data、route execution、delivery/operator acceptance、HIL、safe-to-control、`/cmd_vel`、`/api/base/manual`、NavigateToPose 或 WAVE ROVER UART。

同日新增 `POST /api/o7/consumer-read/tasks/<task_id>/operator/dropoff-acceptance/request?baseUrl=<local-loopback-url>`。UI 必须先选中并加载 consumer detail；按钮只构造 `robot_id`、必须匹配 path 的 `task_id`、`event_id`、`occurred_at_ms`、可选 `operator_action_id` / `operator_display_name`、至少一个安全 `evidence_ref`/`evidence_refs`、可选 `summary` 和小型 primitive `metadata`，再由 PC Node adapter 固定转发 O6 `POST /api/o6/archive/events`，写入固定 `event_type=operator.dropoff_acceptance`。adapter 只允许本机 HTTP 回环 base URL，拒绝 path/body task mismatch、unknown fields、caller 覆盖 event_type、缺少身份字段、无效时间戳、unsafe ref、credential/raw/base64-like 内容、完整 URL、绝对路径、ROS topic/control 字符串、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART 和任何真实能力 true claim。O7 只接受 O6 `schema=trashbot.o6.archive_events.v1`、`source=local_mock_event_archive`、`proof_status=not_proven`、`archive_event_written=true`、`write_status=created|updated`、task/robot/event identity、`event_type=operator.dropoff_acceptance` 和固定 false 字段全部匹配的响应，并返回 `trashbot.pc_tools_workstation.o7_operator_dropoff_action_capture_result.v1` receipt。该 receipt 的 proof boundary 固定为 `software_proof_o6_o7_operator_dropoff_action_capture_only`，并固定 `real_operator_action_proven=false`、`delivery_success=false`、`route_execution_success=false`、`safe_to_control=false`、`hil_pass=false`、`robot_control_executed=false`、`connects_cloud_production=false`；它只证明 selected-task operator action request construction、O6 local archive event write/readback receipt 和 O7 UI receipt 展示，不证明 real operator action、delivery success、route execution、HIL、safe-to-control、production cloud、`/cmd_vel`、`/api/base/manual`、NavigateToPose 或 WAVE ROVER UART。

2026-07-14 09:33 CST 起，本地 workstation UI test 会把同一 `记录 operator dropoff capture` DOM 流程落盘为 `sprints/2026.07.14_09-33_o7_operator_dropoff_browser_artifact/artifacts/o7_operator_dropoff_browser_artifact.json`，schema 为 `trashbot.pc_tools_workstation.o7_operator_dropoff_browser_artifact.v1`。该 artifact 只记录 selected `task_id`、按钮文案、O7 endpoint path、receipt schema/status、`event_type=operator.dropoff_acceptance`、固定 `real_operator_action_proven=false`、`delivery_success=false`、`route_execution_success=false`、`safe_to_control=false`、`hil_pass=false`、`robot_control_executed=false`、`connects_cloud_production=false`、`not_proven` 和 proof boundary `software_proof_o7_operator_dropoff_browser_artifact_only`。它是 local browser/DOM software proof：不包含截图、raw DOM、credential、生产 URL 或真实云 payload，也不证明真实手机、production cloud/DB/queue/OSS/CDN/4G/SIM、real operator action、delivery success、route execution、HIL、safe-to-control、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART 或机器人运动。

同日新增 `POST /api/o7/consumer-read/tasks/<task_id>/voice/tts-draft/request?baseUrl=<local-loopback-url>`。UI 必须先选中并加载 consumer detail；按钮只构造 `robot_id`、必须匹配 path 的 `task_id`、`event_id`、basename 级 `evidence_ref`、`occurred_at_ms`、短 `draft_text`、可选 `voice_profile`、`locale` 和 primitive `metadata`，再由 PC Node adapter 固定转发 O6 `POST /api/o6/archive/events`，写入 O6 白名单 `event_type=voice.tts_draft`。adapter 只允许本机 HTTP 回环 base URL，拒绝 path/body task mismatch、unknown fields、缺少身份字段、空/过长 draft、unsupported locale/profile、unsafe evidence ref、credential/raw/base64-like 内容、完整 URL、绝对路径、ROS topic/control 字符串、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART、音频 payload、真实 voice API 文案和任何真实能力 true claim。O7 只接受 O6 `schema=trashbot.o6.archive_events.v1`、`source=local_mock_event_archive`、`proof_status=not_proven`、`archive_event_written=true`、`write_status=created|updated`、task/robot/event identity、`event_type=voice.tts_draft` 和固定 false 字段全部匹配的响应，并返回 `trashbot.pc_tools_workstation.o7_voice_tts_draft_request_result.v1` receipt。该 receipt 的 proof boundary 固定为 `software_proof_o6_o7_voice_tts_draft_event_write_only`，并固定 `tts_send_enabled=false`、`speaker_dispatch_enabled=false`、`real_voice_api_connected=false`、`real_asr_tts_runtime_connected=false`、`safe_to_control=false`、`delivery_success=false`、`robot_control_executed=false`、`connects_cloud_production=false`；它只证明 selected-task voice/TTS 草稿事件被安全写入本机 O6/O7 readback，不证明真实语音 API、真实 TTS 发送/播放、喇叭调度、production cloud、delivery、HIL 或 safe-to-control。

同日新增 `POST /api/o7/consumer-read/tasks/<task_id>/voice/speaker-ack/request?baseUrl=<local-loopback-url>`。UI 必须先选中并加载 consumer detail；按钮只构造 `robot_id`、必须匹配 path 的 `task_id`、`event_id`、`ack_status=ack|failure`、basename 级 `evidence_ref`、`occurred_at_ms`、可选 `failure_reason_code`、`summary` 和 primitive `metadata`，再由 PC Node adapter 固定转发 O6 `POST /api/o6/archive/events`。adapter 只允许本机 HTTP 回环 base URL，拒绝 path/body task mismatch、unknown fields、caller 覆盖 `event_type`、缺少身份字段、unsupported `ack_status`、无效时间戳、unsafe evidence ref、credential/raw/base64-like 内容、完整 URL、绝对路径、ROS topic/control 字符串、`/cmd_vel`、`/api/base/manual`、NavigateToPose、WAVE ROVER UART、音频 payload、真实 speaker/voice API 文案和任何真实能力 true claim。O7 将 `ack_status=ack` 映射为 `event_type=voice.speaker_ack`，将 `ack_status=failure` 映射为 `event_type=voice.speaker_failure`，并只接受 O6 `schema=trashbot.o6.archive_events.v1`、`source=local_mock_event_archive`、`proof_status=not_proven`、`archive_event_written=true`、`write_status=created|updated`、task/robot/event identity 和固定 false 字段全部匹配的响应。receipt schema 固定为 `trashbot.pc_tools_workstation.o7_voice_speaker_ack_event_result.v1`，proof boundary 固定为 `software_proof_o6_o7_voice_speaker_ack_event_write_only`，并固定 `speaker_dispatch_enabled=false`、`real_speaker_ack_proven=false`、`tts_send_enabled=false`、`real_voice_api_connected=false`、`real_asr_tts_runtime_connected=false`、`safe_to_control=false`、`delivery_success=false`、`robot_control_executed=false`、`connects_cloud_production=false`；它只证明 selected-task speaker ACK/failure event 被安全写入本机 O6/O7 readback，不证明真实音频播放、真实 speaker ACK、真实 voice API、真实 ASR/TTS runtime、production cloud、route execution、delivery、HIL 或 safe-to-control。

同日新增 `GET /api/o7/consumer-read/tasks/<task_id>/mission-evidence/export?baseUrl=<local-loopback-url>&format=json`。UI 必须先选中并加载 consumer detail；没有 selected task、detail 未加载、detail fail-closed 或 task mismatch 时按钮保持禁用，不能伪造导出成功。PC Node adapter 只允许本机 HTTP 回环 base URL，拒绝非回环 URL、credentials/query/hash、非 `format=json`、空 task id、O6 schema mismatch、O6 task mismatch、危险 true 字段、unsafe ref/path/URL/raw/base64/control 内容，并固定读取 O6 `GET /api/o6/consumer/tasks/<task_id>?view=default&include=...`。成功时返回 `trashbot.pc_tools_workstation.o7_mission_evidence_bundle_export_result.v1`，`export_status=local_mock_mission_evidence_bundle_ready`，`proof_scope=software_proof_o7_o6_mission_evidence_bundle_export_only`，只聚合 mission evidence bundle receipt 摘要：mission events、field evidence、same-task replay packet/readiness、delivery result/readiness、route/closure/material sections（包含 `bounded_route_execution_gate_material` 和 `bounded_route_terminal_result_material`，并计入 `counts.material_section_count`）的 schema/status/proof scope、数量、basename sample refs、blocked reasons 和 not_proven。该 o7_mission_evidence_bundle_export 只是不归档的 local/mock receipt，不回显 raw artifact body、真实路径、token、完整 URL 或真实 dataset；固定 `safe_to_control=false`、`delivery_success=false`、`route_execution_success=false`、`hil_pass=false`、`connects_cloud_production=false`、`robot_control_executed=false`、`real_cloud_db_connected=false`、`real_oss_connected=false`，不证明 production cloud、route execution、delivery success、HIL、safe-to-control、real dataset export 或 O5 external evidence。

`trashbot.pc_tools_workstation.o7_consumer_task_detail.v1` 现在额外派生三个同 task 的消费面摘要和两个 MVP section：

- `artifact_bundle`，schema 为 `trashbot.o6.artifact_bundle.v1`。它优先从 O6 主路径上的 bundle alias / wrapper 读取，同一 `task_id` 的 route/replay/keyframe/evidence refs、request summary 和固定 false fields 都只作为只读摘要。任何危险 true 字段、unsafe ref、schema mismatch 或空 refs 都必须 fail closed。
- `artifact_bundle_consumer_ingest`，schema 为 `trashbot.o6.artifact_bundle_consumer_ingest.v1`。它只补充 bundle ingest 的来源、run id、manifest gate 和 field evidence summary，不扩展真实写入能力边界。
- `artifact_bundle_readiness`，schema 为 `trashbot.pc_tools_workstation.o7_consumer_artifact_bundle_readiness.v1`。它围绕同一 `task_id` 汇总 route/replay/keyframe/evidence/review item/sample/ref 计数、`artifact_access_probe` counts、`offline_artifact_seed_smoke` counts、样本 refs、blocked reasons 和 next required evidence；route replay / labeling 的阻塞原因和样本媒体 refs 优先来自 bundle / preflight，旧 fallback 只做兼容兜底。
- `same_task_mission_evidence_gate`，schema 为 `trashbot.o6.same_task_mission_evidence_gate.v1` 或源 `trashbot.same_task_mission_evidence_gate.v1`。它只展示同一 `task_id` 下 O5 terminal/cloud result 与 route execution materials 是否配对、terminal source/schema/ref、linked flags、blocked reasons 和 next required evidence；`same_task_mission_gate_ready_not_success_proof` 仍固定 `delivery_success=false`，不表示真实送达成功、真实 production cloud 或真实 route execution。
- `field_operator_confirmation_material`，schema 为 `trashbot.pc_tools_workstation.o7_field_operator_confirmation_material.v1`。它默认随 O7 consumer detail include 读取，并从 O6 top-level、`field_evidence`、`field_motion_evidence_packet`、`artifact_bundle`、`artifact_bundle_consumer_ingest` 或 `artifact_bundle_readiness` 中归一 operator report、operator confirmation、安全预检布尔位、same-task 消费、linked route/delivery material 和 next required evidence。status 只允许 `field_operator_confirmation_material_ready_not_delivery_proof` 或 fail-closed 派生状态，proof scope 固定 `software_proof_field_operator_confirmation_material_only`；缺字段、task mismatch、dangerous true、unsafe text/list 或 proof scope mismatch 必须 fail-closed。该 section 永远固定 `delivery_success=false`、`safe_to_control=false`、`primary_actions_enabled=false`、`robot_control_executed=false`、`route_execution_success=false`、`hil_pass=false`、`connects_cloud_production=false`。
- `same_task_field_material_packet`，schema 为 `trashbot.o6.same_task_field_material_packet.v1` 或源 `trashbot.same_task_field_material_packet.v1`。它只展示同一 `task_id` 下 `map_yaml`、`route_csv`、keyframes、route bag / rosbag、replay JSONL 等准现场材料是否已被消费，固定输出 `same_task_id_consumed`、`live_or_field_material_consumed`、present/missing materials、top-level `sample_refs` list，以及 per-material `basename` / `size_bytes` / `sha256_prefix` / `sample_refs` / `count` / `present` 摘要；若上游仍输出旧 dict-shaped `sample_refs`，O7 会按 legacy per-material summary 兼容消费。`map_yaml` 缺失只记 optional gap，不阻断 route/keyframe/route bag/replay 的材料消费展示。`ready_not_delivery_proof` 只表示材料已进入 O6/O7 主路径，不表示真实 route execution、真实 delivery success 或 production cloud。
- 从 `2026.07.10_08-14_same_task_mission_artifact_credit_gate` 起，O7 consumer detail 还必须把 O6 credit fields 原样消费到 same-task gate 摘要：`same_task_id_consumed`、`mission_artifact_delta`、`live_or_field_command_executed`、`support_only_reason`、`okr_credit_allowed`。其中 `okr_credit_allowed=false` 时，UI 必须明确保持 support-only/blocked 语义，不能把 `same_task_mission_gate_ready_not_success_proof` 或 checklist `materials_ready_not_success_proof` 解释成 O5/O6/O7 主进度已可计分。
- `same_task_mission_material_checklist`，schema 为 `trashbot.pc_tools_workstation.o7_same_task_mission_material_checklist.v1`。它由 O7 workstation 从 O6 consumer detail 主路径里的 `same_task_mission_evidence_gate` 派生，面向 operator 展示下一步要补齐的 mission material，而不是重复原始 JSON。顶层固定 `delivery_success=false`、`safe_to_control=false`、`primary_actions_enabled=false`、`robot_control_executed=false`，且 `connects_cloud_production=false`。`status`/`overall_status` 只允许 `fail_closed`、`blocked_not_proven`、`materials_ready_not_success_proof`；其中 `materials_ready_not_success_proof` 仅表示同 task 软件材料可读，不证明真实送达成功。
- checklist 顶层也会复用 `okr_credit_allowed`、`support_only_reason`、`same_task_id_consumed`、`live_or_field_command_executed` 四个 gate 字段，方便 operator 在不回看原始 gate JSON 的情况下判断当前任务是 mission-credit ready 还是 support-only 守护。
- `artifact_access_probe`，schema 为 `trashbot.o6.artifact_access_probe.v1` 或 `not_loaded`。它只消费 O6 local/mock 小文件读取摘要：`counts.requested/readable/blocked/missing`、最多少量 sample probes、basename `ref`、`detected_type`、`size_bytes`、sha256 prefix、blocked reasons 和 next required evidence。schema mismatch、危险 true 字段、unsafe ref 或缺失 probe 都必须 fail closed；缺失 probe 会让 `artifact_bundle_readiness.status=derived_blocked_not_proven` 并合并 `artifact_access_probe_missing` / `real_or_offline_artifact_access_probe_for_selected_task`。
- `offline_artifact_seed_smoke`，schema 为 `trashbot.o6.offline_artifact_seed_smoke.v1` 或 `not_loaded`。它只消费 O6 离线 seed smoke software proof：`counts.route/replay/keyframe/evidence/sample/readable/blocked/missing`、最多少量 sample refs、sha256 prefix、blocked reasons 和 next required evidence。schema mismatch、危险 true 字段、allowlist root 回显、unsafe ref 或缺失 section 都必须 fail closed；缺失 section 会让 `artifact_bundle_readiness.status=derived_blocked_not_proven` 并合并 `offline_artifact_seed_smoke_missing` / `real_or_offline_artifact_seed_smoke_for_selected_task`。

- `route_replay_mvp`，schema 为 `trashbot.pc_tools_workstation.o7_consumer_route_replay_mvp.v1`。它只从 O6 detail 的 `trajectory`、`events`、`evidence` 和 `field_evidence` 白名单字段生成 `trajectory.frame_count`、`trajectory.current_frame`、最多 5 条 `sample_frames`、`events_timeline.sample`、`evidence_refs.sample_refs`、`evidence_refs.keyframe_refs` 和 `cursor_contract`。`cursor_contract.supported_actions` 仅表示浏览器本地支持 `previous_frame` / `next_frame` / `reset_cursor` / `toggle_playing`；`playback_available=false`、`safe_to_play=false`、`sends_to_robot=false`、`safe_to_control=false`、`delivery_success=false`、`primary_actions_enabled=false`、`robot_control_executed=false` 固定不变。
- `labeling_mvp`，schema 为 `trashbot.pc_tools_workstation.o7_consumer_labeling_mvp.v1`。它只从 O6 detail 的 `labeling` 白名单字段生成 `review_items.current_item`、最多 5 条 review item、`media_ref`、`evidence_ref`、`current_labels.sample`、`label_schema`、`allowed_label_types`、`draft_labels.sample` 和 `submit_receipt`。`submit_receipt.status=submit_blocked_fail_closed`、`receipt_id=not_created`、`submit_enabled=false`、`rollback_enabled=false`、`dataset_export_available=false`、`real_annotation_api_connected=false`、`cloud_write_executed=false` 固定不变。

`same_task_mission_material_checklist.items[]` 每项至少包含 `id`、`label`、`material_status`、`source_summary`、`blocked_reasons`、`next_required_evidence`、`owner_hint`。当前固定覆盖 `same_task_identity`、`terminal_cloud_result`、`route_execution_material`、`same_task_field_material_packet`、`delivery_record`、`operator_confirmation`、`route_pose_progress`、`production_cloud_readback`、`safety_invariants` 九项。O6 gate 缺失、gate schema mismatch、`task_id` mismatch、危险 true 字段、unsafe ref/path/token/base64、`blocked_reasons` 或 `next_required_evidence` 类型不安全时必须 fail-closed；UI 只展示 operator 材料清单，不连接真实公网云、不读硬件、不发送控制命令、不启用 submit/TTS/nav/control。该 checklist 不证明 production cloud、live route execution、真实 delivery record、真实 operator confirmation 或 delivery success。

同一主路径新增两个 PC-only local/mock action contract：

- `trashbot.pc_tools_workstation.o7_annotation_submit_result.v1` 来自 PC 后端 `POST /api/o7/consumer-read/tasks/<task_id>/annotations/submit?baseUrl=<local-loopback-url>`。PC adapter 只把安全 label 摘要转给 O6 `POST /api/o6/archive/labels`，并要求 O6 返回 `trashbot.o6.archive_labeling.v1`、HTTP 2xx、`write_status=created|updated` 和所有危险字段 false。
- `trashbot.pc_tools_workstation.o7_annotation_dataset_export_result.v1` 来自 PC 后端 `GET /api/o7/consumer-read/tasks/<task_id>/annotations/export?baseUrl=<local-loopback-url>&format=jsonl`。PC adapter 只支持 `format=jsonl`，调用 O6 `GET /api/o6/archive/labels/<task_id>/export?format=jsonl`，并要求 O6 返回 `trashbot.o6.annotation_dataset_export.v1`、`export_status=local_mock_export_ready` 和所有危险字段 false。
- 两个 adapter 都拒绝非 HTTP、非回环、credentials、query/hash、空 task id、schema mismatch、fetch 失败、非 object 响应、未知 submit body 字段、token/password/secret/credential/bearer 字样以及 O6 response 中的危险 true 字段。
- 两个 PC result 都必须固定 `proof_status=not_proven`、`submit_enabled=false`、`rollback_enabled=false`、`dataset_export_available=false`、`real_annotation_api_connected=false`、`real_dataset_export_connected=false`、`cloud_write_executed=false`、`connects_cloud_production=false`、`safe_to_control=false`、`primary_actions_enabled=false`、`delivery_success=false`、`robot_control_executed=false`。

UI 的 `Consumer-detail route replay player` 优先消费 `route_replay_mvp`，Play/Pause、Previous/Next、Reset、range cursor 都只改浏览器内存，不调用 API、不写后端、不发送机器人命令。UI 的 `Consumer-detail labeling queue primary path` 优先消费 `labeling_mvp`，展示当前 review item、media/evidence ref、current/draft labels、label schema、allowed types、fail-closed submit receipt，以及 PC adapter 返回的 local/mock submit receipt / local/mock jsonl export result；真实 `submit_enabled`、真实 `dataset_export_available` 和 `rollback_enabled` 都固定 false。缺 detail、unknown task、task id mismatch、trajectory/events/evidence/keyframe refs 缺失、labeling/review item/schema/allowed types/draft labels 缺失、blocked/not_proven/error/cancel 状态或 adapter fail-closed 时必须显示 blocker 并禁用 local/mock submit/export 操作。该主路径不证明真实云归档、真实地图叠加、真实关键帧媒体可访问、真实 annotation API、真实标注提交、真实训练集导出、真实机器人运动、真实控制、生产云连接或 O7 完成。
