...
install()
{
	if [ -f "./$scriptname" ]; then
		if [ "${osver[0]}" = 'Ubuntu' ] ; then
			i="$(dpkg -s iw | grep installed)"
			if [ $? -eq 1 ] ;then
				apt update
				apt -y install iw
				i="$(dpkg -s iw | grep installed)"
				if [ $? -eq 1 ] ;then
					...
					echo "Internet access is required for installation"
					return
				fi	
			fi
...
setupssid()
{
	...
	do
		IFS=$'\n:$\t' localwifi=($((iw dev wlan0 scan ap-force | grep -E "SSID:") 2>&1)) >/dev/null 2>&1
