...
PHONE_ACK_LOOKUP_PENDING_STATUS_GUARD_BOUNDARY = (
    "software_proof_docker_cloud_ack_lookup_pending_status_guard"
)
PHONE_ACK_LOOKUP_PENDING_STATUS_CAPABILITY = "cloud_ack_lookup_pending_status_guard"
PHONE_ACK_LOOKUP_PENDING_ACK_SEMANTICS = "ack_lookup_pending_not_delivery_success"
PHONE_ACK_LOOKUP_PENDING_SAFE_PHONE_COPY = (
...
PHONE_ACK_ACCEPTED_RESULT_PENDING_CAPABILITY = "cloud_ack_accepted_result_pending_guard"
PHONE_ACK_ACCEPTED_RESULT_PENDING_DEGRADATION_STATE = "ack_accepted_result_pending"
...
PHONE_READINESS_NEXT_ACTION_COPY = {
    ...
    "continue_polling_or_contact_support": "继续等待小车处理该命令；如果持续没有 ACK，请联系支持。",
...
def _remote_readiness_for_ack_lookup_pending(*, queue_persisted=False, auth_state="required"):
    # 缺失 ACK 是读取侧 pending，不代表失败、取消或送达；手机只拿 canonical 降级态继续轮询。
    return {
        "capability": PHONE_ACK_LOOKUP_PENDING_STATUS_CAPABILITY,
        "remote_ready": False,
        ...
        "ack_semantics": PHONE_ACK_LOOKUP_PENDING_ACK_SEMANTICS,
        "safe_to_control": False,
        "delivery_success": False,
        "primary_actions_enabled": False,
        "proof_boundary": PHONE_ACK_LOOKUP_PENDING_STATUS_GUARD_BOUNDARY,
    }


def _remote_readiness_for_ack_accepted_result_pending(
    *,
    command_id="",
    queue_persisted=False,
    auth_state="required",
):
    # ACK accepted/processing 只证明控制面已受理；没有终态结果时必须阻断主操作。
    return {
...
def _ack_is_accepted_processing_without_terminal_result(ack, operator_status=None):
    # remote ACK 里的 acked/accepted/processing 只代表 bridge/API 已受理；缺终态证据时走 O5 pending guard。
...
class MockCloudStore:
    ...
    def _readiness_locked(self, robot_id, robot):
        ...
        elif media_state or status_degradation == "media_degraded":
        ...
        elif status_degradation == "ack_lookup_pending":
            retry_hint = "continue_polling_or_contact_support"
            degradation_state = "ack_lookup_pending"
        elif cancel_pending:
        ...
        if degradation_state == "media_degraded":
            media_state = media_state if media_state in {"oss_write_failed", "cdn_unavailable"} else "oss_write_failed"
            readiness["media_state"] = media_state
            readiness["remote_ready"] = False
            readiness["delivery_success"] = False
            readiness["primary_actions_enabled"] = False
            readiness["ack_semantics"] = {
                "cdn_unavailable": "media_not_fetchable_not_delivery_success",
                "oss_write_failed": "media_not_persisted_not_delivery_success",
            }[media_state]
            ...
            readiness["safe_phone_copy"] = {
                "cdn_unavailable": "CDN 暂不可达，媒体不可拉取；这不是送达成功。",
                "oss_write_failed": "OSS 写入失败，媒体未持久化；这不是送达成功。",
            }[media_state]
            readiness["proof_boundary"] = str(
                status.get("proof_boundary")
                or ack_operator_status.get("proof_boundary")
        ...
        if degradation_state == "ack_lookup_pending":
            # ACK 查询缺失只说明机器人尚未处理该命令；不能推进 cursor 或解释成任何交付结果。
            readiness.update(
                _remote_readiness_for_ack_lookup_pending(
                    queue_persisted=bool(self.state_path),
        ...
        if degradation_state == PHONE_ACK_ACCEPTED_RESULT_PENDING_DEGRADATION_STATE:
            readiness.update(
                _remote_readiness_for_ack_accepted_result_pending(
                    command_id=last_command_ack,
                    queue_persisted=bool(self.state_path),
                    auth_state=readiness.get("auth_state", "required"),
                )
    ...
    def get_ack(self, robot_id, command_id):
        with self._lock:
            ...
            readiness = self._readiness_locked(str(robot_id or "").strip(), robot)
        if not ack:
            # 404 仍保留 ack_not_found，但同时给手机 canonical readiness，避免 UI 从错误文案里猜状态。
            payload = remote_error("ack_not_found", "ack not found for command")
            payload["remote_readiness"] = _remote_readiness_for_ack_lookup_pending(
                queue_persisted=bool(self.state_path),
                auth_state="required" if self.auth_required else "mock_not_required",
            )
            return 404, payload
        return 200, {"ok": True, "ack": dict(ack), "remote_readiness": readiness}
...
def build_phone_readiness(
    ...
    *,
    ...
    elif remote_state == "ack_lookup_pending":
        primary_state = "waiting_for_command_ack"
        next_action = "continue_polling_or_contact_support"
        can_continue = False
        support_level = "remote_ack_lookup_pending"
    elif remote_state == PHONE_ACK_ACCEPTED_RESULT_PENDING_DEGRADATION_STATE:
        primary_state = PHONE_ACK_ACCEPTED_RESULT_PENDING_DEGRADATION_STATE
    ...
    if remote_state == "media_degraded":
    ...
    if remote_state == "ack_lookup_pending":
        # ACK lookup pending 只允许手机继续等待或联系支持；任何乐观字段都要被覆盖为 fail-closed。
        safe_remote_readiness.update(
            {
                "capability": PHONE_ACK_LOOKUP_PENDING_STATUS_CAPABILITY,
                "degradation_state": "ack_lookup_pending",
    ...
    if remote_state == PHONE_ACK_ACCEPTED_RESULT_PENDING_DEGRADATION_STATE:
        # accepted/processing ACK 不能当作 delivery/dropoff/cancel 结果，手机端只能等待或联系支持。
        safe_remote_readiness.update(
            {
                "capability": PHONE_ACK_ACCEPTED_RESULT_PENDING_CAPABILITY,
                "degradation_state": PHONE_ACK_ACCEPTED_RESULT_PENDING_DEGRADATION_STATE,
            ...
            if remote_route:
                ...
                try:
                    ...
                    if route_name == "ack":
                        ...
                        payload = _with_remote_auth_state(payload, self._remote_auth_state())
