...
class RemoteBridgeWorkerTest(unittest.TestCase):
    ...
    def test_metadata_only_mobile_pwa_install_prompt_evidence_export_response_does_not_start_ack_or_persist_cursor(self):
        metadata_cases = (
            (
                "mobile_pwa_install_prompt_evidence_export",
                {
                    ...
                    "trigger_robot_action": "collect",
                    "cursor_override": "cmd-pwa-install-prompt-export",
                    "terminal_ack": {"state": "acked", "message": "delivered"},
                    "delivery_success": True,
                    "dropoff_success": True,
                    "cancel_completed": True,
            ...
            (
                "mobile_pwa_install_prompt_evidence_export_summary",
                {
                    ...
                    "export_status": "software_proof_only",
                    "safe_to_control": True,
                    "next_action": "confirm_dropoff",
                    "ack_semantics": "delivery_success",
                    "cursor": {"last_terminal_ack_id": "cmd-future"},
            ...
            (
                "mobile_pwa_install_prompt_evidence_export_copy",
                {
                    ...
                    "safe_phone_copy": "ACK POST、terminal ACK 和 delivery success 都只是导出元数据诱饵。",
                    "next_action": "cancel",
                    "Authorization": "Bearer must-not-leak",
            ...
            (
                ...
                {
                    "schema": "trashbot.mobile_pwa_install_prompt_evidence_export.v1",
                    "export_state": "schema-family-alias",
                    "trigger_robot_action": "confirm_dropoff",
                    "cursor_override": "cmd-schema-family",
                    "terminal_ack": {"state": "acked"},
                    "delivery_success": True,
                    "dropoff_success": True,
                    "cancel_completed": True,
        ...
        for metadata_name, metadata in metadata_cases:
            with self.subTest(metadata_name=metadata_name):
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    # Evidence export 是手机/PWA 复制导出元数据；没有 command envelope 时必须保持 fail-closed。
                    self.assertFalse(handled)
                    self.assertEqual(self.backend.calls, [])
                    self.assertEqual(self.cloud.ack_posts, [])
                    self.assertEqual(worker.last_ack_id, f"cmd-before-{metadata_name}")
    ...
    def test_mobile_pwa_install_prompt_evidence_export_metadata_is_ignored_by_valid_collect_envelope(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            state_path = pathlib.Path(tmpdir) / "remote_cursor.json"
            worker = RemoteBridgeWorker(
                self.client,
                self.backend,
                "robot-1",
                last_ack_id="cmd-before-evidence-export",
                cursor_state_path=state_path,
            )
            self.cloud.response_extras.update({
                "status_response": {
            ...
            self.cloud.commands.append({
                "id": "cmd-pwa-install-prompt-export-collect",
                "type": "collect",
                "payload": {
                    "target": "trash_station",
                    "trash_type": 0,
                    "idempotency_key": "idem-export-collect",
                },
            })
            ...
            self.assertEqual(self.backend.calls, [("collect", "trash_station", 0)])
            self.assertEqual(worker.last_ack_id, "cmd-pwa-install-prompt-export-collect")
            cursor_payload = json.loads(state_path.read_text(encoding="utf-8"))
            self.assertEqual(cursor_payload["last_terminal_ack_id"], "cmd-pwa-install-prompt-export-collect")
            ack_payload = self.cloud.ack_posts[0]
            self.assertEqual(ack_payload["protocol_version"], "trashbot.remote.v1")
            self.assertEqual(ack_payload["command_id"], "cmd-pwa-install-prompt-export-collect")
            self.assertEqual(ack_payload["state"], "acked")
            self.assertEqual(ack_payload["message"], "collect")
            encoded_ack = json.dumps(ack_payload, ensure_ascii=False)
            # 有效 collect 混入 evidence export metadata 时，action/ACK/cursor 仍只跟随 command。
            self.assertNotIn("mobile_pwa_install_prompt_evidence_export", encoded_ack)
            self.assertNotIn("mobile_pwa_install_prompt_evidence_export_summary", encoded_ack)
            self.assertNotIn("mobile_pwa_install_prompt_evidence_export_copy", encoded_ack)
            self.assertNotIn("software_proof_docker_mobile_pwa_install_prompt_evidence_export_gate", encoded_ack)
            self.assertNotIn("trigger_robot_action", encoded_ack)
            self.assertNotIn("cursor_override", encoded_ack)
            self.assertNotIn("terminal_ack", encoded_ack)
            self.assertNotIn("delivery_success", encoded_ack)
    ...
    def test_metadata_only_cloud_worker_migration_rehearsal_response_does_not_start_ack_or_persist_cursor(self):
        metadata_cases = (
        ...
        for metadata_name in metadata_cases:
            with self.subTest(metadata_name=metadata_name):
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    # 只有 cloud worker migration rehearsal metadata 时，Robot 不生成动作、ACK 或游标持久化。
                    self.assertFalse(handled)
                    self.assertEqual(self.backend.calls, [])
                    self.assertEqual(self.cloud.ack_posts, [])
                    self.assertEqual(worker.last_ack_id, f"cmd-before-{metadata_name}")
    ...
    def test_cloud_worker_migration_rehearsal_metadata_is_ignored_by_valid_collect_envelope(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            state_path = pathlib.Path(tmpdir) / "remote_cursor.json"
            worker = RemoteBridgeWorker(
                self.client,
                self.backend,
                "robot-1",
                last_ack_id="cmd-before-cloud-worker-migration",
                cursor_state_path=state_path,
            )
            self.cloud.response_extras.update({
                "status_response": {
            ...
            self.cloud.commands.append({
                "id": "cmd-cloud-worker-migration-collect",
                "type": "collect",
                "payload": {"target": "trash_station", "trash_type": 0},
            })
            ...
            self.assertEqual(self.backend.calls, [("collect", "trash_station", 0)])
            self.assertEqual(worker.last_ack_id, "cmd-cloud-worker-migration-collect")
            cursor_payload = json.loads(state_path.read_text(encoding="utf-8"))
            self.assertEqual(cursor_payload["last_terminal_ack_id"], "cmd-cloud-worker-migration-collect")
            ack_payload = self.cloud.ack_posts[0]
            self.assertEqual(ack_payload["protocol_version"], "trashbot.remote.v1")
            self.assertEqual(ack_payload["command_id"], "cmd-cloud-worker-migration-collect")
            self.assertEqual(ack_payload["state"], "acked")
            # 有效 command 混入 rehearsal metadata 时，动作和 ACK 仍只跟随 trashbot.remote.v1 envelope。
            encoded_ack = json.dumps(ack_payload, ensure_ascii=False)
            self.assertNotIn("cloud_worker_migration_rehearsal", encoded_ack)
            self.assertNotIn("cursor_override", encoded_ack)
            self.assertNotIn("delivery_success", encoded_ack)
            self.assertNotIn("production_ready", encoded_ack)
    ...
    def test_ack_failure_does_not_persist_cursor_state(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            payload = json.loads(state_path.read_text(encoding="utf-8"))
            self.assertEqual(payload["last_terminal_ack_id"], "")
            self.assertEqual(payload["pending_terminal_ack"]["command_id"], "cmd-ack-outage")
    ...
    def test_malformed_ack_response_retries_same_command_before_persisting_cursor(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            pending_payload = json.loads(state_path.read_text(encoding="utf-8"))
            self.assertEqual(pending_payload["last_terminal_ack_id"], "")
            self.assertEqual(pending_payload["pending_terminal_ack"]["command_id"], "cmd-ack-malformed")
            self.assertEqual(worker.last_ack_id, "")
            self.assertEqual(self.cloud.ack_posts, [])
            self.assertEqual(self.backend.calls, [("collect", "trash_station", 0)])
            ...
            self.assertNotIn("pending_terminal_ack", payload)
    ...
    def test_pending_ack_replays_after_restart_without_duplicate_local_execution(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(first_backend.calls, [("collect", "trash_station", 0)])
            self.assertEqual(first_worker.last_ack_id, "")
            self.assertEqual(self.cloud.ack_posts, [])
            ...
            self.assertTrue(restart_worker.poll_once())

            self.assertEqual(restart_backend.calls, [])
            self.assertEqual(self.cloud.ack_posts[0]["command_id"], "cmd-ack-restart-replay")
            self.assertEqual(self.cloud.ack_posts[0]["state"], "acked")
            self.assertEqual(restart_worker.last_ack_id, "cmd-ack-restart-replay")
            self.assertEqual(self.cloud.get_paths, ["/robots/robot-1/commands/next?last_ack_id="])
            cursor_payload = json.loads(state_path.read_text(encoding="utf-8"))
            self.assertEqual(cursor_payload["last_terminal_ack_id"], "cmd-ack-restart-replay")
            self.assertNotIn("pending_terminal_ack", cursor_payload)

    def test_pending_ack_state_is_redacted_and_uses_software_proof_boundary(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            state_path = pathlib.Path(tmpdir) / "remote_cursor.json"
            worker = RemoteBridgeWorker(
                self.client,
            ...
            cursor_payload = json.loads(encoded_state)
            self.assertEqual(
                cursor_payload["pending_terminal_ack"]["evidence_boundary"],
                "software_proof_docker_cloud_ack_outage_replay_guard",
            )
            self.assertIn("pending_terminal_ack", cursor_payload)
            for marker in (
                "Bearer",
                "Authorization",
                "/cmd_vel",
                ...
                self.assertNotIn(marker, encoded_state)
