...
class MobileWebRouteTaskReviewTest(unittest.TestCase):
    ...
    def test_route_task_field_run_reconciliation_panel_is_read_only(self):
        index = self.read_web("index.html")
        app = self.read_web("app.js")
        fixture = json.loads(FIXTURE.read_text(encoding="utf-8"))
        doc = DOC.read_text(encoding="utf-8")
        summary = fixture["route_task_field_run_reconciliation"]

        # reconciliation 只把现场材料复账结果转成 phone-safe 摘要；不能参与主控制授权链。
        self.assertIn("routeTaskFieldRunReconciliationTitle", index)
        self.assertIn("路线任务现场复账", index)
        self.assertIn("routeTaskFieldRunReconciliationVerdict", index)
        self.assertIn("routeTaskFieldRunReconciliationEvidenceRef", index)
        self.assertIn("routeTaskFieldRunReconciliationSameRef", index)
        self.assertIn("routeTaskFieldRunReconciliationMaterials", index)
        self.assertIn("routeTaskFieldRunReconciliationNextSteps", index)
        self.assertIn("routeTaskFieldRunReconciliationControls", index)
        self.assertIn("routeTaskFieldRunReconciliationNotProven", index)
        self.assertIn("software_proof_docker_route_task_field_run_reconciliation_gate", index)
        self.assertIn("routeTaskFieldRunReconciliationFromStatus", app)
        self.assertIn("routeTaskFieldRunReconciliationCandidate", app)
        self.assertIn("route_task_field_run_reconciliation", app)
        self.assertIn("route_task_field_run_reconciliation_summary", app)
        self.assertIn("diagnosticsReadiness.route_task_field_run_reconciliation", app)
        self.assertIn("diagnosticsSummary.route_task_field_run_reconciliation_summary", app)
        self.assertIn("nestedDiagnosticsSummary.route_task_field_run_reconciliation_summary", app)
        self.assertNotRegex(app, r"renderRouteTaskFieldRunReconciliation[\s\S]*fetchJson\(ENDPOINTS\.(start|confirm_dropoff|cancel)")
        self.assertIn("Start Delivery", index)
        self.assertIn("Confirm Dropoff", index)
        self.assertIn("Cancel", index)
        self.assertIn("actionGate.enabled === true && permitted === true", app)

        self.assertEqual(summary["schema"], "trashbot.route_task_field_run_reconciliation.v1")
        self.assertEqual(summary["evidence_boundary"], "software_proof_docker_route_task_field_run_reconciliation_gate")
        self.assertTrue(summary["same_evidence_ref_required"])
        self.assertFalse(summary["delivery_success"])
        self.assertFalse(summary["primary_actions_enabled"])
        self.assertIn("reconciliation_verdict", summary)
        self.assertIn("safe_evidence_ref", summary)
        self.assertIn("materials_status", summary)
        self.assertIn("operator_next_steps", summary)
        self.assertIn("not_proven", summary)
        self.assertIn("delivery success", json.dumps(summary, ensure_ascii=False))
        self.assertIn("route_task_field_run_reconciliation_summary", fixture["phone_readiness"])
        self.assertIn("route_task_field_run_reconciliation", doc)
        self.assertIn("software_proof_docker_route_task_field_run_reconciliation_gate", doc)
        self.assertIn("delivery_success=false", doc)
        self.assertIn("primary_actions_enabled=false", doc)
        self.assertIn("not HIL", doc)
        self.assertIn("not delivery success", doc)

    def test_route_task_field_run_reconciliation_missing_and_unsafe_fields_fail_closed(self):
        index = self.read_web("index.html")
        app = self.read_web("app.js")
        fixture_text = FIXTURE.read_text(encoding="utf-8")

        # 缺 reconciliation summary 时默认 blocked；敏感字段必须走 safeFieldRunReconciliationText 降级。
        self.assertIn("blocked_missing_route_task_field_run_reconciliation_summary", app)
        self.assertIn("未提供 materials status 摘要", app)
        self.assertIn("UNSAFE_FIELD_RUN_RECONCILIATION_TEXT", app)
        self.assertIn("safeFieldRunReconciliationText", app)
        self.assertIn("raw robot response", app)
        self.assertIn("complete artifact", app)
        self.assertIn("credential-bearing url", app)
        self.assertIn("/cmd_vel", app)
        ...
        self.assertIn("traceback", app)
        self.assertIn("checksum", app)
        self.assertNotIn("routeTaskFieldRunReconciliationRawArtifact", index)
        self.assertNotIn("routeTaskFieldRunReconciliationLocalPath", index)
        self.assertNotIn("routeTaskFieldRunReconciliationTraceback", index)
        self.assertNotRegex(fixture_text, r"/Users/|/tmp/|/ws/|checksum|traceback|Authorization|Bearer|OSS_ACCESS_KEY|raw robot response|full execution bundle")
