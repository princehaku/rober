...
class MobileWebRouteTaskReviewTest(unittest.TestCase):
    ...
    def test_route_task_field_run_execution_pack_panel_is_read_only(self):
        index = self.read_web("index.html")
        app = self.read_web("app.js")
        fixture = json.loads(FIXTURE.read_text(encoding="utf-8"))
        doc = DOC.read_text(encoding="utf-8")
        summary = fixture["route_task_field_run_execution_pack"]

        # execution pack 只把现场执行材料转成 phone-safe 摘要；不能参与主控制授权链。
        self.assertIn("routeTaskFieldRunExecutionPackTitle", index)
        self.assertIn("路线任务现场执行包", index)
        self.assertIn("routeTaskFieldRunExecutionPackStatus", index)
        self.assertIn("routeTaskFieldRunExecutionPackEvidenceRef", index)
        self.assertIn("routeTaskFieldRunExecutionPackSameRef", index)
        self.assertIn("routeTaskFieldRunExecutionPackMaterials", index)
        self.assertIn("routeTaskFieldRunExecutionPackFirstRunCommands", index)
        self.assertIn("routeTaskFieldRunExecutionPackRerunCommands", index)
        self.assertIn("routeTaskFieldRunExecutionPackNextSteps", index)
        self.assertIn("routeTaskFieldRunExecutionPackNotProven", index)
        self.assertIn("software_proof_docker_route_task_field_run_execution_pack_gate", index)
        self.assertIn("routeTaskFieldRunExecutionPackFromStatus", app)
        self.assertIn("routeTaskFieldRunExecutionPackCandidate", app)
        self.assertIn("route_task_field_run_execution_pack", app)
        self.assertIn("route_task_field_run_execution_pack_summary", app)
        self.assertIn("diagnosticsReadiness.route_task_field_run_execution_pack", app)
        self.assertIn("diagnosticsSummary.route_task_field_run_execution_pack_summary", app)
        self.assertIn("nestedDiagnosticsSummary.route_task_field_run_execution_pack_summary", app)
        self.assertNotRegex(app, r"renderRouteTaskFieldRunExecutionPack[\s\S]*fetchJson\(ENDPOINTS\.(start|confirm_dropoff|cancel)")
        self.assertIn("Start Delivery", index)
        self.assertIn("Confirm Dropoff", index)
        self.assertIn("Cancel", index)
        self.assertIn("actionGate.enabled === true && permitted === true", app)

        self.assertEqual(summary["schema"], "trashbot.route_task_field_run_execution_pack.v1")
        self.assertEqual(summary["evidence_boundary"], "software_proof_docker_route_task_field_run_execution_pack_gate")
        self.assertTrue(summary["same_evidence_ref_required"])
        self.assertIn("field_run_manifest", summary)
        self.assertIn("required_material_templates", summary)
        self.assertIn("first_run_commands", summary)
        self.assertIn("rerun_commands", summary)
        self.assertIn("operator_next_steps", summary)
        self.assertIn("not_proven", summary)
        self.assertIn("delivery success", json.dumps(summary, ensure_ascii=False))
        self.assertIn("route_task_field_run_execution_pack_summary", fixture["phone_readiness"])
        self.assertIn("route_task_field_run_execution_pack", doc)
        self.assertIn("software_proof_docker_route_task_field_run_execution_pack_gate", doc)
        self.assertIn("not HIL", doc)
        self.assertIn("not delivery success", doc)

    def test_route_task_field_run_execution_pack_missing_and_unsafe_fields_fail_closed(self):
        index = self.read_web("index.html")
        app = self.read_web("app.js")
        fixture_text = FIXTURE.read_text(encoding="utf-8")

        # 缺 execution pack summary 时默认 blocked；敏感字段必须走 safeFieldRunExecutionPackText 降级。
        self.assertIn("blocked_missing_route_task_field_run_execution_pack_summary", app)
        self.assertIn("未提供 required materials 摘要", app)
        self.assertIn("UNSAFE_FIELD_RUN_EXECUTION_PACK_TEXT", app)
        self.assertIn("safeFieldRunExecutionPackText", app)
        self.assertIn("raw robot response", app)
        self.assertIn("complete artifact", app)
        self.assertIn("credential-bearing url", app)
        self.assertIn("/cmd_vel", app)
        ...
        self.assertIn("traceback", app)
        self.assertIn("checksum", app)
        self.assertNotIn("routeTaskFieldRunExecutionPackRawArtifact", index)
        self.assertNotIn("routeTaskFieldRunExecutionPackLocalPath", index)
        self.assertNotIn("routeTaskFieldRunExecutionPackTraceback", index)
        self.assertNotRegex(fixture_text, r"/Users/|/tmp/|/ws/|checksum|traceback|Authorization|Bearer|OSS_ACCESS_KEY|raw robot response|full execution bundle")
