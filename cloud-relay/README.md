...
>
> **O7 RTC signaling/media contract**：`GET /api/o7/rtc-signaling/contract` 返回 `trashbot.o7.rtc_signaling_contract.v1` 静态 fail-closed 协议入口清单。它不要求 bearer、不读取 env token、不执行 network probe、不创建 WebRTC session、不读取硬件、不发送控制，只列出后续机器人/云 relay 必须真实打通的 signaling endpoint、session/idempotency、offer/answer、ICE candidates、video/audio media tracks、pose/elevator realtime events、credential handling、observability/evidence refs、failure/timeout semantics 和 forbidden actions。响应固定 `source=software_proof`、`proof_status=not_proven`、`network_probe_executed=false`、`webrtc_session_created=false`、`media_transport_connected=false`、`video_track_received=false`、`realtime_pose_stream_connected=false`、`real_ros2_tf_connected=false`、`safe_to_control=false`、`sends_commands=false`、`reads_hardware=false`、`robot_control_executed=false`、`delivery_success=false`。该入口只是 PC 后续 probe 与板端 RTC 对接前置，不证明 RTC/视频/ROS2 `/tf` 已通。
>
